...
function InvalidErrorEntry() {
    ...
    console.log(errortypeinvalid, "errortypeinvalid")
    const getCodeView = async (id, type, olddate, data) => {
    ...
    const editSubmitErrorLoad = async (e) => {
        ...
        if (penaltyErrorUpload.projectvendorfirst == "Please Select Project Vendor" && penaltyErrorUpload.projectvendordouble == "Please Select Project Vendor" && penaltyErrorUpload.projectvendorthird == "Please Select Project Vendor") {
            ...
            setPopupContentMalert("Cannot Move This Projectvendor,Process Need to Update Move Status in Error Type");
            setPopupSeverityMalert("warning");
            handleClickOpenPopupMalert();
        }
        else {
    ...
    const addSerialNumber = (datas) => {
        const itemsWithSerialNumber = datas?.map((item, index) => {
            ...
            return {
                ...
                date: item.mode == "Bulkuplod" ? moment(item.dateformatted).format("DD/MM/YYYY") : moment(item.date).format("DD/MM/YYYY"),
                olddate: item.date,
                mode: item.mode
            }
        });
            ...
            cellRenderer: (params) => (
                <Grid sx={{ display: "flex" }}>
                    ...
                    <Button
                        ...
                        size="small"
                        onClick={() => {
                            getCodeView(params.data.id, params.data.mode, params.data.olddate, params.data);
                        }}
                    >
                        Action
                    </Button>&ensp;
