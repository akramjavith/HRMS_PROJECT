...
import { Box, Button, Checkbox, Dialog, DialogActions, DialogContent, FormControl, Grid, IconButton, List, ListItem, ListItemText, MenuItem, OutlinedInput, Popover, Select, Table, TableBody, TableHead, TextField, Typography, Chip } from "@mui/material";
...
function Assignexperience() {
  let exportColumnNames = ["Company", "Branch", "Unit", "Team", "Work Mode", "Emp Code", "Name", "DOJ", "Actual Exp", "Mode", "Value", "Mode Exp", "End Exp", "End-Exp Date", "Prod Exp", "End Tar", "End-Tar Date", "Target Exp", "Process Code", "Exp. Process Code+Month", "Tar. Process Code+Month"];
  let exportRowValues = ["company", "branch", "unit", "team", "workmode", "empcode", "companyname", "doj", "experience", "assignExpMode", "modevalue", "modeexp", "endexp", "endexpdate", "prodexp", "endtar", "endtardate", "targetexp", "processcode", "processcodeexp", "processcodetar"];
  ...
  const getCode = async (e) => {
    ...
    try {
      ...
      if (res?.data?.suser.assignExpLog?.length > 0) {
        ...
        setOldDate(lastexplog.updatedate);
        setAssignExperience({
  ...
  // SAME DATE Error Popup model
  const [isSameDateOpen, setIsSameDateOpen] = useState(false);

  const handleOpenSameDate = () => {
    setIsSameDateOpen(true);
  };
  const handleCloseSameDate = () => {
    setIsSameDateOpen(false);
  };
  ...
  const editSubmit = (e) => {
    ...
    if (assignExperience.assignExpMode === "Please Select Mode") {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (oldDate === assignExperience.updatedate) {
      handleOpenSameDate();
    } else {
      sendRequestt();
    }
      ...
      const itemsWithSerialNumber = employees.map((item, index) => {
        if (Array.isArray(item.assignExpLog) && item.assignExpLog.length > 0) {
          ...
          return {
            ...
            processcodeexp: item.doj && modevalue?.expmode === "Manual" ? `${modevalue.salarycode}${differenceInMonthsexp <= 9 ? `0${differenceInMonthsexp}` : differenceInMonthsexp}` : `${getProcessCode}${differenceInMonthsexp <= 9 ? `0${differenceInMonthsexp}` : differenceInMonthsexp}`,
            processcodetar: item.doj && modevalue?.expmode === "Manual" ? `${modevalue.salarycode}${differenceInMonthstar <= 9 ? `0${differenceInMonthstar}` : differenceInMonthstar}` : `${getProcessCode}${differenceInMonthstar <= 9 ? `0${differenceInMonthstar}` : differenceInMonthstar}`,
          };
        } else {
  ...
  return (
    <Box>
      ...
      <Dialog open={isSameDateOpen} onClose={handleCloseSameDate} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        <DialogContent sx={{ width: "350px", textAlign: "center", alignItems: "center" }}>
          <Typography variant="h6">{"Date is Not changed, Is it ok to continue with this date?"}</Typography>
        </DialogContent>

        <DialogActions>
          <Button variant="contained" color="primary" onClick={sendRequestt}>
            ok
          </Button>
          <Button sx={buttonStyles.btncancel} onClick={handleCloseSameDate}>
            Cancel
          </Button>
        </DialogActions>
      </Dialog>
