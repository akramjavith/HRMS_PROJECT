...
function TrainingPostponedList({ com }) {
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("User_Postponed_Training_Panel.pdf");
    };
            ...
            renderCell: (params) => (
                <Grid sx={{ display: "flex" }}>
                    ...
                    {isUserRoleCompare?.includes("vtrainingpostponeduserpanel") && (
                        <Link to={`/task/training/master/traininguserpanelview/${params.row.id}`} style={{ textDecoration: "none", color: "#fff", minWidth: "0px" }}>
    ...
    const columns = [
        ...
        { title: "Online Test Date", field: "onlinetestdate" },
        { title: "Online Test Time", field: "onlinetesttime" },
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "User Postponed Training Panel.png");
                });
            });
        }
    };
    ...
    const ListPageLoadDataOnProgress = async () => {
        try {
            ...
            let ans = res_task?.data?.trainingforuser?.filter(data => new Date() > new Date(data.onlinetestdate))
            if (ans?.length > 0) {
                let answer = ans?.map(item => {
                    axios.put(
                        `${SERVICE.SINGLE_TRAINING_FOR_USER}/${item?._id}`,
                        {
                            headers: {
                                Authorization: `Bearer ${auth.APIToken}`,
                            },
    ...
    const ListPageLoadDataPostPoned = async () => {
        try {
            ...
            setRaiseTicketList(res_task?.data?.trainingforuser)
            setQueueCheck(true);
        } catch (err) {setQueueCheck(true);handleApiError(err, setShowAlert, handleClickOpenerr);}
    }
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>User Postponed Training Panel</Typography>
                ...
                <>
                    <Box sx={userStyle.dialogbox}>
                        {isUserRoleCompare?.includes("ltrainingpostponeduserpanel") && (
                            <>
                                <Typography sx={userStyle.SubHeaderText}>Manage User Postponed Training Panel</Typography>
                                    ...
                                    <Table aria-label="customized table" id="raisetickets" ref={componentRef}>
                                        <TableHead sx={{ fontWeight: "600" }}>
                                            <StyledTableRow>
                                                ...
                                                <StyledTableCell>Online Test Date</StyledTableCell>
                                                <StyledTableCell>Online Test Time</StyledTableCell>
                                            ...
                                            {filteredData?.length > 0 ? (
                                                filteredData?.map((row, index) => (
                                                    <StyledTableRow key={index}>
                                                        ...
                                                        <StyledTableCell>{row.onlinetestdate}</StyledTableCell>
                                                        <StyledTableCell>{row.onlinetesttime}</StyledTableCell>
