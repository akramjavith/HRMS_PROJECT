...
function TrainingForUserCompleted({ com }) {
    ...
    // Excel
    const fileName = "User_Training _Panel-Completed";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "User_Training _Panel-Completed",
        pageStyle: "print",
    });
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "Training User Panel - Completed List.png");
                })
                .catch((error) => {
    ...
    const ListPageLoadDataCompleted = async () => {
        ...
        try {
            ...
            const final = res_task?.data?.trainingforuser?.length > 0 ?
                res_task?.data?.trainingforuser?.filter(data => data.username === isUserRoleAccess?.companyname && ["Completed", "Finished By Others", "Not Applicable to Me"]?.includes(data?.taskstatus)) : [];
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>User Training  Panel - Completed List</Typography>
            ...
            <ExportData
                ...
                filename={"Training User Panel - Completed List"}
...
export default TrainingForUserCompleted;
