...
function Individualaiseticketist() {
  ...
  const handleExportXL = async (isfilter) => {
    if (isfilter === "filtered") {
      ...
      let res_task = await axios.post(SERVICE.RAISETICKET_INDIVIDUALFILTER, {
        headers: {
...
let overall;
    ...
    doc.save("IndividualRaiseTicketList.pdf");
  };
...
const sendRequestToChangeStatus = async () => {
          ...
          {isUserRoleCompare?.includes("iindividualticketlist") && (
            ...
            >
              <InfoOutlinedIcon style={{ fontSize: "large" }} />
                ...
                {(isUserRoleCompare?.includes("vraiseticket") &&   params.row.status === "Open") && (
                        <Button
                            sx={userStyle.buttondelete}
                            onClick={(e) => {
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "IndividualRaiseTicket_List.png");
        });
      });
    }
  };
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <Typography sx={userStyle.HeaderText}>Individual Raise Ticket List</Typography>
