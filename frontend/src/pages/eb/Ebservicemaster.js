...
function EbServiceMaster() {
  ...
  const [isChageStatus, setIsChangeStatus] = useState(false);
  ...
  const [ebservicemaster, setEbservicemaster] = useState({
    ...
    servicenumber: "",
    servicedate: formattedDate, startdate: formattedDate,
    openkwh: "",
    kvah: "",
    sectionid: "",
    sectionname: "",
  ...
  const [getserviceOldData, setGetServiceOldata] = useState({
    company: "Please Select Company",
    branch: "Please Select Branch",
    unit: "Please Select Unit",
    floor: "Please Select Floor",
  ...
  const initialColumnVisibility = {
    ...
    servicenumber: true,
    vendor: true,
    servicedate: true, startdate: true,
  ...
  const delEbservice = async () => {
    ...
    try {
      if (Ebservicessid) {
        ...
        await fetchEbservicemaster("Filtered");
        handleCloseMod();
        setSelectedRows([]);
        setPage(1);
      }
  ...
  const sendRequest = async () => {
    ...
    try {
      let subprojectscreate = await axios.post(SERVICE.EBSERVICEMASTER_CREATE, {
        ...
        startdate: String(ebservicemaster.startdate),
  ...
  const handleSubmit = (e) => {
    ...
    const isNameMatch = ebservicemasters.some(
      (item) =>
        ...
        item.servicedateold == String(ebservicemaster.servicedate) &&
        item.area.some((data) => areas.includes(data))
    );

    const isServiceMatch = ebservicemasters.some(
      (item) =>
        item.servicenumber == String(ebservicemaster.servicenumber)
    );
    ...
    if (ebservicemaster.company === "Please Select Company") {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (ebservicemaster.startdate === "") {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (isServiceMatch) {
      setPopupContentMalert("This Service Number Already Exists!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else {
      sendRequest();
  ...
  const getCode = async (e, servicenumber, status) => {
    try {
      ...
      setIsChangeStatus(status === "change" ? true : false);
      setEbservicemasterEdit(res?.data?.sebservicemaster);
      setGetServiceOldata(res?.data?.sebservicemaster)
  ...
  console.log(getserviceOldData, "getserviceOldData")
  //editing the single data...
  const sendEditRequest = async () => {
    setPageName(!pageName)
    let employ = selectedOptionsCateEdit.map((item) => item.value);
    const startDate = new Date(ebservicemasterEdit.startdate);
    startDate.setDate(startDate.getDate() - 1);
    const reducedDate = startDate.toISOString().split('T')[0]; // Format back to "YYYY-MM-DD"
    ...
    try {
      let res = await axios.put(
        `${SERVICE.EBSERVICEMASTER_SINGLE}/${subprojectsid}`,
        {
          ...
          startdate: String(ebservicemasterEdit.startdate),
          ...
          servicelog: isChageStatus ? [...ebservicemasterEdit.servicelog, {
            ...getserviceOldData,
            changedusername: String(isUserRoleAccess.companyname),
            enddate: String(reducedDate),
          }] : [...ebservicemasterEdit.servicelog],
          updatedby: [
            ...updateby,
            {
              name: String(isUserRoleAccess.companyname),
      ...
      if (isChageStatus) {
        selectedOptionsCateEdit.map((item => {
          axios.post(SERVICE.EBREADINGDETAIL_CREATE, {
            headers: {
              'Authorization': `Bearer ${auth.APIToken}`
            },
            ...
            company: String(getserviceOldData.company),
            branch: String(getserviceOldData.branch),
            unit: String(getserviceOldData.unit),
            floor: String(getserviceOldData.floor),
            area: String(item.value),
            servicenumber: String(getserviceOldData.servicenumber),
            readingmode: String("Open Entry"),
            time: currtime,
            date: String(getserviceOldData.startdate),
            enddate: String(reducedDate),
            createdAt: new Date(),
            openkwh: String(getserviceOldData.openkwh),
            kvah: String(getserviceOldData.kvah),
            addedby: [
              {
                name: String(isUserRoleAccess.companyname),
                date: String(new Date()),
  ...
  const editSubmit = async (e) => {
    ...
    const isserviceMatch = resdata.some((item) => item.servicenumber == String(ebservicemasterEdit.servicenumber));
    if (ebservicemasterEdit.company === "Please Select Company") {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (ebservicemasterEdit.startdate === "") {
      setPopupContentMalert("Please Select Start Date!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (isChageStatus && new Date(ebservicemasterEdit.startdate) <= new Date(getserviceOldData.startdate)) {
      setPopupContentMalert("This Start Date Already Exists!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    ...
    } else if (isChageStatus === false && isNameMatch) {
      setPopupContentMalert("Data Already Exists!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (isChageStatus === false && isserviceMatch) {
      setPopupContentMalert("This Service Number Already Exists!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else {
  ...
  const fetchEbservicemaster = async (e) => {
    setPageName(!pageName)
    ...
    try {
      if (e === "Filtered") {
        let res_vendor = await axios.post(SERVICE.EBSERVICE_FILTER, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        ...
        setEbservicemasters(res_vendor?.data?.ebservicemasters?.map((item, index) => ({
          ...item,
          id: item._id,
          serialNumber: index + 1,
          servicedate: moment(item?.servicedate).format("DD/MM/YYYY"),
          startdate: moment(item?.startdate).format("DD/MM/YYYY"),
          servicedateold: item?.servicedate,
          ...
          servicestatus: item?.servicelog?.length > 0 ? true : false
  ...
  useEffect(() => {
    addSerialNumber(ebservicemasters);
  }, [ebservicemasters]);
        ...
        return (
          <Grid sx={{ display: "flex" }}>
            <Button
              ...
              style={{
                margin: "5px",
                backgroundColor: params.value === "LIVE"
                  ? "green"
                  : params.value === "CLOSED"
                    ? "red"
                    : "blue",
        ...
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("eebservicemaster") && (
            <Button
              ...
              onClick={() => {
                getCode(params.data.id, params.data.servicenumber, 'change');
              }}
            >
              CHANGE
            </Button>
          ...
          &ensp;
          {(isUserRoleCompare?.includes("eebservicemaster") && params?.data?.servicestatus) && (
            <Button
              variant="contained"
              color="primary"
              onClick={() => {
                window.open(
                  `/eb/ebervicemasterchange/${params.data.id}`,
                  "_blank"
                );
              }}
            >
          ...
          {isUserRoleCompare?.includes("eebservicemaster") && (
            <Button
              ...
              onClick={() => {
                // handleClickOpenEdit();
                getCode(params.data.id, params.data.servicenumber, 'edit');
              }}
            >
          ...
          {isUserRoleCompare?.includes("debservicemaster") && (
            <Button
              ...
              onClick={(e) => {
                rowData(params.data.id, params.data.servicenumber);
              }}
            >
  ...
  return (
    <Box>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      id="component-outlined"
                      type="date"
                      value={ebservicemaster.startdate}
                      onChange={(e) => {
                        setEbservicemaster({
                          ...ebservicemaster,
      ...
      <Box>
        ...
        >
          <Box sx={{ height: "auto", padding: "20px" }}>
            <>
              <form onSubmit={editSubmit}>
                ...
                <Grid container spacing={2}>
                  <Grid item md={6} xs={12} sm={12}>
                    <Typography sx={userStyle.HeaderText}>
                      {isChageStatus ? "Change" : "Edit"} EB Services Master
                    </Typography>
                  </Grid>
                  {isChageStatus && (
                    <Grid item md={6} xs={12} sm={12}>
                      <Typography sx={userStyle.SubHeaderText}>
                        Service Number : {ebservicemasterEdit.servicenumber}
                      </Typography>
                    </Grid>
                  )}
                </Grid>
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput
                        id="component-outlined"
                        type="date"
                        value={ebservicemasterEdit.startdate}
                        onChange={(e) => {
                          setEbservicemasterEdit({
                            ...ebservicemasterEdit,
                            startdate: e.target.value,
              ...
              <Grid container spacing={2} style={userStyle.dataTablestyle}>
                ...
                <Grid item md={2} xs={6} sm={6}>
                  <AggregatedSearchBar
                    ...
                    setPage={setPage}
                    maindatas={ebservicemasters}
                    setSearchedString={setSearchedString}
                    searchQuery={searchQuery}
                    setSearchQuery={setSearchQuery}
                    paginated={false}
                    totalDatas={ebservicemasters}
                  />
                </Grid>
              </Grid>
              <br />
                  ...
                  <Box style={{ width: "100%", overflowY: "hidden" }}>
                    <>
                      <AggridTable
                        ...
                        selectedRowsCat={selectedRowsCat}
                        setSelectedRowsCat={setSelectedRowsCat}
                        gridRefTable={gridRefTable}
                        paginated={false}
                        filteredDatas={filteredDatas}
                        pagenamecheck={"EB Services Master List"}
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {moment(ebservicemasterEdit.startdate).format(
                      "DD/MM/YYYY"
                    )}
                  </Typography>
                </FormControl>
