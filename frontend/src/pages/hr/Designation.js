...
function Designation() {
  ...
  let exportColumnNames = ['Group', 'Name', 'Description', 'Notice Period'];
  let exportRowValues = ['group', 'name', 'description', 'noticeperiod'];
  ...
  const fetchDesignation = async () => {
    ...
    try {
      ...
      setDesignations(res_designation?.data?.designation?.map((item) => ({
        ...item,
        noticeperiod: ` ${item.noticeperiodto} - ${item.noticeperiodfrom} `
      })));
      setIsDesignation(true);
    } catch (err) { setIsDesignation(true); handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (
      designation.group === undefined ||
      designation.group === "" ||
      designation.group == "Please Select Group"
    ) {
      ...
      (designation.noticeperiodto === "" || designation.noticeperiodto == 0)
    ) {
  ...
  const fetchDesignationAll = async () => {
    ...
    try {
      ...
      setdesignationsalledit(
        res_designation?.data?.designation.filter(
          (item) => item._id != desinationid._id
        )
      );
    } catch (err) { handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  ...
  const editSubmit = (e) => {
    ...
    if (
      desinationid.group === "" ||
      desinationid.group == "Please Select Group"
    ) {
      setPopupContentMalert("Please select Group");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
      ...
      (desinationid.noticeperiodto === "" || desinationid.noticeperiodto == 0)
    ) {
      setPopupContentMalert("Please Enter Notice period No.Of Months");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {desinationid.noticeperiodto} -{" "}
                    {desinationid.noticeperiodfrom}
                  </Typography>
                </FormControl>
              </Grid>
            </Grid>
            ...
            {selectedRowsCount > 0 ?
              <>
                ...
                <Button sx={buttonStyles.buttonsubmit}
                  onClick={(e) => delDesignationcheckbox(e)}
                > OK </Button>
              </>
              :
