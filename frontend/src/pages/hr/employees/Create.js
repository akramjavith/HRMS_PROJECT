...
function MultistepForm() {
  ...
  const fetchOverAllSettings = async () => {
    try {
      ...
      let filter = lastObject?.todos.filter(
        (item) =>
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      let employees_data = await axios.post(SERVICE.USER_CREATE, {
        ...
        callingname: String(employee.firstname),
  ...
  const nextStepLog = () => {
    ...
    if (getDepartment !== "Internship" && !selectedTeam) {
      ...
      missingFieldstwo.push("Please Select Team");
    }
        ...
        <Grid container spacing={2}>
          ...
          <Grid item md={10} xs={12} sm={12}>
            ...
            <Box sx={userStyle.dialogbox}>
              <Grid container spacing={1}>
                <Grid item md={8} xs={0} sm={4}>
                  <Typography sx={userStyle.SubHeaderText} onClick={() => { console.log(companyEmailDomain) }}>
              ...
              <Grid container spacing={2}>
                <Grid item md={2.8} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      value={enableLoginName ? String(third) : employee.username}
                      readOnly
                    />
                  </FormControl>
                </Grid>
