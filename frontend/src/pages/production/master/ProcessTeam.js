...
function ProcessTeam() {
      ...
      valueProcessCat.forEach((processValue) => {
        brandCreatePromises.push(
          axios.post(SERVICE.CREATE_PROCESS_AND_TEAM, {
            ...
            company: String(documentFiles?.length === 0 ? processTeam.company : ''),
            branch: String(documentFiles?.length === 0 ? processTeam.branch : ''),
  ...
  const handleSubmit = (e) => {
    ...
    const isNameMatch = processTeamArray?.some((item) => item.company === processTeam.company && item.branch === processTeam.branch && item.unit === processTeam.unit && item.team === processTeam.team && processopt.includes(item.process));
    if (documentFiles.length === 0 && processTeam.company === 'Please Select Company') {
      setPopupContentMalert('Please Select Company');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (documentFiles.length === 0 && processTeam.branch === 'Please Select Branch') {
      setPopupContentMalert('Please Select Branch');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (documentFiles.length === 0 && processTeam.unit === 'Please Select Unit') {
      setPopupContentMalert('Please Select Unit');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (documentFiles.length === 0 && processTeam.team === 'Please Select Team') {
  ...
  const sendEditRequest = async () => {
    setPageName(!pageName);
    try {
      let res = await axios.put(`${SERVICE.SINGLE_PROCESS_AND_TEAM}/${processId}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        company: String(documentFilesEdit?.length === 0 ? processTeamEdit.company : ''),
        branch: String(documentFilesEdit?.length === 0 ? processTeamEdit.branch : ''),
        unit: String(documentFilesEdit?.length === 0 ? processTeamEdit.unit : ''),
        team: String(documentFilesEdit?.length === 0 ? processTeamEdit.team : ''),
        process: String(documentFilesEdit?.length === 0 ? processTeamEdit.process : ''),
  ...
  const editSubmit = (e) => {
    e.preventDefault();
    const isNameMatch = processTeamArrayEdit?.some((item) => item.company == processTeamEdit.company && item.branch == processTeamEdit.branch && item.unit == processTeamEdit.unit && item.team == processTeamEdit.team && item.process == processTeamEdit.process);
  ...
  const fetchProcessQueueArray = async () => {
    ...
    try {
      let res_freq = await axios.post(
        SERVICE.ALL_PROCESS_AND_TEAM_ASSIGNBRANCH,
        {
          assignbranch: accessbranch,
        },
        {
  ...
  const fetchProcessQueueAll = async () => {
    ...
    try {
      ...
      setProcessTeamArrayEdit(res_freq?.data?.processteam.filter((item) => item._id !== processTeamEdit._id));
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Process Team" modulename="Production" submodulename="Report SetUp" mainpagename="Process Team Mapping" subpagename="" subsubpagename="" />
      <>
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <Selects
                        maxMenuHeight={250}
                        options={accessbranch
                          ?.filter((comp) => processTeam.company === comp.company)
                          ?.map((data) => ({
                            label: data.branch,
                            value: data.branch,
                          }))
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <Selects
                        maxMenuHeight={250}
                        options={accessbranch
                          ?.filter((comp) => processTeam.company === comp.company && processTeam.branch === comp.branch)
                          ?.map((data) => ({
                            label: data.unit,
                            value: data.unit,
                          }))
        ...
        <>
          {isUserRoleCompare?.includes('lprocessteammapping') && (
            <Box sx={userStyle.container}>
              {/* ******************************************************EXPORT Buttons****************************************************** */}
              <Grid item xs={8}>
                <Typography sx={userStyle.importheadtext}>Process Team List</Typography>
              </Grid>
      ...
      <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="sm" fullWidth={true}>
        <Box sx={{ padding: '20px 50px' }}>
          <>
            <Typography sx={userStyle.HeaderText}> View Process Team</Typography>
            <br /> <br />
            <Grid container spacing={2}>
      ...
      <Box>
        ...
        >
          <Box sx={{ padding: '20px 50px' }}>
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>Edit Process Team</Typography>
              </Grid>
              <br />
              <Grid container spacing={2}>
                <Grid item md={6} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      placeholder="Please Select Company"
                      value={{
                        label: processTeamEdit.company === '' ? 'Please Select Company' : processTeamEdit.company,
                        value: processTeamEdit.company === '' ? 'Please Select Company' : processTeamEdit.company,
                      }}
                      onChange={(e) => {
                        setProcessTeamEdit({
                          ...processTeamEdit,
                ...
                <Grid item md={6} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      maxMenuHeight={250}
                      options={accessbranch
                        ?.filter((comp) => processTeamEdit.company === comp.company)
                        ?.map((data) => ({
                          label: data.branch,
                          value: data.branch,
                        }))
                      ...
                      placeholder="Please Select Branch"
                      value={{
                        label: processTeamEdit.branch === '' ? 'Please Select Branch' : processTeamEdit.branch,
                        value: processTeamEdit.branch === '' ? 'Please Select Branch' : processTeamEdit.branch,
                      }}
                      onChange={(e) => {
                        setProcessTeamEdit({
                          ...processTeamEdit,
                ...
                <Grid item md={6} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      maxMenuHeight={250}
                      options={accessbranch
                        ?.filter((comp) => processTeamEdit.company === comp.company && processTeamEdit.branch === comp.branch)
                        ?.map((data) => ({
                          label: data.unit,
                          value: data.unit,
                        }))
                      ...
                      placeholder="Please Select Unit"
                      value={{
                        label: processTeamEdit.unit === '' ? 'Please Select Unit' : processTeamEdit.unit,
                        value: processTeamEdit.unit === '' ? 'Please Select Unit' : processTeamEdit.unit,
                      }}
                      onChange={(e) => {
                        setProcessTeamEdit({
                          ...processTeamEdit,
                ...
                <Grid item md={6} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      placeholder="Please Select Team"
                      value={{
                        label: processTeamEdit.team === '' ? 'Please Select Team' : processTeamEdit.team,
                        value: processTeamEdit.team === '' ? 'Please Select Team' : processTeamEdit.team,
                      }}
                      onChange={(e) => {
                        setProcessTeamEdit({
                          ...processTeamEdit,
                ...
                <Grid item md={6} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      value={{
                        label: processTeamEdit.process === '' ? 'Please Select Process' : processTeamEdit.process,
                        value: processTeamEdit.process === '' ? 'Please Select Process' : processTeamEdit.process,
                      }}
                      onChange={(e) => {
                        setProcessTeamEdit({
                          ...processTeamEdit,
      ...
      />
      {/* INFO */}
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Process Team Info" addedby={addedby} updateby={updateby} />
