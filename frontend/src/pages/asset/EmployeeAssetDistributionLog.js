...
function EmployeeAssetDistributionLog() {
    ...
    const distributionid = useParams().id;
    ...
    const [uploadedImagesEdit, setUploadedImagesEdit] = useState([]);
    const [uploadedImagesDelete, setUploadImagesDelete] = useState([]);
    ...
    const handleFileDeleteEditUploaded = (index) => {
        setUploadedImagesEdit((prevFiles) => {
            const fileToDelete = prevFiles[index];

            // Store the deleted file separately
            setUploadImagesDelete((prevDeleted) => [...prevDeleted, fileToDelete]);
    ...
    const handleRemarkChangeUploadedEdit = (value, index) => {
        setUploadedImagesEdit((prev) =>
            prev.map((file, i) =>
                i === index ? { ...file, remarks: value } : file
            )
        );
    ...
    const handleCaptureImagenear = () => {
        if (gridRefNeartat.current) {
            html2canvas(gridRefNeartat.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "AssetEmployeeDistributionLog.png");
                });
            });
        }
    };
    ...
    const getCodeNear = async (e) => {
        try {
            ...
            await fetchUnAssignedIPAddress(res?.data?.semployeeasset?.assetmaterialcode, setAccordianCreateEdit);
            ...
            setUploadedImagesEdit(res?.data?.semployeeasset?.images);
            fetchAreaEdit(
    ...
    const getviewCodeNear = async (e) => {
        try {
            ...
            await fetchUnAssignedIPAddress(res?.data?.semployeeasset?.assetmaterialcode, setAccordianCreateView);
    ...
    const sendEditRequestNear = async () => {
        ...
        try {
            ...
            if (uploadEdit.length > 0) {
                uploadEdit.forEach((item, index) => {
                    formData.append("images", item.file); // Append image file
                    formData.append(`remarks[${index}]`, item.remarks || ""); // Append corresponding remark
                });
            }


            if (uploadedImagesDelete.length > 0) {
                formData.append("uploadedImagesDelete", JSON.stringify(uploadedImagesDelete));
            }

            const jsonData = {
                uploadedImagesEdit: uploadedImagesEdit.length > 0 ? uploadedImagesEdit : [],
            ...
            let res = await axios.put(
                `${SERVICE.EMPLOYEEASSET_SINGLE}/${maintenanceid}`, formData,
                {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
    ...
    const fetchMaintentanceIndividual = async () => {
        ...
        setLoading(true)
        try {
            let res = await axios.get(`${SERVICE.EMPLOYEEASSET_SINGLE}/${distributionid}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            let res_employee = await axios.post(`${SERVICE.ASSET_DISTRIBUTION_LOG_DATAS}`, {
                assetmaterialcode: res?.data?.semployeeasset?.assetmaterialcode,
            }, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            const ans = res_employee?.data?.logdata?.length > 0 ? res_employee?.data?.logdata : []
    ...
    const handleprintNear = useReactToPrint({
        ...
        documentTitle: "Asset Employee Distribution Log",
        pageStyle: "print",
    });
    ...
    const fetchUnAssignedIPAddress = async (code, setState) => {
        const result = await getTextAfterFirstDash(code);
        try {
    ...
    return (
        <Box>
            <Headtitle title={"ASSET EMPLOYEE DISTRIBUTION LOG"} />
            {/* ****** Header Content ****** */}
            {/* <Typography sx={userStyle.HeaderText}>
        Asset Employee Distribution
      </Typography> */}
            <PageHeading
                title="Asset Employee Distribution Log"
                modulename="Asset"
                ...
                >
                                    ...
                                    <FormControl fullWidth size="small">
                                        ...
                                        <Selects
                                            ...
                                            onChange={(e) => {
                                                ...
                                                fetchUnAssignedIPAddress(e.value, setAccordianCreateEdit);
                                            }}
                                        />
                                    </FormControl>
                                </Grid>
                    ...
                    /> */}
                            ...
                            <Grid item lg={12} md={12} sm={12} xs={12}>
                                {uploadEdit?.length > 0 && uploadEdit?.map((data, index) => {
                                                ...
                                                >
                                                    {data?.file?.type?.includes("image/") ? (
                                                        <img
                                                            ...
                                                            onLoad={() => URL.revokeObjectURL(fileURL)} // Free up memory when image loads
                                                        />
                                                    ) : (
                                                        <img
                                ...
                                {uploadedImagesEdit?.length > 0 && uploadedImagesEdit?.map((data, index) => {
                                    return (
                                        <Grid
                                            container
                                            key={index}
                                        ...
                                        >
                                            ...
                                            <Grid item md={1} sm={2} xs={2}>
                                                ...
                                                >
                                                    {data?.mimetype?.includes("image/") ? (
                                                        <img
                                                            src={data.preview ? data.preview : `${SERVICE.VEIW_ASSET_REGISTER_FILES}/${data.filename}`} // Use the correct server path
                                                            alt={data.name}
                                                            height={40}
                                                            style={{
                                                                maxWidth: "100%",
                                            ...
                                            <Grid item md={4} sm={4} xs={4}>
                                                <TextField
                                                    ...
                                                    placeholder="Enter remarks"
                                                    value={data?.remarks || ""}
                                                    onChange={(e) => handleRemarkChangeUploadedEdit(e.target.value, index)}
                                                    fullWidth
                                                />
                                            </Grid>
                                            ...
                                            >
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => handleFileDeleteEditUploaded(index)}
                                                >
                                                    <FaTrash
                                                        style={{ fontSize: "18px", color: "#a73131" }}
                                                    />
                ...
                {isUserRoleCompare?.includes("lemployeeassetdistributionregister") && (
                    <Box sx={userStyle.container}>
                        ...
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>
                                {" "}
                                Employee Asset Distribution Log List
                            </Typography>
                        </Grid>
            ...
            <ExportData
                ...
                filename={"AssetEmployeeDistributionLog"}
            ...
            <InfoPopup
                ...
                heading="Employee Asset Distribution Log Info"
                addedby={addedby}
                updateby={updateby}
            />
...
export default EmployeeAssetDistributionLog;