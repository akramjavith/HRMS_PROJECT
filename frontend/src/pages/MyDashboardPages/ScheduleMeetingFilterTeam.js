...
function ScheduleMeetingFilter() {
  ...
  const { isUserRoleCompare,listPageAccessMode, isUserRoleAccess, pageName, setPageName, isAssignBranch, buttonStyles } = useContext(UserRoleAccessContext);
 ...
 if(res.data.resultAccessFilter.length > 0){
      ...
      let res_employee = await axios.post(SERVICE.SCHEDULE_MEETING_FILTER_LIST_TEAM, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  hierarchyempnames: [isUserRoleAccess.companyname],
      ...
      const { company, branch, department, team, companyname, _id, username } = isUserRoleAccess;

      const result = res_employee?.data?.filteredschedulemeeting?.filter((data) => {
        ...
        if (participantsIncludesAll || interviewerIncludesAll) {
          const companyMatch = data?.company?.includes(company) || data?.hostcompany?.includes(company);
          const branchMatch = data?.branch?.includes(branch) || data?.hostbranch?.includes(branch);
          const departmentMatch = data?.department?.includes(department) || data?.hostdepartment?.includes(department);
          const teamMatch = data?.team?.includes(team) || data?.hostteam?.includes(team);
          const participantsOrInterviewerIncludes = data?.participants?.includes(companyname) || data?.interviewer?.includes(companyname);

          if ((companyMatch && branchMatch && departmentMatch && teamMatch && (participantsIncludesAll || participantsOrInterviewerIncludes)) || interviewerIncludesAll || participantsOrInterviewerIncludes || data?.addedby?.some((item) => item?.name === username)) {
            return true;
          }
        }

        return data?.participantsid?.includes(_id) || data?.participants?.includes(companyname) || data?.meetinghostid?.includes(_id) || data?.interviewer?.includes(companyname) || data?.interviewscheduledby === _id || data?.addedby?.some((item) => item?.name === username);
      });

      const resdata = isUserRoleAccess.role.includes('Manager') ? res_employee?.data?.filteredschedulemeeting : result;
      ...
      setMeetingArray(
        resdata.map((item, index) => ({
          ...
          branchvenue: item.branchvenue?.map((t, i) => `${i + 1 + '. '}` + t).toString(),
          floorvenue: item.floorvenue?.map((t, i) => `${i + 1 + '. '}` + t).toString(),
          team: item.team?.map((t, i) => `${i + 1 + '. '}` + t).toString(),
          department: item.department?.map((t, i) => `${i + 1 + '. '}` + t).toString(),
          date: moment(item.date).format('DD-MM-YYYY'),
          participants: item.participants?.map((t, i) => `${i + 1 + '. '}` + t).toString(),
          interviewer: item.interviewer?.map((t, i) => `${i + 1 + '. '}` + t).toString(),
        }))
      );
    }
    } catch (err) {
  ...
  return (
    <Box>
      ...
      <PageHeading title="Schedule Meeting Filter" modulename="Human Resources" submodulename="HR" mainpagename="Meeting" subpagename="Schedule Meeting Filter" subsubpagename="" />
        ...
        <>
          <>
            <Box sx={userStyle.container}>
              <Grid item xs={8}>
                <Typography sx={userStyle.importheadtext}>Schedule Meeting Filter List</Typography>
              </Grid>
                ...
                <Grid item md={2.5} xs={12} sm={12}>
                  ...
                  <FormControl fullWidth size="small">
                    <Selects
                      ...
                      value={repeatTypeOption.find((option) => option.value === meetingState.repeattype)}
                      onChange={(e) => {
                        setMeetingState({
                          ...meetingState,
                          repeattype: e.value,
