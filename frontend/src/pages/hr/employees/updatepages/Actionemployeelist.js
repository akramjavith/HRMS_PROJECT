...
function Noticeperiodactionemployeelist() {
    ...
    const handleCompanyChange = (options) => {
        ...
        setValueEmployeeCat([]);
        setSelectedOptionsEmployee([]);
        setValueEmp([]); setFilterState((prev) => ({
            ...prev,
            employee: "Please Select Employee",
            branch: "",
            unit: "",
        ...
        if (isAssigned) {
            if (groupDetails) {
                ...
                if (
                    ...
                    replaceName === "Please Choose Replace name"
                ) {
                    setLoading(false);
            ...
            if (status !== "Hold") {
                ...
                let DeleteSuperVisor =
                    ...
                    hierarchyDeleteData.map((data, i) => {
                        ...
                        if (
                            data.supervisorchoose.length > 1 &&
                            !checkSame &&
                            !checkSamesup
                        ) {
                ...
                const userUpdateReportingTo = await axios.post(
                    `${SERVICE.USER_REPORTINGTO_CHANGE}`,
                    {
                        headers: {
                            Authorization: `Bearer ${auth.APIToken}`,
                        },
