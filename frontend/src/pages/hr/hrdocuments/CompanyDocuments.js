    ...
    const fetchOverallExcelDatas = async () => {
        ...
        try {
           let res_freq = await axios.post(`${SERVICE.ACCESSIBLEBRANCHALL_COMPANY_DOCUMENTPREPARATION}`, {
            ...
            setOverallExcelDatas(res_freq?.data?.companydocumentPreparation?.filter(data => data?.printingstatus === "Not-Printed"))
    ...
    const TemplateDropdownsValue = async (comp , bran , e) => {
        try {
            ...
            if (res?.data?.templatecontrolpanel?.length > 0) {
                const answer = res?.data?.templatecontrolpanel?.length > 0 ? res?.data?.templatecontrolpanel?.find(data => data?.company === comp && data?.branch === bran) : ""
                const ans = answer ?
                    answer?.templatecontrolpanellog[answer?.templatecontrolpanellog?.length - 1] : ""
                    ;
                const toCompanyBranchOptions = answer ? answer?.toCompany?.map(data => ({
                    ...data,
    ...
    const answer = async (person, index) => {
        ...
        try {
            ...
            const [res, res_emp, res_emp_break] = await Promise.all([
                axios.get(SERVICE.COMPANY_TEMPLATECREATION, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
                }),
...
${signature ? `<img src="${signature}" alt="Signature" style="postion:absolute; z-index:-1; width: 200px; height: 30px;"" /> <br/>` : ""}
  ...
  ` : "")
    ...
    const handleSubmit = (e) => {
        ...
        // const isNameMatchInside = checkingArray?.some((item) => item.template?.toLowerCase() === documentPrepartion.template?.toLowerCase());
        ...
        if (documentPrepartion.template === "" || documentPrepartion.template === "Please Select Template Name") {
            ...
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: "orange" }} />
                    <p style={{ fontSize: "20px", fontWeight: 900 }}>{"Document with  Template Name and Date already exists!"}</p>
                </>
            );
            handleClickOpenerr();
        }
    ...
    const fetchBrandMaster = async () => {
        ...
        try {
           let res_freq = await axios.post(`${SERVICE.ACCESSIBLEBRANCHALL_COMPANY_DOCUMENTPREPARATION}`, {
            ...
            setTemplateCreationArrayCreate(res_freq?.data?.companydocumentPreparation?.filter(data => data?.printingstatus === "Not-Printed"));
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "CompanyDocuments.png");
                });
            });
        }
    };
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("CompanyDocuments.pdf");
    };
    // Excel
    const fileName = "CompanyDocuments";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "CompanyDocuments",
        pageStyle: "print",
    });
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Company Document Preparation</Typography>
                        ...
                        <>
                            <Grid container spacing={2}>
                                ...
                                {/* <Grid item md={3} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        <Typography>
                                            Branch Address<b style={{ color: "red" }}>*</b>
                                        </Typography>
                                        <TextareaAutosize
                                            aria-label="minimum height"
                                            minRows={5}
                                            value={branchaddress?.branchaddress}
                                        />
                                    </FormControl>
                                </Grid> */}
