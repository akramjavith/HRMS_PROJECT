...
function Stockmanagerequest() {
  ...
  let exportColumnNames = ['Company', 'Branch', 'Unit', 'Floor', 'Area', 'Location', 'Date', 'Time', 'Expected Days', 'Due Date', 'Request Mode For', 'AssetType', 'Asset', 'Material', 'Component', 'Subcomponent', 'Product Details', 'UOM', 'Quantity'];
  let exportRowValues = ['company', 'branch', 'unit', 'floor', 'area', 'location', 'requestdate', 'requesttime', 'expectdays', 'duedate', 'requestmode', 'assettype', 'asset', 'material', 'component', 'subcomponent', 'productdetails', 'uom', 'quantity'];
  ...
  console.log(Number(getoldqty), Number(stockmanagemaster.maximumquantity), Number(getoldqty), Number(stockmanagemaster.quantitynew), Number(stockmanagemaster.maximumquantity), '285');
  const handleStockArray = () => {
    const totalArray = stockArray.reduce((acc, sum) => acc + Number(sum.quantitynew), 0);

    const isNameMatch = stockArray.some((item) => item.materialnew == stockmanagemaster.materialnew && item.uomnew === String(stockmanagemaster.uomnew) && item.quantitynew == stockmanagemaster.quantitynew);
    if (stockmanagemaster.materialnew === 'Please Select Material' || stockmanagemaster.materialnew === '') {
      setPopupContentMalert('Please Select Material!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (stockmanagemaster.uomnew === '' || stockmanagemaster.uomnew === undefined) {
      setPopupContentMalert('Please Select UOM!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (stockmanagemaster.quantitynew === '' || stockmanagemaster.quantitynew === undefined) {
      setPopupContentMalert('Please Enter Quantity!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (Number(stockmanagemaster.maximumquantity) < Number(stockmanagemaster.quantitynew)) {
      setPopupContentMalert(`Please Enter Less than or Equal to ${stockmanagemaster.maximumquantity}!`);
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (Number(getoldqty) + Number(stockmanagemaster.quantitynew) + totalArray > Number(stockmanagemaster.maximumquantity)) {
      setPopupContentMalert(`Maximum Quantity Reached!`);
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (isNameMatch) {
      ...
      let findData = uomcodes.find((item) => item.name === stockmanagemaster.uomnew);
  ...
  const [selectedProductname, setSelectedProductname] = useState('Please Select Material Name');
  const [selectedProductnameedit, setSelectedProductnameedit] = useState('Please Select Material Name');
  ...
  const fetchVomMaster = async (e) => {
    try {
      ...
      let getdata = res_vom.data.managestockitems.filter((data) => {
        return data.itemname === e.value && data.stockcategory === stockmanagemaster.stockcategory && data.stocksubcategory === stockmanagemaster.stocksubcategory;
      });
  ...
  const fetchMaterialNew = async (e) => {
    try {
      ...
      const resultall = res.data.managestockitems.filter((data) => {
        return data.stockcategory === stockmanagemaster.stockcategory && data.stocksubcategory === e.value;
      });
  ...
  const fetchLocation = async (e) => {
    try {
      ...
      let result = res_type.data.locationgroupings.filter((d) => d.branch === newcheckbranch && d.floor === stockmanagemaster.floor && d.area === e).map((data) => data.location);
      let ji = [].concat(...result);
      let jiii = ji.map((data) => data);
      const all = [
        { label: 'ALL', value: 'ALL' },
  ...
  const fetchspecificationEdit = async () => {
    try {
      ...
      let result = res.data.assetworkstation?.filter((d) => d.workstation === stockmanagemasteredit.material);
  ...
  const { isUserRoleCompare, isAssignBranch, allProjects, isUserRoleAccess, allCompany, allBranch, allUnit, allTeam, pageName, setPageName, buttonStyles } = useContext(UserRoleAccessContext);
  ...
  const sendRequest = async () => {
    ...
    try {
      let stockcreate = await axios.post(SERVICE.STOCKMANAGE_CREATE, {
        ...
        expectdays: String(stockmanagemaster.expectdays),
        duedate: String(stockmanagemaster.duedate),
        workstation: String(stockmanagemaster.workcheck ? stockmanagemaster.workstation : ''),
        workcheck: String(stockmanagemaster.workcheck),
        assettype: String(stockmanagemaster.assettype === undefined ? '' : stockmanagemaster.assettype),
        asset: String(stockmanagemaster.asset),
        subcomponent: subcomp,
        material: String(stockmanagemaster.material === 'Please Select Material' ? '' : stockmanagemaster.material),
        component: String(stockmanagemaster.component === 'Please Select Component' ? '' : stockmanagemaster.component),
        productdetails: String(stockmanagemaster.productdetails),
        uom: stockmanagemaster.uom === 'Please Select UOM' ? '' : String(stockmanagemaster.uom),
        quantity: Number(stockmanagemaster.quantity),
        updating: String(''),

        requestmode: String(stockmanagemaster.requestmode),
        stockcategory: stockmanagemaster.stockcategory === 'Please Select Stock Category' ? '' : String(stockmanagemaster.stockcategory),
        stocksubcategory: stockmanagemaster.stocksubcategory === 'Please Select Stock Sub Category' ? '' : String(stockmanagemaster.stocksubcategory),
    ...
    if (!isStockMaterial) {
      const isNameMatch = stockmanages.some(
        (item) =>
          ...
          item.productdetails.toLowerCase() === stockmanagemaster.productdetails.toLowerCase() &&
      ...
      if (stockmanagemaster.requestdate === '') {
        ...
        setPopupSeverityMalert('info');
        handleClickOpenPopupMalert();
      } else if (stockmanagemaster.expectdays === '') {
        setPopupContentMalert('Please Enter Expected Days!');
        setPopupSeverityMalert('info');
        handleClickOpenPopupMalert();
      } else if (stockmanagemaster.requestmode === '' || stockmanagemaster.requestmode === 'Please Select Request Mode') {
        setPopupContentMalert('Please Select Request Mode For!');
        setPopupSeverityMalert('info');
        handleClickOpenPopupMalert();
      } else if (stockmanagemaster.material === '' || stockmanagemaster.material === 'Please Select Material') {
        setPopupContentMalert('Please Select Material!');
        setPopupSeverityMalert('info');
        handleClickOpenPopupMalert();
      } else if (stockmanagemaster.component === '' || stockmanagemaster.component === 'Please Select Component') {
        ...
        setPopupSeverityMalert('info');
        handleClickOpenPopupMalert();
      } else if (stockmanagemaster.uom === '' || stockmanagemaster.uom === 'Please Select UOM') {
        setPopupContentMalert('Please Select UOM!');
        setPopupSeverityMalert('info');
        handleClickOpenPopupMalert();
      ...
      // let response = await axios.post(SERVICE.STOCK_MANAGE_ALERT_COUNT, {
      //   headers: {
      //     Authorization: `Bearer ${auth.APIToken}`,
      //   },
      //   itemname: stockmanagemaster.materialnew,
      // })
      // setAlerts(response.data.stockmanage.length);

      console.log(alerts, 'aldfkie');
      const isNameMatch = stockMaterial.some(
        (item) =>
          ...
          item.stockmaterialarray.some((obj1) =>
            stockArray.some(
              (obj2) =>
                obj1.uomnew == obj2.uomnew &&
                obj1.quantitynew == obj2.quantitynew &&
      ...
      //   handleClickOpenPopupMalert();
      // }
      // else if (stockmanagemaster.quantitynew == "") {
      //   setPopupContentMalert("Please Enter Quantity");
      //   setPopupSeverityMalert("info");
      //   handleClickOpenPopupMalert();
      // }
      else if (stockmanagemaster.quantitynew > stockmanagemaster.maximumquantity) {
        setPopupContentMalert(`Please Enter Less than or Equal to ${stockmanagemaster.maximumquantity}!`);
        setPopupSeverityMalert('info');
        handleClickOpenPopupMalert();
  ...
  const getCode = async (e) => {
    ...
    try {
      ...
      let result = res1.data.assetworkstation.filter((d) => d.workstation === res?.data?.sstockmanage.material);
  ...
  const getviewCode = async (e) => {
    ...
    try {
      ...
      let setDataOne = codeValues.find((item1) => res?.data?.sstockmanage.uom === item1.name);
  ...
  const sendEditRequest = async () => {
    ...
    try {
      let res = await axios.put(`${SERVICE.STOCKMANAGE_SINGLE}/${maintenanceid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        ...
        expectdays: String(stockmanagemasteredit.expectdays),
        duedate: String(stockmanagemasteredit.duedate),
  ...
  const editSubmit = (e) => {
    ...
    const isNameMatch = stockmanageedit.some(
      (item) =>
        ...
        item.productdetails.toLowerCase() === String(stockmanagemasteredit.productdetails).toLowerCase() &&
    ...
    if (stockmanagemasteredit.company === 'Please Select Company') {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (stockmanagemasteredit.expectdays === '') {
      setPopupContentMalert('Please Enter Expected Days!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (stockmanagemasteredit.component === '' || stockmanagemasteredit.component === 'Please Select Component') {
      setPopupContentMalert('Please Select Component!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (stockmanagemasteredit.uom === '' || stockmanagemasteredit.uom === 'Please Select UOM') {
      setPopupContentMalert('Please Select UOM!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
  ...
  const fetchOldQuantity = async (material) => {
    try {
      let res_vendor = await axios.post(SERVICE.GET_OLD_MATERIAL_QTY, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        material: material,
      ...
      setOldQty(res_vendor.data.oldtotalqty);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  const fetchStockedit = async (e) => {
    try {
      ...
      setStockmanageedit(res_project?.data?.stockmanage.filter((item) => item._id !== e));
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  const columnDataTable = [
    ...
    {
      field: 'expectdays',
      headerName: 'Expected Days',
      flex: 0,
      width: 100,
      hide: !columnVisibility.expectdays,
      headerClassName: 'bold-header',
    },
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: 'flex' }}>
          {/* {isUserRoleCompare?.includes("estockrequest") && ( */}
          <Button
          ...
          {/* {isUserRoleCompare?.includes("dstockrequest") && ( */}
          <Button
          ...
          {/* )} */}
          {/* {isUserRoleCompare?.includes("vstockrequest") && ( */}
          <Button
            sx={userStyle.buttonedit}
            onClick={() => {
          ...
          {/* )} */}
          {/* {isUserRoleCompare?.includes("istockrequest") && ( */}
          <Button
            sx={userStyle.buttonedit}
            onClick={() => {
              handleClickOpeninfo();
  ...
  const customValueRendererUnitFrom = (valueCate, _employeename) => {
    return valueCate.length ? valueCate.map(({ label }) => label).join(', ') : 'Please select Subcomponent';
  };
  ...
  const handleChangephonenumber = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === '') {
      setStockmanagemaster({ ...stockmanagemaster, expectdays: inputValue });
    }
  };

  const handlePurchaseDateChange = (e) => {
    const { value } = e.target;
    setStockmanagemaster({ ...stockmanagemaster, requestdate: value });
  };
  ...
  const handleChangephonenumberedit = (e) => {
    ...
    const inputValue = e.target.value;
    if (regex.test(inputValue) || inputValue === '') {
      setStockmanagemasteredit({ ...stockmanagemasteredit, expectdays: inputValue });
    }
  };

  const handlePurchaseDateChangeEdit = (e) => {
    const { value } = e.target;
    setStockmanagemasteredit({ ...stockmanagemasteredit, requestdate: value });
  };
  ...
  const calculateExpiryDate = () => {
    if (stockmanagemaster.estimationtime !== '' && stockmanagemaster.requestdate) {
      const currentDate = new Date(stockmanagemaster.requestdate);
      let expiryDate = new Date(currentDate);
      expiryDate.setDate(currentDate.getDate() + parseInt(stockmanagemaster.expectdays));
      ...
      setStockmanagemaster({
        ...stockmanagemaster,
        duedate: formattedempty, // Format date as needed
      });
    }
  };

  const calculateExpiryDateEdit = () => {
    if (stockmanagemasteredit.estimationtime !== '' && stockmanagemasteredit.requestdate) {
      const currentDate = new Date(stockmanagemasteredit.requestdate);
      let expiryDate = new Date(currentDate);
      expiryDate.setDate(currentDate.getDate() + parseInt(stockmanagemasteredit.expectdays));
  ...
  useEffect(() => {
    calculateExpiryDate();
  }, [stockmanagemaster.expectdays, stockmanagemaster.requestdate]);

  useEffect(() => {
    calculateExpiryDateEdit();
  }, [stockmanagemasteredit.expectdays, stockmanagemasteredit.requestdate]);
  ...
  const handleChangeqty = (e) => {
    ...
    const inputValue = e.target.value;
    if (regex.test(inputValue) || inputValue === '') {
      setStockmanagemaster({ ...stockmanagemaster, quantity: inputValue });
    }
  };
  const handleChangeqtynew = (e) => {
    // const regex = /^\d*\.?\d*$/;
    // const regex = /^\d+$/; // Allows only whole numbers
    const regex = /^[1-9]\d*$/;
    const inputValue = e.target.value;
    if (regex.test(inputValue) || inputValue === '') {
      setStockmanagemaster({ ...stockmanagemaster, quantitynew: inputValue });
    }
  };

  console.log(stockmanagemaster.duedate, 'de');

  const rowid = useParams().id;

  const fetchstockReorderFetch = async () => {
    try {
      let res = await axios.post(SERVICE.STOCK_REORDER_FETCH, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        id: rowid,
      ...
      let result = res.data.stock.tododetails.find((d) => d._id === rowid);
      console.log(result, 'result');
      setStockmanagemaster({
        ...stockmanagemaster,
        company: res.data.stock.company,
        branch: res.data.stock.branch,
        unit: res.data.stock.unit,
        floor: res.data.stock.floor,
        area: res.data.stock.area,
        location: res.data.stock.location,
        requestmode: res.data.stock.requestmode,
        materialnew: result?.materialnew,
        stockcategory: result?.category,
        stocksubcategory: result?.subcategory,
        uomnew: result?.uomnew,
        productdetailsnew: result?.productdetailsnew,
      });
  ...
  return (
    <Box>
      ...
      <PageHeading title="Stock Request" modulename="Asset" submodulename="Stock" mainpagename="Stock Manage Request" subpagename="" subsubpagename="" />
      {/* {isUserRoleCompare?.includes("astockrequest") && ( */}
      <>
        <Box sx={userStyle.selectcontainer}>
          <>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.importheadtext}>Add Stock Request</Typography>
              </Grid>
            </Grid>
            <br />
            <Grid container spacing={2}>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput id="component-outlined" readOnly value={stockmanagemaster.company} />
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput id="component-outlined" readOnly value={stockmanagemaster.branch} />
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput id="component-outlined" readOnly value={stockmanagemaster.unit} />
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput id="component-outlined" readOnly value={stockmanagemaster.floor} />
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput id="component-outlined" readOnly value={stockmanagemaster.area} />
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput id="component-outlined" readOnly value={stockmanagemaster.location} />
                </FormControl>
              </Grid>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  <Typography>
                    Expected Days<b style={{ color: 'red' }}>*</b>{' '}
                  </Typography>
                  <OutlinedInput id="component-outlined" type="number" value={stockmanagemaster.expectdays} onChange={(e) => handleChangephonenumber(e)} />
                </FormControl>
              </Grid>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput
                    id="component-outlined"
                    type="text"
                    value={stockmanagemaster.duedate}
                    // onChange={(e) => {
                    //   setStockmanagemaster({ ...stockmanagemaster, duedate: e.target.value });
                    // }}
                  />
                </FormControl>
              </Grid>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput id="component-outlined" readOnly value={stockmanagemaster.requestmode} />
                </FormControl>
              </Grid>
                ...
                <>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput id="component-outlined" readOnly value={stockmanagemaster.stockcategory} />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput id="component-outlined" readOnly value={stockmanagemaster.stocksubcategory} />
                    </FormControl>
                  </Grid>
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput id="component-outlined" readOnly value={stockmanagemaster.materialnew} />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                  ...
                  <Grid item md={3} sm={12} xs={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput
                        ...
                        //   });
                        // }}
                        onChange={(e) => handleChangeqtynew(e)}
                      />
                    </FormControl>
                  </Grid>
                ...
                <>
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput id="component-outlined" type="text" value={stockmanagemaster.assettype} readOnly />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput id="component-outlined" type="text" value={stockmanagemaster.asset} readOnly />
                    </FormControl>
                  </Grid>
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <MultiSelect options={Specificationsub} value={selectedsubcomponent} onChange={handleUnitChangeFrom} valueRenderer={customValueRendererUnitFrom} labelledBy="Please Select SubComponent" />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} sm={12} xs={12}>
                    <FormControl fullWidth size="small">
      ...
      <Box>
        ...
        >
          <Box sx={{ padding: '20px 50px' }}>
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>Edit Stock Asset Request</Typography>
              </Grid>
              <br />
              <Grid container spacing={2}>
                <Grid item md={3} xs={12} sm={12}>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput id="component-outlined" type="number" value={stockmanagemasteredit.expectdays} onChange={(e) => handleChangephonenumberedit(e)} />
                  </FormControl>
                </Grid>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      id="component-outlined"
                      type="text"
                      value={stockmanagemasteredit.duedate}
                      // onChange={(e) => {
                      //   setStockmanagemaster({ ...stockmanagemaster, duedate: e.target.value });
                      // }}
                    />
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput value={stockmanagemasteredit.requestmode} disabled={true} />
                  </FormControl>
                </Grid>
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect options={Specificationsubedit} value={selectedsubcomponentedit} onChange={handleUnitChangeFromEdit} valueRenderer={customValueRendererUnitFromEdit} labelledBy="Please Select SubComponent" />
                  </FormControl>
                </Grid>
                <Grid item md={3} sm={12} xs={12}>
                  <FormControl fullWidth size="small">
      ...
      {/* ****** Table Start ****** */}
      {/* {isUserRoleCompare?.includes("lstockrequest") && ( */}
      <>
        <Box sx={userStyle.container}>
          {/* ******************************************************EXPORT Buttons****************************************************** */}
          <Grid item xs={8}>
            <Typography sx={userStyle.importheadtext}>Stock Asset Request List</Typography>
          </Grid>
      ...
      <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="md" fullWidth={true} sx={{ marginTop: '95px' }}>
        <Box sx={{ padding: '20px 50px' }}>
          <>
            <Typography sx={userStyle.HeaderText}> View Stock Asset Request </Typography>
            <br /> <br />
            <Grid container spacing={2}>
              ...
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>{moment(stockmanagemasteredit.requestdate).format('DD/MM/YYYY')}</Typography>
                </FormControl>
              </Grid>
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  <Typography variant="h6">Time</Typography>
                  <Typography>{stockmanagemasteredit.requesttime}</Typography>
                </FormControl>
              </Grid>
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  <Typography variant="h6">Expected Days</Typography>
                  <Typography>{stockmanagemasteredit.expectdays}</Typography>
                </FormControl>
              </Grid>
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
              ...
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>{stockmanagemasteredit.productdetails}</Typography>
                </FormControl>
              </Grid>
              ...
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>{stockmanagemasteredit.uomcode !== '' || stockmanagemasteredit.uomcode !== undefined ? `${stockmanagemasteredit.quantity}#${stockmanagemasteredit.uomcode}` : ''}</Typography>
                </FormControl>
              </Grid>
            </Grid>
            <br /> <br /> <br />
          ...
          <>
            ...
            <Grid container spacing={2} sx={{ display: 'flex', justifyContent: 'center' }}>
              <Grid item lg={2.5} md={2.5} sm={2} xs={12}>
                <Button variant="contained" color="primary" sx={buttonStyles.buttonsubmit} onClick={handleSubmitvendor}>
                  Submit
                </Button>
              </Grid>
              <Grid item lg={2.5} md={2.5} sm={2} xs={12}>
              ...
              <Grid item lg={2.5} md={2.5} sm={2} xs={12}>
                <Button sx={userStyle.btncancel} onClick={handleCloseviewalertvendor}>
                  Cancel
                </Button>
              </Grid>
            </Grid>
      ...
      />
      {/* INFO */}
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Stock Asset Request Info" addedby={addedby} updateby={updateby} />
