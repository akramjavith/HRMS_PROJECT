  ...
  const fetchLeaveCriteria = async (empname, empdepartment, empdesignation, leavetype, empdoj, empid) => {
    ...
    try {
      ...
      filteredData?.forEach((d) => {
        ...
        let previousYearDataBefore = res_statuspre.data.departmentdetails?.filter((dep) => {
        ...
        if (d.leaveautocheck === true && d.leaveautodays === 'Month') {
          ...
          // }
            ...
            // Check if the user has completed one month from their date of joining
            const oneMonthPassed = currentYear > dojYear || (currentYear === dojYear && currentMonth > dojMonth) || (currentYear === dojYear && currentMonth === dojMonth && currentDate.getDate() >= dojDate);
            // console.log(parseInt(d.leaveautoincrease) === 1, '2747')
            if (parseInt(d.leaveautoincrease) === 1) {
              // console.log(parseInt(d.leaveautoincrease) === 1, '2749')
              if (oneMonthPassed) {
                const leaveAutoIncrease = parseInt(d.leaveautoincrease);
                ...
                console.log(totalAvailableDays, 'oneMonthPassed if 2759')
              }
              else {
                if (currentDate.getDate() > dojDate && currentMonth <= dojMonth) {
                  totalAvailableDays -= (currentDate.getDate() - dojDate);
                  console.log(totalAvailableDays, 'oneMonthPassed else 2764')
                }
              }
            }
            else {
            ...
            console.log(remainingLeaveDays < 0 ? 0 : remainingLeaveDays, '2813')
            setAvailableDays(remainingLeaveDays < 0 ? 0 : remainingLeaveDays);
    ...
    if (Accessdrop === "HR" && selectedOptionsCompany.length === 0) {
      ...
      if (leavecriterias.length > 0) {
        ...
        leavecriterias.forEach((d) => {
          ...
          //     const [day, month, year] = all.formattedDate.split('/').map(Number);
          //     if ((parseInt(val.year) === year) && (val.month === monthstring[month - 1]) && (val.week === all.weekNumberInMonth)) {
          //       console.log(val.year, year)
          //       console.log(val.month, monthstring[month - 1])
          //       console.log(val.week, all.weekNumberInMonth)
          //       console.log(val.day)
          //       dayNamesArray.push(val.day);
          //     }
          //   })
          ...
          // console.log(userDaysArray); // This will print arrays of matched days for each user
          ...
          if (d.leaverespecttoweekoff === true && (hasBeforeDay || hasAfterDay) && !hasWeekOffDay) {
            setShowAlert(
              <>
                <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: "orange" }} />
                <p style={{ fontSize: "20px", fontWeight: 900 }}>{`Please select Weekoff date (${updatedDatesWeekOffDates.map(d => d)}) with these selected date`}</p>
              </>
            );
            handleClickOpenerr();
          }
            ...
            setShowAlert(
              <>
                <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: "orange" }} />
                <p style={{ fontSize: "20px", fontWeight: 900 }}>{`You are not allowed to take leave on ${allUsers.filter(val => uniqueDayNames.includes(val.dayName)).map(day => day.dayName)}`}</p>
              </>
            );
            handleClickOpenerr();
          }
            ...
            if (d.tookleavecheck === true && checkTookLeaveDays) {
              setShowAlert(
                <>
                  <ErrorOutlineOutlinedIcon style={{ fontSize: "100px", color: "orange" }} />
                  <p style={{ fontSize: "20px", fontWeight: 900 }}>{`You are not allowed to take leave on ${allUsers.filter(val => uniqueDayNames.includes(val.dayName)).map(day => day.dayName)}. If you want to apply leave on this day it will be calculate as a Double Lop. Do you want to apply?`}</p>
                </>
              );
        ...
        <Box sx={userStyle.dialogbox}>
          <>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.importheadtext}> Add Apply Leave</Typography>
              </Grid>
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <Selects
                        ...
                        onChange={(e) => {
                          setAppleave({ ...appleave, employeename: e.value, employeeid: e.empcode, reportingto: e.reportingto, department: e.department, designation: e.designation, doj: e.doj, boardingLog: e.boardingLog, workmode: e.workmode, date: "", todate: "", noofshift: '', leavetype: 'Please Select LeaveType' });
                          setAllUsers([]);
                          setGetSelectedDates([]);
                          setAvailableDays('');
