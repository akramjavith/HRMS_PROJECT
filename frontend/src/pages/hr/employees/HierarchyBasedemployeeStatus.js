...
const HierarchyBasedEmployeeLoginStatus = () => {
  ...
  let listpageaccessby =
    listPageAccessMode?.find(
      (data) =>
        ...
        data.subsubpagename === "Team Login Status"
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "HierarchyBasedEmployeeLoginStatus.png");
        });
      });
    }
  };
  ...
  const fetchBranch = async () => {
    try {
      let res_branch = await axios.post(
        SERVICE.HIERARCHY_BASED_USER_LOGIN_STATUS_DEFAULT,
        {
          headers: {
      ...
      const transformedData = transformData(
        res_branch?.data?.resultAccessFilter
      );
  ...
  const TableFilter = async () => {
    ...
    try {
      let res_employee = await axios.post(
        SERVICE.HIERARCHY_BASED_USER_LOGIN_STATUS,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      const transformedData = transformData(
        res_employee?.data?.resultAccessFilter
      );
      setLoginStatus(transformedData);
      setIsBtn(false);
    } catch (err) {
  ...
  const downloadPdf = (isfilter) => {
    ...
    doc.save("HierarchyBasedEmployeeLoginStatus.pdf");
  };

  // Excel
  const fileName = "HierarchyBasedEmployeeLoginStatus";
  let excelno = 1;
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "HierarchyBasedEmployeeLoginStatus",
    pageStyle: "print",
  });
  ...
  const columnDataTable = [
          ...
          onChange={() => {
            ...
            if (selectedRows.includes(params.row.orginalid)) {
              updatedSelectedRows = selectedRows.filter(
                (selectedId) => selectedId !== params.row.orginalid
              );
            } else {
    ...
    },
    {
      ...
      renderCell: (params) => {
        const colorName =
          params.row.matched === "Primary WorkStation"
            ? "Blue"
            : params.row.status === "Secondary WorkStation"
            ? "Green"
            : params.row.status === "Work From Home"
            ? "Red"
            : "Orange";
        return (
  ...
  return (
    <>
      ...
      <Typography sx={userStyle.HeaderText}>
        Hierarchy Based Employee Login Status
      </Typography>
      {/* ****** Table Start ****** */}
      {isUserRoleCompare?.includes("lteamloginstatus") && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>
                Hierarchy Based Employee Login Status List{" "}
              </Typography>
            </Grid>
            <br />
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes("excelteamloginstatus") && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes("csvteamloginstatus") && (
                    <>
                      <Button
                        onClick={(e) => {
                          setIsFilterOpen(true);
                  ...
                  {isUserRoleCompare?.includes("printteamloginstatus") && (
                    <>
                  ...
                  {isUserRoleCompare?.includes("pdfteamloginstatus") && (
                    <>
                      <Button
                        sx={userStyle.buttongrp}
                        onClick={() => {
                  ...
                  {isUserRoleCompare?.includes("imageteamloginstatus") && (
                    <Button
                      sx={userStyle.buttongrp}
...
export default HierarchyBasedEmployeeLoginStatus;
