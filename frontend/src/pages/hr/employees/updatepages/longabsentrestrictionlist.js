...
import Longabsentrestrictionmanual from "./longabsentrestrictionmanual";
...
function EmployeeDetailStatus() {
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Long Absent Restriction List"),
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "Long Absent Restriction Self List.png");
                })
                .catch((error) => {
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "Long Absent Restriction Self List",
        pageStyle: "print",
    });
    ...
    const handleAllowLogin = async () => {
        try {
            ...
            let res = await axios.put(`${SERVICE.USER_SINGLE_PWD}/${recentData?.id}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
    ...
    const isChecklistAssigned = async () => {
        try {
            ...
            const isAvailable = res?.data?.checklistverificationmasters?.some(
                (item) => item.subsubpage === "Long Absent Restriction List"
            );
    ...
    const sendRequest = async () => {
        ...
        try {
            let subprojectscreate = await axios.post(
                SERVICE.GETFILTEREUSERDATALONGABSEND,
                {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
            ...
            console.log(subprojectscreate?.data?.filterallDatauser, "subprojectscreate?.data?.filterallDatauser")
            ...
            setEmployeesoverall(resselfdata.map((item, index) => ({
                _id: item?._id,
                serialNumber: index + 1,
                company: item?.company,
                userstatus: item.userstatus,
    ...
    return (
        <Box>
            <NotificationContainer />
            <Headtitle title={"Long Absent Restriction List"} />
            {/* ****** Header Content ****** */}
            <PageHeading
                ...
                subsubpagename="Long Absent Restriction List"
            />

            {isUserRoleCompare?.includes("alongabsentrestrictionlist") && (
                <>
                ...
                <>
                    {isUserRoleCompare?.includes("llongabsentrestrictionlist") && (
                        <>
                                ...
                                <Grid container spacing={2}>
                                    <Grid item xs={8}>
                                        <Typography sx={userStyle.SubHeaderText}>
                                            Long Absent Restriction Self List
                                        </Typography>
                                    </Grid>
                                    <Grid item xs={4}>
                            ...
                            <br />
                            <Longabsentrestrictionmanual isManualUser={isManualUser} setManualUpdate={setManualUpdate} isAssigned={isAssigned} setRecentData={setRecentData} setShowAlert={setShowAlert} handleClickOpenerr={handleClickOpenerr} />
                        </>
                    )}
                </>
            )}
            ...
            <ExportData
                ...
                itemsTwo={employees ?? []}
                filename={"Long Absent Restriction Self List"}
