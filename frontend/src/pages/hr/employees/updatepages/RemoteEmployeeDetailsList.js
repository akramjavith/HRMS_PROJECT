...
function RemoteEmployeeDetailsList() {
  ...
  const getCode = async (e, mode) => {
    try {
      ...
      const lastworkmode =
        res?.data?.suser.addremoteworkmode.length > 0
          ? res?.data?.suser.addremoteworkmode[
              res?.data?.suser.addremoteworkmode.length - 1
            ]
          : res?.data?.suser.addremoteworkmode;
  ...
  const sendRequestt = async () => {
    ...
    try {
      let res = await axios.put(`${SERVICE.USER_SINGLE_PWD}/${boredit}`, {
        ...
        workstationinput: primaryWorkStationInput,
  ...
  const handleExportXL = (isfilter) => {
    ...
    exportToExcel(formatData(dataToExport), "Remote Employee Details List");
    setIsFilterOpen(false);
  };
  ...
  const downloadPdf = (isfilter) => {
    ...
    doc.save("Remote Employee Details List.pdf");
  };
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Grid sx={{ display: "flex" }} gap={2}>
          ...
          {isUserRoleCompare?.includes("eremoteemployeedetailslist") && (
            <Link to={`/remoteemployeedetailslog/${params.row.id}`}>
  ...
  return (
    <Box>
      ...
      {isUserRoleCompare?.includes("aremoteemployeedetailslist") && (
        <>
        ...
        >
          <Box sx={userStyle.dialogbox}>
            <>
              ...
              <Grid container spacing={2}>
                {pfesiform?.workstationinput && (
                  <Grid item md={4} sm={12} xs={12}>
                    <Typography>
                      Work Station: {pfesiform?.workstationinput}
                    </Typography>
                  </Grid>
                )}
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <TextareaAutosize
                      ...
                      readOnly={true}
                      // onChange={(e) => {
                      //     setAddremoteemployeeWorkmode({ ...addremoteemployeeWorkmode, wfhconfigurationdetails: e.target.value });
                      // }}
                    />
                  </FormControl>
                </Grid>
              ...
              <br /> <br /> <br />
              <Grid container>
                <Button variant="contained" onClick={handleCloseModView}>
                  Back
                </Button>
              </Grid>
            </>
