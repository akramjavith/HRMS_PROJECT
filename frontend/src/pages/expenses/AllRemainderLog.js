...
function AllRemainderLog() {
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage.toBlob(gridRefTableImg.current)
        .then((blob) => {
          saveAs(blob, "All Remainder Log.png");
        })
        .catch((error) => {
  ...
  // Excel
  const fileName = "AllRemainderLog";

  //print...
  const componentRef = useRef();
  const handleprint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: "All Remainder Log",
    pageStyle: "print",
  });
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex", padding: "10px" }}>
          {isUserRoleCompare?.includes("eallremainder") && !params?.data?.finalbillstatus && (
          ...
          {isUserRoleCompare?.includes("vallremainder") && (
            <Button
              onClick={() => {
  ...
  return (
    <Box>
      <Headtitle title={"ALL REMAINDER LOG"} />
      {/* ****** Header Content ****** */}
      <Typography sx={userStyle.HeaderText}>
        All Remainder List Log
      </Typography>
      ...
      >
        <Box sx={{ width: "550px", padding: "20px 50px" }}>
          <>
            <Typography sx={userStyle.HeaderText}>All Remainder Log Info</Typography>
            <br />
            <br />
            <Grid container spacing={2}>
...
export default AllRemainderLog;
