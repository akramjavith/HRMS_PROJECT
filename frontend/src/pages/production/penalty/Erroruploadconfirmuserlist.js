...
function Erroruploadconfirmuserlist() {
  ...
  const tableOptions = [
    { label: 'Error Upload Confirm Overall Report List', value: 'Error Upload Confirm Overall Report List' },
  ...
  const exportColumnNames = ['Employee Name', 'Branch', 'Unit', 'Project Vendor', 'Date', 'Queue Name', 'Login ID', 'Accuracy', 'Total Fields', 'Error Count', 'Auto Count', 'Manual Total', 'Manual Error', 'Upload Count'];
  const exportRowValues = ['companyname', 'branchname', 'unitname', 'projectvendor', 'date', 'queuename', 'loginid', 'accuracy', 'totalfields', 'errorcount', 'autocount', 'manualtotal', 'manualerror', 'Manual Error', 'uploadcount'];
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, 'Error Upload Confirm Overall Report.png');
        })
        .catch((error) => {
  ...
  let listpageaccessby =
    listPageAccessMode?.find((data) => data.modulename === 'Quality' && data.submodulename === 'Penalty' && data.mainpagename === 'Penalty Setup' && data.subpagename === 'Penalty Calculation' && data.subsubpagename === 'Error Upload Confirm Overall Report')?.listpageaccessmode || 'Overall';
  ...
  const fetchBatchFilter = async () => {
    ...
    setLoader(true);
    try {
      let res_employee = await axios.post(SERVICE.ERROR_UPLOAD_CONFIRM_USER_LIST, {
        headers: {
      ...
      const filtered = res_employee?.data?.resultAccessFilter;
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String('Error Upload Confirm Overall Report'),
  ...
  return (
    <Box>
      {/* <Headtitle title={"Error Upload Confirm Overall Report"} /> */}
      {/* ****** Header Content ****** */}
      <PageHeading title="Error Upload Confirm Overall Report" modulename="Quality" submodulename="Penalty" mainpagename="Penalty Setup" subpagename="Penalty Calculation" subsubpagename="Error Upload Confirm Overall Report" />
      <Box>
        ...
        <>
          <Box sx={userStyle.dialogbox}>
            <>
              <Grid item xs={8}>
                <Typography sx={userStyle.importheadtext}>Error Upload Confirm Overall Report List</Typography>
              </Grid>
              <Grid container spacing={2} style={userStyle.dataTablestyle}>
                ...
                >
                  <Box>
                    {isUserRoleCompare?.includes('excelerroruploadconfirmoverallreport') && (
                      <>
                        <Button
                          onClick={(e) => {
                    ...
                    {isUserRoleCompare?.includes('csverroruploadconfirmoverallreport') && (
                      <>
                        <Button
                          onClick={(e) => {
                            setIsFilterOpen(true);
                    ...
                    {isUserRoleCompare?.includes('printerroruploadconfirmoverallreport') && (
                      <>
                    ...
                    {isUserRoleCompare?.includes('pdferroruploadconfirmoverallreport') && (
                      <>
                        <Button
                          sx={userStyle.buttongrp}
                          onClick={() => {
                    ...
                    {isUserRoleCompare?.includes('imageerroruploadconfirmoverallreport') && (
      ...
      <ExportData
        ...
        itemsTwo={isuser ?? []}
        filename={'Error Upload Confirm Overall Report'}
...
export default Erroruploadconfirmuserlist;
