...
function Documentedit() {
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Document Details List.png");
        });
      });
    }
  };
  ...
  const getCode = async (e) => {
    ...
    try {
      ...
      if (
        resNew?.data?.semployeedocument &&
        resNew?.data?.semployeedocument?.files?.length > 0
      ) {
  ...
  const handleFileUpload = (event) => {
    ...
    if (showAlert) {
      setAlertMessage(
        "File size greater than 1MB, please upload a file below 1MB."
      );
    } else {
  ...
  return (
    <Box>
      ...
      <PageHeading
        ...
        subsubpagename="Document Info Update"
      />
