...
function AssetWorkStationUnassigned() {
  ...
  const { isUserRoleAccess, isUserRoleCompare, pageName, setPageName, isAssignBranch, buttonStyles, workStationSystemName, } = useContext(
  ...
  const [columnVisibility, setColumnVisibility] = useState(
    {}
    // { ...initialColumnVisibility, ...allComponents, }
  );
  ...
  //     });
  //     const result = res_workstations?.data?.locationgroupings.flatMap((item) =>
  ...
  //             subcabinname: combinstationItem.cabinname,
  //             subsubcabinname: subTodo.subcabinname,
  //             _id: item._id,
  //             finalcabinname: subTodo.subcabinname,
  //           }))
  //           : [{
  //             company: item.company,
  //             branch: item.branch,
  ...
  //             subsubcabinname: "",
  //             _id: item._id,
  //             finalcabinname: combinstationItem.cabinname,
  //           }]
  //       )
  //     );
  //     const updatedresult = result?.map(mainObj => {
  //       // Split workstation into multiple cabins
  //       const workstations = mainObj.finalcabinname;
  ...
  //         return {
  //           ...mainObj,
  //           shortname: match.systemshortname
  //             ? `${match.systemshortname}`
  //             : "" // Join updated workstations back with commas
  //         };
  //       }
  ...
  const fetchEmployee = async () => {
    ...
    try {
      ...
      const workstationMap = new Map(
        workStationSystemName?.map(secObj => [
          `${secObj.company}-${secObj.branch}-${secObj.unit}-${secObj.floor}-${secObj.cabinname}`,
          secObj.systemshortname || "",
        ])
      );

      // Process the data efficiently using reduce
      const updatedresult = res_workstations?.data?.locationgroupings.reduce((acc, item) => {
        item.combinstation.forEach(combinstationItem => {
          if (combinstationItem.subTodos.length) {
            combinstationItem.subTodos.forEach(subTodo => {
              const key = `${item.company}-${item.branch}-${item.unit}-${item.floor}-${subTodo.subcabinname}`;
              acc.push({
                ...item,
                subcabinname: combinstationItem.cabinname,
                subsubcabinname: subTodo.subcabinname,
                finalcabinname: subTodo.subcabinname,
                shortname: workstationMap.get(key) || "",
              });
            });
          } else {
            const key = `${item.company}-${item.branch}-${item.unit}-${item.floor}-${combinstationItem.cabinname}`;
            acc.push({
              ...item,
              subcabinname: combinstationItem.cabinname,
              subsubcabinname: "",
  ...
  const oldrowDataTable = filteredData.map((item, index) => {
    return {
      ...
      finalcabinname: item.finalcabinname,
      ...
      assetGroupingDatas
        .filter(
          (asset) =>
            ...
            asset.workstation === row.finalcabinname &&
  ...
  const columnDataTable = [
...

    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("vassetworkstationgroupingreport") && (
            <Button
  ...
  console.log(columnDataTable, newArrayOfObjects, "columndatatable")
        ...
        <Grid container>
          <Grid item md={4}>
            <Button
              ...
              onClick={() => setColumnVisibility({ ...initialColumnVisibility, ...allComponents })}
            >
              Show All
            </Button>
          </Grid>
            ...
            <Button
              ...
              onClick={() => {
                ...
                columnDataTable.forEach((column) => {
                  ...
                  allComponents[column.field] = false;
                  // Set hide property to true
                });
                setColumnVisibility({ ...newColumnVisibility, ...allComponents });
              }}
            >
              {" "}
              Hide All
        ...
        >
          ...
          >
            ...
            >
              ...
              {datasToAllot?.shortname ? `${datasToAllot?.subsubcabinname}(${datasToAllot?.shortname})` : `${datasToAllot?.subsubcabinname}`} ?
            </Typography>
          </DialogContent>
          <DialogActions>
            <Button
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>{maintentancemasteredit.shortname}</Typography>
                </FormControl>
              </Grid>
