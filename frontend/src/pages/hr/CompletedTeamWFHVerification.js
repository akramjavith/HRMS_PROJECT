...
function CompletedTeamPermissionVerification({ data, setUpdated }) {
  ...
  // let exportRowValuescrt = ["employeename", "employeeid", "date", "fromtime", "requesthours", "endtime", "reasonforworkfromhome", "status",]
  ...
  const handleExportXL = async (isfilter) => {
    ...
    saveAs(new Blob([buffer]), `Completed Team WFH Verification.${fileExtension}`);
    setIsFilterOpen(false);
  };
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: 'Completed Team WFH Verification',
    pageStyle: 'print',
  });
  ...
  const handleCaptureImage = () => {
    if (gridRefImageApprovedPerm.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, 'Completed Team WFH Verification.png');
        })
        .catch((error) => {
    ...
    <Box>
      ...
      {isUserRoleCompare?.includes('lteampermissionverification') && (
        <>
          {/* ******************************************************EXPORT Buttons****************************************************** */}
          <Grid item xs={8}>
            <Typography sx={userStyle.importheadtext}>Completed Team WFH Verification List</Typography>
          </Grid>
      ...
      {/* <ExportData
                ...
                itemsTwo={items ?? []}
                filename={"Completed Team WFH Verification"}
