...
console.log(connectivityOption,"connectivityOption")
  const fetchHardwareSpecificaiton = async () => {
    try {
      let res = await axios.get(SERVICE.ALL_HARDWARESPECIFICATION, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });

      // const connectivityItems = res.data?.hardwarespecification?.filter(
      //   (d) => d.type === "Connectivity"
      // );
      // const TypeItems = res.data?.hardwarespecification?.filter(
      //   (d) => d.type === "Asset Specification Type"
      // );
  
      // // Map to get only the names
      // const namesconnectivity = connectivityItems.map(item => ({label:item.name,value:item.name}));
      // const namestype = TypeItems.map(item => ({label:item.name,value:item.name}));

      // setConnectivityOption(namesconnectivity)
      // setTypeOption(namestype)

      const groupedOptions = {};

    res.data?.hardwarespecification?.forEach(item => {
      if (!groupedOptions[item.type]) {
        groupedOptions[item.type] = [];
      }
      groupedOptions[item.type].push({ label: item.name, value: item.name });
    });

    // Now set individually

    setTypeOption(groupedOptions["Asset Specification Type"] || []);
    setModelOption(groupedOptions["Asset Model"] || []);
    setSizeOption(groupedOptions["Asset Size"] || []);
    setVariantOption(groupedOptions["Asset Variant"] || []);
    setBrandOption(groupedOptions["Brand Master"] || []);
    setCapacityOption(groupedOptions["Asset Capacity"] || []);
    setPanelTypeOption(groupedOptions["Panel Type"] || []);
    setScreenResolutionOption(groupedOptions["Screen Resolution"] || []);
    setConnectivityOption(groupedOptions["Connectivity"] || []);

    setDataRateOption(groupedOptions["Data Range"] || []);
    setCompatibleDevicesOption(groupedOptions["Compatible Device"] || []);
    setOutputPowerOption(groupedOptions["Output Power"] || []);
    setCoolingFanCountOption(groupedOptions["Cooling Fan Count"] || []);
    setClockSpeedOption(groupedOptions["Clock Speed"] || []);


    
    setCoreOption(groupedOptions["Cores"] || []);
    setSpeedOption(groupedOptions["Speed"] || []);
    setFrequencyOption(groupedOptions["Frequency"] || []);
    setOutputOption(groupedOptions["Output"] || []);
    setEthernetPortsOption(groupedOptions["Ethernet Ports"] || []);


    setDistanceOption(groupedOptions["Distance"] || []);
    setLengthOption(groupedOptions["Length"] || []);
    setSlotOption(groupedOptions["Slot"] || []);
    setNoOfChannelsOption(groupedOptions["Number Of Channels"] || []);
    setColoursOption(groupedOptions["Colours"] || []);
