...
function Managecategory() {
  ...
  const { isUserRoleCompare, isUserRoleAccess, buttonStyles, isAssignBranch, pageName, setPageName } = useContext(UserRoleAccessContext);
  ...
  const handleSubmit = async (e) => {
    ...
    const isNameMatch = overallDataDuplicate?.some(
      (item) => item.project == managecategory.project && item.mode == managecategory.mode && item.category == managecategory.category && item.time == managecategory.time && item.subcategory.some((data) => selectedSubCategoryValue.includes(data)) && item.schedule == managecategory.schedule
    );
  ...
  const getCode = async (e) => {
    ...
    try {
      ...
      setSelectedSubCategoryEditForm([...res?.data?.smanagecategory?.subcategory.map((t) => ({ ...t, label: t, value: t }))]);
      setSubcategories([...res?.data?.smanagecategory?.subcategory.map((t) => ({ ...t, label: t, value: t }))]);
      ...
      setTaskGroupingArrayEdit(overallDataDuplicate.filter((item) => item._id !== e));
      handleClickOpenEdit();
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  ...
  const sendEditRequest = async () => {
    setPageName(!pageName);
    try {
      let res = await axios.put(`${SERVICE.MANAGECATEGORY_SINGLE}/${taskgroupingId}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const fetchTaskGroupingArray = async () => {
    ...
    try {
      ...
      setTaskGroupingFilterArray(
        res_freq?.data?.managecategory?.map((t, index) => ({
          ...t,
          Sno: index + 1,
          project: t.project,
          mode: t.mode,
  ...
  const fetchEmployee = async () => {
    ...
    try {
      ...
      const ansOverall =
        res_employee?.data?.totalProjectOverall?.length > 0
          ? res_employee?.data?.totalProjectOverall?.map((item, index) => ({
              ...item,
  ...
  const fetchAllProjects = async () => {
    ...
    try {
      ...
      const projall = [
        ...res_queue?.data?.projmaster.map((d) => ({
          ...d,
          label: d.name,
          value: d.name,
        })),
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Category Idle Time" modulename="Production" submodulename="SetUp" mainpagename="Manage Category" subpagename="" subsubpagename="" />
                ...
                <>
                  ...
                  <Grid item md={3} sm={12} xs={12}>
                    ...
                    <Grid sx={{ display: 'flex', gap: '15px' }}>
                      <Button sx={buttonStyles.buttonsubmit} disabled={btnSubmit} onClick={handleSubmit}>
                        Submit
                      </Button>
        ...
        <>
          {isUserRoleCompare?.includes('lmanagecategory') && (
            <Box sx={userStyle.container}>
              {/* ******************************************************EXPORT Buttons****************************************************** */}
              <Grid item xs={8}>
                <Typography sx={userStyle.importheadtext}>Manage Category List</Typography>
              </Grid>
              ...
              <Box>
                <Pagination page={searchQuery !== '' ? 1 : page} pageSize={pageSize} totalPages={searchQuery !== '' ? 1 : totalPages} onPageChange={handlePageChange} pageItemLength={filteredDatas?.length} totalProjects={searchQuery !== '' ? filteredDatas?.length : totalProjects} />
              </Box>
              {/* ****** Table End ****** */}
            </Box>
          )}
      ...
      <Dialog open={openInfo} onClose={handleCloseinfo} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        <Box sx={{ width: '550px', padding: '20px 50px' }}>
          <>
            <Typography sx={userStyle.HeaderText}>Manage Category Info</Typography>
            <br />
            <br />
            <Grid container spacing={2}>
      ...
      <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="sm" fullWidth={true}>
        <Box sx={{ padding: '20px 50px' }}>
          <>
            <Typography sx={userStyle.HeaderText}> View Manage Category</Typography>
            <br /> <br />
            <Grid container spacing={2}>
      ...
      <Box>
        ...
        >
          <Box sx={{ padding: '20px 50px' }}>
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>Edit Manage Category</Typography>
              </Grid>
              <br />
              <Grid container spacing={2}>
                <Grid item md={6} xs={12} sm={12}>
                ...
                <Grid item md={6} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect options={subCtegoryOpt} value={selectedSubCategoryEditFrom} onChange={handleSubCategoryFromEdit} valueRenderer={customValueRendererSubcategoryFrom} labelledBy="Please Select SubCategory" />
                  </FormControl>
                </Grid>
                <Grid item md={6} xs={12} sm={12}>
                  <Typography>
      ...
      />
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Manage Category Info" addedby={addedby} updateby={updateby} />
