...
function TeamLeaveVerification() {
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "TeamLeaveVerification.png");
        });
      });
    }
  };
  ...
  const fetchLeaveType = async () => {
    try {
      ...
      setLeaveTypeData(
        res_leavetype?.data?.leavetype.map((data) => ({
          ...data,
  ...
  const delApply = async (e) => {
    try {
      if (Applysid) {
        ...
        await fetchApplyleave(leaveVerification);
        handleCloseMod();
        setSelectedRows([]);
        setPage(1);
      }
  ...
  const fetchUsersRandom = async (value, date) => {
    ...
    if (date === "") {
      ...
      getSelectedDates.includes(moment(date, "YYYY-MM-DD").format("DD/MM/YYYY"))
    ) {
      setShowAlert(
        <>
          <ErrorOutlineOutlinedIcon
  ...
  const fetchUsersRandomEdit = async (value, date) => {
    let check = checkDuplicateEdit.includes(
    ...
    if (todate === "") {
      ...
      setShowAlert(
        <>
          ...
          />
          <p style={{ fontSize: "20px", fontWeight: 900 }}>
            {"These Date Range is Already Exists"}
          </p>
        </>
      );
      handleClickOpenerr();
  ...
  const fetchLeaveVerification = async () => {
    try {
      ...
      setLeaveVerification(mapEmployeeNames);
  ...
  const fetchApplyleave = async (leaveverificationdata) => {
    try {
      let res_vendor = await axios.get(SERVICE.APPLYLEAVE_FILTERED, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      ...
      let answer =
        isUserRoleAccess?.role?.includes("Manager") ||
          ...
          : res_vendor?.data?.applyleaves.filter((data) =>
  ...
  // Excel
  const fileName = "TeamLeaveVerification";
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "TeamLeaveVerification",
    pageStyle: "print",
  });
  ...
  const downloadPdf = (isfilter) => {
    ...
    doc.save("TeamLeaveVerification.pdf");
  };
  ...
  useEffect(() => {
  }, [appleaveEdit, appleave, applyleaves]);
  ...
  const columnDataTable = [
    ...
    },
    {
      ...
      flex: 0,
      width: 250,
      hide: !columnVisibility.leavetype,
      headerClassName: "bold-header",
    },
    {
      field: "date",
    ...
    },
    {
      ...
      flex: 0,
      width: 250,
      hide: !columnVisibility.numberofdays,
      headerClassName: "bold-header",
    },
    {
      ...
      flex: 0,
      width: 250,
      hide: !columnVisibility.reasonforleave,
      headerClassName: "bold-header",
    },
    {
      field: "status",
      ...
      renderCell: (params) => (
        <Grid sx={{ display: "flex" }}>
          {/* {(!(
            ...
            ["Approved", "Rejected"].includes(params.row.status)) ||
            (isUserRoleCompare?.includes("eapplyleave") && (
              <Button
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <Typography sx={userStyle.HeaderText}>Team Leave Verification</Typography>
                    ...
                    "excelteamleaveverification"
                      ...
                      <>
                        ...
                        <Button onClick={(e) => {
                          ...
                          fetchApplyleave(leaveVerification)
                          setFormat("xl")
