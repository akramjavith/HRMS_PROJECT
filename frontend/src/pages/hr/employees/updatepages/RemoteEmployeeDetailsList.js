  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      let empData = updatedData?.map((item, index) => ({
        ...item, serialNumber: index + 1, shortname: item.workstationinput ? item?.workstationinput?.slice(0, 15) : "",
    ...
    <Box>
      ...
      {isUserRoleCompare?.includes("menuremoteemployeedetailslist") && (
        <>
          ...
          <br />
          <RemoteEmployeePendingDetailsList tableData={employeesPending} setReload={setReload} tableLoader={tableLoader} branchData={branchData}
            unitData={unitData} />
        </>
      )}{" "}
