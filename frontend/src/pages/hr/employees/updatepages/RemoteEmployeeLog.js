...
function RemoteEmployeeLog() {
  ...
  const getCode = async (e, dataid) => {
    try {
      ...
      const lastworkmode = res?.data?.suser.addremoteworkmode.filter(
      ...
      setPrimaryWorkStationInput(lastworkmode[0]?.workstationinput);
    } catch (err) {
  ...
  const fetchEmployee = async () => {
    try {
      ...
      setBankdetail(true);
      setEmpDetails(res_employee?.data?.suser);
      let data_emp = res_employee?.data?.suser?.addremoteworkmode;
  ...
  const handleExportXL = (isfilter) => {
    ...
    exportToExcel(formatData(dataToExport), "Remote Employee Log List");
    setIsFilterOpen(false);
  };
  ...
  const downloadPdf = (isfilter) => {
    ...
    doc.save("Remote Employee Log List.pdf");
  };
