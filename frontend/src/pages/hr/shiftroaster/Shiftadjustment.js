...
import { Box, Typography, OutlinedInput, TableBody, TableRow, TableCell, Select, Badge, TextareaAutosize, MenuItem, Dialog, DialogContent, DialogActions, FormControl, Grid, Paper, Table, TableHead, TableContainer, Button, Popover, TextField, IconButton, InputAdornment, Tooltip } from "@mui/material";
...
import ShiftAdjustmentApprovedListTable from "./Shiftadjustmentapprovedlist";
...
function ShiftAdjustmentList() {
    ...
    const { isUserRoleAccess, isUserRoleCompare, isAssignBranch, allUsersLimit, alldepartment, pageName, setPageName, buttonStyles, } = useContext(UserRoleAccessContext);
    ...
    const [allUsersAdjTableApproved, setAllUsersAdjTableApproved] = useState([])
    const [getAdjStatusApprovedDates, setGetAdjStatusApprovedDates] = useState([])
    const [adjApply, setAdjApply] = useState(false);
    const [adjApproved, setAdjApproved] = useState(false);
    ...
    const [filterUser, setFilterUser] = useState({ mode: "Employee" });
    ...
    const [filteredDataItemsFinalAdj, setFilteredDataItemsFinalAdj] = useState(allUsers);
    const [filteredDataItemsAdjList, setFilteredDataItemsAdjList] = useState(allUsersAdjTable);
    const [filteredRowDataFinalAdj, setFilteredRowDataFinalAdj] = useState([]);
    const [filteredDataItemsAdjListApproved, setFilteredDataItemsAdjListApproved] = useState(allUsersAdjTableApproved);
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Shift Adjustment"),
    ...
    }, [allUsersLimit, valueCompany, selectedBranch, selectedUnit]);
    ...
    const getAllDate = (daysArrayTemp) => {
        // Pass daysArrayTemp to setColumnVisibilityFinalAdj after it's populated
    ...
    let initialColumnVisibilityFinalAdj = {
        serialNumber: true,
        checkbox: true,
        empcode: true,
        username: true,
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            async function sendBatchRequest(batch) {
                ...
                try {
                    ...
                    // Filter out allUsersLimit based on reasondate and doj date
            ...
            getAllResults().then(async (results) => {
                ...
                filteredItems.map(user =>
                    user.shiftallot.map(allot => {
                        if (allot.adjustmentstatus == true || allot.adjdate === 'Not Allotted') {
                            if (allot.adjstatus === 'Adjustment') {
                ...
                let resultarrApproved = []
                filteredItems.map(user =>
                    user.shiftallot.map(allot => {
                        if (allot.adjustmentstatus == true || allot.adjdate === 'Not Allotted') {
                            if (allot.adjstatus === 'Approved' || allot.adjstatus === 'Reject') {
                                resultarrApproved.push({ ...allot });
                            }
                        }
                    })
                );
                ...
                setFilteredDataItemsAdjList(resultarr);

                setAllUsersAdjTableApproved(resultarrApproved);
                setFilteredDataItemsAdjListApproved(resultarrApproved);
                ...
                let result = valueDep.length > 0 ?
                    (itemsWithSerialNumber.filter((data) => valueDep.includes(data.department)).map((item, index) => ({
                        ...item,
                ...
                setAllUsers(result);
                setFilteredDataItemsFinalAdj(result);
                setSearchQueryFinalAdj("");
                setTotalPagesFinalAdj(Math.ceil(result.length / pageSizeFinalAdj))
                setAllUsersShiftFinal(filteredItems);
                setAllShiftAdj(false);
                setAdjApply(false);
                setAdjApproved(false);
        ...
        } catch (err) { setAllShiftAdj(true); setAdjApply(true); setAdjApproved(true); setAllFinalAdj(true); handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
    };
    ...
    const getCode = (data, rowdata, shiftlabel, rowadjstatus) => {
        handleClickOpenEdit();
        ...
        rowdata?.shiftallot.forEach(value => {
            if (value.date === undefined && value.selectedDate === data.formattedDate) {
                setGetShiftAdjMatchedId(value._id)
            }
    ...
    const getSelectedDateShift = (value, empcodevalue) => {
        // Find the corresponding row in filteredDataItemsFinalAdj based on the provided empcodevalue
        const row = filteredDataItemsFinalAdj.find(item => item.empcode === empcodevalue);
        if (row) {
    ...
    // const isSecondShiftValid = (firstShift, secondShift) => {
    ...
    // const isContinuousShiftValid = (firstShift, secondShift) => {
    //     // Adjust for midnight transition
    //     const firstShiftEnd = firstShift.endTime < firstShift.startTime ? firstShift.endTime + 1440 : firstShift.endTime;
    //     const secondShiftStart = secondShift.startTime;
    ...
    const handleSubmit = () => {
        if (shiftRoasterUserEdit.adjustmenttype === 'Add On Shift') {
            if (shiftRoasterUserEdit.selectedColumnDate === "") {
                ...
                // // If secondmode is "Double Shift", perform the time check
                ...
                //     if (!isSecondShiftValid(selectedColumnShiftTime, changeShiftTypeTime)) {
                ...
                //     if (!isContinuousShiftValid(selectedColumnShiftTime, changeShiftTypeTime)) {
    ...
    const getShiftForDateAdj = (column, matchingItem, matchingItemAllot, isWeekOffWithAdjustment, isWeekOffWithManual, boardingLog, isWeekOff, matchingDoubleShiftItem, department, matchingRemovedItem, matchingAssignShiftItem) => {
        ...
        //     return `${matchingItem.selectedShifTime.split(' - ')[0]}to${matchingItem.selectedShifTime.split(' - ')[1]}`;
        // }
        // else
    ...
    const onFilterChangedFinalAdj = () => {
        if (gridApi) {
            ...
            if (Object.keys(filterModel).length === 0) {
                ...
                setFilteredRowDataFinalAdj(filteredDataFinalAdj);
            }
        }
    };
    ...
    const columnDataTableFinalAdj = [
        ...
        ...daysArray.map((column, index) => {
            ...
            return {
                ...
                // hide: !columnVisibilityFinalAdj[fieldName],
                flex: 0,
                width: 180,
                filter: false,
                sortable: false,
                cellRenderer: (params) => {
                    let filteredRowDataFinalAdj = params.data.shiftallot?.filter((val) => val.empcode == params.data.empcode);
                    const matchingItem = filteredRowDataFinalAdj.find(item => item.adjdate == column.formattedDate);
                                ...
                                <Box sx={{ margin: '5px', }}>
                                    ...
                                    <Button
                                        ...
                                        sx={{
                                            ...
                                            pointerEvents: dayData?.adjstatus === 'Not Allotted' ? 'auto' : (dayData?.adjstatus === 'Approved' || dayData?.adjstatus === '' || matchingItem && matchingItem?.secondmode) ? 'none' : 'auto',
                                            '&:hover': {
                                                color: dayData?.adjstatus === "Week Off" ? '#892a23' : dayData?.adjstatus === 'Manual' ? '#052106' : dayData?.adjstatus === 'Adjustment' ? 'white' : dayData?.adjstatus === 'Approved' ? '#052106' : isWeekOff ? '#892a23' : dayData?.adjstatus === '' ? 'none' : 'white',
                                        ...
                                        onClick={(e) => {
                                            if (matchingItem && matchingItem?.adjstatus === 'Approved') {
                                                ...
                                                getCode(column, params.data, dayData?.shiftlabel, dayData?.adjstatus);
                                            }
                                        }}
                                    >
    ...
    const applyNormalFilterFinalAdj = (searchValue) => {
        ...
        setFilteredDataItemsFinalAdj(filtered);
        setPageFinalAdj(1);
    };
    ...
    const applyAdvancedFilterFinalAdj = (filters, logicOperator) => {
        ...
        setFilteredDataItemsFinalAdj(filtered); // Update filtered data
    ...
    const handleResetSearchFinalAdj = () => {
        ...
        setSearchQueryFinalAdj("");
        setFilteredDataItemsFinalAdj(allUsers);
    };
    ...
    const filteredDataFinalAdj = filteredDataItemsFinalAdj?.slice((pageFinalAdj - 1) * pageSizeFinalAdj, pageFinalAdj * pageSizeFinalAdj);
    const totalPagesFinalAdjOuter = Math.ceil(filteredDataItemsFinalAdj?.length / pageSizeFinalAdj);
    ...
    return (
        <Box>
                        ...
                        {allShiftAdj ?
                            ...
                            <>
                                <Box sx={{ width: "100%", }} className={"ag-theme-quartz"} ref={gridRefImageFinalAdj} >
                                    <AgGridReact
                                        rowData={filteredDataItemsFinalAdj}
            ...
            <Box>
                <ShiftAdjustmentListTable allUsersAdjTable={allUsersAdjTable} adjApply={adjApply} fetchUsers={fetchUsers} filteredDataItemsAdjList={filteredDataItemsAdjList} setFilteredDataItemsAdjList={setFilteredDataItemsAdjList} />
            </Box>
            <Box>
                <ShiftAdjustmentApprovedListTable allUsersAdjTableApproved={allUsersAdjTableApproved} adjApproved={adjApproved} fetchUsers={fetchUsers} filteredDataItemsAdjListApproved={filteredDataItemsAdjListApproved} setFilteredDataItemsAdjListApproved={setFilteredDataItemsAdjListApproved} />
            </Box>
            <Box>
                <ShiftFinalListTable allUsersShiftFinal={allUsersShiftFinal} allFinalAdj={allFinalAdj} daysArray={daysArray} allUserDates={allUserDates} overAllDepartment={overAllDepartment} valueDep={valueDep} />
            </Box>
