...
function TrainingUserCompletedStatus({ com }) {
            ...
            cellRenderer: (params) => (
                <Grid sx={{ display: "flex" }}>
                    {isUserRoleCompare?.includes("itrainingpostponeduserpanel") && (
                        <Link to={`/task/training/master/traininguserresponse/${params.data.id}`} style={{ textDecoration: "none", color: "#fff", minWidth: "0px" }}>
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "Training User-Completed List.png");
                })
                .catch((error) => {
    ...
    const ListPageLoadDataCompleted = async () => {
        try {
            ...
            const answer = res_task?.data?.trainingforuser?.length > 0 ? res_task?.data?.trainingforuser?.filter(data => data.username === isUserRoleAccess?.companyname && ["Completed", "Finished By Others", "Not Applicable to Me"]?.includes(data?.taskstatus))?.map((item, index) => ({
    ...
    return (
        <Box>
            ...
            <ExportData
                ...
                filename={"Training User - Completed List"}
