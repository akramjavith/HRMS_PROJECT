...
function EmployeeAssetDistribution() {
 ...
 const uniqueId = uuidv4();
    try {
      ...
      let freqCreate = await axios.post(SERVICE.EMPLOYEEASSET_CREATE, formData, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      ...
      setUpload([])
      await handleFileUpload(upload, "empdistribution", uniqueId);
      await fetchMaintentanceIndividual();
            ...
            const uploadNextChunk = async () => {
              try {
                if (end < selectedFile.size) {
                  ...
                  try {
                    const response = await axios.post(SERVICE.UPLOAD_CHUNK_EMPDISTRIBUTION, formData, {
                      headers: {
    ...
    const getMultipleFilesAsObjects = async (filenames, type, uniqueId) => {
      ...
      for (const name of filenames) {
        const res = await axios.post(
          SERVICE.EMPDISTRIBUTION_TODO_EDIT_FETCH,
  ...
  const getCodeNear = async (e) => {
    try {
      ...
      const filesbill = await getMultipleFilesAsObjects(res?.data?.semployeeasset?.images, "empdistribution", res?.data?.semployeeasset?.uniqueId);
      setoldfileNamesBill(res?.data?.semployeeasset?.images.map((d) => `${res?.data?.semployeeasset?.uniqueId}$empdistribution$${d}`));

      handleFetchBill(filesbill, res?.data?.semployeeasset?.images);
  ...
  const sendEditRequestNear = async () => {
    ...
    try {
      ...
      await handleFileUpload(uploadEdit, "empdistribution", maintentancemasteredit.uniqueId);
      await fetchMaintentanceIndividual();
    ...
    const handleFileDeleteOld = async (filenames) => {
      try {
        let res_project = await axios.post(SERVICE.EDIT_OLDDATA_DELETE_EMPDISTRIBUTION, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          filenames: filenames,
            ...
            <>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item lg={12} md={12} sm={12} xs={12}>
                  {upload?.length > 0 && upload?.map((data, index) => {
                    const fileURL = URL.createObjectURL(data.file); // Generate a URL for the file preview
