...
function ResumemailAttachments() {
    ...
    try {
      ...
      if (documentFiles.length > 0) {
        documentFiles.forEach((item, index) => {
          formData.append("resumedoc", item.file);
        });
      }
      if (files.length > 0) {
        files.forEach((item, index) => {
          formData.append("uploaddocfile", item.file); // Append image file
          formData.append(`remark[${index}]`, item.remark || ""); // Append corresponding remark
        });
      }
      ...
      let subprojectscreate = await axios.post(
        SERVICE.RESUMEMAILATTACHMENTS_CREATE, formData,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const getDownloadFile = async (files) => {
  ...
  const handleFileUploadTodo = (event) => {
    ...
    for (let i = 0; i < uploadedFiles.length; i++) {
      ...
      const isDuplicate = files.some((file) => file?.file?.name === uploadedFileName);
      if (isDuplicate) {
  ...
  const handleDownload = (fileState) => {
    if (!fileState) {
      alert("No file available for download!");
      return;
    }

    const fileURL = URL.createObjectURL(fileState); // Create a URL for the file
    const link = document.createElement("a");
    link.href = fileURL;
    link.download = fileState.name; // Set the download name
    document.body.appendChild(link);
                        ...
                        {files &&
                          files?.map((file, index) => (
                            <StyledTableRow key={index}>
                              ...
                              >
                                <a
                                  style={{ color: "#357ae8", cursor: "pointer", textDecoration: "underline" }}
