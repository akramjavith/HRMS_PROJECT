...
exports.getAllPenaltytotalfielduploadViewAllUploadFile = catchAsyncErrors(async (req, res, next) => {
    let penaltytotalfielduploads;
    try {
const {filename,projectvendor,queuename,date}=req.body
...
exports.getAllPenaltytotalfielduploadFilter = catchAsyncErrors(async (req, res, next) => {
    ...
    try {
        ...
        }, { projectvendor: 1, mode: 1, process: 1, queuename: 1, filename: 1,date:1, addedby: 1, loginid: 1, accuracy: 1, totalfields: 1, errorcount: 1, autocount: 1 }).lean();
...
exports.getAllValidationErrorFilter = catchAsyncErrors(async (req, res, next) => {
    let validatefinal, validatefinalData, penaltyerrorupload, bulkerrorupload, errorcontrol,peanltytotalfieldupload;
    try {

        const { projectvendor, process, loginid, fromdate, todate} = req.body;
        ...
        query.status = { $nin: ["Valid", "Invalid"] };
        querypenalty.status = { $nin: ["Valid", "Invalid"] };

        errorcontrol = await PenaltyErrorControl.find({}, { projectvendor: 1, process: 1, mode: 1}).lean()
        peanltytotalfieldupload = await Penaltytotalfieldupload.find({  loginid: { $in: loginid },date :{ $gte: fromdate, $lte: todate }}, { isedited: 1, iseditedtotal: 1, mode: 1,projectvendor:1,queuename:1,date:1, loginid:1 })
        ...
        penaltyerrorupload = await PenaltyErrorUpload.find(querypenalty, {
            ...
            line: 1, errorvalue: 1, correctvalue: 1, link: 1, doclink: 1, mode: 1,validatestatus:1,status:1,greentext:1
        })


        bulkerrorupload = await BulkErrorUpload.find(query, {
            editedcount: 1,date:1,
        ...
        // console.log(validatefinalData,"validatefinalData")

        const userids = [...new Set(validatefinalData.map(item => item.loginid))];
        let loginids = await ClientUserid.find({ loginallotlog: { $exists: true, $ne: [] },  userid: { $in: userids } });
        ...
        validatefinal = validatefinalData.map(upload => {
            const loginInfo = loginids.find((login) => login.userid === upload.loginid &&
                ...
                const penaltytotalfielduploadInfo = peanltytotalfieldupload.find(
                    error => 
                    error.projectvendor === upload.projectvendor &&
                error.queuename == upload.process &&
                error.loginid === upload.loginid &&
                error.date == (upload.mode === "Bulkupload" ? upload.dateformatted : upload.date )
            );
            ...
            return {
                ...
                manaulerrorcheck: penaltytotalfielduploadInfo && penaltytotalfielduploadInfo.isedited == "true" ? "YES": "NO",
                erroruploadconfirmcheck: penaltytotalfielduploadInfo && penaltytotalfielduploadInfo.iseditedtotal == "true" ? "YES": "NO",
        ...
        try {
            ...
            result = prodresult.map((item) => {
                ...
                return {
                    ...
                    iseditedtotal:item.iseditedtotal,
                ...
                resulted = result
                    .map((userObj) => {
                        ...
                        return {
                            ...
                            iseditedtotal: userObj.iseditedtotal,
...
//error upload confirm completed

exports.getAllErrorUploadHierarchyListCompleted = catchAsyncErrors(
    async (req, res, next) => {
        let resultall = [];
        let result, reportingtobaseduser, hierarchyfirstlevel,
            clientuserid,
...
exports.getAllValidationErrorFilterStatusCompleted = catchAsyncErrors(async (req, res, next) => {
    let validatefinal, validatefinalData, penaltyerrorupload, bulkerrorupload, errorcontrol,peanltytotalfieldupload;
    try {
        ...
        querypenalty.status = { $in: ["Valid", "Invalid"] };
...
// console.log(penaltytotalfielduploadInfo,"penaltytotalfielduploadInfo")
   ...
   //console.log(validatefinal[0], 'sks')
    } catch (err) {
        console.log(err, "validateerror")
        return next(new ErrorHandler("Records not found!", 404));
    }
...
//error upload confirm user list
exports.getAllErrorUploadConfirmUserList = catchAsyncErrors(
    async (req, res, next) => {
        let result, 
            clientuserid,
            resultAccessFilter,
        ...
        try {
            ...
            let clientidsmap;
                ...
                let usersss = await User.find().lean();
                const companyNames = usersss.map((user) => user.companyname);
                let clientids = await ClientUserid.find(
                    {
                        // projectvendor: { $in: vendorNames },
...
exports.getAllInvalidValidOverallReport = catchAsyncErrors(async (req, res, next) => {
    let validatefinal, validatefinalData, penaltyerrorupload, bulkerrorupload, errorcontrol,peanltytotalfieldupload;
    try {
        ...
        querypenalty.status =  "Invalid";

        query.movedstatus =  { $ne: true };
        querypenalty.movedstatus =  { $ne: true };
...
exports.getAllValidOverallReport = catchAsyncErrors(async (req, res, next) => {
    let validatefinal, validatefinalData, penaltyerrorupload, bulkerrorupload, errorcontrol,peanltytotalfieldupload;
    try {
