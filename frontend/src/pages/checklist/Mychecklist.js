...
function MyInterviewCheckList() {
  ...
  const formatData = (data) => {
    return data.map((item, index) => {
      return {
        Sno: index + 1,
        "Module Name":
          (filteredChangesTwo !== null ? item.module : item.modulename) || "",
  ...
  const handleCaptureImageTwo = () => {
    if (gridRefTableImgTwo.current) {
      domtoimage
        .toBlob(gridRefTableImgTwo.current)
        .then((blob) => {
  ...
  const [searchQueryManageAssigned, setSearchQueryManageAssigned] =
    useState("");
  // image
  const handleCaptureImageAssigned = () => {
    if (gridRefTableImgAssigned.current) {
      domtoimage
        .toBlob(gridRefTableImgAssigned.current)
        .then((blob) => {
          saveAs(blob, "My Checklist.png");
        })
        .catch((error) => {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex", gap: "20px" }}>
          ...
          <Button
            ...
            variant="contained"
            sx={{ backgroundColor: "#0696b3" }}
            onClick={() => {
              params?.data?.mainpage === "Apply Leave"
                ? window.open("/leave/applyleave", "_blank")
                : params?.data?.mainpage === "Apply Permission"
                  ? window.open("/permission/applypermission", "_blank")
                  : params?.data?.subsubpage === "Exit List"
                    ? window.open("/updatepages/exitlist", "_blank")
                    : params?.data?.subsubpage === "Long Absent Restriction List"
                      ? window.open("/employee/longabsentrestrictionlist", "_blank")
                      : params?.data?.mainpage === "Apply Work From Home"
                      ? window.open("/applyworkfromhome", "_blank")
  ...
  const updateIndividualData = async (index) => {
    ...
    try {
      ...
      if (searchItem) {
        ...
        setSingleDataId(assignbranches?.data?.result?._id);
        await fecthDBDatas(getDetails);
      }
  ...
  // Manage Columns
  const [isManageColumnsOpenAssigned, setManageColumnsOpenAssigned] =
    useState(false);
  ...
  const [WFHDatas, setWFHDatas] = useState([]);
  ...
  const showDataToTable = async (datas, leavedatas) => {
    ...
    try {
      ...
      if (
        ...
        datas.module == "Leave&Permission" &&
        datas.submodule == "Work From Home" &&
        datas.mainpage == "Apply Work From Home"
      ) {
        setFromWhere("applywfh");

        let mainFilter = overallGroupDatas?.filter(
          (data) =>
            data?.modulename === "Leave&Permission" &&
            data.submodule === "Work From Home" &&
            data.mainpage === "Apply Work From Home"
        );
        ...
        let toViewDatas = WFHDatas.map((data) => {
          let estimationdata = foundData?.relatedDatas
            ?.filter((item, index, array) => {
              let isMinutes = array.some(
                (item) =>
  ...
  const fetchGroupDatas = async (checklistAll, checkverfication) => {
    ...
    try {
      ...
      console.log(showValues?.updatedArray, "showValues?.updatedArray");
  ...
  const fetchUnassignedCandidates = async () => {
    ...
    try {
      ...
      const [
        ...
        axios.get(SERVICE.APPLIEDWORKFROMHOMELIST),
      ]);
      ...
      let appliedWFHOnly = appliedworkfromhomes?.data?.applyworkfromhomes
        ?.map((data) => {
      ...
      console.log(showValues?.result, "showValues?.result")
      let showDatas = showValues?.result
        ...
        .map((item) => {
          ...
          else if (item.mainpage === "Apply Work From Home") {
            return {
              ...item,
              countnew: appliedWFHOnly?.length,
            };
          }
        });
  ...
  const handleExportXLAssigned = (isfilter) => {
    const dataToExport =
      isfilter === "filtered"
        ? filteredChangesAssigned !== null
          ? filteredRowDataAssigned
          : filteredDataAssigned
        : itemsAssigned;
  ...
  const addSerialNumber = (datas) => {
    let notCompletedData = datas?.filter(
      (data) => data?.updatestatus != "Completed"
    );
    let isLongLeaveNotIncluded = datas?.filter((data) => {
      // return data?.updatestatus == "Completed"
      if (
        data?.updatestatus === "Completed" &&
        Array.isArray(data.longleaveabsentaprooveddatechecklist) && // Ensures it's an array
  ...
  const addSerialNumberAssigned = (datas) => {
    const itemsWithSerialNumberAssigned = datas
      ?.filter((data) => data.updatestatus == "Completed")
      .map((item, index) => {
        let timeCalc;
        let showTime;
        let dateTimeWithDuration;
        ...
        if (fromWhere == "applyleave") {
          ...
          if (
            item?.longleaveabsentaprooveddatechecklist?.includes(
  ...
  const totalPagesAssigned = Math.ceil(
    filteredDatasAssigned.length / pageSizeAssigned
  );
              ...
              <ListItemText
                ...
                primary={
                  <Switch
                    ...
                    onChange={() =>
                      toggleColumnVisibilityAssigned(column.field)
                    }
                  />
                }
                secondary={
        ...
        <Grid container>
          <Grid item md={4}>
            <Button
              ...
              sx={{ textTransform: "none" }}
              onClick={() =>
                setColumnVisibilityAssigned(initialColumnVisibilityAssigned)
              }
            >
              Show All
            </Button>
                    ...
                    >
                      ...
                      <Box sx={{ display: "flex", gap: "20px" }}>
                        ...
                        <Button
                          ...
                          sx={{ backgroundColor: "#0696b3" }}
                          onClick={() => {
                            pagesDetails.mainpage === "Apply Leave"
                              ? window.open("/leave/applyleave", "_blank")
                              ...
                              pagesDetails?.mainpage === "Apply Permission"
                                ...
                                : pagesDetails.subsubpage === "Exit List"
                                  ? window.open("/updatepages/exitlist", "_blank")
                                  : pagesDetails.subsubpage ===
                                    "Long Absent Restriction List"
                                    ? window.open(
                                      "/employee/longabsentrestrictionlist",
                                      "_blank"
                                    )
                                    : pagesDetails.mainpage ===
                                    ...
                                    : window.open(
                                      "/interview/myinterviewchecklist",
                                      "_blank"
                                    );
                          }}
                        ></Button>
                              ...
                              pagesDetails?.mainpage === "Apply Permission"
                                ...
                                : pagesDetails.subsubpage === "Exit List"
                                  ...
                                  : pagesDetails.subsubpage ===
                                    "Long Absent Restriction List"
                                    ? window.open(
                                      "/updatedpages/employeestatuscompleted",
                                      "_blank"
                                    )
                                    : window.open(
                                      "/interview/myinterviewchecklist",
                  ...
                  <Grid container spacing={2} style={userStyle.dataTablestyle}>
                    <Grid item md={2} xs={12} sm={12}>
                      <Box>
                        ...
                        >
                          ...
                          <MenuItem value={50}>50</MenuItem>
                          <MenuItem value={100}>100</MenuItem>
                          <MenuItem value={overallItemsAssigned?.length}>
                            All
                          </MenuItem>
                        </Select>
                      </Box>
                    ...
                    <Grid item md={2} xs={6} sm={6}>
                      <AggregatedSearchBar
                        ...
                        addSerialNumber={addSerialNumberAssigned}
                        setPage={setPageAssigned}
                        maindatas={candidates}
                        setSearchedString={setSearchedStringAssigned}
