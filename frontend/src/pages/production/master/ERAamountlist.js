...
function EraAmountList() {
    ...
    const fetchEmployee = async () => {
        ...
        try {
            ...
            if (!res_employee.data || !res_employee.data.result) {
                throw new Error("No data received");
            }

            const { result = [], totalProjects = 0, totalPages = 0 } = res_employee.data;
            ...
            setTotalProjects(totalProjects);
            setTotalPages(totalPages);
            setPageSize(result.length > 0 ? pageSize : 10);
            setPage(result.length > 0 ? page : 1);
