...
import {
  Box, Button, Checkbox, Dialog, DialogActions, Radio,
  RadioGroup, FormControlLabel, DialogContent, FormControl, Grid, IconButton, InputAdornment, List, ListItem, ListItemText, MenuItem, OutlinedInput, Popover, Select, TextareaAutosize, TextField, Tooltip, Typography
} from "@mui/material";
...
import { FaFileCsv, FaFileExcel, FaFilePdf, FaPrint, FaSearch, FaTrash } from "react-icons/fa";
...
function Advance() {
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      const jsonData = {
        ...
        requestyear: String(selectedYear),
        requestmonth: String(selectmonthname),
        requestdate: String(advance.requestdate),
        ...
        empcode: String(advance.access === "Self" ? isUserRoleAccess.empcode : employeeDetails.empcode),
        companyname: String(isUserRoleAccess.companyname),
        shifttiming: String(isUserRoleAccess.shifttiming),
        company: advance.access === "Self" ? isUserRoleAccess.company : employeeDetails.company,
        branch: advance.access === "Self" ? isUserRoleAccess.branch : employeeDetails.branch,
        unit: advance.access === "Self" ? isUserRoleAccess.unit : employeeDetails.unit,
        team: advance.access === "Self" ? isUserRoleAccess.team : employeeDetails.team,
        employeename: advance.access === "Self" ? String(isUserRoleAccess.companyname) : String(employeeDetails.companyname),
      ...
      let grpcreate = await axios.post(SERVICE.ADVANCE_CREATE, formData, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setAdvance(grpcreate.data);
      await fetchAllGroup();
      await fetchAllGroupArray();
      setAdvance({
        advanceamount: "",
  ...
  }, [advanceApprovalMonth, advance.employeename])
  ...
  const handleSubmit = (e) => {
    ...
    if (advance.access === "Please Select Access") {
      setPopupContentMalert("Please Select Access");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    }
    else if (advance.advanceamount === "") {
    ...
    else if (advance.requestdate === "") {
      setPopupContentMalert("Please Select Request Date");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    }
    else if (isNameMatch) {
      setPopupContentMalert(`Advance Amount already exists for ${selectmonthname} Month!`);
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    }

    else if (approvedAdvance.monthDiff !== null) {
      setPopupContentMalert(`Eligible for Advance After ${approvedAdvance.monthDiff} Month!`);
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    }
    else if (advance.access === "Others" && isUserRoleCompare.includes("lassignadvance") && selectedOptionsCompany.length === 0) {
      setPopupContentMalert("Please Select Company");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert()
    }
    else if (advance.access === "Others" && isUserRoleCompare.includes("lassignadvance") && selectedOptionsBranch.length === 0) {
      setPopupContentMalert("Please Select Branch");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert()
    }
    else if (advance.access === "Others" && isUserRoleCompare.includes("lassignadvance") && selectedOptionsUnit.length === 0) {
      setPopupContentMalert("Please Select Unit");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert()
    }
    else if (advance.access === "Others" && isUserRoleCompare.includes("lassignadvance") && selectedOptionsTeam.length === 0) {
      setPopupContentMalert("Please Select Team");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert()
    }
    else if (advance.access === "Others" && advance.employeename === "Please Select Employee Name") {
  ...
  const fetchAllGroup = async () => {
    ...
    try {
      let res_grp = await axios.post(SERVICE.ADVANCEBYASSIGNBRANCH_DEDUCTINNEXTMONTH, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setGroupCheck(true);
      setGroups(res_grp?.data?.advance.map((item, index) => ({
        ...item,
        serialNumber: (page - 1) * pageSize + index + 1
      })));
      setTotalProjects(res_grp?.data?.advance?.length > 0 ? res_grp?.data?.totalProjects : 0);
      setTotalPages(res_grp?.data?.advance?.length > 0 ? res_grp?.data?.totalPages : 0);
      setPageSize((data) => {
        return res_grp?.data?.advance?.length > 0 ? data : 10;
      });
      setPage((data) => {
        return res_grp?.data?.advance?.length > 0 ? data : 1;
      });
      console.timeEnd("fetchAllGroup")
  ...
  const fetchAllGroupArray = async () => {
    setPageName(!pageName)
    try {
      let res_grp = await axios.post(SERVICE.ADVANCEBYASSIGNBRANCH_DEDUCTINNEXTMONTH_EXPORTS, {
        assignbranch: accessbranch
      }, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
      ...
      console.log(res_grp?.data?.advance);
      setAdvanceArray(res_grp?.data?.advance);
    } catch (err) { handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  useEffect(() => {
    fetchAllGroupArray();
  ...
  const addSerialNumber = (datas) => {
    const itemsWithSerialNumber = datas?.map((item, index) => ({ ...item, }));
  ...
  const getDownloadFile = async (id) => {
    try {
      let response = await axios.get(`${SERVICE.ADVANCE_SINGLE}/${id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      if (response?.data?.sadvance?.document?.length !== 0) {

        response?.data?.sadvance?.document?.map((item) => {
          if (!item) return false;
          const imageUrl = `${SERVICE.VEIW_ADVANCE_FILES}/${item.filename}`;
  ...
  const getDownloadFileView = async (id) => {
    try {
      const imageUrl = `${SERVICE.VEIW_ADVANCE_FILES}/${id}`;
      window.open(imageUrl, '_blank'); // Open image in a new tab
    } catch (err) { handleApiError(err, setShowAlert, handleClickOpenerr); }
  };
  ...
  const columnDataTable = [
    // {
    //   field: "checkbox",
    //   headerName: "Checkbox", // Default header advanceamount
    //   headerStyle: {
    ...
    { field: "advanceamount", headerName: "Advance Amount", flex: 0, width: 130, hide: !columnVisibility.advanceamount, headerClassName: "bold-header", pinned: 'left', },
          ...
          <Box sx={userStyle.dialogbox}>
            <>
              <Grid container spacing={2} sx={{ display: "flex", justifyContent: "space-between" }}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>Add Advance</Typography>
                </Grid>
                {(isUserRoleCompare.includes("lassignadvance") || isUserRoleAccess?.role?.includes("Manager")) && (
                  <>
                    <Grid item md={3} sm={12} xs={12}>
                      <FormControl size="small" fullWidth>
                        ...
                        <Selects
                          ...
                          value={{ label: advance.access, value: advance.access }}
              ...
              <Grid container spacing={2}>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      onChange={(e) => {
                        handleYearChange(e)
                        setAdvance({ ...advance, requestdate: "" });
                      }
                      }
                    />
                  </FormControl>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      styles={colourStyles}
                      options={selectedYear === "Select Year" ? [] : selectedMont}
                ...
                <Grid item md={3} xs={12} sm={6}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      value={advance.requestdate}
                      type="date"
                      onChange={(e) => {
                        setAdvance({ ...advance, requestdate: e.target.value });
                      }}
                      id="datefrom"
                    />
                  </FormControl>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <TextareaAutosize
                      ...
                      onChange={(e) => {
                        setAdvance({ ...advance, description: e.target.value });
                      }}
                    />
                  </FormControl>
                </Grid>
                    ...
                    <Grid item md={3} xs={12} sm={6}>
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          ...
                          value={{ label: advance.employeename, value: advance.employeename }}
                          onChange={((e) => {
                    ...
                    documentFilesView.map((file, index) => (
                      <>
                        <Grid container spacing={2}>
                          ...
                          <Grid></Grid>
                          <Grid item md={1} sm={6} xs={6}>
                            <VisibilityOutlinedIcon style={{ fontsize: "large", color: "#357AE8", cursor: "pointer" }} onClick={() => getDownloadFileView(file?.filename)} />
                          </Grid>
                        </Grid>
                      </>
                    ))}
