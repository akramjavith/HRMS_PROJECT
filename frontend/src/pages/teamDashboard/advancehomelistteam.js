...
function AdvanceHomeList() {
  ...
  const { isUserRoleCompare, isUserRoleAccess, isAssignBranch, pageName, setPageName, buttonStyles,listPageAccessMode } = useContext(UserRoleAccessContext);
  ...
  const getCode = async (e, advanceamount) => {
    ...
    try {
      ...
      updateDateValueEdit(
        ...
        res?.data?.sadvance?.requestmonth === 'January'
          ? 1
          : res?.data?.sadvance?.requestmonth === 'February'
          ? 2
          : res?.data?.sadvance?.requestmonth === 'March'
          ? 3
          : res?.data?.sadvance?.requestmonth === 'April'
          ? 4
          : res?.data?.sadvance?.requestmonth === 'May'
          ? 5
          : res?.data?.sadvance?.requestmonth === 'June'
          ? 6
          : res?.data?.sadvance?.requestmonth === 'July'
          ? 7
          : res?.data?.sadvance?.requestmonth === 'August'
          ? 8
          : res?.data?.sadvance?.requestmonth === 'September'
          ? 9
          : res?.data?.sadvance?.requestmonth === 'October'
          ? 10
          : res?.data?.sadvance?.requestmonth === 'November'
          ? 11
          : res?.data?.sadvance?.requestmonth === 'December'
          ? 12
          : ''
      );
    } catch (err) {
  ...
  const editSubmit = (e) => {
    e.preventDefault();
    fetchGroupAll();
    const isNameMatch = allGroupEdit.some((item) => item?.requestmonth?.toLowerCase() === selectmonthnameEdit?.toLowerCase() && item?.companyname?.toLowerCase() === isUserRoleAccess?.companyname?.toLowerCase());
  ...
  const fetchAllGroup = async () => {
    ...
    try {
      ...
      let res_grp = await axios.post(SERVICE.ADVANCE_TEAM_LIST_TEAM, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      ...
      setGroupCheck(true);
      setGroups(
        res_grp?.data?.advance.map((t, index) => ({
          ...t,
  ...
  return (
    <Box>
      ...
      {isUserRoleCompare?.includes('aadvance') && (
        <>
          <PageHeading title="Advance (Deduct In Next Month Salary)" modulename="PayRoll" submodulename="Loan & Advance" mainpagename="Advance" subpagename="" subsubpagename="" />
        </>
      )}
      <Box>
        {/* Edit DIALOG */}
              ...
              <form onSubmit={editSubmit}>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item md={6} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      <Typography>
                        Request Year<b style={{ color: 'red' }}>*</b>
                      </Typography>
                      <Selects styles={colourStyles} options={years} value={{ label: selectedYearEdit, value: selectedYearEdit }} onChange={handleYearChangeEdit} />
                    </FormControl>
                  </Grid>
                  <Grid item md={6} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      <Typography>
                        Request Month<b style={{ color: 'red' }}>*</b>
                      </Typography>
                      <Selects styles={colourStyles} options={selectedYearEdit === 'Select Year' ? [] : months} value={{ label: selectmonthnameEdit, value: selectmonthnameEdit }} onChange={handleMonthChangeEdit} />
                    </FormControl>
                  </Grid>
                  <Grid item md={6} xs={12} sm={6}>
                    <FormControl fullWidth size="small">
          ...
          <Box sx={userStyle.container}>
            ...
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              <Grid item md={2} xs={6} sm={6}>
                <AggregatedSearchBar columnDataTable={columnDataTable} setItems={setItems} addSerialNumber={addSerialNumber} setPage={setPage} maindatas={groups} setSearchedString={setSearchedString} searchQuery={searchQuery} setSearchQuery={setSearchQuery} />
              </Grid>
            </Grid>
            <br />
      ...
      />
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Advance Info" addedby={addedby} updateby={updateby} />
