...
import { Box, Button, Checkbox, Dialog, DialogActions, DialogContent, FormControl, Grid, IconButton, List, ListItem, ListItemText, MenuItem, OutlinedInput, Popover, Select, TextField, Typography, RadioGroup, FormControlLabel, Radio, Tooltip, InputAdornment } from '@mui/material';
...
function Nonproductionunitallot() {
  ...
  const editSubmit = (e) => {
    ...
    const filtDup = nonproductionArray
      ...
      ?.some((item) => {
        return item?.category === assignedByAllotEdit.category && item?.subcategory === assignedByAllotEdit.subcategory;
      });
  ...
  const columnDataTable = [
    ...
    { field: 'employeename', headerName: 'Employee Name', flex: 0, width: 170, hide: !columnVisibility.employeename, headerClassName: 'bold-header' },
    {
      field: 'actions',
      headerName: 'Action',
      flex: 0,
        ...
        <Grid container>
          <Grid sx={{ display: 'flex' }}>
            {isUserRoleCompare?.includes('enon-productionunitallot') && (
              <Link to={`/production/nonproductionallot/${params.data.id}`} style={{ textDecoration: 'none', color: 'white', float: 'right' }}>
  ...
  const fetchEmployeeOverall = async () => {
    setPageName(!pageName);
    const queryParams = {
      company: nonproductionunitAllot.company,
      branch: nonproductionunitAllot.branch,
    };
    try {
      let res_employee = await axios.post(SERVICE.USER_NONPRODUCTIONALLOT_EXPORTS, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setAssignedbyOverall(
        res_employee?.data?.result?.map((item, index) => ({
          ...item,
  ...
  const fetchEmployee = async () => {
    ...
    try {
      let res_employee = await axios.post(SERVICE.USER_NONPRODUCTIONALLOT, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      ...
      setTotalProjects(res_employee?.data?.result?.length > 0 ? res_employee?.data?.totalProjects : 0);
      setTotalPages(res_employee?.data?.result?.length > 0 ? res_employee?.data?.totalPages : 0);
      setPageSize((data) => {
        return res_employee?.data?.result?.length > 0 ? data : 10;
      });
      setPage((data) => {
        return res_employee?.data?.result?.length > 0 ? data : 1;
      });
  ...
  const fetchEmployeeForSearch = async () => {
    setTaskcategorycheck(false);
    setPageName(!pageName);
  ...
  const handleResetSearch = async () => {
    ...
    try {
      let res_vendor = await axios.post(SERVICE.USER_NONPRODUCTIONALLOT, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Non Production Unit Allot" modulename="Production" submodulename="Non Production" mainpagename="Non-production Setup" subpagename="Non-production Unit Allot" subsubpagename="" />
      {/* ****** Header Content ****** */}
