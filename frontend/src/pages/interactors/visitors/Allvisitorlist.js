...
function AllListVisitors() {
  ...
  let exportColumnNames = ["Status", "Company", "Branch",
    "Unit", "Date", "Visitor's ID", "Visitor Name",
    "Visitor Type", "Visitor Mode", "Visitor Purpose",
    "Visitor Contact No", "IN Time", "OUT Time",
  ];
  let exportRowValues = ["addvisitorinstatus", "company",
    "branch", "unit", "date", "visitorid",
    "visitorname", "visitortype", "visitormode", "visitorpurpose",
    "visitorcontactnumber", "intime", "outtime"];
  ...
  const { isUserRoleCompare, pageName, buttonStyles, allTeam, allUsersData, setPageName, isAssignBranch, isUserRoleAccess } =
      ...
      cellRenderer: (params) => (
        <Grid>
          <div className="page-pdf" textAlign={'center'}>
            {params.data?.interactorstatus === "visitorenquiry" || params.data?.enquirystatusforinteractorstatus === true ?
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          ...
          >
            ...
            }} variant="contained" style={{ fontWeight: "50px" }} disabled={params.data.interactorstatus === "addresume"}>Linked Datas</Button>
          </Link>
          {/* } */}
          {isUserRoleCompare?.includes("eallvisitors") && (
            <Link
              to={`/interactor/master/editvisitors/${params.data.id}/allvisitor/?status=${params.data.interactorstatus}`}
          ...
          {isUserRoleCompare?.includes("vallvisitors") && (
            <Link
              to={`/interactor/master/viewvisitors/${params.data.id}/allvisitor/?status=${params.data.interactorstatus}`}
              style={{ textDecoration: "none", color: "#fff", minWidth: "0px" }}
            >
              <Button sx={userStyle.buttonedit} style={{ minWidth: "0px" }}>
          ...
          {isUserRoleCompare?.includes("vallvisitors") && (
            <Link
              to={`/interactor/master/followupvisitor/${params.data.id}/allvisitor/?status=${params.data.interactorstatus}`}
              style={{ textDecoration: "none", color: "#fff", minWidth: "0px" }}
            >
              <Button sx={userStyle.buttonedit} style={{ minWidth: "0px" }}>
  ...
  let rowDataTable = filteredDatas.map((item, index) => {
    return {
      ...
      intime: item?.intime,
      enquirystatusforinteractorstatus: item.enquirystatusforinteractorstatus,
      requestvisitorfollowupstatus: item.requestvisitorfollowupstatus,
      outtime: item?.outtime,
      interactorstatus: item?.interactorstatus,
      isBtnEnable: item?.isBtnEnable, // Default to `false` if no match is found          
      files: item.files,
      addvisitorin: item.addvisitorin,
      addvisitorinstatus: (item?.requestvisitorfollowupstatus || item.interactorstatus === "requestfollowupvisitor") ? "Request Followup Visitor" : item.interactorstatus === "visitorenquiry" ? "Visitor Enquiry" : item.addvisitorin ? "Visitor In" : "Visitor Out",
    };
  });
  ...
  const handleAutoSelect = async () => {
    ...
    try {
      ...
      setInteractorTypeArray(res_freqType?.data?.interactorType?.map((item) => ({
        label: item?.name,
        value: item?.name,
      })));
      setSelectedTypeEdit(res_freqType?.data?.interactorType?.map((item) => ({
        label: item?.name,
        value: item?.name,
      })));
      setSelectedTypeCat(res_freqType?.data?.interactorType?.map((a, index) => {
        return a.name;
      }));
      ...
      setInteractorModeArray(res_freqMode?.data?.interactormode?.map((item) => ({
        label: item?.name,
        value: item?.name,
      })));
      setSelectedModeEdit(res_freqMode?.data?.interactormode?.map((item) => ({
        label: item?.name,
        value: item?.name,
      })));
      setSelectedModeCat(res_freqMode?.data?.interactorType?.map((a, index) => {
        return a.name;
      }));
      ...
      let result = res?.data?.manageTypePG.filter(
        (d) => res_freqType?.data?.interactorType?.some((typ) =>
          d.interactorstype === typ?.name)
      );

      let ans = result.flatMap((data) => data.interactorspurpose);


      setInteractorPurposeArray(ans?.map((item) => ({
        label: item,
        value: item,
      })));
      setSelectedPurposeEdit(ans?.map((item) => ({
        label: item,
        value: item,
      })));
      setSelectedPurposeCat(ans?.map((a, index) => {
        return a;
      }));
    ...
    try {
      ...
      const jsonData = {
        ...
        meetingpersoncompany:
          vendor.meetingdetails === true ? [...vendor.meetingpersoncompany] : [],
        meetingpersonbranch:
          vendor.meetingdetails === true ? [...vendor.meetingpersonbranch] : [],
        meetingpersonunit:
          vendor.meetingdetails === true ? [...vendor.meetingpersonunit] : [],
        meetingpersondepartment:
          vendor.meetingdetails === true ? [...vendor.meetingpersondepartment] : [],
        meetingpersonteam:
          vendor.meetingdetails === true ? [...vendor.meetingpersonteam] : [],
        ...
        meetinglocationcompany:
          vendor.meetingdetails === true ? [...vendor.meetinglocationcompany] : [],
        meetinglocationbranch:
          vendor.meetingdetails === true ? [...vendor.meetinglocationbranch] : [],
        meetinglocationunit:
          vendor.meetingdetails === true ? [...vendor.meetinglocationunit] : [],
        meetinglocationfloor:
          vendor.meetingdetails === true ? [...vendor.meetinglocationfloor] : [],
  ...
  const [AllvisitorAllData, setAllvisitorAllData] = useState([])
  ...
  const fetchVendor = async () => {
    ...
    try {
      let res = await axios.post(SERVICE.SKIPPEDALL_VISITORS, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      ...
      setVendormaster(itemsWithSerialNumber.map((item, index) => {
        ...
        return {
          ...
          enquirystatusforinteractorstatus: item.enquirystatusforinteractorstatus,
          requestvisitorfollowupstatus: item.requestvisitorfollowupstatus,
          isBtnEnable: !mgTypePurpose?.find((itemmg) =>
            itemmg?.interactorstype?.toLowerCase() === item?.followuparray?.visitortype?.toLowerCase() &&
            itemmg?.interactorspurpose?.some((purpose) => purpose?.toLowerCase() === item?.followuparray?.visitorpurpose?.toLowerCase())
          )?.addcandidate || false, // Default to `false` if no match is found          
          files: item.files,
          addvisitorin: item.addvisitorin,
      ...
      setAllvisitorAllData(res?.data?.totalProjectsAllData?.map((item, index) => {
        const IsAvaile = allCandidate?.some(
          (data) =>
            data.firstname?.toLowerCase() + data.lastname?.toLowerCase() === String(item.visitorname?.replace(/\s+/g, '')?.toLowerCase())
        );
  ...
  const fetchVendorEmployee = async () => {
    ...
    try {
      let res = await axios.post(SERVICE.SKIPPED_VISITORS_EXPORTS_ALL, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "All Visitors List",
    pageStyle: "print",
  });
  ...
  const customValueRendererEditTypeFrom = (valueCate, _employeename) => {
    return valueCate.length
      ...
      : <span style={{ color: "hsl(0, 0%, 20%)" }}>Please Select Type</span>
  };
  ...
  const customValueRendererEditPurposeFrom = (valueCate, _employeename) => {
    return valueCate.length
      ? valueCate.map(({ label }) => label).join(", ")
      : <span style={{ color: "hsl(0, 0%, 20%)" }}>Please Select Purpose</span>
  };
  ...
  const fetchInteractorPurpose = async (e) => {
    ...
    try {
      ...
      let result = res?.data?.manageTypePG.filter(
        (d) => e.some((typ) => d.interactorstype === typ?.value)
      );
  ...
  return (
    <Box>
      <Headtitle title={"ALLLIST VISITORS"} />
      <PageHeading
        title="All Visitors"
            ...
            <Grid item md={3} xs={12} sm={12}>
              <FormControl fullWidth size="small">
                ...
                <MultiSelect
                  ...
                  options={interactorTypeArray}
                  value={selectedTypeEdit}
                  onChange={handleTypeEditChange}
                  valueRenderer={customValueRendererEditTypeFrom}
                />
              </FormControl>
            </Grid>
            <Grid item md={3} xs={12} sm={12}>
              <FormControl fullWidth size="small">
                ...
                <MultiSelect
                  ...
                  options={interactorModeArray}
                  value={selectedModeEdit}
                  onChange={handleModeEditChange}
                  valueRenderer={customValueRendererEditModeFrom}
                />
              </FormControl>
            </Grid>
            <Grid item md={3} xs={12} sm={12}>
              <FormControl fullWidth size="small">
                ...
                <MultiSelect
                  ...
                  options={interactorPurposeArray}
                  value={selectedPurposeEdit}
                  onChange={handlePurposeEditChange}
                  valueRenderer={customValueRendererEditPurposeFrom}
                />
              </FormControl>
            </Grid>
      ...
      <InfoPopup
        ...
        heading="All Visitors List Info"
        addedby={addedby}
        updateby={updateby}
      />
      <DeleteConfirmation
