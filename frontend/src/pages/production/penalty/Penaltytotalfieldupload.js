...
function PenaltyTotalFieldUpload() {
    ...
    const [selectedRowsPenaltytotalfield, setSelectedRowsPenaltytotalfield] = useState([]);
    ...
    const handleSelectionChange = (newSelection) => {
        setSelectedRowsPenaltytotalfield(newSelection.selectionModel);
    };
    ...
    const handleCloseModcheckbox = () => {
        ...
        setSelectedRowsPenaltytotalfield([]);
        setSelectAllChecked(false);
    };
    const [isDeleteOpenalert, setIsDeleteOpenalert] = useState(false);
    const handleClickOpenalert = async () => {
        setIsHandleChange(true);
        if (selectedRowsPenaltytotalfield.length === 0) {
            ...
            let resPayrun = await axios.post(`${SERVICE.PENALTYDAY_CREATED_BULK_DELETE}`, {
                ...
                dates: selectedRowsPenaltytotalfield.map((item) => ({ date: item.olddate })),
            });
            ...
            if (resPayrun.data.count > 0 && resPayrun.data.notIncluded.length == 0) {
                ...
                let selectedRowsfiltered = selectedRowsPenaltytotalfield.filter((item) => resPayrun.data.notIncluded.some((d) => item.olddate == d));
                setSelectedRowsPenaltytotalfield(selectedRowsfiltered);
                ...
                setIsDeleteOpencheckbox({ open: true, data: selectedRowsPenaltytotalfield });
            }
    ...
    const getRowClassName = (params) => {
        if (selectedRowsPenaltytotalfield.includes(params.row.id)) {
    ...
    const fetchTargetPointsData = async () => {
        ...
        try {
            ...
            setTargetPoints(
                Res?.data?.penaltytotalfielduploads
                    ?.filter((item) => item.accuracy !== 'NA')
                    .map((item, index) => ({
                        ...item,
    ...
    const exportColumnNames3 = ['Project Vendor', 'Date', 'Queue Name', 'Login ID', 'Accuracy', 'Total Fields', 'Error Count', 'Auto Count'];
    const exportRowValues3 = ['projectvendor', 'date', 'queuename', 'loginid', 'accuracy', 'totalfields', 'errorcount', 'autocount'];
    ...
    const rowDataTableviewAll = filteredDataviewAll.map((item, index) => {
        ...
        return {
            ...
            autocount: typeof item.autocount === 'number' ? parseFloat(item.autocount.toFixed(2)).toString() : item.autocount,
        };
    });
    ...
    const deleteSingleListView = async () => {
        ...
        try {
            ...
            setPage(1);
            await getviewCodeall(deletesingleDataView);
            await fetchTargetPointsData();
            setPopupContent('Deleted Successfully');
            setPopupSeverity('success');
            handleClickOpenPopup();
    ...
    const deleteSingleBulkdataDelete = async (data) => {
        ...
        setPageName(!pageName);
        try {
            const deletePromises = await axios.post(SERVICE.MULTIPLEPENALTYTOTALFIELDUPLOAD_SINGLE, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                ids: [...data],
    ...
    const sendEditRequestView = async () => {
        ...
        try {
            ...
            await getviewCodeall(editsingleData);
            await fetchTargetPointsData();
    ...
    // pdf.....
    const exportColumnNames2 = ['Project Vendor', 'Date', 'Queue Name', 'Login ID', 'Accuracy', 'Total Fields', 'Error Count', 'Auto Count'];
    const exportRowValues2 = ['projectvendor', 'date', 'queuename', 'loginid', 'accuracy', 'totalfields', 'errorcount', 'autocount'];
    // Excel
    ...
    const rowsWithCheckboxes = rowDataTable.map((row) => ({
        ...
        checkbox: selectedRowsPenaltytotalfield.includes(row.id),
    }));
    // Show All Columns functionality
    ...
    const getviewCodeall = async (data) => {
        setPageName(!pageName);
        try {
            setProductionfirstViewcheck(false);
            let res = await axios.post(SERVICE.PENALTYTOTALFIELDUPLOAD_VIEW_FILE, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            ...
            let getFilenames = res?.data?.penaltytotalfielduploads
    ...
    const exportColumnNames = ['Project Vendor', 'Queue Name', 'File Name', 'Created By', 'Created Date & Time'];
    const exportRowValues = ['projectvendor', 'queuename', 'filename', 'createdby', 'createddate'];
        ...
        if (!acceptedExtensions.includes(fileExtension) && file !== null) {
            ...
            promise.then((d) => {
                let uniqueArrayfinal = d.filter((item) => {
                    ...
                    return !targetPoints.some(
                        (tp) =>
                            ...
                            tp.date == penaltyTotalfieldUpload.date &&
                ...
                const dataArray = uniqueArrayfinal.map((item) => {
                    const totalFields = item['Queue Total Fields'];
                    const accuracy = !isNaN(item['Queue Accuracy']) ? (item['Queue Accuracy'] * 100).toFixed(2) : item['Queue Accuracy'];
                    // console.log(accuracy,totalFields,Number(totalFields * (1 - accuracy / 100))?.toFixed(0), 'acc');

                    let autocounttotal = Number(totalFields * (1 - accuracy / 100))?.toFixed(0);
                    console.log(autocounttotal,"autocounttotal")
                    return {
                        ...
                        accuracy: String(accuracy).includes("NA") ? "" : accuracy,
                        filename: file.name,
                        manualerror: 0,
                        totalfields: totalFields,
                        errorcount: item['Error Count'],
                        autocount: Number(autocounttotal), // Set the calculated autoCount
                        addedby: [
                            {
                                name: String(isUserRoleAccess.companyname),
                                date: String(new Date()),
    ...
    const sendJSON = async () => {
        ...
        if (selectedSheet === 'Please Select Sheet') {
            ...
            let uniqueArrayfinalDup = uploadExceldata.filter(
                (item, index, self) => index === self.findIndex((tp) => tp.projectvendor == item.projectvendor && tp.date == item.date && tp.queuename == item.queuename && tp.loginid == item.loginid && tp.accuracy == item.accuracy && tp.totalfields == item.totalfields && tp.errorcount == item.errorcount)
            );
            let uniqueArrayfinal = uniqueArrayfinalDup.filter(
                (item) => !targetPoints.some((tp) => tp.projectvendor == item.projectvendor && tp.date == item.date && tp.queuename == item.queuename && tp.loginid == item.loginid && tp.accuracy == item.accuracy && tp.totalfields == item.totalfields && tp.errorcount == item.errorcount)
            );
    ...
    const fetchFilterAllData = async () => {
        ...
        try {
            ...
            setTargetPoints(
                res?.data?.penaltytotalfielduploaddatefilter
                    .filter((item) => item.accuracy !== 'NA')
                    .map((item, index) => ({
                        ...item,
                        serialNumber: index + 1,
            ...
            //   return getFilenames.find((obj) => obj.filename === filename);
            // });
            console.log(getFilenames, "getFilenames")
            // const  uniqueArray = getFilenames.reduce((acc, current) => {
            //     const existingItem = acc.find((item) =>
            //       item.filename === current.filename &&
            //   item.projectvendor === current.projectvendor &&
            //   item.queuename === current.queuename &&
            //   item.date === current.date
            //   );

            //     if (existingItem) {
            //       existingItem.filename = current.filename;
            //       existingItem.projectvendor = current.projectvendor;
            //       existingItem.queuename = current.queuename;
            //       existingItem.date = current.date;
            //       existingItem.addedby = current.addedby;
            //       existingItem.fromdate = current.fromdate;
            //       existingItem.todate = current.todate;
            //     } else {
            //       acc.push({
            //         filename: current.filename,
            //         projectvendor: current.projectvendor,
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <PageHeading title="Penalty Total Field Upload" modulename="Quality" submodulename="Penalty" mainpagename="Penalty Setup" subpagename="Penalty Calculation" subsubpagename="Penalty Total Field Upload" />
                            ...
                            <Grid item md={4} xs={12} sm={6}>
                                <Grid container>
                                    <Grid item md={5} xs={12} sm={6}>
                                        <Typography>
                                            {' '}
                                            Queue Name<b style={{ color: 'red' }}>*</b>
                                        </Typography>
                                    </Grid>
                                    <Grid item md={7} xs={12} sm={6}>
                                        <FormControl fullWidth size="small">
                        ...
                        <Grid container spacing={2}>
                            <Grid item md={6} xs={12} sm={6}>
                                <Button
                                    variant="contained"
                                    color="success"
                                    disabled={penaltyTotalfieldUpload.loginid !== 'Please Select Login ID' || penaltyTotalfieldUpload.accuracy != '' || penaltyTotalfieldUpload.totalfields != '' || penaltyTotalfieldUpload.errorcount != ''}
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item md={4} xs={12} sm={6}>
                                <Grid container>
                                    <Grid item md={5} xs={12} sm={6}>
                                        <Typography>
                                            Total Fields<b style={{ color: 'red' }}>*</b>
                                        </Typography>
                                    </Grid>
                                    <Grid item md={7} xs={12} sm={6}>
                                        <FormControl fullWidth size="small">
                            ...
                            <Grid item md={4} xs={12} sm={6}>
                                <Grid container>
                                    <Grid item md={5} xs={12} sm={6}>
                                        <Typography>
                                            Error Count<b style={{ color: 'red' }}>*</b>
                                        </Typography>
                                    </Grid>
                                    <Grid item md={7} xs={12} sm={6}>
                                        <FormControl fullWidth size="small">
                            ...
                            <Box sx={userStyle.container}>
                                ...
                                {loaderSecond ? (
                                    ...
                                    <AggridTable
                                        ...
                                        selectedRowsPenaltytotalfield={selectedRowsPenaltytotalfield}
                                        setSelectedRowsPenaltytotalfield={setSelectedRowsPenaltytotalfield}
                                        pagenamecheck={'Penalty Total Field'}
                        ...
                        {!productionfirstViewCheck ? (
                            ...
                            <>
                                <AggridTable
                                    ...
                                    items={itemsviewAll}
                                    selectedRows={selectedRowsPenaltytotalfield}
                                    setSelectedRows={setSelectedRowsPenaltytotalfield}
            ...
            <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Penalty Total Field Upload Info" addedby={addedby} updateby={updateby} />
            <InfoPopup openInfo={openInfoFirst} handleCloseinfo={handleCloseinfoFirst} heading="Upload List Info" addedby={addedby} updateby={updateby} />
