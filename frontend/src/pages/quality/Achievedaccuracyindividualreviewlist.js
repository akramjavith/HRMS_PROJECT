...
function AchievedAccuracyIndividualReviewList() {
    ...
    const fetchAchievedAccuracyIndividual = async () => {
        ...
        try {
            ...
            // Transform achieved accuracy data
            let data_emp = res_employee?.data?.achievedaccuracyindividual.flatMap((data) =>
            ...
            // Map users by employeename for faster lookup
            let definedUsersMap = new Map(users.data.users.map(data => [
                data.companyname, {
                    company: data.company,
                    branch: data.branch,
                    unit: data.unit,
            ...
            // Filter allotted users and map to finalAllottedList
            let finalAllottedList = res.data.clientuserid
                .filter(data => data.allotted === "allotted")
                .map(data => {
                    let foundData = definedUsersMap.get(data.empname) || {};
                    return {
                        ...foundData,
                        date: data?.date,
                        loginid: data?.userid,
            ...
            // Combine data_emp with finalAllottedList to create allottedCombinedData
            let allottedCombinedData = finalAllottedList.map(data => {
                let foundData = data_emp.find(item =>
                    item.loginid === data.loginid &&
                    moment(item.date).format("YYYY-MM-DD") === moment(data.date).format("YYYY-MM-DD") &&
            ...
            // Filter unallotted data directly from data_emp
            let unAllottedCombinedData = data_emp.filter(data =>
                !allottedCombinedData.some(item => item.id === data.id)
            ).map(data => ({
                ...data,
                company: null,
                branch: null,
            ...
            // Combine allotted and unallotted lists, filter based on access branch
            let toShowList = [...allottedCombinedData, ...unAllottedCombinedData].filter(item => {
                if (item.company) {
                    return accessbranch.some(branch =>
                        branch.company === item.company &&
            ...
            // Create a map from minimum accuracy data for lookup
            let minimumaccuracyMap = new Map(minAcc.data.accuracymaster.map(item =>
                [`${item.projectvendor}_${item.queue}`, item.minimumaccuracy]
            ));

            // Map minimum accuracy to each item in toShowList
            let getShowList = toShowList.map(data => ({
                ...data,
                minimumaccuracy: minimumaccuracyMap.get(`${data.project}_${data.queue}`) || ""
            }));

            // Map expected accuracy statuses
            let expArray = expectedaccuracyDetails.data.expectedaccuracy;

            let finalList = getShowList.map(data => {
                let statuses = {};
                expArray.forEach(item => {
                    if (data.project === item.project && data.queue === item.queue &&
                        data.accuracy >= item.expectedaccuracyfrom && data.accuracy <= item.expectedaccuracyto) {
                        if (item.mode === "Client") {
                            statuses.clientstatus = `${item.statusmode} ${item.percentage} %`;
                        } else if (item.mode === "Internal") {
                            statuses.internalstatus = `${item.statusmode} ${item.percentage} %`;
                        }
                    }
                });
                return {
                    ...data,
                    clientstatus: statuses.clientstatus || "NIL",
                    internalstatus: statuses.internalstatus || "NIL"
                };
            });
    ...
    const fetchAchievedAccuracyIndividualArray = async () => {
        ...
        try {
            ...
            setEmployeesArray(finalList.map((item, index) => ({
                ...item,
    ...
    let exportColumnNames = ["Date", "Project", "Vendor", "Queue", "Minimum Accuracy", "Achieved Accuracy", "Client Status",
        "Internal Status", "Loginid", "Company", "Branch", "Unit", "Team", "Employeename", "Totalfield"];
    let exportRowValues = ["date", "project", "vendor", "queue", "minimumaccuracy", "accuracy", "clientstatus", "internalstatus", "loginid", "company", "branch", "unit", "team", "employeename", "totalfield"];
    // Excel
