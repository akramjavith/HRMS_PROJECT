...
import { Box, Button, Checkbox, Dialog, RadioGroup, Radio, FormControlLabel, DialogActions, Tooltip, InputAdornment, DialogContent, Divider, FormControl, Grid, IconButton, List, ListItem, ListItemText, MenuItem, OutlinedInput, Popover, Select, TextField, Typography } from '@mui/material';
...
import { FaDownload, FaFileCsv, FaFileExcel, FaFilePdf, FaSearch, FaPrint, FaTrash } from 'react-icons/fa';
...
function RevenueAmountList() {
  ...
  const fetchProcessQueue = async (pro) => {
    ...
    try {
      ...
      let data_set = res_freq?.data?.processqueuename.filter((data) => pro.includes(data.branch) && selectedCompanyFrom.some((comp) => comp.value === data.company));
  ...
  const fetchRevenueamountsDataArray = async () => {
    setPageName(!pageName);
    try {
  ...
  const handleResetSearch = async () => {
    ...
    try {
      let res_employee = await axios.post(SERVICE.REVENUEAMOUNTS_FILTER, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      const filteredData = res_employee?.data?.revenueamountFilters || [];

      const ans = res_employee?.data?.revenueamountFilters?.length > 0 ? res_employee?.data?.revenueamountFilters : [];
  ...
  const fetchEmployee = async () => {
    const queryParams = {
      ...
      assignbranch: accessbranch || [],
      company: selectedCompanyFrom?.length > 0 ? selectedCompanyFrom.map((item) => item.value) : [],
      branch: selectedBranchFrom?.length > 0 ? selectedBranchFrom.map((item) => item.value) : [],
      processcode: processnames,
      allFilters: additionalFilters.length > 0 || (selectedColumn && filterValue) ? [...additionalFilters, { column: selectedColumn, condition: selectedCondition, value: filterValue }] : [],
      logicOperator,
      searchQuery: searchQuery || '',
    };

    try {
      const res_employee = await axios.post(SERVICE.REVENUEAMOUNT_SORT, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });

      const ans = res_employee.data.result || [];
      const anstarget = res_employee.data.totalProjectsdata || [];
      ...
      const itemsWithSerialNumberTotal = anstarget.map((item, index) => ({
        ...item,
        id: item._id,
        serialNumber: (page - 1) * pageSize + index + 1,
        company: item.company,
      ...
      setTotalProjects(res_employee.data.totalProjects || 0);
      setTotalPages(res_employee.data.totalPages || 0);
      ...
      setPopupContentMalert('An error occurred while fetching records!');
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String('Revenue Amount List'),
  ...
  const deleteSingleList = async () => {
    ...
    try {
      ...
      setFilteredRowData([]);
      setFilteredChanges(null);
      await fetchRevenueamountsDataArray();
      // await fetchEmployee();
      await fetchFilter();
  ...
  const fetchFilter = async () => {
    ...
    try {
      const response = await axios.post(SERVICE.REVENUEAMOUNTS_FILTER, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      const filteredData = response?.data?.revenueamountFilters || [];
      ...
      setPageSize((data) => {
        return filteredData?.length > 0 ? data : 10;
      });
      setPage((data) => {
        return filteredData?.length > 0 ? data : 1;
      });
      setSearchQuery('');
      ...
      setLoaderFilter(false);
      console.error(err, 'Error fetching target points');
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: 'flex' }}>
          {isUserRoleCompare?.includes('erevenueamountlist') && (
            <Button
          ...
          {isUserRoleCompare?.includes('drevenueamountlist') && (
            <Button
          ...
          {isUserRoleCompare?.includes('vrevenueamountlist') && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
          ...
          {isUserRoleCompare?.includes('irevenueamountlist') && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Revenue Amount List" modulename="Production" submodulename="SetUp" mainpagename="Revenue Amount List" subpagename="" subsubpagename="" />
      <br />
      {isUserRoleCompare?.includes('arevenueamountlist') && (
        <Box sx={userStyle.selectcontainer}>
          <Grid container spacing={2}>
            <Grid item md={12} xs={12} sm={12}>
              <Typography sx={userStyle.HeaderText}>Revenue Amount Filter</Typography>
            </Grid>
          </Grid>
          <br />
          <Grid container spacing={2}>
            ...
            <Grid item md={3} xs={12} sm={12}>
              <FormControl fullWidth size="small">
                ...
                <MultiSelect
                  options={isAssignBranch
                    ?.filter((comp) => selectedCompanyFrom.map((data) => data.value).includes(comp.company))
                    ?.map((data) => ({
                      label: data.branch,
                      value: data.branch,
                    }))
            ...
            <Grid item md={3} xs={12} sm={12}>
              ...
              <FormControl fullWidth size="small">
                <MultiSelect options={ProcessOptions} styles={colourStyles} value={selectedProcessFrom} onChange={handleProcessChangeFrom} valueRenderer={customValueRendererProcessFrom} labelledBy="Please Select Process" />
              </FormControl>
            </Grid>
          </Grid>
          <br /> <br />
          <Grid container spacing={2}>
            <Grid item md={3} xs={12} sm={6}>
              <Button variant="contained" color="primary" sx={buttonStyles.buttonsubmit} onClick={handleSubmitFilter}>
                Filter
              </Button>
            </Grid>
      ...
      {loader ? (
        ...
        <>
          {isUserRoleCompare?.includes('lrevenueamountlist') && (
            <>
              <Box sx={userStyle.container}>
                {/* ******************************************************EXPORT Buttons****************************************************** */}
                <Grid item xs={8}>
                ...
                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                  ...
                  >
                    <Box>
                      {isUserRoleCompare?.includes('excelrevenueamountlist') && (
                        <>
                          <Button
                            onClick={(e) => {
                      ...
                      {isUserRoleCompare?.includes('csvrevenueamountlist') && (
                        <>
                          <Button
                            onClick={(e) => {
                              setIsFilterOpen2(true);
                      ...
                      {isUserRoleCompare?.includes('printrevenueamountlist') && (
                        <>
                      ...
                      {isUserRoleCompare?.includes('pdfrevenueamountlist') && (
                        <>
                          <Button
                            sx={userStyle.buttongrp}
                            onClick={() => {
                      ...
                      {isUserRoleCompare?.includes('imagerevenueamountlist') && (
                ...
                &ensp;
                {isUserRoleCompare?.includes('bdrevenueamountlist') && (
                  <Button sx={buttonStyles.buttonbulkdelete} size="small" onClick={handleClickOpenalert}>
                    Bulk Delete
                  </Button>
                )}
                <br />
      ...
      />
      {/* INFO */}
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Upload Data Info" addedby={addedby} updateby={updateby} />
      ...
      <DeleteConfirmation open={isDeleteOpencheckbox} onClose={handleCloseModcheckbox} onConfirm={deleteAllDataList} title="Are you sure?" confirmButtonText="Yes" cancelButtonText="Cancel" />
...
export default RevenueAmountList;
