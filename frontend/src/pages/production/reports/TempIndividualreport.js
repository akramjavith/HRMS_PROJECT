...
function ProductionTempReport() {
    ...
    const fetchProductionFilter = async () => {
        ...
        try {
            ...
            selectedProject.map(d => d.value).forEach(proj => {
                ...
                selectedVendor.map(d => d.value).forEach(vend => {
                    ...
                    if (vendorOpt.some(v => v.projectname === proj && v.name === vend)) {
                        ...
                        result1.push(`${proj}-${vend}`);
                    }
                });
            });
            let projvendor = [...new Set(result1)];
            ...
            let firstDate = new Date(overallState.fromdate);
            ...
            setSourcecheck(true)
            let res_vendor = await axios.post(SERVICE.PRODUCTION_TEMP_REPORT_FILTER, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                company: selectedOptionsCompany,
                branch: selectedOptionsBranch,
                unit: selectedOptionsUnit,
                team: selectedOptionsTeam,
                empname: selectedEmployeeFrom,
                ...
                filename: selectedOptionsCategory.length > 0 ? selectedOptionsCategory.map(item => item.value) : [],
                category: selectedOptionsSubCategory.length > 0 ? selectedOptionsSubCategory.map(item => item.value) : [],
            ...
            // Usage
            let uniqueArray = removeDuplicates(res_vendor.data.mergedData, ['user', 'unitid', 'dateval', 'category', 'filename',
                'flagcount']);
            ...
            //     return data.map(obj => {
            //         const key = `${obj.filename}-${obj.category}-${obj.dateval}-${obj.unitid}-${obj.user}-${obj.flagcount}}`;

            //         // const key = `${obj.filename}-${obj.category}-${obj.dateval}-${obj.unitid}-${obj.user}-${obj.flagcount}-${obj.section}`;
            //         if (seen[key]) {
            //             return { ...obj, dupe: "Yes" };
            //         } else {
            //             seen[key] = true;
            ...
            //     });
            // }
            // let markedData = markDuplicates(res_vendor.data.mergedData);

            let final = uniqueArray
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'Individual Production_Temp_Report.png');
                });
            });
        }
    };
    ...
    const fetchVendor = async (e) => {
        ...
        try {
            ...
            // setVendormasterOpt(res_vendor?.data?.vendormaster);
        } catch (err) { handleApiError(err, setShowAlert, handleClickOpenerr); }
    }
    ...
    const columns = [
        ...
        { title: "Work Took", field: "worktook" },
    ...
    const downloadPdf = () => {
        ...
        doc.save("Individual Production Temp Report.pdf");
    };


    // Excel
    const fileName = "Individual Production Temp Report";

    //print...
    const componentRef = useRef();
    const handleprint = useReactToPrint({
        content: () => componentRef.current,
        documentTitle: 'Individual Production Temp Report',
        pageStyle: 'print'
    });
    ...
    const columnDataTable = [
        ...
        { field: "mode", headerName: "Production Mode", flex: 0, width: 150, hide: !columnVisibility.mode, headerClassName: "bold-header" },
        ...
        { field: "category", headerName: "Sub Category", flex: 0, width: 250, hide: !columnVisibility.category, headerClassName: "bold-header" },
        { field: "unitid", headerName: "Unit Identifier", flex: 0, width: 100, hide: !columnVisibility.unitid, headerClassName: "bold-header" },
        { field: "unitrate", headerName: "Points", flex: 0, width: 100, hide: !columnVisibility.unitrate, headerClassName: "bold-header" },
        { field: "empcode", headerName: "Emp Code", flex: 0, width: 100, hide: !columnVisibility.empcode, headerClassName: "bold-header" },
        { field: "empname", headerName: "Emp Name", flex: 0, width: 200, hide: !columnVisibility.empname, headerClassName: "bold-header" },
        { field: "user", headerName: "LogIn Id", flex: 0, width: 140, hide: !columnVisibility.user, headerClassName: "bold-header" },
        { field: "dateval", headerName: "Date", flex: 0, width: 190, hide: !columnVisibility.dateval, headerClassName: "bold-header" },
        { field: "section", headerName: "Section", flex: 0, width: 100, hide: !columnVisibility.section, headerClassName: "bold-header" },
        ...
        { field: "worktook", headerName: "Work Took", flex: 0, width: 150, hide: !columnVisibility.worktook, headerClassName: "bold-header" },
        { field: "username", headerName: "User Name", flex: 0, width: 100, hide: !columnVisibility.username, headerClassName: "bold-header" },
        // {
    ...
    return (
        <Box>
            <Headtitle title={'Individual Production Temp Report'} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Individual Production Temp Report</Typography>
            {/* {isUserRoleCompare?.includes("aindividualproductiontempreport")
                && (
                    <> */}
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item md={3} xs={12} sm={12}>
                            <FormControl fullWidth >
                                ...
                                {/* <Selects
                                                maxMenuHeight={250}
                                                options={vendorOpt}
                                                value={{ label: overallState.vendor, value: overallState.vendor }}
                                                onChange={(e) => {
                                                    setOverallState({
                                                        ...overallState,
                                                        vendor: e.value,
                        ...
                        <Grid item md={1.5} xs={6} sm={2}>
                            ...
                            <Space wrap>
                                <TimePicker use12Hours format="h:mm:ss A" size="large"
                                    disabled={overallState.shift === "Shift Based"}
                                    value={overallState.totime} defaultValue={dayjs("00:00:00", "HH:mm:ss a")}
            ...
            <br />
            {/* ****** Table Start ****** */}
            {/* {isUserRoleCompare?.includes("lindividualproductiontempreport") && (
                <> */}
            <Box sx={userStyle.container}>
                { /* ******************************************************EXPORT Buttons****************************************************** */}
                <Grid item xs={8}>
                    <Typography sx={userStyle.importheadtext}>Individual Production Temp Report</Typography>
                </Grid>
                ...
                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                    ...
                    <Grid item md={8} xs={12} sm={12} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        <Box >
                            {/* {isUserRoleCompare?.includes("excelindividualproductiontempreport") && ( */}
                            <>
                                <Button
                                    onClick={(e) => {
                            ...
                            {/* )} */}
                            {/* {isUserRoleCompare?.includes("csvindividualproductiontempreport") && ( */}
                            <>
                                <Button
                                    onClick={(e) => {
                                        setIsFilterOpen(true);
                            ...
                            {/* // )} */}
                            {/* {isUserRoleCompare?.includes("printindividualproductiontempreport") && ( */}
                            <>
                                <Button sx={userStyle.buttongrp} onClick={handleprint}>&ensp;<FaPrint />&ensp;Print&ensp;</Button>
                            </>
                            {/* // )}
                                    // {isUserRoleCompare?.includes("pdfindividualproductiontempreport") && ( */}
                            <>
                                <Button
                                    sx={userStyle.buttongrp}
                                    onClick={() => {
                            ...
                            {/* )}
                                    {isUserRoleCompare?.includes("imageindividualproductiontempreport") && ( */}
            ...
            <ExportData
                ...
                itemsTwo={items ?? []}
                filename={"Individual Production Temp Report"}
...
export default ProductionTempReport;