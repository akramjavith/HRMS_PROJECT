...
function Loginedit() {
  ...
  let exportColumnNames = ["Company", "Branch", "Unit", "Team", "Work Mode", "Emp Code", "Employee Name", "Username"];
  let exportRowValues = ["company", "branch", "unit", "team", 'workmode', "empcode", "companyname", "username"];
  ...
  const getCode = async (e) => {
    ...
    try {
      ...
      setEnableLoginName(res?.data?.suser?.usernameautogenerate);
  ...
  const getUserByUsername = async (loginId, authToken) => {
    const users = allUsersData;
    const matchedUser = users.find((user) => user._id === loginId);
  ...
  //loginupdate updateby edit page...
  ...
  const sendRequestt = async () => {
    ...
    try {
      ...
      let res = await axios.put(`${SERVICE.USER_SINGLE}/${logedit}`, {
        ...
        password: String(empPass),
        originalpassword: String(empPass),
  ...
  const handleExportXL = (isfilter) => {
    ...
    exportToExcel(formatData(dataToExport), "LoginUpdatelist");
    setIsFilterOpen(false);
  };
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "LoginUpdatelist",
    pageStyle: "print",
  });

  useEffect(() => {
    const fetchUsername = async () => {
      const username = await getUserByUsername(loginid, authToken);
          ...
          {isUserRoleCompare.includes("Manager") ? (
            <>
              <Grid container spacing={2}>
                <Grid item>
                  {isUserRoleCompare?.includes("ilogininfoupdate") && (
                    <>
                      <Button
            ...
            <>
              <Grid sx={{ display: "flex" }}>
                {isUserRoleCompare?.includes("elogininfoupdate") && (
                  <Button
                    sx={userStyle.buttonedit}
                    onClick={() => {
                      setemppass("");
  ...
  return (
    <Box>
      ...
      <PageHeading
        ...
        subsubpagename="Login Info update"
      />
      <br />
      {isUserRoleCompare?.includes("llogininfoupdate") && (
        <>
        ...
        <>
          <Box sx={userStyle.container}>
            ...
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes("csvlogininfoupdate") && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes("excellogininfoupdate") && (
                    <>
                      <Button
                        onClick={(e) => {
                          setIsFilterOpen(true);
                  ...
                  {isUserRoleCompare?.includes("printlogininfoupdate") && (
                    <>
                  ...
                  {isUserRoleCompare?.includes("pdflogininfoupdate") && (
                    <>
                      <Button
                        sx={userStyle.buttongrp}
                        onClick={() => {
                  ...
                  {isUserRoleCompare?.includes("imagelogininfoupdate") && (
                    <Button
                      sx={userStyle.buttongrp}
                ...
                <Grid item md={6} sm={12} xs={12}>
                  ...
                  {!formError && (
                    <Typography variant="caption" color="grey">
                      <em>(Leave blank to keep current password)</em>
                    </Typography>
                  )}
                  {formError && (
                    <Typography
                      className="error-message"
                      color={formError ? "error" : ""}
                    >
                      {formError}
                    </Typography>
                  )}
      ...
      >
        <Box sx={{ width: "550px", padding: "20px 50px" }}>
          <>
            <Typography sx={userStyle.HeaderText}> Login Info</Typography>
            <br />
            <br />
            <Grid container spacing={2}>
