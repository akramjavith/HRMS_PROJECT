...
function OtherTaskConsolidatedReportSummary() {
    ...
    const fetchSubcategoryDropdowns = async (project, category) => {
        ...
        try {
            ...
            //     },
            //   });
            //   let result = res.data.subcategoryprod.filter((d) => e?.some((item) => d.categoryname === item.value));
    ...
    const fetchEmployee = async () => {
        ...
        try {
            let res_employee = await axios.post(SERVICE.OTHER_TASK_CONSOLIDATED_REPORT_SUMMARY, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            ...
            const itemsWithSerialNumber = ans?.map((item, index) => ({
                ...
                newrate: item.total ? Number(item.newrate) : 0,
            }));
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage
                ...
                .then((blob) => {
                    saveAs(blob, 'Other Task Consolidated Report Summary.png');
                })
                .catch((error) => {
    ...
    const handleCaptureImageView = () => {
        if (gridRefview.current) {
            html2canvas(gridRefview.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'View Other Task Consolidated Report Summary.png');
                });
            });
        }
    };
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: 'Other Task Consolidated Report Summary',
        pageStyle: 'print',
    });
    ...
    const handleprintFlag = useReactToPrint({
        ...
        documentTitle: 'View Other Task Consolidated Report Summary',
        pageStyle: 'print',
    });
    ...
    return (
        <Box>
            <Headtitle title={'Other Task Consolidated Report Summary'} />
            {/* ****** Header Content ****** */}
            {/* <Typography sx={userStyle.HeaderText}>Manage Other Task</Typography> */}
            <PageHeading title="Other Task Consolidated Report Summary" modulename="Other Tasks" submodulename="Other Task & Manual Entry Flag Report" mainpagename="" subpagename="" subsubpagename="" />

            <>
                {isUserRoleCompare?.includes('aothertaskconsolidatedreportsummary') && (
                    <Box sx={userStyle.selectcontainer}>
                        <>
                            <Grid container spacing={2}>
                                <Grid item xs={8}>
                                    <Typography sx={userStyle.importheadtext}>Other Task Consolidated Report Summary Filter</Typography>
                                </Grid>
                            </Grid>
                            <br />
                            <Grid container spacing={2}>
            ...
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes('lothertaskconsolidatedreportsummary') && (
                <>
                    <Box sx={userStyle.container}>
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Other Task Consolidated Report Summary</Typography>
                        </Grid>
                        <Grid container spacing={2} style={userStyle.dataTablestyle}>
                            ...
                            >
                                <Box>
                                    {isUserRoleCompare?.includes('excelothertaskconsolidatedreportsummary') && (
                                        <>
                                            <Button
                                                onClick={(e) => {
                                    ...
                                    {isUserRoleCompare?.includes('csvothertaskconsolidatedreportsummary') && (
                                        <>
                                            <Button
                                                onClick={(e) => {
                                                    setIsFilterOpen(true);
                                    ...
                                    {isUserRoleCompare?.includes('printothertaskconsolidatedreportsummary') && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes('pdfothertaskconsolidatedreportsummary') && (
                                        <>
                                            <Button
                                                sx={userStyle.buttongrp}
                                                onClick={() => {
                                    ...
                                    {isUserRoleCompare?.includes('imageothertaskconsolidatedreportsummary') && (
            ...
            <ExportData
                ...
                filename={'Other Task Consolidated Report Summary'}
...
export default OtherTaskConsolidatedReportSummary;
