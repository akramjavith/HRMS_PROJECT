...
function Deactivateemployeeslist() {
  let exportColumnNames = ['File Name', 'Created Date & Time'];
  let exportRowValues = ['filename', 'date'];
  ...
  const { isUserRoleAccess, isUserRoleCompare, isAssignBranch, pageName, setPageName, buttonStyles, allUsersData, allTeam, allUsersLimit } = useContext(UserRoleAccessContext);
  ...
  const isChecklistAssigned = async () => {
    try {
      ...
      const isAvailable = res?.data?.checklistverificationmasters?.some((item) => item.subsubpage === 'Deactivate Employees List');
      if (isAvailable) {
        setIsAssigned(true);
      } else {
        setIsAssigned(false);
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String('Deactivate Employee List'),
  ...
  const handleAutoSelect = async () => {
    ...
    try {
      const pipeline = [
        {
          $match: {
            // Check if resonablestatus is one of the specified values
            resonablestatus: {
              $in: ['Releave Employee', 'Absconded', 'Hold', 'Terminate'],
            },
            workmode: { $ne: 'Internship' },
            // Ensure successfullyrejoined is not true
            $or: [
              { successfullyrejoined: { $exists: false } }, // Field does not exist
              { successfullyrejoined: { $eq: '' } }, // Field exists but is ""
            ],
          },
        },
        {
      ...
      let employees = filteredDatas?.filter((u) => selectedCompany?.includes(u.company) && selectedBranch?.includes(u.branch) && selectedUnit?.includes(u.unit) && selectedTeam?.includes(u.team) && u?.workmode !== 'Internship').map((u) => u.companyname);
  ...
  }, [notEmployees, valueCompanyCat, valueBranchCat, valueUnitCat, valueTeamCat]);

  const updateEmployees = (pastedNames) => {
    // Your existing update logic...
    const namesArray = Array.isArray(pastedNames) ? pastedNames : [];

    const availableOptions = notEmployees
    ...
    try {
      ...
      let res = await axios.post(
        `${SERVICE.MYCHECKLISTVIEW}`,
        {
          ...
          subsubpage: 'Deactivate Employees List',
  ...
  const deleteChecklist = async () => {
    setDeleteBtn(true);
    const searchItem = datasAvailedDB.find((item) => item.commonid === postID && item.module === 'Human Resources' && item.submodule === 'HR' && item.mainpage === 'Employee' && item.subpage === 'Employee Action Details' && item.subsubpage === 'Deactivate Employees List');
    setPageName(!pageName);
    try {
  ...
  const updateIndividualData = async (index) => {
    ...
    let searchItem = datasAvailedDB.find((item) => item.commonid == postID && item.module == 'Human Resources' && item.submodule == 'HR' && item.mainpage == 'Employee' && item.subpage == 'Employee Action Details' && item.subsubpage == 'Deactivate Employees List');
  ...
  async function fecthDBDatas() {
    ...
    try {
      ...
      let foundData = res?.data?.mychecklist.find((item) => item.commonid == postID && item.subsubpage == 'Deactivate Employees List');
  ...
  const verifySubmit = async (from) => {
    if (!verifyDetails.firstname) {
      setPopupContentMalert('Please Enter First Name!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!verifyDetails.lastname) {
      setPopupContentMalert('Please Enter Last Name!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (verifyDetails.lastname.length < 3) {
      setPopupContentMalert('Last Name must be 3 atleast characters!!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!verifyDetails.legalname) {
      setPopupContentMalert('Please Enter Legal Name!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!verifyDetails.email) {
      setPopupContentMalert('Please Enter Email!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (verifyDetails.email && !validateEmail(verifyDetails.email)) {
      setPopupContentMalert('Please Enter Valid Email!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!verifyDetails.contactpersonal) {
      setPopupContentMalert('Please Enter Contact No (Personal)!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (verifyDetails.contactpersonal.length !== 10) {
      setPopupContentMalert('Please Enter Valid Contact No (Personal)!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!verifyDetails.contactfamily) {
      setPopupContentMalert('Please Enter Contact No (Family)!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (verifyDetails.contactfamily.length !== 10) {
      setPopupContentMalert('Please Enter Valid Contact No (Family)!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!verifyDetails.emergencyno) {
      setPopupContentMalert('Please Enter Emergency No!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (verifyDetails.emergencyno.length !== 10) {
      setPopupContentMalert('Please Enter Valid Emergency No!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!verifyDetails?.panstatus) {
      setPopupContentMalert('Please Select PAN Card Status!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!verifyDetails?.panno && verifyDetails?.panstatus === 'Have PAN') {
      setPopupContentMalert('Please Enter PAN No!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!PanValidate(verifyDetails?.panno) && verifyDetails?.panstatus === 'Have PAN') {
      setPopupContentMalert('Please Enter Valid PAN No!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!verifyDetails?.panrefno && verifyDetails?.panstatus === 'Applied') {
      setPopupContentMalert('Please Enter PAN Application Reference No!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    }
    else if (
      !verifyDetails?.paddresstype ||
      !verifyDetails?.ppersonalprefix ||
      !verifyDetails?.presourcename ||
      !verifyDetails?.plandmarkandpositionalprefix ||
      !verifyDetails?.pgpscoordination ||
      !verifyDetails?.pdoorno ||
      !verifyDetails?.pstreet ||
      !verifyDetails?.parea ||
      !verifyDetails?.plandmark ||
      !verifyDetails?.ptaluk ||
      !verifyDetails?.ppost ||
      !verifyDetails?.ppincode ||
      !selectedCountryp?.name ||
      !selectedStatep?.name ||
      !selectedCityp?.name
    ) {
      setPopupContentMalert("Please Fill All Permanent Address Details!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (
      !verifyDetails?.samesprmnt &&
      (
        !verifyDetails?.caddresstype ||
        !verifyDetails?.cpersonalprefix ||
        !verifyDetails?.cresourcename ||
        !verifyDetails?.clandmarkandpositionalprefix ||
        !verifyDetails?.cgpscoordination ||
        !verifyDetails?.caddresstype ||
        !verifyDetails?.cpersonalprefix ||
        !verifyDetails?.cresourcename ||
        !verifyDetails?.clandmarkandpositionalprefix ||
        !verifyDetails?.cgpscoordination ||
        !verifyDetails?.cdoorno ||
        !verifyDetails?.cstreet ||
        !verifyDetails?.carea ||
        !verifyDetails?.clandmark ||
        !verifyDetails?.ctaluk ||
        !verifyDetails?.cpost ||
        !verifyDetails?.cpincode ||
        !selectedCountryc?.name ||
        !selectedStatec?.name ||
        !selectedCityc?.name)
    ) {
      setPopupContentMalert("Please Fill All Current Address Details!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    }
    else {
      if (from === 'verify') {
        setVerifyDetails((prev) => ({ ...prev, verified: true }));
      } else {
        let previousRejoinedDetails = getDetails.rejoineddetails ? getDetails.rejoineddetails : [];
        navigate(`/rejoinemployeecreate/${bor}`, {
          state: {
            rejoineddetails: [
              ...previousRejoinedDetails,
              {
                dateofrejoining: formattedDate,
                reasonforrejoining: reason.reasonname,
              },
            ],
            verifiedDetails: {
              ...verifyDetails,
  ...
  const handleCloseModVerify = (e, reason) => {
    if (reason && reason === 'backdropClick') return;
  ...
  const handlechangecpincode = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === "") {
      ...
      setVerifyDetails({ ...verifyDetails, cpincode: inputValue });
    }
  };
  const handlechangeppincode = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === "") {
      // Update the state with the valid numeric value
      setVerifyDetails({ ...verifyDetails, ppincode: inputValue });
    }
  };
  const sendRequestReason = async () => {
    ...
    const updateProjectStatus = async () => {
      let projectData = {
        ...
        lastworkday: '',
        // doj: formattedDate,
        // rejoineddetails: [...previousRejoinedDetails]
      };
    ...
    try {
      ...
      if (userDetails !== 'Un-Hold' && empaddform?.workmode !== 'Internship') {
        //rejoin verifyDetails create
        // window.open(`/rejoinemployeecreate/${bor}`);
        ...
        const country = Country.getAllCountries().find((country) => country.name === empaddform.ccountry);
        const state = State.getStatesOfCountry(country?.isoCode).find((state) => state.name === empaddform.cstate);
        const city = City.getCitiesOfState(state?.countryCode, state?.isoCode).find((city) => city.name === empaddform.ccity);

        // Find the corresponding Country, State, and City objects
        const countryp = Country.getAllCountries().find((country) => country.name === empaddform.pcountry);
        const statep = State.getStatesOfCountry(country?.isoCode).find((state) => state.name === empaddform.pstate);
        const cityp = City.getCitiesOfState(state?.countryCode, state?.isoCode).find((city) => city.name === empaddform.pcity);
  ...
  const getCode = async (e, name, details) => {
    ...
    let month = ans && ans[1];
    let year = ans && ans[2];
    let finaldate = day && month && year ? `${year}-${month}-${day}` : details?.doj;
    ...
    try {
      ...
      let searchItem = res1?.data?.mychecklist.find((item) => item.commonid == details?.id && item.module == 'Human Resources' && item.submodule == 'HR' && item.mainpage == 'Employee' && item.subpage == 'Employee Action Details' && item.subsubpage == 'Deactivate Employees List');
      ...
      setEmpaddform({
        ...res?.data?.suser,
        paddresstype: res?.data?.suser?.paddresstype ? res?.data?.suser?.paddresstype : "",
        ppersonalprefix: res?.data?.suser?.ppersonalprefix ? res?.data?.suser?.ppersonalprefix : "",
        presourcename: res?.data?.suser?.presourcename ? res?.data?.suser?.presourcename : "",
        plandmarkandpositionalprefix: res?.data?.suser?.plandmarkandpositionalprefix ? res?.data?.suser?.plandmarkandpositionalprefix : "",
        pgpscoordination: res?.data?.suser?.pgpscoordination ? res?.data?.suser?.pgpscoordination : "",
        caddresstype: res?.data?.suser?.caddresstype ? res?.data?.suser?.caddresstype : "",
        cpersonalprefix: res?.data?.suser?.cpersonalprefix ? res?.data?.suser?.cpersonalprefix : "",
        cresourcename: res?.data?.suser?.cresourcename ? res?.data?.suser?.cresourcename : "",
        clandmarkandpositionalprefix: res?.data?.suser?.clandmarkandpositionalprefix ? res?.data?.suser?.clandmarkandpositionalprefix : "",
        cgpscoordination: res?.data?.suser?.cgpscoordination ? res?.data?.suser?.cgpscoordination : "",
      });
      setLoading(false);
    } catch (err) {
      console.log(err);
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, 'Deactivate Employee List.png');
        })
        .catch((error) => {
  ...
  const exportToExcel = async (csvData, fileName) => {
    ...
    try {
      ...
      worksheet.columns = [
        ...
        { header: 'Last Work Day', key: 'lastworkday', width: 20 },
  ...
  const handleExportXL = (isfilter) => {
    ...
    exportToExcel(dataToExport, 'Deactivate Employee List');
    setIsFilterOpen(false);
  };
  ...
  const downloadPdf = async (isfilter) => {
    ...
    doc.save('Deactivate Employee List.pdf');
  };
    ...
    const itemsWithSerialNumber = datas?.map((item, index) => {
      function getTimeDifference(clockIn, clockOut) {
        const formatTime = (timeStr) => {
          // Convert to 24-hour format using Date object
          return new Date(`01/01/2000 ${timeStr}`);
        };
      ...
      for (let i = count; i > 0; i--) {
        ...
        let timeDiff = clockOUT && getTimeDifference(clockIN, clockOUT);
        if (clockOUT && timeDiff >= 5) {
      ...
      return {
        ...
        reasondate: item?.reasondate ? moment(item.reasondate, 'YYYY-MM-DD').format('DD-MM-YYYY') : '',
        experience: calculateExperience(item.doj),
        lastworkday: finaldate,
      };
    });
  ...
  const rowDataTable = filteredData.map((item) => {
    return {
      ...
      empcode: item.empcode,
      // successfullyrejoined: item?.successfullyrejoined,
      companyname: item.companyname,
      department: item.department,
      dob: item.dob,
      contactpersonal: item.contactpersonal,
  ...
  const columnDataTable = [
    ...
    },
    {
      ...
      headerClassName: 'bold-header',
      pinned: 'left',
      cellRenderer: (params) => renderStatus(params?.data?.resonablestatus),
    },
    {
      field: 'empcode',
      headerName: 'Emp Code',
    ...
    },
    {
      ...
      flex: 0,
      width: 200,
      hide: !columnVisibility.reasonname,
      headerClassName: 'bold-header',
    },
    {
      field: 'lastworkday',
      ...
      cellRenderer: (params) => (
        <>
          <Grid sx={{ display: 'flex', gap: '10px' }}>
            <ButtonGroup size="small" variant="contained" aria-label="Small button group" padding="0" sx={{ height: '20px', marginTop: '10px' }}>
              <Button
                sx={{ fontSize: '0.6rem', backgroundColor: '#0fa1ff' }}
                onClick={(e) => {
                  setLoading(true);

                  getCode(params?.data?.id, 'Re-Join', params?.data);
                }}
              >
                Re-Join
              </Button>
              <Button
                ...
                onClick={(e) => {
                  ...
                  getCode(params?.data?.id, 'Un-Hold', params?.data);
                }}
              >
                Un-Hold
              </Button>
              <Button
                ...
                onClick={(e) => {
                  ...
                  getCode(params?.data?.id, 'Update Join', params?.data);
                }}
              >
                Update Join
              </Button>
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <Headtitle title={'DEACTIVATE EMPLOYEE LIST'} />
      <PageHeading title="Deactivate Employees List" modulename="Human Resources" submodulename="HR" mainpagename="Employee" subpagename="Employee Action Details" subsubpagename="Deactivate Employees List" />
            ...
            {!isBoarding ? (
              ...
              <>
                ...
                <Grid container sx={{ justifyContent: 'center' }}>
                  <Grid>
                    {isUserRoleCompare?.includes('csvdeactivateemployeeslist') && (
                      <>
                        <Button
                          onClick={(e) => {
                    ...
                    {isUserRoleCompare?.includes('exceldeactivateemployeeslist') && (
                      <>
                        <Button
                          onClick={(e) => {
                            setIsFilterOpen(true);
                    ...
                    {isUserRoleCompare?.includes('printdeactivateemployeeslist') && (
                      <>
                    ...
                    {isUserRoleCompare?.includes('pdfdeactivateemployeeslist') && (
                      <>
                        <Button
                          sx={userStyle.buttongrp}
                          onClick={() => {
                    ...
                    {isUserRoleCompare?.includes('imagedeactivateemployeeslist') && (
            ...
            {filteredData &&
              filteredData.map((row, index) => (
                <StyledTableRow key={index}>
                  ...
                  <StyledTableCell>{row.reasondate ? moment(row.reasondate, 'YYYY-MM-DD').format('DD-MM-YYYY') : ''}</StyledTableCell>
                  <StyledTableCell>{row.reportingto}</StyledTableCell>
                  <StyledTableCell>{row.reasonname}</StyledTableCell>
                  <StyledTableCell>{row.lastworkday ? moment(row.lastworkday, 'YYYY-MM-DD').format('DD-MM-YYYY') : ''}</StyledTableCell>
                  {/* <StyledTableCell>
                    {row?.profileimage ? (
                      <img
      ...
      <Dialog open={openviewReleave} onClose={handleClickOpenviewReleave} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="xl" sx={{ marginTop: '85px' }}>
        <Box sx={{ padding: '20px 50px' }}>
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <Typography sx={userStyle.HeaderText}> Deactivate verifyDetails list</Typography>
                  ...
                  <Table sx={{ minWidth: 650 }} aria-label="simple table">
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell align="left" sx={{ fontWeight: '500', fontSize: '1.2rem' }}>
                          Reason For Relieved
                        </TableCell>
                        <TableCell align="left" sx={{ fontWeight: '500', fontSize: '1.2rem' }}>
                          Date of Joining
                    ...
                    <TableBody>
                      {groupDetails?.map((row, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>{row?.verifyDetails && row?.verifyDetails?.map((data, index) => <Typography key={index} variant="body1">{`${index + 1}.${data}, `}</Typography>)}</TableCell>
      ...
      <CheckListTable isTable={isTable} handleCloseTable={handleCloseTable} userId={selectedUserId} from={'deactivate'} />
      ...
      >
        <Box sx={{ padding: '20px 50px' }}>
          <>
            <Grid container spacing={2}>
              <Typography sx={userStyle.HeaderText}>Verify Employee Details</Typography>
            </Grid>
            <br />
            <Grid container spacing={2}>
              ...
              <Grid item md={4} sm={6} xs={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput
                    ...
                    value={verifyDetails.contactpersonal}
                    onChange={(e) => {
                      const regex = /^[0-9]+$/; // Only allows positive integers
              ...
              <Grid item md={4} sm={6} xs={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput
                    ...
                    value={verifyDetails.contactfamily}
                    onChange={(e) => {
                      const regex = /^[0-9]+$/; // Only allows positive integers
                      const inputValue = e.target.value?.slice(0, 10);
                      if (regex.test(inputValue) || inputValue === '') {
              ...
              <Grid item md={4} sm={6} xs={12}>
                <FormControl fullWidth size="small">
                  ...
                  <OutlinedInput
                    ...
                    value={verifyDetails.emergencyno}
                    onChange={(e) => {
                      const regex = /^[0-9]+$/; // Only allows positive integers
                      const inputValue = e.target.value?.slice(0, 10);
                      if (regex.test(inputValue) || inputValue === '') {
              ...
              <Grid item md={4} sm={12} xs={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Selects
                    ...
                    value={{
                      label: verifyDetails.panstatus === '' || verifyDetails.panstatus == undefined ? 'Select PAN Status' : verifyDetails.panstatus,
                      value: verifyDetails.panstatus === '' || verifyDetails.panstatus == undefined ? 'Select PAN Status' : verifyDetails.panstatus,
                    }}
                    onChange={(e) => {
                      setVerifyDetails((prev) => ({
                        ...prev,
              ...
              {verifyDetails?.panstatus === 'Have PAN' && (
                <Grid item md={4} sm={6} xs={12}>
                  <FormControl fullWidth size="small">
                    <Typography>
              ...
              {verifyDetails?.panstatus === 'Applied' && (
                <Grid item md={4} sm={6} xs={12}>
                  <FormControl fullWidth size="small">
                    <Typography>
                      Application Ref No
                    ...
                    <Grid item md={3} sm={12} xs={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          ...
                          styles={colourStyles}
                          value={{
                            label: verifyDetails?.paddresstype === '' ? 'Please Select Address Type' : verifyDetails?.paddresstype,
                            value: verifyDetails?.paddresstype === '' ? 'Please Select Address Type' : verifyDetails?.paddresstype,
                          }}
                          onChange={(e) => {
                            setVerifyDetails({ ...verifyDetails, paddresstype: e.value });
                          }}
                        />
                      </FormControl>
                    </Grid>
                    <Grid item md={3} sm={12} xs={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          ...
                          styles={colourStyles}
                          value={{
                            label: verifyDetails?.ppersonalprefix === '' ? 'Please Select Personal Prefix' : verifyDetails?.ppersonalprefix,
                            value: verifyDetails?.ppersonalprefix === '' ? 'Please Select Personal Prefix' : verifyDetails?.ppersonalprefix,
                          }}
                          onChange={(e) => {
                            setVerifyDetails({ ...verifyDetails, ppersonalprefix: e.value });
                          }}
                        />
                      </FormControl>
                    </Grid>
                    <Grid item md={3} sm={12} xs={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          value={verifyDetails?.presourcename}
                          onChange={(e) => {
                            setVerifyDetails({ ...verifyDetails, presourcename: e.target.value });
                          }}
                        />
                      </FormControl>
                    </Grid>
                    <Grid item md={3} sm={12} xs={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          ...
                          styles={colourStyles}
                          value={{
                            label: verifyDetails?.plandmarkandpositionalprefix === '' ? 'Please Select Landmark and Positional  Prefix' : verifyDetails?.plandmarkandpositionalprefix,
                            value: verifyDetails?.plandmarkandpositionalprefix === '' ? 'Please Select Landmark and Positional  Prefix' : verifyDetails?.plandmarkandpositionalprefix,
                          }}
                          onChange={(e) => {
                            setVerifyDetails({ ...verifyDetails, plandmarkandpositionalprefix: e.value });
                          }}
                        />
                      </FormControl>
                    </Grid>
                    ...
                    <Grid item md={3} sm={12} xs={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          value={verifyDetails.pdoorno}
                          onChange={(e) => {
                            setVerifyDetails({ ...verifyDetails, pdoorno: e.target.value });
                          }}
                        />
                      </FormControl>
                    </Grid>
                    <Grid item md={3} sm={12} xs={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          value={verifyDetails.pstreet}
                          onChange={(e) => {
                            setVerifyDetails({ ...verifyDetails, pstreet: e.target.value });
                          }}
                        />
                      </FormControl>
                    </Grid>
                    <Grid item md={3} sm={12} xs={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          value={verifyDetails.parea}
                          onChange={(e) => {
                            setVerifyDetails({ ...verifyDetails, parea: e.target.value });
                          }}
                        />
                      </FormControl>
                    </Grid>
                    ...
                    {/* <Grid item md={3} sm={12} xs={12}>
                                                        <FormControl fullWidth size="small">
                                                          ...
                                                          <OutlinedInput
                                                            ...
                                                            type="text"
                                                            placeholder="Taluk"
                                                            value={verifyDetails.ptaluk}
                                                            onChange={(e) => {
                                                              setVerifyDetails({ ...verifyDetails, ptaluk: e.target.value });
                                                            }}
                                                          />
                                                        </FormControl>
                                                      </Grid>
                                                      <Grid item md={3} sm={12} xs={12}>
                                                        <FormControl size="small" fullWidth>
                                                          ...
                                                          <OutlinedInput
                                                            ...
                                                            type="text"
                                                            placeholder="Post"
                                                            value={verifyDetails.ppost}
                                                            onChange={(e) => {
                                                              setVerifyDetails({ ...verifyDetails, ppost: e.target.value });
                                                            }}
                                                          />
                                                        </FormControl>
                                                      </Grid> */}
                    ...
                    <Grid item md={3} sm={12} xs={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          value={verifyDetails?.pgpscoordination}
                          onChange={(e) => {
                            setVerifyDetails({
                              ...verifyDetails,
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={4}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={verifyDetails.samesprmnt}
                          onChange={(e) =>
                            setVerifyDetails({
                              ...verifyDetails,
                              samesprmnt: !verifyDetails.samesprmnt,
                            })
                          }
                        />
                      }
                      ...
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          ...
                          <Selects
                            ...
                            styles={colourStyles}
                            value={{
                              label: verifyDetails?.caddresstype === '' ? 'Please Select Address Type' : verifyDetails?.caddresstype,
                              value: verifyDetails?.caddresstype === '' ? 'Please Select Address Type' : verifyDetails?.caddresstype,
                            }}
                            onChange={(e) => {
                              setVerifyDetails({ ...verifyDetails, caddresstype: e.value });
                            }}
                          />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          ...
                          <Selects
                            ...
                            styles={colourStyles}
                            value={{
                              label: verifyDetails?.cpersonalprefix === '' ? 'Please Select Personal Prefix' : verifyDetails?.cpersonalprefix,
                              value: verifyDetails?.cpersonalprefix === '' ? 'Please Select Personal Prefix' : verifyDetails?.cpersonalprefix,
                            }}
                            onChange={(e) => {
                              setVerifyDetails({ ...verifyDetails, cpersonalprefix: e.value });
                            }}
                          />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          ...
                          <OutlinedInput
                            ...
                            type="text"
                            placeholder="Reference Name"
                            value={verifyDetails?.cresourcename}
                            onChange={(e) => {
                              setVerifyDetails({ ...verifyDetails, cresourcename: e.target.value });
                            }}
                          />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          ...
                          <Selects
                            ...
                            styles={colourStyles}
                            value={{
                              label: verifyDetails?.clandmarkandpositionalprefix === '' ? 'Please Select Landmark and Positional  Prefix' : verifyDetails?.clandmarkandpositionalprefix,
                              value: verifyDetails?.clandmarkandpositionalprefix === '' ? 'Please Select Landmark and Positional  Prefix' : verifyDetails?.clandmarkandpositionalprefix,
                            }}
                            onChange={(e) => {
                              setVerifyDetails({ ...verifyDetails, clandmarkandpositionalprefix: e.value });
                            }}
                          />
                        </FormControl>
                      </Grid>
                      ...
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          ...
                          <OutlinedInput
                            ...
                            type="text"
                            placeholder="Locality/Area Name"
                            value={verifyDetails.carea}
                            onChange={(e) => {
                              setVerifyDetails({ ...verifyDetails, carea: e.target.value });
                            }}
                          />
                        </FormControl>
                      </Grid>
                                                        ...
                                                        <Grid item md={3} sm={12} xs={12}>
                                                          <FormControl size="small" fullWidth>
                                                            ...
                                                            <OutlinedInput
                                                              ...
                                                              type="text"
                                                              placeholder="Post"
                                                              value={verifyDetails.cpost}
                                                              onChange={(e) => {
                                                                setVerifyDetails({ ...verifyDetails, cpost: e.target.value });
                                                              }}
                                                            />
                                                          </FormControl>
                                                        </Grid> */}
                      ...
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          ...
                          <OutlinedInput
                            ...
                            type="text"
                            placeholder="Landmark  Name"
                            value={verifyDetails.cgpscoordination}
                            onChange={(e) => {
                              setVerifyDetails({
                                ...verifyDetails,
                    ...
                    <Grid container spacing={2}>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          ...
                          <OutlinedInput id="component-outlined" type="text" placeholder="Address Type" value={verifyDetails?.paddresstype} readOnly />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          <Typography>Personal Prefix</Typography>
                          <OutlinedInput id="component-outlined" type="text" placeholder="Personal Prefix" value={verifyDetails?.ppersonalprefix} readOnly />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          <Typography>Reference Name</Typography>
                          <OutlinedInput id="component-outlined" type="text" placeholder="Reference Name" value={verifyDetails?.presourcename} readOnly />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          <Typography> Landmark & Positional Prefix </Typography>
                          <OutlinedInput id="component-outlined" type="text" placeholder="Landmark & Positional Prefix" value={verifyDetails?.plandmarkandpositionalprefix} readOnly />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          <Typography>Landmark  Name</Typography>
                          <OutlinedInput id="component-outlined" type="text" placeholder="Landmark  Name" value={verifyDetails.plandmark} readOnly />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          <Typography>House/Flat No</Typography>
                          <OutlinedInput id="component-outlined" type="text" placeholder="House/Flat No" value={verifyDetails.pdoorno} readOnly />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          <Typography>Street/Road Name</Typography>
                          <OutlinedInput id="component-outlined" type="text" placeholder="Street/Road Name" value={verifyDetails.pstreet} readOnly />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          <Typography>Locality/Area Name</Typography>
                          <OutlinedInput id="component-outlined" type="text" placeholder="Locality/Area Name" value={verifyDetails.parea} readOnly />
                        </FormControl>
                      </Grid>
                      ...
                      {/* <Grid item md={3} sm={12} xs={12}>
                                                          <FormControl fullWidth size="small">
                                                            <Typography>Taluk</Typography>
                                                            <OutlinedInput id="component-outlined" type="text" placeholder="Taluk" value={verifyDetails.ptaluk} readOnly />
                                                          </FormControl>
                                                        </Grid>
                                                        <Grid item md={3} sm={12} xs={12}>
                                                          <FormControl size="small" fullWidth>
                                                            <Typography>Post</Typography>
                                                            <OutlinedInput id="component-outlined" type="text" placeholder="Post" value={verifyDetails.ppost} readOnly />
                                                          </FormControl>
                                                        </Grid> */}
                      ...
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl size="small" fullWidth>
                          <Typography>Pincode</Typography>
                          <OutlinedInput id="component-outlined" type="text" placeholder="Pincode" value={verifyDetails.ppincode} readOnly />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl size="small" fullWidth>
                          <Typography>GPS Coordination</Typography>
                          <OutlinedInput id="component-outlined" type="text" placeholder="GPS Coordination" value={verifyDetails?.pgpscoordination} readOnly />
                        </FormControl>
                      </Grid>
                    </Grid>
                  </>
            ...
            >
              <Grid item md={3} xs={12} sm={12} sx={{ display: 'flex', justifyContent: 'center' }}>
                <Box sx={{ position: 'relative', width: '120px', height: '40px' }}>
                  {/* Verify Button */}
                  <Fade in={!verifyDetails.verified} timeout={500}>
                    <Button
                      variant="contained"
                      color="primary"
                      sx={{
              ...
              <br />
              <Grid item md={3} xs={12} sm={12}>
                <Button sx={buttonStyles.btncancel} onClick={handleCloseModVerify}>
                  {' '}
                  Cancel{' '}
                </Button>
              </Grid>
...
export default Deactivateemployeeslist;
