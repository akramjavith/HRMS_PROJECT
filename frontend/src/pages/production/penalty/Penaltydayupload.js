...
import { Box, Button, Divider, Checkbox, Dialog, DialogActions, DialogContent, FormControl, Grid, IconButton, List, ListItem, ListItemText, MenuItem, OutlinedInput, Popover, Select, TextField, Typography } from "@mui/material";
...
const generateNewPayData = (id, data, value, neterror, totalfields) => {
    const finalPercentage = Number(((Number(neterror) / Number(totalfields)) * value).toFixed(2));
    return {
        ...data,
        id: id,
        per: value,
        percentage: finalPercentage,
        amount: Number((neterror * finalPercentage).toFixed(2)),
    };
};
...
const TextBoxPlusMinusCellRenderer = (props) => {
    ...
    const { gridRefviewall } = props.context;

    const updateDataOnFord = useCallback(
        (e) => {
            const rowNode = gridRefviewall.current.api.getRowNode(node.data.id);
            console.log(rowNode, node.data, "rowNode");

            const newData = generateNewPayData(node.data.id, node.data, e.target.value, node.data.neterror, node.data.totalfield);
            rowNode.setData(newData);
            // rowNode.updateData(newData);
            // Update the entire row with the new data

            // Optionally, you can refresh cells if needed
            gridRefviewall.current.api.refreshCells({ rowNodes: [rowNode] });
        },
        [gridRefviewall, node.data]
    );

    return (
        <>
            <Box sx={{ display: "flex", justifyContent: "center", maxWidth: "fit-content" }}>
                <input value={node.data.per} style={{ height: "30px", border: "1px solid black", minWidth: "40px" }} onChange={updateDataOnFord} placeholder=""></input>
            </Box>
        </>
    );
};
...
function PenaltyDayUpload() {
    ...
    const [itemsnotconfirmlist, setItemsnotconfirmlist] = useState([]);
    const [pagenotconfirmlist, setPagenotconfirmlist] = useState(1);
    const [pageSizenotconfirmlist, setPageSizenotconfirmlist] = useState(10);
    const [searchQuerynotconfirmlist, setSearchQuerynotconfirmlist] = useState("");
    // page refersh reload
    ...
    const getviewCodeall = async (id) => {
        ...
        try {
            ...
            setProductionoriginalViewAll(
                res?.data?.spenaltydayupload?.uploaddata?.map((item, index) => ({
                    ...item,
                    serialNumber: index + 1,
    ...
    const columnDataTableviewAll = [
        ...
        // },
        {
            ...
            resizable: true,
            ...headerStyle,
            cellRenderer: TextBoxPlusMinusCellRenderer,
        },
    ...
    const readExcel = (file, name, e) => {
        if (name?.split(".")[1] === "xlsx" || name?.split(".")[1] === "xls" || name?.split(".")[1] === "csv") {
            ...
            promise.then((d) => {
                ...
                let uniqueArrayfinal = dataArray.filter(
                    (item) =>
    ...
    const fetchAllPenaltydayArray = async () => {
        ...
        try {
            ...
            setPenaltydayuploadListArray(
                res_queue.data.penaltydayupload?.map((item, index) => ({
                    ...item,
    ...
    const fetchAllPenaltyday = async () => {
        ...
        try {
            ...
            let datas = res_queue.data.penaltydayupload?.map((item) => ({
                ...item,
                olddate: item.date,
                username: item?.addedby[0]?.name,
                createdAt: moment(item?.addedby[0]?.date).format("DD-MM-YYYY hh:mm:ss a"),
                date: moment(item.date).format("DD-MM-YYYY"),
                type: item.type == "nonexcel" ? "Created" : "Uploaded",
            }));

            setPenaltydayuploadList(datas);

            setIsLoader(false);
        } catch (err) {
            handleApiError(err, setIsLoader(false), setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
        }
    };
    const ResetFunc = () => {
        setItems([]);
    ...
    const sendJSON = async () => {
        ...
        const givenDate = new Date(penaltydayupload.date);
        ...
        const isPayrunGenerated = resCheckPayrun.data.payruncontrol > 0 && isMoreThan5Days;
    ...
    const rowData = async (id, date) => {
        ...
        try {
            ...
            let checkWaiverStarted = await axios.post(SERVICE.CHECK_WAIVER_STARTED, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                date: date,
            ...
            if (isPayrunGenerated) {
                ...
                setPopupSeverityMalert("warning");
                handleClickOpenPopupMalert();
            } else if (checkWaiverStarted.data.count > 0) {
                setPopupContentMalert(`Waiver status added`);
                setPopupSeverityMalert("warning");
                handleClickOpenPopupMalert();
            } else {
                ...
                <Grid sx={{ display: "flex" }}>
                    {isUserRoleCompare?.includes("dpenaltyday") && (
                        <Button
                            ...
                            onClick={(e) => {
                                rowData(params.data.id, params.data.olddate);
                            }}
                        >
    ...
    const [notConfirmList, setNotConfirmList] = useState([]);

    //Delete Dialog open
    const [isNotCofirmOpen, setIsNotCofirmOpen] = useState(false);

    const handleNotConfirmOpen = (event) => {
        setIsNotCofirmOpen(true);
    };
    const handleNotConfirmClose = () => {
        setIsNotCofirmOpen(false);
    };
    const gridRefnotconfirmlist = useRef(null);

    //not confirm list
    //Datatable
    const handlePageChangenotconfirmlist = (newPage) => {
        setPagenotconfirmlist(newPage);
    };
    const handlePageSizeChangenotconfirmlist = (event) => {
        setPageSizenotconfirmlist(Number(event.target.value));
        setPagenotconfirmlist(1);
    };
    // Split the search query into individual terms
    const searchTermsnotconfirmlist = searchQuerynotconfirmlist.toLowerCase().split(" ");
    // Modify the filtering logic to check each term
    const filteredDatanotconfirmlists = notConfirmList?.filter((item) => {
        return searchTermsnotconfirmlist.every((term) => Object.values(item).join(" ").toLowerCase().includes(term));
    });
    const filteredDatanotconfirmlist = filteredDatanotconfirmlists.slice((pagenotconfirmlist - 1) * pageSizenotconfirmlist, pagenotconfirmlist * pageSizenotconfirmlist);
    const totalPagesnotconfirmlist = Math.ceil(filteredDatanotconfirmlists.length / pageSizenotconfirmlist);
    const visiblePagesnotconfirmlist = Math.min(totalPagesnotconfirmlist, 3);
    const firstVisiblePagenotconfirmlist = Math.max(1, pagenotconfirmlist - 1);
    const lastVisiblePagenotconfirmlist = Math.min(firstVisiblePagenotconfirmlist + visiblePagesnotconfirmlist - 1, totalPagesnotconfirmlist);
    const pageNumbersnotconfirmlist = [];
    const indexOfLastItemnotconfirmlist = pagenotconfirmlist * pageSizenotconfirmlist;
    const indexOfFirstItemnotconfirmlist = indexOfLastItemnotconfirmlist - pageSizenotconfirmlist;
    for (let i = firstVisiblePagenotconfirmlist; i <= lastVisiblePagenotconfirmlist; i++) {
        pageNumbersnotconfirmlist.push(i);
    }

    const columnDataTablenotconfirmlist = [
        {
            field: "serialNumber",
            headerName: "SNo",
            flex: 0,
    ...
    const rowDataTablenotconfirmlist = filteredDatanotconfirmlist;
    ...
    const handleFilterSave = async () => {
        ...
        try {
            ...
            if (penaltydayupload.company === "Please Select Company") {
                ...
                if (res_Day.data.message === "No Data to Upload") {
                    setPopupContentMalert(res_Day.data.message);
                    setPopupSeverityMalert("info");
                    handleClickOpenPopupMalert();
                    setIsLoaderbtn(false);
                } else if (res_Day.data.message === "Error upload confirmation pending") {
                    setNotConfirmList(res_Day.data.notconfirmlist.map((item, index) => ({ ...item, id: index + 1, date: moment(item.date).format("DD-MM-YYYY"), serialNumber: index + 1 })));
                    handleNotConfirmOpen();
                    // console.log(res_Day.data, "data");
                    // setPopupContentMalert(res_Day.data.message);
                    // setPopupSeverityMalert("info");
                    // handleClickOpenPopupMalert();
                    setIsLoaderbtn(false);
                } else {
                    setPopupContent(res_Day.data.message);
                    setPopupSeverity("success");
                    handleClickOpenPopup();
                    setIsLoaderbtn(false);
                }
            ...
            setIsLoaderbtn(false);
            console.log(err, "err");
            handleApiError(err, setIsLoaderbtn(false), setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
        }
    };
    ...
    return (
        <Box>
            ...
            <PageHeading title="Manage Penalty Day Upload" modulename="Quality" submodulename="Penalty" mainpagename="Penalty Calculation" subpagename="Penalty Day" subsubpagename="" />
                ...
                <Box sx={userStyle.dialogbox}>
                    <Grid container spacing={2}>
                        ...
                        <Grid item md={2} xs={12} sm={12} marginTop={3}>
                            <LoadingButton disabled={dataArrayLength > 0} loading={loaderbtn} onClick={handleFilterSave} color="primary" size="small" sx={{ textTransform: "capitalize" }} loadingPosition="end" variant="contained">
                                {" "}
                                Create
                            </LoadingButton>
                        </Grid>
            ...
            {isUserRoleCompare?.includes("lpenaltyday") && (
                <>
                    <Box sx={userStyle.dialogbox}>
                        {/* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Penalty Day Upload List</Typography>
                        </Grid>
                        <Grid container spacing={2}>
                            <>
                        ...
                        <Grid container spacing={2} style={userStyle.dataTablestyle}>
                            ...
                            <Grid item md={2} xs={6} sm={6}>
                                <AggregatedSearchBar columnDataTable={columnDataTable} setItems={setItemsList} addSerialNumber={addSerialNumber} setPage={setPage} maindatas={penaltydayuploadlist} setSearchedString={setSearchedString} searchQuery={searchQuery} setSearchQuery={setSearchQuery} paginated={false} totalDatas={overallItems} />
                            </Grid>
                        </Grid>
            ...
            {/* viewAll model */}
            <Dialog open={openviewAll} onClose={handleClickOpenviewAll} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" fullWidth={true} maxWidth="1600px" sx={{ marginTop: "50px" }}>
                <DialogContent>
                    <>
                        ...
                        <Grid container style={userStyle.dataTablestyle}>
                            ...
                            <Grid item md={2} xs={6} sm={6}>
                                <AggregatedSearchBar
                                    ...
                                    setSearchedString={setSearchedStringNew}
                        ...
                        {!productionfirstViewCheck ? (
                            ...
                            <>
                                ...
                                >
                                    <AgGridReact columnDefs={columnDataTableviewAll} ref={gridRefviewall} rowData={rowDataTableviewAll} context={context} getRowId={(params) => params.data.id} onGridReady={onGridReady} domLayout={"autoHeight"} />
                                </Box>

                                <Box style={userStyle.dataTablestyle}>
                                    <Box>
                                        Showing {rowDataTableviewAll.length > 0 ? (pageviewAll - 1) * pageSizeviewAll + 1 : 0} to {Math.min(pageviewAll * pageSizeviewAll, itemsviewAll.length)} of {itemsviewAll.length} entries
                                    </Box>
                                    <Box>
                                        <Button onClick={() => handlePageChangeviewAll(1)} disabled={pageviewAll === 1} sx={userStyle.paginationbtn}>
                                            <FirstPageIcon />
                                        </Button>
                                        <Button onClick={() => handlePageChangeviewAll(pageviewAll - 1)} disabled={pageviewAll === 1} sx={userStyle.paginationbtn}>
                                            <NavigateBeforeIcon />
                                        </Button>
                                        {pageNumbersviewall[0] > 1 && <span>...</span>}
                                        {pageNumbersviewall.map((pageNumber) => (
                                            <Button key={pageNumber} sx={userStyle.paginationbtn} onClick={() => handlePageChangeviewAll(pageNumber)} className={pageviewAll === pageNumber ? "active" : ""} disabled={pageviewAll === pageNumber}>
                                                {pageNumber}
                                            </Button>
                                        ))}
                                        {pageNumbersviewall[pageNumbersviewall.length - 1] < totalPagesviewAll && <span>...</span>}
                                        <Button onClick={() => handlePageChangeviewAll(pageviewAll + 1)} disabled={pageviewAll === totalPagesviewAll} sx={userStyle.paginationbtn}>
                                            <NavigateNextIcon />
                                        </Button>
                                        <Button onClick={() => handlePageChangeviewAll(totalPages)} disabled={pageviewAll === totalPagesviewAll} sx={userStyle.paginationbtn}>
                                            <LastPageIcon />
                                        </Button>
                                    </Box>
                                </Box>
            ...
            {/*DELETE ALERT DIALOG */}
            <Dialog open={isNotCofirmOpen} onClose={handleNotConfirmClose} aria-labelledby="alert-dialog-title" maxWidth={"lg"} fullWidth={true} aria-describedby="alert-dialog-description">
                <DialogContent>
                    <Typography variant="h5"> Error upload not confirm </Typography>
                    {/* {notConfirmList} */}
                    <>
                        <Box
                            style={{
                            ...
                            ref={gridRefContainer}
                        >
                            <AgGridReact columnDefs={columnDataTablenotconfirmlist} ref={gridRefnotconfirmlist} rowData={rowDataTablenotconfirmlist} context={context} getRowId={(params) => params.data.id} onGridReady={onGridReady} domLayout={"autoHeight"} />
                        </Box>

                        <Box style={userStyle.dataTablestyle}>
                            <Box>
                                Showing {rowDataTablenotconfirmlist.length > 0 ? (pagenotconfirmlist - 1) * pageSizenotconfirmlist + 1 : 0} to {Math.min(pagenotconfirmlist * pageSizenotconfirmlist, notConfirmList.length)} of {notConfirmList.length} entries
                            </Box>
                            <Box>
                                <Button onClick={() => handlePageChangenotconfirmlist(1)} disabled={pagenotconfirmlist === 1} sx={userStyle.paginationbtn}>
                                    <FirstPageIcon />
                                </Button>
                                <Button onClick={() => handlePageChangenotconfirmlist(pagenotconfirmlist - 1)} disabled={pagenotconfirmlist === 1} sx={userStyle.paginationbtn}>
                                    <NavigateBeforeIcon />
                                </Button>
                                {pageNumbersnotconfirmlist[0] > 1 && <span>...</span>}
                                {pageNumbersnotconfirmlist.map((pageNumber) => (
                                    <Button key={pageNumber} sx={userStyle.paginationbtn} onClick={() => handlePageChangenotconfirmlist(pageNumber)} className={pagenotconfirmlist === pageNumber ? "active" : ""} disabled={pagenotconfirmlist === pageNumber}>
                                        {pageNumber}
                                    </Button>
                                ))}
                                {pageNumbersnotconfirmlist[pageNumbersnotconfirmlist.length - 1] < totalPagesnotconfirmlist && <span>...</span>}
                                <Button onClick={() => handlePageChangenotconfirmlist(pagenotconfirmlist + 1)} disabled={pagenotconfirmlist === totalPagesnotconfirmlist} sx={userStyle.paginationbtn}>
                                    <NavigateNextIcon />
                                </Button>
                                <Button onClick={() => handlePageChangenotconfirmlist(totalPages)} disabled={pagenotconfirmlist === totalPagesnotconfirmlist} sx={userStyle.paginationbtn}>
                                    <LastPageIcon />
                                </Button>
                            </Box>
                        </Box>
                ...
                <DialogActions>
                    <Button variant="contained" color="primary" onClick={handleNotConfirmClose}>
                        Ok
                    </Button>
                </DialogActions>
            </Dialog>
