...
exports.getAllAssetdetail = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    // console.log(assetdetails.length, "alalaalal")
  } catch (err) {
...
const mergeChunksAsset = async (fileName, totalChunks) => {
  const parentDir = path.join(__dirname, '..'); // C:/Users/backend
  const chunkDir = path.join(parentDir, '..', '..', 'mergeChunks');
  const mergedFilePath = path.join(parentDir, '..', '..', 'assetdetails_files');
...
exports.uploadChunkAssetDetails = [
  upload.single("file"),
  catchAsyncErrors(async (req, res, next) => {
    ...
    try {
      ...
      if (chunkNumber === totalChunks - 1) {
        await mergeChunksAsset(fileName, totalChunks);
...
exports.getAllAssetDetailsTodoDelete = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    filenames.forEach((file) => {
      const parentDir = path.join(__dirname, ".."); // C:/Users/backend
      const filePath = path.join(parentDir, "..", "..", "assetdetails_files", file);
...
exports.getAllAssetDetailsTodoEditFetch = catchAsyncErrors(async (req, res, next) => {
  try {
    const { filename } = req.body;

    const parentDir = path.join(__dirname, ".."); // C:/Users/backend
    const filePath = path.join(parentDir, "..", "..", "assetdetails_files", filename);
    console.log(filePath, "filePath");
    if (!fs.existsSync(filePath)) {
