...
function AchievedAccuracyIndividualReviewInternalstatusList() {
  ...
  const { isUserRoleAccess,listPageAccessMode, isUserRoleCompare, allUsersData, isAssignBranch, allTeam, pageName, setPageName, buttonStyles } = useContext(UserRoleAccessContext);
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String('Achieved Accuracy Individual Review Internal Status List'),
  ...
  const fetchVendorDropdowns = async (e) => {
    ...
    try {
      ...
      let filteredDatas = res?.data?.vendormaster
        ?.filter((data) => {
  ...
  const customValueRendererQueue = (valueQueue, _employeename) => {
    return valueQueue.length ? valueQueue.map(({ label }) => label).join(', ') : 'Please Select Queue';
  };
  ...
  const handleSubmission = async () => {
    ...
    if (filterUser.fromdate === '' && filterUser.todate === '') {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (pointsFilter.compare === 'Between' && (pointsFilter.betweenfrom == '' || pointsFilter.betweenfrom == undefined || pointsFilter.betweento == '' || pointsFilter.betweento == undefined)) {
    ...
    try {
      ...
      let request = await axios.post(SERVICE.ACCURACY_LIST_HOME_TEAM, {
      ...
      const itemsWithSerialNumber = [...withComp, ...withOutComp]?.map((item, index) => ({
        ...
        autoerrorcount: Math.round(item?.totalfield * (1 - item.accuracy / 100)),
      }));
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, 'Achieved Accuracy Individual Review Internal Status List.png');
        })
        .catch((error) => {
    ...
    try {
      ...
      let getShowList = toShowList.map((data) => {
        ...
        if (newone) {
          return {
            ...data,
            minimumaccuracy: newone.minimumaccuracy,
          };
        } else {
          return {
            ...data,
  ...
  const handleEmployeesChange = (options) => {
    ...
    const updatedOptions = filteredWorkStation.map((option) => ({
      ...option,
      disabled: maxSelections - 1 > 0 && options.length >= maxSelections - 1 && !options.find((selectedOption) => selectedOption.value === option.value),
    }));
  ...
  let exportColumnNames = ['Date', 'Project', 'Vendor', 'Queue', 'Minimum Accuracy', 'Achieved Accuracy', 'Internal Status', 'Loginid', 'Company', 'Branch', 'Unit', 'Team', 'Employeename', 'Totalfield', 'Auto Error Count'];
  let exportRowValues = ['date', 'project', 'vendor', 'queue', 'minimumaccuracy', 'accuracy', 'internalstatus', 'loginid', 'company', 'branch', 'unit', 'team', 'employeename', 'totalfield', 'autoerrorcount'];
  const downloadPdf = (isfilter) => {
    ...
    doc.save('Achieved Accuracy Individual Review Internal Status List.pdf');
  };
  // Excel
  const fileName = 'Achieved Accuracy Individual Review Internal Status List';
  //print...
  const componentRef = useRef();
  const handleprint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: 'Achieved Accuracy Individual Review Internal Status List',
    pageStyle: 'print',
  });
  useEffect(() => {
  ...
  const rowDataTable = filteredData.map((item, index) => {
    return {
      ...
      autoerrorcount: item.autoerrorcount || 0,
    };
  });
  ...
  const [fileFormat, setFormat] = useState('');
      ...
      const fetchHierarchyBasedEmployee = async () => {
        try {
          let res = await axios.post(SERVICE.HIERARCHY_TEAM_ATTENDANCE_BASED_CLAIM, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
          ...
          setHierarchyemployee(res?.data?.resultAccessFilter)
      }
          catch (err) {
        handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
      }
    ...
    useEffect(() =>{
  fetchHierarchyBasedEmployee()
    },[])
  return (
    <Box>
      {/* ****** Header Content ****** */}
      <Headtitle title={'ACHIEVED ACCURACY INDIVIDUAL REVIEW INTERNAL STATUS LIST'} />
      <PageHeading title="Achieved Accuracy Individual Review Internal Status List" modulename="Quality" submodulename="Accuracy" mainpagename="Achieved Accuracy Individual Review Internal Status List" subpagename="" subsubpagename="" />
      ...
      {isUserRoleCompare?.includes('lachievedaccuracyindividualreviewinternalstatuslist') && (
        <>
          <Box sx={userStyle.selectcontainer}>
              ...
              <>
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      options={allUsersData
                        ?.filter((u) => 
                             hierarchyemployee.includes(u.companyname) &&
                            valueCompanyCat?.includes(u.company) && valueBranchCat?.includes(u.branch) && valueUnitCat?.includes(u.unit) && valueTeamCat?.includes(u.team))
                        .map((u) => ({
                          ...u,
                          label: u.companyname,
                          value: u.companyname,
                ...
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect options={projects} value={selectedOptionsProject} onChange={handleProjectChange} valueRenderer={customValueRendererProject} labelledBy="Please Select Project" />
                  </FormControl>
                </Grid>
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    <Typography>Vendor</Typography>
                    <MultiSelect options={vendors} value={selectedOptionsVendor} onChange={handleVendorChange} valueRenderer={customValueRendererVendor} labelledBy="Please Select Vendor" />
                  </FormControl>
                </Grid>
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    <Typography>Queue </Typography>
                    <MultiSelect options={queueOption} value={selectedOptionsQueue} onChange={handleQueueChange} valueRenderer={customValueRendererQueue} labelledBy="Please Select Queue" />
                  </FormControl>
                </Grid>
                <Grid item md={4} xs={12} sm={6}>
                  <FormControl fullWidth size="small">
                ...
                <Grid item md={2} xs={12} sm={6}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect options={typeOptions} value={typeOptValue} onChange={handleChangeType} valueRenderer={customValueRendererEditCompanyFromEdit} />
                  </FormControl>
                </Grid>
            ...
            <Grid container spacing={2} sx={{ display: 'flex', justifyContent: 'center' }}>
              <Grid item lg={1} md={2} sm={2} xs={12}>
                <Button sx={buttonStyles.buttonsubmit} variant="contained" onClick={handleSubmission}>
                  {' '}
                  Filter{' '}
                </Button>
              </Grid>
          ...
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>Achieved Accuracy Individual Review Internal Status List</Typography>
            </Grid>
      ...
      <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="md" sx={{ marginTop: '50px' }}>
        <Box sx={{ padding: '20px 50px' }}>
          <>
            <Typography sx={userStyle.HeaderText}>View Achieved Accuracy Individual Review Internal Status List</Typography>
            <br />
            <br />
            <Grid container spacing={2}>
              <Grid item md={4} xs={12} sm={12}>
      ...
      <ExportData
        ...
        itemsTwo={employees ?? []}
        filename={'Achieved Accuracy Individual Review Internal Status List'}
...
export default AchievedAccuracyIndividualReviewInternalstatusList;
