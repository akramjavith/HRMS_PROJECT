...
function LoanRequest() {
  ...
  const { isUserRoleCompare,listPageAccessMode, isUserRoleAccess, isAssignBranch, pageName, setPageName, buttonStyles } = useContext(UserRoleAccessContext);
  ...
  //     setPageName(!pageName)
  //     console.time("fetchAllGroupL")
  //     try {
  //         let res_grp = await axios.post(SERVICE.LOANBYASSIGNBRANCH_FOROPTIMIZE, {
  ...
  //         });
  //         setGroupCheck(true);
  //         setGroups(res_grp?.data?.loan)
  //         console.timeEnd("fetchAllGroupL")
  ...
  const fetchAllGroup = async () => {
    ...
    try {
      ...
      let res_grp = await axios.post(SERVICE.LOAN_LIST_TEAM, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      ...
      setGroups(
        res_grp?.data?.loan.map((item, index) => ({
          ...item,
          serialNumber: (page - 1) * pageSize + index + 1,
        }))
      );
      setTotalProjects(res_grp?.data?.loan?.length > 0 ? res_grp?.data?.totalProjects : 0);
      setTotalPages(res_grp?.data?.loan?.length > 0 ? res_grp?.data?.totalPages : 0);
      setPageSize((data) => {
        return res_grp?.data?.loan?.length > 0 ? data : 10;
      });
      setPage((data) => {
        return res_grp?.data?.loan?.length > 0 ? data : 1;
      });
      setGroupCheck(true);
  ...
  const handleResetSearch = async () => {
    ...
    try {
      let res_grp = await axios.post(SERVICE.LOANBYASSIGNBRANCH_PAGINATION, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const fetchAllLoanRequestArray = async () => {
    ...
    try {
      let res_grp = await axios.post(SERVICE.LOANBYASSIGNBRANCH_PAGINATION_EXPORTS, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setLoanRequestArray(res_grp?.data?.loan);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  const sendCalculateLoan = async () => {
    ...
    for (let month = 0; month < tenure; month++) {
      ...
      if (currentMonthIndex === 0 && month !== 0) {
        // Check for January to adjust year
        year++;
      }
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Loan Request" modulename="PayRoll" submodulename="Loan & Advance" mainpagename="Loan Request" subpagename="" subsubpagename="" />
      <Box>
        {/* Edit DIALOG */}
        <Dialog
          open={isEditOpen}
      ...
      <Box>
        <Dialog maxWidth="lg" fullWidth={true} open={statusOpen} onClose={handleStatusClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" sx={{ marginTop: '50px' }}>
          <DialogContent
            sx={{
              padding: '30px 50px',
              overflow: 'visible',
              ...
              {selectStatus.status !== 'Approved' && (
                <Grid container item md={12} xs={12} sm={12}>
                  ...
                  <Grid item md={4} sm={6} xs={12}>
                    <FormControl fullWidth size="small">
                      <Typography variant="h6">Employee Name : {selectStatus.employeename}</Typography>
                      <Typography variant="h6">Emp ID : {selectStatus.empcode}</Typography>
                    </FormControl>
                  </Grid>
                </Grid>
              )}
                ...
                <>
                  <Grid item md={3} xs={6} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput id="component-outlined" type="text" placeholder="Please Enter Loan Amount" value={appliedLoanAmount} readOnly={true} />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={6} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput id="component-outlined" type="text" placeholder="Please Enter Tenure in Month" value={appliedTenure} />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} sm={6} xs={12}>
                    <FormControl fullWidth size="small">
                      <Typography>
                        Applied Start Year<b style={{ color: 'red' }}>*</b>
                      </Typography>
                      <OutlinedInput id="component-outlined" type="text" value={appliedStartYear} readOnly={true} />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} sm={6} xs={12}>
                    <FormControl fullWidth size="small">
                      <Typography>
                        Applied Start Month<b style={{ color: 'red' }}>*</b>
                      </Typography>
                      <OutlinedInput id="component-outlined" type="text" value={appliedStartMonth} readOnly={true} />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} sm={6} xs={12}>
                    <FormControl fullWidth size="small">
                      <Typography>
                        Applied Start Date<b style={{ color: 'red' }}>*</b>
                      </Typography>
                      <OutlinedInput id="component-outlined" type="date" value={appliedStartDate} readOnly={true} />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={6} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput
                        ...
                        onChange={(e) => {
                          ...
                          if (
                            enteredValue === '' ||
                            (enteredValue !== '0' && /^\d+$/.test(enteredValue) && parseInt(enteredValue) <= parseInt(appliedLoanAmount)) // Don't accept greater than appliedLoanAmount
                          ) {
                            setSelectStatus({
                              ...selectStatus,
                              loanamount: enteredValue,
                  ...
                  <Grid item md={3} xs={6} sm={12}>
                    <FormControl fullWidth size="small">
                      <Typography>
                        Approved Start Year<b style={{ color: 'red' }}>*</b>
                      </Typography>
                      <Selects
                        styles={colourStyles}
                        options={startyearOpt}
                  ...
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      <Typography>
                        Approved Start Month<b style={{ color: 'red' }}>*</b>
                      </Typography>
                      <Selects
                        styles={colourStyles}
                  ...
                  <Grid item md={12} sm={12} xs={12}>
                    <TableContainer component={Paper}>
                      <Table sx={{ minWidth: 500 }} aria-label="customized table" id="usertable">
