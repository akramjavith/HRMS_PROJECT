...
function ListProductionPoints() {
  ...
  const fetchManageshortagemaster = async () => {
    ...
    } catch (err) { console.log(err, "021"); handleApiError(err, setShowAlert, handleClickOpenerr); }
  };
  ...
  const fetchTargetPointsData = async () => {
    ...
    } catch (err) { console.log(err, "0225"); handleApiError(err, setShowAlert, handleClickOpenerr); }
  };
  ...
  const fetchProductionLists = async () => {
    try {
      ...
      setLoader(false);
    } catch (err) { console.log(err, "error011"); setLoader(false); handleApiError(err, setShowAlert, handleClickOpenerr); } finally {
      setLoader(false);
    }
  };
  ...
  const fetchEmployee = async () => {
    ...
    } catch (err) { console.log(err, "025124"); handleApiError(err, setShowAlert, handleClickOpenerr); }
  };
  ...
  const fetchTeamAll = async () => {
    ...
    } catch (err) { console.log(err, "error016"); handleApiError(err, setShowAlert, handleClickOpenerr); }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (pointsFilter.fromdate === "" && pointsFilter.todate === "" && pointsFilter.compare === "" &&
      selectedCompany.length === 0 && selectedBranch.length === 0 && selectedUnit.length === 0 && selectedTeam.length === 0 && selectedEmployee.length === 0
  ...
  const handleClear = async (e) => {
    try {
      ...
      setLoader(false);
    } catch (err) { console.log(err, "error012"); setLoader(false); handleApiError(err, setShowAlert, handleClickOpenerr); }
  ...
  const columns = [
    ...
    ...(pointsFilter.fromdate === pointsFilter.todate
      ? [
        { title: "Day Status", field: "daystatus" },
      ]
      : []),
  ];
  ...
  const columnDataTable = [
    ...
    },
    {
      ...
      renderCell: (params) =>
        params.row.target ?
          <Chip sx={{
            fontSize: "10px", background: (params.row.status === "Between 101 - 149%" || params.row.status === "150% and Above") ? "#A3C9AA"
              : params.row.status === "Below Min Points" ? "#FF6868" : "#fdbb56"
          }}
            size="small"
            avatar={params.row.status === "Between 76 - 100%" ? <Avatar>{"👍"}</Avatar> :
              params.row.status === "150% and Above" ? <Avatar>{"🔥"}</Avatar>
                : params.row.status === "Between 101 - 149%" ? <Avatar>{"👌"}</Avatar>
                  : params.row.status === "Below Min Points" ? <Avatar>{"👎"}</Avatar> : ""}
  ...
  const rowDataTable = filteredData.map((item, index) => {
    return {
      ...
      status: item.status,
      daystatus: (item.startDate === item.endDate) ? item.daystatus : "",
    };
  });
        ...
        <Grid container spacing={2}>
          ...
          <Grid item md={2} xs={12} sm={12}>
            <FormControl fullWidth size="small">
              ...
              <MultiSelect
                options={Array.from(new Set(teams?.filter((comp) => selectedBranch.map((item) => item.value).includes(comp.branch) &&
                  selectedUnit.map((item) => item.value).includes(comp.unit))?.map((com) => com.teamname))).map((teamname) => ({
                    label: teamname,
                    value: teamname,
                  }))}
                value={selectedTeam}
              ...
              <MultiSelect
                options={employees
                  ?.filter((comp) => selectedCompany.map((item) => item.value).includes(comp.company) && selectedBranch.map((item) => item.value).includes(comp.branch) &&
                    selectedUnit.map((item) => item.value).includes(comp.unit) && selectedTeam.map((item) => item.value).includes(comp.team))
                  ?.map((com) => ({
                    ...com,
                          ...
                          csvData={
                            pointsFilter.fromdate === pointsFilter.todate ?
                              rowDataTable.map((item, index) => ({
                                ...
                                "Minimum Points": item.daypoint,
                                "Target": item.target,
                                "Points": item.point,
                                "Average Points": item.avgpoint,
                                "Minimum points Diff": item.mindiff,
                                "Target Diff": item.tardiff,
                                "Status": item.status,
                                "Day Status": item.daystatus,
        ...
        <Table sx={{ minWidth: 700 }} aria-label="customized table" id="usertable" ref={componentRef}>
          <TableHead>
            <TableRow>
              ...
              <TableCell>Min Diff</TableCell>
              <TableCell>Tar Diff</TableCell>
            ...
            {rowDataTable &&
              rowDataTable.map((row, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>{row.daypoint}</TableCell>
                  <TableCell>{row.target}</TableCell>
                  <TableCell>{row.point}</TableCell>
                  <TableCell>{row.avgpoint}</TableCell>
                  <TableCell>{row.mindiff}</TableCell>
                  <TableCell>{row.tardiff}</TableCell>
