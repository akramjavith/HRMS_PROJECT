...
function EmployeefacilityreportLog() {
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "EmployeeFacilityReportLog.png");
                })
                .catch((error) => {
    ...
    const columnDataTable = [
        ...
        {
            field: "workstationofficestatus",
            headerName: "If Office",
            flex: 0,
            width: 150,
            hide: !columnVisibility.workstationofficestatus,
            headerClassName: "bold-header",
        },
...

        {
            ...
            cellRenderer: (params) => (
                <Grid sx={{ display: "flex" }}>
                    {isUserRoleCompare?.includes("vemployeefacilityreport") && (
                        <Button
    ...
    const fetchReturnData = async () => {
        ...
        try {
            ...
            let response = await axios.get(`${SERVICE.USER_SINGLE}/${logid}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });

            const itemsWithSerialNumber = response?.data?.suser?.boardingLog?.filter(data => data?.workmode)?.map((item, index) => {
    ...
    return (
        <Box>
            <NotificationContainer />
            <Headtitle title={"EMPLOYEE FACILITY REPORT LOG"} />
            {/* ****** Header Content ****** */}
            <PageHeading
                title="Employee Facility Report Log"
                                ...
                                <Grid container spacing={2}>
                                    <Grid item xs={12}>
                                        <Typography sx={userStyle.SubHeaderText}>
                                            List Employee Facility Report Log
                                        </Typography>
                                    </Grid>
            ...
            <ExportData
                ...
                itemsTwo={employees ?? []}
                filename={"EmployeeFacilityReportLog"}
                ...
                <Box sx={{ padding: "20px 50px" }}>
                    <>
                        <Typography sx={userStyle.HeaderText}>
                            {" "}
                            View Employee Facility Report Log
                        </Typography>
                        <br />
                        <Grid container spacing={2}>
                            ...
                            <Grid item md={6} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6">If Office</Typography>
                                    <Typography>{viewDetails?.workstationofficestatus}</Typography>
                                </FormControl>
                            </Grid>
            ...
            <InfoPopup
                ...
                updateby={infoDetails?.logeditedby}
            />
...
export default EmployeefacilityreportLog;