...
function ValidationErroEntry() {
    ...
    const [validationerror, setValidationerror] = useState({ fromdate: "", todate: "", projectvendor: "Please Select Project Vendor", queuename: "Please Select Queue Name", loginid: "Please Select Loginid" });
    const [validationerrorEdit, setValidationerrorEdit] = useState({ fromdate: "", todate: "", projectvendor: "Please Select Project Vendor", queuename: "Please Select Queue Name", loginid: "Please Select Loginid" });
    const [validations, setValidations] = useState([]);
    const [searchQuery, setSearchQuery] = useState("");
    const [allValidationEdit, setAllValidationEdit] = useState([]);
    ...
    const rowData = async (id, name) => {
        ...
        try {
            ...
            setDeletevalidation(res?.data?.sgroup);
            handleClickOpen();
        } catch (err) {
    ...
    // Alert delete popup
    let validationEditt = deleteValidation._id;
    const deleValidation = async () => {
        setPageName(!pageName)
        try {
            await axios.delete(`${SERVICE.GROUP_SINGLE}/${validationEditt}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            await fetchAllValidation();
            handleCloseMod();
            setPage(1);
    ...
    const delValidationcheckbox = async () => {
        setPageName(!pageName)
        try {
    ...
    // let updateby = viewsingleData.updatedby;
    ...
    const fetchAllValidation = async () => {
        ...
        try {
            ...
            setValidations(res_grp?.data?.groups.map((item, index) => ({
                ...item,
    ...
    //get all project.
    const fetchValidationAll = async () => {
        setPageName(!pageName)
        try {
            let res_grp = await axios.get(SERVICE.GROUP, {
                headers: {
            ...
            setAllValidationEdit(
                res_grp?.data?.groups.filter((item) => item._id !== validationerrorEdit._id)
            );
        } catch (err) {
            handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
        }
    ...
    useEffect(() => {
        fetchValidationAll();
    }, [isEditOpen, validationerrorEdit]);
    ...
    const columnDataTable = [
        ...
        },
        {
            ...
            headerClassName: "bold-header",
            // cellRenderer: (params) => (
            //     console.log(params.data.greentext, "greentext"),
            //     < Grid sx={{ display: "flex", alignItems: "center" }}>
            //         <Typography sx={{ color: params.data.greentext == true ? "green" : "#181d1fe8", fontSize: "14px", fontWeight: 0, textAlign: "center" }}>{params.data.projectvendor}</Typography>
    ...
    const fetchFilteredDatas = async () => {
        ...
        try {
            ...
            const itemsWithSerialNumber = subprojectscreate?.data?.validatefinal?.map((item, index) => ({
                ...item,
            ...
            let res_project = await axios.get(SERVICE.PENALTY_TOTAL_FIELD_VALIDATION_ENTRY_FILTER, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            ...
            let datacheck = res_project?.data?.penaltytotalfielduploads
            ...
            let finaldata = itemsWithSerialNumber.filter(data => data.name === isUserRoleAccess.companyname).map((item, index) => {
                ...
                return {
                    ...
                    greentext: datacheck.some(d =>
                        // console.log(new Date(d.date), new Date(dateFinal), new Date(d.date) === new Date(dateFinal), "aaiekdi"),

                        d.projectvendor === item.projectvendor &&
                        d.date === dateFinal &&
                        // new Date(d.date) === new Date(dateFinal)
                        d.queuename === item.process &&
                        d.loginid === item.loginid
    ...
    const fetchPeanltyTotalFieldUploadValidation = async () => {
        try {
            let res_project = await axios.get(SERVICE.PENALTY_TOTAL_FIELD_VALIDATION_ENTRY_FILTER, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
            ...
            setValidation(res_project?.data?.penaltytotalfielduploads)
    ...
    useEffect(() => {
        ...
        fetchBulkErrorPointsData();
        // fetchPeanltyTotalFieldUploadValidation();
    }, [])
    ...
    return (
        <Box>
                    ...
                    <Grid container spacing={2}>
                        <Grid item md={3} xs={12} sm={6}>
                            <FormControl fullWidth size="small">
                                ...
                                <Typography sx={{ color: "cornflowerblue" }}>{viewsingleData.process}</Typography>
                        ...
                        <Grid item md={3} xs={12} sm={6}>
                            <FormControl fullWidth size="small">
                                ...
                                <Typography sx={{ color: "cornflowerblue" }}>{viewsingleData.fieldname}</Typography>
                        ...
                        <Grid item md={3} xs={12} sm={6}>
                            <FormControl fullWidth size="small">
                                ...
                                <Typography sx={{ color: "red" }}>{viewsingleData.errorvalue}</Typography>
                        ...
                        <Grid item md={3} xs={12} sm={6}>
                            <FormControl fullWidth size="small">
                                ...
                                <Typography sx={{ color: "green" }}>{viewsingleData.correctvalue}</Typography>
            ...
            <DeleteConfirmation
                ...
                onConfirm={delValidationcheckbox}
                title="Are you sure?"
                confirmButtonText="Yes"
                cancelButtonText="Cancel"
            />
