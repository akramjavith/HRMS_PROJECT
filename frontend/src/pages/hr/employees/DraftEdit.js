...
function DraftEdit() {
  ...
  const allotWorkStation = async () => {
    try {
      ...
      let autoWorkStation = `W${selectedBranchCode
        ?.slice(0, 2)
        ?.toUpperCase()}${selectedUnitCode
          ?.slice(0, 2)
          ?.toUpperCase()}_${formattedWorkstationCode}_${finalUsername?.toUpperCase()}`;
  ...
  const fetchbranchCodes = async (selectedBranch, selectedCompany) => {
    try {
      ...
      const branchCode = req?.data?.branch?.find(
        (item) =>
  ...
  const fetchHandlerEdit = async () => {
    try {
      ...
      setEmployee({
        ...
        religion: response?.data?.sdraft?.religion || "",
  ...
  const fetchDepartments = async () => {
    try {
      ...
      let filteredDepartments = req?.data?.departmentdetails?.filter(
  ...
  const createUser = async () => {
    ...
    try {
      ...
      const paswordupdateday =
        resSetting?.data?.overallsettings?.[0]?.passwordupdatedays ?? "0";
  ...
  const nextStepLog = () => {
    ...
    if (employee.wordcheck === true && employee.empcode?.length > 12) {
      newErrorsLog.empcode = (
        <Typography style={{ color: "red" }}>
          Manual Empcode must be less than 12 digit
        </Typography>
      );
