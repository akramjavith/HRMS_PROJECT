...
const PaymentDueRemainderLogList = ({ ExistingVisitors, handleCloseModEdit }) => {
    ...
    let exportColumnNames = ["Mode Of Payment", "Payment Date", "Paid Amount", "Balance", "Ref No", "Description"];
    let exportRowValues = ['modeofpayments', 'payamountdate', 'payamount', 'balanceamount', "refno", "description"]
    ...
    const fetchTaskcategory = async () => {
        ...
        try {
            ...
            setTaskcategorys(ExistingVisitors?.map((item) => ({
                ...item,
                payamountdate: moment(item.payamountdate).format("DD-MM-YYYY hh:mm a")
            })));
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: 'Payment Due Reminder Log',
        pageStyle: 'print'
    });
    ...
    const renderFilePreviewMulterUploaded = async (file) => {
        ...
        const imageUrl = `${SERVICE.VEIW_EXPENSE_FILES}/${file}`;
    ...
    const columnDataTable = [
        ...
        { field: "modeofpayments", headerName: "Mode Of Payment", flex: 0, width: 150, hide: !columnVisibility.modeofpayments, headerClassName: "bold-header" },
        { field: "payamountdate", headerName: "Payment Date", flex: 0, width: 150, hide: !columnVisibility.payamountdate, headerClassName: "bold-header" },
        { field: "payamount", headerName: "Paid Amount", flex: 0, width: 150, hide: !columnVisibility.payamount, headerClassName: "bold-header" },
        { field: "balanceamount", headerName: "Balance", flex: 0, width: 150, hide: !columnVisibility.balanceamount, headerClassName: "bold-header" },
        { field: "refno", headerName: "Ref No", flex: 0, width: 120, hide: !columnVisibility.refno, headerClassName: "bold-header" },
        { field: "description", headerName: "Description", flex: 0, width: 120, hide: !columnVisibility.description, headerClassName: "bold-header" },
        {
            field: "attachments",
            ...
            width: 180,
            minHeight: "40px",
            hide: !columnVisibility.attachments,
            pinned: "left",
            // lockPinned: true,
            renderCell: (params) => (
                <Grid>
                    {params?.row?.attachments &&
                        <div className="page-pdf">
                            <Button
                                onClick={() => {
                                    renderFilePreview(params?.row?.attachments);
                                }}
                    ...
                    {params?.row?.filename &&
                        <div className="page-pdf">
                            <Button
                                onClick={() => {
                                    renderFilePreviewMulterUploaded(params?.row?.filename);
                                }}
                                className="next-pdf-btn pdf-button">
                                View
                            </Button>
    ...
    const rowDataTable = filteredData.map((item, index) => {
        ...
        return {
            ...
            payamountdate: item.payamountdate,
    ...
    return (
        <Box>
                    ...
                    <DialogActions>
                        {selectedRowsCount > 0 ?
                            <>
                                ...
                                <Button variant="contained" color='error' sx={buttonStyles.buttonsubmit}
                                    onClick={(e) => delTaskCatecheckbox(e)}
                                > OK </Button>
                            </>
                            :
                            <Button variant="contained" color='error' onClick={handleCloseModcheckbox} sx={buttonStyles.btncancel}>Ok</Button>
                        }
                    </DialogActions>
                </Dialog>
            ...
            <ExportData
                ...
                itemsTwo={items ?? []}
                filename={"Payment Due Reminder Log List"}
...
export default PaymentDueRemainderLogList;