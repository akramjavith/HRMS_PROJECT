...
function RaiseProblemClosed() {
    ...
    try {
      ...
      let data = res_branch?.data?.raises
        ...
        ?.map((item, index) => ({
          ...
          closeddate: moment(item?.closedAt).format("DD-MM-YYYY"),
          closedtime: moment(item?.closedAt).format("hh:mm:ss A"),
        }));
      return data;
  ...
  const columns = [
    ...
    { title: "Closed Time", field: "closedtime" },
  ...
  const downloadPdf = async (isfilter) => {
    ...
    doc.save("RaiseProblemClosed.pdf");
  };
  ...
  const columnDataTable = [
...

    {
      ...
      flex: 0,
      width: 100,
      hide: !columnVisibility.closeddate,
      headerClassName: "bold-header",
    },
    {
      field: "closedtime",
      ...
      flex: 0,
      width: 130,
      hide: !columnVisibility.closedtime,
      headerClassName: "bold-header",
    },
    {
      field: "closedby",
      ...
      flex: 0,
      width: 130,
      hide: !columnVisibility.closedby,
      headerClassName: "bold-header",
    },
    // {
    //   field: "actions",
    ...
    //         <Link
    //           to={`/production/raiseproblemview/${params.row.id}/view`}
      ...
      renderCell: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("vraiseproblem") && (
            <Link
              to={`/production/raiseproblemview/${params.row.id}/statusupdate/closed`}
  ...
  // Excel
  const fileName = "RaiseProblemClosed";

  //print...
  const componentRef = useRef();
  const handleprint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: "RaiseProblemClosed",
    pageStyle: "print",
  });
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "RaiseProblemClosed.png");
        });
      });
    }
  };
  ...
  return (
    <Box>
      <Headtitle title={"RAISE PROBLEM CLOSED"} />
      {/* ****** Header Content ****** */}
      <PageHeading
        title="List Raise Problem Closed"
        modulename="Support"
        submodulename="Raise Problem Filter"
        mainpagename="Raise Problem Closed"
        subpagename=""
        subsubpagename=""
      />

      <>
        {isUserRoleCompare?.includes("lraiseproblemclosed") && (
          <>
            <Box sx={userStyle.container}>
              ...
              <Grid container sx={{ justifyContent: "center" }}>
                <Grid>
                  {isUserRoleCompare?.includes("excelraiseproblemclosed") && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes("csvraiseproblemclosed") && (
                    <>
                      <Button
                        onClick={(e) => {
                          setIsFilterOpen(true);
                  ...
                  {isUserRoleCompare?.includes("printraiseproblemclosed") && (
                    <>
                  ...
                  {isUserRoleCompare?.includes("pdfraiseproblemclosed") && (
                    <>
                      <Button
                        sx={userStyle.buttongrp}
                        onClick={() => {
                  ...
                  {isUserRoleCompare?.includes("imageraiseproblemclosed") && (
                    <Button
                      sx={userStyle.buttongrp}
                  ...
                  {rowDataTable?.length > 0 ? (
                    rowDataTable?.map((row, index) => (
                      <StyledTableRow key={index}>
                        ...
                        <TableCell>{row.closedtime}</TableCell>
...
export default RaiseProblemClosed;
