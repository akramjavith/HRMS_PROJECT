...
const ActionCell = ({ params, sendDataToParentUINonProduction, fetchBatchFilter,length }) => {
  ...
  const handleUpdateStatus = async (params) => {
    try {
      let res = await axios.post(`${SERVICE.IDELTIMEWORK_APPROVE_UPDATE}`, {
        ...
        completed:length === 1 ? "Completed" : ""
      });
    
   await sendDataToParentUINonProduction(params, "Approved");
  ...
  const handleRejectStatus = async (id, reasonstatus) => {
    try {
     ...
     await sendDataToParentUINonProduction(params, "Rejected");
  ...
  return (
    <Box>
      <Grid container spacing={1}>
        <Grid item xs={6} md={6}>
          <Button variant="contained" size="small" onClick={() => handleUpdateStatus(params)}>
            Approve
          </Button>
          &ensp;&ensp;
...
function ViewWaitForApprove({ sendDataToParentUIStock, handleCloseOpenviewalertwaitforapprove, getview, setGetview }) {
...
const [overalldata,setOveralldata] = useState()
  ...
  const handlegetView = async (params,status) => {
    console.log(params,status,"577")
    try{
  ...
  return (
    <Box>
      ...
      <ExportData
        ...
        itemsTwo={getview ?? []}
        filename={"View Wait For Approve List"}
...
export default ViewWaitForApprove;
