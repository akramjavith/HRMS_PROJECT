...
function EmployeeDetailStatus() {
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Long Absent Restrcition List"),
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "Long Absent Restrcition List.png");
                });
            });
        }
    };
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <PageHeading
                title="Long Absent Restrcition List"
