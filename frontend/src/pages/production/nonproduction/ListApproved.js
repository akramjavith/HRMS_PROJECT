...
function ListApproved({ listapproved, taskcategoryCheckApproved, totalProjectsApprovedval, totalPagesApprovedval, base, category, subcategory, fromdate, todate, taskcategorysAssignExports }) {
  ...
  const handleFilterClickForApproved = async () => {
    ...
    try {
      let res_vendor = await axios.post(SERVICE.NONPRODUCTIONFILTERLIST_APPROVED_PAGINATION, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const filteredSelectedColumn = columnDataTableAssign.filter((data) => data.field !== 'checkbox' && data.field !== 'actions' && data.field !== 'serialNumber');
  ...
  const filteredColumnsAssign = columnDataTableAssign.filter((column) => column.headerName.toLowerCase().includes(searchQueryManageAssign.toLowerCase()));
  ...
  const manageColumnsContentAssign = (
    <Box style={{ padding: '10px', minWidth: '325px', '& .MuiDialogContent-root': { padding: '10px 0' } }}>
      ...
      <Box sx={{ position: 'relative', margin: '10px' }}>
        <TextField label="Find column" variant="standard" fullWidth value={searchQueryManageAssign} onChange={(e) => setSearchQueryManageAssign(e.target.value)} sx={{ marginBottom: 5, position: 'absolute' }} />
      </Box>
      <br />
      <br />
        ...
        <List sx={{ overflow: 'auto', height: '100%' }}>
          {filteredColumnsAssign.map((column) => (
            <ListItem key={column.field}>
              <ListItemText
                sx={{ display: 'flex' }}
                primary={<Switch sx={{ marginTop: '-5px' }} size="small" checked={columnVisibilityAssign[column.field]} onChange={() => toggleColumnVisibilityAssign(column.field)} />}
