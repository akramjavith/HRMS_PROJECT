...
function ProductionUnAllotTemp() {
    ...
    const [produnallot, setProdUnAllot] = useState({
        ...
        filename: "Choose Category", addedby: "", updatedby: "",
    });
    ...
    const [isBulkEditOpenSec, setIsBulkEditOpenSec] = useState(false);
    const handleClickOpenBulkEditSection = () => {
        setIsBulkEditOpenSec(true);
    };
    const handleClickCloseBulkEditSection = (e, reason) => {
        if (reason && reason === "backdropClick") return;
        setIsBulkEditOpenSec(false);
    };
    ...
    let exportColumnNames = [
        ...
        'Date', 'Project Name', 'Category', 'SubCategory', 'Production', 'Mode'
    ]
    let exportRowValues = [
    ...
    const fetchProductionFilter = async () => {
        ...
        try {
            ...
            let res_vendor = await axios.post(SERVICE.PRODUCTION_UNALLOT_FILTER_TEMP, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            ...
            const groupedData = UpdatedValue.reduce((acc, curr) => {
                ...
                // console.log(key, 'key')
                acc[key] = acc[key] || { ...curr, productioncount: 0, ids: [], datenew: datenew }; // Initialize _id as an array
                ...
                acc[key].ids.push({
                    ...
                    category: curr.unallotcategory ? curr.unallotcategory : curr.filename,
                    subcategory: curr.unallotsubcategory ? curr.unallotsubcategory : curr.category,
                    mode: curr.mode,
                    project: curr.vendor,
                });
                return acc;
            ...
            let updateMrate = mergedDataval.map((curr) => {
                ...
                let findmrate = unitratesdata.find((d) => d.project === vendorupdated && d.category === filename && d.subcategory === subcategory);

                let valuefind = subcates.find((item) => {
                    return item.project === vendorupdated && item.categoryname === filenameupdated && item.name === subcategory;
                });
    ...
    const fetchProductionFilterUpdateAllot = async () => {
        try {
            ...
            let UpdatedValue = res_vendor.data.mergedData
            // .map((d) => {
            //     let datecurr = d.mode === "Manual" ? d.formatteddatetime : d.formatteddatetime?.split(" ")[0];
            //     return {
            //         ...d,
            //         datenew: datecurr,
            //     };
            ...
            const groupedData = UpdatedValue.reduce((acc, curr) => {
                let datenew = curr.formatteddatetime.includes("GMT+0530") ? moment(new Date(curr.formatteddatetime)).format("YYYY-MM-DD") : curr.mode === "Manual" ? curr.formatteddatetime : curr.formatteddatetime?.split(" ")[0];
                const [finalfilename] = curr.filename.split(".x")
                let finalcategory = curr.unallotcategory ? curr.unallotcategory : finalfilename
                let finalsubcategory = curr.unallotsubcategory ? curr.unallotsubcategory : curr.category
                const key = `${finalcategory}-${finalsubcategory}-${datenew}`;
    ...
    const fetchProjMaster = async () => {
        try {
            ...
            let filteredData = res_project?.data?.projmaster.filter(d => d.name === "SDS_Quickclaim")
            const projectopt = filteredData.map((item) => ({
                ...item,
                label: item.name,
                value: item.name
    ...
    // const getAllot = async (filename, category, mode, vendor, ids, unitrate, flagcount) => {
    //     try {
    //         // setBtnallotloader(true)
    //         handleOpenLoadingMessage()
    //         setAllotid(ids)
    //         const filenamelistviewAll = filename && filename?.split(".x");
    //         const filenamelist = filenamelistviewAll && filenamelistviewAll[0];

    //         let resunit = await axios.get(SERVICE.PRODUCTION_UNITRATE, {
    //             headers: {
    //                 Authorization: `Bearer ${auth.APIToken}`,
    //             },
    ...
    //         let matchunitrate = resunit.data.unitsrate.find(d => d.category == filenamelist && d.subcategory == category)
    //         let mrateval = matchunitrate && matchunitrate.mrate
    //         if (matchunitrate && mrateval != "0") {


    //             // let matchunitrate = resunit.data.unitsrate.find(d => d.category == filenamelist && d.subcategory == category)

    //             let mrateval = matchunitrate.mrate

    //             // Define a function to update data in batches
    //             async function updateDataInBatches(items) {
    //                 const chunkSize = 1200; // Define the size of each pagination chunk
    ...
    //                         const endpoint = mode === "Production" ? `${SERVICE.PRODUCTION_TEMP_UPLOAD_SINGLE}/${item._id}` : `${SERVICE.PRODUCTION_INDIVIDUAL_SINGLE}/${item._id}`;
    ...
    //                             originalunitrate: 0,
    //                             unitrate: Number(mrateval) * item.flagcount,
    //                         });
    //                     });
    ...
    //             if (ids.length > 0) {
    //                 updateDataInBatches(ids)
    //                     .then(() => {
    //                         handleCloseLoadingMessage()
    //                         setShowAlert(
    //                             <>
    //                                 <CheckCircleOutline sx={{ fontSize: "100px", color: 'orange' }} />
    //                                 <p style={{ fontSize: '20px', fontWeight: 900 }}>{"Alloted"}</p>
    //                             </>
    //                         );
    //                         handleClickOpenerrAllot();
    ...
    //                 <>
    //                     <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: 'orange' }} />
    //                     <p style={{ fontSize: '20px', fontWeight: 900 }}>{mrateval == "0" ? "Mrate value 0 in unit rate,Please Update unit rate" : "Please Update This Unit Rate Value"}</p>
    //                 </>
    //             );
    //             handleClickOpenerr();
    ...
    // const getSection = async (filename, category, mode, vendor, ids, unitrate, flagcount) => {
    //     try {
    //         // setBtnallotloader(true)
    //         handleOpenLoadingMessage()
    //         setAllotid(ids)
    ...
    //         // // Example usage
    //         const objectsWithMissingName = ids.filter(d => d.section === undefined || d.section == "" || d.section === null);
    //         console.log(objectsWithMissingName, "objectsWithMissingName")
    //         if (matchunitrate && mrateval != "0" && objectsWithMissingName.length == 0) {
    ...
    //                             unitrate: Number(mrateval) * item.flagcount,
    //                             section: (Number(mrateval) * item.flagcount) * Number(item.section)
    //                         });
    //                     });
    ...
    //                 <>
    //                     <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: 'orange' }} />
    //                     <p style={{ fontSize: '20px', fontWeight: 900 }}>{mrateval == "0" ? "Mrate value 0 in unit rate,Please Update unit rate" : objectsWithMissingName.length > 0 ? "Section Field Not Present in Production" : "Please Update This Unit Rate Value"}</p>
    //                 </>
    //             );
    //             handleClickOpenerr();
        ...
        try {
            ...
            if (mrate != "0" && mrate != "" && mrate !== undefined && unitflag !== undefined && unitflag != "0") {
                async function updateDataInBulk(items) {
                    ...
                    console.log(updatePayload.length, 'updatePayload')
                    try {
                        await axios.post(
                ...
                if (validItems.length > 0) {
                    updateDataInBulk(validItems)
                        .then(() => {
                            ...
                            setShowAlert(
                                <>
                                    <CheckCircleOutline sx={{ fontSize: "100px", color: "#007F73" }} />
                                    <p style={{ fontSize: "20px", fontWeight: 900 }}>{"Alloted"}</p>
                                </>
                            );
                            handleClickOpenerr();
                    ...
                    setShowAlert(
                        <>
                            <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: "orange" }} />
                            <p style={{ fontSize: "20px", fontWeight: 900 }}>{"Please Update Unit Rate Value"}</p>
                        </>
                    );
                    handleClickOpenerrinfo();
                }
                ...
                setShowAlert(
                    <>
                        <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: "orange" }} />
                        <p style={{ fontSize: "20px", fontWeight: 900 }}>{mrate == "0" || mrate == undefined ? "Mrate value 0 in unit rate,Please Update unit rate" : "Please Update This Unit Rate Value"}</p>
                    </>
                );
                handleClickOpenerrinfo();
            }
    ...
    const getSection = async (filename, category, mode, vendor, ids, unitrate, flagcount, id, mrate) => {
        try {
            if (ids.every((d) => d.mrate !== undefined && d.mrate != "0" && d.mrate != "") && ids.every((d) => d.section !== undefined)) {
                async function updateDataInBulk(items) {
                    const updatePayload = items.map((item) => ({
                        _id: item._id,
                        mode: item.mode,
                    ...
                    updateDataInBulk(validItems)
                        .then(() => {
                            setShowAlert(
                                <>
                                    <CheckCircleOutline sx={{ fontSize: "100px", color: "#007F73" }} />
                                    <p style={{ fontSize: "20px", fontWeight: 900 }}>{validItems.length === filteredIds.length ? "Alloted" : "Present unitrate value only updated"}</p>
                                </>
                            );
                            handleClickOpenerr();
                            setBulkloader(false);
                ...
                setShowAlert(
                    <>
                        ...
                        <p style={{ fontSize: "20px", fontWeight: 900 }}>
                            {ids.every((d) => d.section === undefined) ? "Section not Present" : mrate == "0" || mrate == undefined ? "Mrate value 0 in unit rate,Please Update unit rate" : "Please Update This Unit Rate Value"}
                        </p>
                    </>
                );
                handleClickOpenerrinfo();
    ...
    const getviewCodeall = async (ids, mode) => {
        try {
            ...
            setPageView(1)
            if (mode == "Production") {
                let res = await axios.post(SERVICE.PRODUCTION_UNALLOT_FILTER_VIEW_TEMP, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
    ...
    const delupload = async () => {
        try {
            ...
            async function updateDataInBatches(items) {
                ...
                try {
                    await axios.post(SERVICE.BULK_DELETE_UNITRATE_UNALLOT_TEMP,
                        {
                            updates: updatePayload,
                        },
                        {
    ...
    const editupload = async () => {
        try {
            ...
            //         const updateRequests = chunk.map((item) => {
            //             const endpoint = modests === "Production" ? `${SERVICE.PRODUCTION_TEMP_UPLOAD_SINGLE}/${item._id}` : `${SERVICE.PRODUCTION_INDIVIDUAL_SINGLE}/${item._id}`;
            //             return axios.put(endpoint, {
            //                 headers: {
            //                     Authorization: `Bearer ${auth.APIToken}`,
            //                 },
            ...
            async function updateDataInBatches(items) {
                ...
                try {
                    await axios.post(SERVICE.PRODUCTION_TEMP_UNITRATE_BULK_UPDATECATSUBCATEGORY,
                        {
                            updates: updatePayload,
                        },
                        {
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'Zero UnitRate UnAllot Temp.png');
                });
            });
        }
    };
    ...
    const [isErrorOpeninfo, setIsErrorOpeninfo] = useState(false);
    ...
    const handleEditSubmit = (e) => {
        e.preventDefault();
        if (produnallot.filename === "Please Select Category") {
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: 'orange' }} />
                    <p style={{ fontSize: '20px', fontWeight: 900 }}>{"Please Select Project"}</p>
    ...
    const downloadPdf = () => {
        ...
        doc.save("Zero UnitRate UnAllot Temp.pdf");
    };


    // Excel
    const fileName = "Zero UnitRate UnAllot Temp";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: 'Zero UnitRate UnAllot Temp',
        pageStyle: 'print'
    });
    ...
    const downloadPdfView = () => {
        ...
        doc.save("Zero UnitRate UnAllot Temp View.pdf");
    };


    // Excel
    const fileNameView = "Zero UnitRate UnAllot Temp View";
    ...
    const handleprintView = useReactToPrint({
        content: () => componentRef.current,
        documentTitle: 'Zero UnitRate UnAllot Temp View',
        pageStyle: 'print'
    });
    ...
    const addSerialNumber = () => {
        ...
        // const itemsWithSerialNumber = productionFilter?.map((item, index) => ({ ...item, serialNumber: index + 1 }));
        ...
        const itemsWithSerialNumber = filtered?.map((item, index) => {
            ...
            return {
                ...
                formatteddatetime: item.mode === "Manual" ? moment(item.formatteddatetime).format("DD/MM/YYYY") : item.formatteddatetime,
    ...
    // console.log(subcategories.find(item => item.categoryname == params.row.filename && item.name == params.row.category) &&
    //     subcategories.find(item => item.categoryname == params.row.filename && item.name == params.row.category).mode === "Allot", "5555555")
    ...
    const columnDataTable = [
                ...
                <Checkbox
                    ...
                    onChange={() => {
                        ...
                        if (selectedRows.includes(params.row.id)) {
                            ...
                            // console.log(updatedSelectedRowsIds, "updatedSelectedRowsIds");
                        } else {
            ...
            renderCell: (params) => {
                ...
                return (
                    <Grid container sx={{ display: "flex", justifyContent: "left", alignItems: "center", width: "100%" }}>
                        <Grid item md={6} sm={6} lg={6} sx={{ display: "flex", alignItems: "center" }}>
                            {params.row.btnmode === "Allot" && (
                                params.row.mrate === undefined || params.row.mrate === "" || params.row.mrate == 0 ?

                                    <Link to={"/production/productionunitrate"} target="_blank"> <Typography sx={{ fontSize: "inherit" }}>Unitrate Not Yet Allot</Typography></Link>
                                    :
                                    <LoadingButton
                                        onClick={() => {
                            ...
                            {params.row.btnmode === "Section" && (
                                <>
                                    <LoadingButton
                                        ...
                                        sx={{
                                            minWidth: "98px",
                                            textTransform: updatedSectionIdsDisabled.includes(params.row.id) ? "capitalize" : "uppercase",
                                            padding: updatedSectionIdsDisabled.includes(params.row.id) ? "6px 4px" : "6px 7px",
                                            border: updatedSectionIdsDisabled.includes(params.row.id) ? "2px solid #1775ce" : "none",
                                            fontSize: updatedSectionIdsDisabled.includes(params.row.id) ? "11px" : "inherit",
                                        }}
                                        loadingPosition="end"
                                        disabled={updatedSectionIdsDisabled.includes(params.row.id)}
                                        color="primary"
                                        variant="contained"
                                    >
                                        {" "}
                                        {/* <span>Section &ensp;</span> */}
                                        {updatedSectionIdsDisabled.includes(params.row.id) ? "Section updated" : "Section"}
                                    </LoadingButton>
                                </>
                            )}
                        </Grid>
                        <Grid item md={6} sm={6} lg={6}>
                            {isUserRoleCompare?.includes("vzerounitrateunallottemp") && (
                                <Button
                            ...
                            {isUserRoleCompare?.includes("ezerounitrateunallottemp") && (
                                <Button
                                    sx={{ minWidth: "50px", padding: "6px 8px" }}
                                    onClick={() => {
                            ...
                            {isUserRoleCompare?.includes("dzerounitrateunallottemp") && (
                                <Button
                                    sx={{ minWidth: "50px", padding: "6px 8px" }}
                                    onClick={(e) => {
        ...
        { field: "projectname", headerName: "Project", flex: 0, width: 140, hide: !columnVisibility.projectname, headerClassName: "bold-header" },
        ...
        { field: "productioncount", headerName: "Production", flex: 0, width: 100, hide: !columnVisibility.productioncount, headerClassName: "bold-header" },
    ...
    const addSerialNumberView = () => {
        const itemsWithSerialNumberView = viewall?.map((item, index) => ({
            ...
            formatteddatetime: moment(item.formatteddatetime).format("DD/MM/YYYY"),
    ...
    const columnDataTableview = [

        {
            ...
            flex: 0, width: 100, headerClassName: "bold-header"
        },
        { field: "formatteddatetime", headerName: "Date", flex: 0, width: 190, headerClassName: "bold-header" },
        { field: "project", headerName: "Project", flex: 0, width: 190, headerClassName: "bold-header" },
        { field: "filename", headerName: "Category", flex: 0, width: 250, headerClassName: "bold-header" },
        { field: "category", headerName: "Sub Category", flex: 0, width: 250, headerClassName: "bold-header" },
        { field: "mode", headerName: "Mode", flex: 0, width: 150, headerClassName: "bold-header" },
    ...
    const rowDataTableView = filteredDataView.map((item, index) => {
        ...
        return {
            ...
            mode: item.mode,
            project: item.vendor,
            filename: item.unallotcategory ? item.unallotcategory : filenamelist,
            category: item.unallotsubcategory ? item.unallotsubcategory : item.category,
            unitrate: item.points,
            unitid: item.unitid,
            user: item.user,
            formatteddatetime: item.formatteddatetime,
                ...
                <Grid container>
                    <Grid item md={4}>
                        <Button
                            variant="text"
                            sx={{ textTransform: 'none', }}
                            onClick={() => setColumnVisibilityView(initialColumnVisibilityView)}
                        >
                            Show All
                        </Button>
                    </Grid>
    ...
    const handleBulkUpdate = async () => {
        try {
            ...
            console.log(mratezerovaldata, 'mratezerovaldata')
            if (selectedRowsids.length !== mratezerovaldata.length && mratezerovaldata.length > 0) {
                ...
                setShowAlert(
                    <>
                        <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: "orange" }} />
                        <p style={{ fontSize: "18px", fontWeight: 900 }}>Mrate value 0 in UnitRate,</p>
                        <p style={{ fontSize: "18px", fontWeight: 900 }}>Please Update UnitRate</p>
                    </>
                );
                handleClickOpenerrinfo();
    ...
    const BulkUpdate = async () => {
        try {
            ...
            // Call the function to update data in bulk
            const validItems = filteredIds.filter((item) => item.mrate !== undefined && item.unitflag !== undefined);
        ...
        try {
            ...
            if (findmratezerovaldata) {
                ...
                if (mratezeroval && mratezerovaldata.length > 0) {
                    ...
                    handleClickOpenBulkEditSection();
    ...
    const BulkUpdateSection = async () => {
        try {
            ...
            // Call the function to update data in bulk
            const validItems = filteredIds.filter((item) => item.mrate !== undefined && item.unitflag !== undefined && item.section !== undefined);
    ...
    return (
        <Box>
            <Headtitle title={'Zero_Unit_Rate_UnAllot_Temp'} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Zero Unit Rate UnAllot Temp</Typography>
            {isUserRoleCompare?.includes("azerounitrateunallottemp")
                && (
                    <>
            ...
            <br />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lzerounitrateunallottemp") && (
                <>
                    <Box sx={userStyle.container}>
                        { /* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.SubHeaderText}>Zero Unitrate UnAllot Temp</Typography>
                        </Grid>
                        ...
                        <Grid container style={userStyle.dataTablestyle}>
                            <Grid item md={2} xs={12} sm={12}>
                                <Box>
                                    ...
                                    <Select id="pageSizeSelect" size="small" value={pageSize}
                                        MenuProps={{
                                            PaperProps: {
                                                style: {
                                                    maxHeight: 180,
                            ...
                            <Grid item md={8} xs={12} sm={12} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                <Box >
                                    {isUserRoleCompare?.includes("excelzerounitrateunallottemp") && (
                                        <>
                                            <Button onClick={(e) => {
                                    ...
                                    {isUserRoleCompare?.includes("csvzerounitrateunallottemp") && (
                                        <>
                                            <Button onClick={(e) => {
                                                setIsFilterOpen(true);
                                                setFormat("csv")
                                    ...
                                    {isUserRoleCompare?.includes("printzerounitrateunallottemp") && (
                                        <>
                                            <Button sx={userStyle.buttongrp} onClick={handleprint}>&ensp;<FaPrint />&ensp;Print&ensp;</Button>
                                        </>
                                    )}
                                    {isUserRoleCompare?.includes("pdfzerounitrateunallottemp") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("imagezerounitrateunallottemp") && (
            ...
            >
                <Box sx={{ padding: "20px" }}>
                    ...
                    { /* ******************************************************EXPORT Buttons****************************************************** */}
                    <Grid item xs={8}>
                        <Typography sx={userStyle.HeaderText}>View Zero Unitrate UnAllot Temp</Typography>
                    </Grid>
                        ...
                        <Grid item md={2} xs={12} sm={12}>
                            <Box>
                                <FormControl fullWidth size="small" >
                                    ...
                                    <Select id="pageSizeSelect" size="small" value={pageSizeview}
                                        MenuProps={{
                                            PaperProps: {
                                                style: {
                                                    maxHeight: 180,
                                        ...
                                        onChange={handlePageSizeChangeview} sx={{ width: "77px" }}>
                                        <MenuItem value={1}>1</MenuItem>
                                        <MenuItem value={5}>5</MenuItem>
                                        <MenuItem value={10}>10</MenuItem>
                                        <MenuItem value={25}>25</MenuItem>
                        ...
                        <>
                            ...
                            <Grid container spacing={2}>
                                <Grid item md={12} xs={12} sm={12}>
                                    <Typography sx={userStyle.HeaderText}>Edit Zero Unitrate UnAllot Temp</Typography>
                                </Grid>
                            </Grid>
                            <br />
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item md={4} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <Selects
                                            // options={categoryOpt.filter((d) => d.categoryname === produnallot.filename).map((d) => ({
                                            //     ...d,
                                            //     label: d.name,
                                            //     value: d.name,
                                            // }))}
                                            options={Array.from(new Set(categoryOpt.filter((d) => d.categoryname === produnallot.filename).map((d) => d.name)))
                                                .map((subcategory) => ({
                                                    label: subcategory,
                                                    value: subcategory,
                                                }))}
                            ...
                            <Grid container spacing={2}>
                                <Grid item md={6} xs={6} sm={6}>
                                    {/* <Button variant="contained" type="submit" onClick={editupload}>
                                        Update
                                    </Button> */}
          ...
          <Button variant="contained" color="error" onClick={handleCloseLoadingMessage}>
            ...
            <Box>
                <Dialog open={isErrorAllotOpen} onClose={handleCloseerrAllot} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                    ...
                    {/* <DialogActions>
            <Button variant="contained" color="error" onClick={handleCloseerrAllot}>
              ok
            </Button>
          </DialogActions> */}
                </Dialog>
            ...
            {/* ALERT DIALOG */}
            <Dialog open={isBulkEditOpen} onClose={handleClickCloseBulkEdit} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                <DialogContent sx={{ width: "350px", textAlign: "center", alignItems: "center" }}>
                ...
                <DialogActions>
                    ...
                    <LoadingButton onClick={BulkUpdate} loading={bulkloader} color="error" loadingPosition="end" variant="contained">
                        {" "}
                        <span>Yes &ensp;</span>
                    </LoadingButton>
                </DialogActions>
            ...
            {/* ALERT DIALOG */}
            <Dialog open={isBulkEditOpenSec} onClose={handleClickCloseBulkEditSection} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                <DialogContent sx={{ width: "350px", textAlign: "center", alignItems: "center" }}>
                    <Typography sx={{ textAlign: "center" }}>{showAlert}</Typography>
                </DialogContent>
                <DialogActions>
                    ...
                    <LoadingButton onClick={(e) => BulkUpdateSection(filteredIds)} loading={bulkloaderSec} color="error" loadingPosition="end" variant="contained">
                        {" "}
                        <span>Yes &ensp;</span>
                    </LoadingButton>
                </DialogActions>
            ...
            <ExportData
                ...
                itemsTwo={items ?? []}
                filename={"Zero Unitrate UnAllot Temp"}
...
export default ProductionUnAllotTemp;