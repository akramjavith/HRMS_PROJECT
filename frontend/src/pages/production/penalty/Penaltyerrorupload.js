...
import { Box, Button, Checkbox, Dialog, DialogActions, InputAdornment, DialogContent, Divider, FormControl, Grid, Tooltip, IconButton, List, ListItem, ListItemText, MenuItem, OutlinedInput, RadioGroup, Popover, Select, TextField, Typography, Radio, FormControlLabel } from '@mui/material';
...
function PenaltyErrorUpload() {
  ...
  const [selectedRowsPenaltyerror, setSelectedRowsPenaltyerror] = useState([]);
  ...
  const [loadingdeloverallOne, setloadingdeloverallOne] = useState(false);
  const [loadingdeloverallTwo, setloadingdeloverallTwo] = useState(false);
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String('Penalty Error Upload'),
  ...
  const customValueRendererProject = (valueCate, _employeename) => {
    return valueCate.length ? valueCate.map(({ label }) => label).join(', ') : 'Please Select ProjectVendor';
  };
  ...
  const customValueLoginId = (valueCate, _employeename) => {
    return valueCate.length ? valueCate.map(({ label }) => label).join(', ') : 'Please Select LoginId';
  };
  ...
  const [targetPointsOverallExports, setTargetPointsOverallExports] = useState([]);
  ...
  const handleResetSearch = async () => {
    ...
    try {
      let res_employee = await axios.post(SERVICE.PENALTYERRORUPLOADS_SORT, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  useEffect(() => {
    if (selectProjectVendor?.length > 0 && selectedProcess.length > 0 && selectedSLoginId.length > 0) {
      fetchEmployee();
    }
  ...
  const handleSelectionChange = (newSelection) => {
    setSelectedRowsPenaltyerror(newSelection.selectionModel);
  };
  ...
  //Delete model
  const [isDeleteOpencheckbox, setIsDeleteOpencheckbox] = useState({ open: false, data: [] });
  const handleClickOpencheckbox = () => {
    setIsDeleteOpencheckbox(true);
  };
  const handleCloseModcheckbox = () => {
    setIsDeleteOpencheckbox({ open: false, data: [] });
    setSelectedRowsPenaltyerror([]);
    setSelectAllChecked(false);
  };
  const [isDeleteOpenalert, setIsDeleteOpenalert] = useState(false);
  const handleClickOpenalert = async () => {
    setIsHandleChange(true);
    if (selectedRowsPenaltyerror.length === 0) {
      setIsDeleteOpenalert(true);
    } else {
      let resPayrun = await axios.post(`${SERVICE.PENALTYDAY_CREATED_BULK_DELETE}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },

        dates: selectedRowsPenaltyerror.map((item) => ({ date: item.olddate })),
      });

      if (resPayrun.data.count > 0 && resPayrun.data.notIncluded.length == 0) {
        setShowAlert(
          <>
            <ErrorOutlineOutlinedIcon sx={{ fontSize: '100px', color: 'orange' }} />
            <p style={{ fontSize: '20px', fontWeight: 900 }}>{'Penalty Day Already Generated for these dates!'}</p>
          </>
        );
        handleClickOpenerr();
      } else if (resPayrun.data.count > 0 && resPayrun.data.notIncluded.length > 0) {
        let selectedRowsfiltered = selectedRowsPenaltyerror.filter((item) => resPayrun.data.notIncluded.some((d) => item.olddate == d));
        setSelectedRowsPenaltyerror(selectedRowsfiltered);
        // console.log(selectedRowsfiltered, resPayrun.data.notIncluded, "selectedRowsfiltered");
        setAlertMsg('Penalty Day  already generated,some dates are linked, do you want to delete others?');
        setIsDeleteOpencheckbox({ open: true, data: selectedRowsfiltered });
      } else {
        setAlertMsg('Are you sure?');
        setIsDeleteOpencheckbox({ open: true, data: selectedRowsPenaltyerror });
      }
  ...
  const getRowClassName = (params) => {
    if (selectedRowsPenaltyerror.includes(params.row.id)) {
  ...
  const fetchClientUserIDEdit = async (proj) => {
    ...
    try {
      ...
      setClientLoginIDOptEdit(alluseridNamesadmin);
    } catch (err) {
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
  };

  const fetchTargetPointsDataOne = async () => {
    setPageName(!pageName);
    setTableOneLoader(true);
    setloadingdeloverallOne(true);

    try {
      let Res = await axios.post(SERVICE.PENALTYERRORUPLOADS_DATE_FILTER, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      ...
      let getFilenames = Res?.data?.penaltyerroruploadpoints.filter((item) => item.filename !== 'nonexcel');
      ...
      setTargetPointsFilename(itemsWithSerialNumber);
      setTableOneLoader(false);
      setloadingdeloverallOne(false);
    } catch (err) {
      setTableOneLoader(true);
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
  ...
  const handleFilterSubmitOne = (e) => {
    ...
    if (filterUser.fromdate === '' && filterUser.todate === '') {
      ...
      fetchTargetPointsDataOne();
    }
  };

  const handleFilterClearOne = async (e) => {
    setPageName(!pageName);
    e.preventDefault();
  ...
  const [targetPointsForDuplicate, setTargetPointsForDuplicate] = useState([]);

  const fetchTargetPointForDuplicate = async () => {
    setPageName(!pageName);
    ...
    try {
      ...
      setTargetPointsForDuplicate(Res?.data?.penaltyerroruploadpoints);
    } catch (err) {
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
  };
  ...
  // useEffect(() => {
  //     fetchTargetPointForDuplicate();
  // }, [])

  //submit option for saving
  const handleFilterSubmitTwo = (e) => {
    e.preventDefault();
  ...
  const handleFilterClearTwo = async (e) => {
    setPageName(!pageName);
    e.preventDefault();
    setTotalPages(0);
    ...
    setSelectedProcess([]);
    setSelectedLoginId([]);
    setProcessQueueArrayFilter([]);
    setClientLoginIDOptFilter([]);
    setTargetPoints([]);
    setTotalProjects(0);
    setPopupContent('Cleared Successfully');
    setPopupSeverity('success');
  ...
  const handleSubmit = async (e) => {
    ...
    let dupe = Res?.data?.penaltyerroruploadpoints;

    let Respenalty = await axios.post(SERVICE.BULKRRORUPLOADS_DUPLICATE_CHECK_WITH_PENALTYERRORUPLOAD, {
      headers: {
        Authorization: `Bearer ${auth.APIToken}`,
      },
    ...
    const isNameMatchPeanltyDuplicate = Respenalty.data.penaltyerroruploadpoints > 0;
    ...
    const isNameMatch = dupe?.some(
      (item) =>
        ...
        //item.date == penaltyErrorUpload.date &&
    ...
    if (penaltyErrorUpload.fromdate === '' || penaltyErrorUpload.todate === '') {
      ...
      handleClickOpenPopupMalert();
    } else if (penaltyErrorUpload.projectvendor === 'Please Select Project Vendor') {
      setPopupContentMalert('Please Select Project Vendor');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
  ...
  const exportColumnNames3 = ['Project Vendor', 'Process', 'Login ID', 'Date', 'Error File Name', 'Document Number', 'Document Type', 'Field Name', 'Line', 'Error Value', 'Correct Value', 'Link', 'Doc Link'];
  const exportRowValues3 = ['projectvendor', 'process', 'loginid', 'date', 'errorfilename', 'documentnumber', 'documenttype', 'fieldname', 'line', 'errorvalue', 'correctvalue', 'link', 'doclink'];
  ...
  const handleCaptureImageviewall = () => {
    if (gridRefTableImgviewall.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, 'Penalty Error Upload.png');
        })
        .catch((error) => {
  ...
  const sendRequest = async () => {
    ...
    setPageName(!pageName);
    try {
      let res = await axios.post(SERVICE.PENALTYERRORUPLOADS_CREATE, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      ...
      await fetchTargetPointForDuplicate();
  ...
  const rowDataSingleDelete = async (id, olddate) => {
    setPageName(!pageName);
    try {
      ...
      if (checkpenaltyday) {
        ...
        setDeletesingledata(Res?.data?.spenaltyerroruploadpoints);
  ...
  const deleteSingleList = async () => {
    ...
    setPageName(!pageName);
    try {
      const deletePromises = await axios.delete(`${SERVICE.PENALTYERRORUPLOADS_SINGLE}/${deleteSingleid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const deleteSingleBulkdataDelete = async (data) => {
    setLoader(true);
    setPageName(!pageName);
    try {
      if (data.length != 0) {
        setLoader(true);
        const deletePromises = await axios.post(SERVICE.MULTIPLEPENALTYERRORUPLOAD_SINGLE, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          ids: data,
  ...
  const fetchTargetPointsAllData = async (id) => {
    ...
    try {
      ...
      let getArray = Res?.data?.penaltyerroruploadpoints.filter((item) => item._id !== id);
  ...
  const rowdatasingleedit = async (id, olddate) => {
    setPageName(!pageName);
    try {
      let res_data = await axios.post(SERVICE.CHECK_PENALTYDAY_CREATED, {
        headers: {
      ...
      if (checkpenaltyday) {
        ...
        const prevData = Res?.data?.spenaltyerroruploadpoints;
        ...
        fetchClientUserIDEdit(prevData.projectvendor);
        fetchProcessQueueEdit(prevData.projectvendor);
  ...
  const rowdatasingleview = async (id) => {
    ...
    try {
      ...
      setViewDate(formatDate(Res?.data?.spenaltyerroruploadpoints?.date));
      handleClickOpenview();
    } catch (err) {
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
  ...
  const rowdatasingleinfo = async (id) => {
    ...
    try {
      ...
      setinfosingleData(Res?.data?.spenaltyerroruploadpoints);
      handleClickOpeninfo();
    } catch (err) {
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
  ...
  const editSubmit = async (e) => {
    ...
    let Respenalty = await axios.post(SERVICE.BULKRRORUPLOADS_DUPLICATE_CHECK_WITH_PENALTYERRORUPLOAD, {
      ...
      dateformatted: String(editsingleData.date),
  ...
  const sendEditRequest = async () => {
    ...
    setPageName(!pageName);
    try {
      let res = await axios.put(`${SERVICE.PENALTYERRORUPLOADS_SINGLE}/${editid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        projectvendor: String(editsingleData.projectvendor),
  ...
  // pdf.....
  const exportColumnNames2 = ['Project Vendor', 'Process', 'Login ID', 'Date', 'Error File Name', 'Document Number', 'Document type', 'Field Name', 'Line', 'Error Value', 'Correct value', 'Link', 'Doc Link'];
  const exportRowValues2 = ['projectvendor', 'process', 'loginid', 'date', 'errorfilename', 'documentnumber', 'documenttype', 'fieldname', 'line', 'errorvalue', 'correctvalue', 'link', 'doclink'];
  // Excel
  // const fileName = "Penalty Error Upload";
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: 'flex' }}>
          {isUserRoleCompare?.includes('eerrorupload') && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
                rowdatasingleedit(params.data.id, params.data.olddate);
              }}
            >
              <EditOutlinedIcon style={{ fontsize: 'large' }} sx={buttonStyles.buttonedit} />
            </Button>
          )}
          {isUserRoleCompare?.includes('derrorupload') && (
            <Button
              sx={userStyle.buttondelete}
              onClick={(e) => {
                rowDataSingleDelete(params.data.id, params.data.olddate);
              }}
            >
              <DeleteOutlineOutlinedIcon style={{ fontsize: 'large' }} sx={buttonStyles.buttondelete} />
            </Button>
          )}
          {isUserRoleCompare?.includes('verrorupload') && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
          ...
          {isUserRoleCompare?.includes('ierrorupload') && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
  ...
  function formatDate(dateString) {
    ...
    const formattedYear = dateParts[0]?.padStart(2, '0');
  ...
  const rowsWithCheckboxes = rowDataTable.map((row) => ({
    ...
    checkbox: selectedRowsPenaltyerror.includes(row.id),
  }));
  // Show All Columns functionality
  ...
  const getviewCodeall = async (filename, data) => {
    setPageName(!pageName);
    try {
      ...
      //   },
      // });
      let res = await axios.post(SERVICE.PENALTY_ERROR_UPLOAD_CLIENT_USER_LIMITED, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      ...
      let getFilenames = res?.data?.penaltyerroruploadpoints.filter((item) => item.filename === filename);
  ...
  const rowDatafileNameDelete = async (filename, oldfromdate, oldtodate) => {
    setPageName(!pageName);
    try {
      let res_data = await axios.post(SERVICE.CHECK_PENALTYDAY_CREATED_UPLOAD_WISE, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
    ...
    try {
      if (deleteFilenameData.length != 0) {
        ...
        if (deletePromises?.data?.success) {
          ...
          setPopupSeverity('success');
          handleClickOpenPopup();
          await fetchTargetPointsDataOne();
          setLoader(false);
        } else {
          setLoader(false);
          handleCloseMod();
  ...
  const exportColumnNames = ['From Date', 'To Date', 'Project Vendor', 'Process', 'File Name', 'Created By', 'Created Date & Time'];
  const exportRowValues = ['fromdate', 'todate', 'projectvendor', 'process', 'filename', 'createdby', 'createddate'];
        ...
        <Grid sx={{ display: 'flex' }}>
          {isUserRoleCompare?.includes('derrorupload') && (
            <Button
              sx={userStyle.buttondelete}
              onClick={(e) => {
                rowDatafileNameDelete(params.data.filename, params.data.oldfromdate, params.data.oldtodate);
              }}
            >
              <DeleteOutlineOutlinedIcon style={{ fontsize: 'large' }} sx={buttonStyles.buttondelete} />
            </Button>
          ...
          {isUserRoleCompare?.includes('verrorupload') && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
                getviewCodeall(params.data.filename, params.data);
              }}
            >
              <VisibilityOutlinedIcon style={{ fontsize: 'large' }} sx={buttonStyles.buttonview} />
            </Button>
  ...
  const readExcel = (file) => {
    ...
    const requiredColumns = ['Project Vendor', 'Process', 'Login ID', 'Date', 'Error Filename', 'Document Number', 'Document Type', 'Field Name', 'Line', 'Error Value', 'Correct Value', 'Link', 'Doc Link'];
    ...
    if (!acceptedExtensions.includes(fileExtension) && file !== null) {
      if (!acceptedExtensions.includes(fileExtension) && file !== null) {
        ...
        const promise = new Promise((resolve, reject) => {
          ...
          fileReader.onload = (e) => {
            ...
            const parseAndFormatDate = (dateValue) => {
              if (dateValue instanceof Date) {
                // If it's a Date object, format directly
                return moment(dateValue).format('YYYY-MM-DD');
              } else if (typeof dateValue === 'string') {
                // Try parsing date strings in various formats
                let parsedDate = moment(dateValue, ['DD-MM-YYYY', 'DD.MM.YYYY', 'YYYY-MM-DD', 'DD/MM/YYYY', 'YYYY/MM/DD', 'D-MM-YYYY', 'D.MM.YYYY', 'YYYY-MM-D', 'D/MM/YYYY', 'DD-M-YYYY', 'DD.M.YYYY', 'YYYY-M-DD', 'DD/M/YYYY', 'D-M-YYYY', 'D.M.YYYY', 'YYYY-M-D', 'D/M/YYYY'], true);
            ...
            // Format all date fields in the data
            const formattedData = data.map((row) => {
              return {
                ...row,
                Date: parseAndFormatDate(row.Date), // Format 'Date' field
              };
            });
          };
      ...
      const promise = new Promise((resolve, reject) => {
        ...
        fileReader.onload = (e) => {
          ...
          // Check if the data array is empty or has only header row
          if (data.length === 0 || (data.length === 1 && Object.values(data[0]).every((value) => value === undefined || value === ''))) {
            setPopupContentMalert('The sheet is empty or contains only column headers');
            setPopupSeverityMalert('error');
            handleClickOpenPopupMalert();
            reject(new Error('Sheet is empty or contains only headers'));
            return;
          }
          ...
          console.log(formattedData, data, 'oiji');
      ...
      promise.then(async (d) => {
        let uniqueArrayfinal = d.filter((item) => {
          return !targetPointsForDuplicate.some(
            (tp) =>
              ...
              tp.date == item['Date'] && // Use formattedTpDate here
              tp.errorfilename == item['Error Filename'] &&
              ...
              tp.link == item['Link'] &&
              tp.doclink == item['Doc Link']
          );
        });

        let Respenalty = await axios.post(SERVICE.BULKRRORUPLOAD_DUPLICATE_CHECK_WITH_PENALTYERRORUPLOAD_FILE, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        ...
        const existingKeys = new Set(Respenalty.data.penaltyerroruploadpoints);
        // 3. Create key for each item in bulkerror and filter only unique items
        const getBulkKey = (item) => `${item['Project Vendor']}-${item['Process']}-${item['Login ID']}-${item['Date']}-${item['Error Filename']}-${item['Document Number']}-${item['Document Type']}-${item['Field Name']}-${item['Line']}-${item['Error Value']}-${item['Correct Value']}`;

        const existingKeysbulk = new Set(uniqueArrayfinal.map(getBulkKey));

        const uniqueToUpload = uniqueArrayfinal.filter((item) => !existingKeys.has(getBulkKey(item)));
        // console.log(uniqueToUpload, penaltyErrorUpload, 'fdata');

        // if (!uniqueToUpload.every(t => new Date(t['Date']) >= new Date(penaltyErrorUpload.fromdate)&& new Date(t['Date'])) <= new Date(penaltyErrorUpload.todate)) {
        if (!uniqueToUpload.every((t) => new Date(t['Date']) >= new Date(penaltyErrorUpload.fromdate) && new Date(t['Date']) <= new Date(penaltyErrorUpload.todate))) {
          setPopupContentMalert('Uploaded File Date Not Matched in From and To Date');
          setPopupSeverityMalert('warning');
          handleClickOpenPopupMalert();
        } else if (uniqueToUpload.length == 0) {
          setPopupContentMalert('No Data To Upload');
          setPopupSeverityMalert('warning');
          handleClickOpenPopupMalert();
        } else {
          if (uniqueToUpload.length !== d.length) {
            setPopupContentMalert(uniqueToUpload.length != d.length ? ` Duplicate data and Points field Not a number data's are Removed` : uniqueToUpload.length != d.length ? `${d.length - uniqueToUpload.length}  Duplicate or data Removed` : ` Data's Points field is Not a Number Removed`);
            setPopupSeverityMalert('info');
            handleClickOpenPopupMalert();
          }

          const dataArray = uniqueToUpload.map((item) => {
            return {
              fromdate: penaltyErrorUpload.fromdate,
              todate: penaltyErrorUpload.todate,
              ...
              date: item['Date'],
              errorfilename: item['Error Filename'],
              documentnumber: item['Document Number'],
              documenttype: item['Document Type'] ? item['Document Type'] : '',
              fieldname: item['Field Name'] ? item['Field Name'] : '',
              filename: file.name,
              line: item['Line'] ? item['Line'] : '',
              errorvalue: item['Error Value'] ? item['Error Value'] : '',
              correctvalue: item['Correct Value'] ? item['Correct Value'] : '',
              link: item['Link'] ? item['Link'] : '',
              doclink: item['Doc Link'] ? item['Doc Link'] : '',
              addedby: [
                {
                  name: String(isUserRoleAccess.companyname),
                  date: String(new Date()),
  ...
  const sendJSON = async () => {
    ...
    if (selectedSheet === 'Please Select Sheet') {
      ...
      let uniqueArrayfinalDup = uploadExceldata.map((item) => ({
        ...item,
        mode: 'Errorupload',
      }));
      ...
      //         )
      // );
      // let uniqueArray = uniqueArrayfinal.filter(
      //     (item) =>
      //         !targetPoints.some(
      //             (tp) =>
      //                 tp.fromdate == penaltyErrorUpload.fromdate &&
      ...
      try {
        ...
        xmlhttp.open('POST', SERVICE.PENALTYERRORUPLOADS_CREATE);
        xmlhttp.setRequestHeader('Content-Type', 'application/json;charset=UTF-8');
        if (uniqueArrayfinalDup.length > 0) {
  ...
  const ExportsHead = () => {
    ...
    if (penaltyErrorUpload.fromdate === '' || penaltyErrorUpload.todate === '') {
      ...
      new CsvBuilder(fileDownloadName).setColumns(['Project Vendor', 'Process', 'Login ID', 'Date', 'Error Filename', 'Document Number', 'Document Type', 'Field Name', 'Line', 'Error Value', 'Correct Value', 'Link', 'Doc Link'], []).exportFile();
    }
  };
  ...
  return (
    <Box>
      <Headtitle title={'Penalty Error Upload'} />
      {/* ****** Header Content ****** */}
      <PageHeading title="Penalty Error Upload" modulename="Quality" submodulename="Penalty" mainpagename="Process Penalty" subpagename="Error Upload" subsubpagename="" />
      {isUserRoleCompare?.includes('aerrorupload') && (
        <Box sx={userStyle.selectcontainer}>
          <>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.importheadtext}>Penalty Error Upload</Typography>
              </Grid>
            </Grid>
            <br />
            <Grid container spacing={2}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={6} xs={12} sm={6}>
                <a href={`/penalty/penaltyerrorexcel`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                  <Button variant="contained" color="primary" size="small" sx={{ backgroundColor: '#d32f2f', color: 'white' }}>
                    Excel Upload
                  </Button>
                </a>
              </Grid>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={4} xs={12} sm={6}>
                <Grid container>
                  <Grid item md={5} xs={12} sm={6}>
                    <Typography>
                      Error Value<b style={{ color: 'red' }}>*</b>
                    </Typography>
                  </Grid>
                  <Grid item md={7} xs={12} sm={6}>
                    <FormControl fullWidth size="small">
              ...
              <Grid item md={4} xs={12} sm={6}>
                <Grid container>
                  <Grid item md={5} xs={12} sm={6}>
                    <Typography>
                      Correct Value<b style={{ color: 'red' }}>*</b>
                    </Typography>
                  </Grid>
                  <Grid item md={7} xs={12} sm={6}>
                    <FormControl fullWidth size="small">
      ...
      {loader ? (
        ...
        <>
          {isUserRoleCompare?.includes('lerrorupload') && (
            <>
              <Box sx={userStyle.selectcontainer}>
                <Grid container spacing={2}>
                  ...
                  <Grid item lg={1} md={1} sm={1} xs={6}>
                    <Box sx={(theme) => ({ mt: { lg: 3, md: 2, sm: 1, xs: 0 } })}>
                      <LoadingButton onClick={handleFilterSubmitOne} loading={loadingdeloverallOne} sx={buttonStyles.buttonsubmit} loadingPosition="end" variant="contained">
                        Filter
                      </LoadingButton>
                    </Box>
                  </Grid>
                  <Grid item lg={1} md={1} sm={1} xs={6}>
                    <Box sx={(theme) => ({ mt: { lg: 3, md: 2, sm: 1, xs: 0 } })}>
                      <Button onClick={handleFilterClearOne} sx={buttonStyles.btncancel}>
                        Clear
                      </Button>
                    </Box>
                  </Grid>
              ...
              <Box sx={userStyle.container}>
                ...
                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                  ...
                  >
                    <Box>
                      {isUserRoleCompare?.includes('excelerrorupload') && (
                        <>
                          <Button
                            onClick={(e) => {
                      ...
                      {isUserRoleCompare?.includes('csverrorupload') && (
                        <>
                          <Button
                            onClick={(e) => {
                              setIsFilterOpen1(true);
                      ...
                      {isUserRoleCompare?.includes('printerrorupload') && (
                        <>
                      ...
                      {isUserRoleCompare?.includes('pdferrorupload') && (
                        <>
                          <Button
                            sx={userStyle.buttongrp}
                            onClick={() => {
                      ...
                      {isUserRoleCompare?.includes('imageerrorupload') && (
          ...
          {isUserRoleCompare?.includes('lerrorupload') && (
            <>
              <Box sx={userStyle.container}>
                ...
                <Grid container spacing={2}>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <MultiSelect maxMenuHeight={300} options={projOpt} styles={colourStyles} value={selectProjectVendor} onChange={handleProjecvendor} valueRenderer={customValueRendererProject} labelledBy="Please Select ProjectVendor" />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <MultiSelect maxMenuHeight={300} options={processOptFilter} styles={colourStyles} value={selectedProcess} onChange={handleProcess} valueRenderer={customValueRendererProcess} labelledBy="Please Select Process" />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <MultiSelect maxMenuHeight={300} options={loginIdOptFilter} styles={colourStyles} value={selectedSLoginId} onChange={handleLoginId} valueRenderer={customValueLoginId} labelledBy="Please Select LoginId" />
                    </FormControl>
                  </Grid>
                  <Grid item md={1} xs={12} sm={12} marginTop={3}>
                    <Button variant="contained" sx={buttonStyles.buttonsubmit} onClick={handleFilterSubmitTwo} loading={loadingdeloverallTwo}>
                      Filter
                    </Button>
                  </Grid>
                  <Grid item md={0.5} xs={12} sm={12} marginTop={3}>
                    <Button onClick={handleFilterClearTwo} sx={buttonStyles.btncancel}>
                      Clear
                    </Button>
                  </Grid>
                </Grid>{' '}
                ...
                &ensp;
                {isUserRoleCompare?.includes('bderrorupload') && (
                ...
                {tableTwoLoader ? (
                  ...
                  <>
                    <AggridTableForPaginationTable
                      ...
                      selectedRowsPenaltyerror={selectedRowsPenaltyerror}
                      setSelectedRows={setSelectedRows}
                      selectedRows={selectedRows}
                      setSelectedRowsPenaltyerror={setSelectedRowsPenaltyerror}
                      pagenamecheck={'Penalty Error Upload'}
        ...
        <DialogContent>
          <Grid container spacing={2}>
            <Grid item md={12} xs={12} sm={6}>
              <Typography sx={userStyle.HeaderText}>View Penalty Error Upload</Typography>
            </Grid>
          </Grid>
          <br /> <br />
          <Grid container spacing={2}>
      ...
      <Dialog open={isEditOpen} onClose={handleCloseModEdit} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="lg" fullWidth={true} sx={{ marginTop: '50px' }}>
        <DialogContent>
          <Grid container spacing={2}>
            <Grid item md={12} xs={12} sm={6}>
              <Typography sx={userStyle.HeaderText}>Edit Penalty Error Upload</Typography>
            </Grid>
          </Grid>
          <br />
          <Grid container spacing={2}>
      ...
      <ExportData
        ...
        itemsTwo={targetPointsOverallExports ?? []}
      ...
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Penalty Error Upload Info" addedby={addedby} updateby={updateby} />
      ...
      {/* <DeleteConfirmation open={isDeleteOpencheckbox} onClose={handleCloseModcheckbox} onConfirm={deleteSingleBulkdataDelete} title="Are you sure?" confirmButtonText="Yes" cancelButtonText="Cancel" /> */}
      <Dialog open={isDeleteOpencheckbox.open} onClose={handleCloseModcheckbox} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        <DialogContent sx={{ width: '350px', textAlign: 'center', alignItems: 'center' }}>
        ...
        <DialogActions>
          ...
          <Button autoFocus variant="contained" color="error" onClick={(e) => deleteSingleBulkdataDelete(isDeleteOpencheckbox.data)}>
            {' '}
            OK{' '}
          </Button>
        </DialogActions>
