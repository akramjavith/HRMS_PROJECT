...
import { Box, Typography, OutlinedInput, TextareaAutosize, Dialog, Select, MenuItem, TableRow, TableCell, TableBody, DialogContent, DialogActions, FormControl, Grid, Paper, Table, TableHead, TableContainer, Button, List, ListItem, ListItemText, Popover, Checkbox, TextField, IconButton } from "@mui/material";
...
const Education = () => {
  ...
  const fetchUnits = async () => {
    try {
      ...
      setIsEducation(true)
    } catch (err) {setIsEducation(true);handleApiError(err, setShowAlert, handleClickOpenerr);}
  }
  ...
  const handleExportXL = (isfilter) => {
    ...
    exportToExcel(formatData(dataToExport), 'Education');
    setIsFilterOpen(false);
  };
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      handleClickOpenerr();
      setIsBtn(false)
    } catch (err) {setIsEducation(true);setIsBtn(false);handleApiError(err, setShowAlert, handleClickOpenerr);}
  }
            ...
            <>
              ...
              <Grid container spacing={2}>
                <Grid item md={2.5} xs={12} sm={6}>
                  <>
                    <Button variant="contained" onClick={handleSubmit} disabled={isBtn}>Submit</Button>
                  </>
                </Grid>
                <Grid item md={2.5} xs={12} sm={6}>
                  <>
