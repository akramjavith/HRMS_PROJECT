...
function DeactivateInternlist() {
  ...
  const isChecklistAssigned = async () => {
    try {
      ...
      const isAvailable = res?.data?.checklistverificationmasters?.some((item) => item.subsubpage === 'Deactivate Intern List');
      if (isAvailable) {
        setIsAssigned(true);
      } else {
        setIsAssigned(false);
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String('Deactivate Intern List'),
      ...
      const pipeline = [
        {
          $match: {
            ...
            resonablestatus: {
              $in: ['Absconded', 'Not Joined', 'Hold', 'Terminate', 'Postponed', 'Rejected', 'Closed', 'Releave Employee'],
            },
  ...
  const updateEmployees = (pastedNames) => {
    ...
    const availableOptions = notEmployees?.filter((comp) => valueCompanyCat?.includes(comp.company) && valueBranchCat?.includes(comp.branch) && valueUnitCat?.includes(comp.unit) && valueTeamCat?.includes(comp.team))?.map((data) => data.companyname.replace(/\s*\.\s*/g, '.').trim());
  ...
  const deleteChecklist = async () => {
    setDeleteBtn(true);
    const searchItem = datasAvailedDB.find((item) => item.commonid === postID && item.module === 'Human Resources' && item.submodule === 'HR' && item.mainpage === 'Employee' && item.subpage === 'Intern details' && item.subsubpage === 'Deactivate Intern List');
    setPageName(!pageName);
    try {
  ...
  const updateIndividualData = async (index) => {
    ...
    let searchItem = datasAvailedDB.find((item) => item.commonid == postID && item.module == 'Human Resources' && item.submodule == 'HR' && item.mainpage == 'Employee' && item.subpage == 'Intern details' && item.subsubpage == 'Deactivate Intern List');
  ...
  async function fecthDBDatas() {
    ...
    try {
      ...
      let foundData = res?.data?.mychecklist.find((item) => item.commonid == postID && item.subsubpage == 'Deactivate Intern List');
  ...
  const verifySubmit = async (from) => {
    if (!verifyDetails.firstname) {
      ...
      if (from === 'verify') {
        ...
        navigate(`/rejoininterncreate/${bor}`, {
          state: {
            rejoineddetails: [
  ...
  const getCode = async (e, name, details) => {
    ...
    try {
      ...
      let searchItem = res1?.data?.mychecklist.find((item) => item.commonid == details?.id && item.module == 'Human Resources' && item.submodule == 'HR' && item.mainpage == 'Employee' && item.subpage == 'Intern details' && item.subsubpage == 'Deactivate Intern List');
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, 'Deactivate Intern List.png');
        })
        .catch((error) => {
  ...
  const handleExportXL = (isfilter) => {
    ...
    exportToExcel(dataToExport, 'Deactivate Intern List');
    setIsFilterOpen(false);
  };
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <Headtitle title={'DEACTIVATE INTERN LIST'} />
      <PageHeading title="Deactivate Intern List" modulename="Human Resources" submodulename="HR" mainpagename="Employee" subpagename="Intern details" subsubpagename="Deactivate Intern List" />
              ...
              <>
                ...
                <Grid container sx={{ justifyContent: 'center' }}>
                  <Grid>
                    ...
                    {isUserRoleCompare?.includes('exceldeactivateinternlist') && (
                      <>
                        <Button
                          onClick={(e) => {
                            setIsFilterOpen(true);
                    ...
                    {isUserRoleCompare?.includes('printdeactivateinternlist') && (
                      <>
                    ...
                    {isUserRoleCompare?.includes('imagedeactivateinternlist') && (
