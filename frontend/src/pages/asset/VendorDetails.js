...
function VendorDetails() {
  ...
  let exportColumnNames = ['Vendor ID', 'Vendor Name', 'Email ID', 'Phone No', 'WhatsApp No', 'Address', 'Country', 'State', 'City', 'Pincode', 'GST No', 'Landline', 'Contact Person Name', 'Credit Days', 'Mode Of Payments', 'Payment Frequency', 'Status'];
  let exportRowValues = ['vendorid', 'vendorname', 'emailid', 'phonenumber', 'whatsappnumber', 'address', 'country', 'state', 'city', 'pincode', 'gstnumber', 'landline', 'contactperson', 'creditdays', 'modeofpayments', 'paymentfrequency', 'vendorstatus'];
  ...
  const handlePasteEdit = async (event) => {
    setIsLoading(true);
    event.preventDefault();
    const clipboardItems = event.clipboardData.items;
    for (let item of clipboardItems) {
      if (item.type.startsWith("image")) {
        ...
        if (file) {
          previewFile(file); // Preview the image
          await handleChangeImageDragEdit({ dataTransfer: { files: [file] } }); // Process the image
        }
      }
    }
  };
      ...
      image.onload = async () => {
        try {
          ...
          if (detections.length > 0) {
            ...
            console.log(response?.data?.matchfound, 'response?.data?.matchfound');
            if (response?.data?.matchfound) {
              setIsLoading(false);
              setPopupContentMalert('Image Already In Use!');
              setPopupSeverityMalert('info');
  ...
  const handleClickOpenalert = async () => {
    try {
      ...
      if (selectedRows.length === 0) {
        ...
        setCheckScheduleBills(resschedule?.data?.schedulepaymentbills);
        setCheckNotAddedBills(resschedule?.data?.notaddedbills);
        ...
        let schedulebills = resschedule?.data?.schedulepaymentbills.map((t) => t.vendor).flat();
        let notaddedbills = resschedule?.data?.notaddedbills.map((t) => t.vendor).flat();
        ...
        if (
          ...
          (resschedule?.data?.schedulepaymentbills).length > 0 ||
          (resschedule?.data?.notaddedbills).length > 0 ||
          ...
          setOveraldeletecheck({
            ...
            schedulebills: [...new Set(schedulebills)],
            notaddedbills: [...new Set(notaddedbills)],
            stock: [...new Set(stock)],
          ...
          setCheckAssetdetail([]);
          setCheckSchedule([]);
          setCheckScheduleBills([]);
          setCheckNotAddedBills([]);
          setCheckstock([]);
          setCheckmanualstock([]);
        } else {
  ...
  useEffect(() => {
    setVendoredit({
      ...vendoredit,
      whatsappnumber: vendoredit.phonecheck === true ? vendoredit.phonenumber : vendoredit.whatsappnumber,
    });
  }, [isEditOpen]);
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Button
          ...
          style={{
            padding: '5px',
            background: params.data.vendorstatus === 'Active' ? 'green' : params.data.vendorstatus === 'In Active' ? 'red' : 'brown',
            color: 'white',
            fontSize: '10px',
            width: '90px',
            fontWeight: 'bold',
  ...
  const [checkScheduleBills, setCheckScheduleBills] = useState([]);
  const [checkNotAddedBills, setCheckNotAddedBills] = useState([]);
  ...
  const delaccountheadwithoutlink = async () => {
    try {
      let valfilter = [...overalldeletecheck.vendorgrouping, ...overalldeletecheck.expense, ...overalldeletecheck.assetdetail, ...overalldeletecheck.schedule, ...overalldeletecheck.stock, ...overalldeletecheck.manualstockentry];
  ...
  const [ifscModalOpenEdit, setIfscModalOpenEdit] = useState(false);
  const [bankDetailsEdit, setBankDetailsEdit] = useState(null);
  const handleModalCloseEdit = () => {
    setIfscModalOpenEdit(false);
    setBankDetailsEdit(null); // Reset bank details
  };

  const handleModalOpenEdit = () => {
    setIfscModalOpenEdit(true);
  };
  ...
  const fetchBankDetails = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`https://ifsc.razorpay.com/${vendor.ifsccode}`);
  ...
  const fetchBankDetailsEdit = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`https://ifsc.razorpay.com/${vendoredit.ifsccode}`);
      if (response.status === 200) {
        setBankDetailsEdit(response.data);
        setLoading(false);
      } else {
        setPopupContentMalert('Bank Details Not Found!');
        setPopupSeverityMalert('info');
  ...
  const handleFileDeleteOld = async (filenames) => {
    try {
      let res_project = await axios.post(SERVICE.EDIT_OLDDATA_DELETE_VENDOR_DETAILS, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        filenames: filenames,
            ...
            const uploadNextChunk = async () => {
              try {
                if (end < selectedFile.size) {
                  ...
                  try {
                    const response = await axios.post(SERVICE.UPLOAD_CHUNK_VENDOR_DETAILS, formData, {
                      headers: {
  ...
  uniqueId: uniqueId,
        ...
        files: allUploadedFiles.concat(refImage, refImageDrag, capturedImages).map(item => ({name:item.name})),
    ...
    const getMultipleFilesAsObjects = async (filenames, type, uniqueId) => {
      ...
      for (const name of filenames) {
        const res = await axios.post(
          SERVICE.VENDOR_DETAILS_TODO_EDIT_FETCH,
  ...
  const getCode = async (e, vendorname) => {
    ...
    try {
      ...
      const filesbill = await getMultipleFilesAsObjects(res?.data?.svendordetails?.files.map(d => d.name), "bill", res?.data?.svendordetails?.uniqueId);
      setoldfileNamesBill(res?.data?.svendordetails?.files.map((d) => `${res?.data?.svendordetails?.uniqueId}$bill$${d.name}`));

      handleFetchBill(filesbill,res?.data?.svendordetails?.files);
      ...
      const country = Country.getAllCountries().find((country) => country.name === res?.data?.svendordetails?.country);
      const state = State.getStatesOfCountry(country?.isoCode).find((state) => state.name === res?.data?.svendordetails?.state);
      const city = City.getCitiesOfState(state?.countryCode, state?.isoCode).find((city) => city.name === res?.data?.svendordetails?.city);
  ...
  const getOverallEditSection = async (e) => {
    ...
    ${res?.data?.assetdetail?.length > 0 ? 'Asset List ,' : ''}
      ${res?.data?.schedulepayment?.length > 0 ? 'Schedule Payment Master,' : ''}
      ${res?.data?.schedulepaymentbills?.length > 0 ? 'Schedule Payment Bills,' : ''}
      ${res?.data?.notaddedbills?.length > 0 ? 'Not Added Bills,' : ''}
  ...
  const getOverallEditSectionUpdate = async () => {
    try {
      ...
      sendEditRequestOverall(res?.data?.vendorgrouping, res?.data?.expense, res?.data?.assetdetail, res?.data?.schedulepayment, res?.data?.schedulepaymentbills, res?.data?.notaddedbills, res?.data?.stock, res?.data?.manualstockentry);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };

  const sendEditRequestOverall = async (vendorgrouping, expense, assetdetail, schedulepayment, schedulepaymentbills, notaddedbills, stock, manualstockentry) => {
    try {
      ...
      if (schedulepayment.length > 0) {
        let answ = schedulepayment.map(async (d) => {
          // Update vendor field
          let updatedStatusLog = d.statuslog.map((log) => (log.vendor === ovProj ? { ...log, vendor: vendoredit.vendorname } : log));

          await axios.put(`${SERVICE.SINGLE_SCHEDULEPAYMENTMASTER}/${d._id}`, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
            vendor: String(vendoredit.vendorname),
            statuslog: updatedStatusLog, // Updating the statuslog array
          });
        });
      }
      if (schedulepaymentbills.length > 0) {
        let answ = schedulepaymentbills.map((d, i) => {
          let res = axios.put(`${SERVICE.SINGLE_OTHERPAYMENTS}/${d._id}`, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
            vendor: String(vendoredit.vendorname),
      ...
      if (notaddedbills.length > 0) {
        let answ = notaddedbills.map((d, i) => {
          let res = axios.put(`${SERVICE.SINGLE_SCHEDULEPAYMENT_NOTADDEDBILLS}/${d._id}`, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
            vendor: String(vendoredit.vendorname),
  ...
  const sendEditRequest = async () => {
    ...
    try {
      let res = await axios.put(`${SERVICE.SINGLE_VENDORDETAILS}/${vendorid}`, {
        ...
        bankname: filtered.includes('Bank Transfer') ? String(vendoredit.bankname) : '',
        bankbranchname: filtered.includes('Bank Transfer') ? vendoredit.bankbranchname : '',
        accountholdername: filtered.includes('Bank Transfer') ? vendoredit.accountholdername : '',
        accountnumber: filtered.includes('Bank Transfer') ? vendoredit.accountnumber : '',
        ...
        cardholdername: filtered.includes('Card') ? vendoredit.cardholdername : '',
        cardtransactionnumber: filtered.includes('Card') ? vendoredit.cardtransactionnumber : '',
        ...
        cardsecuritycode: filtered.includes('Card') ? vendoredit.cardsecuritycode : '',

        faceDescriptor: vendoredit?.faceDescriptor?.length > 0 ? vendoredit?.faceDescriptor : [],

        // files: allUploadedFilesEdit.concat(refImageedit, refImageDragedit, capturedImagesedit),

        files: [...refImageedit, ...refImageDragedit, ...capturedImagesedit].map(item => ({name:item.name})),

        uniqueId: vendoredit.uniqueId,
        chequenumber: filtered.includes('Cheque') ? vendoredit.chequenumber : '',
        updatedby: [
          ...updateby,
          {
            name: String(isUserRoleAccess.companyname),
      ...
      await handleFileUpload([...refImageedit, ...refImageDragedit, ...capturedImagesedit], "bill", vendoredit.uniqueId);
      await fetchVendor();
      setStdCode('');
      setLanNumber('');
      handleCloseModEdit();
  ...
  const editSubmit = async (e) => {
    ...
    const isNameMatch = allVendoredit.some((item) => item.vendorname.toLowerCase() === vendoredit.vendorname.toLowerCase());
    ...
    // }
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendoredit.vendorstatus === '' || !vendoredit.vendorstatus) {
      setPopupContentMalert('Please Select Status!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendoredit.paymentfrequency === 'Please Select Payment Frequency') {
      setPopupContentMalert('Please Select Payment Frequency!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendoredit.paymentfrequency === 'Monthly' && (vendoredit.monthlyfrequency === '' || !vendoredit.monthlyfrequency)) {
      setPopupContentMalert('Please Select Monthly Date!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendoredit.paymentfrequency === 'Weekly' && (vendoredit.weeklyfrequency === '' || !vendoredit.weeklyfrequency)) {
      setPopupContentMalert('Please Select Weekly Day!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    }
    ...
    //   handleClickOpenPopupMalert();
    // }
    else if (modeofpayEdit.includes('Bank Transfer') && (vendoredit.bankname === 'Please Select Bank Name' || vendoredit.bankname === '')) {
      setPopupContentMalert('Please Select Bank Name!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (modeofpayEdit.includes('Bank Transfer') && vendoredit.bankbranchname === '') {
      setPopupContentMalert('Please Enter Bank Branch Name!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (modeofpayEdit.includes('Bank Transfer') && vendoredit.accountholdername === '') {
      setPopupContentMalert('Please Enter Account Holder Name!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (modeofpayEdit.includes('Bank Transfer') && vendoredit.accountnumber === '') {
      setPopupContentMalert('Please Enter Account Number!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (modeofpayEdit.includes('Bank Transfer') && vendoredit.ifsccode === '') {
      setPopupContentMalert('Please Enter IFSC Code!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (modeofpayEdit.includes('Card') && vendoredit.cardholdername === '') {
      setPopupContentMalert('Please Enter Card Holder Name!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (modeofpayEdit.includes('Card') && vendoredit.cardtransactionnumber === '') {
      setPopupContentMalert('Please Enter Card Transaction Number!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (modeofpayEdit.includes('Card') && (vendoredit.cardtype === 'Please Select Card Type' || vendoredit.cardtype === '')) {
      setPopupContentMalert('Please Select Card Type!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (modeofpayEdit.includes('Card') && (vendoredit.cardmonth === 'Month' || vendoredit.cardmonth === '')) {
      setPopupContentMalert('Please Select Expire Month!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (modeofpayEdit.includes('Card') && (vendoredit.cardyear === 'Year' || vendoredit.cardyear === '')) {
      setPopupContentMalert('Please Select Expire Year!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendoredit.modeofpayments === 'Card' && vendoredit.cardsecuritycode === '') {
      setPopupContentMalert('Please Enter Card Security Code!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (modeofpayEdit.includes('Cheque') && vendoredit.chequenumber === '') {
      setPopupContentMalert('Please Enter Cheque Number!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
  ...
  const fetchVendor = async () => {
    ...
    try {
      ...
      setVendormaster(
        res_vendor?.data?.vendordetails.map((item, index) => ({
          ...item,
  ...
  const fetchVendorAll = async () => {
    ...
    try {
      ...
      setAllVendoredit(res_vendor?.data?.vendordetails.filter((item) => item._id !== vendoredit._id));
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  const getLinkedLabelItem = (overalldeletecheck) => {
    const { vendorgrouping = [], expense = [], schedule = [], schedulebills = [], notaddedbills = [], assetdetail = [], stock = [], manualstockentry = [] } = overalldeletecheck;
    const labels = [];
    ...
    schedulebills.forEach((item) => labels.push(item));
    notaddedbills.forEach((item) => labels.push(item));
  ...
  const getLinkedLabel = (overalldeletecheck) => {
    const { vendorgrouping = [], expense = [], assetdetail = [], schedule = [], schedulebills = [], notaddedbills = [], stock = [], manualstockentry = [] } = overalldeletecheck;
    const labels = [];
    ...
    if (schedulebills.length > 0) labels.push('Schedule Payment Bills');
    if (notaddedbills.length > 0) labels.push('Not Added Bills');
  ...
  const getFilteredUnits = (vendormaster, selectedRows, overalldeletecheck) => {
    const { vendorgrouping = [], expense = [], assetdetail = [], schedule = [], schedulebills = [], notaddedbills = [], stock = [], manualstockentry = [] } = overalldeletecheck;
    const allConditions = [...new Set([...vendorgrouping, ...expense, ...assetdetail, ...schedule, ...schedulebills, ...notaddedbills, ...stock, ...manualstockentry])];
  ...
  return (
    <Box>
      ...
      <PageHeading title="Vendor Details" modulename="Asset" submodulename="Master" mainpagename="Vendor Master" subpagename="" subsubpagename="" />
      <>
          ...
          <Box sx={userStyle.dialogbox}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  {' '}
                  <Typography sx={{ fontWeight: 'bold' }}>Add Vendor1</Typography>{' '}
                </Grid>
              </Grid>
              <br />
              <Grid container spacing={2}>
              ...
              <Grid container spacing={2} sx={{ display: 'flex', justifyContent: 'center' }}>
                <Grid item lg={1} md={2} sm={2} xs={12}>
                  <Button onClick={handleSubmit} disabled={isBtn} sx={buttonStyles.buttonsubmit} loadingPosition="end" variant="contained">
                    Submit
                  </Button>
                </Grid>
                <Grid item lg={1} md={2} sm={2} xs={12}>
      ...
      {isUserRoleCompare?.includes('lvendormaster') && (
        <>
          <Box sx={userStyle.container}>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.SubHeaderText}>Vendor List</Typography>
              </Grid>
            </Grid>
            <br />
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              <Grid item md={2} xs={12} sm={12}>
                <AggregatedSearchBar columnDataTable={columnDataTable} setItems={setItems} addSerialNumber={addSerialNumber} setPage={setPage} maindatas={vendormaster} setSearchedString={setSearchedString} searchQuery={searchQuery} setSearchQuery={setSearchQuery} />
              </Grid>
            </Grid>
            <br />
            <Button
              ...
              <form onSubmit={editSubmit}>
                <Grid container spacing={2}>
                  <Grid item md={12} xs={12} sm={12}>
                    <Typography sx={userStyle.HeaderText}>Edit Vendor Details</Typography>
                    <Typography>Vendor ID: {vendoredit.vendorid}</Typography>
                  </Grid>
                </Grid>
                <br />
                <Grid container spacing={2}>
                  ...
                  <Grid item md={3} xs={12} sm={3} sx={{ display: 'flex' }}>
                    <FormControl fullWidth size="small">
                      <Typography>
                        Mode of Payments<b style={{ color: 'red' }}>*</b>
                      </Typography>
                      ...
                      <Grid item md={3} xs={12} sm={3}>
                        <FormControl fullWidth size="small">
                          ...
                          <Selects
                            ...
                            placeholder="Please Choose Bank Name"
                            value={{
                              label: vendoredit.bankname === '' ? 'Please Select Bank Name' : vendoredit.bankname,
                              value: vendoredit.bankname === '' ? 'Please Select Bank Name' : vendoredit.bankname,
                            }}
                            onChange={(e) => {
                              setVendoredit({
                                ...vendoredit,
                      ...
                      <Grid item md={6} xs={12} sm={12}>
                        <FormControl fullWidth size="small">
                          ...
                          <Selects
                            ...
                            placeholder="Please Select Card Type"
                            value={{
                              label: vendoredit.cardtype === '' ? 'Please Select Card Type' : vendoredit.cardtype,
                              value: vendoredit.cardtype === '' ? 'Please Select Card Type' : vendoredit.cardtype,
                            }}
                            onChange={(e) => {
                              setVendoredit({
                                ...vendoredit,
                          ...
                          <Grid item md={6} xs={12} sm={6}>
                            <FormControl fullWidth size="small">
                              <Selects
                                ...
                                id="select7"
                                value={{
                                  label: vendoredit.cardmonth === '' ? 'Month' : vendoredit.cardmonth,
                                  value: vendoredit.cardmonth === '' ? 'Month' : vendoredit.cardmonth,
                                }}
                                onChange={(e) => {
                                  setVendoredit({
                                    ...vendoredit,
                          ...
                          <Grid item md={6} xs={12} sm={6}>
                            <FormControl fullWidth size="small">
                              <Selects
                                ...
                                placeholder="Year"
                                value={{
                                  label: vendoredit.cardyear === '' ? 'Year' : vendoredit.cardyear,
                                  value: vendoredit.cardyear === '' ? 'Year' : vendoredit.cardyear,
                                }}
                                id="select8"
                                onChange={(e) => {
                                  setVendoredit({
      ...
      <Box>
        <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" fullWidth={true} maxWidth="md" sx={{ marginTop: '47px' }}>
          <Box sx={{ padding: '20px 50px' }}>
            <>
              <Typography sx={userStyle.HeaderText}> View Vendor Deatils</Typography>
              <br /> <br />
              <Grid container spacing={2}>
                <Grid item md={6} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
      ...
      />
      {/* INFO */}
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Vendor Info" addedby={addedby} updateby={updateby} />
        ...
        <DialogContent sx={{ minWidth: '750px', height: '850px' }}>
          <Grid container spacing={2}>
            <Grid item lg={12} md={12} sm={12} xs={12}>
              ...
              {/* {showDragField ? ( */}
              <div onDragOver={handleDragOveredit} onDrop={handleDropedit} onPaste={handlePasteEdit}>
                {previewURLedit && refImageDragedit.length > 0 ? (
                  <>
                    {refImageDragedit.map((file, index) => (
                      <>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                handleColorChangeCapturedEdit(e, index);
                              }}
                              style={{
                                width: '30px',
                                height: '30px',
                          ...
                          <LoadingButton
                            onClick={(e) => {
                              handleSubmitNewEdit(index, 'captured');
                            }}
                  ...
                  <Grid item md={1} sm={1} xs={1}>
                    <Grid sx={{ display: 'flex' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        ...
                        <LoadingButton
                          onClick={(e) => {
                            handleSubmitNewEdit(index, 'upload');
                          }}
          ...
          <Typography variant="h6" sx={{ color: 'black', textAlign: 'center' }}>
            {(() => {
              ...
              const conditions = [
                ...
                { check: checkScheduleBills?.length > 0, label: 'Schedule Payment Bills' },
                { check: checkNotAddedBills?.length > 0, label: 'Not Added Bills' },
      ...
      <Modal open={ifscModalOpen} onClose={handleModalClose} aria-labelledby="simple-modal-title" aria-describedby="simple-modal-description" sx={{ marginTop: '80px' }}>
        ...
        >
          ...
          <OutlinedInput type="text" placeholder="Enter IFSC Code" name="ifsccode" style={{ height: '30px', margin: '10px' }} value={vendor.ifsccode} onChange={handleInputChange} />
      ...
      {/* edit */}
      <Modal open={ifscModalOpenEdit} onClose={handleModalCloseEdit} aria-labelledby="simple-modal-title" aria-describedby="simple-modal-description" sx={{ marginTop: '80px' }}>
        <div
          style={{
            margin: 'auto',
            backgroundColor: 'white',
        ...
        >
          ...
          >
            <Typography variant="h6">Enter IFSC Code</Typography>
            <IconButton onClick={handleModalCloseEdit}>
              <CloseIcon />
            </IconButton>
          </div>
          <OutlinedInput type="text" placeholder="Enter IFSC Code" name="ifsccode" style={{ height: '30px', margin: '10px' }} value={vendoredit.ifsccode} onChange={handleInputChangeEdit} />
          <LoadingButton variant="contained" loading={loading} color="primary" sx={{ borderRadius: '20px', marginLeft: '5px' }} onClick={fetchBankDetailsEdit}>
            Get Branch
          </LoadingButton>
          <br />
          {bankDetailsEdit && (
            <div>
              <Typography variant="subtitle1">Bank Name: {bankDetailsEdit.BANK}</Typography>
              <Typography variant="subtitle1">Branch Name: {bankDetailsEdit.BRANCH}</Typography>
              <Button
                variant="contained"
                sx={{ borderRadius: '20px', padding: '0 10px', ...buttonStyles?.buttonsubmit }}
                onClick={(e) => {
                  const matchedBank = accounttypes.find((bank) => {
                    ...
                    return labelBeforeHyphen.toLowerCase()?.trim() === bankDetailsEdit.BANK.toLowerCase()?.trim();
                  });

                  setVendoredit({
                    ...vendoredit,
                    bankbranchname: String(bankDetailsEdit.BRANCH),
                    ifsccode: vendoredit.ifsccode,
                    bankname: matchedBank?.value,
                  });
        ...
        <DialogContent sx={{ textAlign: 'center', alignItems: 'center' }}>
          ...
          <Typography variant="h6" sx={{ color: 'black', textAlign: 'center' }}>
            {(overalldeletecheck.vendorgrouping?.length > 0 ||
              ...
              overalldeletecheck.schedulebills?.length > 0 ||
              overalldeletecheck.notaddedbills?.length > 0) && (
                // overalldeletecheck.stock?.length > 0 ||
                // overalldeletecheck.manualstockentry?.length > 0
                <>
                  <span style={{ fontWeight: '700', color: '#777' }}>{getLinkedLabelItem(overalldeletecheck)}</span> was linked in <span style={{ fontWeight: '700', color: '#777' }}>{getLinkedLabel(overalldeletecheck)}</span>
                  {shouldShowDeleteMessage(vendormaster, selectedRows, overalldeletecheck) && <Typography>Do you want to delete others?...</Typography>}
                </>
              )}
          </Typography>
        </DialogContent>
