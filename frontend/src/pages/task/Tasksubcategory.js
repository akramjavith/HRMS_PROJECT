...
function TaskSubcategory() {
    ...
    const [tasksubcategoryEdit, setTasksubcategoryEdit] = useState({ category: "Please Select Category", subcategoryname: "", description: "" })
    ...
    const handleExportXL = (isfilter) => {
        if (isfilter === "filtered") {
            ...
            exportToCSV(
                tasksubcategorys.map((item, index) => ({
                    "Sno": index + 1,
                    Category: item.category,
                    "Name": item.subcategoryname,
                    Description: item.description,
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'Task_Subcategory.png');
                });
            });
        }
    };
    ...
    const fetchTaskcategory = async () => {
        try {
            ...
            const categoryall = [...task?.data?.taskcategorys?.map((d) => (
                {
                    ...d,
    ...
    const delTaskSubcategory = async (e) => {
        ...
        try {
            if (tasksubcategorysid) {
                await axios.delete(`${SERVICE.TASKSUBCATEGORY_SINGLE}/${e}`, {
                    headers: {
                        'Authorization': `Bearer ${auth.APIToken}`
                    }
                });
        ...
        try {
            ...
            if (taskschedulelog?.length > 0) {
                let answ = taskschedulelog.map((d, i) => {
                    ...
                    const answerCate = d?.taskdesignationlog?.filter(data => data?.subcategory === ovSubcategory)?.map((item
                    ) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (tasksubcategory.category === "" || tasksubcategory.category === "Please Select Category") {
            setShowAlert(
                <>
    ...
    const sendEditRequest = async () => {
        try {
            ...
            await fetchTasksubcategory();getOverallEditSectionUpdate();
            handleCloseModEdit();
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: 'orange' }} />
    ...
    const editSubmit = (e) => {
        ...
        if (tasksubcategoryEdit.category === "" || tasksubcategoryEdit.category === "Please Select Category") {
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon
                        sx={{ fontSize: "100px", color: "orange" }}
    ...
    // Excel
    const fileName = "Task_Subcategory";

    const [tasksubcategoryData, setTasksubcategroyData] = useState([]);
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: 'Task_Subcategory',
        pageStyle: 'print'
    });
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Task Subcategory</Typography>
            ...
            <Box>
                ...
                >
                    ...
                    <DialogActions>
                        ...
                        <Button autoFocus variant="contained" color='error'
                            onClick={(e) => delTaskSubcategory(tasksubcategorysid)}
                        > OK </Button>
                    </DialogActions>
                </Dialog>
                ...
                >
                    <Box sx={{ width: '550px', padding: '20px 50px' }}>
                        <>
                            <Typography sx={userStyle.HeaderText}>Task Subcategory Info</Typography>
                            <br /><br />
                            <Grid container spacing={2}>
                                <Grid item md={12} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                            ...
                            <Grid item md={4} xs={12} sm={12} >
                                <FormControl fullWidth size="small">
                                    ...
                                    <Typography sx={{
                                        ...
                                        whiteSpace: "pre-wrap",
                                    }}>{tasksubcategoryEdit.description}</Typography>
                                </FormControl>
                            </Grid>
                    ...
                    <DialogActions>
                        {selectedRowsCount > 0 ?
                            <>
                                ...
                                <Button autoFocus variant="contained" color='error'
                                    onClick={(e) => delTaskSubcheckbox(e)}
                                > OK </Button>
                            </>
                            :
