...
import { Box, InputAdornment, Button, Checkbox, TextareaAutosize, Dialog, DialogActions, DialogContent, FormControl, Grid, IconButton, List, ListItem, ListItemText, MenuItem, OutlinedInput, Popover, Select, Table, TableBody, TableHead, TextField, Typography } from '@mui/material';
...
function StockManagement() {
  ...
  const { isUserRoleCompare, isAssignBranch, pageName, setPageName, isUserRoleAccess, buttonStyles, allCompany, allBranch, allUnit, allTeam } = useContext(UserRoleAccessContext);
  ...
  //alert model for vendor details
  const [openviewalertvendorstock, setOpenviewalertvendrostock] = useState(false);
  // view model
      ...
      if (stockManage.requestmode === 'Asset Material') {
        ...
        let singlehandtotal = singlehand.reduce((acc, current) => {
          const existingItemIndex = acc.findIndex(
            (item) => item.company === current.company && item.branch === current.branch && item.unit === current.unit && item.floor === current.floor && item.area === current.area && item.location === current.location && item.productname === current.productname
          );
        ...
        let merge = getfilter.map((item) => {
          let matchItems = getassettotal.find((d) => item.company === d.company && item.branch === d.branch && item.unit === d.unit && item.floor === d.floor && item.area === d.area && item.location === d.location && item.productname === d.material);

          let findquantity = singlehandtotal.find((d) => item.company === d.company && item.branch === d.branch && item.unit === d.unit && item.floor === d.floor && item.area === d.area && item.location === d.location && item.productname === d.productname);

          let findreturnquantity = singlereturntotal.find((d) => item.company === d.company && item.branch === d.branch && item.unit === d.unit && item.floor === d.floor && item.area === d.area && item.location === d.location && item.productname === d.productname);

          let matchHandItems = singlehandtotal.length > 0 ? (findquantity ? findquantity.countquantity : 0) : 0;
          let matchReturnItems = singlereturntotal.length > 0 ? (findreturnquantity ? findreturnquantity.countquantity : 0) : 0;

          if (matchItems || matchHandItems || matchReturnItems) {
            return {
              ...item,
              purchasecount: Number(item.purchasecount),
              usedcount: Number(matchItems ? matchItems.countquantity : 0) + Number(matchHandItems) - Number(matchReturnItems),
              balancedcount: Number(item.purchasecount) - (Number(matchItems ? matchItems.countquantity : 0) + Number(matchHandItems) - Number(matchReturnItems)),
        ...
        let getstocktotal = resultstock.reduce((acc, current) => {
          const existingItemIndex = acc.findIndex(
            (item) => item.company === current.company && item.branch === current.branch && item.unit === current.unit && item.floor === current.floor && item.area === current.area && item.location === current.location && item.materialnew === current.materialnew
          );
        ...
        let merge = getfilter.map((item) => {
          let matchItems = getstocktotal.find((d) => item.company === d.company && item.branch === d.branch && item.unit === d.unit && item.floor === d.floor && item.area === d.area && item.location === d.location && item.materialnew === d.materialnew);
          ...
          if (matchItems) {
            return {
              ...
              balancedcount: Number(item.purchasecountstock) - Number(matchItems.quantitynew),
            };
          } else {
            return {
              ...item,
  ...
  const fetchArea = async (e) => {
    try {
      ...
      let result = res_type.data.areagroupings.filter((d) => d.branch === stockManagehand.branch && d.floor === e).map((data) => data.area);
  ...
  const fetchLocation = async (e) => {
    try {
      ...
      let result = res_type.data.locationgroupings.filter((d) => d.branch === stockManagehand.branch && d.floor === stockManagehand.floor && d.area === e).map((data) => data.location);
      let ji = [].concat(...result);
      const all = [
        { label: 'ALL', value: 'ALL' },
        ...ji.map((d) => ({
  ...
  const getCode = async (company, branch, unit, floor, area, location, productname, assettype, asset, component) => {
    try {
      setStockedit({
        ...stockedit,
        company: company,
  ...
  const getCodeStockLog = async (company, branch, unit, floor, area, location, material) => {
    try {
      ...
      let stockviewlog = resultstock.filter((item) => item.materialnew === material);
  ...
  const getCodeAssetLog = async (company, branch, unit, floor, area, location, productname) => {
    try {
  ...
  const getCodeAssetReturnLog = async (company, branch, unit, floor, area, location, productname) => {
    try {
      ...
      let reshandeeturn = await axios.post(SERVICE.STOCKPURCHASELIMITED_HAND_TODO_RETURN, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        company: company,
      ...
      let getfilterstockreturn = reshandeeturn.data.stock.reduce((acc, current) => {
        const existingItemIndex = acc.findIndex((item) => item.employeenameto === current.employeenameto);
      ...
      let merge = getfilterstock.map((item) => {
        let matchItems = getfilterstockreturn.find((d) => item.employeenameto === d.employeenameto);
        if (matchItems) {
          return {
            ...item,
            countquantity: Number(item.countquantity) - Number(matchItems.countquantity),
            returnqty: 0,
          };
        } else {
          return {
  ...
  //Edit model...
  const handleClickOpenEdit = (company, branch, unit, floor, area, location, productname, balancedcount, usedcount) => {
    setIsEditOpen(true);
  ...
  //return model...
  const handleClickOpenEditReturn = (company, branch, unit, floor, area, location, productname, balancedcount, handovercount) => {
  ...
  const columnDatatable = [
        ...
        <Grid sx={{ display: 'flex' }}>
          {params.data.requestmode === 'Asset Material' && (
            <Button
              ...
              color="primary"
              onClick={() => {
                getCode(params.data.company, params.data.branch, params.data.unit, params.data.floor, params.data.area, params.data.location, params.data.productname, params.data.assettype, params.data.asset, params.data.component);
              }}
            >
              Assign
            </Button>
...

    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: 'flex' }}>
          {params.data.requestmode === 'Asset Material' && params.data.balancedcount > 0 && (
            <Grid sx={{ display: 'flex', gap: '2px' }}>
              ...
              <Button variant="contained" color="primary" size="small" onClick={() => handleClickOpenEdit(params.data.company, params.data.branch, params.data.unit, params.data.floor, params.data.area, params.data.location, params.data.productname, params.data.balancedcount, params.data.usedcount)}>
                Allot
              </Button>
            </Grid>
          )}
          ...
          {params.data.requestmode === 'Asset Material' && params.data.balancedcount > 0 && (
            <Grid sx={{ display: 'flex' }}>
              ...
              <Button
                ...
                onClick={(e) => {
                  getCodeAssetReturnLog(params.data.company, params.data.branch, params.data.unit, params.data.floor, params.data.area, params.data.location, params.data.productname);
                }}
              >
                Return Count
              </Button>
        ...
        <Grid sx={{ display: 'flex' }}>
          {params.data.requestmode === 'Asset Material' && (
            <Button
              ...
              onClick={(e) => {
                getCodeAssetLog(params.data.company, params.data.branch, params.data.unit, params.data.floor, params.data.area, params.data.location, params.data.productname);
              }}
            >
        ...
        <Grid sx={{ display: 'flex' }}>
          {params.data.requestmode === 'Stock Material' && (
            <Button
              sx={userStyle.buttonedit}
              onClick={(e) => {
                getCodeStockLog(params.data.company, params.data.branch, params.data.unit, params.data.floor, params.data.area, params.data.location, params.data.productname);
              }}
            >
              {/* <VisibilityOutlinedIcon
                                style={{ fontsize: "large" }}
  ...
  let columnsnew = stockManage.requestmode === 'Stock Material' ? columnDatatableStock : columnDatatable;
  ...
  const filteredColumns = columnsnew.filter((column) => column.headerName.toLowerCase().includes(searchQueryManage.toLowerCase()));
  ...
  const sendRequestStockReturn = async () => {
    let postdata = todoscheck.filter((item) => item.returnqty != 0 && item.returnqty != '');
    try {
  ...
  const [isManageColumnsOpenviewasset, setManageColumnsOpenviewasset] = useState(false);
  ...
  const [searchQueryManageViewasset, setSearchQueryManageViewasset] = useState('');
  ...
  const [columnVisibilityViewasset, setColumnVisibilityViewasset] = useState(initialColumnVisibilityViewasset);
  ...
  const filteredDatasViewasset = itemsViewasset?.filter((item) => {
    return searchTermsViewasset.every((term) => Object.values(item).join(' ').toLowerCase().includes(term));
  });

  const filteredDataViewasset = filteredDatasViewasset?.slice((pageViewasset - 1) * pageSizeviewasset, pageViewasset * pageSizeviewasset);

  const totalPagesViewasset = Math.ceil(filteredDatasViewasset?.length / pageSizeviewasset);

  const visiblePagesviewasset = Math.min(totalPagesViewasset, 3);

  const firstVisiblePageviewasset = Math.max(1, pageViewasset - 1);
  const lastVisiblePageViewasset = Math.min(firstVisiblePageviewasset + visiblePagesviewasset - 1, totalPagesViewasset);

  const pageNumbersViewasset = [];

  const indexOfLastItemviewasset = pageViewasset * pageSizeviewasset;
  const indexOfFirstItemViewasset = indexOfLastItemviewasset - pageSizeviewasset;
  ...
  const columnDataTableviewasset = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Typography
          sx={{
            backgroundColor: params.data.handover === 'handover' ? '#53d2cb' : params.data.handover === 'return' ? '#ffa50099' : '#c88695',
            borderRadius: '4px',
            padding: '4px',
          }}
        >
          {params.data.handover === 'handover' ? 'Handover' : params.data.handover === 'return' ? 'Return' : 'Assign'}
        </Typography>
      ),
    },
    {
  ...
  useEffect(() => {
    // Retrieve column visibility from localStorage (if available)
    const savedVisibilityViewasset = localStorage.getItem('columnVisibilityasset');
  ...
  useEffect(() => {
    // Save column visibility to localStorage whenever it changes
    localStorage.setItem('columnVisibilityViewasset', JSON.stringify(columnVisibilityViewasset));
  }, [columnVisibilityViewasset]);

  // // Function to filter columns based on search query
  const filteredColumnsViewasset = columnDataTableviewasset.filter((column) => column.headerName.toLowerCase().includes(searchQueryManageViewasset.toLowerCase()));
  ...
  const manageColumnsContentviewasset = (
    ...
    >
      ...
      <Box sx={{ position: 'relative', margin: '10px' }}>
        <TextField label="Find column" variant="standard" fullWidth value={searchQueryManageViewasset} onChange={(e) => setSearchQueryManageViewasset(e.target.value)} sx={{ marginBottom: 5, position: 'absolute' }} />
      </Box>
      <br />
      <br />
      <DialogContent sx={{ minWidth: 'auto', height: '200px', position: 'relative' }}>
        <List sx={{ overflow: 'auto', height: '100%' }}>
          {filteredColumnsViewasset.map((column) => (
            <ListItem key={column.field}>
              <ListItemText
                sx={{ display: 'flex' }}
                primary={<Switch sx={{ marginTop: '-5px' }} size="small" checked={columnVisibilityViewasset[column.field]} onChange={() => toggleColumnVisibilityviewasset(column.field)} />}
                secondary={column.field === 'checkbox' ? 'Checkbox' : column.headerName}
                // secondary={column.headerName }
              />
            </ListItem>
      ...
      <DialogActions>
        <Grid container>
          <Grid item md={4}>
            <Button variant="text" sx={{ textTransform: 'none' }} onClick={() => setColumnVisibilityViewasset(initialColumnVisibilityViewasset)}>
              Show All
            </Button>
          </Grid>
          <Grid item md={4}></Grid>
  ...
  const [searchQueryViewassetreturn, setSearchQueryViewassetreturn] = useState('');
  const [isManageColumnsOpenviewassetreturn, setManageColumnsOpenviewassetreturn] = useState(false);
  ...
  const [selectedRowsViewassetreturn, setSelectedRowsViewassetreturn] = useState([]);
  const [searchQueryManageViewassetreturn, setSearchQueryManageViewassetreturn] = useState('');
  ...
  const [columnVisibilityViewassetreturn, setColumnVisibilityViewassetreturn] = useState(initialColumnVisibilityViewassetreturn);

  const [itemsViewassetreturn, setItemsViewassetreturn] = useState([]);

  const addSerialNumberViewassetreturn = () => {
    const itemsWithSerialNumberViewassetreturn = assetlog?.map((item, index) => ({
      ...item,
      serialNumber: index + 1,
      // addedbyname: (item.addedby[0].empname),
      addedby: moment(item.addedby[0]?.date).format('DD-MM-YYYY hh:mm:ss a'),
  ...
  // Split the search query into individual terms
  const searchTermsViewassetreturn = searchQueryViewassetreturn.toLowerCase().split(' ');
  // Modify the filtering logic to check each term
  const filteredDatasViewassetreturn = itemsViewassetreturn?.filter((item) => {
    return searchTermsViewassetreturn.every((term) => Object.values(item).join(' ').toLowerCase().includes(term));
  });

  const filteredDataViewassetreturn = filteredDatasViewassetreturn?.slice((pageViewassetreturn - 1) * pageSizeviewassetreturn, pageViewassetreturn * pageSizeviewassetreturn);

  const totalPagesViewassetreturn = Math.ceil(filteredDatasViewassetreturn?.length / pageSizeviewassetreturn);
  ...
  const lastVisiblePageViewassetreturn = Math.min(firstVisiblePageviewassetreturn + visiblePagesviewassetreturn - 1, totalPagesViewassetreturn);

  const pageNumbersViewassetreturn = [];

  const indexOfLastItemviewassetreturn = pageViewassetreturn * pageSizeviewassetreturn;
  const indexOfFirstItemViewassetreturn = indexOfLastItemviewassetreturn - pageSizeviewassetreturn;

  for (let i = firstVisiblePageviewassetreturn; i <= lastVisiblePageViewassetreturn; i++) {
  ...
  const rowDataTableViewassetreturn = filteredDataViewassetreturn.map((item, index) => {
    return {
      id: item.serialNumber,
      serialNumber: item.serialNumber,
      ids: item._id,
  ...
  const rowsWithCheckboxesViewassetreturn = rowDataTableViewassetreturn.map((row) => ({
    ...row,
    // Create a custom field for rendering the checkbox
  ...
  useEffect(() => {
    // Retrieve column visibility from localStorage (if available)
    const savedVisibilityViewassetreturn = localStorage.getItem('columnVisibilityassetreturn');
    if (savedVisibilityViewassetreturn) {
      setColumnVisibilityViewassetreturn(JSON.parse(savedVisibilityViewassetreturn));
    }
  }, []);

  useEffect(() => {
    // Save column visibility to localStorage whenever it changes
    localStorage.setItem('columnVisibilityViewassetreturn', JSON.stringify(columnVisibilityViewassetreturn));
  }, [columnVisibilityViewassetreturn]);

  // // Function to filter columns based on search query
  const filteredColumnsViewassetreturn = columnDataTableviewassetreturn.filter((column) => column.headerName.toLowerCase().includes(searchQueryManageViewassetreturn.toLowerCase()));
  ...
  return (
    <Box>
      ...
      <PageHeading title="Manage Stock Management" modulename="Asset" submodulename="Stock" mainpagename="Stock Management" subpagename="" subsubpagename="" />
          ...
          <Box sx={userStyle.selectcontainer}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>Add Stock Management</Typography>
                </Grid>
              </Grid>
              <br />
              <Grid container spacing={2}>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      ...
                      // }))}
                      options={accessbranch
                        ?.filter((comp) => stockManage.companyto === comp.company)
                        ?.map((data) => ({
                          label: data.branch,
                          value: data.branch,
                        }))
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      ...
                      // }))}
                      options={accessbranch
                        ?.filter((comp) => selectedBranchTo.map((item) => item.value).includes(comp.branch))
                        ?.map((data) => ({
                          label: data.unit,
                          value: data.unit,
                        }))
      ...
      {isUserRoleCompare?.includes('lstockmanagement') && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>Stock Management List</Typography>
            </Grid>
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              <Grid item md={2} xs={12} sm={12}>
                <AggregatedSearchBar columnDataTable={columnsnew} setItems={setItems} addSerialNumber={addSerialNumber} setPage={setPage} maindatas={stock} setSearchedString={setSearchedString} searchQuery={searchQuery} setSearchQuery={setSearchQuery} paginated={false} totalDatas={stock} />
              </Grid>
            </Grid>
            <br />
      ...
      <Box>
        ...
        >
          <Box sx={{ padding: '20px 50px' }}>
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>Handover Count</Typography>
              </Grid>
              <br />
              <Grid container spacing={2}>
                <Grid item md={3} xs={12} sm={12}>
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      options={employeesall
                        ?.filter((comp) => stockManagehand.company === comp.company && stockManagehand.branch === comp.branch && stockManagehand.unit === comp.unit && stockManagehand.team === comp.team)
                        ?.map((com) => ({
                          ...com,
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      ...
                      onChange={(e) => {
                        setStockManagehand({
                          ...stockManagehand,
                          countquantity: e.target.value > 0 ? e.target.value : 0,
                        });
                      }}
                    />
                  </FormControl>
      ...
      <Box>
        ...
        >
          <Box sx={{ padding: '20px 50px' }}>
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>Usage Count</Typography>
              </Grid>
              <br />
              <Grid container spacing={2}>
                <Grid item md={3} xs={12} sm={12}>
      ...
      <Box>
        {/* Edit DIALOG */}
        <Dialog open={isEditOpenReturn} onClose={handleCloseModEditReturn} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="lg" fullWidth={true}>
          <Box sx={{ padding: '20px ' }}>
            <>
              <Grid container spacing={2}>
              ...
              <Grid container spacing={2}>
                ...
                <Table>
                  <TableHead>
                    ...
                    <StyledTableCell sx={{ padding: '5px 10px !important' }}> {'Employee'}</StyledTableCell>
                    <StyledTableCell sx={{ padding: '5px 10px !important' }}> {'Actual Quantity'}</StyledTableCell>
                    <StyledTableCell sx={{ padding: '5px 10px !important' }}> {'Return Quantity'}</StyledTableCell>
                        ...
                        <>
                          {editingIndexcheck === index ? (
                            <StyledTableRow key={index}>
                              ...
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.company}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.branch}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.unit}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.floor}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.area}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.location}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.productname}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.employeenameto}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.countquantity}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}>
                                <OutlinedInput
                                  ...
                                  value={quantityedit}
                                  onChange={(e) => {
                                    handleChangephonenumberEdit(e, item.countquantity);
                                    // getHighestEmpcodeForBranchhigh(valuecateedit)
                                  }}
                                />
                              </StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {moment(item.addedby ? item.addedby[0]?.date : '').format('DD-MM-YYYY hh:mm:ss a')}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}>
                                <Button
                                  variant="contained"
                                  style={{
                            ...
                            <StyledTableRow>
                              ...
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.employeenameto}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.countquantity}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.returnqty}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}> {moment(item.addedby ? item.addedby[0]?.date : '').format('DD-MM-YYYY hh:mm:ss a')}</StyledTableCell>
                              <StyledTableCell sx={{ padding: '5px 10px !important' }}>
                                {' '}
                                <Button
              ...
              <Grid container spacing={2}>
                ...
                <br />
                <Grid item md={6} xs={12} sm={12}>
                  <Button sx={userStyle.btncancel} onClick={handleCloseModEditReturn}>
                    {' '}
                    Cancel{' '}
                  </Button>
                </Grid>
      ...
      <Dialog
        ...
        fullWidth={true}
      >
        <AssetDetails sendDataToParentUI={handleDataFromChildUIDeign} stockedit={stockedit} handleCloseviewalertvendor={handleCloseviewalertvendor} />
      </Dialog>
      <br />
      <Dialog
        ...
        fullWidth={true}
      >
        <Stockmaster sendDataToParentUIStock={handleDataFromChildUIDeignStock} openpop={!openviewalertvendorstock} stockmaterialedit={stockmaterialedit} handleCloseviewalertvendorstock={handleCloseviewalertvendorstock} />
      </Dialog>
      <Dialog open={openView} onClose={handleViewOpen} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" fullWidth={true} maxWidth="lg">
        <Box sx={{ padding: '20px' }}>
          {/* ******************************************************EXPORT Buttons****************************************************** */}
          <Grid container spacing={2}>
            <Grid item xs={8} md={8} sm={8}>
              <Typography sx={userStyle.importheadtext}>View Stock Material Log</Typography>
            </Grid>
      ...
      <Dialog open={openViewasset} onClose={handleViewOpenAsset} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" fullWidth={true} maxWidth="lg" sx={{ marginTop: '95px' }}>
        <Box sx={{ padding: '20px' }}>
          {/* ******************************************************EXPORT Buttons****************************************************** */}
          <Grid container spacing={2}>
            <Grid item xs={8} md={8} sm={8}>
              <Typography sx={userStyle.importheadtext}>View Asset Material Log</Typography>
            </Grid>
            <Grid item xs={4} md={4} sm={4}>
              <Box sx={{ display: 'flex', justifyContent: 'end', width: '100%' }}>
                <Button variant="contained" color="primary" onClick={handlViewCloseAsset}>
                  {' '}
                  Back{' '}
                </Button>
              </Box>
          ...
          <Grid style={userStyle.dataTablestyle}>
            ...
            <Box>
              <FormControl fullWidth size="small">
                <Typography>Search</Typography>
                <OutlinedInput id="component-outlined" type="text" value={searchQueryViewasset} onChange={handleSearchChangeviewasset} />
              </FormControl>
            </Box>
          </Grid>
          <br />
          ...
          <>
            ...
            <Box style={userStyle.dataTablestyle}>
              <Box>
                Showing {filteredDataViewasset.length > 0 ? (pageViewasset - 1) * pageSizeviewasset + 1 : 0} to {Math.min(pageViewasset * pageSizeviewasset, filteredDatasViewasset.length)} of {filteredDatasViewasset.length} entries
              </Box>
              <Box>
                <Button onClick={() => setPageViewasset(1)} disabled={pageViewasset === 1} sx={userStyle.paginationbtn}>
                  <FirstPageIcon />
                </Button>
                <Button onClick={() => handlePageChangeView(pageViewasset - 1)} disabled={pageViewasset === 1} sx={userStyle.paginationbtn}>
                  <NavigateBeforeIcon />
                </Button>

                {pageNumbersView?.map((pageNumberViewasset) => (
                  <Button key={pageNumberViewasset} sx={userStyle.paginationbtn} onClick={() => handlePageChange(pageNumberViewasset)} className={pageViewasset === pageNumberViewasset ? 'active' : ''} disabled={pageViewasset === pageNumberViewasset}>
                    {pageNumberViewasset}
                  </Button>
                ))}
                {lastVisiblePageViewasset < totalPagesViewasset && <span>...</span>}
                <Button onClick={() => handlePageChangeViewasset(pageViewasset + 1)} disabled={pageViewasset === totalPagesViewasset} sx={userStyle.paginationbtn}>
                  <NavigateNextIcon />
                </Button>
                <Button onClick={() => setPageView(totalPagesViewasset)} disabled={pageViewasset === totalPagesViewasset} sx={userStyle.paginationbtn}>
                  <LastPageIcon />
                </Button>
              </Box>
            </Box>
