...
function ResolverReasonmaster() {
  ...
  const downloadPdf = (isfilter) => {
    ...
    doc.save("Resolver_Reason_Master.pdf");
  };
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Resolver_Reason_Master.png");
        });
      });
    }
  };
...
const getOverallEditSectionOverallDelete = async (ids) => {
  ...
  const rowData = async (id, name) => {
    try {
      let res = await axios.get(
        `${SERVICE.RESOLVERREASONMASTER_SINGLE}/${id}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const getOverallEditSectionDelete = async (e) => {
    try {
      let res = await axios.post(
        SERVICE.OVERALL_RESOLVER_REASONMASTER_TICKET_DELETE,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      setGetOverallCountDelete(`This Resolver Reason data is linked in 
       ${res.data.raiseticket.length > 0 ? "Raise Ticket Master," : ""}s`);
      if (res?.data?.count > 0) {
  ...
  const sendRequest = async () => {
    ...
    try {
      let subprojectscreate = await axios.post(
        SERVICE.RESOLVERREASONMASTER_CREATE,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
    ...
    if (valueCat?.length == 0) {
      ...
      setShowAlert(
        <>
          ...
          />
          <p style={{ fontSize: "20px", fontWeight: 900 }}>
            {"Resolver Reason Master already exist!"}
          </p>
        </>
      );
      handleClickOpenerr();
  ...
  const sendEditRequest = async () => {
    try {
      let res = await axios.put(
        `${SERVICE.RESOLVERREASONMASTER_SINGLE}/${subprojectsid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  return (
    <Box>
      ...
      <Typography sx={userStyle.HeaderText}>Resolver Reason Master</Typography>
      {/* ****** Header Content ****** */}
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes("excelresolverreasonmaster") && (
                    ...
                    //         ?.map((t, i) => `${i + 1 + ". "}` + t)
                    //         .toString(),
                    //       Typereason: t.typereason,
                    //       Reasons: t?.reasons,
                    //       ReasonName: t?.namereason,
                    //     }))}
                    //     fileName={fileName}
                    //   />
                    // </>
