...
function DepartmentLog() {
  ...
  const getviewCode = async (e) => {
    ...
    try {
      ...
      fetchDepartmentChange(
        { value: res?.data?.suser?.department },
        res?.data?.suser?.doj
      );
  ...
  const editSubmit = (e) => {
    ...
    e.preventDefault();
    if (
      departmentlog.startdate !== "Please Select Startdate" && prevLogDates?.includes(departmentlog.startdate)
    ) {
  ...
  const fetchDepartmentChange = async (e, Doj) => {
    ...
    try {
      ...
      let monthSet = response.data.departmentdetails?.filter(
        (data) => data.department == e?.deptname
      );
      let monthSetEmpty = response.data.departmentdetails?.filter(
        (data) => data.department == e?.deptname
      );
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("vdepartmentlog") && (
            <Link
              to={`/updatepages/departmentloglist/${params.data.id}`}
              target="_blank"
