...
function AggridTable({ rowDataTable, columnDataTable, columnVisibility, page, setPage, pageSize, totalPages, setColumnVisibility,
    selectedRows, setSelectedRows, gridRefTable, totalDatas, rowHeight = 50, setFilteredRowData, setSelectedRowsRequestPurchase, selectedRowsRequestPurchase,
    filteredRowData, gridRefTableImg, itemsList, selectedRowsAssetList, setSelectedRowsAssetList, pagenamecheck, setSelectedRowsPenaltyerror,
    ...
    const handlePageChange = (newPage) => {
        ...
        if (pagenamecheck === "Penalty Error Upload") {
    ...
    const handleSelectionChange = () => {
        ...
        const selectedRowsRequestToPurchase = gridRefTable?.current?.api?.getSelectedRows();
        const selectedRowsPeanltyerrorupload = gridRefTable?.current?.api?.getSelectedRows();
        ...
        if (pagenamecheck === "Stock Request Purchase") {
            const extractedIdsRequestPurchase = selectedRowsRequestToPurchase.map((item) => item);
        ...
        if (pagenamecheck === "Penalty Error Upload") {
            const extractedIdsPenaltyerror = selectedRowsPeanltyerrorupload.map((item) => item);
