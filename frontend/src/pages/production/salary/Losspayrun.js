...
import { Box, Typography, Dialog, DialogContent, Checkbox, List, ListItem, ListItemText, Popover, TextField, IconButton, Select, OutlinedInput, FormControl, MenuItem, DialogActions, Grid, Paper, Table, TableHead, TableContainer, Button, TableBody } from "@mui/material";
        ...
        try {
          ...
          let result = filtered.flatMap((item, index) => {
            ...
            return {
              ...
              lopcalculation: getFinalLop(getAttModeLop(item.attendanceautostatus ? item.attendanceautostatus : getattendancestatus(updatedClockInStatus, updatedClockOutStatus)), getAttModeLopType(item.attendanceautostatus ? item.attendanceautostatus : getattendancestatus(updatedClockInStatus, updatedClockOutStatus))),
              lopcount: getCount(getFinalLop(getAttModeLop(item.attendanceautostatus ? item.attendanceautostatus : getattendancestatus(updatedClockInStatus, updatedClockOutStatus)), getAttModeLopType(item.attendanceautostatus ? item.attendanceautostatus : getattendancestatus(updatedClockInStatus, updatedClockOutStatus)))),
              ...
              paidpresent: getFinalPaid(getAttModePaidPresent(item.attendanceautostatus ? item.attendanceautostatus : getattendancestatus(updatedClockInStatus, updatedClockOutStatus)), getAttModePaidPresentType(item.attendanceautostatus ? item.attendanceautostatus : getattendancestatus(updatedClockInStatus, updatedClockOutStatus))),
              paidpresentday: getAssignLeaveDayForPaid(
                getFinalPaid(getAttModePaidPresent(item.attendanceautostatus ? item.attendanceautostatus : getattendancestatus(updatedClockInStatus, updatedClockOutStatus)), getAttModePaidPresentType(item.attendanceautostatus ? item.attendanceautostatus : getattendancestatus(updatedClockInStatus, updatedClockOutStatus)))
              ),
...
function Losspayrun() {
  ...
  const handleBulkDownloadExcel = async () => {
    ...
    if (selectedRows.length === 0) {
      ...
      downloadExcelBulk(finaldata, "Pay Run");
    }
  };

  const downloadExcelBulk = (data, filename) => {
  ...
  const columnDataTableCalcList = [
    {
      ...
      renderHeader: (params) => (
        <CheckboxHeader
          ...
          onSelectAll={() => {
            if (rowDataTableCalcList.length === 0) {
            ...
            if (selectAllChecked) {
              ...
              const allRowIds = rowDataTableCalcList.map((row) => row.id);
              setSelectedRows(allRowIds);
              const allRowMonths = rowDataTableCalcList.map((row) => row.month);
