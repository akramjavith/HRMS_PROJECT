...
exports.getAllDayPointsUploadLimited = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload;
  try {
    daypointsupload = await DayPointsUpload.find({}, { filename: 1, date: 1, type: 1 });
  } catch (err) {
    return next(new ErrorHandler("Records not found!", 404));
  }
  if (!daypointsupload) {
...
exports.dayPointsDatasFetch = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload, minpoints, productionupload, productionuploads, users, departments;
  try {
...
exports.getEmployeeProductionLastThreeMonths = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    daypointsupload = await DayPointsUpload.find({ date: { $gte: result.fromdate, $lte: result.todate } }, { uploaddata: 1, date: 1 });
...
exports.dayPointsfilterHome = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    if (daypointsupload.length > 0) {
      ...
      let answer = daypointsupload.flatMap((data) =>
        data.uploaddata.filter((item, index) => index <= 6)
          .map((upload) => ({
            companyname: upload.companyname,
            name: upload.name,
            empcode: upload.empcode,
      ...
      const filteredArray = answer.filter((item, index) => index <= 5).map((obj1) => {
        const splitDate = obj1.date.split("-");
        const oldyear = splitDate[0];
        const oldmonth = splitDate[1];
      ...
      const [
        ...
        RevenueAmount.find({ processcode: { $in: processcodes } }, { branch: 1, company: 1, processcode: 1, amount: 1 }),
      ]);
