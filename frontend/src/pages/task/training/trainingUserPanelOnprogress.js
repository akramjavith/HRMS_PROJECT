...
function TrainingForUserOnProgress({ com }) {
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "Training User Panel - OnProgress List.png");
                })
                .catch((error) => {
    ...
    const ListPageLoadDataOnProgress = async () => {
        ...
        try {
            ...
            const final = res_task?.data?.trainingforuser?.length > 0 ? res_task?.data?.trainingforuser?.filter(data => data?.username
                === isUserRoleAccess?.companyname && ["Paused", "Pending"]?.includes(data?.taskstatus) && data.taskdetails !== "Manual")?.map((item, index) => ({
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>User Training Panel - OnProgress List</Typography>
            ...
            <ExportData
                ...
                filename={"Training User Panel - OnProgress List"}
