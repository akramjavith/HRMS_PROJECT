...
function TempCategoryTimeLog() {
    ...
    let exportColumnNames = ["Branch Name", "From Date",
        // "To Date", 
        "User Name", "Create Date"];
    let exportRowValues = ["branch", "fromdate",
        // "todate", 
        "username", "createdAt"];
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "Temp Category Time Log.png");
                })
                .catch((error) => {
    ...
    const rowData = async (id, name) => {
        setPageName(!pageName)
        try {
            let res = await axios.get(`${SERVICE.SINGLE_TEMP_CATEGORY_TIME_LOG_}/${id}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            setDeletegroup(res?.data?.stempcategorytimelog);
            handleClickOpen();
        } catch (err) {
            handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
        }
    ...
    const deleGroup = async () => {
        setPageName(!pageName)
        try {
            await axios.delete(`${SERVICE.SINGLE_TEMP_CATEGORY_TIME_LOG_}/${groupEditt}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const delGroupcheckbox = async () => {
        ...
        try {
            const deletePromises = selectedRows?.map((item) => {
                return axios.delete(`${SERVICE.SINGLE_TEMP_CATEGORY_TIME_LOG_}/${item}`, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
                });
    ...
    const getCode = async (e, name) => {
        setPageName(!pageName)
        try {
            let res = await axios.get(`${SERVICE.SINGLE_TEMP_CATEGORY_TIME_LOG_}/${e}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            handleClickOpenEdit();
            setProductionEdit(res?.data?.stempcategorytimelog);
        } catch (err) {
            handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
        }
    };
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "Temp Category Time Log List",
        pageStyle: "print",
    });
...

        {
            ...
            cellRenderer: (params) => (
                <Grid sx={{ display: "flex" }}>
                    {/* {isUserRoleCompare?.includes("etempcategorytimelog") && (
                        <Button
                    ...
                    {isUserRoleCompare?.includes("dtempcategorytimelog") && (
                        <Button
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Temp Category Time Log"),
    ...
    return (
        <Box>
            <PageHeading
                ...
                subpagename="Temp Category Time Log"
                subsubpagename=""
            />

            {isUserRoleCompare?.includes("atempcategorytimelog") && (
                <>
                ...
                <>
                    <Box sx={userStyle.container}>
                        ...
                        <Grid container spacing={2} style={userStyle.dataTablestyle}>
                            ...
                            >
                                <Box>
                                    {isUserRoleCompare?.includes("exceltempcategorytimelog") && (
                                        <>
                                            <Button
                                                onClick={(e) => {
                                    ...
                                    {isUserRoleCompare?.includes("csvtempcategorytimelog") && (
                                        <>
                                            <Button
                                                onClick={(e) => {
                                                    setIsFilterOpen(true);
                                    ...
                                    {isUserRoleCompare?.includes("printtempcategorytimelog") && (
                                        <>
                                            <Button
                                                sx={userStyle.buttongrp}
                                                onClick={handleprint}
                                    ...
                                    {isUserRoleCompare?.includes("pdftempcategorytimelog") && (
                                        <>
                                            <Button
                                                sx={userStyle.buttongrp}
                                                onClick={() => {
                                    ...
                                    {isUserRoleCompare?.includes("imagetempcategorytimelog") && (
                                        <>
                                            <Button
                                                sx={userStyle.buttongrp}
                        ...
                        &ensp;
                        {isUserRoleCompare?.includes("bdtempcategorytimelog") && (
                            <Button
                                variant="contained"
            ...
            >
                <Box sx={{ width: "550px", padding: "20px 50px" }}>
                    <>
                        <Typography sx={userStyle.HeaderText}> View Temp Category Time Log</Typography>
                        <br /> <br />
                        <Grid container spacing={2}>
            ...
            <ExportData
                ...
                filename={"Temp Category Time Log"}
...
export default TempCategoryTimeLog;
