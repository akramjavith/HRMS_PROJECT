...
function Reasonmaster() {
  ...
  let exportColumnNames = ['Category', 'Subcategory', 'Sub Subcategory', 'Type', 'Reason'];
  let exportRowValues = [
    'categoryreason',
    'subcategoryreason',
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Ticket/Master/Reason Master"),
  ...
  const getOverallEditSectionDelete = async (e) => {
    ...
    try {
      ...
      setGetOverallCountDelete(`This Reason Data is linked in 
       ...
       ${res?.data?.check?.length > 0 ? "CheckPoint Master," : ""}
  ...
  const handleSubmit = (e) => {
    ...
    if (valueCat?.length == 0) {
      ...
      setPopupContentMalert('Reason Master already exists!');
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
  ...
  const fetchReasonmaster = async () => {
    ...
    try {
      ...
      const answers = res_vendor?.data?.reasonmasters?.length > 0 ? res_vendor?.data?.reasonmasters?.map((item, index) => ({
