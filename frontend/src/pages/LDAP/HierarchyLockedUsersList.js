...
function LockedUsersList() {
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Hierarchy Locked Users List"),
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, "Hierarchy Locked Users List.png");
        })
        .catch((error) => {
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Hierarchy Locked Users List",
    pageStyle: "print",
  });
...

    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("ehierarchylockeduserslist") &&
            (params?.data?.isLocked ? (
              <>
                <Button
                  ...
                  variant="contained"
                  onClick={() => {
                    unLockUserOne(params?.data?.username);
                  }}
                >
                  Unlock User
                </Button>
              ...
              <>
                {/* <Button
                  ...
                  variant="contained"
                  onClick={() => {
                    lockUserOne(params?.data?.username);
                  }}
                >
                  Lock User
                </Button> */}
          ...
          {isUserRoleCompare?.includes("vhierarchylockeduserslist") && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
  ...
  const editSubmit = (e) => {
    ...
    const isNameMatch = employees
      ?.filter((item) => item?.id !== editDetails.id)
      ?.some(
        (item) =>
          item.rocketchatusername?.trim().toLowerCase() ===
          editDetails.rocketchatusername?.trim().toLowerCase()
      );
    if (isNameMatch) {
      ...
      editDetails.rocketchatemail !== "" &&
      !isValidEmail(editDetails.rocketchatemail)
    ) {
  ...
  const handleSubmitOld = (e) => {
    ...
    const isNameMatch = employees?.some(
      (item) =>
        item.rocketchatusername?.trim().toLowerCase() ===
        addDetails.rocketchatusername?.trim().toLowerCase()
    );
    if (isNameMatch) {
      setPopupContentMalert("User Already Exist!");
      setPopupSeverityMalert("info");
      ...
      addDetails.rocketchatemail !== "" &&
      !isValidEmail(addDetails.rocketchatemail)
    ) {
      setPopupContentMalert("Please Enter Valid Email!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
  ...
  const sendRequest = async () => {
    ...
    try {
      setIsLoadingNew(true);
      const response = await axios.post(SERVICE.HIERARCHYBASEDLOCKEDUSERSLIST, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        ...
        pagename: 'menuhierarchylockeduserslist',
  ...
  const fetchRocketChatUsers = async () => {
    ...
    setTableLoader(true);
    try {
      let response = await axios.post(SERVICE.LOCKEDANDUNLOCKEDUSERSLIST, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  return (
    <Box>
      <NotificationContainer />
      <Headtitle title={"Hierarchy Locked Users List"} />
      {/* ****** Header Content ****** */}
      <PageHeading
        title="Hierarchy Locked Users List"
        modulename="LDAP"
        submodulename="Hierarchy Locked Users List"
        mainpagename=""
        subpagename=""
        subsubpagename=""
      />
      {isUserRoleCompare?.includes("ahierarchylockeduserslist") && (
        <>
          <Box sx={userStyle.selectcontainer}>
            <>
              <Typography sx={userStyle.importheadtext}>Hierarchy Locked Users Filter</Typography>
              <br />
      ...
      {tableLoader ? (
        ...
        <>
          {isUserRoleCompare?.includes("lhierarchylockeduserslist") && (
            <>
              <Box sx={userStyle.container}>
                {/* ******************************************************EXPORT Buttons****************************************************** */}
                <Grid container spacing={2}>
                ...
                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                  ...
                  >
                    <Box>
                      {isUserRoleCompare?.includes("excelhierarchylockeduserslist") && (
                        <>
                          <Button
                            onClick={(e) => {
                      ...
                      {isUserRoleCompare?.includes("csvhierarchylockeduserslist") && (
                        <>
                          <Button
                            onClick={(e) => {
                              setIsFilterOpen(true);
                      ...
                      {isUserRoleCompare?.includes("printhierarchylockeduserslist") && (
                        <>
                          <Button
                            sx={userStyle.buttongrp}
                            onClick={handleprint}
                      ...
                      {isUserRoleCompare?.includes("pdfhierarchylockeduserslist") && (
                        <>
                          <Button
                            sx={userStyle.buttongrp}
                            onClick={() => {
                      ...
                      {isUserRoleCompare?.includes("imagehierarchylockeduserslist") && (
                        <Button
                          sx={userStyle.buttongrp}
      ...
      <ExportData
        ...
        itemsTwo={employees ?? []}
        filename={"Hierarchy Locked Users List"}
        ...
        <Box sx={{ padding: "20px 50px" }}>
          <>
            <Typography sx={userStyle.HeaderText}>
              {" "}
              View Locked/Unlocked User Details
            </Typography>
            <br />
            <Grid container spacing={2}>
        ...
        >
          <Box sx={{ padding: "20px 50px" }}>
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>
                  Edit Locked/Unlocked User Details
                </Typography>
              </Grid>
              <br />
              <Grid container spacing={2}>
        ...
        >
          <Box sx={{ padding: "20px 50px" }}>
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>
                  Create New Locked/Unlocked User
                </Typography>
              </Grid>
              <br />
              <Grid container spacing={2}>
