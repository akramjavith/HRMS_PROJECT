...
const JobRolesPage = () => {
  ...
  const fetchAllApproveds = async () => {
    try {
      let res_queue = await axios.get(SERVICE.CLOSEDALLJOBOPENINGS, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });

      let result = res_queue?.data?.jobopenings?.filter((d) => d.remotejob == false);
      setJobOpening(result);

      let result1 = res_queue?.data?.jobopenings?.filter((d) => d.remotejob == true);
      setJobRemote(result1);

      // Remove duplicates from departments
      let uniqueDepartments = Array.from(new Set(res_queue?.data?.jobopenings.map((t) => t.department)));
      setDepartments(
      ...
      const uniqueJobTypes = Array.from(new Set(res_queue?.data?.jobopenings.map((t) => t.jobtype)));
      setJobTypes(
      ...
      const uniqueJobLocation = Array.from(new Set(res_queue?.data?.jobopenings.map((t) => `${t.city}, ${t.state}`))).filter((location) => location.trim() !== ",");
  ...
  return (
    <Box sx={{ background: "white", position: "relative" }}>
      ...
      <Box sx={{ position: "fixed", background: "white", width: "100%", height: "65px", padding: "0px", top: "0px", left: "0px", zIndex: 1 }}>
        <Header />{" "}
      </Box>{" "}
      <br /> <br />
      {isMobile ? (
        <>
          <Box>
            {isUserRoleCompare?.includes("lcareer") && (
              <>
                <Grid container spacing={4}>
                  ...
                  <Grid item sm={12} xs={12}>
                    <Box sx={{ display: "flex", justifyContent: "center" }}>
                      <img src={Jobimg} alt="leaf" style={{ width: "80%", marginTop: "-30px !important", "@media only screen and (max-width: 600px) and (min-width: 340px)": { margin: "0px auto" } }} />
                    </Box>
                  </Grid>
                </Grid>
              </>
              ...
              <Box sx={{ marginLeft: "125px" }}>
                <Typography
                  ...
                  sx={{
                    ...
                    fontWeight: 900,
                    fontFamily: "FiraSans !important",
                    "@media only screen and (max-width: 600px) and (min-width: 200px)": { fontSize: "28px !important", fontWeight: 900 },
                  }}
                >
                  {" "}
                  Join Us{" "}
                ...
                <br />
                <Box>
                  <Typography variant="subtitle2" className="cardcntleft" sx={{ color: "#000001 !important", lineHeight: "30px !important", fontFamily: "FiraSansRegular !important", fontSize: "22px !important", "@media only screen and (max-width: 1010px) and (min-width: 600px)": { fontSize: "17px !important" }, "@media only screen and (max-width: 600px) and (min-width: 340px)": { fontSize: "16px !important" }, "@media only screen and (max-width: 339px) and (min-width: 199px)": { fontSize: "15px !important" } }}>
                    Accelerate your career with {overallSettings?.companyname}, that gives you the platform to do amazing things. It's more than just a job, it's an opportunity to grow.{" "}
                  </Typography>{" "}
                  <br />
                </Box>
                  ...
                  {departmentData.data.map((row, innerIndex) => (
                    <Grid container key={innerIndex}>
                      <Grid item lg={12} md={12} sm={12} xs={12}>
                        <Link to={`/appcareer/jobdescriptions/${row.recruitmentname}/${row._id}`} style={{ color: "#000001 ", textDecoration: "none" }}>
                          <Grid container sx={{ border: "2px solid #8080801f", borderRadius: "5px", padding: "20px", cursor: "pointer", "&:hover": { background: "#1976d20f" } }}>
                            ...
                            <Grid item lg={3} md={3} sm={5} xs={12} sx={{ display: "flex" }}>
                              <Typography sx={{ fontFamily: "FiraSansRegular !important", fontSize: "16px", letterSpacing: "0.5px" }}>
                                {row.city + "," + " " + row.state} <br /> Work Type: {row.jobtype}{" "}
                              </Typography>{" "}
                              &ensp;
  ...
  return (
    <Box>
      <Box sx={{ width: "100%", overflowX: "hidden" }}>
        <Box component="main" sx={{ maxWidth: "1600px", margin: "0 auto", padding: "0px !important" }}>
          <JobRolesPage />
        </Box>
      </Box>
    </Box>
