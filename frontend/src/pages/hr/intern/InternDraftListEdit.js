...
function InternDraftEdit() {
  ...
  const rowData = async () => {
    try {
      ...
      let isThere = res?.data?.sdraft?.attendancemode
        ? res?.data?.sdraft?.attendancemode?.map((data) => ({
          ...data,
          label: data,
          value: data,
        }))
      ...
      setValueAttMode(
        res?.data?.sdraft?.attendancemode
          ? res?.data?.sdraft?.attendancemode.map((data) => data)
          : []
      );
      ...
      var filteredWorks;
      if (res?.data?.sdraft?.unit === "" && res?.data?.sdraft?.floor === "") {
        filteredWorks = workStationOpt?.filter(
          (u) =>
            u.company === res?.data?.sdraft?.company &&
            u.branch === res?.data?.sdraft?.branch
        );
      } else if (res?.data?.sdraft?.unit === "") {
        filteredWorks = workStationOpt?.filter(
          (u) =>
            u.company === res?.data?.sdraft?.company &&
            u.branch === res?.data?.sdraft?.branch &&
            u.floor === res?.data?.sdraft?.floor
        );
      } else if (res?.data?.sdraft?.floor === "") {
        filteredWorks = workStationOpt?.filter(
          (u) =>
            u.company === res?.data?.sdraft?.company &&
            u.branch === res?.data?.sdraft?.branch &&
            u.unit === res?.data?.sdraft?.unit
        );
      } else {
        filteredWorks = workStationOpt?.filter(
          (u) =>
      ...
      let primaryWorkstationNew = res?.data?.sdraft?.workstation[0];
      let findLabel = workstationsFinal?.find((item) =>
      ...
      const wfhcount = res?.data?.sdraft?.wfhcount || 0;
  ...
  const handleSubmitAddtodo = (e) => {
    ...
    const Namematch = addAddQuaTodo.some(
      (data, index) =>
        ...
        data.addInst?.trim()?.toLowerCase() == addInst?.trim()?.toLowerCase() &&
        data.duration?.trim()?.toLowerCase() ==
        duration?.trim()?.toLowerCase() &&
        data.remarks?.trim()?.toLowerCase() == remarks?.trim()?.toLowerCase()
    );
    e.preventDefault();
  ...
  const handleSubmitWorkSubmit = (e) => {
    ...
    const isDuplicate = workhistTodo?.some(
      (entry) =>
        entry.empNameTodo?.trim()?.toLowerCase() ===
        empNameTodo?.trim()?.toLowerCase() &&
        entry.desigTodo?.trim()?.toLowerCase() ===
        desigTodo?.trim()?.toLowerCase() &&
        entry.joindateTodo === joindateTodo &&
        entry.leavedateTodo === leavedateTodo &&
        entry.dutiesTodo?.trim()?.toLowerCase() ===
        dutiesTodo?.trim()?.toLowerCase() &&
        entry.reasonTodo?.trim()?.toLowerCase() ===
        reasonTodo?.trim()?.toLowerCase()
    );
  ...
  const fetchDepartmentSingle = async (department) => {
    try {
      ...
      let production = req.data.departmentdetails.find(
        (d) => d.deptname === department
      )?.prod;
  ...
  const fetchHandlerEdit = async () => {
    try {
      ...
      const domainsArray = companyEmailDomain
        ?.split(",")
        .map((domain) => domain.trim());
      let branchCode = await fetchbranchCodes(
      ...
      let newEmpCode = await EmployeeCodeAutoGenerate(
        ...
        response?.data?.sdraft?.branch || "",
        branchCode || "",
        response?.data?.sdraft?.doj || moment().format("YYYY-MM-DD")
      );
      setNewval(newEmpCode);
      setCreateRocketChat({
        create: response?.data?.sdraft?.createrocketchat || false,
        email: response?.data?.sdraft?.rocketchatemail || "",
        roles: response?.data?.sdraft?.rocketchatroles || [],
      });
      let usernames = response?.data?.sdraft?.username?.toLowerCase();
      // Check if the domainsArray has any domains
      const companyEmails =
        domainsArray?.length > 0
          ? domainsArray.map((domain) => `${usernames}@${domain}`).join(",")
      ...
      setOldUserCompanyname(response?.data?.sdraft);
      setBankTodo(
      ...
      let isThere = response?.data?.sdraft?.attendancemode
        ? response?.data?.sdraft?.attendancemode?.map((data) => ({
          ...data,
          label: data,
          value: data,
        }))
      ...
      setSelectedAttMode(isThere);
      setValueAttMode(
        response?.data?.sdraft?.attendancemode
          ? response?.data?.sdraft?.attendancemode.map((data) => data)
          : []
      );
      fetchSuperVisorDropdowns(
        response?.data?.sdraft?.team,
      ...
      let designationGrpName = alldesignation?.find(
        (data) =>
          response?.data?.sdraft?.designationlog[0]?.designation === data?.name
      )?.group;
      ...
      let allDesignations = alldesignation
        ...
        ?.map((item) => item?.name);
      setOldDesignation(response?.data?.sdraft?.designationlog[0]?.designation);
      ...
      if (response?.data?.sdraft?.assignExpLog?.lenth === 0) {
        setAssignExperience({
          ...assignExperience,
          updatedate: response?.data?.sdraft?.doj,
        });
      } else {
        setAssignExperience({
          ...assignExperience,
          assignExpMode:
            response?.data?.sdraft?.assignExpLog[0]?.expmode ||
            "Auto Increment",
          assignExpvalue: response?.data?.sdraft?.assignExpLog[0]?.expval || 0,
          assignEndExpDate:
            response?.data?.sdraft?.assignExpLog[0]?.endexpdate !== ""
              ? moment(
                response?.data?.sdraft?.assignExpLog[0]?.endexpdate
              ).format("YYYY-MM-DD")
              : "",
          assignEndTarDate:
            response?.data?.sdraft?.assignExpLog[0]?.endtardate !== ""
              ? moment(
                response?.data?.sdraft?.assignExpLog[0]?.endtardate
              ).format("YYYY-MM-DD")
              : "",
          assignEndTarvalue: response?.data?.sdraft?.assignExpLog[0]?.endtar,
          assignEndExpvalue: response?.data?.sdraft?.assignExpLog[0]?.endexp,
          updatedate:
            response?.data?.sdraft?.assignExpLog[0]?.updatedate !== ""
              ? moment(
                response?.data?.sdraft?.assignExpLog[0]?.updatedate
              ).format("YYYY-MM-DD")
              : "",
      ...
      let secondaryWorkstation = Array.isArray(
      ...
      let isProd = false;
      if (response?.data?.sdraft?.department) {
        isProd = await fetchDepartmentSingle(
          response?.data?.sdraft?.department
        );
      }
      setEmployee({
        ...savedEmployee,
        ...
        prod: isProd,
        profileimage: response?.data?.sdraft?.profileimage || "",
        ...
        callingname:
          savedEmployee?.callingname === ""
            ? savedEmployee?.firstname?.includes(" ")
              ? savedEmployee?.firstname?.split(" ")[0]
      ...
      setValueCate(
        response?.data?.sdraft?.boardingLog[0]?.weekoff
          ? response?.data?.sdraft?.boardingLog[0]?.weekoff
          : []
      );
      setOldData({
  ...
  // let newval1 = `${selectedBranchCode?.toString() || ""
  //   }INT${currYear}${currMonth}${currDay}0001`;
  ...
  const fetchDepartments = async () => {
    try {
      ...
      let filteredDepartments = req?.data?.departmentdetails?.filter((obj) =>
        obj.deptname.toLowerCase().includes("intern")
      );
      setDepartment(
  ...
  const addReferenceTodoFunction = () => {
    const isNameMatch = referenceTodo?.some(
      (item) =>
        ...
        singleReferenceTodo.name?.trim()?.toLowerCase()
    );
    const newErrorsLog = {};
  ...
  const handleBranchChange = (event) => {
    ...
    debouncedEmployeeCodeAutoGenerate(
      ...
      selectedBranch,
      event.code,
      employee?.doj || moment().format("YYYY-MM-DD")
    );

    var newEmpCode =
      branchCode[0].code.substring(0, 2) + employee.empcode.substring(2);

    // Update the branch input value with the new branch code
    employee.empcode = newEmpCode;
  ...
  const SendDraftEdit = async () => {
    ...
    try {
      ...
      let employees_draft = await axios.put(`${SERVICE.DRAFT_SINGLE}/${id}`, {
        ...
        dot: String(employee.doj), //saved doj on dot for purpose
        ...
        boardingLog: [
          {
            ...
            shiftgrouping: String(employee.shiftgrouping),
            logeditedby: [],
            updateddatetime: String(new Date(employee?.doj)),
  ...
  const nextStep = (employeenameduplicate) => {
    ...
    if (!final) {
      newErrors.profile = (
        <Typography style={{ color: "red" }}>
          Profile Image must be required
        </Typography>
      );
    }
  ...
  const nextStepLog = () => {
    ...
    if (employee?.intStartDate === "" || !employee?.intStartDate) {
      newErrorsLog.internstartdate = (
        <Typography style={{ color: "red" }}>
          Intern Start Date must be required
        </Typography>
      );
      missingFieldstwo.push("Intern Start Date");
    }
    if (employee?.intStartDate === "" || !employee?.intStartDate) {
      newErrorsLog.intEndDate = (
        <Typography style={{ color: "red" }}>
          Intern End Date must be required
        </Typography>
      );
    ...
    if (!employee.doj) {
      newErrorsLog.doj = (
        <Typography style={{ color: "red" }}>DOT must be required</Typography>
      );
    ...
    //     </Typography>
    //   );
    //   missingFieldstwo.push("Vaild COmpany Email");
    // }
    ...
    if (
      employee.wordcheck === true &&
      newval?.toLowerCase() === employee.empcode?.toLowerCase()
    ) {
      newErrorsLog.empcode = (
        <Typography style={{ color: "red" }}>
          Empcode Auto and Manual Cant be Same
        </Typography>
      );
  ...
  const handleSubmitMulti = (e) => {
    ...
    if (createRocketChat?.create && createRocketChat?.email === "") {
      newErrorsLog.rocketchatemail = (
        <Typography style={{ color: "red" }}>Please Select Email</Typography>
      );
    ...
    if (createRocketChat?.create && createRocketChat?.roles?.length === 0) {
      newErrorsLog.rocketchatrole = (
        <Typography style={{ color: "red" }}>Please Select Role</Typography>
      );
  ...
  const createUser = async () => {
    ...
    try {
        ...
        if (uniqueEntries?.length > 0) {
          for (const item of uniqueEntries) {
            const res_queue = await axios.post(SERVICE.HIRERARCHI_CREATE, {
              ...
              empcode: String(
                employee.wordcheck === false ? newEmpCode : employee.empcode
              ),
      ...
      const paswordupdateday =
        resSetting?.data?.overallsettings[0]?.passwordupdatedays || "";
      ...
      let employees_data = await axios.post(SERVICE.USER_CREATE, {
        ...
        emergencyno: String(employee.emergencyno),
        doj: String(employee?.doj),
        dot: String(employee.doj), //saved for purpose
        aadhar: String(employee.aadhar),
        panno: String(employee.panstatus === "Have PAN" ? employee.panno : ""),
        panstatus: String(employee.panstatus),
        panrefno: String(
        ...
        passexpdate: new Date(
          new Date().setDate(new Date().getDate() + Number(paswordupdateday))
        ),
      ...
      await axios.delete(`${SERVICE.DRAFT_SINGLE}/${id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const renderStepOne = () => {
    return (
      <>
        <Headtitle title={"INTERN DRAFT EDIT"} />
                      ...
                      <Grid item md={9} sm={9} xs={9}>
                        <FormControl size="small" fullWidth>
                          <OutlinedInput
                            ...
                            onChange={(e) => {
                              const cname = e?.target?.value?.includes(" ")
  ...
  const ShiftTypeOptions = [
    ...
    { label: "1 Month Rotation", value: "1 Month Rotation" },
  ];
                ...
                <Grid item md={4} sm={6} xs={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      id="component-outlined"
                      type="text"
                      placeholder="Please Enter Work Mode"
                  ...
                  <Grid item md={4} sm={12} xs={12}>
                    <Grid container spacing={2}>
                      <Grid item md={6} sm={12} xs={12}>
                        ...
                        {errorsLog.internstartdate && (
                          <div>{errorsLog.internstartdate}</div>
                        )}
                      </Grid>
                      <Grid item md={6} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                        ...
                        {errorsLog.internenddate && (
                          <div>{errorsLog.internenddate}</div>
                        )}
                      </Grid>
                    </Grid>
                  </Grid>
                  ...
                  <>
                    ...
                    <Grid item md={3.5} sm={12} xs={12}>
                      <FormControl size="small" fullWidth>
                        ...
                        <Selects
                          options={filteredWorkStation.filter(
                            (item, index, self) => {
                              return (
                                index ===
  ...
  const handleSalaryfix = (
    ...
    let findexp = monthSets.find(
      (d) =>
        ...
        new Date(todayNow) >= new Date(d.fromdate) &&
        new Date(todayNow) <= new Date(d.todate)
    );
    let findDate = findexp ? findexp.fromdate : curMonStartDate;
                  ...
                  <>
                    {assignExperience.assignExpMode === "Exp Stop" ||
                      ...
                      <Grid item md={4} xs={12} sm={4}>
                        ...
                        {accessibleErrors.assignexpvalue && (
                          <div>{accessibleErrors.assignexpvalue}</div>
                        )}
                      </Grid>
                    )}
                  </>
                  ...
                  <>
                    <Grid item md={4} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        />
                        {accessibleErrors.rocketchatemail && (
                          <div>{accessibleErrors.rocketchatemail}</div>
                        )}
                      </FormControl>
                    </Grid>
                    <Grid item md={4} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        />
                        {accessibleErrors.rocketchatrole && (
                          <div>{accessibleErrors.rocketchatrole}</div>
                        )}
                      </FormControl>
                    </Grid>
                  </>
          ...
          <>
            ...
            <Grid item md={12} sm={12} xs={12}>
              ...
              >
                <Tooltip
                  title={
                    showDupProfileVIsitor?.some(
                      (data) => data?.modelName === "Employee"
                    )
                ...
                >
                  <span>
                    <Button
                      ...
                      disabled={showDupProfileVIsitor?.some(
                        (data) => data?.modelName === "Employee"
                      )}
                      onClick={() => {
                        UploadWithDuplicate();
                ...
                <Button
                  sx={buttonStyles.btncancel}
                  onClick={UploadWithoutDuplicate}
                >
...
export default InternDraftEdit;
