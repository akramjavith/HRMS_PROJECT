    ...
    const fetchEmployee = async () => {
        ...
        try {
            if ((isUserRoleCompare?.includes("lminimumpointscalculation") && isUserRoleCompare?.includes("lminimumpoints"))) {
                ...
                let [res_freq, res_vendor,
                    //  res_employee, 
                    Res,
                    //  Revenue,
                    dept, res_status] = await Promise.all([
            ...
            if ((isUserRoleCompare?.includes("lminimumpointscalculation") && isUserRoleCompare?.includes("lminimumpoints"))) {
                ...
                const itemsWithSerialNumber = res_employee1.data.users?.map(async (item, index) => {
                    try {
                        ...
                        let res_salary_rev = await axios.post(SERVICE.SALARYSLAB_REVENUE_BY_CODE, {
                        ...
                        console.log(manageshortagemasters.length, differenceInMonthsexp, findShortage, "dssfs")

                        // GROSS VALUE
                        let grossValue = modevalue && modevalue.expmode == "Manual" ? modevalue.gross :
                            findSalDetails ? (Number(findSalDetails.basic) + Number(findSalDetails.hra) + Number(findSalDetails.conveyance) + Number(findSalDetails.medicalallowance) + Number(findSalDetails.productionallowance) + Number(findSalDetails.otherallowance)) : ""
                        let egvalue = findShortage ? Number(grossValue) + Number(findShortage.amount) : 0;
                        console.log(grossValue, egvalue, "dssfs")
