...
function IndividualUserShiftWeekOffPresent() {
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Individual User Shift Weekoff Present"),
    ...
    const handleStartDayChangeBasedOnId = async () => {
        setShiftstatusDrop([]);
        setShiftCalDay("");
        try {
            let res = await axios.post(SERVICE.WEEKOFFPRESENT_DAY_FILTER_INDIVIDUAL, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            ...
            if (res?.data?.weekoffpresents.length === 0) {
                setPopupContentMalert(`This employee does not have any data matches. Kindly contact the administrator to confirm the 'Weekoff Setting'.`);
                setPopupSeverityMalert("warning");
                handleClickOpenPopupMalert();
                setFilterUser({
                    ...filterUser,
                ...
                setShiftCalDay("");
                setGetSelectedWeekoffDataId("");
            }
            else {
                const newShiftStartDay = res?.data?.weekoffpresents[0].shiftstartday;
                const nextAvailableDate = getNextShiftStartDate(newShiftStartDay);
                const toDate = calculateToDate(nextAvailableDate, Number(res?.data?.weekoffpresents[0].shiftdaytotal));
                ...
                setShiftCalDay(res?.data?.weekoffpresents[0].shiftdaytotal);
                setGetSelectedWeekoffDataId(res?.data?.weekoffpresents[0]._id);
            }
        }
        catch (err) { handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
    }
    useEffect(() => {
        handleStartDayChangeBasedOnId();
    }, []);
    ...
    const fetchUsers = async () => {
        ...
        setLoader(true);
        setIsBtn(true);
        // console.log(getSelectedWeekoffDataId, [isUserRoleAccess.companyname], 'getSelectedWeekoffDataId')
        // console.log(filterUser.fromdate, filterUser.todate)
        try {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (filterUser.fromdate === "" && filterUser.todate === "dd-mm-yyyy") {
    ...
    // Excel
    const fileName = "Individual User Shift Weekoff Present";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "Individual User Shift Weekoff Present",
        pageStyle: "print",
    });
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("Individual User Shift Weekoff Present.pdf");
    };
    ...
    const handleCaptureImage = () => {
        if (gridRefImageUserShift.current) {
            domtoimage.toBlob(gridRefImageUserShift.current)
                .then((blob) => {
                    saveAs(blob, "Individual User Shift Weekoff Present.png");
                })
                .catch((error) => {
    ...
    return (
        <Box>
            <Headtitle title={'Individual User Shift Weekoff Present'} />
            {/* ****** Header Content ****** */}
            <PageHeading
                title="Individual User Shift Weekoff Present"
                modulename="Human Resources"
                submodulename="HR"
                mainpagename="Attendance"
                subpagename="Attendance Reports"
                subsubpagename="Individual User Shift Weekoff Present"
            />
            {isUserRoleCompare?.includes("lindividualusershiftweekoffpresent")
                && (
                    <>
                        ...
                        <Box sx={userStyle.container}>
                            { /* ******************************************************EXPORT Buttons****************************************************** */}
                            <Grid item xs={8}>
                                <Typography sx={userStyle.importheadtext}>Individual User Shift Weekoff Present List</Typography>
                            </Grid>
                            <Grid container spacing={2} style={userStyle.dataTablestyle}>
                                ...
                                <Grid item md={8} xs={12} sm={12} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                    <Box >
                                        {isUserRoleCompare?.includes("excelindividualusershiftweekoffpresent") && (
                                            <>
                                        ...
                                        {isUserRoleCompare?.includes("csvindividualusershiftweekoffpresent") && (
                                            <>
                                        ...
                                        {isUserRoleCompare?.includes("printindividualusershiftweekoffpresent") && (
                                            <>
                                                <Button sx={userStyle.buttongrp} onClick={handleprint}>&ensp;<FaPrint />&ensp;Print&ensp;</Button>
                                            </>
                                        )}
                                        {isUserRoleCompare?.includes("pdfindividualusershiftweekoffpresent") && (
                                            <>
                                        ...
                                        {isUserRoleCompare?.includes("imageindividualusershiftweekoffpresent") && (
...
export default IndividualUserShiftWeekOffPresent;