...
import { Box, Typography, OutlinedInput, Dialog, TableBody, Select, DialogContent, DialogActions, FormControl, MenuItem, Grid, Table, TableHead, Button } from '@mui/material';
...
const DEP_FIELDS = ['date', 'process', 'errorfilename', 'documentnumber', 'documenttype', 'fieldname', 'line', 'errorvalue', 'correctvalue', 'link', 'doclink'];

const DEP_QUEUE_FIELDS = ['date', 'process', 'errorfilename', 'fieldname', 'line', 'errorvalue', 'correctvalue', 'link', 'doclink'];
...
const ExcelSheet = () => {
  ...
  const { isUserRoleCompare, buttonStyles, isServerCurrentdatetime } = useContext(UserRoleAccessContext);
  ...
  const handleSubmit = async () => {
    ...
    const filteredData = updatedData.filter((row) => row.some((cell) => cell !== null && cell !== '')).map((row) => Array.from({ length: updatedData[0].length }, (_, i) => row[i] ?? ''));

    const DEP_FIELDS = ['date', 'process', 'errorfilename', 'documentnumber', 'documenttype', 'fieldname', 'line', 'errorvalue', 'correctvalue', 'link', 'doclink'];

    const DEP_QUEUE_FIELDS = ['date', 'process', 'errorfilename', 'fieldname', 'line', 'errorvalue', 'correctvalue', 'link', 'doclink'];

    const fieldMap = penaltyErrorUpload.type === 'DEP Audit Summary Report' ? DEP_FIELDS : penaltyErrorUpload.type === 'DEP Queue Type Audit Summary Report' ? DEP_QUEUE_FIELDS : [];
