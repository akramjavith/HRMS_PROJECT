...
function ProcessMonthSet() {
  ...
  const [selectedProcessvalue, setselectedProcessOptionsEdit] = useState([]);

  let [valueProcessAdd, setValueProcessAdd] = useState([]);
  const customValueRendererDepartmentAdd = (valueProcessAdd, _companies) => {
    return valueProcessAdd.length ? valueProcessAdd.map(({ label }) => label)?.join(",") :
      <span style={{ color: "hsl(0, 0%, 20%)" }}>Please Select Department</span>
  }

  const handleProcessChangeAdd = (options) => {
    setValueProcessAdd(
      options.map(a => {
        return a.value;
      })
    ...
    setselectedProcessOptionsEdit(options);
  }
  const [selectedYearvalue, setselectedYearOptionsEdit] = useState([]);

  let [valueYearAdd, setValueYearAdd] = useState([]);
  const customValueRendererYearAdd = (valueYearAdd, _companies) => {
    return valueYearAdd.length ? valueYearAdd.map(({ label }) => label)?.join(",") :
      <span style={{ color: "hsl(0, 0%, 20%)" }}>Please Select Year</span>
  }

  const handleYearChangeAdd = (options) => {
    setValueYearAdd(
      options.map(a => {
        return a.value;
      })
    )
    setselectedMonthOptionsEdit([])
    setValueMonthAdd([])
    // BranchDropDowns(options)
    setselectedYearOptionsEdit(options);
  }

  const [selectedMonthvalue, setselectedMonthOptionsEdit] = useState([]);

  let [valueMonthAdd, setValueMonthAdd] = useState("");
  const customValueRendererMonthAdd = (valueMonthAdd, _companies) => {
    return valueMonthAdd.length ? valueMonthAdd.map(({ label }) => label)?.join(",") :
      <span style={{ color: "hsl(0, 0%, 20%)" }}>Please Select Month</span>
  }

  const handleMonthChangeAdd = (options) => {
    setValueMonthAdd(
      options.map(a => {
        return a.label;
      })
    ...
    // BranchDropDowns(options)
    setselectedMonthOptionsEdit(options);
  }
  ...
  let exportRowValues = ['process', 'year', 'month', 'fromdate', 'todate', 'totaldays'];
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Process Month Set"),
  ...
  const handleFilter = (e) => {
    ...
    if (valueProcessAdd.length === 0 && selectedYearvalue.length === 0 && selectedMonthvalue.length === 0) {
      setPopupContentMalert("Please Select Process");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else {
      fetchProcessSetFilter()
      fetchProcessSetFilterExports()
    }
  }



  const fetchProcessSetFilter = async () => {
    ...
    const queryParams = {
      ...
      process: valueProcessAdd,
      year: valueYearAdd,
      monthname: valueMonthAdd,
    };
    ...
    try {
      let res_status = await axios.post(SERVICE.GETPROCESSMONTHSETBYPAGINATION, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });

      const ans = res_status?.data?.processmonthsets?.length > 0 ? res_status?.data?.processmonthsets : []
      ...
      setAllProcessSet(ans);
      setLoader(false)
      // setAllProcessSetOverallData(itemstotalDatasOverallData);
    } catch (err) {
      setLoader(false)
  ...
  const fetchProcessSetFilterExports = async () => {
    setPageName(!pageName)
    ...
    try {
      let res_status = await axios.post(SERVICE.GETPROCESSMONTHSETBYPAGINATION_EXPORTS, queryParams, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      ...
      setAllProcessSetOverallData(ans);
      // setAllProcessSetOverallData(itemstotalDatasOverallData);
    } catch (err) {
  ...
  const fetchProcessSetSearch = async () => {
    setPageName(!pageName)
    setLoader(true)
  ...
  const sendEditRequest = async () => {
    ...
    try {
      ...
      setIsOpen(false)
    } catch (err) { setIsOpen(false); handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  const editSubmit = (e) => {
    e.preventDefault();
  ...
  const [allProcessSetOverallData, setAllProcessSetOverallData] = useState([])
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage.toBlob(gridRefTableImg.current)
        .then((blob) => {
          saveAs(blob, "Process Month Set.png");
        })
        .catch((error) => {
  ...
  const columnDataTable = [
    ...
    { field: "process", headerName: "Process", flex: 0, width: 150, hide: !columnVisibility.process, headerClassName: "bold-header", pinned: 'left', },
    { field: "year", headerName: "Year", flex: 0, width: 100, hide: !columnVisibility.year, headerClassName: "bold-header", pinned: 'left', },
  ...
  const handlecleared = (e) => {
    ...
    setFilteredRowData([])
    setAllProcessSet([])
    setAllProcessSetOverallData([]);

    setValueProcessAdd([])
    setselectedProcessOptionsEdit([])
    // setValueDesignationAdd([])
    setselectedYearOptionsEdit([])
    setValueYearAdd([])
    setselectedMonthOptionsEdit([])
    setValueMonthAdd([])
    setFilteredChanges(null)
  ...
  return (
    <Box>
      ...
      {isUserRoleCompare?.includes("lprocessmonthset") && (
        <Box sx={userStyle.selectcontainer}>
          <>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.importheadtext}>Process Month Set Filter</Typography>
              </Grid>
            </Grid>
            <br />
            <Grid container spacing={2}>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <MultiSelect
                    ...
                    onChange={handleProcessChangeAdd}
                  />
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <MultiSelect
                    maxMenuHeight={300}
                    options={valueYearAdd?.length === 0 ? [] : months}
                    value={selectedMonthvalue}
                    valueRenderer={customValueRendererMonthAdd}
                    onChange={handleMonthChangeAdd}
                  />
                </FormControl>
              </Grid>
          ...
          <Grid container spacing={2} style={userStyle.dataTablestyle}>
            ...
            <Grid item md={2} xs={12} sm={12}>
              {/* <AggregatedSearchBar
                    ...
                    setPage={setPage}
                    maindatas={allprocessSet}
                    setSearchedString={setSearchedString}
                    searchQuery={searchQuery}
                    setSearchQuery={setSearchQuery}
                    paginated={true}
                    totalDatas={allProcessSetOverallData}
          ...
          {loader ?
            ...
            <>
              <AggridTableForPaginationTable
                ...
                itemsList={allProcessSetOverallData}
              />
      ...
      <ExportData
        ...
        itemsTwo={allProcessSetOverallData ?? []}
        filename={"Process Month Set"}
