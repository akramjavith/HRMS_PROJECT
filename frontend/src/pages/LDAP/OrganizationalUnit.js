...
function OrganizationalUnit() {
  ...
  const [selectedTypeEdit, setSelectedTypeEdit] =
    useState("Please Select Type");
  ...
  const fetchOrganizational = async () => {
    ...
    try {
      ...
      const itemsWithSerialNumber = res_vendor?.data?.ouList?.map(
        (item, index) => {
  ...
  return (
    <Box>
      ...
      <PageHeading
        ...
        modulename="LDAP"
        submodulename="Organizational Unit"
        mainpagename=""
        subpagename=""
        subsubpagename=""
      />
