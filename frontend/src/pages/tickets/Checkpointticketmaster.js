...
function Checkpointticket() {
  ...
  const handleClosePdfFilterMod = () => {
        ...
        if (isfilter === "filtered") {
            exportToCSV(
                filteredData?.map((item, index) => ({
                    ...
                    Type: item.type,
                    Reason: item.reason,
                    CheckpointGroup: item.checkpointgrp,
                })),
                fileName,
            );
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("CheckpointTicketMaster.pdf");
    };
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "CheckpointTicketMaster.png");
        });
      });
    }
  };
...
useEffect(() => {
    ...
    if (valueCat?.length == 0) {
      ...
      setShowAlert(
        <>
          ...
          />
          <p style={{ fontSize: "20px", fontWeight: 900 }}>
            {"Checkpoint Ticket Master already exists!"}
          </p>
        </>
      );
      handleClickOpenerr();
  ...
  const fetchCheckpointticketmaster = async () => {
    try {
      ...
      setCheckpointticketmasters(res_vendor?.data?.checkpointticketmasters);
  ...
  const columns = [
    ...
    { title: "Checkpoint Group", field: "checkpointgrp" },
  ];
  ...
  const checkcatsubgrp = checkpointticketmasters?.map((item, index) => ({
    ...item,
  ...
  const addSerialNumber = () => {
    const itemsWithSerialNumber = checkpointticketmasters?.map(
      (item, index) => ({ serialNumber: index + 1 ,
        id: item._id,
        category: item.category?.map((t, i) => `${i + 1 + ". "}` + t).toString(),
        subcategory: item.subcategory
          ?.map((t, i) => `${i + 1 + ". "}` + t)
