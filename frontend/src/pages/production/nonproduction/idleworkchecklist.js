...
const ActionCell = ({ params, fetchBatchFilter }) => {
  ...
  const handleUpdateStatus = async (id) => {
    try {
      let res = await axios.put(
        `${SERVICE.IDLETIMEWORK_SINGLE}/${id}`,
  ...
  const handleRejectStatus = async (id, reasonstatus) => {
    try {
      let res = await axios.put(`${SERVICE.IDLETIMEWORK_SINGLE}/${id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        status:"Reject",
  ...
  return (
    <Grid container spacing={1}>
      <Grid item xs={6} md={6}>
        ...
        //  onClick={() => handleUpdateStatus(params.data.id)}
         >
          View
        </Button>&ensp;&ensp;
