...
function WorkmodeLog({ boardinglogs, userID }) {
    ...
    const rowDataArray = async () => {
        ...
        try {
            ...
            const getresarr = getarr.filter((data, index) => {
                return data.ischangeworkmode === true;
            });
    ...
    // Excel
    const fileName = "Workmode Log List";

    // print...
    const componentRef = useRef();
    const handleprint = useReactToPrint({
        content: () => componentRef.current,
        documentTitle: "Workmode Log List",
        pageStyle: "print",
    });
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "Workmode Log List.png");
                });
            });
        }
    };
