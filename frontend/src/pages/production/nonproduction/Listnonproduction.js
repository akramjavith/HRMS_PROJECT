...
function NonproductionList() {
    ...
    const handleFilterClickForExports = async () => {
        ...
        try {
            let res_vendor = await axios.post(SERVICE.NONPRODUCTIONFILTERLIST_EXPORT, queryParams, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });

            setTaskcategorysForExports(res_vendor?.data?.lists?.assignlist?.map((item, index) => ({
                ...item,
                serialNumber: index + 1
            })))

            setTaskcategorysAssignExports(res_vendor?.data?.lists?.approvedlist?.map((item, index) => ({
                ...item,
                serialNumber: index + 1
            })))

            setTaskcategorysallExports(res_vendor?.data?.lists?.rejectlist?.map((item, index) => ({
                ...item,
                serialNumber: index + 1
            })))
    ...
    const handleFilterClick = async () => {
        ...
        try {
            let res_vendor = await axios.post(SERVICE.NONPRODUCTIONFILTERLIST, queryParams, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });
            setTaskcategorys(res_vendor?.data?.lists?.assignlist?.map((item, index) => ({
                ...item,
    ...
    const handleFilterClickForAssign = async () => {
        ...
        try {
            let res_vendor = await axios.post(SERVICE.NONPRODUCTIONFILTERLIST_ASSIGN_PAGINATION, queryParams, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });
    ...
    const addSerialNumber = (datas) => {
        setItems(datas);
        setOverallItems(datas?.map((item, index) => ({
            ...item,
            id: item._id
        })));
    };
