...
function RaiseTicketReport() {
    ...
    if (isfilter === "filtered") {
      exportToCSV(
        filteredDatas?.map((item, index) => ({
          ...
          "Ticket Number": (item.raiseticketcount),
  ...
  const downloadPdf = async (isfilter) => {
    ...
    doc.save("RaiseTicketReport.pdf");
  };
  ...
  //get all project.
  const fetchAllRaisedTickets = async (from) => {
    try {
  ...
  const handleClear = async () => {
    await fetchAllRaisedTickets("clear");
    setBtnSubmit(false)
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "RaiseTicket_Report.png");
        });
      });
    }
  };
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <Typography sx={userStyle.HeaderText}>Raise Ticket Report</Typography>
