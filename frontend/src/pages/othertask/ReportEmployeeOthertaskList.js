...
function ReportEmployeeOthertaskList() {
    ...
    const fetchEmployee = async () => {
        setPageName(!pageName)
        try {
            let res_employee = await axios.post(SERVICE.ALL_OTHERTASKEMPLOYEE_SORT, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                page: Number(page),
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "Report Employee Other Task.png");
                });
            });
        }
    };
    ...
    return (
        <Box>
            ...
            <ExportData
                ...
                filename={"Report Employee Other Task List"}
