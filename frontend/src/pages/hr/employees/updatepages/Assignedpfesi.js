...
function Assignpfesi() {
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Assigned PF-ESI Detail List"),
  ...
  const getCode = async (e) => {
    ...
    try {
      ...
      const setData =
        res?.data?.suser.assignpfesilog.length > 0
          ? res?.data?.suser.assignpfesilog[
          res?.data?.suser.assignpfesilog.length - 1
          ]
          : res?.data?.suser.assignpfesilog;

      setPfesiForm(res?.data?.suser);
      setEditPfesiForm(setData);
      setDuplicateArray(res?.data?.suser?.assignpfesilog);
      handleClickOpenEdit();
  ...
  let pfesilog = pfesiform?.assignpfesilog ? pfesiform?.assignpfesilog : [];
  ...
  const editSubmit = (e) => {
    ...
    const IsSameDate = duplicateArray.some(
      (item) =>
        item.esideduction === editPfesiform.esideduction &&
        item.esideduction === editPfesiform.esideduction &&
        item.pfdeduction === editPfesiform.pfdeduction &&
        item.uan === editPfesiform.uan &&
        item.pfmembername === editPfesiform.pfmembername &&
        item.insurancenumber === editPfesiform.insurancenumber &&
        item.ipname === editPfesiform.ipname &&
        item.pffromdate === editPfesiform.pffromdate &&
        item.pfenddate === editPfesiform.pfenddate &&
        item.esifromdate === editPfesiform.esifromdate &&
        item.esideduction === editPfesiform.esideduction
    );
  ...
  //  PDF
  let exportColumnNames = ['Company', 'Branch', 'Unit', 'Team', "Work Mode", 'Emp Code', 'Name']
  let exportRowValues = ['company', 'branch', 'unit', 'team', 'workmode', 'empcode', 'companyname']
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Assigned PF-ESI Detail",
    pageStyle: "print",
  });
  ...
  const columnDataTable = [
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          <Link to={`/updatepages/assignedpfesi/${params.data.id}`}>
            <Button
              variant="contained"
              sx={{
                minWidth: "15px",
          ...
          &ensp;
          {isUserRoleCompare?.includes("eassignpf-esi") && (
            <Button
              variant="contained"
              onClick={() => {
  ...
  const handleClearFilter = () => {
    ...
    setEmployees([]);
    setSelectedOptionsEmployee([]);
    setValueEmp([]); setPopupContent("Cleared Successfully");
    setPopupSeverity("success");
    handleClickOpenPopup();
  };
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading
        title="Manage Assigned PF-ESI Detail"
                  ...
                  >
                    ...
                    <OutlinedInput
                      ...
                      onChange={(e) => {
                        ...
                        setEditPfesiForm((prevState) => ({
                          ...prevState,
                          pffromdate: newFromDate,
                          pfenddate: prevState.pfenddate && new Date(prevState.pfenddate) > new Date(newFromDate) ? prevState.pfenddate : ""
                        }));
                      }}
                      style={{ width: "224px" }}
                    />
                  ...
                  >
                    ...
                    <OutlinedInput
                      ...
                      onChange={(e) => {
                        ...
                        const selectedFromDate = new Date(editPfesiform.pffromdate);
                        ...
                        if (selectedToDate >= selectedFromDate && selectedToDate >= new Date(selectedFromDate)) {
                          ...
                          setEditPfesiForm({
                            ...editPfesiform,
                            pfenddate: "" // Reset to empty string if the condition fails
                          });
                        }
                  ...
                  >
                    ...
                    <OutlinedInput
                      ...
                      onChange={(e) => {
                        ...
                        setEditPfesiForm((prevState) => ({
                          ...prevState,
                          esifromdate: newFromDate,
                          esienddate: prevState.esienddate && new Date(prevState.esienddate) > new Date(newFromDate) ? prevState.esienddate : ""
                        }));
                      }}
                      style={{ width: "224px" }}
                    />
                ...
                <Grid item md={6} xs={12} sm={12}>
                  ...
                  >
                    ...
                    <OutlinedInput
                      ...
                      onChange={(e) => {
                        const selectedToDate = new Date(e.target.value);
                        const selectedFromDate = new Date(editPfesiform.esifromdate);
                        const formattedDatePresent = new Date() // Assuming you have a function to format the current date
                        if (selectedToDate >= selectedFromDate && selectedToDate >= new Date(selectedFromDate)) {
                          setEditPfesiForm({
                            ...editPfesiform,
                          ...
                          setEditPfesiForm({
                            ...editPfesiform,
                            esienddate: "" // Reset to empty string if the condition fails
                          });
                        }
