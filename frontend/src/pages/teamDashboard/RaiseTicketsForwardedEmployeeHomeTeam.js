...
function RaiseTicketForwardedEmployee() {
  ...
  const fetchAllRaisedTicketsed = async () => {
    ...
    try {
      // let response = await axios.post(SERVICE.RAISE_TICKET_FORWARDED_EMPLOYEES, {
      //   headers: {
      ...
      // });
       ...
       let res = await axios.post(SERVICE.HIERARCHY_TEAM_ATTENDANCE_BASED_CLAIM, {
          ...
          let response = await  axios.post(SERVICE.RAISE_TICKET_FORWARDED_TEAM, {
                          headers: {
                            Authorization: `Bearer ${auth.APIToken}`,
                          },
        ...
        <Grid sx={{ display: 'flex' }}>
          {isUserRoleCompare?.includes('vmyactionableticket') && (
            <Link to={`/tickets/raiseticketfilterview/${params.data.id}`} target="_blank" style={{ textDecoration: 'none', color: '#fff', minWidth: '0px' }}>
              <Button variant="contained" sx={buttonStyles.butonsubmit} style={{ minWidth: '0px' }}>
                View
              </Button>
            </Link>
          )}
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: 'RaiseTicket_Forwarded_Tickets',
    pageStyle: 'print',
  });
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, 'Raise Ticket Forwarded Employee List.png');
        })
        .catch((error) => {
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <Typography sx={userStyle.HeaderText}>Raise Ticket - Forwarded Tickets</Typography>
        ...
        <>
          <Box sx={userStyle.container}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.SubHeaderText}>Raise Ticket - Forwarded Tickets</Typography>
              </Grid>
              {/* <Grid item xs={4}>
      ...
      <ExportData
        ...
        filename={'Raise Ticket Forwarded Employee List'}
...
export default RaiseTicketForwardedEmployee;
