...
function MyShiftAdjustmentListTable({ allUsersAdjTable, adjApply, fetchUsers }) {
   ...
   `;
    ...
    const columnDataTable = [
                ...
                <Grid >
                    {
                        params.row.adjustmenttype === "Shift Weekoff Swap" ?
                            ...
                            params.row.adjustmenttype === "WeekOff Adjustment" ?
                                (<Grid sx={{ display: 'block', fontSize: '12px', }}>
                                    {`Week off Date : ${params.row.todate}`} <br />
                                    {`Adjustment For : ${params.row.selectedDate} `} <br />
        ...
        //             <Grid sx={{ display: "flex" }}>
        //                 {isUserRoleCompare?.includes("emyshiftadjustment") && (
        //                     <StyledTableCell>
        //                         <Button
        //                             variant="outlined"
        //                             color="primary"
    ...
    const rowDataTable = filteredDataAdjApply?.map((item, index) => {
        ...
        return {
            ...
            // request: item.adjustmenttype == "Add On Shift" ? (`${item.adjustmenttype} : ${item.adjdate}`) : (`${item.adjustmenttype} : ${item.adjdate}`),
    ...
    // Excel
    const fileNameMyAdjList = "Adjustment Apply List";
    const [fileFormatMyAdjList, setFormatMyAdjList] = useState('')
    const fileTypeMyAdjList = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8';
    const fileExtensionMyAdjList = fileFormatMyAdjList === "xl" ? '.xlsx' : '.csv';
    const exportToCSVMyAdjList = (csvData, fileNameMyAdjList) => {
        const ws = XLSX.utils.json_to_sheet(csvData);
        const wb = { Sheets: { 'data': ws }, SheetNames: ['data'] };
        const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
        const data = new Blob([excelBuffer], { type: fileTypeMyAdjList });
        FileSaver.saveAs(data, fileNameMyAdjList + fileExtensionMyAdjList);
    }

    const handleExportXLMyAdjList = (isfilter) => {
        if (isfilter === "filtered") {
            exportToCSVMyAdjList(
        ...
        setIsFilterOpenMyAdjList(false)
    };
    ...
    const downloadPdfMyAdjList = (isfilter) => {
    ...
    return (
        <Box>
                            ...
                            <Grid item md={8} xs={12} sm={12} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                                <Box>
                                    ...
                                    {isUserRoleCompare?.includes("pdfmyshiftroaster") && (
                                        <>
                                            {/* <Button sx={userStyle.buttongrp} onClick={() => downloadPdfMyAdjList()}>
                                                <FaFilePdf />
                        ...
                        {!adjApply ?
                            ...
                            <>
                                <Box style={{ width: "100%", overflowY: "hidden", }}>
                                    <StyledDataGrid onClipboardCopy={(copiedString) => setCopiedData(copiedString)} rows={rowsWithCheckboxes} columns={columnDataTable.filter((column) => columnVisibility[column.field])} onSelectionModelChange={handleSelectionChange} selectionModel={selectedRows} autoHeight={true} ref={gridRefAdjTable} id="settableexcel"
            ...
            <Dialog open={isFilterOpenMyAdjList} onClose={handleCloseFilterModMyAdjList} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                <DialogContent sx={{ textAlign: "center", alignItems: "center", justifyContent: "center" }}>

                    <IconButton
                        aria-label="close"
                        onClick={handleCloseFilterModMyAdjList}
                        sx={{
                            position: "absolute",
                            right: 8,
                            top: 8,
                ...
                <DialogActions>
                    <Button
                        ...
                        onClick={(e) => {
                            handleExportXLMyAdjList("filtered")
                        }}
                    >
                    ...
                    <Button autoFocus variant="contained"
                        onClick={(e) => {
                            handleExportXLMyAdjList("overall")
                        }}
                    >
            ...
            {/*Export pdf Data  */}
            <Dialog open={isPdfFilterOpenMyAdjList} onClose={handleClosePdfFilterModMyAdjList} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                <DialogContent sx={{ textAlign: "center", alignItems: "center", justifyContent: "center" }}>
                    <IconButton
                        aria-label="close"
                        onClick={handleClosePdfFilterModMyAdjList}
                        sx={{
                            position: "absolute",
                            right: 8,
                            top: 8,
                ...
                <DialogActions>
                    <Button
                        variant="contained"
                        onClick={(e) => {
                            downloadPdfMyAdjList("filtered")
                            setIsPdfFilterOpenMyAdjList(false);
                        }}
                    >
                        Export Filtered Data
                    </Button>
