...
import { Box, Typography, Dialog, DialogContent, OutlinedInput, DialogActions, Grid, Select, MenuItem, FormControl, Button, List, ListItem, ListItemText, Popover, Checkbox, TextField, IconButton } from "@mui/material";
...
function Submodulelistview() {
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'Submodulelistview.png');
                });
            });
        }
    };
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            {/* <Typography sx={userStyle.HeaderText}>Sub Module List View</Typography> */}
            <PageHeading
