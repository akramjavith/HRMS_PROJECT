...
function Paycontrol() {
  ...
  let exportColumnNames = ['Company', 'Branch', 'Unit', 'Team', 'Employee Status', 'Filter Type', 'Employee Name', 'Achieved Type', 'Achieved Value', 'Newgross Type', 'Newgross Value', 'Production Achieved', 'New Gross', 'Salary Type', 'Deduction Type'];
  let exportRowValues = ['company', 'branch', 'unit', 'team', 'empstatus', 'filtertype', 'empname', 'achievedsymbol', 'achieved', 'newgrosssymbol', 'newgross', 'finalachieved', 'finalgross', 'salraytype', 'deductiontype'];

  let exportColumnNamesNearTat = ['Company', 'Employee Status', 'Filter Type', 'Department', 'Branch', 'Unit', 'Team', 'Employee Name', 'Achieved Type', 'Achieved Value', 'Newgross Type', 'Newgross Value', 'Production Achieved', 'New Gross', 'Salary Type', 'Deduction Type'];
  let exportRowValuesNearTat = ['company', 'empstatus', 'filtertype', 'userdepartment', 'userbranch', 'userunit', 'userteam', 'empname', 'achievedsymbol', 'achieved', 'newgrosssymbol', 'newgross', 'finalachieved', 'finalgross', 'salraytype', 'deductiontype'];
  ...
  const handleDepartmentChangeTo = (options) => {
    ...
    setSelectedEmployeeTo([]);
    fetchEmployeeNamesBasedTeam(payruncontrol.company, payruncontrol.empstatus, options, 'department');
  };
  ...
  const handleDepartmentChangeToEdit = (options) => {
    ...
    fetchEmployeeNamesBasedTeamEdit(payruncontroledit.company, payruncontroledit.empstatus, options, selectedBranchToEdit, selectedUnitToEdit, selectedTeamToEdit, 'department');
  ...
  const handleBranchChangeToEdit = (options) => {
    ...
    fetchEmployeeNamesBasedTeamEdit(payruncontroledit.company, payruncontroledit.empstatus, selectedDepartmentToEdit, options, selectedUnitToEdit, selectedTeamToEdit, 'branch');
    setSelectedUnitToEdit([]);
    setSelectedTeamToEdit([]);
    setSelectedEmployeeToEdit([]);
  };
  ...
  const handleUnitChangeToEdit = (options) => {
    ...
    fetchEmployeeNamesBasedTeamEdit(payruncontroledit.company, payruncontroledit.empstatus, selectedDepartmentToEdit, selectedBranchToEdit, options, selectedTeamToEdit, 'unit');
    setSelectedTeamToEdit([]);
    setSelectedEmployeeToEdit([]);
  };
  ...
  const handleTeamChangeToEdit = (options) => {
    ...
    fetchEmployeeNamesBasedTeamEdit(payruncontroledit.company, payruncontroledit.empstatus, selectedDepartmentToEdit, selectedBranchToEdit, selectedUnitToEdit, options, 'team');
    setSelectedEmployeeToEdit([]);
  };
  ...
  const handleSubmit = (e) => {
    ...
    const isNameMatch = payruncontrolmaster?.some(
      (item) =>
        item.company === payruncontrol.company &&
        item.empstatus === payruncontrol.empstatus &&
        item.filtertype === payruncontrol.filtertype &&
        (selectedDepartmentTo.length > 0 ? item.department.some((item) => selectedDepartmentTo.map((item) => item.value).includes(item)) : true) &&
        (selectedBranchTo.length > 0 ? item.branch.some((item) => selectedBranchTo.map((item) => item.value).includes(item)) : true) &&
        (selectedUnitTo.length > 0 ? item.unit.some((item) => selectedUnitTo.map((item) => item.value).includes(item)) : true) &&
        (selectedTeamTo.length > 0 ? item.team.some((item) => selectedTeamTo.map((item) => item.value).includes(item)) : true) &&
  ...
  const getCode = async (e, idgrp, name) => {
    ...
    try {
      ...
      let checkDepartment =
        res?.data?.spayruncontrol?.department?.length > 0
          ...
          : [];
      let checkBranch =
        res?.data?.spayruncontrol?.branch?.length > 0
          ...
          : [];
      let checkUnit =
        res?.data?.spayruncontrol?.unit?.length > 0
          ...
          : [];
      let checkTeam =
        res?.data?.spayruncontrol?.team?.length > 0
  ...
  const editSubmit = (e) => {
    ...
    const isNameMatch = allProjectedit?.some(
      (item) =>
        ...
        (selectedDepartmentToEdit.length > 0 ? item.department.some((item) => selectedDepartmentToEdit.map((item) => item.value).includes(item)) : true) &&
        (selectedBranchToEdit.length > 0 ? item.branch.some((item) => selectedBranchToEdit.map((item) => item.value).includes(item)) : true) &&
        (selectedUnitToEdit.length > 0 ? item.unit.some((item) => selectedUnitToEdit.map((item) => item.value).includes(item)) : true) &&
        (selectedTeamToEdit.length > 0 ? item.team.some((item) => selectedTeamToEdit.map((item) => item.value).includes(item)) : true) &&
        (empNames.length > 0
          ? item.empname.some((item) => {
              if (item === 'ALL') {
                return true;
  ...
  const editSubmitNear = (e) => {
    ...
    const isNameMatch = allProjecteditIndidvidual?.some(
      (item) =>
        item.company === payruncontroledit.company &&
        item.empstatus === payruncontroledit.empstatus &&
        item.filtertype === payruncontroledit.filtertype &&
  ...
  const fetchPayrunControl = async () => {
    ...
    try {
      let res_project = await axios.post(
        SERVICE.PAYRUNCONTROLBYASSIGNBRANCH,
        {
  ...
  const downloadPdf = (isfilter) => {
    ...
    const dataWithSerial = isfilter === 'filtered' ? rowDataTable.map((row) => ({ ...row, serialNumber: serialNumberCounter++ })) : items.map((row) => ({ ...row, serialNumber: serialNumberCounter++ }));
  ...
  const downloadPdf2 = (isfilter) => {
    ...
    // Modify row data to include serial number
    const dataWithSerial = isfilter === 'filtered' ? rowDataTableNearTat.map((row) => ({ ...row, serialNumber: serialNumberCounter++ })) : itemsneartat.map((row) => ({ ...row, serialNumber: serialNumberCounter++ }));
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Pay Run Control Settings" modulename="PayRoll" submodulename="PayRoll Setup" mainpagename="Pay Run Control" subpagename="" subsubpagename="" />
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      onChange={(e) => {
                        ...
                        fetchEmployeeNamesBasedTeam(payruncontrol.company, e.value, e.value, 'empstatus');
                        setSelectedBranchTo([]);
                        setSelectedEmployeeTo([]);
                        setSelectedDepartmentTo([]);
                        setSelectedUnitTo([]);
                  ...
                  <>
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <MultiSelect
                          options={accessbranch
                            ?.filter((comp) => payruncontrol.company === comp.company)
                            ?.map((data) => ({
                              label: data.branch,
                              value: data.branch,
                            }))
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <MultiSelect
                          options={accessbranch
                            ?.filter((comp) => payruncontrol.company === comp.company && selectedBranchTo.map((item) => item.value).includes(comp.branch))
                            ?.map((data) => ({
                              label: data.unit,
                              value: data.unit,
                            }))
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <MultiSelect
                          options={Array.from(new Set(teamsall?.filter((comp) => selectedBranchTo.map((item) => item.value).includes(comp.branch) && selectedUnitTo.map((item) => item.value).includes(comp.unit))?.map((com) => com.teamname))).map((teamname) => ({
                            label: teamname,
                            value: teamname,
                          }))}
                  ...
                  <>
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <MultiSelect options={departmentOpt} value={selectedDepartmentTo} onChange={handleDepartmentChangeTo} valueRenderer={customValueRendererDepartmentTo} labelledBy="Please Select Department" />
                      </FormControl>
                    </Grid>
                  </>
                  ...
                  <>
                    ...
                    {payruncontrol?.choosestatus === 'New Gross' && (
                      <Grid item md={4} xs={12} sm={12}>
                        <Grid container>
                          <Typography>
                            New Gross(Month) {payruncontrol.newgrosssymbol === 'Between' ? 'from to' : ''}
                            <b style={{ color: 'red' }}>*</b>
                          </Typography>
                          <Grid item md={12} xs={12} sm={12} sx={{ display: 'flex' }}>
                            <Select
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      onChange={(e) => {
                        ...
                        fetchEmployeeNamesBasedTeamEdit(payruncontroledit.company, e.value, selectedDepartmentToEdit, selectedBranchToEdit, selectedUnitToEdit, selectedTeamToEdit, 'empstatus');
                        setSelectedDepartmentToEdit([]);
                        setSelectedUnitToEdit([]);
                        setSelectedTeamToEdit([]);
                        setSelectedEmployeeToEdit([]);
                ...
                {['Individual', 'Team']?.includes(payruncontroledit.filtertype) ? (
                  <>
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <MultiSelect
                          options={accessbranch
                            ?.filter((comp) => payruncontroledit.company === comp.company)
                            ?.map((data) => ({
                              label: data.branch,
                              value: data.branch,
                            }))
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <MultiSelect
                          options={accessbranch
                            ?.filter((comp) => payruncontroledit.company === comp.company && selectedBranchToEdit.map((item) => item.value).includes(comp.branch))
                            ?.map((data) => ({
                              label: data.unit,
                              value: data.unit,
                            }))
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <MultiSelect
                          options={Array.from(new Set(teamsall?.filter((comp) => selectedBranchToEdit.map((item) => item.value).includes(comp.branch) && selectedUnitToEdit.map((item) => item.value).includes(comp.unit))?.map((com) => com.teamname))).map((teamname) => ({
                            label: teamname,
                            value: teamname,
                          }))}
                  ...
                  <>
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        <Typography>Department</Typography>
                        <MultiSelect options={departmentOpt} value={selectedDepartmentToEdit} onChange={handleDepartmentChangeToEdit} valueRenderer={customValueRendererDepartmentToEdit} labelledBy="Please Select Department" />
                      </FormControl>
                    </Grid>
                  </>
                  ...
                  <>
                    ...
                    {payruncontroledit?.choosestatus === 'New Gross' && (
                      <Grid item md={4} xs={12} sm={12}>
                        <Grid container>
                          <Typography>
                            New Gross(Month) {payruncontroledit.newgrosssymbol === 'Between' ? 'from to' : ''}
                            <b style={{ color: 'red' }}>*</b>
                          </Typography>
                          <Grid item md={12} xs={12} sm={12} sx={{ display: 'flex' }}>
                            <Select
              ...
              <Grid container spacing={2}>
                ...
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput id="component-outlined" type="text" size="small" value={payruncontroledit.filtertype} />
                  </FormControl>
                </Grid>
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput id="component-outlined" type="text" size="small" value={payruncontroledit.empstatus} />
                  </FormControl>
                </Grid>
                {['Department']?.includes(payruncontroledit.filtertype) ? (
                  <Grid item md={4} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput id="component-outlined" type="text" size="small" value={payruncontroledit.userdepartment} />
                    </FormControl>
                  </Grid>
                ) : ['Individual', 'Team']?.includes(payruncontroledit.filtertype) ? (
                  <>
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput id="component-outlined" type="text" size="small" value={payruncontroledit.userbranch} />
                      </FormControl>
                    </Grid>
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput id="component-outlined" type="text" size="small" value={payruncontroledit.userunit} />
                      </FormControl>
                    </Grid>
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput id="component-outlined" type="text" size="small" value={payruncontroledit.userteam} />
                      </FormControl>
                    </Grid>
                  </>
                ) : ['Branch']?.includes(payruncontroledit.filtertype) ? (
                        ...
                        ?.filter((comp) =>
                          selectedDepartmentToEdit.length > 0
                            ? payruncontroledit.company === comp.company &&
                              selectedDepartmentToEdit.map((item) => item.value).includes(comp.department) &&
                            ...
                            : payruncontroledit.company === comp.company &&
                              selectedBranchToEdit.map((item) => item.value).includes(comp.branch) &&
                              selectedUnitToEdit.map((item) => item.value).includes(comp.unit) &&
                              selectedTeamToEdit.map((item) => item.value).includes(comp.team) &&
                              (payruncontroledit.empstatus === 'Live Employee' ? comp.resonablestatus != 'Releave Employee' || comp.resonablestatus != 'Hold' || comp.resonablestatus != 'Terminate' || comp.resonablestatus != 'Absconded' : payruncontroledit.empstatus == comp.resonablestatus)
                        )
                        ?.map((com) => ({
                          ...com,
        ...
        <Box sx={{ padding: '10px 30px' }}>
          <>
            ...
            <Grid container spacing={2}>
              ...
              {payruncontroledit.department?.length > 0 && (
                <Grid item md={4} xs={12} sm={6}>
                  <FormControl fullWidth size="small">
              ...
              {payruncontroledit.branch?.length > 0 && (
                <Grid item md={4} xs={12} sm={6}>
                  <FormControl fullWidth size="small">
              ...
              {payruncontroledit.unit?.length > 0 && (
                <Grid item md={4} xs={12} sm={6}>
                  <FormControl fullWidth size="small">
              ...
              {payruncontroledit.team?.length > 0 && (
                <Grid item md={4} xs={12} sm={6}>
                  <Box>
              ...
              <Grid item md={2} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  {/* <Typography variant="h6">QR Code</Typography> */}
                  <FormControlLabel
                    control={<Checkbox sx={{ '& .MuiSvgIcon-root': { fontSize: 40, marginTop: 1 } }} checked={payruncontroledit?.choosetype} color="primary" />}
                    // sx={{marginTop: 1}}
                    label="Choose Any"
                  />
                </FormControl>
      ...
      <Dialog open={openviewnear} onClose={handleClickOpenviewnear} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="md" fullWidth={true} sx={{ marginTop: '50px' }}>
        <Box sx={{ padding: '10px 30px' }}>
          <>
            ...
            <Grid container spacing={2}>
              ...
              {viewpaynear?.department?.length > 0 && (
                <Grid item md={4} xs={12} sm={6}>
                  <FormControl fullWidth size="small">
                    <Typography variant="h6"> Department</Typography>
              ...
              <Grid item md={4} xs={12} sm={6}>
                <FormControl fullWidth size="small">
                  <Typography variant="h6"> Employee Name</Typography>
                  <Typography>{viewpaynear.empname}</Typography>
                </FormControl>
              </Grid>
