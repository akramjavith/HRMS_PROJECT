...
function ProductionOriginalUpload() {
  ...
  const fetchUploads = async () => {
    try {
      let Res = await axios.get(SERVICE.PRODUCTION_ORGINAL_UNIQID, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setProductionoriginalid(Res.data.productionoriginal);
      setDataupdated("");
      setCheckingSts("");
    } catch (err) {
    ...
    // }
          ...
          fileReader.onload = async (e) => {
            ...
            if (wb.SheetNames.includes(sheetName)) {
              ...
              while (!headingsFound && rowIndex < maxRows) {
                ...
                if ((cellValueA === "Category" && (cellValueB === "Unit Identifier" || cellValueB === "User") && cellValueC === "User" && cellValueD === "Date" && cellValueE === "Unit Rate") || (cellValueA === "Category" && cellValueC === "Date" && cellValueD === "Unit Rate")) {
                  headingsFound = true;
                } else {
                  rowIndex++;
                }
      ...
      Promise.all(promises)
        .then(async (allData) => {
          ...
          const fromDate = new Date(productionoriginal.fromdate);
          ...
          const isValidDate = (dateString) => {
            ...
            return date.toDateString() === fromDate.toDateString() || date.toDateString() === oneDayBefore.toDateString() || date.toDateString() === oneDayAfter.toDateString();
          };
            ...
            for (let i = 0; i < checkUnique.length; i += batchSize) {
              ...
              try {
                const response = await axios.post(
                  SERVICE.PRODUCTION_UPLOAD_OVERALL_FETCH_LIMITEDNEW,
                  {
                    checkunique: batch,
                  },
                  {
            ...
            if (dupeData.length === 0) {
                ...
                const updatedFileData = file.data.map((item) => {
                  ...
                  for (let keyword of keywords) {
                    if (!item.Category.includes(keyword) && keyword) {
                      ...
                      const match = file.data.find((otherItem) => otherItem.Category === balancingCategory && otherItem["Unit Identifier"] === item["Unit Identifier"] && otherItem.User === item.User && otherItem.Date === item.Date && otherItem.filename === item.filename && otherItem.uniqueKey !== item.uniqueKey);
              ...
              if (["Details", "Details 1"].includes(productionoriginal.sheetnumber)) {
                ...
                let nonDuplicatesWithAlternativeNames = nonDuplicates.map((item) => {
                  ...
                  const ORate = flagstatusorgval === "Yes" ? Number(resultObjectCate.flagmanualcalcorg) : Number(item["Unit Rate"]);
                  return {
                    project: projectname,
                    ...
                    points: (ORate * 8.333333333333333).toFixed(4),
                    oratelog: [
                      {
                        orate: ORate,
                        trate: "",
                ...
                let duplicatesFromUnitRate = findandupdate.filter((oldItem) => {
                  const resultObjectCate = categories.find((i) => i.project === productionoriginal.vendor.split("-")[0] && i.name === oldItem.filename);
                  let flagstatusorgval = resultObjectCate && resultObjectCate.flagstatusorg ? resultObjectCate.flagstatusorg : "No";
                  let flagstatusval = resultObjectCate && resultObjectCate.flagstatus ? resultObjectCate.flagstatus : "No";

                  return unitsRateData.some((ur) => {
                    const oldURate = flagstatusorgval === "Yes" ? Number(resultObjectCate.flagmanualcalcorg) : Number(oldItem["Unit Rate"]);

                    return projectname === ur.project && oldItem.Category === ur.subcategory && (ur.category === oldItem.filename || (oldItem.Category === "Snippet Verification" && oldItem.filename === "SmartZone Verify")) && Number(ur.orate) !== Number(oldURate);
                  });
                });
                ...
                const duplicatesFromUnitRateFinalORATELOG = duplicatesFromUnitRate.filter((oldItem) => {
                  ...
                  let flagstatusorgval = resultObjectCate && resultObjectCate.flagstatusorg ? resultObjectCate.flagstatusorg : "No";
                  let flagstatusval = resultObjectCate && resultObjectCate.flagstatus ? resultObjectCate.flagstatus : "No";
                  const ORate = flagstatusorgval === "Yes" ? Number(resultObjectCate.flagmanualcalcorg) : Number(oldItem["Unit Rate"]);

                  if (matchObject) {
                    oldItem.orate = ORate; // Update the trate property of ur
                    oldItem.mrate = matchObject.mrate === "" ? ORate : matchObject.mrate;
                    oldItem.points = ORate * 8.333333333333333;
  ...
  const sendJSON = async () => {
              ...
              const arrayOfArrays = [
                ...
                ...fileDataFromExcelArray
                  .map((d) => {
                    ...
                    let flagstatusorgval = resultObjectCate && resultObjectCate.flagstatusorg ? resultObjectCate.flagstatusorg : "No";
                    let flagstatusval = resultObjectCate && resultObjectCate.flagstatus ? resultObjectCate.flagstatus : "No";
                    let flagcalc = flagstatusorgval === "Yes" ? Math.round(Number(d["Unit Rate"]) / Number(resultObjectCate.flagmanualcalcorg)) : d["Flag Count"] === undefined ? 1 : d["Flag Count"];

                    return {
                      ...d,
                      "Flag Count": flagcalc,
                      "Unit Rate": flagstatusorgval === "Yes" ? Number(resultObjectCate.flagmanualcalcorg) : d["Unit Rate"],
                  ...
                  const updatedData = [
                    ...
                    ...fileDataFromExcelArray
                      .map((d) => {
                        ...
                        let flagstatusorgval = resultObjectCate && resultObjectCate.flagstatusorg ? resultObjectCate.flagstatusorg : "No";
                        let flagstatusval = resultObjectCate && resultObjectCate.flagstatus ? resultObjectCate.flagstatus : "No";
                        let flagcalc = flagstatusorgval === "Yes" ? Math.round(Number(d["Unit Rate"]) / Number(resultObjectCate.flagmanualcalcorg)) : flagstatusval === "Yes" ? Math.round(Number(d["Unit Rate"]) / Number(d.trate)) : d["Flag Count"] === undefined ? 1 : d["Flag Count"];
                        ...
                        return {
                          ...d,
                          "Unit Rate": flagstatusorgval === "Yes" ? Number(resultObjectCate.flagmanualcalcorg) : Number(d.trate),
    ...
    if (excelArray.length > 0) {
      try {
        const productionResponse = await axios.post(SERVICE.PRODUCTION_ORGINAL_CREATE, productionuploaddata, {
          headers: {
          ...
          const uploadData = async (filename, items, index, onProgress) => {
            ...
            for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
              ...
              const batchRequestBody = batchItems.map((item) => {
                ...
                let flagstatusorgval = resultObjectCate && resultObjectCate.flagstatusorg ? resultObjectCate.flagstatusorg : "No";
                let flagstatusval = resultObjectCate && resultObjectCate.flagstatus ? resultObjectCate.flagstatus : "No";
                let flagcalc = flagstatusorgval === "Yes" ? Math.round(Number(item["Unit Rate"]) / Number(resultObjectCate.flagmanualcalcorg)) : productionoriginal.sheetnumber == 1 && flagstatusval === "Yes" ? Math.round(Number(item["Unit Rate"]) / Number(item.trate)) : item["Flag Count"] === undefined ? 1 : item["Flag Count"];

                return {
                  // category: item["Category"],
                  unitid: item["Unit Identifier"],
                  user: item["User"],
                  unitrate: flagstatusorgval === "Yes" ? Number(resultObjectCate.flagmanualcalcorg) : productionoriginal.sheetnumber == 1 && flagstatusval === "Yes" ? item.trate : Number(item["Unit Rate"]),
              ...
              try {
                if (batchRequestBody.length > 0) {
                  if (batchRequestBody.length > 0) {
                    ...
                    // });
                    const res = await axios.post(SERVICE.PRODUCTION_UPLOAD_CREATE, batchRequestBody, {
                      headers: {
                        "Content-Type": "application/json;charset=UTF-8",
                      },
                    });
  ...
  const fetchProductionoriginal = async () => {
    try {
      let res_vendor = await axios.get(SERVICE.PRODUCTION_ORGINAL_LIMITED_LASTTHREE, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });

      setProductionsOriginal(res_vendor?.data?.productionoriginal);
    } catch (err) {
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
  };
  ...
  return (
    <Box>
      <Headtitle title={"Production Original Upload"} />
      <PageHeading title=" Production Original Upload" modulename="Production" submodulename="Upload" mainpagename="Original" subpagename="Production Original Upload" subsubpagename="" />
      {/* ****** Header Content ****** */}
      {/* <Typography sx={userStyle.HeaderText}>Production Original Upload</Typography> */}
      {isUserRoleCompare?.includes("aproductionoriginalupload") && (
        <>
          <Box sx={userStyle.dialogbox}>
            <>
              <Grid container>
                <Grid item md={3.5} sm={8} xs={12}>
                  <Typography sx={userStyle.importheadtext}>Manage Production Original Upload</Typography>
                </Grid>
                          ...
                          {[...[{ vendor: "Vendor", fromdate: "From-To-Date", percent: "%", createddate: "CreatedDate", addedby: [{ name: "Added By" }] }], ...productionsOriginal].map((item, ind) => {
                            return (
                              <Grid container spacing={1}>
                                ...
                                <Grid item md={2.5} sm={2.5} xs={2.5} sx={{ wordBreak: "break-all" }}>
                                  <Typography sx={{ fontSize: item.fromdate === "From-To-Date" ? "12px" : "10px" }}> {item.fromdate === "From-To-Date" ? "From-To-Date" : `${moment(item.fromdate)?.format("DD-MM-YYYY")} - ${moment(item.todate)?.format("DD-MM-YYYY")}`}</Typography>
                                </Grid>
                                <Grid item md={2.5} sm={2.5} xs={2.5} sx={{ wordBreak: "break-all" }}>
                                ...
                                <Grid item md={3.5} sm={3.5} xs={3.5} sx={{ wordBreak: "break-all" }}>
                                  <Typography sx={{ fontSize: (item.addedby ? item.addedby[0]?.name : "") === "Added By" ? "12px" : "10px" }}> {(item.addedby ? item.addedby[0]?.name : "").length > 25 ? `${(item.addedby ? item.addedby[0]?.name : "").substring(0, 25)}...` : item.addedby ? item.addedby[0]?.name : ""}</Typography>
                                </Grid>
                ...
                <Grid item md={1.7} sm={12} xs={12} sx={{ display: "flex", justifyContent: "end" }}>
                  <Link to="/production/productionupload">
                ...
                <Grid item md={4} xs={12} sm={6}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      onChange={(e) => {
                        setProductionoriginal({ ...productionoriginal, datetimezone: e.value, sheetnumber: e.value === "(GMT +11:30) Central Standard Time / (GMT +10:30) Central Daylight Time" ? 1 : "Details 1" });
                      }}
                    />
                  </FormControl>
                </Grid>
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      options={[
                        ...
                        { label: "1", value: "1", isDisabled: productionoriginal.datetimezone !== "(GMT +11:30) Central Standard Time / (GMT +10:30) Central Daylight Time" },
                      ]}
                      isDisabled={fileLength > 0 || productionoriginal.datetimezone === "(GMT +11:30) Central Standard Time / (GMT +10:30) Central Daylight Time"}
