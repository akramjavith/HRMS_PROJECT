...
function ManageShortageMaster() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const isNameMatch = manageshortagemastersDup.some((item) => item.department?.toLowerCase() == manageshortagemaster.department?.toLowerCase() && item.from == manageshortagemaster.from && item.to == manageshortagemaster.to && item.amount == manageshortagemaster.amount);
  ...
  const fetchManageshortagemasterArray = async () => {
    ...
    try {
      ...
      setManageshortagemastersFilterArray(
        res_status?.data?.manageshortagemasters?.map((t, index) => ({
          ...t,
  ...
  const fetchEmployee = async () => {
    ...
    try {
      ...
      setManageshortagemasters(
        itemsWithSerialNumber?.map(({ amount, serialNumber, experienceinmonth, department, _id }) => ({
          amount,
          serialNumber,
          experienceinmonth,
          department,
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Manage Shortage Master" modulename="PayRoll" submodulename="PayRoll Setup" mainpagename="Shortage Master" subpagename="" subsubpagename="" />
      {!statusCheck ? (
                          ...
                          <OutlinedInput
                            ...
                            onChange={(e) => {
                              ...
                              if (value === '' || (parseFloat(value) > 0 && !isNaN(value))) {
                                // Allow empty string or a number greater than 0
      ...
      />
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Shortage Master Info" addedby={addedby} updateby={updateby} />
      <DeleteConfirmation open={isDeleteOpen} onClose={handleCloseMod} onConfirm={delManageshortagemaster} title="Are you sure?" confirmButtonText="Yes" cancelButtonText="Cancel" />
      <DeleteConfirmation open={isDeleteOpencheckbox} onClose={handleCloseModcheckbox} onConfirm={delManageshortagemastercheckbox} title="Are you sure?" confirmButtonText="Yes" cancelButtonText="Cancel" />
