...
function Interviewquestion() {
      ...
      exportToCSV(
        rowDataTable.map((item, index) => {
          return {
            ...
            Question: item.name,
            "Sub Questions": item.doyouhaveextraquestion,
            "Attend By": item.testattendby,
          };
        }),
        fileName
      );
  ...
  const columns = [
    ...
    { title: "Sub Questions", field: "doyouhaveextraquestion" },
    { title: "Attend By", field: "testattendby" },
  ];
  ...
  const downloadPdf = (isfilter) => {
    ...
    doc.save("Interview Question Masters.pdf");
  };
  ...
  const exportallData = async () => {
    ...
    try {
      ...
      setCandidateExport(res_project?.data?.interviewquestions);
    } catch (err) {
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Interview_Question.png");
        });
      });
    }
  };
  ...
  const filteredDatas = projmaster?.filter((item) => {
  ...
  return (
    <Box>
      ...
      <PageHeading
        title="Manage Interview Question Master"
      ...
      <Box>
        ...
        >
          <Box sx={{ width: "550px", padding: "20px 50px" }}>
            <>
              <Typography sx={userStyle.HeaderText}>
                Interview Question Master Info
              </Typography>
              <br />
              <br />
              <Grid container spacing={2}>
              ...
              {rowDataTable &&
                rowDataTable.map((row, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{row.doyouhaveextraquestion}</TableCell>
                    <TableCell>{row.testattendby}</TableCell>
                  </TableRow>
                ))}
            </TableBody>
          </Table>
