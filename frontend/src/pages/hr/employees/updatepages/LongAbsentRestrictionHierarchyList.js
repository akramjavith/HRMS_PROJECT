...
function LongAbsentRestrictionHierarchyList() {
  ...
  let exportColumnNames = ['Current Status', 'Company', 'Branch', 'Unit', 'Team', 'Leave Applied', 'Emp Code', 'Name'];
  let exportRowValues = ['userstatus', 'company', 'branch', 'unit', 'team', 'leaveapplied', 'empcode', 'companyname'];

  let exportColumnNamesNear = ['Current Status', 'Company', 'Branch', 'Unit', 'Team', 'Leave Applied', 'Emp Code', 'Name'];
  let exportRowValuesNear = ['userstatus', 'company', 'branch', 'unit', 'team', 'leaveapplied', 'empcode', 'companyname'];
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, 'Long Absent Restriction Hierarchy List.png');
        });
      });
    }
  };
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      let answer = subprojectscreate?.data?.filterallDatauser.filter((item) => isAssignBranch.some((branch) => branch.company === item.company && branch.branch === item.branch && branch.unit === item.unit));

      let ans = answer.filter((d) => d.checklistassigned || dateArray?.some((data) => d.longleaveabsentaprooveddate?.includes(data)));

      let anspending = answer.filter((d) => !d.checklistassigned && !dateArray?.some((data) => d.longleaveabsentaprooveddate?.includes(data)));
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Long Absent Restriction Hierarchy List" modulename="Human Resources" submodulename="HR" mainpagename="Employee" subpagename="Employee Status Details" subsubpagename="Team Long Absent Restriction List" />
          ...
          <Box sx={{ ...userStyle.container }}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  <Typography sx={userStyle.importheadtext}>Long Absent Restriction Hierarchy List</Typography>
                </Grid>
      ...
      {loader ? (
        ...
        <>
          {isUserRoleCompare?.includes('lteamlongabsentrestrictionlist') && (
            <>
              <Box sx={userStyle.container}>
                {/* ******************************************************EXPORT Buttons****************************************************** */}
                <Grid container spacing={2}>
                  <Grid item xs={8}>
                    <Typography sx={userStyle.SubHeaderText}>Long Absent Restriction List</Typography>
                  </Grid>
                  <Grid item xs={4}>
                ...
                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                  ...
                  >
                    <Box>
                      {isUserRoleCompare?.includes('excelteamlongabsentrestrictionlist') && (
                        <>
                          <Button
                            onClick={(e) => {
                      ...
                      {isUserRoleCompare?.includes('csvteamlongabsentrestrictionlist') && (
                        <>
                          <Button
                            onClick={(e) => {
                              setIsFilterOpen(true);
                      ...
                      {isUserRoleCompare?.includes('printteamlongabsentrestrictionlist') && (
                        <>
                      ...
                      {isUserRoleCompare?.includes('pdfteamlongabsentrestrictionlist') && (
                        <>
                          <Button
                            sx={userStyle.buttongrp}
                            onClick={() => {
                      ...
                      {isUserRoleCompare?.includes('imageteamlongabsentrestrictionlist') && (
          ...
          {isUserRoleCompare?.includes('lteamlongabsentrestrictionlist') && (
            <>
              <Box sx={userStyle.container}>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={8}>
                    <Typography sx={userStyle.SubHeaderText}>Long Absent Restriction Completed List</Typography>
                  </Grid>
                  <Grid item xs={4}>
                    {isUserRoleCompare?.includes('lmychecklist') && (
                      <>
      ...
      <ExportData
        ...
        itemsTwo={pending ?? []}
        filename={'Long Absent Restriction Hierarchy List'}
      ...
      <ExportData
        ...
        itemsTwo={employees ?? []}
        filename={'Long Absent Restriction Hierarchy List Completed'}
        ...
        >
          ...
          <DialogActions sx={{ justifyContent: "center", pb: 2 }}>
            <Button
              ...
              color="error"
              onClick={() => {
                handleUndoLogin(rowDetails, "undo")// Redirect
              }}
            >
              Yes
            </Button>
