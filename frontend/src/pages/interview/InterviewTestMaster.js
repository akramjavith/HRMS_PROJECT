...
function InterviewTestMaster() {
    ...
    if (isfilter === "filtered") {
      exportToCSV(
        rowDataTable?.map((t, index) => ({
          ...
          "Duration (HH:MM)": t.durationhours,
        })),
        fileName
      );
  ...
  const columns = [
    ...
    { title: "Duration (HH:MM)", field: "durationhours" },
  ];
  ...
  const downloadPdf = (isfilter) => {
    ...
    doc.save("Interview Test Master.pdf");
  };
  ...
  const handlesubmit = async (e) => {
    ...
    if (
      ...
      interviewTestMaster.durationhours === "00" &&
      interviewTestMaster.durationminutes === "00"
    ) {
      setShowAlert(
        <>
          {" "}
    ...
    try {
      await Promise.all(
        subcategoriess.map(async (data) => {
          await axios.post(
            SERVICE.INTERVIEWTESTMASTER_CREATE,
            {
  ...
  const editSubmit = (e) => {
    ...
    if (
      ...
      interviewTestMasterEdit.durationhours == "00" &&
      interviewTestMasterEdit.durationminutes == "00"
    ) {
      setShowAlert(
        <>
          <ErrorOutlineOutlinedIcon
    ...
    // else if (
    //   interviewTestMasterEdit.totalmarks == "" ||
    //   interviewTestMasterEdit.totalmarks == undefined
    // ) {
    //   setShowAlert(
    //     <>
    //       <ErrorOutlineOutlinedIcon
    ...
    //   handleClickOpenerr();
    // } else if (
    //   interviewTestMasterEdit.eligiblemarks == "" ||
    //   interviewTestMasterEdit.eligiblemarks == undefined
    // ) {
    //   setShowAlert(
    //     <>
    //       <ErrorOutlineOutlinedIcon
    ...
    //   handleClickOpenerr();
    // } else if (
    //   Number(interviewTestMasterEdit.eligiblemarks) >
    //   Number(interviewTestMasterEdit.totalmarks)
    // ) {
    //   setShowAlert(
    //     <>
    //       <ErrorOutlineOutlinedIcon
    ...
    //       />
    //       <p style={{ fontSize: "20px", fontWeight: 900 }}>
    //         {"Eligible Marks Should be Less Than or Equals to Total Marks!"}
    //       </p>
    //     </>
    //   );
    //   handleClickOpenerr();
  ...
  const sendEditRequest = async () => {
    ...
    try {
      ...
      let res = await axios.put(
        `${SERVICE.INTERVIEWTESTMASTER_SINGLE}/${subprojectsid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Interview test master.png");
        });
      });
    }
  };
  ...
  const rowData = async (id, name) => {
    setPageName(!pageName);
    try {
      let res = await axios.get(`${SERVICE.INTERVIEWTESTMASTER_SINGLE}/${id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
    ...
    try {
      if (Checkpointticketsid) {
        ...
        if (overallcheck?.data?.mayidelete) {
          await axios.delete(`${SERVICE.INTERVIEWTESTMASTER_SINGLE}/${e}`, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
          });
    ...
    try {
      if (selectedRows?.length > 0) {
        const deletePromises = selectedRows?.map((item) => {
          return axios.delete(`${SERVICE.INTERVIEWTESTMASTER_SINGLE}/${item}`, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
          });
  ...
  const fetchCategoryBasedEdit = async (e) => {
    ...
    try {
      ...
      const result = res_category?.data?.interviewcategory.filter(
        (data, index) => {
          return (
            e === data.categoryname && data.mode == "Online or Interview Test"
          );
        }
      );
  ...
  const getCode = async (e) => {
    ...
    try {
      const [res, res_vendor] = await Promise.all([
        axios.get(`${SERVICE.INTERVIEWTESTMASTER_SINGLE}/${e}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        }),
  ...
  const getviewCode = async (e) => {
    setPageName(!pageName);
    try {
      let res = await axios.get(`${SERVICE.INTERVIEWTESTMASTER_SINGLE}/${e}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const fetchInterviewOrders = async () => {
    setPageName(!pageName);
    try {
      let res_vendor = await axios.get(SERVICE.INTERVIEWTESTMASTER, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  // Excel
  const fileName = "Interview test master";

  //print...
  const componentRef = useRef();
  const handleprint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: "Interview Test Master",
    pageStyle: "print",
  });
  ...
  const columnDataTable = [
    ...
    },
    {
      ...
      flex: 0,
      width: 160,
      hide: !columnVisibility.durationhours,
      headerClassName: "bold-header",
    },

    {
      ...
      renderCell: (params) => (
        <Grid>
          {isUserRoleCompare?.includes("einterviewtestmaster") && (
            <Button
          ...
          {isUserRoleCompare?.includes("dinterviewtestmaster") && (
            <Button
          ...
          {isUserRoleCompare?.includes("vinterviewtestmaster") && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
                handleClickOpenview();
          ...
          {isUserRoleCompare?.includes("iinterviewtestmaster") && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
                handleClickOpeninfo();
  ...
  return (
    <Box>
      <Headtitle title={"INTERVIEW TEST MASTER"} />
      {/* ****** Header Content ****** */}
      <PageHeading
        title="Manage Interview Test Master"
        modulename="Interview"
        submodulename="Interview Setup"
        mainpagename="Interview Test Master"
        subpagename=""
        subsubpagename=""
      />
      {isUserRoleCompare?.includes("ainterviewtestmaster") && (
        <Box sx={userStyle.dialogbox}>
          <>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.importheadtext}>
                  Add Interview Test Master
                </Typography>
                  ...
                  <Grid item md={6} xs={12} sm={6}>
                    <FormControl fullWidth size="small">
                      <Selects
                        ...
                        placeholder="Hrs"
                        value={{
                          label: interviewTestMaster.durationhours,
                          value: interviewTestMaster.durationhours,
                        }}
                        onChange={(e) => {
                          setInterviewTestMaster({
                            ...interviewTestMaster,
                            durationhours: e.value,
                          });
                        }}
                      />
                    </FormControl>
                  ...
                  <Grid item md={6} xs={12} sm={6}>
                    <FormControl fullWidth size="small">
                      <Selects
                        ...
                        placeholder="Mins"
                        value={{
                          label: interviewTestMaster.durationminutes,
                          value: interviewTestMaster.durationminutes,
                        }}
                        onChange={(e) => {
                          setInterviewTestMaster({
                            ...interviewTestMaster,
                  ...
                  <Grid item md={6} xs={12} sm={6}>
                    <FormControl fullWidth size="small">
                      <Selects
                        ...
                        placeholder="Hrs"
                        value={{
                          label: interviewTestMasterEdit.durationhours,
                          value: interviewTestMasterEdit.durationhours,
                        }}
                        onChange={(e) => {
                          setInterviewTestMasterEdit({
                            ...interviewTestMasterEdit,
                  ...
                  <Grid item md={6} xs={12} sm={6}>
                    <FormControl fullWidth size="small">
                      <Selects
                        ...
                        placeholder="Mins"
                        value={{
                          label: interviewTestMasterEdit.durationminutes,
                          value: interviewTestMasterEdit.durationminutes,
                        }}
                        onChange={(e) => {
                          setInterviewTestMasterEdit({
                            ...interviewTestMasterEdit,
      ...
      <br />
      {/* ****** Table Start ****** */}
      {isUserRoleCompare?.includes("linterviewtestmaster") && (
        <>
          <Box sx={userStyle.container}>
            ...
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes("excelinterviewtestmaster") && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes("csvinterviewtestmaster") && (
                    <>
                      <Button
                        onClick={(e) => {
                          setIsFilterOpen(true);
                  ...
                  {isUserRoleCompare?.includes("printinterviewtestmaster") && (
                    <>
                  ...
                  {isUserRoleCompare?.includes("pdfinterviewtestmaster") && (
                    <>
                      <Button
                        sx={userStyle.buttongrp}
                        onClick={() => {
                  ...
                  {isUserRoleCompare?.includes("imageinterviewtestmaster") && (
                    <Button
                      sx={userStyle.buttongrp}
            ...
            &ensp;
            {isUserRoleCompare?.includes("bdinterviewtestmaster") && (
              <Button
                variant="contained"
                color="error"
          ...
          >
            <TableHead>
              <TableRow>
                ...
                <TableCell>Duration(HH:MM)</TableCell>
              </TableRow>
            </TableHead>
              ...
              {rowDataTable &&
                rowDataTable?.map((row, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{"Online or Interview Test"}</TableCell>
        ...
        <Box sx={{ width: "750px", padding: "20px 50px" }}>
          <>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  <Typography variant="h6">Duration(HH:MM)</Typography>
                  <Typography>
                    {interviewTestMasterEdit?.durationhours}
                  </Typography>
                </FormControl>
              ...
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>Online or Interview Test</Typography>
                </FormControl>
              </Grid>
            </Grid>
            <br /> <br /> <br />
...
export default InterviewTestMaster;
