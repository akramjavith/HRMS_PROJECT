...
function UnAllottedChecklist() {
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "Unallotted Checklists.png");
                });
            });
        }
    };
    ...
    const fetchChecklistverification = async () => {
        setPageName(!pageName)
        try {
            let res_status = await axios.post(SERVICE.GETALLCHECKLISTBYPAGINATIONNOTASSIGNED, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                page: Number(page),
            ...
            setChecklistverificationmasters(res_status?.data?.checklisttypes);
