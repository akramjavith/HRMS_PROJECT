...
function TaskUserScheduleLog() {
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("TaskScheduleGroupinglog.pdf");
    };
    ...
    const fetchTaskGrouping = async () => {
        try {
            ...
            setLoader(true);
            setTaskGroupingArray(res_freq?.data?.staskdesignationgrouping?.taskdesignationlog);
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "TaskScheduleGroupinglog.png");
                });
            });
        }
    };
    ...
    // Excel
    const fileName = "TaskScheduleGroupinglog";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "TaskScheduleGroupinglog",
        pageStyle: "print",
    });
    ...
    return (
        <Box>
            <Headtitle title={"TASK SCHEDULE GROUPING LOG"} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Manage Task Schedule Log</Typography>
