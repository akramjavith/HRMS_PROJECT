...
function QueueTypeReportSummary() {
  ...
  let exportColumnNames = ['Project', 'Vendor', 'Date', 'Category', 'Type', 'Sub Category', 'Count', 'ORate', 'Flag Count', 'Old Total', 'New Rate', 'New Total', 'Difference'];

  let exportRowValues = ['vendor', 'vendornew', 'formatteddate', 'filenameupdated', 'type', 'category', 'matchcount', 'orate', 'flagcount', 'oldtotal', 'newrate', 'newtotal', 'difference'];
  //    today date fetching
  ...
  const fetchBatchFilter = async (batchNum) => {
    ...
    setLoaderList(true);
    try {
      let res_employee = await axios.post(SERVICE.PRODUCTIONUPLOAD_QUEUE_TYPE_MASTER_COUNT, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: 'Queue Type Summary',
    pageStyle: 'print',
  });
  ...
  const columnDataTable = [
    ...
    { field: 'vendor', headerName: 'Project', flex: 0, width: 150, hide: !columnVisibility.vendor, headerClassName: 'bold-header' },
    { field: 'vendornew', headerName: 'Vendor', flex: 0, width: 100, hide: !columnVisibility.vendornew, headerClassName: 'bold-header' },
    { field: 'filenameupdated', headerName: 'Category', flex: 0, width: 350, hide: !columnVisibility.filenameupdated, headerClassName: 'bold-header' },
    { field: 'type', headerName: 'Type', flex: 0, width: 200, hide: !columnVisibility.type, headerClassName: 'bold-header' },
    { field: 'category', headerName: 'SubCategory', flex: 0, width: 500, hide: !columnVisibility.category, headerClassName: 'bold-header' },
    { field: 'matchcount', headerName: 'Count', flex: 0, width: 130, hide: !columnVisibility.matchcount, headerClassName: 'bold-header' },
    ...
    { field: 'oldtotal', headerName: 'Old Total', flex: 0, width: 120, hide: !columnVisibility.oldtotal, headerClassName: 'bold-header' },
    { field: 'newrate', headerName: 'New Rate', flex: 0, width: 130, hide: !columnVisibility.newrate, headerClassName: 'bold-header' },
    { field: 'newtotal', headerName: 'New Total', flex: 0, width: 130, hide: !columnVisibility.newtotal, headerClassName: 'bold-header' },
    { field: 'difference', headerName: 'Difference', flex: 0, width: 130, hide: !columnVisibility.difference, headerClassName: 'bold-header' },
  ];
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String('Queue Type Summary'),
  ...
  return (
    <Box>
      <Headtitle title={'Queue Type Summary'} />
      {/* ****** Header Content ****** */}
      <PageHeading title="Queue Type Summary" modulename="Production" submodulename="SetUp" mainpagename="Queue Type Summary" subpagename="" subsubpagename="" />
      <>
        <Box sx={userStyle.selectcontainer}>
          <Grid item xs={8}>
            <Typography sx={userStyle.importheadtext}>Queue Type Summary</Typography>
          </Grid>
          <>
      ...
      <br />
      {/* ****** Table Start ****** */}
      {isUserRoleCompare?.includes('lqueuetypesummary') && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>Queue Type Summary List</Typography>
            </Grid>
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              <Grid item md={8} xs={12} sm={12} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <Box>
                  {isUserRoleCompare?.includes('excelqueuetypesummary') && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes('csvqueuetypesummary') && (
                    <>
                      <Button
                        onClick={(e) => {
                          setIsFilterOpen(true);
                  ...
                  {isUserRoleCompare?.includes('printqueuetypesummary') && (
                    <>
                  ...
                  {isUserRoleCompare?.includes('pdfqueuetypesummary') && (
                    <>
                      <Button
                        sx={userStyle.buttongrp}
                        onClick={() => {
                  ...
                  {isUserRoleCompare?.includes('imagequeuetypesummary') && (
                    <>
...
export default QueueTypeReportSummary;
