...
const ActionCell = ({ params, fetchBatchFilter }) => {
  ...
  const allottedTotalMinutes = Number(params.data.alloteddays) * 24 * 60 + Number(params.data.allotedhours) * 60 + Number(params.data.allotedminutes);
  const actualTotalMinutes = Number(params.data.days) * 24 * 60 + Number(params.data.hours) * 60 + Number(params.data.minutes);
  let data = {};
  if (actualTotalMinutes > allottedTotalMinutes) {
    const [fromHours, fromMinutes] = params.data.fromtime.split(":").map(Number);
    ...
    // Step 2: Add allotted time
    const totalMinutesToAdd = Number(params.data.alloteddays) * 24 * 60 + Number(params.data.allotedhours) * 60 + Number(params.data.allotedminutes);
    fromDate.setMinutes(fromDate.getMinutes() + totalMinutesToAdd);

    // Step 3: Format new time back to HH:mm
    const newHours = fromDate.getHours().toString().padStart(2, "0");
    const newMinutes = fromDate.getMinutes().toString().padStart(2, "0");

    const totime = `${newHours}:${newMinutes}`;
    data = {
      days: params.data.alloteddays,
      hours: params.data.allotedhours,
      minutes: params.data.allotedminutes,
      totime,
      totalhours: `${params.data.allotedhours}h ${params.data.allotedminutes}m`,
      approvestatus: true,
    };
  } else {
    data = {
  ...
  const handleUpdateStatus = async (id) => {
    try {
      let res = await axios.put(
        `${SERVICE.NONPRODUCTION_SINGLE}/${id}`,
        data, // pass your update data directly here
        {
          headers: {
