...
import {
  ...
  DialogTitle, FormControl, FormControlLabel, FormGroup, Grid, MenuItem, OutlinedInput, Paper, Select,
  Table,
  TableBody,
  TableContainer,
  TableHead,
...
import { documentTypeOption, followUpActionOption } from '../../../components/Componentkeyword';
...
function AddVisitors() {
  ...
  const [vendor, setVendor] = useState({
    ...
    date: date,
    requestvisitorfollowupdate: date,
    prefix: 'Mr',
    visitorname: '',
    intime: '',
  ...
  console.log(selectedCountryp, "selectedCountryp")
  ...
  function handleChangeImage(e) {
    ...
    if (file && file.size < maxFileSize) {
      ...
      processFile(file).then(NewDataNew => { setNewimageNew(NewDataNew) });
  ...
  const sendRequest = async (type) => {
    ...
    try {
      ...
      const jsonData = {
        ...
        pcountry: String(vendor.pcountry),
        pstate: String(vendor.pstate),
        pcity: String(vendor.pcity),
        ...
        meetingpersoncompany: vendor.meetingdetails === true ? [...valueCompanyCat] : [],
        meetingpersonbranch: vendor.meetingdetails === true ? [...valueBranchCat] : [],
        meetingpersonunit: vendor.meetingdetails === true ? [...valueUnitCat] : [],
        meetingpersondepartment: vendor.meetingdetails === true ? [...valueDepartmentCat] : [],
        meetingpersonteam: vendor.meetingdetails === true ? [...valueTeamCat] : [],
        meetingpersonemployeename: String(vendor.meetingdetails === true ? vendor.meetingpersonemployeename : ''),
        meetinglocationcompany: vendor.meetingdetails === true ? [...valueCompanyLocationCat] : [],
        meetinglocationbranch: vendor.meetingdetails === true ? [...valueBranchLocationCat] : [],
        meetinglocationunit: vendor.meetingdetails === true ? [...valueUnitLocationCat] : [],
        meetinglocationfloor: vendor.meetingdetails === true ? [...valueFloorLocationCat] : [],
        ...
        followupdate: visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument ? "" : String(vendor.followupaction === 'Required' ? vendor.followupdate : ''),
        followuptime: visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument ? "" : String(vendor.followupaction === 'Required' ? vendor.followuptime : ''),
        followupaction: visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument ? "" : vendor.followupaction,
        visitorbadge: String(vendor.visitorbadge),
        equipmentborrowed: String(path === 'interactor/addvisitorin' ? '' : vendor.equipmentborrowed),
        outtime: String(path === 'interactor/addvisitorin' ? '' : vendor.outtime),
        remark: String(path === 'interactor/addvisitorin' ? '' : vendor.remark),
        visitorsurvey: String(path === 'interactor/addvisitorin' ? '' : vendor.visitorsurvey),
        addvisitorin: Boolean(path === 'interactor/addvisitorin'),
        ...
        interactorstatus: visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument || false ? String('requestfollowupvisitor') : String('visitor'),
        addedby: [
          {
      ...
      let addVisitorProfileDetail = await axios.post(SERVICE.VISITORDETAILS_LOG_CREATE, {
        ...
        visitorcontactnumber: String(vendor.visitorcontactnumber),
        visitoremail: String(vendor.visitoremail),
        materialcarrying: [...subCategoryTodo],
      ...
      if (type === 'save1') {
        ...
        setButtonLoad(false);
        if (path === 'interactor/addvisitorin') {
          setTimeout(() => {
            backPage('/interactor/visitoryoutentry');
          }, 1000);
        } else {
          setTimeout(() => {
  ...
  const handleSubmit = async (e, type) => {
      ...
      (item) =>
        ...
        (!vendor.meetingdetails || // If meetingdetails is false, skip the following conditions
          ...
          (item.meetingpersoncompany.some((data) => compopt.includes(data)) &&
            ...
            item.meetingpersonemployeename === vendor.meetingpersonemployeename &&
        ...
        (vendor.followupaction !== 'Required' || // If followupaction is not "Required", skip the following conditions
          ...
          (item.followupdate == vendor.followupdate && item.followuptime == vendor.followuptime))
    );
    ...
    if (checkContactEmailForpopup) {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendor.meetingdetails === true && valueDepartmentCat?.length == 0) {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendor.meetingdetails === true && vendor.meetingpersonemployeename === 'Please Select Employee Name') {
      setPopupContentMalert('Please Select Meeting-Person Employee Name');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendor.meetingdetails === true && valueCompanyLocationCat?.length == 0) {
      setPopupContentMalert('Please Select Meeting-Location Company');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendor.meetingdetails === true && valueBranchLocationCat?.length == 0) {
      setPopupContentMalert('Please Select Meeting-Location Branch');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendor.meetingdetails === true && valueUnitLocationCat?.length == 0) {
      setPopupContentMalert('Please Select Meeting-Location Unit');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendor.meetingdetails === true && valueFloorLocationCat?.length == 0) {
      setPopupContentMalert('Please Select Meeting-Location Floor');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (vendor.meetingdetails === true && vendor.meetinglocationarea === 'Please Select Area') {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (path !== 'interactor/addvisitorin' && vendor.outtime === '') {
      setPopupContentMalert('Please Select OUT Time');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (path !== 'interactor/addvisitorin' && !(visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument || false) && (vendor.followupaction === 'Please Select Follow Up Action' || vendor.followupaction === '')) {
      setPopupContentMalert('Please Select Follow Up Action');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (!visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument
  ...
  const handleCloseModEditProfileCheck = async (e, reason) => {
    ...
    setVendor({
      ...
      personalprefix: e?.personalprefix || "Please Select Personal Prefix",
      referencename: e?.referencename || "",
      landmarkpositionprefix: e?.landmarkpositionprefix || "Please Select Landmark Position Prefix",
    ...
    setSelectedCityp(City.getCitiesOfState(State.getStatesOfCountry(Country.getAllCountries().find((country) => country.name === e?.pcountry)?.isoCode).find((state) => state.name === e?.pstate)?.countryCode, State.getStatesOfCountry(Country.getAllCountries().find((country) => country.name === e?.pcountry)?.isoCode).find((state) => state.name === e?.pstate)?.isoCode).find((city) => city.name === e?.pcity));
  ...
  return (
    <Box>
      ...
      <PageHeading title="Visitors" modulename="Interactors" submodulename="Visitor" mainpagename="Add Visitors" subpagename="" subsubpagename="" />
      <>
        <Box sx={userStyle.dialogbox}>
          <>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                {' '}
                <Typography sx={{ fontWeight: 'bold' }}>Add Visitors</Typography>{' '}
              </Grid>
            </Grid>
            <br />
            <Grid container spacing={2}>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  <Typography>
                    Check Visitor <b style={{ color: 'red' }}>*</b>
                  </Typography>
                  <Selects
                    maxMenuHeight={300}
                    options={checkVisitors}
                    value={{
                      label: visitorCheck,
                      value: visitorCheck,
                    }}
                    onChange={(e) => {
                      setVisitorCheck(e.value);
                      setVisitorCheckName('');
                    }}
                  />
                </FormControl>
              </Grid>
              {visitorCheck === 'Existing Visitor' ? (
                <>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      <Typography>
                      ...
                      <OutlinedInput
                        ...
                        type="text"
                        value={visitorCheckName}
                        placeholder="Please Enter Visitor Name"
                        onChange={(e) => {
                          setVisitorCheckName(e.target.value);
                        }}
                      />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                    <Typography>&nbsp;</Typography>
                    <LoadingButton loading={isGettingVisitorUserCandidate} onClick={handlesubmitexistingvisitorCheck} sx={buttonStyles.buttonsubmit}>
                      Ok
                    </LoadingButton>
                    &nbsp; &nbsp;
                    <Button onClick={handlesubmitexistingvisitorClose} sx={buttonStyles.btncancel}>
                      Cancel
                    </Button>
                  </Grid>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Selects
                    ...
                    onChange={(e) => {
                      const dueDays = visitorsTypeOption?.filter((item) =>
                        item?.interactorstype === vendor.visitortype &&
                        item?.interactorspurpose?.includes(e.value)
                      )[0]?.duedays || "";
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Selects
                    ...
                    value={{
                      label: vendor.landmarkpositionprefix,
              ...
              {/* <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  <Typography>
                    Full Address<b style={{ color: 'red' }}>*</b>
                  </Typography>
                  <TextareaAutosize
                    ...
                    value={
                      [
                        vendor.personalprefix !== "Please Select Personal Prefix" && vendor.personalprefix,
                        vendor.referencename,
                        vendor.landmarkpositionprefix !== "Please Select Landmark Position Prefix" && vendor.landmarkpositionprefix,
                        vendor.landmarkname,
                  ...
                  <TextareaAutosize
                    ...
                    value={
                      [
                        ...
                        .reduce((acc, part) => {
                          // If part already ends with comma, just add space
                          if (acc.endsWith(',')) {
                            return `${acc} ${part}`;
                          }
                          // If current part starts with comma, just add it
                          if (part.startsWith(',')) {
                            return `${acc}${part}`;
                          }
              ...
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <TextareaAutosize
                    ...
                    value={
                      [
                        vendor.personalprefix !== "Please Select Personal Prefix" ? vendor.personalprefix : null,
                        vendor.referencename,
                        vendor.landmarkpositionprefix !== "Please Select Landmark Position Prefix" ? vendor.landmarkpositionprefix : null,
                        vendor.landmarkname,
                        vendor.houseflatnumber,
                        vendor.streetroadname,
                        vendor.localityareaname,
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormGroup>
                  <FormControlLabel
                    control={<Checkbox checked={vendor.meetingdetails} />}
                    onChange={(e) =>
                      setVendor({
                        ...vendor,
                ...
                <>
                  ...
                  <Grid item md={4} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <Selects
                        maxMenuHeight={300}
                        options={[...new Set(filteredAreas.filter((item) => valueFloorLocationCat?.includes(item.floor) && valueBranchLocationCat?.includes(item.branch) && item?.boardingareastatus).flatMap((item) => item.area))].map((location) => ({
                          label: location,
                          value: location,
                        }))}
            ...
            <Grid container spacing={2} sx={{ display: 'flex', justifyContent: 'center' }}>
              ...
              <Grid item md={2} sm={2} xs={12}>
                <Button variant="contained" color="primary" sx={buttonStyles.buttonsubmit} onClick={(e) => handleSubmit(e, 'save2')} disabled={buttonLoad}>
                  Save & add another
                </Button>
              </Grid>
              <Grid item md={1} sm={2} xs={12}>
