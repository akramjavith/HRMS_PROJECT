...
function DeactivateemployeeslistView() {
  ...
  const { isUserRoleAccess, isUserRoleCompare, isAssignBranch,
    ...
    setPageName,
    buttonStyles,
    allUsersData, allTeam, allUsersLimit } = useContext(
      UserRoleAccessContext
    );
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage.toBlob(gridRefTableImg.current)
        .then((blob) => {
          saveAs(blob, "Deactivate Employee List View.png");
        })
        .catch((error) => {
      ...
      cellRenderer: (params) => (
        <>
          <Grid sx={{ display: "flex", gap: '10px' }}>
            ...
            <Link
              to={`/view/${params?.data?.id}/deactivateemployeelist`}
              style={{
                textDecoration: "none",
                color: "#fff",
              }}
  ...
  return (
    <Box>
      ...
      <PageHeading
        ...
        subsubpagename="Deactivate Employees List View"
      />
