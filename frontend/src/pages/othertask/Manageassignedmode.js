...
function Manageassignedmode() {
    ...
    const [manageAssignedByArrayEdit, setManageAssignedbyArrayEdit] = useState([])
    ...
    const rowData = async (id, name) => {
        ...
        try {              
            let res = await axios.get(`${SERVICE.MANAGEASSIGNED_SINGLE}/${id}`, {
                headers: {
    ...
    const editSubmit = async (e) => {
        ...
        const isNameMatch = resdata.some(item => item.manageassignedname.toLowerCase() == manageAssignedByEdit.manageassignedname.toLowerCase());
    ...
    const fetchEmployee = async () => {
        setPageName(!pageName)
        try {              
            let res_employee = await axios.post(SERVICE.ALL_MANAGEASSIGNED_SORT, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                page: Number(page),
            ...
            setManageAssignedbyArrayEdit(itemsWithSerialNumber.filter((item) => {
                return item._id !== manageId
            }))
            setManageAssignedByEdit(itemsWithSerialNumber)
    ...
    const fetchSourceAll = async () => {
        ...
        try {              
            ...
            return res_meet?.data?.manageassignedmode.filter(item => item._id !== manageAssignedByEdit._id)
        } catch (err) { handleApiError(err, setShowAlert, handleClickOpenerr); }
    }
    //print...
                            ...
                            <Grid item md={2} xs={12} sm={12}>
                                <Box>
                                    ...
                                    <Select id="pageSizeSelect" value={pageSize}
                                        ...
                                        {/* <MenuItem value={(manageAssigned?.length)}>All</MenuItem> */}
                                    </Select>
                                </Box>
                            </Grid>
