...
function Erroruploadconfirm() {
  ...
  const tableOptions = [
    { label: 'Error Upload Confirm List', value: 'Error Upload Confirm List' },
    { label: 'Manual Error Not Confirm List', value: 'Manual Error Not Confirm List' },
    { label: 'Completed List', value: 'Completed List' },
  ];
  ...
  const customValueRendererCompany = (valueTable, _categoryname) => {
    return valueTable?.length ? valueTable.map(({ label }) => label)?.join(', ') : 'Please Select Table Mode';
  };
  ...
  const exportColumnNames = ['Employee Name', 'Branch', 'Unit', 'Project Vendor', 'Date', 'Queue Name', 'Login ID', 'Accuracy', 'Total Fields', 'Error Count', 'Auto Count', 'Manual Total', 'Manual Error', 'Upload Count', 'Level'];
  const exportRowValues = ['companyname', 'branchname', 'unitname', 'projectvendor', 'date', 'queuename', 'loginid', 'accuracy', 'totalfields', 'errorcount', 'autocount', 'manualtotal', 'manualerror', 'Manual Error', 'uploadcount', 'level'];

  const exportColumnNamesnot = ['Employee Name', 'Branch', 'Unit', 'Project Vendor', 'Date', 'Queue Name', 'Login ID', 'Accuracy', 'Total Fields', 'Error Count', 'Auto Count', 'Manual Error', 'Upload Count', 'Level'];
  const exportRowValuesnot = ['companyname', 'branchname', 'unitname', 'projectvendor', 'date', 'queuename', 'loginid', 'accuracy', 'totalfields', 'errorcount', 'autocount', 'manualerror', 'uploadcount', 'level'];

  const exportColumnNamescom = ['Employee Name', 'Branch', 'Unit', 'Project Vendor', 'Date', 'Queue Name', 'Login ID', 'Accuracy', 'Total Fields', 'Error Count', 'Auto Count', 'Manual Error', 'Upload Count', 'Level'];
  const exportRowValuescom = ['companyname', 'branchname', 'unitname', 'projectvendor', 'date', 'queuename', 'loginid', 'accuracy', 'totalfields', 'errorcount', 'autocount', 'manualerror', 'uploadcount', 'level'];
  ...
  const [isusernotcompleted, setisuserNotcompleted] = useState([]);
  ...
  const handleprintnot = useReactToPrint({
    ...
    documentTitle: 'Not Completed List',
    pageStyle: 'print',
  });
  ...
  const columnDataTable = [
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <>
          {isUserRoleCompare?.includes('eerroruploadconfirmsupervisoredit') && (
            <Grid sx={{ display: 'flex' }}>
              <Button
                ...
                size="small"
                onClick={() => {
                  sendEditRequest(params.data.id, params.data.manualtotal, 'manualtotal', params.node.data.manualerror);
                }}
              >
                UPDATE
              </Button>
 ...
 disabled={  Number(params.data.totalfields) !== 0  }                size="small"
  ...
  console.log(rowDataTable, 'rowdat');
  ...
  const [columnVisibilitynot, setColumnVisibilitynot] = useState(initialColumnVisibilitynot);
  ...
  useEffect(() => {
    addSerialNumbernot(isusernotcompleted);
  }, [isusernotcompleted]);
  ...
  const filteredDatasnot = itemsnot?.filter((item) => {
    return searchTermsnot.every((term) => Object.values(item).join(' ').toLowerCase().includes(term));
  });

  const filteredDatanot = filteredDatasnot?.slice((pagenot - 1) * pageSizenot, pagenot * pageSizenot);
  ...
  const lastVisiblePagenot = Math.min(firstVisiblePagenot + visiblePagesnot - 1, totalPagesnot);
  ...
  const columnDataTablenot = [
    ...
    },
    {
      ...
      flex: 0,
      width: 130,
      hide: !columnVisibilitynot.companyname,
      headerClassName: 'bold-header',
      pinned: 'left',
    },
    {
    ...
    },
    {
      ...
      flex: 0,
      width: 130,
      hide: !columnVisibilitynot.unitname,
      headerClassName: 'bold-header',
      pinned: 'left',
    },
    {
    ...
    },
    {
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilitynot.accuracy,
      headerClassName: 'bold-header',
    },
    {
      field: 'totalfields',
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilitynot.totalfields,
      headerClassName: 'bold-header',
    },

    {
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilitynot.errorcount,
      headerClassName: 'bold-header',
    },
    {
      field: 'autocount',
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilitynot.autocount,
      headerClassName: 'bold-header',
    },
    // {
    //   headerName: 'User Manual Error (DoubleClick Editable)',
    ...
    },
    {
      ...
      minHeight: '40px !important',
      sortable: false,
      hide: !columnVisibilitynot.actions1,
      headerClassName: 'bold-header',
      cellRenderer: (params) => (
        <>
          {
    ...
    },
    {
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilitynot.uploadcount,
      headerClassName: 'bold-header',
    },
    {
      field: 'level',
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilitynot.level,
      headerClassName: 'bold-header',
    },
  ];
  // console.log(filteredData, "lll")
  ...
  // // Function to filter columns based on search query
  const filteredColumnsnot = columnDataTablenot.filter((column) => column.headerName.toLowerCase().includes(searchQueryManagenot.toLowerCase()));
  ...
  const manageColumnsContentnot = (
    ...
    >
      ...
      <Box sx={{ position: 'relative', margin: '10px' }}>
        <TextField label="Find column" variant="standard" fullWidth value={searchQueryManagenot} onChange={(e) => setSearchQueryManagenot(e.target.value)} sx={{ marginBottom: 5, position: 'absolute' }} />
      </Box>
      <br />
      <br />
      <DialogContent sx={{ minWidth: 'auto', height: '200px', position: 'relative' }}>
        <List sx={{ overflow: 'auto', height: '100%' }}>
          {filteredColumnsnot.map((column) => (
            <ListItem key={column.field}>
              <ListItemText
                sx={{ display: 'flex' }}
                primary={<Switch sx={{ marginTop: '-5px' }} size="small" checked={columnVisibilitynot[column.field]} onChange={() => toggleColumnVisibilitynot(column.field)} />}
                secondary={column.field === 'checkbox' ? 'Checkbox' : column.headerName}
                // secondary={column.headerName }
              />
            </ListItem>
      ...
      <DialogActions>
        <Grid container>
          <Grid item md={4}>
            <Button variant="text" sx={{ textTransform: 'none' }} onClick={() => setColumnVisibilitynot(initialColumnVisibilitynot)}>
              Show All
            </Button>
          </Grid>
          <Grid item md={4}></Grid>
  ...
  const columnDataTablecom = [
    ...
    },
    {
      ...
      flex: 0,
      width: 130,
      hide: !columnVisibilitycom.branchname,
      headerClassName: 'bold-header',
      pinned: 'left',
    },
    {
      ...
      flex: 0,
      width: 130,
      hide: !columnVisibilitycom.unitname,
      headerClassName: 'bold-header',
      pinned: 'left',
    },
    {
      ...
      flex: 0,
      width: 130,
      hide: !columnVisibilitycom.projectvendor,
      headerClassName: 'bold-header',
      pinned: 'left',
    },
    {
    ...
    },
    {
      ...
      flex: 0,
      width: 130,
      hide: !columnVisibilitycom.queuename,
      headerClassName: 'bold-header',
    },
    {
      field: 'loginid',
      ...
      flex: 0,
      width: 130,
      hide: !columnVisibilitycom.loginid,
      headerClassName: 'bold-header',
    },
    {
      field: 'accuracy',
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilitycom.accuracy,
      headerClassName: 'bold-header',
    },
    {
      field: 'totalfields',
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilitycom.totalfields,
      headerClassName: 'bold-header',
    },

    {
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilitycom.errorcount,
      headerClassName: 'bold-header',
    },
    {
      field: 'autocount',
...

    {
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilitycom.uploadcount,
      headerClassName: 'bold-header',
    },
    {
      field: 'level',
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilitycom.level,
      headerClassName: 'bold-header',
    },
  ];
  // console.log(filteredData, "lll")
  ...
  const manageColumnsContentcom = (
    ...
    >
      ...
      <Box sx={{ position: 'relative', margin: '10px' }}>
        <TextField label="Find column" variant="standard" fullWidth value={searchQueryManagecom} onChange={(e) => setSearchQueryManagecom(e.target.value)} sx={{ marginBottom: 5, position: 'absolute' }} />
      </Box>
      <br />
      <br />
      <DialogContent sx={{ minWidth: 'auto', height: '200px', position: 'relative' }}>
        <List sx={{ overflow: 'auto', height: '100%' }}>
          {filteredColumnscom.map((column) => (
            <ListItem key={column.field}>
              <ListItemText
                sx={{ display: 'flex' }}
                primary={<Switch sx={{ marginTop: '-5px' }} size="small" checked={columnVisibilitycom[column.field]} onChange={() => toggleColumnVisibilitycom(column.field)} />}
                secondary={column.field === 'checkbox' ? 'Checkbox' : column.headerName}
                // secondary={column.headerName }
              />
            </ListItem>
  ...
  const fetchBatchFilter = async () => {
    ...
    try {
      let [res_employee, res_completed] = await Promise.all([
        axios.post(SERVICE.ERROR_UPLOAD_CONFIRM_HIERARCHY, {
          headers: {
        ...
        axios.post(SERVICE.ERROR_UPLOAD_CONFIRM_HIERARCHY_COMPLETED, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          hierachy: modeselection.value,
      ...
      const filtered = res_employee?.data?.resultAccessFilter.filter((item) => item.isedited === 'true' && item != null);

      if (valueTable?.includes('Error Upload Confirm List')) {
        let finalDataAll = filtered?.map((item, index) => {
          return {
            ...item,
        ...
        // setFilteredRowData(finalDataAll)
        setisuser(finalDataAll);
        setTableCheck(valueTable);
      }

      const filterednot = res_employee?.data?.resultAccessFilter.filter((item) => item.isedited !== 'true' && item != null);
      if (valueTable?.includes('Manual Error Not Confirm List')) {
        let finalDataAllnot = filterednot?.map((item, index) => {
          return {
            ...item,
        ...
        // setFilteredRowDatanot(finalDataAllnot)
        setisuserNotcompleted(finalDataAllnot);
        setTableCheck(valueTable);
      }

      const filteredcom = res_completed?.data?.resultAccessFilter.filter((item) => item != null);

      if (valueTable?.includes('Completed List')) {
        let finalDataAllcom = filteredcom?.map((item, index) => {
          return {
            ...item,
        ...
        // setFilteredRowDatacom(finalDataAllcom)
        setisusercompleted(finalDataAllcom);
        setTableCheck(valueTable);
      }
      setIsLoading(false);
      setLoader(false);
  ...
  const sendEditRequest = async (id, manualvalue,data, type, manualerror) => {
    ...
    try {
      if (type == 'manualerror') {
        ...
        if (manualerror === '' || manualerror == undefined || manualerror == 'undefined') {
          setPopupContentMalert('Please Enter User Manual Error');
          setPopupSeverityMalert('warning');
          handleClickOpenPopupMalert();
        } else if (manualvalue === '' || manualvalue == undefined) {
          setPopupContentMalert('Please Enter Value');
          setPopupSeverityMalert('warning');
          handleClickOpenPopupMalert();
        } else {
 ...
 console.log(data.uploadcount,manualvalue, "manualvalue")
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Error Upload Confirm" modulename="Quality" submodulename="Penalty" mainpagename="Penalty Setup" subpagename="Penalty Calculation" subsubpagename="Error Upload Confirm" />
      <Box>
        ...
        {tableCheck?.includes('Error Upload Confirm List') ? (
          <>
            <Box sx={userStyle.dialogbox}>
              <>
                <Grid item xs={8}>
                ...
                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                  ...
                  >
                    <Box>
                      {isUserRoleCompare?.includes('excelerroruploadconfirm') && (
                        <>
                          <Button
                            onClick={(e) => {
                      ...
                      {isUserRoleCompare?.includes('csverroruploadconfirm') && (
                        <>
                          <Button
                            onClick={(e) => {
                              setIsFilterOpen(true);
                      ...
                      {isUserRoleCompare?.includes('printerroruploadconfirm') && (
                        <>
                      ...
                      {isUserRoleCompare?.includes('pdferroruploadconfirm') && (
                        <>
                          <Button
                            sx={userStyle.buttongrp}
                            onClick={() => {
                      ...
                      {isUserRoleCompare?.includes('imageerroruploadconfirm') && (
        ...
        {tableCheck?.includes('Manual Error Not Confirm List') ? (
          <>
            <Box sx={userStyle.dialogbox}>
              <>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>Manual Error Not Confirm List</Typography>
                </Grid>
                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                  <Grid item md={2} xs={12} sm={12}>
                    <Box>
                      ...
                      >
                        ...
                        <MenuItem value={50}>50</MenuItem>
                        <MenuItem value={100}>100</MenuItem>
                        <MenuItem value={isusernotcompleted?.length}>All</MenuItem>
                      </Select>
                    </Box>
                  </Grid>
                  <Grid
        ...
        <br />
        {tableCheck?.includes('Completed List') ? (
          <>
            <Box sx={userStyle.dialogbox}>
              <>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>Completed List</Typography>
                </Grid>
                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                  <Grid item md={2} xs={12} sm={12}>
                    <Box>
      ...
      <ExportData
        ...
        itemsTwo={isusernotcompleted ?? []}
        filename={'Manual Error Update Confirm_List'}
