...
function RaiseTicketForwardedEmployee() {
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("RaiseTicket_Forwarded_Tickets.pdf");
    };
    ...
    const fetchAllRaisedTicketsed = async () => {
        try {
            ...
            setRaiseTicketList(response?.data?.raisetickets);
            setQueueCheck(true);
    ...
    // Excel
    const fileName = "RaiseTicket_Forwarded_Tickets";
    let snos = 1;
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "RaiseTicket_Forwarded_Tickets.png");
                });
            });
        }
    };
