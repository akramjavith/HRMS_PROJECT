...
import { FaFileCsv, FaFileExcel, FaFilePdf, FaPlus, FaPrint, FaTrash, FaSearch } from "react-icons/fa";
...
function Noticeperiodapply() {
  ...
  const handleCheckboxChangeEdit = (event) => {
    setIsCheckedEdit(event.target.checked);
    setSelectedFilesedit([]);
  };

  const [filteredRowDataNew, setFilteredRowDataNew] = useState([]);
  const [filteredChangesNew, setFilteredChangesNew] = useState(null);
  ...
  const [employeesListNew, setEmployeesListNew] = useState([]);
  ...
  const handlePageSizeChangeNew = (event) => {
    setPageSizeNew(Number(event.target.value));
    setPageNew(1);
  };
  ...
  const gridRefTableImgNew = useRef(null);
  // image
  const handleCaptureImageNew = () => {
    if (gridRefTableImgNew.current) {
      domtoimage.toBlob(gridRefTableImgNew.current)
        .then((blob) => {
          saveAs(blob, "Notice Period Apply.png");
        })
        .catch((error) => {
  ...
  const [searchQueryManageNew, setSearchQueryManageNew] = useState("");
  // Manage Columns
  ...
  const idsNew = openNew ? "simple-popover" : undefined;
  ...
  const fetchNoticeperiodlist = async () => {
    ...
    try {
      ...
      let mappedUsers = response?.data?.users?.map((item) => item?.companyname);
      setOverallItems(res?.data?.noticeperiodapply.filter((data) => !mappedUsers?.includes(data?.empname)));
      setOverallItemsNew(res?.data?.noticeperiodapply.filter((data) => mappedUsers?.includes(data?.empname)));
  ...
  const [noticePeriodArrayNew, setNoticePeriodArrayNew] = useState([])
  const fetchNoticeperiodlistArrayNew = async () => {
    setPageName(!pageName);
    try {
      let res = await axios.post(SERVICE.NOTICEPERIODAPPLYBYASSIGNBRANCH, {
        assignbranch: accessbranch
      ...
      setNoticePeriodArrayNew(res?.data?.noticeperiodapply);
    } catch (err) { handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };

  useEffect(() => {
    fetchNoticeperiodlistArrayNew()
  }, [isFilterOpen])
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const isLoanExistUser =
      Accessdrop === 'Manager'
        ? overallSate.filter((item) => item.employeename?.toLowerCase() === selectedempname?.toLowerCase() && item.empcode?.toLowerCase() === selectedempcode?.toLowerCase())
        : overallSate.filter((item) => item.employeename?.toLowerCase() === isUserRoleAccess.companyname?.toLowerCase() && item.empcode?.toLowerCase() === isUserRoleAccess.empcode?.toLowerCase());
    ...
    if (isNoticeperiod > 0) {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (isUserRoleAccess.role.includes('Manager') && Access === 'Manager' && (selectedempname === '' || selectedempname === 'Please Select Employeename')) {
      setPopupContentMalert('Please Select Employeename');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
      // setIsBtn(false)
    } else if (selectedFiles.length < 1 && !isChecked) {
      ...
      if (filteredDesignationDates && filteredDesignationDates.length > 0) {
        if (filteredDesignationDates[0].noticeperiodfrom === 'Month') {
          ...
          let startMonthDate = new Date(noticePeriod.noticedate);
          ...
          const fsdate = resfinaldate[0]?.formattedDate?.split('/');
          finaldate = fsdate[2] + '-' + fsdate[1] + '-' + fsdate[0];
          ...
          function add10Days(inputDate) {
            return moment(inputDate).clone().add(10, 'days').format('YYYY-MM-DD');
          }

          function subtract10Days(inputDate) {
            return moment(inputDate).clone().subtract(10, 'days').format('YYYY-MM-DD');
          }

          function subtract7Days(inputDate) {
            return moment(inputDate).clone().subtract(7, 'days').format('YYYY-MM-DD');
          }

          function add7Days(inputDate) {
            return moment(inputDate).clone().add(7, 'days').format('YYYY-MM-DD');
          }
          ...
          let dayName = moment(finaldate).format('dddd').toLowerCase();


          if (!daysNameArray?.includes(dayName)) {
            const subtractedDate = subtract10Days(finaldate);
            const dayNameAfter = moment(subtractedDate).format('dddd').toLowerCase();
            const dayOfMonthSub = moment(subtractedDate).date();

            if (
              daysNameArray?.includes(dayNameAfter) &&
              dayOfMonthSub >= from &&
              dayOfMonthSub <= to
            ) {

              finaldate = subtractedDate;
            } else {

              const addedDate = add10Days(finaldate);
              const dayNameAfterAdd = moment(addedDate).format('dddd').toLowerCase();
              const dayOfMonthAdd = moment(addedDate).date();

              if (
                daysNameArray?.includes(dayNameAfterAdd) &&
                dayOfMonthAdd >= from &&
                dayOfMonthAdd <= to
              ) {
                ...
                const getDayNamesInRange = (year, month, from, to) => {
                  const dayNames = [];
                ...
                // Main function to get nearest date and day names
                const getNearestDateWithDayNames = (baseDateStr, from, to) => {

                  const baseDate = new Date(baseDateStr);
                  const year = baseDate.getFullYear();
                  const month = baseDate.getMonth(); // 0-based

                  const getCandidateDates = (y, m) => {
                    return Array.from({ length: to - from + 1 }, (_, i) => new Date(y, m, from + i));
                  };

                  const candidatesCurrent = getCandidateDates(year, month);
                  const candidatesNext = getCandidateDates(year, month + 1);
                  const candidatesPrev = getCandidateDates(year, month - 1);


                  const allCandidates = [
                    ...candidatesCurrent.map((d) => ({ date: d, source: 'current' })),
                    ...candidatesNext.map((d) => ({ date: d, source: 'next' })),
                    ...candidatesPrev.map((d) => ({ date: d, source: 'prev' })),
                  ];

                  const nearest = allCandidates.reduce((closest, current) => {
                    const currDiff = Math.abs(current.date - baseDate);
                    const closestDiff = Math.abs(closest.date - baseDate);
                    return currDiff < closestDiff ? current : closest;
                  });

                  return {
                    nearestDate: nearest.date.toLocaleDateString('en-CA'),
                    source: nearest.source,
                    dayNames: {
                      previous: getDayNamesInRange(year, month, from, to),       // current month
                      current: getDayNamesInRange(year, month + 1, from, to),    // next month
                      next: getDayNamesInRange(year, month + 2, from, to),       // previous month
                    },
                  };
                };

                // ✅ Example usage
                const result = getNearestDateWithDayNames(finaldate, from, to);
                let dayNameResult = moment(result?.nearestDate).format('dddd').toLowerCase();


                if (daysNameArray?.includes(dayNameResult)) {
                  finaldate = result?.nearestDate;
                } else {
                  const getMatchingDate = (dates, reverse = false) => {
                    const list = reverse ? [...dates].reverse() : dates;
                    for (let i = 0; i < list.length; i++) {
                      const dayNameNew = moment(list[i]).format('dddd').toLowerCase();
                      if (daysNameArray.includes(dayNameNew)) {
                        return list[i];
                      }
                    }
                    return null;
                  ...
                  const dayNames = result?.dayNames;
                  let dates;

                  if (result?.source === 'next') {
                    dates = dayNames?.next?.map((item) => item?.date) || [];
                    const matchedDate = getMatchingDate(dates);
                    if (matchedDate) finaldate = matchedDate;


                  } else if (result?.source === 'previous') {
                    dates = dayNames?.previous?.map((item) => item?.date) || [];
                    const matchedDate = getMatchingDate(dates, true); // reverse loop
                    if (matchedDate) finaldate = matchedDate;


                  } else if (result?.source === 'current') {
                    dates = dayNames?.current?.map((item) => item?.date) || [];
                    const matchedDate = getMatchingDate(dates);
                    if (matchedDate) finaldate = matchedDate;
            ...
            const baseDate = moment(finaldate, "YYYY-MM-DD");

            //checking
            const filteredDates = [];

            for (let day = from; day <= to; day++) {
              const current = baseDate.clone().date(day); // create date from baseDate
              const dayNameNew = current.format("dddd"); // get full day name like "Tuesday"

              if (daysNameArray.includes(dayNameNew?.toLowerCase())) {
                filteredDates.push(current.format("YYYY-MM-DD")); // or push the moment object if needed
              }
            }

            const subtractedDate = baseDate.clone().subtract(7, 'days');
            const addedDate = baseDate.clone().add(7, 'days');

            const candidates = [];

            // Check if original date is within range
            if (baseDate.date() >= from && baseDate.date() <= to) {
              candidates.push(baseDate);
            }

            if (subtractedDate.date() >= from && subtractedDate.date() <= to) {
              candidates.push(subtractedDate);
            }

            if (addedDate.date() >= from && addedDate.date() <= to) {
              candidates.push(addedDate);
            }

            if (candidates.length > 0) {
              // Choose the one with the smallest absolute difference in days from original finaldate
              let nearest = candidates[0];
              let minDiff = Math.abs(baseDate.diff(nearest, 'days'));

              for (let i = 1; i < candidates.length; i++) {
                const diff = Math.abs(baseDate.diff(candidates[i], 'days'));
                if (diff < minDiff) {
              ...
              finaldate = nearest.format("YYYY-MM-DD");
              if (!filteredDates?.includes(finaldate)) {
                const targetDate = moment(finaldate, 'YYYY-MM-DD');

                // Convert all dates to moment objects and sort them
                ...
                for (let date of sortedDates) {
                  if (date.isBefore(targetDate)) {
                    lower = date;
                  } else if (date.isAfter(targetDate) && !higher) {
                    higher = date;
                    break; // we can stop early once we found the first higher
                  }
                }
                if (lower) {
                  finaldate = lower.format("YYYY-MM-DD");
                } else if (higher) {
                  finaldate = higher.format("YYYY-MM-DD");
                }
              }
            } else {
              // No valid date fell within range: pick nearest boundary day (from or to)
              const day = baseDate.date();
              const diffToFrom = Math.abs(day - from);
              const diffToTo = Math.abs(day - to);
              const nearestDay = diffToFrom <= diffToTo ? from : to;

              finaldate = baseDate.clone().date(nearestDay).format("YYYY-MM-DD");
              if (!filteredDates?.includes(finaldate)) {
                const targetDate = moment(finaldate, 'YYYY-MM-DD');
  ...
  const getCode = async (e, name) => {
    ...
    try {
      ...
      setIsCheckedEdit(res?.data?.snoticeperiodapply?.hrupload);
  ...
  const rowDataNew = async (id, name) => {
    setPageName(!pageName);
    try {
      let res = await axios.get(`${SERVICE.NOTICEPERIODAPPLY_SINGLE}/${id}`, {
        headers: {
      ...
      if (requestDateStatusEdit && requestDateEdit === "") {
        ...
        let res = await axios.put(
          `${SERVICE.NOTICEPERIODAPPLY_SINGLE}/${refsid}`,
          {
            ...
            recheckStatus: "false",
            hrupload: Boolean(isCheckedEdit),
            rechecknoticereq: "",
            updatedby: [
              ...updateby,
              {
  ...
  const editSubmit = (e) => {
    ...
    if (isNoticeperiodEdit > 0) {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (selectedFilesedit.length < 1 && !isCheckedEdit) {
  ...
  const initialColumnVisibilityNew = {
    checkbox: true,
    actions: true,
    serialNumber: true,
    branch: true,
  ...
  const [columnVisibilityNew, setColumnVisibilityNew] = useState(
    initialColumnVisibilityNew
  );
  ...
  const [totalDatasNew, setTotalDatasNew] = useState(0);
  ...
  const fetchChecklisttype = async () => {
    ...
    try {
      ...
      setEmployeesList(itemsWithSerialNumber);
      setLoading(false);
  ...
  const [advancedFilterNew, setAdvancedFilterNew] = useState(null);


  const [logicOperatorNew, setLogicOperatorNew] = useState("AND");

  const [selectedColumnNew, setSelectedColumnNew] = useState("");
  const [selectedConditionNew, setSelectedConditionNew] = useState("Contains");
  const [filterValueNew, setFilterValueNew] = useState("");
  const [additionalFiltersNew, setAdditionalFiltersNew] = useState([]);
  const [isSearchActiveNew, setIsSearchActiveNew] = useState(false);
  const conditionsNew = ["Contains", "Does Not Contain", "Equals", "Does Not Equal", "Begins With", "Ends With", "Blank", "Not Blank"]; // AgGrid-like conditions

  // Search bar
  const [anchorElSearchNew, setAnchorElSearchNew] = React.useState(null);
  const handleClickSearchNew = (event) => {
    setAnchorElSearchNew(event.currentTarget);
    localStorage.removeItem("filterModel");
  };
  const handleCloseSearchNew = () => {
    setAnchorElSearchNew(null);
    setSearchQueryNew("");
  };

  const openSearchNew = Boolean(anchorElSearchNew);
  const idSearchNew = openSearchNew ? 'simple-popover' : undefined;

  const handleAddFilterNew = () => {
    if (selectedColumnNew && filterValueNew || ["Blank", "Not Blank"].includes(selectedConditionNew)) {
      setAdditionalFiltersNew([
        ...additionalFiltersNew,
        { column: selectedColumnNew, condition: selectedConditionNew, value: filterValueNew }
      ]);
      setSelectedColumnNew("");
      setSelectedConditionNew("Contains");
      setFilterValueNew("");
    }
  };

  // Show filtered combination in the search bar
  const getSearchDisplayNew = () => {
    if (advancedFilterNew && advancedFilterNew.length > 0) {
      return advancedFilterNew.map((filter, index) => {
        let showname = columnDataTableNew.find(col => col.field === filter.column)?.headerName;
        return `${showname} ${filter.condition} "${filter.value}"`;
      }).join(' ' + (advancedFilterNew.length > 1 ? advancedFilterNew[1].condition : '') + ' ');
    }
    return searchQueryNew;
  };

  useEffect(() => {
    fetchChecklisttypeNew();
  }, [pageNew, pageSizeNew, searchQueryNew])



  const fetchChecklisttypeNew = async () => {
    setPageName(!pageName)
    // setLoading(true);
    const queryParams = {
      page: Number(pageNew),
      pageSize: Number(pageSizeNew),
    };
    ...
    // Only include advanced filters if they exist, otherwise just use regular searchQuery
    if (allFilters.length > 0 && selectedColumnNew !== "") {

      queryParams.allFilters = allFilters
      queryParams.logicOperator = logicOperator;
    } else if (searchQueryNew) {
      queryParams.searchQuery = searchQueryNew;  // Use searchQuery for regular search
    }
    ...
    try {
      ...
      const itemsWithSerialNumber = ans?.map((item, index) => ({
        ...item,
        serialNumber: (pageNew - 1) * pageSizeNew + index + 1,
        noticedate: item.noticedate === "undefined" ? "" : moment(item.noticedate).format("DD-MM-YYYY"),
        approvenoticereq: moment(item.approvenoticereq).format("DD-MM-YYYY"),
        meetingcompleted: item?.meetingcompleted ? 'Yes' : 'No'
      }));
      ...
      setTotalDatasNew(ans?.length > 0 ? res_status?.data?.totalProjects : 0);
      setTotalPagesNew(ans?.length > 0 ? res_status?.data?.totalPages : 0);
      setPageSizeNew((data) => { return ans?.length > 0 ? data : 10 });
      setPageNew((data) => { return ans?.length > 0 ? data : 1 });
      setEmployeesListNew(itemsWithSerialNumber);
      setLoading(false);
  ...
  const handleResetSearchNew = async () => {
    setPageName(!pageName)


    // Reset all filters and pagination state
    setAdvancedFilterNew(null);
    setAdditionalFiltersNew([]);
    setSearchQueryNew("");
    setIsSearchActiveNew(false);
    setSelectedColumnNew("");
    setSelectedConditionNew("Contains");
    setFilterValueNew("");
    setLogicOperatorNew("AND");
    setFilteredChangesNew(null);
  ...
  const fetchNoticeperiodlistEdit = async () => {
    ...
    try {
      ...
      let mappedUsers = response?.data?.users?.map((item) => item?.companyname);
      setDeactivatedUsers(mappedUsers);
      let answer =
        !isUserRoleAccess.role.includes("Manager") &&
          !isUserRoleAccess.role.includes("HiringManager") &&
          !isUserRoleAccess.role.includes("HR")
      ...
      let finalDatas = answer?.map((item) => ({
        ...item, noticedate: item.noticedate === "undefined" ? "" : moment(item.noticedate).format("DD-MM-YYYY"),
        approvenoticereq: moment(item.approvenoticereq).format("DD-MM-YYYY"),
        meetingcompleted: item?.meetingcompleted ? 'Yes' : 'No'
      }));

      // setEmployeesList(finalDatas.filter((data) => !mappedUsers?.includes(data?.empname)));
      // setEmployeesListNew(finalDatas.filter((data) => mappedUsers?.includes(data?.empname)));
      setLoading(false);
    } catch (err) {
      setLoading(false);
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
  ...
  const fileDataDownloadNew = async (id) => {
    setPageName(!pageName);
    try {
      let res = await axios.get(`${SERVICE.NOTICEPERIODAPPLY_SINGLE}/${id}`, {
        headers: {
      ...
      setFileNew(res?.data?.snoticeperiodapply.files);
      res?.data?.snoticeperiodapply.files.forEach((file) => {
        const link = document.createElement("a");
        link.href = `data:application/octet-stream;base64,${file.base64}`;
        link.download = file.name;
  ...
  const addSerialNumberNew = (datas) => {
  ...
  useEffect(() => {
    addSerialNumberNew(employeesListNew);
  }, [employeesListNew]);
  ...
  const handleSearchChangeNew = (event) => {
    setSearchQueryNew(event.target.value);
  };

  // Split the search query into individual terms
  const searchTermsNew = searchQueryNew.toLowerCase().split(" ");

  // Modify the filtering logic to check each term
  const filteredDatasNew = itemsNew?.filter((item) => {
    return searchTermsNew.every((term) =>
      Object.values(item).join(" ").toLowerCase().includes(term)
    );
  });

  const filteredDataNew = filteredDatasNew.slice(
    (page - 1) * pageSize,
    page * pageSize
  );
        ...
        notice?.rejectStatus === "true"
          ...
          : notice?.cancelstatus === true
            ...
            : notice?.continuestatus === true
              ...
              : notice.recheckStatus === "true" ? notice.rechecknoticereq : "---",
  ...
  const columnDataTableNew = [
    {
      ...
      hide: !columnVisibilityNew.checkbox,
      headerClassName: "bold-header",
    },

    {
      ...
      flex: 0,
      width: 75,
      hide: !columnVisibilityNew.serialNumber,
      pinned: 'left',
    },
    {
      field: "company",
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibilityNew.company,
      pinned: 'left',
    },
    {
      field: "branch",
      ...
      flex: 0,
      width: 200,
      hide: !columnVisibilityNew.branch,
      pinned: 'left',
    },
    {
      field: "unit",
    ...
    },
    {
      ...
      flex: 0,
      width: 200,
      hide: !columnVisibilityNew.empname,
    },
    {
      field: "empcode",
      headerName: "Empcode",
      flex: 0,
      width: 200,
      hide: !columnVisibilityNew.empcode,
    },
    {
      field: "team",
      headerName: "Team",
    ...
    },
    {
      ...
      flex: 0,
      width: 200,
      hide: !columnVisibilityNew.department,
    },
    {
      field: "noticedate",
      headerName: "Apply Date",
      flex: 0,
      width: 200,
      hide: !columnVisibilityNew.noticedate,
    },
    {
      field: "reasonleavingname",
      headerName: "Reason",
      flex: 0,
      width: 200,
      hide: !columnVisibilityNew.reasonleavingname,
    },
    {
      field: "files",
      headerName: "Document",
      ...
      width: 200,
      sortable: false,
      hide: !columnVisibilityNew.actions,
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex", gap: '10px' }}>
          <Button
            sx={{
          ...
          <Button
            ...
            variant="contained"
            onClick={() => {
              fileDataDownloadNew(params.data.id);
            }}
          >
            Download
          </Button>
    ...
    },
    {
      ...
      hide: !columnVisibilityNew.status,
    },
    {
      field: "rejectStatus",
      headerName: "Reason",
      flex: 0,
      width: 200,
      hide: !columnVisibilityNew.rejectStatus,
    },
    {
      field: "approvenoticereq",
      headerName: "Release Date",
      flex: 0,
      width: 200,
      hide: !columnVisibilityNew.approvenoticereq,
    },
    {
      field: "actions",
      headerName: "Action",
  ...
  const filteredSelectedColumnNew = columnDataTableNew.filter(data => data.field !== 'checkbox' && data.field !== "actions" && data.field !== "serialNumber");

  // Create a row data object for the DataGrid
  const rowDataTableNew = itemsNew.map((notice, index) => {
    return {
      id: notice._id,
      index: index + 1,
      serialNumber: notice.serialNumber,
  ...
  const handleShowAllColumnsNew = () => {
    const updatedVisibility = { ...columnVisibilityNew };
    for (const columnKey in updatedVisibility) {
      updatedVisibility[columnKey] = true;
    }
    setColumnVisibilityNew(updatedVisibility);
  };

  // Manage Columns functionality
  const toggleColumnVisibilityNew = (field) => {
    setColumnVisibilityNew((prevVisibility) => ({
      ...prevVisibility,
      [field]: !prevVisibility[field],
    }));
  };
  ...
  // Function to filter columns based on search query
  const filteredColumnsNew = columnDataTableNew.filter((column) =>
    column.headerName.toLowerCase().includes(searchQueryManageNew.toLowerCase())
  );

  // JSX for the "Manage Columns" popover content
  const manageColumnsContentNew = (
    <Box
      sx={{
        padding: "10px",
        minWidth: "325px",
    ...
    >
      ...
      <Box sx={{ position: "relative", margin: "10px" }}>
        <TextField
          ...
          variant="standard"
          fullWidth
          value={searchQueryManageNew}
          onChange={(e) => setSearchQueryManageNew(e.target.value)}
          sx={{ marginBottom: 5, position: "absolute" }}
        />
      </Box>
      <br />
              ...
              <ListItemText
                ...
                primary={
                  <Switch
                    sx={{ marginTop: "-5px" }}
                    size="small"
                    checked={columnVisibilityNew[column.field]}
                    onChange={() => toggleColumnVisibilityNew(column.field)}
                  />
                }
                secondary={column.headerName}
              />
        ...
        <Grid container>
          <Grid item md={4}>
            <Button
              variant="text"
              sx={{ textTransform: "none" }}
              onClick={() => setColumnVisibilityNew(initialColumnVisibilityNew)}
            >
              Show All
            </Button>
          </Grid>
          ...
          <Grid item md={4}>
            <Button
              variant="text"
              sx={{ textTransform: "none" }}
              onClick={() => setColumnVisibilityNew({})}
            >
              Hide All
            </Button>
          </Grid>
  ...
  const [fileFormatNew, setFormatNew] = useState('')
    ...
    <Box>
          ...
          <Box sx={userStyle.container}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={4} xs={12} sm={6}>
                <FormControl fullWidth size="small">
                  <Typography>
                    Upload Files {!isChecked && <b style={{ color: "red" }}>*</b>}
                  </Typography>
              ...
              <Grid item md={4} xs={12} sm={6} mt={2}>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={isChecked}
      ...
      <Box sx={userStyle.container}>
        <Box>
          ...
          {isUserRoleCompare?.includes("lnoticeperiodapply") && (
            <>
              <Typography sx={userStyle.HeaderText} onClick={(e) => {

              }}>{"Notice Period List (Live Users)"}</Typography>
              <br />
              <Box>
              ...
              <Box sx={{ padding: "20px 50px" }}>
                <>
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item md={4} xs={12} sm={6}>
                      <FormControl fullWidth size="small">
                        {isCheckedEdit ? <Typography>
                          Upload Files
                        </Typography> : <Typography>
                          Upload Files <span style={{ color: "red", fontWeight: "bold" }}>*</span>
                        </Typography>}
      ...
      <Box sx={userStyle.container}>
        <Box>
          {isUserRoleCompare?.includes("lnoticeperiodapply") && (
            <>
              <Typography sx={userStyle.HeaderText}>{"Notice Period List (Deactivated Users)"}</Typography>
              <br />
              <Box>
                {/* added to the pagination grid */}
                <Grid style={{ ...userStyle.dataTablestyle, display: 'flex', justifyContent: 'space-between' }}>
                  <Box>
                    ...
                    <Select
                      id="pageSizeSelect"
                      value={pageSizeNew}
                      onChange={handlePageSizeChangeNew}
                      sx={{ width: "77px" }}
                    >
                      <MenuItem value={1}>1</MenuItem>
                      <MenuItem value={5}>5</MenuItem>
                      ...
                      <MenuItem value={50}>50</MenuItem>
                      <MenuItem value={100}>100</MenuItem>
                      <MenuItem value={employeesListNew?.length}>All</MenuItem>
                    </Select>
                  </Box>
                  <Grid container sx={{ justifyContent: "center" }}>
                    <Grid>
                      ...
                      {isUserRoleCompare?.includes("printnoticeperiodapply") && (
                        <>
                          <Button sx={userStyle.buttongrp} onClick={handleprintNew}>
                            &ensp;
                            <FaPrint />
                            &ensp;Print&ensp;
                          </Button>
                  ...
                  {/* <Grid md={4} sm={2} xs={1}>
                    <AggregatedSearchBar columnDataTable={columnDataTableNew} setItems={setItemsNew} addSerialNumber={addSerialNumberNew} setPage={setPageNew} maindatas={employeesListNew} setSearchedString={setSearchedStringNew}
                      searchQuery={searchQueryNew}
                      setSearchQuery={setSearchQueryNew}
                      paginated={false}
                      totalDatas={overallItemsNew}
                    />
                  </Grid> */}
                    ...
                    <FormControl fullWidth size="small">
                      <OutlinedInput size="small"
                        ...
                        endAdornment={
                          <InputAdornment position="end">
                            {advancedFilterNew && (
                              <IconButton onClick={handleResetSearchNew}>
                                <MdClose />
                              </IconButton>
                            )}
                            <Tooltip title="Show search options">
                              <span>
                                <IoMdOptions style={{ cursor: 'pointer', }} onClick={handleClickSearchNew} />
                              </span>
                            </Tooltip>
                          </InputAdornment>}
                        aria-describedby="outlined-weight-helper-text"
                        inputProps={{ 'aria-label': 'weight', }}
                        type="text"
                        value={getSearchDisplayNew()}
                        onChange={handleSearchChangeNew}
                        placeholder="Type to search..."
                        disabled={!!advancedFilterNew}
                      />
                    </FormControl>
                  </Grid>
                ...
                <Button
                  sx={userStyle.buttongrp}
                  onClick={() => {
                    handleShowAllColumnsNew();
                    setColumnVisibilityNew(initialColumnVisibilityNew);
                  }}
                >
                  Show All Columns
                </Button>
                ...
                <>
                  {/* <AggridTable
                    rowDataTable={rowDataTableNew}
                    columnDataTable={columnDataTableNew}
                    columnVisibility={columnVisibilityNew}
                    page={pageNew}
                    setPage={setPageNew}
                    pageSize={pageSizeNew}
                    totalPages={totalPagesNew}
                    setColumnVisibility={setColumnVisibilityNew}
                    isHandleChange={isHandleChangeNew}
                    items={itemsNew}
                    selectedRows={selectedRowsNew}
                    setSelectedRows={setSelectedRowsNew}
                    gridRefTable={gridRefNew}
                    paginated={false}
                    filteredDatas={filteredDatasNew}
                    handleShowAllColumns={handleShowAllColumnsNew}
                    setFilteredRowData={setFilteredRowDataNew}
                    filteredRowData={filteredRowDataNew}
                    setFilteredChanges={setFilteredChangesNew}
                    filteredChanges={filteredChangesNew}
                    gridRefTableImg={gridRefTableImgNew}
                    itemsList={overallItemsNew}
                  /> */}
                  ...
                  <Popover
                    id={idSearchNew}
                    open={openSearchNew}
                    anchorEl={anchorElSearchNew}
                    onClose={handleCloseSearchNew}
                    anchorOrigin={{ vertical: 'bottom', horizontal: 'right', }}
                  >
                    <Box style={{ padding: "10px", maxWidth: '450px' }}>
                      <Typography variant="h6">Advance Search</Typography>
                            ...
                            <Grid container spacing={1}>
                              <Grid item md={12} sm={12} xs={12}>
                                ...
                                <Select fullWidth size="small"
                                  ...
                                  style={{ minWidth: 150 }}
                                  value={selectedColumnNew}
                                  onChange={(e) => setSelectedColumnNew(e.target.value)}
                                  displayEmpty
                                >
                                  <MenuItem value="" disabled>Select Column</MenuItem>
                                  {filteredSelectedColumnNew.map((col) => (
                                    <MenuItem key={col.field} value={col.field}>
                                      {col.headerName}
                                    </MenuItem>
                                  ))}
                              ...
                              <Grid item md={12} sm={12} xs={12}>
                                ...
                                <Select fullWidth size="small"
                                  ...
                                  style={{ minWidth: 150 }}
                                  value={selectedConditionNew}
                                  onChange={(e) => setSelectedConditionNew(e.target.value)}
                                  disabled={!selectedColumnNew}
                                >
                                  {conditionsNew.map((condition) => (
                                    <MenuItem key={condition} value={condition}>
                                      {condition}
                                    </MenuItem>
                                  ))}
                              ...
                              <Grid item md={12} sm={12} xs={12}>
                                <Typography>Value</Typography>
                                <TextField fullWidth size="small"
                                  value={["Blank", "Not Blank"].includes(selectedConditionNew) ? "" : filterValueNew}
                                  onChange={(e) => setFilterValueNew(e.target.value)}
                                  disabled={["Blank", "Not Blank"].includes(selectedConditionNew)}
                                  placeholder={["Blank", "Not Blank"].includes(selectedConditionNew) ? "Disabled" : "Enter value"}
                                  sx={{
                                    '& .MuiOutlinedInput-root.Mui-disabled': {
                                      backgroundColor: 'rgb(0 0 0 / 26%)',
                                    },
                              ...
                              {additionalFiltersNew.length > 0 && (
                                <>
                                  <Grid item md={12} sm={12} xs={12}>
                                    <RadioGroup
                                      row
                                      value={logicOperatorNew}
                                      onChange={(e) => setLogicOperatorNew(e.target.value)}
                                    >
                                      <FormControlLabel value="AND" control={<Radio />} label="AND" />
                                      <FormControlLabel value="OR" control={<Radio />} label="OR" />
                                    </RadioGroup>
                              ...
                              {additionalFiltersNew.length === 0 && (
                                <Grid item md={4} sm={12} xs={12} >
                                  <Button variant="contained" onClick={handleAddFilterNew} sx={{ textTransform: "capitalize" }} disabled={["Blank", "Not Blank"].includes(selectedConditionNew) ? false : !filterValueNew || selectedColumnNew.length === 0}>
                                    Add Filter
                                  </Button>
                                </Grid>
                              )}
                              ...
                              <Grid item md={2} sm={12} xs={12}>
                                <Button variant="contained" onClick={() => {
                                  fetchChecklisttypeNew();
                                  setIsSearchActiveNew(true);
                                  setAdvancedFilterNew([
                                    ...additionalFiltersNew,
                                    { column: selectedColumnNew, condition: selectedConditionNew, value: filterValueNew }
                                  ])
                                }} sx={{ textTransform: "capitalize" }} disabled={["Blank", "Not Blank"].includes(selectedConditionNew) ? false : !filterValueNew || selectedColumnNew.length === 0}>
                                  Search
                                </Button>
                              </Grid>
                            </Grid>
      ...
      <ExportData
        ...
        setIsPdfFilterOpen={setIsPdfFilterOpenNew}
        handleClosePdfFilterMod={handleClosePdfFilterModNew}
        filteredDataTwo={(filteredChangesNew !== null ? filteredRowDataNew : rowDataTableNew) ?? []}
        itemsTwo={overallItemsNew ?? []}
        filename={"Notice Period Apply (Deactivated Users)"}
        exportColumnNames={exportColumnNamesNew}
        exportRowValues={exportRowValuesNew}
        componentRef={componentRefNew}
      />
      <ExportData
        ...
        filename={"Notice Period Apply (Liver Users)"}
