...
function AssetSpecificationGrouping() {
        ...
        try {
            ...
            dataasset.forEach((row) => {
                for (const [key, value] of Object.entries(row)) {
                    if (value && !value.includes("Please Select") && !["vendor", "vendorgroup", "estimationtime",
                        "code", "countquantity", "address", "sub", "warranty", "phonenumber", "_id", "vendorid"].includes(key)) {
                        keysToKeep.add(key);
                    }
                }
            });
    ...
    const getCode = async (e, data) => {
        // console.log(data.component, data.subcomponent, data, "checkcode")
        setPageName(!pageName)
        try {
    ...
    const getOverallEditSection = async (e, subcategoryname, data) => {
        ...
        try {
            ...
            let dataasset = res?.data?.assetdetail.flatMap(item => item.subcomponent.filter(t => t.subname == data.subcomponent).map(d => ({
                ...d,
                component: item.component
            })))
            ...
            let datastock = res?.data?.stock.flatMap(item => item.subcomponent.filter(t => t.subname == data.subcomponent).map(d => ({
                ...d,
                component: item.component
            })))
            ...
            let datamanual = res?.data?.manualstockentry.flatMap(item => item.subcomponent.filter(t => t.subname == data.subcomponent).map(d => ({
                ...d,
                component: item.component
            })))
    ...
    console.log(checkassetdetaileditall, checkstockeditall, checkmanualstockeditall, "fund")
    ...
    const editSubmit = (e) => {
        ...
        console.log(
            subcomponentDataEdit)

        // setCheckAssetdetailedit(checkassetdetaileditall.filter(item =>
        //   (subcomponentDataEdit.brand ? !brandValueCateEdit.includes(item.brand) : true) &&
        //   (subcomponentDataEdit.type ? !typeValueCateEdit.includes(item.type) : true) &&
        //   (subcomponentDataEdit.model ? !modelValueCateEdit.includes(item.model) : true) &&
        //   (subcomponentDataEdit.size ? !sizeValueCateEdit.includes(item.size) : true) &&
        //   (subcomponentDataEdit.variant ? !variantValueCateEdit.includes(item.variant) : true) &&
        //   (subcomponentDataEdit.capacity ? !capacityValueCateEdit.includes(item.capacity) : true) &&
        //   (subcomponentDataEdit.paneltypescreen ? !panelTypeValueCateEdit.includes(item.paneltypescreen) : true) &&
        //   (subcomponentDataEdit.resolution ? !screenResolutionValueCateEdit.includes(item.resolution) : true) &&
        //   (subcomponentDataEdit.connectivity ? !connectivityValueCateEdit.includes(item.connectivity) : true) &&
        //   (subcomponentDataEdit.daterate ? !dataRateValueCateEdit.includes(item.daterate) : true) &&
        //   (subcomponentDataEdit.compatibledevice ? !compatibleDevicesValueCateEdit.includes(item.compatibledevice) : true) &&
        //   (subcomponentDataEdit.outputpower ? !outputPowerValueCateEdit.includes(item.outputpower) : true) &&
        //   (subcomponentDataEdit.collingfancount ? !coolingFanCountValueCateEdit.includes(item.collingfancount) : true) &&
        //   (subcomponentDataEdit.clockspeed ? !clockSpeedValueCateEdit.includes(item.clockspeed) : true) &&
        //   (subcomponentDataEdit.core ? !coreValueCateEdit.includes(item.core) : true) &&
        //   (subcomponentDataEdit.speed ? !speedValueCateEdit.includes(item.speed) : true) &&
        //   (subcomponentDataEdit.frequency ? !frequencyValueCateEdit.includes(item.frequency) : true) &&
        //   (subcomponentDataEdit.output ? !outputValueCateEdit.includes(item.output) : true) &&
        //   (subcomponentDataEdit.ethernetports ? !ethernetPortsValueCateEdit.includes(item.ethernetports) : true) &&
        //   (subcomponentDataEdit.distance ? !distanceValueCateEdit.includes(item.distance) : true) &&
        //   (subcomponentDataEdit.lengthname ? !lengthValueCateEdit.includes(item.lengthname) : true) &&
        //   (subcomponentDataEdit.slot ? !slotValueCateEdit.includes(item.slot) : true) &&
        //   (subcomponentDataEdit.noofchannels ? !noOfChannelsValueCateEdit.includes(item.noofchannels) : true) &&
        //   (subcomponentDataEdit.colours ? !coloursValueCateEdit.includes(item.colours) : true)

        // ));
        // setCheckstockedit(checkstockeditall.filter(item =>
        //   (subcomponentDataEdit.brand ? !brandValueCateEdit.includes(item.brand) : true) &&
        //   (subcomponentDataEdit.type ? !typeValueCateEdit.includes(item.type) : true) &&
        //   (subcomponentDataEdit.model ? !modelValueCateEdit.includes(item.model) : true) &&
        //   (subcomponentDataEdit.size ? !sizeValueCateEdit.includes(item.size) : true) &&
        ...
        // ));
        // setCheckmanualstockedit(checkmanualstockeditall.filter(item =>
        //   (subcomponentDataEdit.brand ? !brandValueCateEdit.includes(item.brand) : true) &&
        //   (subcomponentDataEdit.type ? !typeValueCateEdit.includes(item.type) : true) &&
        //   (subcomponentDataEdit.model ? !modelValueCateEdit.includes(item.model) : true) &&
        //   (subcomponentDataEdit.size ? !sizeValueCateEdit.includes(item.size) : true) &&
        ...
        // ) {
        //     return anotherArray.filter((item) => {

        //         const brandMatch = subcomponentDataEdit.brand || brandValueCateEdit.includes(item.brand);
        //         const typeMatch = subcomponentDataEdit.type || typeValueCateEdit.includes(item.type);
        //         const modelMatch = subcomponentDataEdit.model || modelValueCateEdit.includes(item.model);
        //         const sizeMatch = subcomponentDataEdit.size || sizeValueCateEdit.includes(item.size);
        //         const variantMatch = subcomponentDataEdit.variant || variantValueCateEdit.includes(item.variant);
        //         const capacityMatch = subcomponentDataEdit.capacity || capacityValueCateEdit.includes(item.capacity);
        //         const panelTypeMatch = subcomponentDataEdit.paneltypescreen || panelTypeValueCateEdit.includes(item.paneltypescreen);
        //         const resolutionMatch = subcomponentDataEdit.resolution || screenResolutionValueCateEdit.includes(item.resolution);
        //         const connectivityMatch = subcomponentDataEdit.connectivity || connectivityValueCateEdit.includes(item.connectivity);
        //         const dataRateMatch = subcomponentDataEdit.daterate || dataRateValueCateEdit.includes(item.daterate);
        //         const compatibleDeviceMatch = subcomponentDataEdit.compatibledevice || compatibleDevicesValueCateEdit.includes(item.compatibledevice);
        //         const outputPowerMatch = subcomponentDataEdit.outputpower || outputPowerValueCateEdit.includes(item.outputpower);
        //         const coolingFanCountMatch = subcomponentDataEdit.collingfancount || coolingFanCountValueCateEdit.includes(item.collingfancount);
        //         const clockSpeedMatch = subcomponentDataEdit.clockspeed || clockSpeedValueCateEdit.includes(item.clockspeed);
        //         const coreMatch = subcomponentDataEdit.core || coreValueCateEdit.includes(item.core);
        //         const speedMatch = subcomponentDataEdit.speed || speedValueCateEdit.includes(item.speed);
        //         const frequencyMatch = subcomponentDataEdit.frequency || frequencyValueCateEdit.includes(item.frequency);
        //         const outputMatch = subcomponentDataEdit.output || outputValueCateEdit.includes(item.output);
        //         const ethernetPortsMatch = subcomponentDataEdit.ethernetports || ethernetPortsValueCateEdit.includes(item.ethernetports);
        //         const distanceMatch = subcomponentDataEdit.distance || distanceValueCateEdit.includes(item.distance);
        //         const lengthMatch = subcomponentDataEdit.lengthname || lengthValueCateEdit.includes(item.lengthname);
        //         const slotMatch = subcomponentDataEdit.slot || slotValueCateEdit.includes(item.slot);
        //         const noOfChannelsMatch = subcomponentDataEdit.noofchannels || noOfChannelsValueCateEdit.includes(item.noofchannels);
        //         const coloursMatch = subcomponentDataEdit.colours || coloursValueCateEdit.includes(item.colours);
        ...
        //             connectivityMatch &&
        //             dataRateMatch &&
        //             compatibleDeviceMatch &&
        //             outputPowerMatch &&
        //             coolingFanCountMatch &&
        //             clockSpeedMatch &&
        //             coreMatch &&
        //             speedMatch &&
        //             frequencyMatch &&
        ...
        //     });
        // }
        // setCheckAssetdetailedit(
        //     filterArray(
        //         checkassetdetaileditall,
        //         brandValueCateEdit,
        //         typeValueCateEdit,
        //         modelValueCateEdit,
        //         sizeValueCateEdit,
        ...
        if (assetSpecGrpingEdit.assetmaterial === "Please Select Asset Material") {
            ...
            (
                ...
                !checkassetdetailedit.some(item => brandValueCateEdit.includes(item.brand)) ||
                !checkstockedit.some(item => brandValueCateEdit.includes(item.brand)) ||
                !checkmanualstockedit.some(item => brandValueCateEdit.includes(item.brand))
            )
            &&
