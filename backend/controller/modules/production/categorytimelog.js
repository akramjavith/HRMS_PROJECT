...
function addTime(time, count) {
  ...
  const finalMinutes = String(Math.floor((totalSeconds % 3600) / 60)).padStart(2, "0");
  const finalSeconds = String(totalSeconds % 60).padStart(2, "0");

  return `${finalHours}:${finalMinutes}:${finalSeconds}`;
}
// get All categoryTimeLog Name => /api/allcategorytimelog
exports.getAllcategoryTimeLog = catchAsyncErrors(async (req, res, next) => {
  let categorytimelog;
  try {
...
exports.CategoryTimelogCalculationold = catchAsyncErrors(async (req, res, next) => {
  let categorytimelog;
  try {
    ...
    let userQuery = {
      ...
      $or: [{ reasondate: { $exists: false } }, { reasondate: { $eq: "" } }, { reasondate: { $lte: fromdate } }],
    };

    let query = { fromdate: fromdate };

    const [loginids, ProductionUploads, users, timepoints] = await Promise.all([
    ...
    let mergedDataallfirst = ProductionUploads.map((upload) => {
      ...
      let logininfoname =
        loginallot.length > 0 && filteredDataDateTime && filteredDataDateTime.empname
...
exports.addcategoryTimeLog = catchAsyncErrors(async (req, res, next) => {
  let acategorytimelog = await categoryTimeLog.create(req.body);
...
exports.getSinglecategoryTimeLog = catchAsyncErrors(async (req, res, next) => {
...
exports.updatecategoryTimeLog = catchAsyncErrors(async (req, res, next) => {
  const id = req.params.id;
  if (!ucategorytimelog) {
    return next(new ErrorHandler("Data not found!", 404));
  }
...
exports.deletecategoryTimeLog = catchAsyncErrors(async (req, res, next) => {
  const id = req.params.id;

  let dcategorytimelog = await categoryTimeLog.findByIdAndRemove(id);
