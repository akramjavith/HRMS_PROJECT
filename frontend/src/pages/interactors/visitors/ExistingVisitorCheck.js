...
const ExistingVisitor = ({ ExistingVisitors, handleCloseModEdit }) => {
    ...
    const fetchTaskcategory = async () => {
        ...
        try {
            ...
            setTaskcategorys(ExistingVisitors);
    ...
    const columnDataTable = [
        ...
        { field: "intime", headerName: "Time", flex: 0, width: 120, hide: !columnVisibility.intime, headerClassName: "bold-header" },
        { field: "visitorpurpose", headerName: "Purpose", flex: 0, width: 120, hide: !columnVisibility.visitorpurpose, headerClassName: "bold-header" },
        { field: "status", headerName: "Status", flex: 0, width: 150, hide: !columnVisibility.status, headerClassName: "bold-header" },
        {
            field: "round", headerName: "Interview Round", flex: 0, width: 150, hide: !columnVisibility.round,
            headerClassName: "bold-header",
            renderCell: (params) => (
                <Grid sx={{ display: 'flex', alignItems: "center" }}>
                    {params?.row?.modelName === "Candidate" && params?.row?.lastInterviewRound?.roundname ?
                        <Link
                            to={`/interviewrounds/${params.row.jobopeningsid}`}
                            target="_blank"
                        >
                            <Button>

                                {params?.row?.lastInterviewRound?.roundname || "No Round Name"} <OpenInNewIcon />
                            </Button>

                        </Link>
                        : params?.row?.status === "Employee" || params?.row?.status === "Visitor" ?
                            <Button
                                sx={{ ...userStyle.buttonedit, opacity: 0.5, pointerEvents: "none" }}
                                disabled
                            >
        ...
        },
        {
            ...
            renderCell: (params) => (
                <Grid sx={{ display: 'flex' }}>
                    ...
                    {params?.row?.modelName === "Candidate" ?
    ...
    const rowDataTable = filteredData.map((item, index) => {
        function convertTo12HourFormat(time24) {
            if (!time24 || typeof time24 !== "string") return ""; // Handle undefined/null cases

            const timeParts = time24.split(":").map(Number);
            if (timeParts.length < 2 || isNaN(timeParts[0]) || isNaN(timeParts[1])) return "Invalid Time"; // Ensure valid split
        ...
        return {
            ...
            visitorname: item?.visitorname || item?.companyname || item?.fullname || "",
            company: item?.company || "",
            branch: item?.branch || "",
            unit: item?.unit || "",
            date: item?.date ? moment(item.date).format("DD-MM-YYYY") : "",
            intime: convertTo12HourFormat(item?.intime),
            visitorpurpose: item?.visitorpurpose || "",
            alldata: item || {},
            status: item.modelName === "Candidate" && item?.lastInterviewRound?.roundstatus === "Interview Scheduled" || item?.lastInterviewRound?.roundstatus === "Completed" ? `${item?.modelName} - ${item?.overallstatus}` : item?.modelName,
            jobopeningsid: item.jobopeningsid,
            modelName: item.modelName,
            overallstatus: item.overallstatus,
            lastInterviewRound: item.lastInterviewRound,
        };
    });
