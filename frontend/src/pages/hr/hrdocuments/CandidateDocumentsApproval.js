...
function CandidateDocumentsApproval() {
    ...
    const handleBranchChange = (options) => {
        ...
        setValueTemplate([]);
        setSelectedOptionsTemplate([]);
        TemplateDropdowns(valueCompanyCat, values)
    };
    ...
    const handleTemplateChange = (options) => {
        ...
        setValueTemplate(values);
        setSelectedOptionsTemplate(options);

    };
    const customValueRendererTemplate = (valueTemplate, _categoryname) => {
        return valueTemplate?.length
            ? valueTemplate?.map(({ label }) => label)?.join(", ")
    ...
    const handleAutoSelect = async () => {
        ...
        try {
            ...
            TemplateDropdowns(selectedCompany, selectedBranch);
    ...
    const TemplateDropdowns = async (company, branch) => {
        setPageName(!pageName);
        try {
            let res = await axios.post(SERVICE.APPROVAL_CANDIDATE_TEMPLATECREATION, {
                headers: {
            ...
            const templateValues = res?.data?.templates?.length > 0 ? res?.data?.templates : [];
            setTemplateValues(
                templateValues?.map((data) => ({
                    label: `${data.documentname}--(${data.company}--${data.branch})`,
                    value: `${data.documentname}--(${data.company}--${data.branch})`,
                    tempname: `${data.name}--(${data.company}--${data.branch})`,
                }))
            );
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Human Resource/HR Documents/Approval User Documents"),
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "Approval Documents For Candidates.png");
                })
                .catch((error) => {
    ...
    const handleSubmit = (e) => {
        ...
        else if (selectedOptionsTemplate?.length < 1) {
            setPopupContentMalert("Please Select Document");
            setPopupSeverityMalert("info");
            handleClickOpenPopupMalert();
        }
        else if (selectedOptionsApproveStatus?.length < 1) {
    ...
    const handleSubmitDates = (e) => {
        e.preventDefault();
        getApprovalDocument(rowIdPassing, CandiddateIdPassing);
    }
    ...
    const fetchAppprovalTemplateDocuments = async () => {
        setPageName(!pageName);
        try {
            let res_vendor = await axios.post(SERVICE.APPROVAL_CANDIDATE_DOCUMENTS, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            ...
            setLoader(true);
            const answer = res_vendor?.data?.documentPreparation?.length > 0 ? res_vendor?.data?.documentPreparation?.map((item, index) => ({
                ...item,
                ...
                approveddate: item?.approval === "approved" ? moment(item.approveddate).format("DD-MM-YYYY hh:mm:ss a") : "-",
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: 'Approval Documents For Candidates',
        pageStyle: 'print'
    });
    ...
    const getApprovalDocument = async (selectedIds, candidateid) => {
        ...
        try {
            if (selectedIds?.length > 0) {
                // Create an array of axios PUT requests
                const updateRequests = selectedIds.map((data, index) => {
                    const headers = {
                        Authorization: `Bearer ${auth.APIToken}`,
                    };

                    return Promise.all([
                        axios.put(`${SERVICE.SINGLE_CANDIDATE_DOCUMENTPREPARATION}/${data}`, {
                            approval: "sentforapproval",
                            approvalsentdate: new Date(),
                        }, { headers }),

                        axios.put(`${SERVICE.CANDIDATES_SINGLE}/${candidateid[index]}`, {
                            approval: "sentforapproval",
                            approvalsentdate: new Date(),
                            approvalid: data
                        }, { headers })
                ...
                await fetchAppprovalTemplateDocuments();
                setLoader(false)
    ...
    const getViewFile = async (id) => {
        ...
        let response = await axios.get(`${SERVICE.SINGLE_CANDIDATE_DOCUMENTPREPARATION}/${id}`, {
            headers: {
                Authorization: `Bearer ${auth.APIToken}`,
            },
        });
    ...
    const columnDataTable = [
        ...
        },
        {
            ...
            cellRenderer: (params) => (
                <Grid>
                    {
                        params?.data?.approval === "Sent to Approval" ?
                            <Typography color="#009688" marginTop={1.5}>Sent to approval</Typography>
                            :
                            params?.data?.approval === "Approved" ?
                                <Typography color="#4caf50" marginTop={1.5}>Approved</Typography> :
                                <Button
                                    variant="contained"
                                    color={"warning"}
                                    onClick={() => {
        ...
        },
        {
            ...
            flex: 0,
            width: 100,
            hide: !columnVisibility.approvedby,
            headerClassName: "bold-header",
            pinned: "left",
        },
        {
            ...
            width: 220,
            minHeight: "40px",
            hide: !columnVisibility.viewdocument,
            pinned: "left",
            cellRenderer: (params) => (
                <Grid>
                    {
                        ...
                        <Button
                            ...
                            color={"warning"}
                            onClick={() => {
                                // checkingData([params.data.id])
                                getViewFile(params?.data?.id)
                            }}
                            sx={userStyle.buttonview}
                        >
                            View
    ...
    return (
        <Box>
            <Headtitle title={'APPROVAL CANDIDATE DOCUMENTS'} />
            <PageHeading
                title="Approval Candidate Documents"
                modulename="Human Resources"
                submodulename="HR Documents"
                mainpagename="Approval Candidate Documents"
                subpagename=""
                subsubpagename=""
            />
            {isUserRoleCompare?.includes("lapprovalcandidatedocuments")
                && (
                    <>
                        <Box sx={userStyle.dialogbox}>
                            <>
                                <Grid container spacing={2}>
                                    <Grid item xs={8}>
                                        <Typography sx={userStyle.importheadtext}>Approval Candidate Documents List</Typography>
                                    </Grid>
                                </Grid><br />
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item md={3} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            ...
                                            <MultiSelect
                                                ...
                                                labelledBy="Please Select Document"
                                            />
                                        </FormControl>
                                    </Grid>
                                    <Grid item md={3} xs={12} sm={12}>
                                    ...
                                    <Grid item md={4} xs={12} sm={12} sx={{ marginTop: "25px" }}>
                                        <Grid container spacing={3}>
                                            <Grid item md={4} xs={12} sm={6}>
                                                <LoadingButton loading={isBtn} onClick={(e) => handleSubmit(e)} variant='contained' sx={buttonStyles.buttonsubmit} >Filter</LoadingButton>
                                            </Grid>
                                            <Grid item md={4} xs={12} sm={6}>
            ...
            {isUserRoleCompare?.includes("lapprovalcandidatedocuments") && (
                <>
                    <Box sx={userStyle.container}>
                        {/* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Approval User Documents List</Typography>
                        </Grid>
                        <Grid container spacing={2} style={userStyle.dataTablestyle}>
                            ...
                            >
                                <Box>
                                    {isUserRoleCompare?.includes("excelapprovalcandidatedocuments") && (
                                    ...
                                    {isUserRoleCompare?.includes("csvapprovalcandidatedocuments") && (
                                    ...
                                    {isUserRoleCompare?.includes("printapprovalcandidatedocuments") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("pdfapprovalcandidatedocuments") && (
                                    ...
                                    {isUserRoleCompare?.includes("imageapprovalcandidatedocuments") && (
                        ...
                        &ensp;
                        {isUserRoleCompare?.includes("beapprovalcandidatedocuments") && (
                ...
                <Dialog open={isDeleteOpencheckbox} onClose={handleCloseModcheckbox} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                    <DialogContent sx={{ width: "450px", textAlign: "center", alignItems: "center" }}>
                        ...
                        <Typography variant="h5" sx={{ color: "red", textAlign: "center" }}>
                            The documents have been sent for candidate approval. Are you sure you want to proceed?
                        </Typography>
                        <br />
                    </DialogContent>
                    <DialogActions>
            ...
            <ExportData
                ...
                itemsTwo={approvalTemplates ?? []}
                filename={"Approval Documents For Candidates"}
...
export default CandidateDocumentsApproval;