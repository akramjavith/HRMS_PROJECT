...
exports.getAllDayPointsUploadTemp = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload;
  try {
    daypointsupload = await DayPointsUploadTemp.find();
  } catch (err) {
...
// get All ClientUserID Name => /api/clientuserids
exports.getAllDayPointsUploadLimitedTemp = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload;
  try {
    daypointsupload = await DayPointsUploadTemp.find({}, { filename: 1, date: 1, type: 1 });
  } catch (err) {
    return next(new ErrorHandler("Records not found!", 404));
  }
  if (!daypointsupload) {
...
exports.updateDayPointsSingleUploadTemp = catchAsyncErrors(
  async (req, res, next) => {
    const subid = req.params.id;
    req.body.id = subid;
    try {
      const uploaddata = await DayPointsUploadTemp.findOneAndUpdate(
...
exports.dayPointsfilterTemp = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    let users = await User.find({}, { department: 1, company: 1, branch: 1, unit: 1, team: 1, empcode: 1, companyname: 1, assignExpLog: 1, processlog: 1, doj: 1 });
...
// get All ClientUserID Name => /api/clientuserids
exports.dayPointsDatasFetchTemp = catchAsyncErrors(async (req, res, next) => {
...
exports.dayPointsMonthYearFilterTemp = catchAsyncErrors(async (req, res, next) => {
...
exports.dayPointsMonthYearFilterNxtMonthTemp = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload, answer;
  try {
    const { ismonth, isyear, } = req.body;
    ...
    //   answer = daypointsupload.flatMap((data) =>
    //   data.uploaddata
    // );
    if (fromdate && todate) {
      conditions.push({
...
// Create new DayPointsUploadTemp=> /api/clientuserid/new
exports.addDayPointsUploadTemp = catchAsyncErrors(async (req, res, next) => {
  let adaypointsupload = await DayPointsUploadTemp.create(req.body);
...
// get Signle DayPointsUploadTemp => /api/clientuserid/:id
exports.getSingleDayPointsUploadTemp = catchAsyncErrors(async (req, res, next) => {
  const id = req.params.id;

  let sdaypointsupload = await DayPointsUploadTemp.findById(id);
...
// update DayPointsUploadTemp by id => /api/clientuserid/:id
exports.updateDayPointsUploadTemp = catchAsyncErrors(async (req, res, next) => {
  const id = req.params.id;
  let udaypointsupload = await DayPointsUploadTemp.findByIdAndUpdate(id, req.body);
  if (!udaypointsupload) {
    return next(new ErrorHandler("Day Points Upload not found!", 404));
  }
  return res.status(200).json({ message: "Updated successfully" });
...
// delete DayPointsUploadTemp by id => /api/clientuserid/:id
exports.deleteDayPointsUploadTemp = catchAsyncErrors(async (req, res, next) => {
  const id = req.params.id;

  let ddaypointsupload = await DayPointsUploadTemp.findByIdAndRemove(id);
...
// get All ClientUserID Name => /api/clientuserids
exports.checkDayPointdateTemp = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload;
  try {
    daypointsupload = await DayPointsUploadTemp.find({ date: req.body.date }, { _id: 1 });
  } catch (err) {
...
exports.getEmployeeProductionLastThreeMonthsTemp = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload = [];
  try {
    ...
    daypointsupload = await DayPointsUploadTemp.find({ date: { $gte: result.fromdate, $lte: result.todate } }, { uploaddata: 1, date: 1 });

    return res.status(200).json({
      // count: products.length,
      daypointsupload, finalFromToDate,
      query, result
    });
  } catch (err) {
    console.log(err.message);
...
exports.getAllDayPointsUploadTempDateOnly = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload;
  try {
    daypointsupload = await DayPointsUploadTemp.find({}, { date: 1 });
  } catch (err) {
    return next(new ErrorHandler("Records not found!", 404));
  }
  if (!daypointsupload) {
...
exports.getDayPointIdByDateTemp = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload;
  try {

    daypointsupload = await DayPointsUploadTemp.findOne({ date: req.body.date }, { _id: 1 });
...
exports.getAllDayPointTempByDate = catchAsyncErrors(async (req, res, next) => {
  let daypointsupload, productionconsolidated;
  try {


    let daypointsuploadall = await DayPointsUploadTemp.find({ date: { $gte: req.body.fromdate, $lte: req.body.todate } }, { uploaddata: 1 });

    console.log(req.body, daypointsuploadall.length, "DAYTEMP")
