...
import {
    ...
    Grid, IconButton,
    InputAdornment,
    List, ListItem, ListItemText, MenuItem, OutlinedInput, Popover,
    Radio,
    RadioGroup,
  ...
  `;
    ...
    const sendRequest = async () => {
        ...
        try {
            ...
            let grpcreate = await axios.post(SERVICE.LOAN_CREATE, formData, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const fetchAllGroup = async () => {
        ...
        try {
            ...
            setGroups(res_grp?.data?.loan.map((item, index) => ({
                ...item,
    ...
    const fetchAllLoanArray = async () => {
        ...
        try {
            ...
            setLoanArray(res_grp?.data?.loan)
    ...
    const getDownloadFile = async (id) => {
        try {
            let response = await axios.get(`${SERVICE.LOAN_SINGLE}/${id}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            if (response?.data?.sloan?.document?.length !== 0) {

                response?.data?.sloan?.document?.map((item) => {
                    if (!item) return false;
                    const imageUrl = `${SERVICE.VEIW_LOAN_FILES}/${item.filename}`;
    ...
    const getDownloadFileView = async (id) => {
        try {
            const imageUrl = `${SERVICE.VEIW_LOAN_FILES}/${id}`;
            window.open(imageUrl, '_blank'); // Open image in a new tab
        } catch (err) { handleApiError(err, setShowAlert, handleClickOpenerr); }
    };
