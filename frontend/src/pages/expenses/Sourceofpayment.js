...
function Sourceofpayments() {
  ...
  const checkUsedByOtherBulk = async (data) => {
    let collectedDatas = selectedRows.map((item) => {
      ...
      let categoryName = item?.sourcename;
      let subCategory = item?.paymentmodereal;

      let combinedData = subCategory.map(sub => ({
        source: categoryName,
        paymentmode: sub,
        id: item.id
      }));
    ...
    try {
      let res = await axios.post(SERVICE.SOURCE_OF_PAYMENT_OVERALL_DELETE, {
  ...
  const checkUsedByOther = async (data) => {
    let categoryName = data?.sourcename;
    let subCategory = data?.paymentmodereal;
  ...
  const getCode = async (e, name) => {
    ...
    try {
      ...
      setOldCategory(res?.data?.SourceofPy?.sourcename);
  ...
  const sendEditRequest = async () => {
    ...
    try {
      ...
      let categoryUpdate = await axios.post(SERVICE.SOURCE_OF_PAYMENT_OVERALL_UPDATE, {
        oldsource: oldCategory,
        source: String(sourceofpaymentEdit.sourcename)
      })
      handleCloseerrNew()
      await fetchAssignedBy();
      handleCloseModEdit();
  ...
  const editSubmit = async (e) => {
    ...
    if (interactorPurposeOptValueReq.length === 0) {
      ...
      if (sourceofpaymentEdit.sourcename !== oldCategory) {
        const [resNew] = await Promise.all([

          axios.post(SERVICE.SOURCE_OF_PAYMENT_OVERALL_EDIT, {
            source: oldCategory
          })
        ]);
  ...
  const addSerialNumber = () => {
    const itemsWithSerialNumber = sources?.map((item, index) => ({
      ...
      paymentmodereal: item.paymentmode,
        ...
        <Grid sx={{ display: "flex" }}>
          ...
          {isUserRoleCompare?.includes("dsourceofpayments") && (
            <Button
              ...
              onClick={(e) => {
                rowData(params.row.id); checkUsedByOther(params.row)
              }}
            >
  ...
  const rowDataTable = filteredData.map((item, index) => {
    return {
      ...
      paymentmodereal: item.paymentmodereal,
      paymentmode: item.paymentmode,
    };
  });
