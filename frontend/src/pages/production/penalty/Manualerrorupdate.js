...
function ManulErrorUpdate() {
  ...
  const exportColumnNames = ['Employee Name', 'Branch', 'Unit', 'Project Vendor', 'Date', 'Queue Name', 'Login ID', 'Accuracy', 'Total Fields', 'Error Count', 'Auto Count', 'Upload Count'];
  const exportRowValues = ['name', 'branch', 'unit', 'projectvendor', 'date', 'queuename', 'loginid', 'accuracy', 'totalfields', 'errorcount', 'autocount', 'uploadcount'];
  ...
  const sendEditRequest = async (id, manualvalue, data) => {
    ...
    try {
      if (manualvalue === '' || manualvalue == undefined) {
        ...
        setUpdatedids((prev) => [...prev, editid]);
  ...
  return (
    <Box>
      ...
      <PageHeading title=" Manual Error Update" modulename="Quality" submodulename="Penalty" mainpagename="Penalty Setup" subpagename="Penalty Calculation" subsubpagename="Manual Error Update" />

      <Box sx={userStyle.container}>
        <Box>
          {/* ****** Header Content ****** */}
          <Typography sx={userStyle.HeaderText}>Manual Error Update List</Typography>
            ...
            <>
              <Box>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item lg={1} md={1} sm={1} xs={6}>
                    <Box sx={(theme) => ({ mt: { lg: 3, md: 2, sm: 1, xs: 0 } })}>
                      <Button onClick={handleFilterSubmitOne} sx={buttonStyles.buttonsubmit} variant="contained">
                        Filter
                      </Button>
                    </Box>
                  </Grid>
                ...
                <Grid container sx={{ justifyContent: 'center' }}>
                  <Grid>
                    {isUserRoleCompare?.includes('csvmanualerrorupdate') && (
                      <>
                        <Button
                          onClick={(e) => {
                    ...
                    {isUserRoleCompare?.includes('excelmanualerrorupdate') && (
                      <>
                        <Button
                          onClick={(e) => {
                            setIsFilterOpen(true);
                    ...
                    {isUserRoleCompare?.includes('printmanualerrorupdate') && (
                      <>
                    ...
                    {isUserRoleCompare?.includes('pdfmanualerrorupdate') && (
                      <>
                        <Button
                          sx={userStyle.buttongrp}
                          onClick={() => {
                    ...
                    {isUserRoleCompare?.includes('imagemanualerrorupdate') && (
