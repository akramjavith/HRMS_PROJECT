...
function ListRejected({
    listrejected, taskcategoryCheckrejected, totalProjectsRejectedval,
    totalPagesRejectedval, base, category, subcategory, fromdate, todate, taskcategorysallExports
}) {
    ...
    const handleFilterClickForRejected = async () => {
        ...
        try {
            let res_vendor = await axios.post(SERVICE.NONPRODUCTIONFILTERLIST_REJECTED_PAGINATION, queryParams, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                }
            });
            setTaskcategorysall(res_vendor?.data?.lists?.rejectlist?.map((item, index) => ({
                ...item,
