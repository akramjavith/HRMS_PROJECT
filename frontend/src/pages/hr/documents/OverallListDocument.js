...
function OverallDocumentsList() {
    ...
    if (isfilter === "filtered") {
      ...
      exportToCSV(
        documentsList.map((t, index) => ({
          ...
          Categoryname: t.categoryname.join(","),
          Subcategoryname: t.subcategoryname.join(","),
          Type: t.type === "Please  Select Type" ? "" : t.type,
          Module: t.module.join(","),
          Customer: t.customer.join(","),
          Queue: t.queue.join(","),
          Process: t.process.join(","),
  ...
  const downloadPdf = (isfilter) => {
    ...
    doc.save("OverallDocumentsList.pdf");
  };
  ...
  const getDownloadFile = async (id) => {
    try {
      ...
      if (response?.data?.sdocument?.document?.length === 0) {
        ...
        //   // Handle PDF file
        //   // const newTab = window.open();
        //   // newTab.document.write('<iframe width="100%" height="100%" src="' + pdfContentArray.preview + '"></iframe>');
        ...
        //         </head>
        //         <body>
        //           <iframe width="100%" height="100%" src="${pdfContentArray.preview}" sandbox="allow-same-origin allow-scripts"></iframe>
        //         </body>
        //       </html>
        //     `);
        //   };
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "OverallDocumentsList.png");
        });
      });
    }
  };
      ...
      renderCell: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("eoveralldocumentlist") && (
            <Link
              to={`/documentlist/edit/${params.row.id}/overalldocumentlist`}
          ...
          {isUserRoleCompare?.includes("voveralldocumentlist") && (
            <Link
              to={`/documentlist/view/${params.row.id}/overalldocumentlist`}
