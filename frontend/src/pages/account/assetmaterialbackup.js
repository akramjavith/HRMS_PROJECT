...
function Assetmaterial() {
        ...
        try {
            ...
            if (selectedRows.length === 0) {
                ...
                const [resspecification, resspecificationgrp, resassetdetail, resmatip, resmaindetmaster, resworkgrp,
                    resprob, resmainmaster, resempdis, resstockmanage, resstock, resmanual] = await Promise.all([

                        axios.post(SERVICE.OVERALL_DELETE_ASSET_MATERIAL_LINKED_DATA, {
                            headers: {
                                Authorization: `Bearer ${auth.APIToken}`,
                            },
                            matassetspecification: value,
                        }),
                        axios.post(SERVICE.OVERALL_DELETE_ASSET_MATERIAL_LINKED_DATA, {
                            headers: {
                                Authorization: `Bearer ${auth.APIToken}`,
                ...
                setCheckmaindetails(resmaindetmaster?.data?.maintenancedetailsmaster);
                setCheckassetwrkgrp(resworkgrp?.data?.assetworkstationgrouping);
                setCheckproblem(resprob?.data?.assetproblem);
                setCheckmaintmaster(resmainmaster?.data?.maintenancemaster);
                setCheckAssetempdis(resempdis?.data?.assetempdistribution);
                ...
                let assetspecificationgrp = resspecificationgrp?.data?.assetspecificationgrp.map(t => t.assetmaterial).flat();
                let assetdetail = resassetdetail?.data?.assetdetail.map(t => t.material).flat();

                let assetmaterialip = resmatip?.data?.assetmaterialip.map(t => t.assetmaterial).flat();
                let maintenancedetailsmaster = resmaindetmaster?.data?.maintenancedetailsmaster.map(t => t.assetmaterial).flat();
                let assetworkstationgrouping = resworkgrp?.data?.assetworkstationgrouping.map(t => t.assetmaterial).flat();

                let assetproblem = resprob?.data?.assetproblem.map(t => t.material).flat();
                let maintenancemaster = resmainmaster?.data?.maintenancemaster.map(t => t.assetmaterial.split("-")[0]).flat();
                let assetempdistribution = resempdis?.data?.assetempdistribution.map(t => t.assetmaterial).flat();


                let stockmanage = resstockmanage?.data?.stockmanage.map(t => t.material).flat();
                let stock = resstock?.data?.stock.map(t => t.productname).flat();
                let manualstockentry = resmanual?.data?.manualstockentry.map(t => t.productname).flat();
                ...
                if (
                    ...
                    (resmaindetmaster?.data?.maintenancedetailsmaster).length > 0 ||
                    (resworkgrp?.data?.assetworkstationgrouping).length > 0 ||
                    (resprob?.data?.assetproblem).length > 0 ||
                    (resmainmaster?.data?.maintenancemaster).length > 0 ||
                    (resempdis?.data?.assetempdistribution).length > 0 ||
  ...
  ${res?.data?.assetproblem?.length > 0 ? "Asset Problem Master ," : ""}
    ...
    const sendEditRequestOverall = async (
        ...
        try {
            ...
            if (maintenancedetailsmaster.length > 0) {
                let answ = maintenancedetailsmaster.map((d, i) => {
    ...
    const editSubmit = (e) => {
        ...
        else if (assetedit.name != ovProj && ovProjCount > 0) {
            setShowAlertpop(
                <>
