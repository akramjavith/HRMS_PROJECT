...
function DesignationLogGrouping() {
    ...
    const addSerialNumber = () => {
        const itemsWithSerialNumber = DesignationLog?.map((item, index) => ({
    ...
    const fetchAllDesignationLog = async () => {
        ...
        try {
            ...
            setDesignationLog(res_area?.data?.combinedResults);
            setIsAttandance(false)
        } catch (err) { setIsAttandance(false); handleApiError(err, setShowAlert, handleClickOpenerr); }
    };
