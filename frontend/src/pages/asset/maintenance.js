...
function Maintentance() {
  ...
  const fetchMaintentanceOverall = async () => {
    try {
      let res = await axios.get(SERVICE.MAINTENANCE_EXCEL_LIMITED, {
        headers: {
      ...
      setMaintentanceOverall(
        res.data.maintenances?.map((item, index) => ({
          ...item,
          id: item._id,
  ...
  // console.log(overallFilterdataAll.length, overallFilterdata.length, "data")
  ...
  const addTodo = () => {
    ...
    if (maintentancemaster?.hour === "" || maintentancemaster?.min === "" || maintentancemaster?.timetype === "") {
      setPopupContentMalert("Please Select Hour, Minutes and Type!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (addReqTodo?.some((data) => data?.hour === maintentancemaster?.hour && data?.min === maintentancemaster?.min && data?.timetype === maintentancemaster?.timetype)) {
  ...
  const addTodoEdit = () => {
    ...
    if (maintentancemasteredit?.hour === "" || maintentancemasteredit?.hour === undefined || maintentancemasteredit?.min === "" || maintentancemasteredit?.min === undefined || maintentancemasteredit?.timetype === "" || maintentancemasteredit?.timetype === undefined) {
      setPopupContentMalert("Please Select Hour, Minutes and Type!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (addReqTodoEdit?.some((data) => data?.hour === maintentancemasteredit?.hour && data?.min === maintentancemasteredit?.min && data?.timetype === maintentancemasteredit?.timetype)) {
      setPopupContentMalert("Already Added!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else {
  ...
  const { isUserRoleCompare, pageName, setPageName, isUserRoleAccess, isAssignBranch, buttonStyles, allfloor, allareagrouping, alllocationgrouping, allUsersData, allUnit, allTeam, allCompany, allBranch } = useContext(UserRoleAccessContext);
  ...
  const vendorid = async (value) => {
    try {
      ...
      let getdetails = res?.data?.vendordetails.find((item) => item.vendorname == value);
  ...
  const delProject = async () => {
    ...
    try {
      ...
      await handleFileDeleteOld(deleteproject.documentfiles.map((d) => `${deleteproject.uniqueId}$maintenance$${d}`));
  ...
  const sendRequest = async (data) => {
    ...
    try {
      let maintenancecreate = await axios.post(SERVICE.MAINTENTANCE_CREATE, {
        ...
        monthdate: maintentancemaster.frequency === "Monthly" || maintentancemaster.frequency === "Date Wise" ? maintentancemaster.monthdate : "",
        weekdays: maintentancemaster.frequency === "Weekly" || maintentancemaster.frequency === "Day Wise" ? valueWeekly : [],
        annumonth: maintentancemaster.frequency === "Annually" ? maintentancemaster.annumonth : "",
        annuday: maintentancemaster.frequency === "Annually" ? maintentancemaster.annuday : "",
        ...
        description: agenda,
        // documentfiles: documentFiles.map((item) => item.name),
        documentfiles: documentFiles.map((item) => item.name),
        uniqueId: uniqueId,
        maintenancelog: [
          {
      ...
      await handleFileUpload(documentFiles, "maintenance", uniqueId);
      await fetchMaintentance("filtered");
      setBtnLoad(false);
          ...
          const uploadNextChunk = async () => {
            try {
              if (end < selectedFile.size) {
                ...
                try {
                  const response = await axios.post(SERVICE.UPLOAD_CHUNK_MAINTANANCE, formData, {
                    headers: {
  ...
  const handleSubmit = (e) => {
    ...
    if (maintentancemaster.company === "Please Select Company") {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemaster.assetmaterial === "Please Select AssetMaterial") {
      setPopupContentMalert("Please Select AssetMaterial!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (valueAssetCodeCat?.length === 0) {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemaster.schedule === "Time Based" && addReqTodo?.length == 0) {
      setPopupContentMalert("Atleast Add One Data in Time todo!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemaster.schedule === "Time Based" && todoSubmit === true) {
      setPopupContentMalert("Please Update the todo and Submit!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if ((maintentancemaster.frequency === "Monthly" || maintentancemaster.frequency === "Date Wise") && maintentancemaster.monthdate === "") {
      setPopupContentMalert("Please Select Monthly Day!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if ((maintentancemaster.frequency === "Weekly" || maintentancemaster.frequency === "Day Wise") && selectedWeeklyOptions?.length === 0) {
      setPopupContentMalert("Please Select Days!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemaster.frequency === "Annually" && maintentancemaster.annumonth === "") {
      setPopupContentMalert("Please Select Month!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemaster.frequency === "Annually" && maintentancemaster.annuday === "") {
      setPopupContentMalert("Please Select Day!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemaster?.duration === undefined || maintentancemaster?.duration === "00:00" || maintentancemaster?.duration?.includes("Mins")) {
  ...
  const getMultipleFilesAsObjects = async (filenames, type, uniqueId) => {
    ...
    for (const name of filenames) {
      const res = await axios.post(
        SERVICE.MAINTENANCE_TODO_EDIT_FETCH,
  ...
  const handleFetchBill = (data, remarks) => {
    ...
    Promise.all(fileReaders).then((originalFiles) => {
      setdocumentFilesEdit(newSelectedFiles);
  ...
  const getCode = async (e) => {
    ...
    try {
      ...
      let assetCode =
        res?.data?.smaintenance?.assetmaterialcode?.length > 0
          ? res?.data?.smaintenance?.assetmaterialcode?.map((data) => ({
              label: data,
              value: data,
            }))
          : [];

      setValueAssetCodeEditCat(res?.data?.smaintenance?.assetmaterialcode || []);
      ...
      setMaterialDetailsOptEdit(
        findMatchingDetails?.length > 0
          ? findMatchingDetails?.map((item) => ({
              label: item?.maintenancedescriptionmaster,
              value: item?.maintenancedescriptionmaster,
            }))
          : []
      ...
      const [hourscal, minutescal] = res?.data?.smaintenance.duration.split(":");
      ...
      fetchAreaEdit(res?.data?.smaintenance?.branch, res?.data?.smaintenance?.floor);
      fetchAllLocationEdit(res?.data?.smaintenance?.branch, res?.data?.smaintenance?.floor, res?.data?.smaintenance?.area);
      setAddReqTodoEdit(res?.data?.smaintenance?.timetodo);
      // setdocumentFilesEdit(res?.data?.smaintenance?.documentfiles);

      const filesbill = await getMultipleFilesAsObjects(res?.data?.smaintenance?.documentfiles, "maintenance", res?.data?.smaintenance?.uniqueId);
      setoldfileNamesBill(res?.data?.smaintenance?.documentfiles.map((d) => `${res?.data?.smaintenance?.uniqueId}$maintenance$${d}`));

      handleFetchBill(filesbill, res?.data?.smaintenance?.documentfiles);
      ...
      let getdetails = resv?.data?.vendordetails.find((item) => item.vendorname == res?.data?.smaintenance.vendor);
      let getvendorinfo = getdetails ? getdetails : {};
  ...
  const sendEditRequest = async () => {
    ...
    try {
      let res = await axios.put(`${SERVICE.MAINTENTANCE_SINGLE}/${maintenanceid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        ...
        timetodo: maintentancemasteredit.schedule === "Time Based" ? addReqTodoEdit : [],
        monthdate: maintentancemasteredit.frequency === "Monthly" || maintentancemasteredit.frequency === "Date Wise" ? maintentancemasteredit.monthdate : "",
        weekdays: maintentancemasteredit.frequency === "Weekly" || maintentancemasteredit.frequency === "Day Wise" ? valueWeeklyEdit : [],
        annumonth: maintentancemasteredit.frequency === "Annually" ? maintentancemasteredit.annumonth : "",
        annuday: maintentancemasteredit.frequency === "Annually" ? maintentancemasteredit.annuday : "",
        ...
        uniqueId: maintentancemasteredit.uniqueId,
        documentfiles: documentFilesEdit.map((item) => item.name),
        maintenancelog: [
          ...maintenancelog,
          {
            company: String(maintentancemasteredit.company),
      ...
      await handleFileUpload(documentFilesEdit, "maintenance", maintentancemasteredit.uniqueId);
  ...
  const handleFileDeleteOld = async (filenames) => {
    try {
      let res_project = await axios.post(SERVICE.EDIT_OLDDATA_DELETE_MAINTENANCE, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        filenames: filenames,
  ...
  const editSubmit = (e) => {
    ...
    if (maintentancemasteredit.company === "Please Select Company") {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemasteredit.assetmaterial === "Please Select AssetMaterial") {
      setPopupContentMalert("Please Select AssetMaterial!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (valueAssetCodeEditCat?.length === 0) {
      setPopupContentMalert("Please Select Asset Material Code!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemasteredit.schedule === "Time Based" && addReqTodoEdit?.length == 0) {
      setPopupContentMalert("Atleast Add One Data in Time todo!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemasteredit.schedule === "Time Based" && todoSubmitEdit === true) {
      setPopupContentMalert("Please Update the todo and Submit!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if ((maintentancemasteredit.frequency === "Monthly" || maintentancemasteredit.frequency === "Date Wise") && maintentancemasteredit.monthdate === "") {
      setPopupContentMalert("Please Select Monthly Day!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if ((maintentancemasteredit.frequency === "Weekly" || maintentancemasteredit.frequency === "Day Wise") && selectedWeeklyOptionsEdit?.length === 0) {
      setPopupContentMalert("Please Select Days!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemasteredit.frequency === "Annually" && maintentancemasteredit.annumonth === "") {
      setPopupContentMalert("Please Select Month!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemasteredit.frequency === "Annually" && maintentancemasteredit.annuday === "") {
      setPopupContentMalert("Please Select Day!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemasteredit?.duration === undefined || maintentancemasteredit?.duration === "00:00" || maintentancemasteredit?.duration?.includes("Mins")) {
      setPopupContentMalert("Please Select Duration!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    ...
    //   handleClickOpenerr();
    // }
    else if (!maintentancemasteredit?.needvendor) {
      setPopupContentMalert("Please Choose Yes/No, If You Need Vendor or not!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemasteredit?.needvendor === "Yes" && (vendorGroupEdit === "" || vendorGroupEdit === "Choose Vendor Group")) {
      setPopupContentMalert("Please Choose Vendor Group!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (maintentancemasteredit.description === "" || maintentancemasteredit.description === "<p><br></p>") {
      setPopupContentMalert("Please Enter Description!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    }
  ...
  const fetchArea = async (e) => {
    let result = allareagrouping.filter((d) => d.branch === newcheckbranch && d.floor === e).map((data) => data.area);
  ...
  const fetchLocation = async (e) => {
    let result = alllocationgrouping.filter((d) => d.branch === newcheckbranch && d.floor === maintentancemaster.floor && d.area === e).map((data) => data.location);
    let ji = [].concat(...result);
    const all = [
      { label: "ALL", value: "ALL" },
      ...ji.map((d) => ({
  ...
  const fetchMaintentance = async (e) => {
    setPageName(!pageName);
  ...
  const fetchMaintentanceDelete = async () => {
    ...
    try {
      ...
      setOverallFilterdataAll(
        itemsWithSerialNumberTotal.map((item, index) => {
          return {
            ...item,
            id: item._id,
            assetmaterial: item.assetmaterial,
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          ...
          {isUserRoleCompare?.includes("emaintenancemaster") && (
            <Link to={`/asset/maintenanceuserlog/${params.data.id}`} style={{ textDecoration: "none", color: "#fff", minWidth: "0px" }}>
  ...
  const customValueRendererAssetCode = (valueAssetCodeCat, _categoryname) => {
    return valueAssetCodeCat?.length ? valueAssetCodeCat.map(({ label }) => label)?.join(", ") : "Please Select Asset Material Code";
  };
  ...
  const customValueRendererAssetCodeEdit = (valueAssetCodeEditCat, _categoryname) => {
    return valueAssetCodeEditCat?.length ? valueAssetCodeEditCat.map(({ label }) => label)?.join(", ") : "Please Select Asset Material Code";
  };
  ...
  //company multiselect
  //team multiselect
  const [materialOptlist, setMaterialoptlist] = useState([]);
  ...
  const handleAssetMaterials = async () => {
    try {
      ...
      setMaterialoptlist(assetmaterialuniqueArray);
    } catch (err) {
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
  };
  ...
  return (
    <Box>
      ...
      <PageHeading title="Maintenance Master" modulename="Asset" submodulename="Maintenance" mainpagename="Maintenance Master" subpagename="" subsubpagename="" />
          ...
          <Box sx={userStyle.selectcontainer}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>Add Maintenance</Typography>
                </Grid>
              </Grid>
              <br />
              <Grid container spacing={2}>
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      options={
                        accessbranch
                          ? accessbranch
                              ?.filter((comp) => maintentancemaster.company === comp.company)
                              ?.map((data) => ({
                                label: data.branch,
                                value: data.branch,
                              }))
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      options={
                        accessbranch
                          ? accessbranch
                              ?.filter((comp) => maintentancemaster.company === comp.company && maintentancemaster.branch === comp.branch)
                              ?.map((data) => ({
                                label: data.unit,
                                value: data.unit,
                              }))
                          ...
                          ? Array.from(
                              new Set(
                                materialOpt
                                  .filter(
                                    (subpro) =>
                                      locations.map((item) => item.value).includes(subpro.location) &&
                                      subpro.maintenance === true &&
                                      subpro.company === maintentancemaster.company &&
                                      subpro.branch === maintentancemaster.branch &&
                                      subpro.unit === maintentancemaster.unit &&
                                      subpro.floor === maintentancemaster.floor &&
                          ...
                          : Array.from(
                              new Set(
                                materialOpt
                                  .filter(
                                    (subpro) =>
                                      subpro.location === maintentancemaster.location && subpro.maintenance === true && subpro.company === maintentancemaster.company && subpro.branch === maintentancemaster.branch && subpro.unit === maintentancemaster.unit && subpro.floor === maintentancemaster.floor && subpro.area === maintentancemaster.area
                                  )
                                  // .map((t) => t.component)
                                  // .reduce((acc, curr) => acc.concat(curr), [])
                                  .map((t) => ({
                          ...
                          : Array.from(
                              new Set(
                                materialOpt
                                  .filter(
                                    (subpro) =>
                                      subpro.location === maintentancemaster.location &&
                                      subpro.maintenance === true &&
                                      subpro.company === maintentancemaster.company &&
                                      subpro.branch === maintentancemaster.branch &&
                                      subpro.unit === maintentancemaster.unit &&
                ...
                <Grid item md={12} sm={12} xs={12}>
                  <Grid container spacing={1}>
                    ...
                    {(maintentancemaster.frequency === "Monthly" || maintentancemaster.frequency === "Date Wise") && (
                      <>
                    ...
                    {(maintentancemaster.frequency === "Weekly" || maintentancemaster.frequency === "Day Wise") && (
                      <>
                        <Grid item lg={3} md={3} sm={12} xs={12}>
                          <FormControl fullWidth size="small">
                            <Typography>
                            ...
                            <li key={index}>
                              <Grid container spacing={2}>
                                {editingIndexcheck === index ? (
                                  // index == 0
                                  <>
                                    <Grid item lg={8} md={10} sm={12} xs={12} marginTop={2}>
                                      <Grid container>
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      options={
                        accessbranch
                          ? accessbranch
                              ?.filter((comp) => maintentancemaster.companyto === comp.company)
                              ?.map((data) => ({
                                label: data.branch,
                                value: data.branch,
                              }))
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      options={
                        accessbranch
                          ? accessbranch
                              ?.filter((comp) => maintentancemaster.companyto === comp.company && bnuserto?.includes(comp.branch))
                              ?.map((data) => ({
                                label: data.unit,
                                value: data.unit,
                              }))
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      options={allTeam
                        ?.filter((comp) => maintentancemaster.companyto === comp.company && bnuserto?.includes(comp.branch) && unituserto?.includes(comp.unit))
                        ?.map((data) => ({
                          label: data.teamname,
                          value: data.teamname,
                        }))
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      options={[
                        { label: "ALL", value: "ALL" },
                        ...allUsersData
                          ?.filter((comp) => maintentancemaster.companyto === comp.company && selectedBranchTo.map((item) => item.value).includes(comp.branch) && selectedUnitTo.map((item) => item.value).includes(comp.unit) && selectedTeamTo.map((item) => item.value).includes(comp.team))
                          ?.map((com) => ({
                            ...com,
                  ...
                  <>
                    ...
                    <Grid item md={3} sm={12} xs={12}>
                      <FormControl fullWidth>
                        ...
                        <TextareaAutosize aria-label="minimum height" minRows={5} value={vendorgetid?.address} readOnly />
                      </FormControl>
                    </Grid>
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        <Typography>Phone </Typography>
                        <OutlinedInput id="component-outlined" type="text" value={vendorgetid?.phonenumber} readOnly />
                      </FormControl>
                    </Grid>
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        <Typography>Email </Typography>
                        <OutlinedInput id="component-outlined" type="text" value={vendorgetid?.emailid} readOnly />
                      </FormControl>
                    </Grid>
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput id="component-outlined" type="text" value={vendorgetid?.phonenumberone} readOnly />
                      </FormControl>
                    </Grid>
                  </>
                )}
      ...
      <Box>
        ...
        <Dialog open={isEditOpen} onClose={handleCloseModEdit} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="lg" fullWidth={true} sx={{ marginTop: "95px" }}>
          <Box sx={{ padding: "20px 50px" }}>
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>Edit Maintenance</Typography>
              </Grid>
              <br />
              <Grid container spacing={2}>
                <Grid item md={3} xs={12} sm={12}>
                          ...
                          ? Array.from(
                              new Set(
                                materialOpt
                                  .filter(
                                    (subpro) =>
                                      locationsEdit.map((item) => item.value).includes(subpro.location) &&
                                      subpro.maintenance === true &&
                                      subpro.company === maintentancemasteredit.company &&
                                      subpro.branch === maintentancemasteredit.branch &&
                                      subpro.unit === maintentancemasteredit.unit &&
                                      subpro.floor === maintentancemasteredit.floor &&
                                      subpro.area === maintentancemasteredit.area
                                  )
                                  // .map((t) => t.component)
                                  // .reduce((acc, curr) => acc.concat(curr), [])
                                  .map((t) => ({
                          ...
                          : Array.from(
                              new Set(
                                materialOpt
                                  .filter(
                                    (subpro) =>
                                      subpro.location === maintentancemasteredit.location &&
                                      subpro.maintenance === true &&
                                      subpro.company === maintentancemasteredit.company &&
                                      subpro.branch === maintentancemasteredit.branch &&
                                      subpro.unit === maintentancemasteredit.unit &&
                ...
                <Grid item md={12} sm={12} xs={12}>
                  <Grid container spacing={1}>
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <Selects
                          ...
                          placeholder="Please Select Schedule"
                          value={{
                            label: maintentancemasteredit.schedule === "" ? "Please Select Schedule" : maintentancemasteredit.schedule,
                            value: maintentancemasteredit.schedule === "" ? "Please Select Schedule" : maintentancemasteredit.schedule,
                          }}
                          onChange={(e) => {
                            setMaintentancemasteredit({
                              ...maintentancemasteredit,
                    ...
                    {(maintentancemasteredit.frequency === "Monthly" || maintentancemasteredit.frequency === "Date Wise") && (
                      <>
                        <Grid item lg={3} md={3} sm={12} xs={12}>
                          <FormControl fullWidth size="small">
                            <Typography>
                    ...
                    {(maintentancemasteredit.frequency === "Weekly" || maintentancemasteredit.frequency === "Day Wise") && (
                      <>
                        <Grid item lg={3} md={3} sm={12} xs={12}>
                          <FormControl fullWidth size="small">
                            <Typography>
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      options={
                        accessbranch
                          ? accessbranch
                              ?.filter((comp) => maintentancemasteredit.company === comp.company && bnuserto?.includes(comp.branch))
                              ?.map((data) => ({
                                label: data.unit,
                                value: data.unit,
                              }))
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      options={allTeam
                        ?.filter((comp) => maintentancemasteredit.company === comp.company && bnuserto?.includes(comp.branch) && unituserto?.includes(comp.unit))
                        ?.map((data) => ({
                          label: data.teamname,
                          value: data.teamname,
                        }))
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      options={[
                        { label: "ALL", value: "ALL" },
                        ...allUsersData
                          ?.filter((comp) => maintentancemasteredit.companyto === comp.company && selectedBranchToEdit.map((item) => item.value).includes(comp.branch) && selectedUnitToEdit.map((item) => item.value).includes(comp.unit) && selectedTeamToEdit.map((item) => item.value).includes(comp.team))
                          ?.map((com) => ({
                            ...com,
                            label: com.companyname,
                            value: com.companyname,
      ...
      {isUserRoleCompare?.includes("lmaintenancemaster") && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>Maintenance List</Typography>
            </Grid>
            <Grid container spacing={2}>
              <>
      ...
      {/* view model */}
      <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="lg" fullWidth={true} sx={{ marginTop: "95px" }}>
        <Box sx={{ padding: "20px 50px" }}>
          <>
              ...
              <Grid item md={6} xs={12} sm={12}>
                {maintenanceview.schedule === "Time Based" && (
                  <>
                    ...
                    <Typography>{maintenanceview.timetodo?.map((t, i) => `${i + 1 + ". "}` + `${t?.hour}:${t?.min} ${t?.timetype}`)}</Typography>
                  </>
                )}
                {(maintenanceview.frequency === "Monthly" || maintenanceview.frequency === "Date Wise") && (
                  <>
                    <Typography variant="h6">Days</Typography>
                    <Typography>{maintenanceview?.monthdate}</Typography>
                  </>
                )}
                {(maintenanceview?.frequency === "Weekly" || maintenanceview?.frequency === "Day Wise") && (
                  <>
                    <Typography variant="h6">Days</Typography>
                    <Typography>{maintenanceview?.weekdays?.map((t, i) => `${i + 1 + ". "}` + t)}</Typography>
                  </>
                )}
      ...
      />
      {/* INFO */}
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Maintenance Master Info" addedby={addedby} updateby={updateby} />
