...
function VerifiedListPassword({
  ...
  const downloadPdf = (isfilter) => {
    ...
    doc.save("FirewallStatusVerifiedPassword.pdf");
  };
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage.toBlob(gridRefTableImg.current)
        .then((blob) => {
          saveAs(blob, "FirewallStatusVerifiedPassword.png");
        })
        .catch((error) => {
  ...
  //         (data) => data?.firewallstatus === "Verified"
  //       )
  //     );
  //     setLoading(true);
  ...
  const fetchAllApproveds = async (e) => {
    setPageName(!pageName)
    setLoading(true)
    try {
      if (e === "Filtered") {
        let res_queue = await axios.post(SERVICE.PASSWORD_ACCESS_CONTROL_FILTER, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        ...
        let result = res_queue?.data.pass.filter(
          (data) => data?.firewallstatus === "Verified"
        )
        setDocumentsList(
          // res_queue?.data.pass.filter(
  ...
  // Excel
  const fileName = "FirewallStatusVerifiedPassword";
  ...
  const delVendorcheckbox = async () => {
    ...
    try {
      ...
      setTwotable("sdfgdfhfdgghjghjgh");
      handleCloseModcheckbox();
      setSelectedRows([]);
      setSelectAllChecked(false);
      setPage(1);
      await fetchAllApproveds("Filtered"); fetchUnAssignedIP();
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "FirewallStatusVerifiedPassword",
    pageStyle: "print",
  });
  ...
  const handleUpdate = async (e, firewallstatus) => {
    ...
    try {
      ...
      setTwotable("jhvbgfjkasbf");
      await fetchAllApproveds("Filtered");
      setStatus({});
      setBtnSubmit(false);
  ...
  return (
    <Box>
      ...
      <ExportData
        ...
        itemsTwo={items ?? []}
        filename={"FirewallStatusVerifiedPassword"}
...
export default VerifiedListPassword;
