...
const Homelayout = () => {
    ...
    try {
      let [res_Employee, res_leave, res_notice, res_relieve, res_notcheckin, res_News, res_candidate, res_upcoming] = await Promise.all([
        ...
        isUserRoleCompare?.includes('lmanageevents') && isUserRoleCompare?.includes('lnews&events')
          ? axios.post(SERVICE.ALL_EVENT_HOME, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
            assignbranch: accessbranch,
        ...
        isUserRoleCompare?.includes('lassignedcandidates') && isUserRoleCompare?.includes('lrecentjobapplications')
          ? axios.post(SERVICE.CANDIDATES_ALL_COUNT_HOME, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
  ...
  const fetchButtonEvents = async (e) => {
    ...
    let res_employee = [];
    try {
      if (isUserRoleCompare?.includes('lmyinterview') && isUserRoleCompare?.includes('lupcominginterviews')) {
        res_employee = await axios.post(SERVICE.CANDIDATES_ALL_UPCOMING, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          selectedfilter: e,
                            ...
                            <Grid container>
                              ...
                              <Grid item xs={8} md={8} sm={8} sx={{ display: 'flex', justifyContent: 'center', flexDirection: 'column', alignItems: 'center' }}>
                                <Tooltip title={item.title}>
                                  <Typography sx={{ fontWeight: 'bold', fontFamily: 'League Spartan, sans-serif', fontSize: { md: '11px', sm: '10px', xs: '12px' } }}>{item.title.length > 15 ? `${item.title.substring(0, 15)}...` : item.title}</Typography>
                                </Tooltip>
                                <Tooltip title={item.agenda.replace(/<\/?[^>]+(>|$)/g, '')}>
                                  <Typography sx={{ fontSize: { xs: '9px', sm: '10px', md: '12px' } }}> {item.agenda.replace(/<\/?[^>]+(>|$)/g, '').length > 15 ? `${item.agenda.replace(/<\/?[^>]+(>|$)/g, '').substring(0, 15)}...` : item.agenda.replace(/<\/?[^>]+(>|$)/g, '')}</Typography>
                                </Tooltip>
                              </Grid>
                            </Grid>
                          </Grid>
                          ...
                          <Grid item xs={12} sm={6} md={12} key={index}>
                            <Grid container>
                              <Grid item xs={2} sm={2} md={2} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                <Avatar src={item.uploadedimage ? item.uploadedimage : item.gender === 'Male' ? maleimage : femaleimage} alt="Profile" sx={{ width: 40, height: 40 }} />
                              </Grid>
                              <Grid item xs={5} sm={5} md={5} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                <Tooltip title={item.fullname.length > 10 ? item.fullname : ''}>
                                  <Typography sx={{ fontWeight: 800, fontSize: { xs: '9px', sm: '10px', md: '12px' }, textTransform: 'uppercase' }}>{item.fullname.length > 10 ? `${item.fullname.substring(0, 10)}...` : item.fullname}</Typography>
                                </Tooltip>
                                <Typography sx={{ fontWeight: 600, fontSize: { xs: '9px', sm: '10px', md: '12px' } }}> {(item.city === undefined || item.city === "undefined") ? "" : item.city}</Typography>
                              </Grid>
                              <Grid item xs={5} sm={5} md={5} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                <Tooltip title={item.role}>
                                  <Chip sx={{ height: '38px', background: '#b655196e', borderRadius: '28px', fontSize: '12px' }} label={item.role} />
                                </Tooltip>
                              </Grid>
                            </Grid>
                          </Grid>
                        ...
                        {upcomingInterview?.map((item, index) => (
                          ...
                          <Grid item xs={12} md={6} sm={6} key={index}>
                            ...
                            >
                              ...
                              >
                                <Avatar
                                  src={item.uploadedimage ? item.uploadedimage : item.gender === 'Male' ? maleimage : femaleimage}
                                  alt="Profile"
                                  sx={{
                                    width: {
                              ...
                              >
                                ...
                                <Typography sx={{ fontWeight: 600, fontSize: '12px' }}>{(item.city === undefined || item.city === "undefined") ? "" : item.city}</Typography>
                              </Grid>
