...
function Rechecklist() {
  ...
  const { isUserRoleAccess, isUserRoleCompare, pageName, isAssignBranch,
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage.toBlob(gridRefTableImg.current)
        .then((blob) => {
          saveAs(blob, "Notice Period Recheck List.png");
        })
        .catch((error) => {
  ...
  const fetchNoticeperiodlist = async () => {
    ...
    try {
      ...
      setOverallItems(ans?.map((item) => ({
        ...
        approvenoticereq: moment(item.approvenoticereq).format("DD-MM-YYYY"),
        releasedate: item.approvenoticereq,
      })));
    } catch (err) { handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Notice Period Recheck List ",
    pageStyle: "print",
  });
  ...
  return (
    <Box>
          ...
          <Box>
            ...
            {!isBoarding ? (
              ...
              <>
                ...
                <Grid style={{ ...userStyle.dataTablestyle, dispaly: 'flex', justifyContent: 'space-between' }}>
                  <Box>
      ...
      <ExportData
        ...
        filename={"Notice Period Recheck List"}
