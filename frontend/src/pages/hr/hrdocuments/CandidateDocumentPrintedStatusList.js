...
function CandidateDocumentsPrintedStatusList() {
    ...
    const [filterState, setFilterState] = useState({
        type: "Company",
        employeestatus: "Please Select Candidate Status",
    });
    ...
    const TemplateDropdowns = async () => {
        ...
        try {
            let res = await axios.post(SERVICE.CANDIDATE_TEMPLATE_CREATION_ASSIGN_BRANCH, {
    ...
    const handleFilter = () => {
        if (["Company", "Branch"]?.includes(filterState.type) && selectedOptionsCompany?.length === 0) {
            setPopupContentMalert("Please Select Company!");
            setPopupSeverityMalert("warning");
            handleClickOpenPopupMalert();
        }
        else if (["Branch"]?.includes(filterState.type) && selectedOptionsBranch?.length === 0) {
            setPopupContentMalert("Please Select Branch!");
            setPopupSeverityMalert("warning");
            handleClickOpenPopupMalert();
        }
        else if (["Template"]?.includes(filterState.type) && selectedOptionsTemplate?.length === 0) {
    ...
    const fetchBrandMaster = async () => {
        ...
        const queryParams = {
            ...
            approval: valueStatus,
            company: valueCompanyCat,
            branch: valueBranchCat,
        };
        setPageName(!pageName);
        try {
            let res_freq = await axios.post(`${SERVICE.ACCESSBRANCH_CANDIDATE_PRINTED_LIST}`,
                queryParams
                , {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
            ...
            const answer = res_freq?.data?.documentPreparation?.length > 0 ? res_freq?.data?.documentPreparation?.map((item, index) => ({
                ...
                rounds: item.rounds,
                documentneeed: item.documentneeed,
            })) : [];
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "Candidate Printed Documents List.png");
                })
                .catch((error) => {
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "Candidate Document Printed Status List",
        pageStyle: "print",
    });
    ...
    const extractEmailFormat = async (userdetails, name, id, email, pagename) => {
    ...
    const getViewFile = async (name, details) => {
        const fileUrl = `${BASE_URL}/candidateDocuments/${name}`;
    ...
    const downloadFile = async (filename, details) => {
        try {
            const response = await axios.get(
                `${BASE_URL}/candidateDocuments/${filename}`,
                {
                    responseType: "blob",
                }
            );
    ...
    const downloadPdfTesdtTable = async (e, pagename) => {
        ...
        if (pagename !== "Candidate" && headerOptions === "Please Select Print Options") {
            setButtonLoadingPreview(false);
            setPopupContentMalert("Please Select Print Options!");
            setPopupSeverityMalert("info");
            handleClickOpenPopupMalert();
        }
        else if (pagename !== "Candidate" && headerOptions === "With Letter Head" && selectedHeadOpt?.length < 1) {
            ...
            await getUpdatePrintingStatus(e, response.data.scandidateDocumentPreparation?.updatedby)
            const pdfElement = document.createElement("div");
            pdfElement.innerHTML = response.data.scandidateDocumentPreparation.document;

            let headerDisp = pagename === "Candidate" ? response?.data?.scandidateDocumentPreparation?.head : head;
       ...
       `;
            ...
            const addPageNumbersAndHeadersFooters = (doc, watermarkImage, qrCodeImage) => {
                ...
                for (let i = 1; i <= totalPages; i++) {
                    ...
                    const footerX = 5;
                    const footerY = (pageHeight * 1) - (footerDisp === "" ? 15 : footerImgHeight - 3);
            ...
            html2pdf()
                .from(pdfElement)
                .set({
                    margin: response?.data?.scandidateDocumentPreparation?.pagesize == "A3"
                        ? ((headerDisp !== "" && footerDisp !== "") ? [45, 15, 45, 15]
                            : (headerDisp === "" && footerDisp !== "") ? [20, 15, 45, 15]
                                : (headerDisp !== "" && footerDisp === "") ? [45, 15, 20, 15] :
                                    [20, 15, 20, 15])

                        :
                        ((headerDisp !== "" && (footerDisp !== "")) ? [30, 15, 45, 15]
                            : (headerDisp === "" && footerDisp !== "") ? [15, 15, 45, 15]
                                : (headerDisp !== "" && footerDisp === "") ? [45, 15, 15, 15] : [15, 15, 15, 15]),
                        ...
                        qrImg.onload = () => {
                            ...
                            reader.onloadend = async () => {
                                ...
                                let res_module = await axios.post(SERVICE.CANDIDATE_DOCUMENT_PREPARATION_MAIL, {
                                    document: response?.data?.scandidateDocumentPreparation?.approval === "approved" ? response?.data?.scandidateDocumentPreparation?.approvedfilename : base64String,
                                    ...
                                    pagename: pagename,
                                    approval: response?.data?.scandidateDocumentPreparation?.approval
    ...
    const columnDataTable = [
        ...
        },
        {
            ...
            cellRenderer: (params) => (
                <Grid>
                    {params.data?.documentneed === "Candidate Approval" ?
                        <>
                            <Button
                                variant="contained"
                                color="primary"
                                ...
                                onClick={() => {
                                    params?.data?.approval === "approved" ? getViewFile(params?.data?.approvedfilename, params?.data) :
                                        downloadPdfTesdtTable(params?.data?.id, "Candidate")
                                }}
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    extractEmailFormat(params.data, params.data.person, params.data.id, params.data?.email, "Candidate")
                                }}
                            >
                                {params?.data?.mail}
                            </Button>
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    params?.data?.approval === "approved" ?
                                        ...
                                        downloadPdfPrintTable(params?.data?.id, "Candidate")
                                }}
                            >
                                Print
                            </Button>
                        ...
                        <>
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    extractEmailFormat(params.data, params.data.person, params.data.id, params.data?.email)
                                }}
                            >
                                {params?.data?.mail}
                            </Button>
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <PageHeading title="Candidate Document Printed Status List" modulename="Human Resources" submodulename="HR Documents" mainpagename="Candidate Documents" subpagename="Candidate Document Printed Status List" subsubpagename="" />

            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lcandidatedocumentprintedlist") && (
                <>
                    <Box sx={userStyle.selectcontainer}>
                        <NotificationContainer />
                        {/* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Candidate Document Printed Status Filter </Typography>
                        </Grid>
                        <br />
                        <Grid container spacing={2}>
                            ...
                            {["Company", "Branch"]?.includes(filterState.type) && (
                                <>
                                    <Grid item md={3} xs={12} sm={12}>
                                        <Typography>
                            ...
                            {["Template"]?.includes(filterState.type) && (
                                <>
                                    <Grid item md={3} xs={12} sm={12}>
                                        <Typography>
                                        ...
                                        <FormControl size="small" fullWidth>
                                            <MultiSelect
                                                ...
                                                value={selectedOptionsTemplate}
                                                onChange={(e) => {
                                                    handleTemplateChange(e);
                                                }}
                                                valueRenderer={customValueRendererTemplate}
                                                labelledBy="Please Select Template"
                                            />
                                        </FormControl>
                                    </Grid>
                                </>
                    ...
                    <Box sx={userStyle.selectcontainer}>
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Candidate Document Printed Status List</Typography>
                        </Grid>
                        <Grid container spacing={2} style={userStyle.dataTablestyle}>
                            ...
                            >
                                <Box>
                                    {isUserRoleCompare?.includes("excelcandidatedocumentprintedlist") && (
                                    ...
                                    {isUserRoleCompare?.includes("csvcandidatedocumentprintedlist") && (
                                    ...
                                    {isUserRoleCompare?.includes("printcandidatedocumentprintedlist") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("pdfcandidatedocumentprintedlist") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("imagecandidatedocumentprintedlist") && (
                ...
                <Table sx={{ minWidth: 700 }} aria-label="customized table" id="usertable" ref={componentRef}>
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell>Candidate Mode</TableCell>
            ...
            <InfoPopup
                ...
                heading="Candidate Document Printed Status Info"
                addedby={addedby}
                updateby={updateby}
            />
            ...
            <ExportData
                ...
                itemsTwo={items ?? []}
                filename={"Candidate Document Printed Status List"}
...
export default CandidateDocumentsPrintedStatusList;