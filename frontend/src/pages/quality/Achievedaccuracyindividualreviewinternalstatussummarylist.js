...
function AchievedAccuracyIndividualReviewInternalstatusSummaryList() {
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Achieved Accuracy Individual Review Internal Status Summary List"),
    ...
    const handleSubmission = async () => {
        ...
        try {
            ...
            const groupedData = withCompany.reduce((acc, item) => {
                // Create a unique key for grouping
                const key = `${item.company}-${item.branch}-${item.unit}-${item.team}-${item.employeename}-${item.project}`;

                if (!acc[key]) {
                    acc[key] = {
                        ...item,
                        accuracy: 0,  // Store accuracy values
                        totalfield: 0, // Sum totalfield
                        minimumaccuracy: 0,
                        loginid: [],   // Concatenation of loginid
                        internalstatus: [], // Concatenation of internalstatus
                        vendor: [],
                        queue: [],
                        date: [],
                        records: []
                ...
                acc[key].accuracy += parseInt(item.accuracy, 10);
                acc[key].totalfield += parseInt(item.totalfield, 10); // Convert totalfield to number and sum
                acc[key].minimumaccuracy += parseInt(item.minimumaccuracy, 10);
                acc[key].loginid.push(item.loginid);
                acc[key].internalstatus.push(item.internalstatus);

                acc[key].vendor.push(item.vendor);
                acc[key].queue.push(item.queue);
                acc[key].date.push(item.date);
            ...
            const itemsWithSerialNumber = result.map((item, index) => {
                ...
                const avgAccuracy = Math.round(item.accuracy / recordCount);
                const avgTotalfield = Math.round(item.totalfield / recordCount);
                const autoErrorCount = Math.round(avgTotalfield * (1 - (avgAccuracy / 100)));
                const minimumAccuracy = Math.round(item.minimumaccuracy / recordCount)

                return {
                    ...item,
                    serialNumber: index + 1,
                    date: `${moment(filterUser.fromdate).format('DD-MM-YYYY')} - ${moment(filterUser.todate).format('DD-MM-YYYY')}`,
                    accuracy: avgAccuracy,
                    totalfield: avgTotalfield,
                    autoerrorcount: autoErrorCount,
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "Achieved Accuracy Individual Review Internal Status Summary List.png");
                })
                .catch((error) => {
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("Achieved Accuracy Individual Review Internal Status Summary List.pdf");
    };
    // Excel
    const fileName = "Achieved Accuracy Individual Review Internal Status Summary List";
    //print...
    const componentRef = useRef();
    const handleprint = useReactToPrint({
        content: () => componentRef.current,
        documentTitle: "Achieved Accuracy Individual Review Internal Status Summary List",
        pageStyle: "print",
    });
    useEffect(() => {
    ...
    return (
        <Box>
            {/* ****** Header Content ****** */}
            <Headtitle title={"Achieved Accuracy Individual Review Internal Status Summary List"} />
            <PageHeading
                title="Achieved Accuracy Individual Review Internal Status Summary List"
                modulename="Quality"
                submodulename="Accuracy"
                mainpagename="Achieved Accuracy Individual Review Internal Status Summary List"
                subpagename=""
                subsubpagename=""
            />
            ...
            {isUserRoleCompare?.includes("lachievedaccuracyindividualreviewinternalstatuslist") && (
                <>
                    ...
                    <Box sx={userStyle.container}>
                        { /* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Achieved Accuracy Individual Review Internal Status Summary List</Typography>
                        </Grid>
            ...
            >
                <Box sx={{ padding: '20px 50px' }}>
                    <>
                        <Typography sx={userStyle.HeaderText}>View Achieved Accuracy Individual Review Internal Status Summary List</Typography>
                        <br /><br />
                        <Grid container spacing={2}>
                            <Grid item md={4} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6">Date</Typography>
                                    {Array.isArray(overallList.date) ? (
                                        overallList.date.map((item, index) => (
                            ...
                            <Grid item md={4} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    ...
                                    {Array.isArray(overallList.vendor) ? (
                                        overallList.vendor.map((item, index) => (
                                            <Typography key={index}>{item}</Typography>
                                        ))
                                    ) : (
                            ...
                            <Grid item md={4} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    ...
                                    {Array.isArray(overallList.queue) ? (
                                        overallList.queue.map((item, index) => (
                                            <Typography key={index}>{item}</Typography>
                                        ))
                                    ) : (
                            ...
                            <Grid item md={4} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    ...
                                    {Array.isArray(overallList.internalstatus) ? (
                                        overallList.internalstatus.map((item, index) => (
                                            <Typography key={index}>{item}</Typography>
                                        ))
                                    ) : (
                            ...
                            <Grid item md={4} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    ...
                                    {Array.isArray(overallList.loginid) ? (
                                        overallList.loginid.map((item, index) => (
                                            <Typography key={index}>{item}</Typography>
                                        ))
                                    ) : (
            ...
            <ExportData
                ...
                itemsTwo={employees ?? []}
                filename={"Achieved Accuracy Individual Review Internal Status Summary List"}
...
export default AchievedAccuracyIndividualReviewInternalstatusSummaryList;