...
function Prioritymaster() {
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "PriorityMaster.png");
        });
      });
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    const isNameMatch = prioritymasterall.some(
      (item) =>
    ...
    if (valueCat?.length == 0) {
      ...
      setShowAlert(
        <>
          ...
          />
          <p style={{ fontSize: "20px", fontWeight: 900 }}>
            {"These Priority Master data already exists!"}
          </p>
        </>
      );
      handleClickOpenerr();
  ...
  const getCode = async (e, name) => {
    try {
      ...
      setSubSubCategoryValueCateEdit(
        res?.data?.sprioritymaster?.subsubcategory
      );
      setSelectedSubSubCategoryOptionsCateEdit([
  ...
  const sendEditRequest = async () => {
    try {
      let res = await axios.put(
        `${SERVICE.PRIORITYMASTER_SINGLE}/${subprojectsid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      await getOverallEditSectionUpdate();fetchPriorityMasters();
      handleCloseModEdit();
      setShowAlert(
        <>
          {" "}
  ...
  const fetchPriorityMasters = async () => {
    try {
      ...
      setPrioritymasterall(res_vendor?.data?.prioritymaster);
  ...
  const fetchPriorityMastersAll = async () => {
    try {
      ...
      return res_check?.data?.prioritymaster.filter(
        (item) => item._id !== prioritymasterEdit._id
      )
    } catch (err) {handleApiError(err, setShowAlert, handleClickOpenerr);}
  };
  ...
  // Excel
  const fileName = "PriorityMaster";

  //print...
  const componentRef = useRef();
  const handleprint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: "PriorityMaster",
    pageStyle: "print",
  });
  ...
  const addSerialNumber = () => {
    const itemsWithSerialNumber = prioritymasterall?.map((item, index) => ({
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <Typography sx={userStyle.HeaderText}>Priority Master</Typography>
                    ...
                    <Selects
                      options={reasonOptions
                        ?.filter((u) => {
                          ...
                          const typematch =
                            u.typereason === prioritymasterEdit.type;
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {prioritymasterEdit.subcategory
                      ?.map((t, i) => `${i + 1 + ". "}` + t)
                      .toString()}
                  </Typography>
                </FormControl>
              ...
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>
                    {prioritymasterEdit.subsubcategory
                      ?.map((t, i) => `${i + 1 + ". "}` + t)
                      .toString()}
                  </Typography>
                </FormControl>
