...
//   // const rowDatadeleteRecords = async (id, uniqid, userId) => {
...
//   //     socket.on(`deleteProgress:${userId}`, (data) => {
//   //       if (data.error) {
...
//   //       setShowAlert(`Deleting... ${data.progress}%`);
//   //       setloadingdeloverall(data.progress < 100); // Disable button until 100%

//   //       if (data.completed) {
//   //         setShowAlert("Deletion Completed ✅");
//   //       }
//   //     });
//   //   } catch (err) {
//   //     handleApiError(err, setShowAlert, handleClickOpenerr);
...
//   //       data: { filter: { uniqueid: deleteRound.uniqid }, batchSize: 25000, id: deleteRound.id },
//   //     });
//   //     setPopupContentMalert("Deletion job started. Check the backend logs for progress.");
...
//   // useEffect(() => {
//   //   // Listen for global delete progress updates
//   //   socket.on("deleteProgress", (data) => {
...
//   //     setProgress(data);
//   //     setloadingdeloverall(Object.values(data)[0] < 100); // Disable button while deleting
...
//   //   return () => {
//   //     socket.off("deleteProgress"); // Clean up listener
//   //   };
//   // }, []);

//   useEffect(() => {
//     const handleDeleteProgress = (data) => {
//       console.log(data, "data");
//       if (data.error) {
//         setloadingdeloverall(false);
//         alert("Error: " + data.error);
...
//     socket.off("deleteProgress", handleDeleteProgress); // Ensure previous listener is removed
//     socket.on("deleteProgress", handleDeleteProgress); // Add new listener

//     return () => {
//       socket.off("deleteProgress", handleDeleteProgress); // Cleanup on unmount
//     };
//   }, []);
...
//       socket.emit("pingTest", "Hello from frontend!");
//     });

//     socket.on("pongTest", (message) => {
...
//   const delProduction = async () => {
//     handleCloseMod();
//     setloadingdeloverall(true);

//     try {
//       const userId = "user123"; // Unique identifier for the current user

//       await axios.delete(SERVICE.MONTH_UPLOAD_BATCH_DELETE, {
//         data: {
//           filter: { uniqueid: deleteRound.uniqid },
//           batchSize: 25000,
//           id: deleteRound.id,
//           // userId, // Send user ID for tracking
//         },
//       });
//       setPopupContentMalert("Deletion job started. Check progress...");
//       setPopupSeverityMalert("info");
//       handleClickOpenPopupMalert();
//     } catch (err) {
//       console.log(err, "err");
...
//   };
//   console.log(progress?.hasOwnProperty("67b9c24d0ea6dcfc67b18986"), progress["67b9c24d0ea6dcfc67b18986"], "67b9c24d0ea6dcfc67b18986" in progress, Object.keys(progress || {}).includes(String("67b9c24d0ea6dcfc67b18986")));
...
//             // <LoadingButton sx={{ ...userStyle.buttondelete, textTransform: "capitalize" }} variant="contained" color="error" size="small" loading={loadingdeloverall} disabled={loadingdeloverall} onClick={() => rowDatadeleteRecords(params.row.id, params.row.uniqueid, "user123")}>
//             //   {loadingdeloverall && progress[params.row.id] ? progress[params.row.id] : "Start Deletion Process"}
//             // </LoadingButton>
//             <LoadingButton
...
//               color="error"
//               size="small"
//               loading={loadingdeloverall && progress?.hasOwnProperty(params.row.id)}
//               disabled={loadingdeloverall && Object.keys(progress || {}).includes(String(params.row.id))}
//               onClick={() => rowDatadeleteRecords(params.row.id, params.row.uniqueid, "user123")}
//             >
//               {loadingdeloverall && progress?.hasOwnProperty(params.row.id) ? progress[params.row.id] : "Start Deletion Process"}
//             </LoadingButton>
//           )}
...
//               Cancel
//             </Button>
//             <Button onClick={delProduction} color="error" variant="contained">
//               OK
//             </Button>
//           </DialogActions>
//         </Dialog>
