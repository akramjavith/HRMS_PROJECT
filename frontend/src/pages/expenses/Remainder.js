...
function Remaindermaster() {
  ...
  const downloadPdf = (isfilter) => {
    ...
    const dataWithSerial =
      isfilter === "filtered"
        ? rowDataTable?.map((t, index) => ({
            ...
            categoryexpense: t.categoryexpense,
            subcategoryexpense: t.subcategoryexpense,
            frequency: t.frequency,
          }))
        : items?.map((t, index) => ({
            serialNumber: index + 1,
    ...
    doc.save("Remainder.pdf");
  };

  const [remainder, setRemainder] = useState({
    categoryexpense: "Please Select Expense Category",
    subcategoryexpense: "Please Select Expense SubCategory",
  ...
  const [allRemainderedit, setAllRemainderedit] = useState([]);
  ...
  const fetchFrequency = async () => {
    try {
      let res_project = await axios.get(SERVICE.ALL_FREQUENCYMASTER, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      const deptall = [
        ...res_project?.data?.freqMaster?.map((d) => ({
          ...d,
          label: d.name,
          value: d.name,
        })),
      ];
      setFrequencymaster(deptall);
    } catch (err) {
  ...
  const rowData = async (id, name) => {
    ...
    try {
      let res = await axios.get(`${SERVICE.REMAINDER_SINGLE}/${id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setDeleteRemainder(res?.data?.sremainder);
      handleClickOpen();
    } catch (err) {
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
  ...
  const delRemainder = async (e) => {
    ...
    try {
      if (Remaindersid) {
        await axios.delete(`${SERVICE.REMAINDER_SINGLE}/${e}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });
  ...
  const delRemaindercheckbox = async () => {
    ...
    try {
      const deletePromises = selectedRows?.map((item) => {
        return axios.delete(`${SERVICE.REMAINDER_SINGLE}/${item}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });
  ...
  const fetchCategoryExpense = async () => {
    try {
      ...
      const categoryall = [
        ...res_category?.data?.expensecategory.map((d) => ({
          ...d,
  ...
  const fetchCategoryBased = async (e) => {
    try {
      ...
      let data_set = res_category?.data?.expensecategory
        .filter((data) => {
  ...
  const sendRequest = async () => {
    setPageName(!pageName);
    try {
      await axios.post(SERVICE.REMAINDER_CREATE, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const handleSubmit = (e) => {
    setBtnSubmit(true);
    e.preventDefault();
    const isNameMatch = remainders.some(
      (item) =>
    ...
    if (remainder.categoryexpense === "Please Select Expense Category") {
      setShowAlert(
        <>
      ...
      remainder.subcategoryexpense === "Please Select Expense SubCategory"
    ) {
      setShowAlert(
        <>
          <ErrorOutlineOutlinedIcon
          ...
          />
          <p style={{ fontSize: "20px", fontWeight: 900 }}>
            {"Please Select Expense SubCategory"}
          </p>
        </>
      );
      handleClickOpenerr();
  ...
  const getCode = async (e, name) => {
    setPageName(!pageName);
    try {
      let res = await axios.get(`${SERVICE.REMAINDER_SINGLE}/${e}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setRemainderEdit(res?.data?.sremainder);
      fetchCategoryBasedEdit(res?.data?.sremainder?.categoryexpense);
    } catch (err) {
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
  };
  ...
  const sendEditRequest = async () => {
    setPageName(!pageName);
    try {
      await axios.put(`${SERVICE.REMAINDER_SINGLE}/${subprojectsid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const editSubmit = async (e) => {
    ...
    const isNameMatch = resdata.some(
      (item) =>
        item.categoryexpense === remainderEdit.categoryexpense &&
        item.subcategoryexpense === remainderEdit.subcategoryexpense &&
        item.frequency === remainderEdit.frequency
    );
    if (remainderEdit.categoryexpense === "Please Select Expense Category") {
      setShowAlert(
        <>
          <ErrorOutlineOutlinedIcon
            sx={{ fontSize: "100px", color: "orange" }}
      ...
      remainderEdit.subcategoryexpense === "Please Select Expense SubCategory"
    ) {
      setShowAlert(
        <>
          <ErrorOutlineOutlinedIcon
  ...
  const fetchRemainder = async () => {
    setPageName(!pageName);
    try {
      let res_vendor = await axios.get(SERVICE.REMAINDER, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });

      setRemainders(res_vendor?.data?.remainders);
  ...
  const fetchRemainderAll = async () => {
    try {
      ...
      return res_vendor?.data?.remainders.filter(
        (item) => item._id !== remainderEdit._id
      );
    } catch (err) {
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
  ...
  const addSerialNumber = () => {
    const itemsWithSerialNumber = remainders?.map((item, index) => ({
      ...item,
      serialNumber: index + 1,
    }));
  ...
  const columnDataTable = [
    ...
    },
    {
      ...
      flex: 0,
      width: 250,
      hide: !columnVisibility.categoryexpense,
      headerClassName: "bold-header",
    },
    {
      field: "subcategoryexpense",
      headerName: "Expense Subcategory",
      flex: 0,
      width: 250,
      hide: !columnVisibility.subcategoryexpense,
      headerClassName: "bold-header",
    },
    {
      field: "frequency",
...

    {
      ...
      renderCell: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("eremaindermaster") && (
            <Button
          ...
          {isUserRoleCompare?.includes("dremaindermaster") && (
            <Button
          ...
          {isUserRoleCompare?.includes("vremaindermaster") && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
                handleClickOpenview();
          ...
          {isUserRoleCompare?.includes("iremaindermaster") && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
                handleClickOpeninfo();
  ...
  return (
    <Box>
      <Headtitle title={"Remainder"} />
      <PageHeading
        title="Manage Remainder"
        modulename="Expenses"
        submodulename="Remainder"
        mainpagename="Remainder Master"
        subpagename=""
        subsubpagename=""
      />
      {/* ****** Header Content ****** */}
      {isUserRoleCompare?.includes("aremaindermaster") && (
        <>
            ...
            <>
              ...
              <Grid container spacing={2}>
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      styles={colourStyles}
                      value={{
                        label: remainder.categoryexpense,
                        value: remainder.categoryexpense,
                      }}
                      onChange={(e) => {
                        setRemainder({
                          ...remainder,
                          categoryexpense: e.value,
                          subcategoryexpense:
                            "Please Select Expense SubCategory",
                        });
                ...
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    <Typography>
                      Expense Subcategory<b style={{ color: "red" }}>*</b>
                    </Typography>
                    <Selects
                      options={subcategorys}
                      styles={colourStyles}
                      value={{
                        label: remainder.subcategoryexpense,
                        value: remainder.subcategoryexpense,
                      }}
                      onChange={(e) => {
                        setRemainder({
                          ...remainder,
                          subcategoryexpense: e.value,
                        });
                      }}
                    />
                  </FormControl>
                  ...
                  <Grid item md={4} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <Selects
                        ...
                        styles={colourStyles}
                        value={{
                          label: remainderEdit.categoryexpense,
                          value: remainderEdit.categoryexpense,
                        }}
                        onChange={(e) => {
                          setRemainderEdit({
                            ...remainderEdit,
                  ...
                  <Grid item md={4} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <Selects
                        ...
                        styles={colourStyles}
                        value={{
                          label: remainderEdit.subcategoryexpense,
                          value: remainderEdit.subcategoryexpense,
                        }}
                        onChange={(e) => {
                          setRemainderEdit({
                            ...remainderEdit,
                  ...
                  <Grid item md={4} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <Selects
                        ...
                        styles={colourStyles}
                        value={{
                          label: remainderEdit.frequency,
                          value: remainderEdit.frequency,
                        }}
                        onChange={(e) => {
                          setRemainderEdit({
                            ...remainderEdit,
      ...
      <br />
      {/* ****** Table Start ****** */}
      {isUserRoleCompare?.includes("lremaindermaster") && (
        <>
          <Box sx={userStyle.container}>
            ...
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              >
                <Box>
                  {isUserRoleCompare?.includes("excelremaindermaster") && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes("csvremaindermaster") && (
                    <>
                      <Button
                        onClick={(e) => {
                          setIsFilterOpen(true);
                  ...
                  {isUserRoleCompare?.includes("printremaindermaster") && (
                    <>
                  ...
                  {isUserRoleCompare?.includes("pdfremaindermaster") && (
                    <>
                      <Button
                        sx={userStyle.buttongrp}
                        onClick={() => {
                  ...
                  {isUserRoleCompare?.includes("imageremaindermaster") && (
                    <Button
                      sx={userStyle.buttongrp}
            ...
            &ensp;
            {isUserRoleCompare?.includes("bdremaindermaster") && (
              <Button
                variant="contained"
                color="error"
      ...
      <Box>
        ...
        >
          <Box sx={{ width: "550px", padding: "20px 50px" }}>
            <>
              <Typography sx={userStyle.HeaderText}>Remainder Info</Typography>
              <br />
              <br />
              <Grid container spacing={2}>
                <Grid item md={12} xs={12} sm={12}>
      ...
      >
        <Box sx={{ width: "750px", padding: "20px 50px" }}>
          <>
            <Typography sx={userStyle.HeaderText}> View Remainder</Typography>
            <br /> <br />
            <Grid container spacing={2}>
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
...
export default Remaindermaster;
