...
function UsersTaskAllocation() {
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("User_Task_Panel-Task_Allocation.pdf");
    };
    ...
    const fetchAllRaisedTickets = async (date) => {
        try {
            ...
            if (date === "") {
                ...
                let res_task = await axios.get(SERVICE.ALL_TASKSCHEDULEGROUPING, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
                });
                let res_task_Schedule = await axios.get(SERVICE.ALL_TASKSCHEDULEGROUPING, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
                });
                let res_task_Desig = await axios.get(SERVICE.ALL_TASKDESIGNATIONGROUPING_ACTIVE, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
                });
                ...
                const updatedArray = taskStatusDep?.length > 0 ? res_task?.data?.taskschedulegrouping.map(item2 => {
                    ...
                    if (matchingItem) {
                        return { ...item2, priority: matchingItem.priority, description: matchingItem.description, documentfiles: matchingItem.documentfiles };
                    }
                }) : [];
                ...
                let priorityCheck = anstaskUserPanel?.length > 0 ? anstaskUserPanel?.filter((item) => {
                    if (item?.frequency === "Daily" && weekOffShow && holidayShow) {
                        //Shift Basis 
                        if (item?.schedule === "Fixed" && weekOffShow) {
                            return item;

                        }
                        else if (item?.schedule === "Fixed" && weekOffShow) {
                            return item;
                        }
                        //Anytime Basis 
                    ...
                    if ((item?.frequency === "Day Wise" || item?.frequency === "Weekly" && weekOffShow && holidayShow)) {
                        if (item?.schedule === "Fixed") {
                            return item?.weekdays?.includes(dayOfWeek);

                        }
                        else if (item?.schedule === "Fixed") {
                    ...
                    if (item?.frequency === "Monthly" || item?.frequency === "Date Wise") {
                        //Shift Basis 
                        if (item?.schedule === "Fixed") {
                ...
                let userCheck = await axios.get(SERVICE.ALL_TASKFORUSER, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
                });
                let userChecknonschedule = await axios.get(SERVICE.ALL_TASK_NONSCHEDULEGROUPING, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
                });
                ...
                let uniqueElements = updatedAns1?.filter(obj1 => !userCheck?.data?.taskforuser?.some(obj2 =>
                    ...
                    && new Date(date) <= new Date(obj2.shiftEndTime) && obj2?.taskdetails === "schedule"
                ));


                let nonschedule = userChecknonschedule?.data?.tasknonschedulegrouping?.filter(data => data?.employeenames?.includes(isUserRoleAccess?.companyname) && data?.date === moment(new Date(date)).format("YYYY-MM-DD"))
                let result = [];
                ...
                const answerUserNonSchedule = userCheck?.data?.taskforuser?.filter(data => data?.taskdetails === "nonschedule" && data?.taskstatus !== "Completed" && data?.username === isUserRoleAccess?.companyname && data?.taskassigneddate === moment(new Date(date)).format("DD-MM-YYYY"))
                const answerUserSchedule = userCheck?.data?.taskforuser?.filter(data => data?.taskdetails === "schedule" && data?.taskstatus !== "Completed" && data?.username === isUserRoleAccess?.companyname && data?.taskassigneddate === moment(new Date(date)).format("DD-MM-YYYY"))
                const answerUsermanual = userCheck?.data?.taskforuser?.filter(data => data?.taskdetails === "Manual" && data?.taskstatus !== "Completed" && data?.username === isUserRoleAccess?.companyname && data?.taskdate === moment(new Date(date)).format("YYYY-MM-DD"))



                const prioritySchedule = priorityCheck?.length > 0 ? priorityCheck?.map(data => {
                    return { ...data, taskdetails: "schedule" }
                }) : []

                const prioritynonschedule = answerUserNonSchedule?.length > 0 ? answerUserNonSchedule?.map(data => {
                    return { ...data, taskdetails: "nonschedule" }
                }) : []

                const priorityanswerUserNonSchedule = nonschedule?.length > 0 ?
                    nonschedule?.filter(obj1 => !prioritynonschedule?.some(obj2 =>
                        obj1.category === obj2.category
                        && obj1.subcategory === obj2.subcategory
                        && obj1.schedule === obj2.schedule
                        && obj1.date === obj2.taskdate
                        && obj2.username === isUserRoleAccess?.companyname
                    )).map(data => {
                        return { ...data, taskdetails: "nonschedule", tasktime: data.time }
                    }) : []


                setQueueCheck(true)
                setRaiseTicketList([...prioritySchedule, ...answerUsermanual, ...answerUserSchedule, ...prioritynonschedule, ...priorityanswerUserNonSchedule])

                let uniqueElementsNonSchedule = result?.length > 0 ? result?.filter(obj1 => !answerUserNonSchedule?.some(obj2 =>
                    obj1.category === obj2.category
                    && obj1.subcat === obj2.subcat &&
                    obj2?.taskassigneddate === moment(new Date(date)).format("DD-MM-YYYY")
                    && obj2.username === isUserRoleAccess?.companyname
                ...
                function getTimeDifference(time1, time2) {
                    ...
                    return (date1 - date2) / (1000 * 60); // Difference in minutes
                }
                ...
                const answerClosest = closestElements?.filter(data => data?.length > 0)
                ...
                const RemoveDuplicateClosest = removeDuplicates(closestElements, raiseTicketList);
    ...
    const addSerialNumber = () => {
        const itemsWithSerialNumber = raiseTicketList?.map((item, index) => ({
            ...
            tasktime: item?.taskdetails === "nonschedule" ?
                ...
                item.schedule === "Any Time" ? "" :
                    item?.taskdetails === "Manual" ? convertTimeToAMPMFormat(item.tasktime) :
    ...
    // Excel
    const fileName = "User_Task_Panel-Task_Allocation";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "User_Task_Panel-Task_Allocation",
        pageStyle: "print",
    });
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "User_Task_Panel-Task_Allocation.png");
                });
            });
        }
    };
    ...
    const ListPageLoad = async () => {
        try {
            let res_task = await axios.get(SERVICE.ALL_TASKFORUSER, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            let res_task_Desig = await axios.get(SERVICE.ALL_TASKDESIGNATIONGROUPING, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            ...
            let taskStatusDep = res_task_Desig?.data?.taskdesignationgrouping.filter(data => {
                if (data?.type === "Designation") {
                    return data.designation?.includes(isUserRoleAccess?.designation) && data.employeenames?.includes(isUserRoleAccess?.companyname)
                }
                if (data.type === "Department") {
                    return data.department?.includes(isUserRoleAccess?.department) && data.employeenames?.includes(isUserRoleAccess?.companyname)

                }
                if (data.type === "Employee") {
                    return data.company?.includes(isUserRoleAccess?.company) && data.branch?.includes(isUserRoleAccess?.branch) && data.unit?.includes(isUserRoleAccess?.unit) && data.team?.includes(isUserRoleAccess?.team) && data.employeenames?.includes(isUserRoleAccess?.companyname)
                }
            })
            ...
            const DuplicateRemoval = [...new Set(userTaskDesignation)]
            const userDuplicate = DuplicateRemoval?.filter(data => data === isUserRoleAccess?.companyname)
            ...
            let anstaskUserPanelSchedule = res_task?.data?.taskforuser?.length > 0 ? res_task?.data?.taskforuser?.filter(data => data?.username === isUserRoleAccess?.companyname && data?.taskdetails === "schedule" && data.taskstatus === "Assigned") : []
            let anstaskUserPanelNonScheduleFixed = res_task?.data?.taskforuser?.length > 0 ? res_task?.data?.taskforuser?.filter(data => data?.username === isUserRoleAccess?.companyname && data?.taskdetails === "nonschedule" && data?.taskdate === moment(new Date()).format("YYYY-MM-DD") && data?.frequency === "Fixed" && data.taskstatus === "Assigned").sort((a, b) => priorityOrder[a.priority] - priorityOrder[b.priority]) : [];
            let anstaskUserPanelNonScheduleAnyTime = res_task?.data?.taskforuser?.length > 0 ? res_task?.data?.taskforuser?.filter(data => data?.username === isUserRoleAccess?.companyname && data?.taskdetails === "nonschedule" && data?.taskdate === moment(new Date()).format("YYYY-MM-DD") && data?.frequency !== "Fixed" && data.taskstatus === "Assigned") : [];
            ...
            let final = [...finalSchedule, ...anstaskUserPanelNonScheduleFixed, ...anstaskUserPanelNonScheduleAnyTime]
            ...
            const todaysDate = new Date().toLocaleDateString('en-GB', {
                ...
                month: '2-digit',
                year: 'numeric'
            }).split('/').join('-'); // Custom format: 22-03-2024
            ...
            const sortedTasks = otherTasks?.concat(todayTasks);
            setQueueCheck(true);
    ...
    return (
        <Box>
            <Headtitle title={"USER TASK PANEL ALLOCATION"} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>User Task Panel Allocations</Typography>
            ...
            {!queueCheck ? (
                ...
                <>
                    {isUserRoleCompare?.includes("ltasksallocation") && (
                        <>
                            <Box sx={userStyle.dialogbox}>
                                {/* ******************************************************EXPORT Buttons****************************************************** */}
                                <Grid container spacing={2}>
                                    <Grid item xs={8}>
                                        <Typography sx={userStyle.SubHeaderText}>User Task Panel Allocations List</Typography>
                                    </Grid>
                                </Grid>
                                <br></br><br></br><br></br>
                                ...
                                <Grid container sx={{ justifyContent: "center" }}>
                                    <Grid>
                                        {isUserRoleCompare?.includes("exceltasksallocation") && (
                                            // <>
                                        ...
                                        {isUserRoleCompare?.includes("csvtasksallocation") && (
                                            // <>
                                        ...
                                        {isUserRoleCompare?.includes("printtasksallocation") && (
                                            <>
                                        ...
                                        {isUserRoleCompare?.includes("pdftasksallocation") && (
                                            // <>
                                        ...
                                        {isUserRoleCompare?.includes("imagetasksallocation") && (
...
export default UsersTaskAllocation;
