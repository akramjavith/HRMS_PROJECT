...
import { Box, Typography, Dialog, DialogContent, Select, ListItem, ListItemText, TextField, OutlinedInput, FormControl, MenuItem, DialogActions, Grid, Paper, Table, TableHead, Checkbox, IconButton, TableContainer, InputLabel, Button, TableBody, List, TextareaAutosize } from "@mui/material";
...
function ActiveinternList() {
  ...
  const handleCaptureImage = () => {
    ...
    html2canvas(clonedTable).then((canvas) => {
      ...
      link.href = dataURL;
      link.download = "Quick Action Intern List.png";
      link.click();
    });
  };
  ...
  const deleteChecklist = async () => {
    ...
    const searchItem = datasAvailedDB.find(
      (item) =>
        ...
        item.subsubpage === "Active Intern List"
    );
    try {
  ...
  async function fecthDBDatas() {
    try {
      ...
      let foundData = res?.data?.mychecklist.find((item) => item.commonid == postID && item.module == "Human Resources" &&
        item.submodule == "HR" &&
        item.mainpage == "Employee" &&
        item.subpage == "Intern details" &&
        item.subsubpage == "Active Intern List")
  ...
  const getCode = async (e, name, details) => {
    ...
    try {
      ...
      let searchItem = res1?.data?.mychecklist.find((item) => item.commonid == userDetails?._id && item.module == "Human Resources" && item.submodule == "HR" && item.mainpage == "Employee" && item.subpage == "Intern details" && item.subsubpage == "Active Intern List");
      ...
      await hierarchyCheck(res?.data?.suser?.companyname); fetchEmployeeList(res?.data?.suser?.companyname); UserCheckClientId(res?.data?.suser);
      setLoading(false);
    } catch (err) {
      setLoading(false);
      handleApiError(err, setShowAlert, handleClickOpenerr);
  ...
  const downloadPdf = (isfilter) => {
    ...
    const dataWithSerial = isfilter === "filtered" ?
      ...
      employees.map(row => ({
        ...
        contactpersonal: row.contactpersonal,
    ...
    doc.save("Quick Action Intern List.pdf");
  };

  // Excel
  const fileName = "Quick Action Intern List";
  let excelno = 1;
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Quick Action Intern List",
    pageStyle: "print",
  });
    ...
    if (isfilter === "filtered") {
      ...
      exportToCSV(
        employees.map((t, index) => ({
          ...
          Name: t.companyname,
          Department: t.department,
          DateOfBirth: moment(t.dob).format("DD-MM-YYYY"),
          PersonalNumber: t.contactpersonal,
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <Typography sx={userStyle.HeaderText}>Action Intern List</Typography>
      <br />
        ...
        <>
          <Box sx={userStyle.container}>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.SubHeaderText}>Quick Action Intern List</Typography>
              </Grid>
            </Grid>
            {!isBoarding ? (
              <>
              ...
              <>
                    ...
                    {isUserRoleCompare?.includes("csvactiveinternlist") && (
                      <>
                        <Button onClick={(e) => {
                          ...
                          // fetchUnassignedCandidatesArray()
                          setFormat("xl")
                ...
                {/* {isLoader ? ( */}
                {/* ****** Table start ****** */}
                <TableContainer component={Paper} ref={gridRef} id="excelcanvastable">
                      ...
                      {filteredData?.length > 0 ? (
                        filteredData?.map((row, index) => (
                          <StyledTableRow key={index}>
                            ...
                            <StyledTableCell>{moment(row.dob).format("DD-MM-YYYY")}</StyledTableCell>
                            <StyledTableCell>{row.contactpersonal}</StyledTableCell>
                            <StyledTableCell>{moment(row.doj).format("DD-MM-YYYY")}</StyledTableCell>
            ...
            {rowDataTable &&
              rowDataTable.map((row, index) => (
                <StyledTableRow key={index}>
                  ...
                  <StyledTableCell>{row.department}</StyledTableCell>
                  <StyledTableCell>{moment(row.dob).isValid() ? moment(row.dob).format("DD-MM-YYYY") : ''}</StyledTableCell>
                  <StyledTableCell>{row.contactpersonal}</StyledTableCell>
                  <StyledTableCell>{moment(row.doj).format("DD-MM-YYYY")}</StyledTableCell>
                  <StyledTableCell>{row.experience}</StyledTableCell>
                  <StyledTableCell>{row.reportingto}</StyledTableCell>
