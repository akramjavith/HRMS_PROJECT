...
function Duedatemaster() {
  ...
  const getOverallEditSectionOverallDelete = async (ids) => {
    ...
    } catch (err) { setIsDeleteOpencheckbox(true); handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  ...
  const fetchDuemasters = async () => {
    ...
    try {
      ...
      const answer = res_vendor?.data?.duedatemasters?.length > 0 ? res_vendor?.data?.duedatemasters?.map((item, index) => ({
