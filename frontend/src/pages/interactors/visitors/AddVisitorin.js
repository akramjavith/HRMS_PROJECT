...
import { Backdrop, Box, TableContainer, TableBody, Table, TableHead, Button, Checkbox, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, FormControlLabel, FormGroup, Grid, MenuItem, OutlinedInput, Paper, Select, TextareaAutosize, Typography } from '@mui/material';
...
function AddVisitorin() {
  ...
  const UploadWithDuplicate = (e, isUpload) => {
    if (isUpload === 'upload') {
  ...
  const sendRequest = async (type) => {
    ...
    try {
      ...
      const jsonData = {
        ...
        followupaction: visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument ? "" : String(path === 'interactor/addvisitorin' ? '' : vendor.followupaction),
        ...
        followuparray: [
          {
            ...
            followupaction: visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument ? "" : String(vendor.followupaction),
            followupdate: visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument ? "" : String(vendor.followupaction === 'Required' ? vendor.followupdate : ''),
            followuptime: visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument ? "" : String(vendor.followupaction === 'Required' ? vendor.followuptime : ''),
            visitorbadge: String(vendor.visitorbadge),
      ...
      let addVisitorProfileDetail = await axios.post(SERVICE.VISITORDETAILS_LOG_CREATE, {
        ...
        visitorcommonid: String(resdata),
        files: allUploadedFiles.concat(refImage, refImageDrag, capturedImages),
        addedby: [
          {
            name: String(isUserRoleAccess.companyname),
  ...
  const handleSubmit = async (e, type) => {
    ...
    if (checkContactEmailForpopup) {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if (path !== 'interactor/addvisitorin' && (vendor.followupaction === 'Please Select Follow Up Action' || vendor.followupaction === '')) {
  ...
  const handleRemarkChange = (index, newRemark) => {
    setUploadBillsRequestDocument(prevFiles =>
      prevFiles.map((file, i) =>
        i === index ? { ...file, remark: newRemark } : file
      )
    );
  };
  ...
  return (
    <Box>
      <Headtitle title={'ADD VISITORS IN'} />
      {/* ****** Header Content ****** */}

      <PageHeading title="Visitors In" modulename="Interactors" submodulename="Visitor" mainpagename="Visitor In" subpagename="" subsubpagename="" />
      <>
        {isUserRoleCompare?.includes('avisitorin') && (
              ...
              <Grid container spacing={2}>
                ...
                {visitorCheck === 'Existing Visitor' ? (
                  <>
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <Typography>&nbsp;</Typography>
                      <Button onClick={handlesubmitexistingvisitorCheck} sx={buttonStyles.buttonsubmit}>
                        Ok
                      </Button>
                      &nbsp; &nbsp;
                ...
                {!visitorsTypeOption?.filter((item) => item?.interactorstype === vendor.visitortype && item?.interactorspurpose?.includes(vendor.visitorpurpose))[0]?.requestdocument && vendor.followupaction === 'Required' && (
                  <>
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        <Typography>
