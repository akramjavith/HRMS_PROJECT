...
const HomeMinimum = () => {
  ...
  const { isUserRoleCompare, isUserRoleAccess,isAssignBranch,listPageAccessMode } = useContext(UserRoleAccessContext);
  ...
  const fetchEmployee = async () => {
    try {
      if (isUserRoleCompare?.includes('lminimumpointscalculation') && isUserRoleCompare?.includes('lminimumpoints')) {
        let [
          res_freq,
          res_vendor,
          //  res_employee,
          ...
          axios.get(SERVICE.MANAGESHORTAGEMASTER, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
          }),
  ...
  console.log(isUserRoleAccess.companyname,"companyname")
    ...
    try {
      ...
      if (isUserRoleCompare?.includes('lminimumpointscalculation') && isUserRoleCompare?.includes('lminimumpoints')) {
        let [res_applyleave, res_employee1] = await Promise.all([
          ...
          axios.post(SERVICE.USER_EXCEL_DATA_HOME_TEAM, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
        ...
        console.log(res_applyleave,res_employee1,"51")
if(res_employee1.data.users.length > 0){
         let response = await axios.post(SERVICE.MINIMUM_HOME_TEAM, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
             name: res_employee1.data.users.map((item) => item.companyname),
          });
        ...
        let result = response.data.finaluser.flatMap((item, index) => {
          ...
          const absentItems = response.data.finaluser?.filter((d) => d.clockinstatus === 'Absent' && item.empcode === d.empcode && d.clockin === '00:00:00' && d.clockout === '00:00:00');
...
console.log(result.length,"restul")
        const finalresult = [];
        ...
        const itemsWithSerialNumber = res_employee1.data.users?.map(async (item, index) => {
          try {
            ...
            let res_salary_rev = await axios.post(
              SERVICE.SALARYSLAB_REVENUE_BY_CODE,
              {
                code: processcodeexpvalue,
              },
              {
                headers: {
                  Authorization: `Bearer ${auth.APIToken}`,
            ...
            let findSalDetails = res_salary_rev.data.salSlabs.find((d) => d.company === item.company && d.branch === item.branch && d.salarycode === processcodeexpvalue);
            //shortageamount from shortage master
            let findShortage = manageshortagemasters.find((d) => d.department === item.department && differenceInMonthsexp >= Number(d.from) && differenceInMonthsexp <= Number(d.to));
            //revenue amount from revenue  master
            let findRevenueAllow = res_salary_rev.data.revenueAmount.find((d) => d.company === item.company && d.branch === item.branch && d.processcode === processcodeexpvalue);
            ...
            let grossValue =
              modevalue && modevalue.expmode == 'Manual'
                ...
                : '';
            let egvalue = findShortage ? Number(grossValue) + Number(findShortage.amount) : 0;
            let hfvalue = findRevenueAllow ? Number(egvalue) - Number(findRevenueAllow.amount) : 0;
            let i60value = findAcPointVal ? Number(hfvalue) / Number(findAcPointVal.multiplevalue) : 0;
            let j85value = findAcPointVal ? i60value * Number(findAcPointVal.dividevalue) : 0;
            ...
            return {
              ...
              month: monthsArray[currentMonth - 1],
              year: currentYear,
        ...
        <Grid item xs={12} md={6} sm={8}>
          ...
          >
                    ...
                    <Grid container sx={{ m: 1.5 }}>
                      ...
                      <Grid item md={6} lg={6} xs={6} sm={6}>
                        ...
                        <Tooltip title={exercise.companyname?.length > 10 ? exercise.companyname : ''}>
                          ...
                          >
                            {exercise.companyname.length > 10 ? `${exercise.companyname.substring(0, 10)}...` : exercise.companyname}
                          </Typography>
                        </Tooltip>
            ...
            <Grid container sx={{ justifyContent: 'flex-end', marginTop: 'auto' }}>
              <Link to="/myminimumpointsteam" target="_blank">
