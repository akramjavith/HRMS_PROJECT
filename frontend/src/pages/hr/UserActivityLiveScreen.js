...
import { Box, Button, Checkbox, Chip, Dialog, DialogActions, DialogContent, FormControl, Grid, IconButton, List, ListItem, ListItemText, MenuItem, OutlinedInput, Popover, Select, Table, TableBody, TableHead, TextField, Typography } from "@mui/material";
...
function UserActivityLiveScreen() {
  const [isHandleChange, setIsHandleChange] = useState(false);
  const [searchedString, setSearchedString] = useState("");

  let exportColumnNames = ["Employeename", "Employeecode", "Username", "Devicename", "Company", "Branch", "Unit", "Team", "Work Mode", "Department", "Macaddress", "Localip", "Timestamp", "Status", "Screen Sharing"];
  let exportRowValues = ["employeename", "employeecode", "username", "devicename", "company", "branch", "unit", "team", 'workmode', "department", "macaddress", "localip", "timestamp", "status", "live"];
  ...
  const customValueRendererEvent = (valueEventCat, _categoryname) => {
    return valueEventCat?.length ? valueEventCat.map(({ label }) => label)?.join(", ") : "Please Select Event Type";
  };
  ...
  const fetchDropDowns = async () => {
    try {
      ...
      let userName = req?.data?.useractivitylivescreen[0]?.usernames?.map((data) => ({ label: data, value: data }));
      let systemName = req?.data?.useractivitylivescreen[0]?.systems?.map((data) => ({ label: data, value: data }));
  ...
  const { isUserRoleAccess, isUserRoleCompare, allUsersData, allTeam, isAssignBranch, pageName, setPageName, buttonStyles, allUsersLimit } = useContext(UserRoleAccessContext);
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Employee Monitor Live Screen"),
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, "Employee Monitor Live Screen.png");
        })
        .catch((error) => {
  ...
  const handleEmployeesChange = (options) => {
    ...
    const updatedOptions = allWorkStationOpt.map((option) => ({
      ...option,
      disabled: maxSelections > 0 && options.length >= maxSelections && !options.find((selectedOption) => selectedOption.value === option.value),
    }));
  ...
  const customValueRendererEmployees = (valueWorkStation, _filteredWorkStation) => {
    return valueWorkStation.length ? valueWorkStation.map(({ label }) => label).join(", ") : <span style={{ color: "hsl(0, 0%, 20%)" }}>Select Secondary Work Station</span>;
  };
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Employee Monitor Live Screen",
    pageStyle: "print",
  });
  ...
  function openImage(id) {
    const imageUrl = `http://192.168.8.4:7002/api/livescreen/${id}`;
    window.open(imageUrl, "_blank"); // Open image in a new tab
  }

  const ActionRenderer = (params) => {
    const [live, setLive] = useState(params.data.screenshare); // Assume 'live' is the property that we toggle

    const handleToggle = async () => {
      const newStatus = !live; // Toggle the current status
      setLive(newStatus); // Update local state

      try {
        // Send request to update the live status in the backend
        let res = await axios.put(`${SERVICE.LIVE_SCREEN_UPDATE}/${params.data.id}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          live: Boolean(newStatus),
        });
        console.log("Live status updated successfully:", newStatus);
        await fetchEmployee();
      } catch (error) {
        console.error("Error updating live status:", error);
        setLive(live); // Revert the state if there's an error
      }
    };

    return (
      <Grid sx={{ display: "flex", alignItems: "center" }}>
        <Switch checked={live} onChange={handleToggle} inputProps={{ "aria-label": "controlled" }} />
      </Grid>
    );
  };
  const columnDataTable = [
...

    {
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibility.appstatus,
      headerClassName: "bold-header",
      pinned: "left",
      cellRenderer: (params) => (
        <div style={{ display: "flex", alignItems: "center" }}>
          <span
            style={{
              width: "10px",
              height: "10px",
              backgroundColor: params.data.appstatus ? "green" : "red",
              borderRadius: "50%",
              display: "inline-block",
              marginRight: "8px",
            }}
          ></span>
          <span>{params.data.appstatus ? "Online" : "Offline"}</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      flex: 0,
      width: 150,
      hide: !columnVisibility.live,
      headerClassName: "bold-header",
    },
    {
      field: "screenshare",
      headerName: "Share Screen",
      flex: 0,
      width: 150,
      hide: !columnVisibility.screenshare,
      headerClassName: "bold-header",
...

    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("vemployeemonitorlivescreen") && (
            <>
              <Button
              ...
              {params.data.screenshare && params.data.appstatus && (
                <Button
                  // sx={userStyle.buttonedit}
                  variant="text"
                  onClick={() => {
                    openImage(params.data.id);
                  }}
                >
                  VIEW
                </Button>
  ...
  const handleClearFilter = () => {
    setValueAppStatusCat([]);
    setSelectedOptionsAppStatus([]);
    setValueCompanyCat([]);
    ...
    setValueEmployeeCat([]);
    setSelectedOptionsEmployee([]);
    setValueEmp([]); setSelectedOptionsUsernames([]);
  ...
  const appStatusOption = [
    { label: "Online", value: "Online", bool: true },
    { label: "Offline", value: "Offline", bool: false },
  ];

  //App Status multiselect

  const [selectedOptionsAppStatus, setSelectedOptionsAppStatus] = useState(appStatusOption);
  let [valueAppStatusCat, setValueAppStatusCat] = useState([true, false]);

  const handleAppStatusChange = (options) => {
    setValueAppStatusCat(
      options.map((a, index) => {
        return a.bool;
      })
    );
    setSelectedOptionsAppStatus(options);
  };
  const customValueRendererAppStatus = (valueAppStatusCat, _categoryname) => {
    return valueAppStatusCat?.length ? valueAppStatusCat.map(({ label }) => label)?.join(", ") : "Please Select App Status";
  };
  ...
  const customValueRendererUsername = (valueUsernamesCat, _categoryname) => {
    return valueUsernamesCat?.length ? valueUsernamesCat.map(({ label }) => label)?.join(", ") : "Please Select Username";
  };
  ...
  const customValueRendererSystem = (valueSystemsCat, _categoryname) => {
    return valueSystemsCat?.length ? valueSystemsCat.map(({ label }) => label)?.join(", ") : "Please Select System";
  };
  ...
  const handleFilter = () => {
    if (filterState?.type === "Please Select Type" || filterState?.type === "") {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (checked && keyWordSearch?.trim() === "") {
      setPopupContentMalert("Please Enter any Keywords to Search!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (selectedOptionsAppStatus?.length === 0) {
      setPopupContentMalert("Please Select App Sttaus!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (filterState?.type === "Username" && selectedOptionsUsernames?.length === 0) {
      setPopupContentMalert("Please Select Username!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (filterState?.type === "Device Name" && selectedOptionsSystems?.length === 0) {
  ...
  const fetchEmployee = async () => {
    ...
    setBankdetail(true);
    setPageName(!pageName);
    const aggregationPipeline = ["Username", "Device Name"]?.includes(filterState.type)
      ? [
        {
          $match: {
            $and: [
              {
                appstatus: { $in: valueAppStatusCat },
              },
    ...
    try {
      let response = await axios.post(
        SERVICE.DYNAMIC_USERACTIVITY_LIVESCREEN_CONTROLLER,
        {
          aggregationPipeline,
        },
        {
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <Headtitle title={"EMPLOYEE MONITOR Live Screen"} />

      <PageHeading title="Manage Employee Monitor Live Screen" modulename="Human Resources" submodulename="HR" mainpagename="Employee" subpagename="Employee Status Details" subsubpagename="Employee Monitor Live Screen" />
      <br />
      {isUserRoleCompare?.includes("lemployeemonitorlivescreen") && (
        <>
          <Box sx={userStyle.selectcontainer}>
            <Grid container spacing={2}>
              <>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <Typography>
                    App Status<b style={{ color: "red" }}>*</b>
                  </Typography>
                  <FormControl size="small" fullWidth>
                    <MultiSelect
                      options={appStatusOption}
                      value={selectedOptionsAppStatus}
                      onChange={(e) => {
                        handleAppStatusChange(e);
                      }}
                      valueRenderer={customValueRendererAppStatus}
                      labelledBy="Please Select App Status"
                    />
                  </FormControl>
                </Grid>
      ...
      <>
        <Box sx={userStyle.container}>
          {/* ******************************************************EXPORT Buttons****************************************************** */}
          <Grid item xs={8}>
            <Typography sx={userStyle.importheadtext}>Employee Monitor Live Screen List</Typography>
          </Grid>
          <br />
          <Grid container spacing={2} style={userStyle.dataTablestyle}>
            ...
            >
              <Box>
                {isUserRoleCompare?.includes("excelemployeemonitorlivescreen") && (
                  <>
                    <Button
                      onClick={(e) => {
                ...
                {isUserRoleCompare?.includes("csvemployeemonitorlivescreen") && (
                  <>
                    <Button
                      onClick={(e) => {
                        setIsFilterOpen(true);
                ...
                {isUserRoleCompare?.includes("printemployeemonitorlivescreen") && (
                  <>
                ...
                {isUserRoleCompare?.includes("pdfemployeemonitorlivescreen") && (
                  <>
                    <Button
                      sx={userStyle.buttongrp}
                      onClick={() => {
                ...
                {isUserRoleCompare?.includes("imageemployeemonitorlivescreen") && (
            ...
            <Grid item md={2} xs={6} sm={6}>
              <AggregatedSearchBar columnDataTable={columnDataTable} setItems={setItems} addSerialNumber={addSerialNumber} setPage={setPage} maindatas={employees} setSearchedString={setSearchedString} searchQuery={searchQuery} setSearchQuery={setSearchQuery} paginated={false} totalDatas={employees} />{" "}
            </Grid>
          </Grid>
          <br />
      ...
      <Box>
        ...
        >
          <Box sx={userStyle.dialogbox}>
            <>
              <Typography sx={userStyle.SubHeaderText}>Edit Employee Monitor Live Screen Info</Typography>
              <br />
              <br />
              <Grid container spacing={2}>
                ...
                <Grid item md={6} sm={12} xs={12}>
                  <Grid container spacing={2}>
                    <Grid item md={6} sm={12} xs={12}>
                      <Typography sx={{ fontWeight: "bold" }}>Emp Code :</Typography>
                    </Grid>
                    <Grid item md={6} sm={12} xs={12}>
              ...
              <Grid container spacing={2}>
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput id="component-outlined" type="text" value={empaddform.company} readOnly />
                  </FormControl>
                </Grid>
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    <Typography>Branch</Typography>
                    <OutlinedInput id="component-outlined" type="text" value={empaddform.branch} readOnly />
                  </FormControl>
                </Grid>
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    <Typography>Unit</Typography>
                    <OutlinedInput id="component-outlined" type="text" value={empaddform.unit} readOnly />
                  </FormControl>
                </Grid>
              </Grid>
              <br /> <br /> <br />
      ...
      <ExportData
        ...
        itemsTwo={employees ?? []}
        filename={"Employee Monitor Live Screen"}
      ...
      <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="md" fullWidth={true} sx={{ marginTop: "50px" }}>
        <Box sx={{ padding: "20px 50px" }}>
          <>
            <Typography sx={userStyle.HeaderText}> View Employee Monitor Live Screen</Typography>
            <br /> <br />
            <Grid container spacing={2}>
              ...
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>{empaddform?.live}</Typography>
                </FormControl>
              </Grid>
              <Grid item md={6} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <div style={{ display: "flex", alignItems: "center" }}>
                    <span
                      style={{
                        width: "10px",
                        height: "10px",
                        backgroundColor: empaddform?.appstatus ? "green" : "red",
                        borderRadius: "50%",
                        display: "inline-block",
                        marginRight: "8px",
                      }}
                    ></span>
                    <Typography>{empaddform?.appstatus ? "Online" : "Offline"}</Typography>
                  </div>
                </FormControl>
              </Grid>
            </Grid>
...
export default UserActivityLiveScreen;
