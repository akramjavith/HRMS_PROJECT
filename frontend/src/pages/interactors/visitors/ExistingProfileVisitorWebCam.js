...
const ExistingProfileVisitorWebCam = ({ ExistingProfileVisitors, handleCloseerrpopCopy }) => {
        ...
        },
        {
            ...
            renderCell: (params) => (
                <Grid sx={{ display: "flex" }}>
                    {/* {params?.row?.status === "Visitor" &&
                        <> */}
                    <Button sx={userStyle.buttonedit} onClick={() => {
                        // setCopyVisitorData(params.row.alldata)
                        handleCloseerrpopCopy(params.row.alldata)
    ...
    const rowDataTable = filteredData.map((item, index) => {
        const { uploadedimage, ...filteredItem } = item; // Exclude uploadedimage
        ...
        return {
            ...
            branch: item.branch,
            unit: item.unit,
            alldata: filteredItem, // Store item without uploadedimage
            status: item.modelName === "Candidate" && (item?.lastInterviewRound?.roundstatus === "Interview Scheduled" || item?.lastInterviewRound?.roundstatus === "Completed")
                ? `${item?.modelName} - ${item?.overallstatus}`
                : item?.modelName,
    ...
    return (
        <Box>
            ...
            <Box>
                <Dialog
                    open={isErrorOpenpop}
                    onClose={handleCloseerrpopError}
                ...
                >
                    ...
                    <DialogActions>
                        ...
                        <Button
                            ...
                            onClick={handleCloseerrpopError}
                        >
                            Cancel
                        </Button>
                    </DialogActions>
...
export default ExistingProfileVisitorWebCam;