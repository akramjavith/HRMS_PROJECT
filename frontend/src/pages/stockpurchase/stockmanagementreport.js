...
import { Box, InputAdornment, Button, DialogTitle, TextareaAutosize, Checkbox, Dialog, DialogActions, DialogContent, FormControl, Grid, IconButton, List, ListItem, ListItemText, MenuItem, OutlinedInput, Popover, Select, Table, TableBody, TableHead, TextField, Typography } from '@mui/material';
...
function StockManagementReport() {
  ...
  let exportColumnViewUsage = ['Company', 'Branch', 'Unit', 'Floor', 'Area', 'Location', 'Team', 'Employee', 'Quantity', 'Date', 'Time', 'Description'];
  let exportRowValuesViewusage = ['usercompany', 'userbranch', 'userunit', 'userfloor', 'userarea', 'userlocation', 'userteam', 'useremployee', 'countquantity', 'usagedate', 'usagetime', 'description'];
  ...
  //Edit model...
  const handleClickOpenEditused = (data, company, branch, unit, floor, area, location, productname, usagecount, requestmode) => {
  ...
  //Edit model...
  const handleClickOpenEdit = (company, branch, unit, floor, area, location, productname, balancedcount, usedcount, requestmode) => {
    setIsEditOpen(true);
      ...
      cellRenderer: (params) => {
        ...
        return (
          <Button variant="outlined" size="small" sx={buttonStyles}>
            {params.data.handover === 'handover' ? 'HandOver' : params.data.handover === 'return' ? 'Return' : 'Usage Count'}
          </Button>
        );
      },
    },
  ...
  const fetchStock = async () => {
    ...
    let columnsnew1 = stockManagefilter.requestmode === 'Stock Material' ? columnDatatableStock : columnDatatable;
    ...
    try {
      ...
      if (stockManagefilter.requestmode === 'Asset Material') {
        ...
        setStock(res_project.data.stock?.map((item, index) => ({
          ...item,
          serialNumber: index + 1,
          item: item._id,
          addedby: moment(item.addedby[0]?.date).format('DD-MM-YYYY hh:mm:ss a'),
  ...
  return (
    <Box>
      ...
      <PageHeading title="Manage Stock Report" modulename="Asset" submodulename="Stock" mainpagename="Stock Report" subpagename="" subsubpagename="" />
          ...
          <Box sx={userStyle.selectcontainer}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>Add Stock Report</Typography>
                </Grid>
              </Grid>
              <br />
              <Grid container spacing={2}>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      ...
                      // }))}
                      options={accessbranch
                        ?.filter((comp) => stockManagefilter.companyto === comp.company)
                        ?.map((data) => ({
                          label: data.branch,
                          value: data.branch,
                        }))
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect options={statusOption} styles={colourStyles} value={selectedOptions} onChange={handleChangeOptions} valueRenderer={customValueRendererOptions} labelledBy="Please Select Options" />
                  </FormControl>
                </Grid>
              </Grid>
              <br />
      ...
      {isUserRoleCompare?.includes('lstockmanagementreport') && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>Stock Report List</Typography>
            </Grid>
