...
const HomeMinimum = () => {
    ...
    const fetchEmployee = async () => {
        ...
        try {
            ...
            let [res_freq, res_vendor, Res, dept] = await Promise.all([
            ...
            // let filteredMonthsets = dept?.data?.departmentdetails.filter((item) => item.year == selectedYear && item.monthname == selectedMonth);
    ...
    const handleFilter = async () => {
        ...
        try {
            ...
            console.log(res_employee1.data.users);
            ...
            fetchData(pageone).then(async () => {
                ...
                const itemsWithSerialNumber = res_employee1.data.users?.map(async (item, index) => {
                    try {
                        ...
                        return {
                            ...
                            unit: item.unit,
                            team: item.team,
                            findAcPointVal: findAcPointVal,
                            empcode: item.empcode,
                            findShortage: findShortage,
                ...
                let processcodes = itemsWithSerialNumber.map(d => d.processcode)
                let [res_employee, Revenue] = await Promise.all([


                    axios.post(SERVICE.SALARYSLAB_LIMITEDASSIGNBRANCH_HOME, {
                        assignbranch: accessbranchsalary
                    }, {
                        headers: {
                            Authorization: `Bearer ${auth.APIToken}`,
                        },
                        processcode: processcodes
                    }),

                    axios.post(SERVICE.REVENUEAMOUNTSLIMITED_HOME, {
                        headers: {
                            Authorization: `Bearer ${auth.APIToken}`,
                        },
                        processcode: processcodes
                ...
                let salSlabs = res_employee?.data?.salaryslab;
                let revenueAmount = Revenue?.data?.revenueamounts;


                const itemsWithSerialNumber1 = itemsWithSerialNumber.map(async (item, index) => {
                    try {
                        ...
                        //findsalary from salaryslab
                        let findSalDetails = salSlabs.find((d) => d.company === item.company && d.branch === item.branch && d.salarycode === item.processcodeexp);

                        //revenue amount from revenue  master
                        let findRevenueAllow = revenueAmount.find((d) => d.company === item.company && d.branch === item.branch && d.processcode === item.processcodeexp);


                        // GROSS VALUE
                        let grossValue = item.modevalue && item.modevalue.expmode == "Manual" ? item.modevalue.gross : findSalDetails ? (Number(findSalDetails.basic) + Number(findSalDetails.hra) + Number(findSalDetails.conveyance) + Number(findSalDetails.medicalallowance) + Number(findSalDetails.productionallowance) + Number(findSalDetails.otherallowance)) : ""
                        let egvalue = Number(grossValue) + (item.findShortage ? Number(item.findShortage.amount) : 0);

                        let hfvalue = egvalue - (findRevenueAllow ? Number(findRevenueAllow.amount) : 0);
                        let i60value = Number(hfvalue) / (item.findAcPointVal && Number(item.findAcPointVal.multiplevalue));
                        let j85value = i60value * (item.findAcPointVal && Number(item.findAcPointVal.dividevalue));
                        // let totalpaiddaysvalue = findTotalNoOfDays ? Number(findTotalNoOfDays.empshiftdays) - (Number(findTotalNoOfDays.weekoff) + Number(findTotalNoOfDays.holiday)) : 0
                        let totalpaiddaysvalue = findTotalNoOfDays ? (Number(findTotalNoOfDays.empshiftdays) - (Number(findTotalNoOfDays.weekoff))) : 0
                        return {
                            ...item,
                ...
                const results = await Promise.all(itemsWithSerialNumber1);
