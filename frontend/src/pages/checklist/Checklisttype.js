...
import { keywordsOptions, estimateopt, detailsOptions, informationOptions } from "../../components/Componentkeyword";
...
function ChecklistType() {
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, 'Checklist Type.png');
                });
            });
        }
    };
    ...
    const sendRequest = async () => {
        ...
        setPageName(!pageName)
        try {
            await Promise.all(mappedWithAll.map(async (item) => {
                let subprojectscreate = await axios.post(SERVICE.CHECKLISTTYPE_CREATE, {
                    headers: {
                        'Authorization': `Bearer ${auth.APIToken}`
                    },
    ...
    const handleSubmit = async (e) => {
        ...
        if (singleSelectValues.module === "" || singleSelectValues.module == "Please Select Module") {
            ...
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: 'orange' }} />
                    <p style={{ fontSize: '20px', fontWeight: 900 }}>{"Please Add Subsequent Details"}</p>
                </>
            );
            handleClickOpenerr();
        }
            ...
            setShowAlert(
                <>
                    ...
                    />
                    <p style={{ fontSize: "20px", fontWeight: 900 }}>
                        {"Please fill the Details for all the rows!"}
                    </p>
                </>
            );
            handleClickOpenerr();
            ...
            setShowAlert(
                <>
                    ...
                    />
                    <p style={{ fontSize: "20px", fontWeight: 900 }}>
                        {"Duplicate Datas Availed in the rows!"}
                    </p>
                </>
            );
            handleClickOpenerr();
    ...
    const editSubmit = (e) => {
        ...
        if (singleSelectValuesEdit.module === "" || singleSelectValuesEdit.module == "Please Select Module") {
            ...
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: 'orange' }} />
                    <p style={{ fontSize: '20px', fontWeight: 900 }}>{"Please Select Check List!"}</p>
                </>
            );
            handleClickOpenerr();
        }
            ...
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: 'orange' }} />
                    <p style={{ fontSize: '20px', fontWeight: 900 }}>{"Please Enter Details!"}</p>
                </>
            );
            handleClickOpenerr();
        }
            ...
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: 'orange' }} />
                    <p style={{ fontSize: '20px', fontWeight: 900 }}>{"Please Select Information!"}</p>
                </>
            );
            handleClickOpenerr();
        }
    ...
    const fetchChecklisttype = async () => {
        setPageName(!pageName)
        try {
            let res_status = await axios.post(SERVICE.GETALLCHECKLISTBYPAGINATION, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                page: Number(page),
