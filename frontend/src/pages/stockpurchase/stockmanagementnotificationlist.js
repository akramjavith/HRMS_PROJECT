...
function StockManagementNotification() {
  ...
  //Edit model...
  const handleClickOpenEditused = (data, company, branch, unit, floor, area, location, productname, requestmode, balancecount) => {
    fetchUsageAllList(data);
    setusedcountusage(data.usedcount);
    setviewusagecount(data.productname);
    setUsecompany(company);
    setusebranch(branch);
    setuseunit(unit);
    setusefloor(floor);
    setusearea(area);
    setuselocation(location);
    setusematerial(productname);
    setusermode(requestmode);
    setBalancedcount(balancecount);
  ...
  //return model...
  const handleClickOpenEditReturn = (company, branch, unit, floor, area, location, productname, balancedcount, handovercount, requestmode) => {
  ...
  const getCodeAssetReturnLog = async (company, branch, unit, floor, area, location, productname, requestmode, balancecount, usagecount, status) => {
    try {
      ...
      let reshandeeturn = await axios.post(SERVICE.STOCKPURCHASELIMITED_HAND_TODO_RETURN_NOTIFICATION, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        company: company,
  ...
  let exportColumnNames = ['Company', 'Branch', 'Unit', 'Floor', 'Area', 'Location', 'Material', 'Quantity'];
  let exportRowValues = ['ompany', 'branch', 'unit', 'floor', 'area', 'location', 'productname', 'countquantity'];
    ...
    try {
      ...
      let getfilter = single.reduce((acc, current) => {
        const existingItemIndex = acc.findIndex(
          (item) =>
            item.company === current.company && item.branch === current.branch && item.unit === current.unit && item.floor === current.floor && item.area === current.area && item.location === current.location && item.productname === current.productname && item.requestmode === current.requestmode
        );
      ...
      let resall = getfilter?.map((item, index) => {
        const returncount = res_countt.data.stock
          .filter((d) => d.handover === 'return' && d.company === item.company && d.branch === item.branch && d.unit === item.unit && d.floor === item.floor && d.area === item.area && d.location === item.location && d.productname === item.productname)
          .reduce((sum, item) => sum + Number(item.countquantity), 0);

        const usagecount = res_countusage.data.stock
          .filter((d) => d.handover === 'usagecount' && d.company === item.company && d.branch === item.branch && d.unit === item.unit && d.floor === item.floor && d.area === item.area && d.location === item.location && d.productname === item.productname)
          .reduce((sum, item) => sum + Number(item.countquantity), 0);
  ...
  const fetchUsageAllList = async (data) => {
    ...
    try {
      ...
      setUsageCountAlldialog(
        res_usagecount.data?.stock?.map((item, index) => ({
          ...item,
          id: item._id,
          serialNumber: index + 1,
          usagedate: moment(item?.usagedate).format('DD/MM/YYYY'),
        ...
        <Grid sx={{ display: 'flex' }}>
          ...
          <Grid sx={{ display: 'flex' }}>
            ...
            <Button
              ...
              size="small"
              onClick={(e) => {
                getCodeAssetReturnLog(params.data.company, params.data.branch, params.data.unit, params.data.floor, params.data.area, params.data.location, params.data.productname, params.data.requestmode, params.data.balancecount, params.data.usagecount, params.data.status);
              }}
            >
              Return Count
            </Button>
        ...
        <Grid sx={{ display: 'flex' }}>
          ...
          <Grid sx={{ display: 'flex', gap: '2px' }}>
            ...
            <Button
              ...
              size="small"
              onClick={() => handleClickOpenEditused(params.data, params.data.company, params.data.branch, params.data.unit, params.data.floor, params.data.area, params.data.location, params.data.productname, params.data.requestmode, params.data.balancecount)}
            >
              Usage Count
            </Button>
          </Grid>
  ...
  const [columnVisibilityviewusage, setColumnVisibilityviewusage] = useState(initialColumnVisibilityviewusage);
  ...
  const filteredDatasviewusage = itemsviewusage?.filter((item) => {
    return searchTermsviewusage.every((term) => Object.values(item).join(' ').toLowerCase().includes(term));
  });

  const filteredDataviewusage = filteredDatasviewusage.slice((pageviewusage - 1) * pageSizeviewusage, pageviewusage * pageSizeviewusage);
  ...
  const lastVisiblePageviewusage = Math.min(firstVisiblePageviewusage + visiblePagesviewusage - 1, totalPagesviewusage);
  ...
  // Function to filter columns based on search query
  const filteredColumnsviewusage = columnDataTableviewusage.filter((column) => column.headerName.toLowerCase().includes(searchQueryManageviewusage.toLowerCase()));
  // Manage Columns functionality
  ...
  const manageColumnsContentviewusage = (
    ...
    >
      ...
      <Box sx={{ position: 'relative', margin: '10px' }}>
        <TextField label="Find column" variant="standard" fullWidth value={searchQueryManageviewusage} onChange={(e) => setSearchQueryManageviewusage(e.target.value)} sx={{ marginBottom: 5, position: 'absolute' }} />
      </Box>
      <br />
      <br />
      <DialogContent sx={{ minWidth: 'auto', height: '200px', position: 'relative' }}>
        <List sx={{ overflow: 'auto', height: '100%' }}>
          {filteredColumnsviewusage.map((column) => (
            <ListItem key={column.field}>
              <ListItemText
                sx={{ display: 'flex' }}
                primary={<Switch sx={{ marginTop: '-5px' }} size="small" checked={columnVisibilityviewusage[column.field]} onChange={() => toggleColumnVisibilityviewusage(column.field)} />}
                secondary={column.field === 'checkbox' ? 'Checkbox' : column.headerName}
              />
            </ListItem>
          ))}
      ...
      <DialogActions>
        <Grid container>
          <Grid item md={4}>
            <Button variant="text" sx={{ textTransform: 'none' }} onClick={() => setColumnVisibilityviewusage(initialColumnVisibilityviewusage)}>
              Show All
            </Button>
          </Grid>
          <Grid item md={4}></Grid>
    ...
    try {
      ...
      if (viewmode === 'Asset Material') {
        ...
        let merge = getfilter.map((item) => {
          ...
          let findusagecount = singleusagecounttotal.find((d) => item.company === d.company && item.branch === d.branch && item.unit === d.unit && item.floor === d.floor && item.area === d.area && item.location === d.location && item.productname === d.productname);
          ...
          let matchUsageCountItems = singleusagecounttotal.length > 0 ? (findusagecount ? findusagecount.countquantity : 0) : 0;
          ...
          if (matchItems || matchHandItems || matchReturnItems || matchUsageCountItems) {
            ...
            return {
              ...
              //     Number(matchReturnItems)),
              usedcount: allused,
              balancedcount: Number(item.purchasecount) - Number(allused),
        ...
        let merge = getfilter.map((item) => {
          ...
          if (matchHandItems || matchReturnItems || matchUsageCountItems) {
            ...
            return {
              ...
              usedcount: allused,
              balancedcount: Number(item.purchasecountstock) - Number(allused),
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Stock Count" modulename="Production" submodulename="Manual Entry" mainpagename="Approve List" subpagename="" subsubpagename="" />
      ...
      <>
        <Box sx={userStyle.container}>
          ...
          <Grid container spacing={2} style={userStyle.dataTablestyle}>
            ...
            <Grid item md={2} xs={12} sm={12}>
              <Box>
                <AggregatedSearchBar columnDataTable={columnDataTable} setItems={setItems} addSerialNumber={addSerialNumber} setPage={setPage} maindatas={usageCountAll} setSearchedString={setSearchedString} searchQuery={searchQuery} setSearchQuery={setSearchQuery} />
              </Box>
            </Grid>
          </Grid>
          <br />
            ...
            <Grid container spacing={2}>
              ...
              <Table>
                <TableHead>
                  ...
                  <StyledTableCell sx={{ padding: '5px 10px !important' }}> {'Team'}</StyledTableCell>
                  ...
                  <StyledTableCell sx={{ padding: '5px 10px !important' }}> {'Overall Quantity'}</StyledTableCell>
                  <StyledTableCell sx={{ padding: '5px 10px !important' }}> {'Returned Quantity'}</StyledTableCell>
                      ...
                      <>
                        {editingIndexcheck === index ? (
                          <StyledTableRow key={index}>
                            ...
                            <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.usercompany}</StyledTableCell>
                            <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.userbranch}</StyledTableCell>
                            <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.userunit}</StyledTableCell>
                            <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.userteam}</StyledTableCell>
                            {/* <StyledTableCell
                            ...
                            <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.overallcountquantity}</StyledTableCell>
                            <StyledTableCell sx={{ padding: '5px 10px !important' }}> {item.returnedqty}</StyledTableCell>
      ...
      <Box>
        {/* Edit DIALOG */}
        <Dialog open={isEditOpenused} onClose={handleClickOpenEditused} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="lg" fullWidth={true} sx={{ marginTop: '95px' }}>
          <Box sx={{ padding: '20px 50px' }}>
            <>
              <Grid container spacing={2}>
                ...
                <Grid item md={6} xs={12} sm={12}>
                  <Typography sx={userStyle.HeaderText}>
                    <Typography>
                      Material : <b style={{ color: 'cornflowerblue' }}>{viewusagecount}</b>
                    </Typography>
                  </Typography>
                </Grid>
              </Grid>
              ...
              <Grid container spacing={2}>
                <Grid item md={6} xs={12} sm={12}>
                  <Button variant="contained" onClick={handlesubmitstockusaecount}>
                    {' '}
                    Update
                  </Button>
                </Grid>
          ...
          <Box sx={{ padding: '20px' }}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={8} md={8} sm={8}>
                <Typography sx={userStyle.importheadtext}>Usage Count List</Typography>
              </Grid>
              <Grid item xs={4} md={4} sm={4}>
                <Box sx={{ display: 'flex', justifyContent: 'end', width: '100%' }}>
                  <Button variant="contained" color="primary" onClick={handleCloseModEditused}>
                    {' '}
                    Back{' '}
                  </Button>
                </Box>
            ...
            <Popover id={idviewusage} open={isManageColumnsOpenviewusage} anchorEl={anchorElviewusage} onClose={handleCloseManageColumnsviewusage} anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }} transformOrigin={{ vertical: 'center', horizontal: 'right' }}>
      ...
      <Dialog open={isimgviewbilllist} onClose={handlecloseImgcodeviewbilllist} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="md" fullWidth>
        <DialogContent>
          <Typography variant="h6">Images</Typography>
          <Grid container>
      ...
      <Dialog open={openViewasset} onClose={handleViewOpenAsset} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" fullWidth={true} maxWidth="lg" sx={{ marginTop: '95px' }}>
        <Box sx={{ padding: '20px' }}>
          ...
          <Grid container spacing={2}>
            <Grid item xs={8} md={8} sm={8}>
              <Typography sx={userStyle.importheadtext}>View Return Material Log</Typography>
            </Grid>
            <Grid item xs={4} md={4} sm={4}>
              <Box sx={{ display: 'flex', justifyContent: 'end', width: '100%' }}>
      ...
      <Dialog open={openView} onClose={handleViewOpen} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" fullWidth={true} maxWidth="lg" sx={{ marginTop: '95px' }}>
        <Box sx={{ padding: '20px' }}>
          {/* ******************************************************EXPORT Buttons****************************************************** */}
          <Grid container spacing={2}>
            <Grid item xs={8} md={8} sm={8}>
