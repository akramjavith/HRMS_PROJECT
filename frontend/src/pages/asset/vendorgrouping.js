...
function VendorGrouping() {
  ...
  const handleClickOpenalert = async () => {
    try {
      ...
      if (selectedRows.length === 0) {
        ...
        setCheckScheduleBills(resexpense?.data?.schedulepaymentbills);
        ...
        let schedulebills = resexpense?.data?.schedulepaymentbills?.map(t => ({
          name: t.vendorgrouping,
          vendor: t.vendor
        }
        ));
        ...
        if (
          ...
          (resexpense?.data?.schedulepaymentbills).length > 0 ||
  ...
  const delaccountheadwithoutlink = async () => {
    try {
      let valfilter = [
        ...
        ...overalldeletecheck.schedulebills,
  ...
  const sendEditRequestOverall = async (
    expense, assetdetail, schedulepayment, schedulepaymentbills, stock, manualstockentry) => {
    try {
  ...
  const getLinkedLabelItem1 = (overalldeletecheck) => {
    ...
    schedulebills.forEach(item => labels.push(item.name));
  ...
  const getLinkedLabelItem = (overalldeletecheck) => {
    ...
    const uniqueLabels = [
      ...expense, ...schedule, ...schedulebills,
      ...assetdetail, ...stock,
