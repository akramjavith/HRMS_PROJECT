...
function TrainingForUserReport({ com }) {
    ...
    const handleExportXL = async (isfilter) => {
        if (isfilter === "filtered") {
            ...
            let res_task = await axios.post(SERVICE.ALL_TRAINIGFORUSER_REPORTS_OVERALL, {
                headers: {
    ...
    const downloadPdf = async (isfilter) => {
        ...
        if (isfilter === "overall") {
            ...
            overall = res_task?.data?.trainingforuser?.map(item => ({
        ...
        doc.save("User_Training_Panel-Report.pdf");
    };
    ...
    // const filteredDatas = filteredDatas?.slice((page - 1) * pageSize, page * pageSize);
    ...
    // Excel
    const fileName = "User_Training_Panel-Report";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "User_Training_Panel-Report",
        pageStyle: "print",
    });
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "User_Training_Panel-Report.png");
                });
            });
        }
    };
    ...
    const ListPageLoadDataOnprogress = async () => {
        try {
            setQueueCheck(false);
            let res_task = await axios.post(SERVICE.ALL_TRAINIGFORUSER_REPORTS, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                required: valueWeekly,
    ...
    return (
        <Box>
            ...
            <Box sx={userStyle.dialogbox}>
                <>
                    <Typography sx={userStyle.SubHeaderText}>Manage User Training Panel - Reports</Typography>
                    <br />
                    <br />
                    <br />
                    <Grid container spacing={2}>
                        <>
                            ...
                            <Grid item md={1} xs={12} sm={12}>
                                <Typography>Required<b style={{ color: "red" }}>*</b></Typography>
                            </Grid>
                            <Grid item md={3} xs={12} sm={12}>
