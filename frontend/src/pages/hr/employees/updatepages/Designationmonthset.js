...
function DesignationMonthSet() {
  ...
  const fetchDesignationSet = async () => {
    try {
      let res_status = await axios.post(SERVICE.GETDESIGNATIONMONTHSETBYPAGINATION, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        page: Number(page),
      ...
      const ans = res_status?.data?.designationmonthsets?.length > 0 ? res_status?.data?.result : []
  ...
  const handleExportXL = async (isfilter) => {
    ...
    if (isfilter !== "filtered") {
      try {
        const res = await axios.get(SERVICE.DESIGNATIONMONTHSET_ALL, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });

        overallDatas = res?.data?.designationmonthsets?.map((item) => ({
          ...item,
          fromdate: moment(item.fromdate).format("DD-MM-YYYY"),
          todate: moment(item.todate).format("DD-MM-YYYY"),
        }));
    ...
    exportToExcel(formatData(dataToExport), 'DesignationMonthSet');
  ...
  const downloadPdf = async (isfilter) => {
    ...
    doc.save("DesignationMonthSet.pdf");
    setLoading(false);
  };
  ...
  const filteredDatas = alldesignationSet?.filter((item) => {
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <Typography sx={userStyle.HeaderText}>Designation Month Set</Typography>
