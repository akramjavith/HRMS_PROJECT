...
function ListOrganizationDocument({ vendorAuto }) {
  ...
  //       }
  //     } else if (response?.data?.sdocument?.document?.map((item) => item?.preview)) {
  ...
  //       response?.data?.sdocument?.document?.map((item) => {
  //         if (!item) return false;
  //         const imageUrl = `${SERVICE.VEIW_ORGANIZATIONDOCUMENT_FILES}/${item.filename}`;
  ...
  const fileDataDownload = async (id) => { // Added id as parameter
    try {
      ...
      if (res?.data?.sdocument?.document?.length !== 0) {
        ...
        await Promise.all(res.data.sdocument.document.map(async (item) => {
          // You might want to use item.filename or similar instead of the parameter

          const response = await axios.get(
            `${BASE_URL}/api/organizationdocumentfiles/${item.filename}`,
            {
              responseType: "blob",
              headers: {
                Authorization: `Bearer ${auth.APIToken}`,
  ...
  const fetchAllApproveds = async () => {
    setPageName(!pageName)
    console.time("fetchAllApproveds")
    try {
      let res_queue = await axios.get(SERVICE.ORGANIZATIONDOCUMENT_REQUIRED_FIELD, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      ...
      setLoading(true);
      console.timeEnd("fetchAllApproveds")
