...
function AttendanceIndividualStatus() {
    ...
    const { isUserRoleCompare, isUserRoleAccess, isAssignBranch, allUsersLimit, alldepartment, allTeam, pageName, setPageName, buttonStyles, } = useContext(UserRoleAccessContext);
            ...
            if (isUserRoleAccess.role.includes("Manager") || isUserRoleAccess.role.includes("HiringManager") || isUserRoleAccess.role.includes("HR")) {
                ...
                async function sendBatchRequest(batch) {
                    try {
                        ...
                        // console.log(res?.data?.finaluser, 'resd')
                ...
                getAllResults().then(async (results) => {
                    ...
                    let outputdata = finalresult.map((item, index) => ({
                        ...item,
                    ...
                    // outputdata.forEach((item, index, array) => {
                    ...
                    //     }
                    // })
                    let fianlResult = outputdata.filter(data => data.finalDate >= filterUser.fromdate && data.finalDate <= filterUser.todate)?.map((item, index) => ({ ...item, serialNumber: index + 1, }))
                    setUserShifts(fianlResult);
                    setFilteredDataItems(fianlResult);
                    setLoader(false);
                    setSearchQueryAttInd("");
                    setTotalPagesAttInd(Math.ceil(fianlResult.length / pageSizeAttInd));
                }).catch(error => {
                    setLoader(true);
                ...
                getAllResults().then(async (results) => {
                    const finalresult = results.allResults?.filter((data) => {
                        return data.shift != "Week Off" && data.shift != "Not Allotted"
                    })
                    let outputdata = finalresult.map((item) => ({
                        ...item,
                        shift: item.changeshift ? item.changeshift : item.shift,
                        bookby: item.attendanceautostatus ? item.attendanceautostatus : getattendancestatus(item),
                    }))
    ...
    const sendRequestClockIn = async () => {
        setPageName(!pageName)
        // console.log(getAttIdClockIn, 'getAttIdClockIn')
        try {
        ...
        },
        {
            ...
            cellRenderer: (params) =>
                <Grid sx={{ display: "flex", alignItems: "center", }}>
                    {isUserRoleCompare?.includes("eoverallindividualstatus") && (
                        <>
    ...
    let exportColumnNamescrt = ["Company", "Branch", "Unit", "Department", "Emp Code", "Employee Name", "Role", "Book By", "IP Address", "Date", "Shift", "Leave Status", "Permission Status", "ClockIn", "ClockOut",]
    let exportRowValuescrt = ["company", "branch", "unit", "department", "empcode", "username", "role", "bookby", "ipaddress", "date", "shift", 'leavestatus', 'permissionstatus', "clockin", "clockout",]
    ...
    return (
        <Box>
            ...
            />
            {isUserRoleCompare?.includes("loverallindividualstatus") && (
                <>
