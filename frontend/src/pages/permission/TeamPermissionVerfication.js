...
import CompletedTeamPermissionVerification from './CompletedTeamPermissionVerification';
...
function TeamPermissionVerification() {
  ...
  const fetchApplyPermissions = async () => {
    ...
    try {
      let res_employee = await axios.post(SERVICE.PERMISSIONS_FILTERED_HIERARCHY_PAGE_BASED, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  // let exportColumnNamescrt = ["Employee Name", "Employee Id", "Date", "From Time", "Request Hours", "End Time", "Reason For Permission", "Status", "Applied Count", "Approved Count", "Rejected Count"]
  // let exportRowValuescrt = ["employeename", "employeeid", "date", "fromtime", "requesthours", "endtime", "reasonforpermission", "status", "appliedCount", "approvedCount", "rejectedCount"]
  ...
  const handleExportXL = async (isfilter) => {
    ...
    saveAs(new Blob([buffer]), `Team Permission Verification.${fileExtension}`);
    setIsFilterOpen(false);
  };
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Team Permission Verification" modulename="Leave&Permission" submodulename="Permission" mainpagename="Team Permission Verification" subpagename="" subsubpagename="" />
      {/* ****** Table Start ****** */}
      {isUserRoleCompare?.includes('lteampermissionverification') && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>Team Permission Verification List</Typography>
            </Grid>
          ...
          <Grid container spacing={2}>
            <Grid item md={12} xs={12} sm={12}>
             ...
             <Grid item md={6} xs={12} sm={6}>
                <Typography><b>Name:</b><b style={{color:"red"}}>{isInidvidualStatus?.employeename}</b></Typography>
              </Grid>
              <Grid item md={6} xs={12} sm={6}>
                <Typography><b>Date&TotalHours:</b><b style={{color:"red"}}>{`${isInidvidualStatus?.date}  ${isInidvidualStatus?.requesthours}`}</b></Typography>
              </Grid>
              <Grid item md={6} xs={12} sm={6}>
                <Typography><b>StartEndTime:</b><b style={{color:"red"}}>{`${isInidvidualStatus?.fromtime} to ${isInidvidualStatus?.endtime}`}</b></Typography>
              </Grid>
              <Grid item md={6} xs={12} sm={6}>
                <Typography><b>Type:</b><b style={{color:"red"}}>{isInidvidualStatus?.leavetype}</b></Typography>
              </Grid>
              <Grid item md={12} xs={12} sm={12}>
                <Typography><b>OverallHistory:</b><b style={{color:"red"}}>Applied:{isInidvidualStatus?.overAllappliedCount} / Approved:{isInidvidualStatus?.overAllapprovedCount} / Rejected:{isInidvidualStatus?.overAllrejectedCount}</b></Typography>
              </Grid>
              <Grid item md={12} xs={12} sm={12}>
                <Typography><b>CurrentMonthHistory:</b><b style={{color:"red"}}>Applied:{isInidvidualStatus?.monthlyappliedCount} / Approved:{isInidvidualStatus?.monthlyapprovedCount} / Rejected:{isInidvidualStatus?.monthlyrejectedCount}</b></Typography>
              </Grid>
