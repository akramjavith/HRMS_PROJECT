...
function MultistepForm() {
  ...
  const fetchHandlerEdit = async () => {
    try {
      ...
      setSelectedAttMode(isThere);
      setValueAttMode(
        response?.data?.suser?.attendancemode
          ? response?.data?.suser?.attendancemode.map((data) => data)
          : []
      );
      ...
      setEmployee({
        ...
        prod: true,
        profileimage:
          savedEmployee?.profileimage === "null" || !savedEmployee?.profileimage
            ? ""
  ...
  const handleDesignationChange = async (event) => {
    ...
    const groupname = alldesignation?.find(
