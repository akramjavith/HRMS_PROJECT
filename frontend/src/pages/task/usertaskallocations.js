...
function UsersTaskAllocation() {
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Task/Tasks Allocation"),
    ...
    const fetchAllRaisedTickets = async (date) => {
        ...
        try {
            ...
            if (date === "") {
                ...
                setQueueCheck(true)
                const [res_task, res_task_Schedule, res_task_Desig, res_shift_User] = await Promise.all([
                ...
                const DataFinal = [...prioritySchedule, ...answerUsermanual, ...answerUserSchedule, ...prioritynonschedule, ...priorityanswerUserNonSchedule];
                const answerFinal = DataFinal?.length > 0 ? DataFinal?.map((item, index) => ({
                ...
                setRaiseTicketList(answerFinal)
                setQueueCheck(false)
            }
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "User Task Allocation.png");
                })
                .catch((error) => {
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <PageHeading
                title="User Task Panel Allocations"
                modulename="Task"
                submodulename="Tasks Allocation"
                mainpagename=""
                subpagename=""
                subsubpagename=""
            />
                        ...
                        <Grid container spacing={2}>
                            ...
                            {<Grid item md={3} sm={12} xs={12}>
                                <Grid sx={{ display: "flex", justifyContent: "center", gap: "15px" }}>
                                    ...
                                    <Button sx={{ ...buttonStyles.btncancel, marginTop: "1px" }}
                                        onClick={() => { handleClear() }}
                                    >
                                        {" "}
                                        CLEAR
            ...
            <ExportData
                ...
                filename={"User Task Allocation List"}
