...
import { Box, Typography, OutlinedInput, TextField, Select, MenuItem, Dialog, DialogContent, DialogActions, FormControl, Grid, Button, Popover, IconButton, InputAdornment, Tooltip } from "@mui/material";
...
function TeamShiftVerification() {
    ...
    let listpageaccessby =
        listPageAccessMode?.find(
            (data) =>
                ...
                data.mainpagename === "Shift Details" &&
                data.subpagename === "Team Leave Verification" &&
                data.subsubpagename === ""
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            let res_emp = await axios.post(SERVICE.TEAM_SHIFT_HIERARCHY, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            ...
            if (
                // (   // res_emp?.data?.hierarchyfirstlevel > 0 &&
                //     res_emp?.data?.filteredoverallsectorall > 0 &&
                //     res_emp?.data?.resultAccessFilter?.length < 1 &&
                //     ["myallhierarchy", "allhierarchy"]?.includes(modeselection.value))

                // ||


                res_emp?.data?.resultAccessFilterHierarchy > 0 &&
                res_emp?.data?.resultAccessFilter?.length == 0 &&
                ["myallhierarchy", "allhierarchy"]?.includes(modeselection.value)
            ...
            const uniqueData = res_emp.data.resultAccessFilter
            // .filter(
            //     (item, index, self) =>
            //         index === self.findIndex((t) => t.id === item.id)
            // );

            setAllUsersAdjTable(uniqueData);


            setFilteredDataItemsAdjList(res_emp.data.resultAccessFilter);


            setSearchQuerAdjList("");
            setTotalPagesAdjList(Math.ceil(res_emp.data.resultAccessFilter.length / pageSizeAdjList))
            setAdjApply(false)

        } catch (err) { setAdjApply(false); handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
    };
    ...
    const columnDataTableAdjList = [
        ...
        { field: "companyname", headerName: "Name", flex: 0, width: 200, hide: !columnVisibilityAdjList.companyname, headerClassName: "bold-header", pinned: 'left', lockPinned: true, },
            ...
            cellRenderer: (params) => {
                return (
                    params.data.adjstatus === "Not Approved" ? (
                        <Grid sx={{ display: "flex" }}>
                            {isUserRoleCompare?.includes("eteamshiftverification") && (
                                <StyledTableCell>
                                    <Button
                                        variant="outlined"
                                        color="primary"
                        ...
                        <Grid sx={{ display: "flex" }}>
                            {isUserRoleCompare?.includes("dteamshiftverification") && (
    ...
    let exportRowValuescrt = ["branch", "unit", "team", "companyname", "empcode", "adjapplydate", "adjustmenttype", "request", "reason", "adjstatus"]

    // print...
    const componentRefAdjList = useRef();
    const handleprintAdjList = useReactToPrint({
        content: () => componentRefAdjList.current,
        documentTitle: "Team Shift Verification",
        pageStyle: "print",
    });
    ...
    const handleCaptureImageAdjList = () => {
        if (gridRefImageAdjList.current) {
            domtoimage.toBlob(gridRefImageAdjList.current)
                .then((blob) => {
                    saveAs(blob, "Team Shift Verification.png");
                })
                .catch((error) => {
    ...
    return (
        <Box>
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Team Shift Verification</Typography>
            ...
            <br />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lteamshiftverification") && (
                <>
                    <Box sx={userStyle.container}>
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}> Team Shift Verification </Typography>
                        </Grid>
                        {/* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid container style={userStyle.dataTablestyle}>
                            ...
                            <Grid item md={8} xs={12} sm={12} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                                <Box>
                                    {isUserRoleCompare?.includes("excelteamshiftverification") && (
                                        <>
                                            <Button onClick={(e) => {
                                    ...
                                    {isUserRoleCompare?.includes("csvteamshiftverification") && (
                                        <>
                                            <Button onClick={(e) => {
                                                setIsFilterOpenAdjList(true)
                                    ...
                                    {isUserRoleCompare?.includes("printteamshiftverification") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("pdfteamshiftverification") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("imageteamshiftverification") && (
            ...
            <ExportData
                ...
                filename={"Team Shift Verification"}
...
export default TeamShiftVerification;