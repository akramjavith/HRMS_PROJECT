...
const IndividualEmployeeLoginStatus = () => {
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "IndividualEmployeeLoginStatus.png");
                });
            });
        }
    };
    ...
    const fetchBranch = async () => {
        try {
            let res_branch = await axios.post(SERVICE.INDIVIDUAL_USER_LOGIN_STATUS, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                _id:isUserRoleAccess?._id,
            });
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("IndividualEmployeeLoginStatus.pdf");
    };

    // Excel
    const fileName = "IndividualEmployeeLoginStatus";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "IndividualEmployeeLoginStatus",
        pageStyle: "print",
    });
    ...
    return (
        <>
            <Headtitle title={"INDIVIDUAL EMPLOYEE LOGIN STATUS"} />
            <Typography sx={userStyle.HeaderText}>Individual Employee Login Status</Typography>
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lindividualloginstatus") && (
                <>
                    <Box sx={userStyle.container}>
                        {/* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Individual Employee Login Status List </Typography>
                        </Grid>
                        <br />
                        <Grid container spacing={2} style={userStyle.dataTablestyle}>
                            ...
                            <Grid item md={8} xs={12} sm={12} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                                <Box>
                                    {isUserRoleCompare?.includes("excelindividualloginstatus") && (
                                        <>
                                            <Button
                                                onClick={(e) => {
                                    ...
                                    {isUserRoleCompare?.includes("csvindividualloginstatus") && (
                                        <>
                                            <Button
                                                onClick={(e) => {
                                                    setIsFilterOpen(true);
                                    ...
                                    {isUserRoleCompare?.includes("printindividualloginstatus") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("pdfindividualloginstatus") && (
                                        <>
                                            <Button
                                                sx={userStyle.buttongrp}
                                                onClick={() => {
                                    ...
                                    {isUserRoleCompare?.includes("imageindividualloginstatus") && (
...
export default IndividualEmployeeLoginStatus;