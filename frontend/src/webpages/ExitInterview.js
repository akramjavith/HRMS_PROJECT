  ...
  const autoFillCredentials = async () => {
    try {
      let single_cand = await axios.get(
        `${SERVICE.NOTICEPERIODAPPLY_SINGLE}/${candidateid}`
      );
      setUserName(single_cand.data.snoticeperiodapply?.username);
      setPassword(single_cand.data.snoticeperiodapply?.password);
    } catch (err) {
