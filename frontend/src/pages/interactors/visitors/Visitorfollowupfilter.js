...
import AggridTable from "../../../components/AggridTableForFilter";
...
function VisitorFollowupFilter() {
  ...
  const { isUserRoleCompare, pageName, isUserRoleAccess, setPageName, buttonStyles, isAssignBranch } =
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("VisitorsFollowupFilter"),
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("vvisitorsfollowupfilter") && (
            <Link
              to={`/interactor/master/viewvisitors/${params.data.id}/followupfilter`}
  ...
  const filterData = () => {
    if (filterUser.fromdate === "") {
      ...
      const filtered = allData.filter((item) => {
        ...
        return (
          followupDate >= new Date(filterUser.fromdate) &&
          followupDate <= new Date(filterUser.todate)
        );
      });

      setVendormaster(filtered?.map((item, index) => {
        return {
          id: item._id,
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage.toBlob(gridRefTableImg.current)
        .then((blob) => {
          saveAs(blob, "Visitors Followup Filter.png");
        })
        .catch((error) => {
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Visitors Followup Filter",
    pageStyle: "print",
  });
  ...
  return (
    <Box>
      ...
      <br />
      {/* ****** Table Start ****** */}
      {isUserRoleCompare?.includes("lvisitorsfollowupfilter") && (
        <>
          <Box sx={userStyle.container}>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.SubHeaderText}>
                  Visitors Followup Filter List
                </Typography>
              </Grid>
            </Grid>
            <br />
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              >
                <Box>
                  ...
                  {isUserRoleCompare?.includes("csvvisitorsfollowupfilter") && (
                    <>
                      <Button
                        onClick={(e) => {
                          setIsFilterOpen(true);
                  ...
                  {isUserRoleCompare?.includes("pdfvisitorsfollowupfilter") && (
                    <>
                      <Button
                        sx={userStyle.buttongrp}
                        onClick={() => {
            ...
            <Grid container spacing={1} >
              ...
              <Grid item md={1} xs={12} sm={12}>
                <Typography>&nbsp;</Typography>
                <Button
                  sx={buttonStyles.btncancel} onClick={handleClear}
                  variant="contained"
      ...
      <ExportData
        ...
        filename={"Visitors Followup Filter"}
