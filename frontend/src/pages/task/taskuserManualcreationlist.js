...
function TaskUserManualCreationList({ com }) {
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("User_Manual_Creation_List.pdf");
    };
        ...
        },
        {
            ...
            renderCell: (params) => (
                <Grid sx={{ display: "flex" }}>
                    <Typography sx={{ color: params?.row?.taskstatus === "Progress" ? "green" : params?.row?.taskstatus === "Pending" ? "red" : "blue" }}>{params?.row?.taskstatus}</Typography>
                </Grid>
            ),
        },
        {
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "User_Manual_Creation_List.png");
                });
            });
        }
    };
    ...
    const ListPageLoadDataOnprogress = async () => {
        try {
            ...
            let anstaskUserPanel = res_task?.data?.taskforuser?.length > 0 ? res_task?.data?.taskforuser?.filter(data => data?.username
                === isUserRoleAccess?.companyname && data.taskdetails === "Manual") : []
                                        ...
                                        {filteredData?.length > 0 ? (
                                            filteredData?.map((row, index) => (
                                                <StyledTableRow key={index}>
                                                    ...
                                                    <StyledTableCell>{row.taskdate}</StyledTableCell>
