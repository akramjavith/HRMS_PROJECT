...
function FloorLogList({ boardinglogs, userID }) {
  ...
  const rowDataArray = async () => {
    ...
    try {
      ...
      const getresarr = getarr.filter((data, index) => {
        return data.ischangefloor === true;
      });
  ...
  // Excel
  const fileName = "Floor Log List";

  // print...
  const componentRef = useRef();
  const handleprint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: "Floor Log List",
    pageStyle: "print",
  });
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Floor Log List.png");
        });
      });
    }
  };
