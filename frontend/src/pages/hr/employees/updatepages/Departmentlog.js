...
function DepartmentLog() {
  ...
  const sendEditRequest = async () => {
    try {
      ...
      setDepartmentLog({ ...departmentlog, department: "Select Department" });
      setShowAlert(
        <>
  ...
  const editSubmit = (e) => {
    e.preventDefault();
    if (departmentlog.department === "Select Department") {
      setShowAlert(
        <>
          <ErrorOutlineOutlinedIcon
            sx={{ fontSize: "100px", color: "orange" }}
  ...
  const fetchDesignationlog = async () => {
    ...
    } catch (err) {setDesignationlogcheck(true);handleApiError(err, setShowAlert, handleClickOpenerr);}
  };
      ...
      renderCell: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("vdepartmentlog") && (
            <Link
              to={`/updatepages/departmentloglist/${params.row.id}`}
              target="_blank"
