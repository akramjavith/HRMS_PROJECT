...
const Home = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const isCurrentTimeInRangeNew = (start, end, from) => {
        const now = moment(serverTime?.currentNewDate);
        let startTime = moment(`${currentServerDate} ${start}`, 'YYYY-MM-DD hh:mmA').set({
          year: now.year(),
          month: now.month(),
          date: now.date(),
        });
        let endTime = moment(`${currentServerDate} ${end}`, 'YYYY-MM-DD hh:mmA').set({
          year: now.year(),
          month: now.month(),
          date: now.date(),
        });
  ...
  const ClockInButton = () => {
    ...
    const handleClockIn = async () => {
      try {
        const serverTime = await getCurrentServerTime();
        let shiftMidpoint = await getCenterTimeAmPm(currentUserShiftName?.shift)
        const midpointMinutes = await convertToMinutes(shiftMidpoint);
        const serverMinutes = await convertToMinutes(serverTime?.currenttoLocaleTimeString);
  ...
  const fetchPendingTaskCount = async () => {
    try {
      ...
      const [response, taskDateCount] = await Promise.all([
        ...
        axios.get(
          `${SERVICE.PENDING_TASK_COUNT_USERNAME}/?username=${isUserRoleAccess?.companyname}`,
          {
            headers: { Authorization: `Bearer ${auth.APIToken}` },
          }
        )
  ...
  const ClockOutButton = () => {
    ...
    const handleClockOut = async () => {
      try {
        ...
        else if (calculatedTime?.tasklimitcount && Number(calculatedTime?.tasklimitcount) !== 0 &&
          (pendingTaskCount !== 0 &&
            pendingTaskCount < Number(calculatedTime?.tasklimitcount)) &&
          !warningShowed
        ) {
          setAttendanceTaskStatus("count")
          handleClickOpenWarningClockout();
        }
        else if (calculatedTime?.tasklimitcountinday && Number(calculatedTime?.tasklimitcountinday) !== 0 &&

          (attendanceDaysTaskCount !== 0 &&
            attendanceDaysTaskCount < Number(calculatedTime?.tasklimitcountinday)) &&
          !warningShowed
        ) {
          setAttendanceTaskStatus("days")
          handleClickOpenWarningClockout();
    ...
    <>
      ...
      <Dialog open={openRestrictClockout} onClose={handleClickCloseRestrictClockout} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          ...
          >
            <b>
              You have more than {attendanceTaskStatus === "days" ? `${calculatedTime?.tasklimitcountinday}  days of` : calculatedTime?.tasklimitcount} pending tasks. Please complete the tasks
              and clock out.
            </b>          </DialogContentText>
        </DialogContent>
        <DialogActions>
