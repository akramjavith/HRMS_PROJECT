...
function QueueTypeUnassignedReport() {
  ...
  let exportColumnNames = ['Type', 'Project', 'Category', 'SubCategory'];

  let exportRowValues = ['type', 'project', 'categoryname', 'name'];
  //    today date fetching
  ...
  const fetchBatchFilter = async () => {
    ...
    setLoaderList(true);
    try {
      let res_employee = await axios.post(SERVICE.QUEUETYPE_TYPE_UNASSIGNED, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        type: selectedTypeFrom,
        projectvendor: selectedProject.map((d) => d.value),
        category: selectedCategory.map((d) => d.value),
        fromdate: fromdate,
        todate: todate,
      });
  ...
  const fetchAllCategory = async (e) => {
    try {
      let res_module = await axios.post(SERVICE.CATEGORYPROD_LIMITED_REPORT_MULTI, {
        ...
        projectvendor: e.map((item) => item.value),
      });
  ...
  const fetchAllOrate = async (project, category) => {
    setPageName(!pageName);
    try {
      let res_freq = await axios.post(SERVICE.UNITRATE_PRODUCTION_ORATE_CATEGORY_MULTI, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        project: project,
      ...
      const branchdata = [
        ...branches
          .map((d) => ({
            ...d,
            label: selectedTypeFrom === 'Other task queues' ? (['0', '0.000', '', '0.00'].includes(d.mrate) ? '0' : d.mrate) : ['0', '0.000', '', '0.00'].includes(d.orate) ? '0' : d.orate,
            value: selectedTypeFrom === 'Other task queues' ? (['0', '0.000', '', '0.00'].includes(d.mrate) ? '0' : d.mrate) : ['0', '0.000', '', '0.00'].includes(d.orate) ? '0' : d.orate,
          }))
  ...
  const handleCategory = (options) => {
    ...
    fetchAllOrate(
      selectedProject.map((a) => a.value),
      selectedValues
    );
    setSelectedOrate([]);
  };
  const customValueRendererCategory = (valueCate, _employeename) => {
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Queue Type Unassigned Report" modulename="Production" submodulename="SetUp" mainpagename="Queue Type Unassigned Report" subpagename="" subsubpagename="" />
      <>
            ...
            <Grid container spacing={2}>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Selects options={typeMaster} value={{ label: selectedTypeFrom, value: selectedTypeFrom }} onChange={handleTypeChangeFrom} />
                </FormControl>
              </Grid>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  /> */}
                  <MultiSelect options={vendors} value={selectedProject} onChange={handleProject} valueRenderer={customValueRendererProject} labelledBy="Please Select Project" />
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <MultiSelect options={categoryOpt} value={selectedCategory} onChange={handleCategory} valueRenderer={customValueRendererCategory} labelledBy="Please Select Category" />
                </FormControl>
              </Grid>
              {/* <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
