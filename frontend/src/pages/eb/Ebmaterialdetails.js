...
function EBMaterialUsageDetails() {
  ...
  const delRound = async () => {
    ...
    try {
      if (Roundsid) {
        ...
        await fetchRoundmaster("Filtered");
        handleCloseMod();
        setSelectedRows([]);
        setPage(1);
      }
  ...
  const fetchRoundmaster = async (e) => {
    setEbmaterialusagecheck(true);
    setRoundmasters([]);
    try {
      if (e === "Filtered") {
        let res_vendor = await axios.post(SERVICE.EBMATERIALDETAILS_FILTER, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
