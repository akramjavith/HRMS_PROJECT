...
function Companydomain() {
  ...
  const rowData = async (id, name) => {
    ...
    try {
      let res = await axios.get(`${SERVICE.COMPANYDOMAIN_SINGLE}/${id}`, {
        headers: {
  ...
  const handleSubmit = (e) => {
    ...
    function isValidURL(assignedname) {
      try {

        const regex = /^([a-zA-Z0-9-]+\.)*[a-zA-Z0-9-]+\.(com|in|org)$/;
        
    
    const match = assignedname.match(regex);
      if(match){
        return true;}
      } catch (error) {
        return false;
    ...
    if (assignedBy.company === "") {
      ...
      handleClickOpenPopupMalert();
        ...
        setPopupContentMalert("Please Enter a Valid URL!");
        setPopupSeverityMalert("info");
        handleClickOpenPopupMalert();
  ...
  const handleClear = (e) => {
    e.preventDefault();
    setAssignedBy({ assignedname: "",company:"" });
  ...
  const editSubmit = async (e) => {
    ...
    const validurl = isValidURL(assignedByEdit.assignedname)
  ...
  const fetchSourceAll = async () => {
    ...
    try {
      ...
      return res_meet?.data?.companydomainn.filter(
  ...
  const fetchEmployee = async () => {
    ...
    try {
      ...
      const ans =
        res_employee?.data?.companydomainn    
      setAssignedby(ans);
