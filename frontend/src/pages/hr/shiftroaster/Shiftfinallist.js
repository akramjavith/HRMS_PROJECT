...
function ShiftFinalListTable({ allUsersShiftFinal, allFinalAdj }) {
    ...
    //     let filteredUserData = allUsersShiftFinal.flat()?.filter((data) => valueDep.includes(data.department) || valueEmp.includes(data.companyname)).map((item, index) => ({ ...item, serialNumber: index + 1 }));
    //     setItemsFinalAdj(filteredUserData);
    // }
    ...
    const filteredDatasFinalAdj = allUsersShiftFinal?.filter((item) => {
    ...
    const rowDataTableFinalAdj = filteredDataFinalAdj?.map((item, index) => {
        return {
            ...
            //     const matchingItemAllot = filteredRowData.find(item => formatDate(item.date) == column.formattedDate);
            //     const matchingDoubleShiftItem = filteredRowData.find(item => item.todate === column.formattedDate);
            //     const filterBoardingLog = item.boardingLog?.filter((item) => {
            //         // return item.logcreation === "user" || item.logcreation === "shift";
            //         return item;
            //     });
    ...
    //                     return `${column.depstatus === undefined ? '' : column.depstatus} ${column.shiftlabel === undefined ? '' : column.shiftlabel}`
    //                 }),
    //             ];
    //         });
    ...
    const handleExportXLFinalAdj = (isfilter) => {
        ...
        if (isfilter === "filtered") {
            ...
            data = allUsersShiftFinal.map((row, index) => {
                return [
                    row.serialNumber,
                    row.empcode,
                    row.companyname,
    ...
    return (
        <Box>
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Shift List</Typography> <br />
                            ...
                            <>
                                <Box style={{ width: "100%", overflowY: "hidden", }}>
                                    <StyledDataGrid onClipboardCopy={(copiedString) => setCopiedDataFinalAdj(copiedString)} rows={rowsWithCheckboxesFinalAdj} columns={columnDataTableFinalAdj.filter((column) => columnVisibilityFinalAdj[column.field])} onSelectionModelChange={handleSelectionChange} selectionModel={selectedRowsFinalAdj} autoHeight={true} ref={gridRefFinalAdj} id="settableexcel"
                                        ...
                                        hideFooter getRowClassName={getRowClassName}
                                ...
                                <TableRow key={index}>
                                    ...
                                    {/* {daysArray && (
                                        daysArray.map((column, index) => {
                                            ...
                                            // const isWeekOffWithAdjustment = isWeekOff && matchingItem;
                                            // const isWeekOffWithManual = isWeekOff && matchingItemAllot;
                                            const isWeekOff = filterBoardingLog > 0 ? filterBoardingLog[filterBoardingLog.length - 1].weekoff?.includes(column.dayName) : false;
                                            ...
                                            return (
                                                <React.Fragment key={index}>
                                                    <TableCell>
                                                        <Typography variant="body2" sx={{ fontSize: '8px' }}>
                                                            {getShiftForDateFinal(column, matchingItem, matchingItemAllot, isWeekOffWithAdjustment, isWeekOffWithManual, row.shifttiming, filterBoardingLog, isWeekOff, matchingDoubleShiftItem, row.department)}
                                                        </Typography>
                                                    </TableCell>
                                                </React.Fragment>
                                            );
