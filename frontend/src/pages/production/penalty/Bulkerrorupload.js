...
function BulkErrorUpload() {
  ...
  const fetchClientUserIDFilter = async (proj) => {
    ...
    } catch (err) { handleApiError(err, console.log(err, "errr1"), setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  ...
  const addSerialNumberviewAll = (datas) => {
    ...
    const itemsWithSerialNumber = datas?.map(
      (item, index) => ({
        ...
        date: item.mode == "Bulkupload" ? moment(item?.dateformatted).format("DD-MM-YYYY") : moment(item?.date).format("DD-MM-YYYY")
      })
    );
  ...
  const handleClickOpenalert = async () => {
    ...
    if (selectedRowsBulkerror.length === 0) {
      ...
      if (resPayrun.data.count > 0 && resPayrun.data.notIncluded.length == 0) {
        ...
        let selectedRowsfiltered = selectedRowsBulkerror.filter((item) => resPayrun.data.notIncluded.some((d) => item.olddate == d));
  ...
  const fetchClientUserID = async (e) => {
    ...
    try {
      ...
      const filterProjBased = res_freq?.data?.clientuserid.filter((item) => item.projectvendor === e)
      const loginIdOpt = [...filterProjBased.map((d) => ({
        ...d,
        label: d.userid,
        value: d.userid,
  ...
  const fetchTargetPointsData1 = async () => {
    ...
    try {
      ...
      setTargetPointsFilenameArray(uniqueArray.map((row, index) => ({
        ...
        createddate: row.addedby ? moment(row.addedby[0]?.date).format("DD-MM-YYYY hh:mm:ss a") : "",
  ...
  const handleSubmit = async (e) => {
    ...
    let Respenalty = await axios.post(SERVICE.PENALTYERRORUPLOADS_DUPLICATE_CHECK_WITH_BULKERRORUPLOAD, {
      headers: {
        Authorization: `Bearer ${auth.APIToken}`,
      },
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      setLoader(false)
      setIsBtn(false)
    } catch (err) { setLoader(false); setIsBtn(false); handleApiError(err, console.log(err, "errr1"), setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  const handleClear = (e) => {
    e.preventDefault();
    setFileUploadName("");
  ...
  const deleteSingleList = async () => {
    ...
    try {
      ...
      handleClickOpenPopup();
      setLoader(false)
    } catch (err) { console.log(err, "delerror"); setLoader(false); handleApiError(err, console.log(err, "errr1"), setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  ...
  const deleteSingleBulkdataDelete = async (data) => {
    ...
    } catch (err) { console.log(err, "delerror01"); setLoader(false); handleApiError(err, console.log(err, "errr1"), setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  ...
  const sendEditRequest = async () => {
    ...
    try {
      ...
      handleClickOpenPopup();
      setLoader(false)
    } catch (err) { setLoader(false); handleApiError(err, console.log(err, "errr1"), setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  ...
  const getviewCodeall = async (filename) => {
    ...
    } catch (err) { handleApiError(err, console.log(err, "errr1"), setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); } finally {
  ...
  const deleteFilenameList = async () => {
    ...
    } catch (err) { console.log(err, "filename"); setLoader(false); handleApiError(err, console.log(err, "errr1"), setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  // Manage Columns
    ...
    if (penaltyErrorUpload.fromdate === "") {
      ...
      promise.then(async (d) => {
        ...
        if (missingColumns.length > 0) {
          ...
          let Respenaltyupdate = await axios.post(SERVICE.PENALTYERRORUPLOADS_BY_DATE, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
            ...
            date: d.map(item => (formatToDate(item["Audit Date"], `${yeardrop}${symboldrop}${monthdrop}${symboldrop}${datedrop}`))),
          });

          // const isNameMatch = targetPoints?.some((item) =>
          const isNameMatchbulkDuplicate = Respenaltyupdate.data.penaltyerroruploadpoints;
          console.log(isNameMatchbulkDuplicate, d, "isNameMatchbulkDuplicate")
          ...
          let newdataMatcheddata = isNameMatchbulkDuplicate.map(tp => {
            ...
            const Match = d.find((item) => {
              console.log(
                ...
                tp.date == formatToDate(item["Audit Date"], `${yeardrop}${symboldrop}${monthdrop}${symboldrop}${datedrop}`),
          ...
          let Respenalty = await axios.post(SERVICE.PENALTYERRORUPLOADS_DUPLICATE_CHECK_WITH_BULKERRORUPLOAD_FILE, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
          ...
          const getBulkKey = (item) =>
            `${item["Project Vendor"]}$${item["Process"]}$${item["Login Id"]}$${formatToDate(item["Audit Date"], `${yeardrop}${symboldrop}${monthdrop}${symboldrop}${datedrop}`)}$${item["File Name"]}$${item["Document Number"]}$${item["Document Type"]}$${item["Field Name"]}$${item["Line"]}$${item["Error Value"]}$${item["Correct Value"]}`;
  ...
  const sendJSON = async () => {
    ...
    if (selectedSheet === "Please Select Sheet") {
      ...
      let batchRequestBodyPeantly = penaltyEditUpdate.map(d => ({ id: d._id, link: d.link, doclink: d.doclink }))
  ...
  const fetchFilteredDatas = async () => {
    ...
    try {
      ...
      setSelectedRowsBulkerror([]);
      setFilterList(subprojectscreate?.data?.bulkerroruploadpoints.map((item, index) => ({
        ...item,
        serialNumber: index + 1,
        olddate: item.mode == "Bulkupload" ? item.dateformatted : item.date,
        date: item.mode == "Bulkupload" ? moment(item.dateformatted).format("DD-MM-YYYY") : moment(item.date).format("DD-MM-YYYY")
      ...
      setLoaderList(false);
      handleApiError(err, console.log(err, "errr1"), setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
              ...
              <Grid item md={4} xs={12} sm={6}>
                <Grid container>
                  <Grid item md={5} xs={12} sm={6}>
                    <Typography> Project Vendors<b style={{ color: "red" }}>*</b></Typography>
                  </Grid>
