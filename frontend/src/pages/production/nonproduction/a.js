  ...
  const handleConfirmApprove = async () => {
    ...
    try {
      ...
      let res = await axios.put(`${SERVICE.PENALTYCLIENTERROR_SINGLE}/${rowId}`, {
        ...
        approvedcreatedAt: new Date(time),
      });
      await filteredErrorData();
      setPopupContent('Updated Successfully');
      setPopupSeverity('success');
  ...
  return (
    <Grid container>
      <Grid item xs={12} sm={3} md={3}>
        <Button variant="contained" color="primary" size="small" sx={{ textTransform: 'capitalize' }} onClick={handleApproveClick}>
          Approve
        </Button>
      </Grid>
      ...
      <Grid item xs={12} sm={3} md={3}>
        <Button variant="contained" color="error" size="small" sx={{ textTransform: 'capitalize' }} onClick={handleRejectClick}>
          Reject
        </Button>
      </Grid>
      <Grid item xs={12} sm={9} md={9}>
        {showRejectReason && (
          <Button sx={{ textTransform: 'capitalize' }} variant="contained" color="error" size="small" onClick={handleSaveClick}>
            Save Reject
          </Button>
        )}
      </Grid>
      <DeleteConfirmation open={showConfirmDialog} onClose={handleCancelApprove} onConfirm={handleConfirmApprove} title="Are you sure?" confirmButtonText="Yes" cancelButtonText="Cancel" />
    </Grid>
  );
};
...
function ClientErrorMonthAmount() {
  ...
  const filteredErrorData = async () => {
    ...
    try {
      let res_vendor = await axios.post(
        SERVICE.PENALTYCLIENTERROR_DATE_FILTER,
        {
      ...
      cellRenderer: (params) => (
        <Grid>
          {params.data.errorstatus === 'Approved' ? (
            <Button variant="contained" color="success" size="small" sx={{ textTransform: 'capitalize', cursor: 'default' }}>
              {params.data.errorstatus}
            </Button>
          ) : (
            <Grid sx={{ display: 'flex', justifyContent: 'center' }}>
              <ActionCell
                ...
                auth={auth}
                filteredErrorData={filteredErrorData}
              />
            </Grid>
          )}
        </Grid>
  ...
  return (
    <Box>
      ...
      <PageHeading title="Client Error Checklist" modulename="Quality" submodulename="Penalty" mainpagename="Process Penalty" subpagename="Client Error CheckList" subsubpagename="" />
      ...
      {/* Manage Column */}
      <Popover id={idtwo} open={isManageColumnsOpentwo} anchorEl={anchorEltwo} onClose={handleCloseManageColumnstwo} anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }}>
        <ManageColumnsContent
