...
function QueueTypeMaster() {
    ...
    let exportColumnNames = ["Category", "Subcategory", "Type", "O-Rate", "New Rate"];
    let exportRowValues = ["category", "subcategory", "type", "orate", "newrate"];
    ...
    const sendRequest = async () => {
        ...
        try {
            ...
            // selectedSubCategoryValue.map(item => {
            //     let grpcreate = axios.post(SERVICE.PRODUCTION_QUEUETYPEMASTER_CREATE, {
            //         headers: {
            //             Authorization: `Bearer ${auth.APIToken}`,
            //         },
            ...
            //         type: String(managecategory.type),
            //         addedby: [
            //             {
            ...
            // for (let i = 0; i < selectedSubCategoryValue.length; i += batchSize) {
            //     const batch = selectedSubCategoryValue.slice(i, i + batchSize);
            //     batches.push(batch);
            // }
            ...
            //         handleClickOpenPopup();
            //         setloadingdeloverall(false);
            //         // results.forEach((result) => console.log("Success:", result.data));
            ...
            // const combinedData = selectedSubCategoryValue.flatMap((subcategory) =>
            //     selectedTypeFrom.map(d => d.value).map((type) => ({ subcategory, type }))
            // );
            ...
            // for (const batch of batches) {
            //     const promises = batch.map(({ subcategory, type }) =>
            //         axios.post(SERVICE.PRODUCTION_QUEUETYPEMASTER_CREATE, {
            //             headers: {
            //                 Authorization: `Bearer ${auth.APIToken}`,
            //             },
            //             category: String(managecategory.category),
            //             subcategory, // Use the subcategory from the batch
            //             newrate: Number(managecategory.newrate),
            //             type, // Use the type from the batch
            //             addedby: [
            //                 {
            //                     name: String(isUserRoleAccess.companyname),
            //                     date: String(new Date()),
            ...
            for (const batch of batches) {
                const batchPromises = batch.map(({ subcategory, type }) =>
                    axios.post(SERVICE.PRODUCTION_QUEUETYPEMASTER_CREATE, {
                        headers: {
                            Authorization: `Bearer ${auth.APIToken}`,
                        },
    ...
    const handleSubmit = async (e) => {
        ...
        if (managecategory.category === "Please Select Category") {
    ...
    const getCode = async (e, data) => {
        ...
        try {
            ...
            console.log(res.data.squeuetypemaster, "kdk")
            setManageCategoryEdit({ ...res.data.squeuetypemaster, orate: data.orate });

            // setSelectedTypeFormEdit(
            //     Array.isArray(data?.type)
            //         ? data.type.map((t) => ({ label: t, value: t }))
            //         : []
            // );
            fetchAllSubCategory(data.category)
            handleClickOpenEdit();
        } catch (err) {
            handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
        }
    ...
    const editSubmit = async (e) => {
        ...
        if (managecategoryEdit.category === "Please Select Category") {
            setPopupContentMalert("Please Select Category!");
            setPopupSeverityMalert("warning");
            handleClickOpenPopupMalert();
        }
    ...
    const fetchAllQueueTypeMaster = async () => {
        setPageName(!pageName)
        try {
            let res_grp = await axios.get(SERVICE.PRODUCTION_QUEUETYPEMASTER_UNITRATE, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const fetchAllTypemaster = async (proj) => {
        ...
        try {
            ...
            const defaultOptions = [
                ...
                { label: "Low Rate Queues", value: "Low Rate Queues" },
                { label: "KFI&CMS Data Entry Double Key Queues", value: "KFI&CMS Data Entry Double Key Queues" }
            ];
            ...
            // const categoryOpt = [...defaultOptions, ...dynamicOptions];
    ...
    const fetchorate = async (proj) => {
        ...
        try {
            ...
            const orate = res_module.data.queuetypemasters.orate
                                ...
                                <Grid item md={3} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        {/* <Selects
                                            maxMenuHeight={250}
                                            options={typeMaster}
                                            value={{ label: managecategory.type, value: managecategory.type }}
                                            onChange={(e) => {
                                                setManagecategory({
                                                    ...managecategory,
                                                    type: e.value,
                                ...
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item md={3} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            ...
                                            />
                                            {/* <MultiSelect options={typeMaster}
                                                value={selectedTypeFromEdit}
                                                onChange={handleTypeChangeFromEdit}
                                                valueRenderer={customValueRendererTypeFromEdit}
                                                labelledBy="Please Select Type"
                                            /> */}
                                        </FormControl>
                                    </Grid>
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item md={6} xs={12} sm={12} lg={3}>
                                <FormControl fullWidth size="small">
                                    ...
                                    <Typography>{viewcode.orate}</Typography>                                        </FormControl>
                            </Grid>
                            <Grid item md={6} xs={12} sm={12} lg={3}>
                                <FormControl fullWidth size="small">
                                    <Typography>
                                    ...
                                    <Typography>{viewcode.newrate}</Typography>                                        </FormControl>
                            </Grid>
                        </Grid>
                        <br /> <br /> <br />
                        <Grid container spacing={2}>
