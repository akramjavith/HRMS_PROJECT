...
function StockReorder() {
  ...
  const { isUserRoleCompare, pageName, setPageName, isUserRoleAccess, isAssignBranch } = useContext(UserRoleAccessContext);
  ...
  let exportColumnNames = ['Company', 'Branch', 'Unit', 'Floor', 'Area', 'Location', 'Material', 'Quantity(Balanced Count)'];
  let exportRowValues = ['company', 'branch', 'unit', 'floor', 'area', 'location', 'productname', 'balancedcount'];
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: 'flex' }}>
          <a href={`/stockpurchase/stockmanagerequest/${params.data.id}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
            {/* <Button variant="contained" color="error" size="small" sx={{ backgroundColor: '#d32f2f', color: 'white' }}>
              Re-Check
            </Button> */}
            <Button
              variant="contained"
              color="warning" // MUI predefined orange color
  ...
  const fetchStock = async () => {
    ...
    try {
      ...
      let singlehandtotal = singlehand.reduce((acc, current) => {
        const existingItemIndex = acc.findIndex((item) => item.company === current.company && item.branch === current.branch && item.unit === current.unit && item.floor === current.floor && item.area === current.area && item.location === current.location && item.productname === current.productname);
      ...
      let result = single.flatMap((item) => {
        return item.tododetails.map((subItem) => ({
          _id: subItem._id,
          company: item.company,
          branch: item.branch,
          unit: item.unit,
      ...
      let getfilter = result.reduce((acc, current) => {
        const existingItemIndex = acc.findIndex((item) => item.company === current.company && item.branch === current.branch && item.unit === current.unit && item.floor === current.floor && item.area === current.area && item.location === current.location && item.materialnew === current.materialnew);
  ...
  return (
    <Box>
      ...
      <PageHeading title="Manage Stock Reorder" modulename="Asset" submodulename="Stock" mainpagename="Stock Reorder" subpagename="" subsubpagename="" />
      ...
      {isUserRoleCompare?.includes('lstockreorder') && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>Stock Reorder List</Typography>
            </Grid>
