...
function CategoryAndSubcategory() {
  ...
  const getCategoryAndSubcategoryOverall = async () => {
    ...
    try {
      ...
      setCategorySubcategoryListOverall(
        response.data.categoryandsubcategory.map((item, index) => {
          const correctedArray = Array.isArray(item?.subcategoryname) ? item.subcategoryname.map((d) => (Array.isArray(d) ? d.join(',') : d)) : [];
          return {
            id: item._id,
  ...
  const overallBulkdelete = async (ids) => {
    setPageName(!pageName);
    try {
      let overallcheck = await axios.post(`${SERVICE.OVERALL_NONPRODUCTIONCATEGORYMASTER_BULKDELETE}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        id: ids,
  ...
  const handleSubmitEdit = () => {
    ...
    if (isNameMatch) {
      ...
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
    } else if ((singleCategory.categoryname != ovProj || !editTodo?.every((item) => ovProjsub?.includes(item))) && ovProjCount > 0) {
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Category and Subcategory" modulename="Production" submodulename="Non Production" mainpagename="Non-production Setup" subpagename="Category & Subcategory" subsubpagename="" />
      <>
                    ...
                    {editTodo.map((item, index) => {
                      return (
                        <li key={index}>
                          ...
                          <Grid sx={{ display: 'flex' }}>
                            <FormControl fullWidth size="small">
                              <TextareaAutosize aria-label="minimum height" minRows={5} value={item} />
                            </FormControl>
                            &emsp;
          ...
          <DialogActions>
            {selectedRowsCount > 0 ? (
              <>
                ...
                <Button sx={buttonStyles.buttonsubmit} onClick={(e) => delVendorcheckbox(e)}>
                  {' '}
                  OK{' '}
                </Button>
              </>
      ...
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Category and Subcategory Info" addedby={addedby} updateby={updateby} />
    </Box>
  );
}
