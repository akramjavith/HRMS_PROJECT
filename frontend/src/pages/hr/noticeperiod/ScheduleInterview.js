...
function ScheduleInterview({
  ...
  const [meetingState, setMeetingState] = useState({
    ...
    meetingcategory: "Exit Interview",
  ...
  const fetchMeetingCategoryAll = async () => {
    setPageName(!pageName)
    try {
      let res_location = await axios.get(SERVICE.EXITINTERVIEWTESTMASTER, {
        headers: { Authorization: `Bearer ${auth.APIToken}` },
      });
      setMeetingCatOption([
        ...res_location?.data?.interviewtestmasters?.map((t) => ({
          ...t,
          label: `${t.testname} (${t.category}-${t.subcategory})`,
          value: `${t.testname} (${t.category}-${t.subcategory})`,
        })),
      ]);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
  ...
  const fetchAssignInterviewr = async (company, branch, team) => {
    ...
    try {
      ...
      const he = [...new Set(ans.flatMap((d) => d.employee))];
  ...
  const isValidMeetingURL = (url) => {
    const patterns = [
      // Zoom: https://zoom.us/j/1234567890 or https://us02web.zoom.us/j/1234567890
      /^https?:\/\/(www\.)?zoom\.us\/j\/\d+$/,

      // Google Meet: https://meet.google.com/abc-defg-hij
      /^https?:\/\/(www\.)?meet\.google\.com\/[a-z]{3}-[a-z]{4}-[a-z]{3}$/,

      // Microsoft Teams: https://teams.microsoft.com/l/meetup-join/
      /^https?:\/\/(www\.)?teams\.microsoft\.com\/l\/meetup-join\/[^\s]+$/,

      // JioMeet: https://jiomeet.jio.com/short/xyz123 and https://jiomeetpro.jio.com/addtojiomeet?...
      /^https?:\/\/(www\.)?(jiomeet\.jio\.com\/short\/[a-zA-Z0-9]+|jiomeetpro\.jio\.com\/addtojiomeet\?meetingId=\d+&pwd=[a-zA-Z0-9]+&isWebinar=(true|false))$/,
    ];
  ...
  const handleSubmit = (e) => {
    ...
    if (valueCompanyCat?.length == 0) {
      ...
      meetingState.testname === "Please Select Test Name"
    ) {
      setPopupContentMalert('Please Select Test Name');
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    }
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (meetingState.meetingtype === "Please Select Interview Type") {
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      let candidateusername = credentials?.username
      let candidatepassword = credentials?.password
      ...
      let res = await axios.put(
        `${SERVICE.NOTICEPERIODAPPLY_SINGLE}/${meetingValues[5]}`,
        {
          ...
          testname: String(meetingState.testname),
          interviewcategory: String(meetingState.meetingcategory),
          interviewtype: String(meetingState.meetingtype),
          venue: String(
  ...
  const fetchMeeting = async () => {
    ...
    try {
      ...
      let generateUserName =
        (meetingValues[6] === "" || meetingValues[6] === undefined)
          ? generateUniqueUsername(fullnames)
          : meetingValues[6];
      let generatePassword =
        (meetingValues[7] === "" || meetingValues[7] === undefined) ? generatePass() : meetingValues[7];
        ...
        <>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item md={4} xs={12} sm={6}>
              <FormControl fullWidth size="small">
                <Typography>
                  Interview Category<b style={{ color: "red" }}>*</b>
                </Typography>
                <Selects
                  maxMenuHeight={300}
            ...
            <Grid item md={4} xs={12} sm={6}>
              <FormControl fullWidth size="small">
                ...
                <Selects
                  ...
                  placeholder="Please Select Meeting Category"
                  value={{
                    label: meetingState.testname,
                    value: meetingState.testname,
                  }}
                  onChange={(e) => {
                    setMeetingState({
                      ...meetingState,
            ...
            <Grid item md={4} xs={12} sm={6}>
              <FormControl fullWidth size="small">
                ...
                <OutlinedInput
                  ...
                  inputProps={{
                    min: meetingValues[8], // Set your "from" date here
                    max: meetingValues[9]  // Set your "to" date here
                  }}
                />
              </FormControl>
            </Grid>
