...
function ManualEntryTimeStudy() {
  ...
  const handleChangephonenumberflagCompleted = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === "") {
      ...
      if (parseInt(inputValue) > parseInt(ProducionIndividual.totalpages)) {
        ...
        // Update the state with the valid numeric value
        setProducionIndividual({ ...ProducionIndividual, flagcount: inputValue, startpage: getstpage[0], pendingpages: difference });
      }
    }
  };
  ...
  let datemodes =
    isUserRoleCompare.includes("lmanualentrytimestudy") ||
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      let projectscreate = await axios.post(SERVICE.MANUAL_CLIENT_INFO_CREATE, {
        headers: {
        ...
        startdate: projectmaster ? String(ProducionIndividual.startdate) : "",
        starttime: projectmaster ? String(ProducionIndividual.starttime) : "",
        statusmode: projectmaster ? (ProducionIndividual.creationstatus === "New" ? "Started" : String(ProducionIndividual.statusmode)) : String(ProducionIndividual.statusmode) === "Please Select Status" ? "" : String(ProducionIndividual.statusmode),
  ...
  const handleSubmit = async (e) => {
    setPageName(!pageName);
    // e.preventDefault();
    // const isNameMatch = projmasterDup?.some(
    //   (item) =>
    ...
    let res_Dupe = await axios.post(SERVICE.PRODUCTION_TIMESTUDY_DUPECHECK, {
      headers: {
        Authorization: `Bearer ${auth.APIToken}`,
      },
    ...
    let checkDupe = res_Dupe?.data?.timestudy;
  ...
  //   try {
  //     let res_project = await axios.post(SERVICE.MANUAL_CLIENT_INFO_LIMITED, {
  //       headers: {
  //         Authorization: `Bearer ${auth.APIToken}`,
  //       },
  //       username: isUserRoleAccess.username,
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Manual Entry Time Study"),
  ...
  return (
    <Box>
      <Headtitle title={"Manual Entry Time Study"} />
      {/* ****** Header Content ****** */}
      <PageHeading title="Manual Entry Time Study" modulename="Production" submodulename="Manual Entry" mainpagename="Manual Entry Time Study" subpagename="" subsubpagename="" />
          ...
          <Box sx={userStyle.selectcontainer}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>Add Manual Entry Time Study</Typography>
                </Grid>
              </Grid>
              <br />
              <Grid container spacing={2}>
                ...
                <Grid item md={3} xs={12} sm={6}>
                  <FormControl fullWidth size="small">
                    <Typography>All Login</Typography>
                    <Selects
                      options={loginAllotFilter}
                      styles={colourStyles}
                      value={{ label: ProducionIndividual.alllogin, value: ProducionIndividual.alllogin }}
                      onChange={(e) => {
                  ...
                  <>
                    ...
                    <Grid item md={3} xs={12} sm={6}>
                      <FormControl fullWidth size="small">
                        ...
                        <Selects options={datemodes} value={{ label: ProducionIndividual.enddatemode, value: ProducionIndividual.enddatemode }} onChange={(e) => handleEndDateModeChange(e.value)}></Selects>
                      </FormControl>
                    </Grid>
                    <Grid item md={3} xs={12} sm={6}>
                      <Grid container spacing={2}>
...
export default ManualEntryTimeStudy;