...
function UserShiftRoasterList() {
    ...
    const fetchFilteredUsersStatus = async () => {
        ...
        try {
            ...
            getAllResults().then(async (results) => {
                ...
                // const itemsWithSerialNumber = resultBefore?.map((item, index) => {
                ...
                //     // Check if the day before and after a 'Week Off' date is marked as 'Leave' or 'Absent'
                //     if (weekOption2?.includes(item.clockinstatus) && weekOption2?.includes(item.clockoutstatus)) {
                //         // Define the date format for comparison
                //         const itemDate = moment(item.rowformattedDate, "DD/MM/YYYY");

                //         const isPreviousDayAbsent = resultBefore.some(absentItem => moment(absentItem.rowformattedDate, "DD/MM/YYYY").isSame(itemDate.clone().subtract(1, 'days'), 'day') && absentItem.empcode === item.empcode && (item.lopcalculation === 'Yes - Full Day' || item.lopcalculation === 'Yes - Double Day'));

                //         const isNextDayAbsent = resultBefore.some(absentItem => moment(absentItem.rowformattedDate, "DD/MM/YYYY").isSame(itemDate.clone().add(1, 'days'), 'day') && absentItem.empcode === item.empcode && (item.lopcalculation === 'Yes - Full Day' || item.lopcalculation === 'Yes - Double Day'));
                ...
                //         }
                //     }
                //     else if (item.shift === 'Week Off') {
                //         const previousItem = array[index - 1];
                //         const nextItem = array[index + 1];

                //         const isLopRelevant = (entry) =>
                //             entry && (entry.lopcalculation === 'Yes - Full Day' || entry.lopcalculation === 'Yes - Double Day');

                //         const isPrevLop = isLopRelevant(previousItem);
                //         const isNextLop = isLopRelevant(nextItem);
    ...
    const columnDataTableUserShift = [
        ...
        {
            field: "clockinstatus", headerName: "ClockInStatus", flex: 0, width: 200, hide: !columnVisibilityUserShift.clockinstatus,
            cellRenderer: (params) => {
                return (
                    <Grid>
                        <Button size="small"
        ...
        {
            field: "clockoutstatus", headerName: "ClockOutStatus", flex: 0, width: 200, hide: !columnVisibilityUserShift.clockoutstatus,
            cellRenderer: (params) => {
                return (
                    <Grid>
                        <Button size="small"
        ...
        {
            field: "attendanceauto", headerName: "Attendance", flex: 0, width: 200, hide: !columnVisibilityUserShift.attendanceauto,
            cellRenderer: (params) => {
                return (
                    <Grid>
                        <Button size="small"
        ...
        {
            field: "daystatus", headerName: "Day Status", flex: 0, width: 200, hide: !columnVisibilityUserShift.daystatus,
            cellRenderer: (params) => {
                return (
                    <Grid>
                        <Button size="small"
    ...
    const filteredColumns = columnDataTableUserShift.filter((column) =>
        column.headerName.toLowerCase().includes(searchQueryManageUserShift.toLowerCase())
    );
    ...
    return (
        <Box>
            ...
            <PageHeading
                ...
                subsubpagename="User Shift Roater"
            />
            {isUserRoleCompare?.includes("lusershiftroaster") && (
                <>
                    ...
                    <Box sx={userStyle.container}>
                        ...
                        <Button sx={userStyle.buttongrp} onClick={handleOpenManageColumnsUserShift}> Manage Columns  </Button><br /><br />
                        {loader ?
            ...
            {/* Manage Column */}
            <Popover
                id={idManageColumnsUserShift}
                open={isManageColumnsOpenUserShift}
                anchorEl={anchorElUserShift}
                onClose={handleCloseManageColumnsUserShift}
