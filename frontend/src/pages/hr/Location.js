...
import { Box, Typography, OutlinedInput, TableBody, DialogTitle, LinearProgress, TableRow, TableCell, Select, MenuItem, Dialog, DialogContent, DialogActions, FormControl, Grid, Paper, Table, TableHead, TableContainer, Button, List, ListItem, ListItemText, Popover, Checkbox, TextField, IconButton } from "@mui/material";
...
function Location() {
    ...
    const [locationsalledit, setLocationsalledit] = useState([]);
    ...
    const rowData = async (id, name) => {
        try {
            ...
            const [res, resdev] = await Promise.all([
                await axios.get(`${SERVICE.LOCATION_SINGLE}/${id}`, {
                    headers: {
                        'Authorization': `Bearer ${auth.APIToken}`
                    }
                }),
                axios.post(SERVICE.OVERALLLOCATIONCHECK, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
            ...
            setCheckUser(resdev?.data?.count);
    ...
    const delRoundcheckbox = async () => {
        try {
            const deletePromises = selectedRows?.map((item) => {
                return axios.delete(`${SERVICE.LOCATION_SINGLE}/${item}`, {
                    headers: {
                        'Authorization': `Bearer ${auth.APIToken}`
                    }
                });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const isNameMatch = locations.some(item => item.name.toLowerCase() === (location.name).toLowerCase());
        const isCodeMatch = locations.some(item => item.code.toLowerCase() === (location.code).toLowerCase());
    ...
    const getOverallEditSection = async (e) => {
        try {
            let res = await axios.post(SERVICE.OVERALLLOCATIONCHECK, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                location: e,
    ...
    ${res?.data?.areagrouping?.length > 0 ? "Area Grouping" : ""} whether you want to do changes ..??`);
        } catch (err) { handleApiError(err, setShowAlert, handleClickOpenerr); }
    };
    ...
    const fetchLocationAll = async () => {
        try {
            ...
            setLocationsalledit(res_location?.data?.locationdetails.filter(item => item._id !== locatid._id));
        } catch (err) { setIsLocation(true); handleApiError(err, setShowAlert, handleClickOpenerr); }
    }
        ...
        try {
            ...
            const performUploads = async () => {
                try {
                    ...
                    if (
                        locatid.name?.toLowerCase() !==
                        oldLocationName?.toLowerCase()

                    ) {
                        await axios.put(
                            `${SERVICE.LOCATIONOVERALLUPDATE}`,
                            {
                                oldname: oldLocationName,
                                newname: locatid.name,
                            },
                            {
                                headers: {
                                    Authorization: `Bearer ${auth.APIToken}`,
    ...
    const editSubmit = (e) => {
        e.preventDefault();
        fetchLocationAll();
        const isNameMatch = locationsalledit.some(item => item.name.toLowerCase() === (locatid.name).toLowerCase());
        const isCodeMatch = locationsalledit.some(item => item.code.toLowerCase() === (locatid.code).toLowerCase());
        ...
        if (locatid.code === "") {
            ...
            handleClickOpenerr();
        } else if (locatid.name != ovProj && ovProjCount > 0) {
            setShowAlertpop(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: "orange" }} />
    ...
    const addSerialNumber = () => {
        ...
        const itemsWithSerialNumber = locations?.map((item, index) => ({ ...item, serialNumber: index + 1 }));
...

        {
            ...
            renderCell: (params) => (
                <Grid sx={{ display: 'flex' }}>
                    {isUserRoleCompare?.includes("elocation") && (
                        <Button sx={userStyle.buttonedit} onClick={() => {

                            getCode(params.row.id, params.row.name);
                        }}><EditOutlinedIcon style={{ fontsize: 'large' }} /></Button>
                    )}
                    {isUserRoleCompare?.includes("dlocation") && (
                    ...
                    {isUserRoleCompare?.includes("vlocation") && (
                        <Button
                    ...
                    {isUserRoleCompare?.includes("ilocation") && (
                        <Button
                            sx={userStyle.buttonedit}
                            onClick={() => {
                                handleClickOpeninfo();
    ...
    return (
        <Box>
            <Headtitle title={' LOCATION'} />
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Location </Typography>
            {isUserRoleCompare?.includes("alocation")
                && (
                    <Box sx={userStyle.container}>
                        ...
                        <>
                            <Grid container spacing={2}>
                                <Grid item xs={8}>
                                    <Typography sx={userStyle.importheadtext}>Add Location</Typography>
                                </Grid>
                            </Grid><br />
                            ...
                            <Grid container spacing={2}>
                                <Grid item md={2.5} xs={12} sm={6}>
                                    <>
                                        <Button variant='contained' color='primary' sx={{ display: "flex" }} onClick={handleSubmit} disabled={isBtn}>Submit</Button>
                                    </>
                                </Grid>
                                <Grid item md={2.5} xs={12} sm={6}>
                                    <>
            ...
            <Box>
                ...
                >
                    <Box sx={userStyle.container}>
                        <>
                            <Grid container spacing={2}>
                                <Typography sx={userStyle.SubHeadtext}>Edit Location</Typography>
                            </Grid><br />
                            <Grid container spacing={2}>
            ...
            <br />
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("llocation")
                && (
                    <>
                        <Box sx={userStyle.container}>
                            { /* ******************************************************EXPORT Buttons****************************************************** */}
                            ...
                            <Grid container spacing={2} style={userStyle.dataTablestyle}>
                                ...
                                <Grid item md={8} xs={12} sm={12} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                    <Box >
                                        {isUserRoleCompare?.includes("excellocation") && (
                                            <>
                                                <Button
                                                    onClick={(e) => {
                                        ...
                                        {isUserRoleCompare?.includes("csvlocation") && (
                                            <>
                                                <Button
                                                    onClick={(e) => {
                                                        setIsFilterOpen(true);
                                        ...
                                        {isUserRoleCompare?.includes("printlocation") && (
                                            <>
                                                <Button sx={userStyle.buttongrp} onClick={handleprint}>&ensp;<FaPrint />&ensp;Print&ensp;</Button>
                                            </>
                                        )}
                                        {isUserRoleCompare?.includes("pdflocation") && (
                                            <>
                                                <Button
                                                    sx={userStyle.buttongrp}
                                                    onClick={() => {
                                        ...
                                        {isUserRoleCompare?.includes("imagelocation") && (
                            ...
                            {/* {isUserRoleCompare?.includes("bdlocation") && (
                        ...
                        <Dialog open={isCheckOpen} onClose={handleCloseCheck} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                            <DialogContent sx={{ width: "350px", textAlign: "center", alignItems: "center" }}>
                                ...
                                <Typography variant="h6" sx={{ color: "black", textAlign: "center" }}>
                                    {checkUser > 0 ? (
                                        <>
                                            <span style={{ fontWeight: "700", color: "#777" }}>{`${deletelocation.name} `}</span>
                                            was linked
                                        </>
                                    ) : (
                                        ""
