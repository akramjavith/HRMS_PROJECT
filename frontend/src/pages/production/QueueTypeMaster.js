...
function QueueTypeMaster() {
    ...
    const handleTypeChangeFrom = (selected) => {
        const isOtherTaskSelected = selected.some(
            (option) => ["Other task queues", "Other task upload"].includes(option.value)
        );

        // If "Other task queues" is selected, keep only that option
        if (isOtherTaskSelected) {
            setSelectedTypeForm(
                selected.filter((option) => ["Other task queues", "Other task upload"].includes(option.value))
            );
        } else {
                                ...
                                <Grid item md={3} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <MultiSelect
                                            ...
                                            options={typeMaster.map((option) => ({
                                                ...option,
                                                disabled: selectedTypeFrom.some((selected) =>
                                                    ["Other task queues", "Other task upload"].includes(selected.value)
                                                ) && !["Other task queues", "Other task upload"].includes(option.value)
                                            }))}
