...
function BankReleaseBreakup() {
  ...
  const fetchBankRleaseDatas = async () => {
    try {
      ...
      let salaryDataFetched = addserialnumberfilteredTabletwo
        .filter((d) => d.status === "bankrelease" && d.bankreleasestatus === "created" && (d.updatechangedate ? d.updatechangedate == selectedDate : d.paydate == selectedDate))
        .map((item, index) => ({
          ...item,
      ...
      //     const finalPaidStatus = curr.updatedpaidstatus ? curr.updatedpaidstatus : curr.paidstatus;
      //     if (!acc[finalPaidStatus]) {
      //       acc[finalPaidStatus] = { paidstatus: finalPaidStatus, empcount: 0, payamounttotal: 0 };
      //     }
      //     acc[finalPaidStatus].empcount += 1;
      //     acc[finalPaidStatus].paymonth = curr.paymonth;
      //     acc[finalPaidStatus].payyear = curr.payyear;
      //     acc[finalPaidStatus].paydate = curr.updatechangedate ? curr.updatechangedate : curr.paydate;
      //     acc[finalPaidStatus]._id = curr._id;
      //     acc[finalPaidStatus].payamounttotal += Number(curr.payamount);
      //     return acc;
      //   }, {});
      const aggregation = Object.values(
        salaryDataFetched.reduce((acc, curr) => {
          const finalPaidStatus = curr.updatedpaidstatus || curr.paidstatus;
          ...
          if (!acc[finalPaidStatus]) {
            acc[finalPaidStatus] = {
              ...
              empcount: 0,
              payamounttotal: 0,
              paymonth: curr.paymonth,
              payyear: curr.payyear,
              paydate: curr.updatechangedate || curr.paydate,
              _id: curr._id, // This still may be problematic
            };
          }
          acc[finalPaidStatus].paidstatus = finalPaidStatus;
          acc[finalPaidStatus].empcount += 1;
          acc[finalPaidStatus].payamounttotal += Number(curr.payamount);
        ...
        <>
          ...
          <Box>
            ...
            <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
              <Link to={`/production/payroll/bankrelease`}>
            ...
            {isActive ? (
              ...
              <>
                <Box style={{ width: "100%" }} className="ag-theme-quartz" ref={gridRefContainer}>
                  <AgGridReact columnDefs={columnDataTable} ref={gridRefnew} rowData={rowDataTable} getRowId={(params) => params.data.id} onGridReady={onGridReady} onFirstDataRendered={onFirstDataRendered} domLayout={"autoHeight"} getRowClass={getRowClass} />
                </Box>
              </>
            )}
          </Box>
