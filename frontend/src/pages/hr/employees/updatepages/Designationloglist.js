...
function DesignationLogList() {
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Designation Log List.png");
        });
      });
    }
  };
  ...
  const rowData = async () => {
    try {
      ...
      setIsUserName(res?.data?.suser?.companyname);
      setDesignationlogs(res?.data?.suser?.designationlog);
      setDesignationlogcheck(true);
    } catch (err) {
      setDesignationlogcheck(true);
      handleApiError(err, setShowAlert, handleClickOpenerr);
  ...
  const columns = [
    ...
    { title: "Created Date&Time", field: "createdtime" },
  ...
  const downloadPdf = (isfilter) => {
    ...
    const dataWithSerial =
      isfilter === "filtered"
        ...
        : designationlogsArray.map((row) => ({
            ...row,
            serialNumber: serialNumberCounter++,
            startdate: isValidDateFormat(row.startdate)
    ...
    doc.save("Designation Log List.pdf");
  };
