...
function InternEdit() {
  ...
  useEffect(() => {
    ...
    newstate,
    //     loginNotAllot.process,assignExperience.updatedate,
    //     // assignExperience.assignExpMode,
    //     assignExperience.assignExpvalue,
    // assignExperience.assignEndExp,
    //     assignExperience.assignEndExpvalue,assignExperience.assignEndExpvalue,
    //     assignExperience.assignEndExpvalue,assignExperience.assignEndExpDate,
    //     assignExperience.assignEndTar,assignExperience.assignEndTarvalue
  ]);
  ...
  // console.log(oldHierarchyData ,newHierarchyData , oldHierarchyDataSupervisor , lastUpdatedData ,
  //   lastUpdatedData === "Designation"
  //   ? designationGroup
  //   : String(newHierarchyData[0]?.designationgroup),getingOlddatas?.designation , employee.team ,
  //   getingOlddatas?.designation !== selectedDesignation,
  //   ((oldHierarchyData?.length > 0 && newHierarchyData?.length < 1) ||
  //   (oldHierarchyDataSupervisor?.length > 0 && newHierarchyData?.length < 1)))

  // console.log(designationGroup , 'designationGroup')
  // console.log(lastUpdatedData === "Team" && newHierarchyData[0]?.designationgroup !== designationGroup , 'Team Change')
  // console.log(lastUpdatedData === "Designation" && newHierarchyData[0]?.department !== employee?.department, 'Designation Change')
  ...
  const fetchHandlerEdit = async () => {
    try {
      ...
      setGettingOldDatas(response?.data?.suser);
      let designationGrpName = alldesignation?.find(
        (data) => response?.data?.suser?.designation === data?.name
      )?.group;
      let allDesignations = alldesignation
        ?.filter((data) => designationGrpName === data?.group)
        ?.map((item) => item?.name);
      setOldDesignation(response?.data?.suser?.designation);
      ...
      const groupname = req_designation?.data?.designation?.find(
        (data) => data.name === response?.data?.suser?.designation
      );
      ...
      ShiftDropdwonsSecond(response?.data?.suser?.shiftgrouping);
      ...
      fetchDptDesignation(response?.data?.suser?.department);
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      if (changeToDesign === "Replace") {
        let hierarchyCheck = await axios.post(
          SERVICE.CHECKHIERARCHYEDITEMPDETAILSDESIGN,
          {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
            ...
            designation: String(selectedDesignation),
            empcode: String(employee.empcode),
            olddesignation: String(olddesignation),
        ...
        let hierarchyData = hierarchyCheck.data.allCondata;
        let deleteHierarchyOldData = hierarchyCheck.data.allCondataolddata;
        ...
        if (hierarchyData && hierarchyData.length > 0) {
          ...
          if (uniqueEntries.length > 0) {
            for (const item of uniqueEntries) {
              const res_queue = await axios.post(SERVICE.HIRERARCHI_CREATE, {
                ...
                control: String(item.control),
                empcode: String(employee.empcode),
                employeename: String(employee?.companyname),
                access: "all",
                action: Boolean(true),
                addedby: [
                  {
          ...
          let hierarchyCheckupdate = await axios.post(
            SERVICE.CHECKHIERARCHYEDITEMPDETAILSDESIGN,
            {
              headers: {
                Authorization: `Bearer ${auth.APIToken}`,
          ...
          let editHierarchyOldData = hierarchyCheckupdate.data.hirerarchi;

          if (editHierarchyOldData.length > 0) {
            const editPromises = editHierarchyOldData?.map((item) => {
              return axios.put(`${SERVICE.HIRERARCHI_SINGLE}/${item._id}`, {
                headers: {
                  Authorization: `Bearer ${auth.APIToken}`,
                },
          ...
          let hierarchyCheckupdateelse = await axios.post(
            SERVICE.CHECKHIERARCHYEDITEMPDETAILSDESIGN,
            {
              headers: {
                Authorization: `Bearer ${auth.APIToken}`,
          ...
          let editHierarchyOldData = hierarchyCheckupdateelse.data.hirerarchi;
          if (editHierarchyOldData.length > 0) {
            const editPromises = editHierarchyOldData?.map((item) => {
              return axios.put(`${SERVICE.HIRERARCHI_SINGLE}/${item._id}`, {
                headers: {
      ...
      if (oldHierarchyData?.length > 0 && newHierarchyData?.length > 0) {
        ...
        let res = await axios.post(`${SERVICE.HIRERARCHI_CREATE}`, {
          ...
          empteam:
            lastUpdatedData === "Team"
              ? newHierarchyData[0]?.team
              : employee.team,
          addedby: [
            {
              name: String(isUserRoleAccess?.username),
  ...
  const nextStepLog = (action) => {
    ...
    if (
      ...
      setShowAlert(
        <>
          ...
          />
          <p style={{ fontSize: "20px", fontWeight: 900 }}>
            {"Please Choose supervisor to replace"}
          </p>
        </>
      );
      handleClickOpenerr();
      newErrorsLog.accessiblecompany = (
        <Typography style={{ color: "red" }}>
          These Employee is not allowed to change Designation/Team, Update in
          Hierarchy first
        </Typography>
      );
    }
    ...
    if (
      (oldHierarchyDataSupervisor?.length > 0
        ? !hierarchyall
          ...
          !designationsName?.includes(selectedDesignation)
        : true) &&
      getingOlddatas?.team !== selectedTeam &&
      ((oldHierarchyData?.length > 0 && newHierarchyData?.length < 1) ||
        (oldHierarchyDataSupervisor?.length > 0 &&
          newHierarchyData?.length < 1))
    ) {
    ...
    if (
      ...
      newHierarchyData?.length > 0 &&
      lastUpdatedData === "Designation" &&
      newHierarchyData[0]?.department !== employee?.department
    ) {
      setShowAlert(
        <>
          <ErrorOutlineOutlinedIcon
    ...
    if (
      ...
      newHierarchyData?.length > 0 &&
      lastUpdatedData === "Team" &&
      newHierarchyData[0]?.designationgroup !== designationGroup
    ) {
      setShowAlert(
        <>
          <ErrorOutlineOutlinedIcon
          ...
          <p style={{ fontSize: "20px", fontWeight: 900 }}>
            {
              "These employees designations and Team are not the same as in the hierarchy. Update in hierarchy first."
            }
          </p>
        </>
      );
