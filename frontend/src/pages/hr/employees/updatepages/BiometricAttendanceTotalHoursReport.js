...
function BiometricAttendanceTotalHoursReport() {
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("BiometricAttendanceTotalHoursReport"),
    ...
    const sendRequest = async () => {
        ...
        try {
            const response = await axios.post(SERVICE.BIOMETRIC_USERS_ATTENDANCE_TOTALHOURS_REPORT, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
    ...
    const handleSubmit = async (e) => {
        ...
        else if (["Individual", "Branch", "Unit", "Team", "Deactivate"]?.includes(filterState?.type) &&
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "BiometricAttendanceTotalHoursReport.png");
                })
                .catch((error) => {
    ...
    // Excel
    const fileName = "BiometricAttendanceTotalHoursReport";
    //print...
    const componentRef = useRef();
    const handleprint = useReactToPrint({
        content: () => componentRef.current,
        documentTitle: "Biometric Attendance Total Hours Report",
        pageStyle: "print",
    });
    ...
    const columnDataTable = [
        ...
        },
        {
            ...
            flex: 0,
            width: 150,
            hide: !columnVisibility.fixedData,
            headerClassName: "bold-header",
            cellRenderer: (params) => (
                <Grid sx={{ display: 'flex' }}>
                    {isUserRoleCompare?.includes("vbiometricattendancetotalhoursreport") && (
    ...
    return (
        <Box>
            ...
            <PageHeading
                ...
                subsubpagename="Biometric Attendance Total Hours Report"
            />
            {isUserRoleCompare?.includes("lbiometricattendancetotalhoursreport") && (
                <Box sx={userStyle.selectcontainer}>
                    <>
                    <Typography sx={userStyle.importheadtext}>
                                 Biometric Attendance Total Hours Report Filter
                            </Typography>
            ...
            {isUserRoleCompare?.includes("lbiometricattendancetotalhoursreport") && (
                <>
                    <Box sx={userStyle.container}>
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>
                                List Biometric Attendance Total Hours Report
                            </Typography>
                        </Grid>
                        <Grid container spacing={2} style={userStyle.dataTablestyle}>
                            ...
                            >
                                <Box>
                                    {isUserRoleCompare?.includes("excelbiometricattendancetotalhoursreport") && (
                                        <>
                                            <Button
                                                onClick={(e) => {
                                    ...
                                    {isUserRoleCompare?.includes("csvbiometricattendancetotalhoursreport") && (
                                        <>
                                            <Button
                                                onClick={(e) => {
                                                    setIsFilterOpen(true);
                                    ...
                                    {isUserRoleCompare?.includes("printbiometricattendancetotalhoursreport") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("pdfbiometricattendancetotalhoursreport") && (
                                        <>
                                            <Button
                                                sx={userStyle.buttongrp}
                                                onClick={() => {
                                    ...
                                    {isUserRoleCompare?.includes("imagebiometricattendancetotalhoursreport") && (
                                        <Button
                                            sx={userStyle.buttongrp}
                                ...
                                <Table>
                                    <TableHead>
                                        <StyledTableRow>
                                            ...
                                            <StyledTableCell style={tableHeadCellStyle}> {"Total Hours (hrs)"}</StyledTableCell>
                                            <StyledTableCell style={tableHeadCellStyle}> {"Break (mins)"}</StyledTableCell>
                                        </StyledTableRow>
                                    </TableHead>
                                    <TableBody>
                                        {AttendanceList?.fixedData?.map((item, i) => (
                                            <StyledTableRow>
                                                <StyledTableCell style={tableBodyCellStyle}>{i + 1}.</StyledTableCell>
                                                <StyledTableCell style={tableBodyCellStyle}> {item.startTime}</StyledTableCell>
                                                <StyledTableCell style={tableBodyCellStyle}> {item.endTime}</StyledTableCell>
                                                <StyledTableCell style={tableBodyCellStyle}> {item.totalHours}</StyledTableCell>
                                                <StyledTableCell style={tableBodyCellStyle}> {item.break ? item?.break : "-"}</StyledTableCell>
                                            </StyledTableRow>
                                        ))}
                                    </TableBody>
                                </Table>
            ...
            <ExportData
                ...
                filename={"Biometric Attendance Total Hours Report"}
...
export default BiometricAttendanceTotalHoursReport;