...
function UnitLogList({ boardinglogs, userID }) {
  ...
  const handleExportXL = (isfilter) => {
    if (isfilter === "filtered") {
      ...
      exportToCSV(
        poardinglogsTeamArray.map((t, index) => ({
          SNo: index + 1,
          Company: t.company,
          Branch: t.branch,
          Unit: t.unit,
  ...
  const uniqueArray = getresarr.map((item, index) => {
    return   {
      ...
      setBoardinglogsUnit(uniqueArray);
      setUnitlogcheck(true);
    } catch (err) {setUnitlogcheck(true);handleApiError(err, setShowAlert, handleClickOpenerr);}
  };
