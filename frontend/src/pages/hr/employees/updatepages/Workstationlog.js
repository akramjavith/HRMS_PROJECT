...
function WorkstationLogList({ boardinglogs, userID }) {
  ...
  const rowDataArray = async () => {
    ...
    try {
      ...
      const getresarr = getarr.filter((data, index) => {
        return data.ischangeworkstation === true;
      });
  ...
  // Excel
  const fileName = "Workstation Log List";

  // print...
  const componentRef = useRef();
  const handleprint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: "Workstation Log List",
    pageStyle: "print",
  });
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Workstation Log List.png");
        });
      });
    }
  };
...
export default WorkstationLogList;
