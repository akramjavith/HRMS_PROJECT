...
function TempProductionPoints() {
  ...
  const { isUserRoleCompare, isAssignBranch,listPageAccessMode,isUserRoleAccess} = useContext(UserRoleAccessContext);
              ...
              <MultiSelect
                options={employees
                  ?.filter((comp) =>
                     ...
                     selectedCompany.map((item) => item.value).includes(comp.company) && selectedBranch.map((item) => item.value).includes(comp.branch) && selectedUnit.map((item) => item.value).includes(comp.unit) && selectedTeam.map((item) => item.value).includes(comp.team))
                  ?.map((com) => ({
                    ...com,
