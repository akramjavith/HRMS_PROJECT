...
function TeamGrouping() {
  ...
  const { isUserRoleCompare, isUserRoleAccess, allUsersData, isAssignBranch, allTeam ,    pageName,
    setPageName,
...
try {
  ...
  const fetchTeamgrouping = async () => {
     setPageName(!pageName);
try {
      let res_team = await axios.post(SERVICE.TEAMGROUPING_ASSIGNBRANCH,{
      ...
      const answer = res_team?.data?.teamgroupings?.length > 0 ? res_team?.data?.teamgroupings?.map((item, index) => ({
        ...item,
