...
function CompletedTeamLeaveVerification({ data, setUpdated }) {
  ...
  const handleExportXL = async (isfilter) => {
    ...
    saveAs(new Blob([buffer]), `Completed Team Leave Verification.${fileExtension}`);
    setIsFilterOpen(false);
  };
    ...
    <Box>
      ...
      {isUserRoleCompare?.includes('lteamleaveverification') && (
        <>
          {/* ******************************************************EXPORT Buttons****************************************************** */}
          <Grid item xs={8}>
            <Typography sx={userStyle.importheadtext}>Completed Team Leave Verification List</Typography>
          </Grid>
      ...
      <Box>
        <Dialog maxWidth="lg" open={statusOpen} onClose={handleStatusClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
          ...
          <br />
          <DialogActions>
            <Button variant="contained" sx={buttonStyles.buttonsubmit} onClick={() => fetchLeaveHistoryUpdate(selectStatus.employeeid, selectStatus.employeename)}>
              Update
            </Button>
            <Button
