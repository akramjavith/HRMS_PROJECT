...
function TrainingUserPanel() {
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("User_Training_Panel-Pending.pdf");
    };
        ...
        try {
            ...
            if (userShiftDetails !== "" && calculatedTime !== "" && raiseTicketList) {
                ...
                let userStatus = final?.length > 0 ? final?.filter(item2 => {
                    const matchingItem = userCheck?.data?.trainingforuser?.find(data1 => data1?.username === isUserRoleAccess?.companyname && data1.status === "Active" && ["Completed", "Finished By Others", "Not Applicable to Me"]?.includes(data1?.taskstatus) && new Date() <= new Date(data1?.shiftEndTime))
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "User_Training_Panel-Pending.png");
                });
            });
        }
    };
    ...
    const ClickButtonToShowDate = async (ans) => {
        ...
        try {
            ...
            const answer = ans?.map((data) => {
                axios.post(
                    `${SERVICE.CREATE_TRAINING_FOR_USER}`,
                    {
                        headers: {
                            Authorization: `Bearer ${auth.APIToken}`,
                        },
                    ...
                    {isUserRoleCompare?.includes("ltraininguserpanel") && (
                        <>
                            <Box sx={userStyle.dialogbox}>
                                ...
                                <Typography sx={userStyle.SubHeaderText}>User Training Panel - <b style={{ backgroundColor: 'red', color: "white" }}>Pending Trainings</b></Typography>
