...
function Trainingcategory() {
    ...
    const delTrainingCategory = async (e) => {
        ...
        try {
            if (trainingcategorysid) {
                await axios.delete(`${SERVICE.TRAININGCATEGORY_SINGLE}/${e}`, {
                    headers: {
                        'Authorization': `Bearer ${auth.APIToken}`
                    }
                });
    ...
    const delTraningcatecheckbox = async () => {
        try {
            ...
            //     let res = axios.delete(`${SERVICE.TRANINGCATEGORY_SINGLE}/${item}`, {
            //         headers: {
            //             'Authorization': `Bearer ${auth.APIToken}`
            //         },
            //     });
    ...
    const getOverallEditSectionUpdate = async () => {
        try {
            ...
            sendEditRequestOverall(res?.data?.trainingsubcate,
        ...
        try {
            ...
            if (trainuser?.length > 0) {
                let answ = trainuser.map((d, i) => {
                    ...
                    const answerCate = d?.trainingdocuments?.filter(data => data?.category === ovcategory)?.map((item
                    ) => {
            ...
            if (trandetailslog?.length > 0) {
                let answ = trandetailslog.map((d, i) => {
                    ...
                    const answerCate = d?.trainingdetailslog?.filter(data => data?.category === ovcategory)?.map((item
                    ) => {
                        return { ...item, category: trainingcategoryEdit.categoryname }
                    })
    ...
    const sendEditRequest = async () => {
        try {
            ...
            await fetchTrainingcategory();getOverallEditSectionUpdate();
            handleCloseModEdit();
            setShowAlert(
                <>
                    <ErrorOutlineOutlinedIcon sx={{ fontSize: "100px", color: 'orange' }} />
    ...
    const fetchTrainingcategory = async () => {
        ...
        } catch (err) {setTrainingcategorycheck(true);handleApiError(err, setShowAlert, handleClickOpenerr);}
    }
    ...
    // Excel
    const fileName = "Training Category";

    const [trainingcategoryData, setTrainingcategroyData] = useState([]);

    // get particular columns for export excel 
    const getexcelDatas = () => {

        var data = trainingcategorys?.map((t, index) => ({
            "Sno": index + 1,
            "Category Name": t.categoryname,
            "Description": t.description,
        }));
        setTrainingcategroyData(data);
    ...
    useEffect(() => {
        getexcelDatas();
    }, [trainingcategoryEdit, trainingcategory, trainingcategorys])
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Training Category</Typography>
            ...
            <Box>
                ...
                >
                    ...
                    <DialogActions>
                        ...
                        <Button autoFocus variant="contained" color='error'
                            onClick={(e) => delTrainingCategory(trainingcategorysid)}
                        > OK </Button>
                    </DialogActions>
                </Dialog>
                ...
                >
                    <Box sx={{ width: '550px', padding: '20px 50px' }}>
                        <>
                            <Typography sx={userStyle.HeaderText}>Training Category Info</Typography>
                            <br /><br />
                            <Grid container spacing={2}>
                                <Grid item md={12} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                        ...
                        {selectedRowsCount > 0 ?
                            <>
                                ...
                                <Button variant="contained" color='error'
                                    onClick={(e) => delTraningcatecheckbox(e)}
                                > OK </Button>
                            </>
                            :
