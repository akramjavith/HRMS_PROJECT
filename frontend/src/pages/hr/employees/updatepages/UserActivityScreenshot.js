...
function UserActivityScreenshot() {
    ...
    const fetchDropDowns = async () => {
        try {
            ...
            let userName = req?.data?.useractivityscreenshot[0]?.usernames?.map((data) => ({ label: data, value: data }))
            let systemName = req?.data?.useractivityscreenshot[0]?.systems?.map((data) => ({ label: data, value: data }))
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Employee Monitor Log Screenshot"),
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "Employee Monitor Log Screenshot.png");
                })
                .catch((error) => {
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "Employee Monitor Log Screenshot",
        pageStyle: "print",
    });
    ...
    function openImage(filename) {
        const imageUrl = `${SERVICE.VEIW_USERACTIVITY_SCREENSHOT}/${filename}`;
...

        {
            ...
            cellRenderer: (params) => (
                <Grid sx={{ display: "flex" }}>
                    {isUserRoleCompare?.includes("vemployeemonitorlogscreenshot") && (
                        <>
                            <Button
                            ...
                            <Button
                                ...
                                variant="text"
                                onClick={() => {
                                    openImage(params.data.filename);
                                }}
                            >
                                VIEW
                            </Button>
    ...
    const fetchEmployee = async () => {
        ...
        try {
            let response = await axios.post(
                SERVICE.DYNAMIC_USERACTIVITY_SCREENSHOT_CONTROLLER,
                {
                    aggregationPipeline,
                },
                {
    ...
    return (
        <Box>
            <NotificationContainer />
            {/* ****** Header Content ****** */}
            <Headtitle title={"EMPLOYEE MONITOR LOG SCREENSHOT"} />

            <PageHeading
                title="Manage Employee Monitor Log Screenshot"
                modulename="Human Resources"
                submodulename="HR"
                mainpagename="Employee"
                subpagename="Employee Status Details"
                subsubpagename="Employee Monitor Log Screenshot"
            />
            <br />
            {isUserRoleCompare?.includes("lemployeemonitorlogscreenshot") && (
                <>
                ...
                <Box sx={userStyle.container}>
                    ...
                    <Grid item xs={8}>
                        <Typography sx={userStyle.importheadtext}>
                            Employee Monitor Log Screenshot List
                        </Typography>
                    </Grid>
                    <br />
                    <Grid container spacing={2} style={userStyle.dataTablestyle}>
                        ...
                        >
                            <Box>
                                {isUserRoleCompare?.includes("excelemployeemonitorlogscreenshot") && (
                                    <>
                                        <Button
                                            onClick={(e) => {
                                ...
                                {isUserRoleCompare?.includes("csvemployeemonitorlogscreenshot") && (
                                    <>
                                        <Button
                                            onClick={(e) => {
                                                setIsFilterOpen(true);
                                ...
                                {isUserRoleCompare?.includes("printemployeemonitorlogscreenshot") && (
                                    <>
                                ...
                                {isUserRoleCompare?.includes("pdfemployeemonitorlogscreenshot") && (
                                    <>
                                        <Button
                                            sx={userStyle.buttongrp}
                                            onClick={() => {
                                ...
                                {isUserRoleCompare?.includes("imageemployeemonitorlogscreenshot") && (
                    ...
                    <Box sx={userStyle.dialogbox}>
                        <>
                            <Typography sx={userStyle.SubHeaderText}>
                                Edit Employee Monitor Log Screenshot Info
                            </Typography>
                            <br />
                            <br />
                            <Grid container spacing={2}>
            ...
            <ExportData
                ...
                itemsTwo={employees ?? []}
                filename={"Employee Monitor Log Screenshot"}
            ...
            <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" maxWidth="md" fullWidth={true} sx={{ marginTop: '50px' }}>
                <Box sx={{ padding: "20px 50px" }}>
                    <>
                        <Typography sx={userStyle.HeaderText}> View Employee Monitor Log Screenshot</Typography>
                        <br /> <br />
                        <Grid container spacing={2}>
                            ...
                            <Grid item md={6} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    ...
                                    <Typography>{empaddform?.filename}</Typography>
                                </FormControl>
                            </Grid>
                            <Grid item md={6} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6">File Size</Typography>
                                    <Typography>{empaddform?.filesize}</Typography>
                                </FormControl>
                            </Grid>
                            <Grid item md={6} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6">File Path</Typography>
                                    <Typography>{empaddform?.filepath}</Typography>
                                </FormControl>
                            </Grid>
                            <Grid item md={6} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
...
export default UserActivityScreenshot;