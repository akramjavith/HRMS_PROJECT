...
function DiabledUsersList() {
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String('Disabled Users List'),
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, 'Disabled Users List.png');
        })
        .catch((error) => {
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: 'Disabled Users List',
    pageStyle: 'print',
  });
  ...
  return (
    <Box>
      ...
      <Headtitle title={'Disabled Users List'} />
      {/* ****** Header Content ****** */}
      <PageHeading title="Disabled Users List" modulename="LDAP" submodulename="Disabled Users List" mainpagename="" subpagename="" subsubpagename="" />
          ...
          <Box sx={userStyle.dialogbox}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  <Typography sx={userStyle.importheadtext}>Filter Disabled Users List</Typography>
                </Grid>
                                ...
                                options={allUsersData
                                  ?.filter((u) => {
                                    ...
                                    if (valueEmployementTypeCat?.length === 1 && valueEmployementTypeCat.includes('Internship')) {
                                      return u.workmode === 'Internship' && valueCompanyCat?.includes(u.company) && valueBranchCat?.includes(u.branch) && valueUnitCat?.includes(u.unit) && valueTeamCat?.includes(u.team);
                                    }
                                    ...
                                    return valueCompanyCat?.includes(u.company) && valueBranchCat?.includes(u.branch) && valueUnitCat?.includes(u.unit) && valueTeamCat?.includes(u.team);
                                  })
                                  ?.map((u) => ({
                                    label: u.companyname,
                                    value: u.companyname,
          ...
          {isUserRoleCompare?.includes('llockeduserslist') && (
            <>
              <Box sx={userStyle.container}>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={10}>
                    <Typography sx={userStyle.SubHeaderText}>Disabled Users List</Typography>
                  </Grid>
                  <Grid item xs={2}>
                    {isUserRoleCompare?.includes('alockeduserslist') && (
                      <>
...
export default DiabledUsersList;
