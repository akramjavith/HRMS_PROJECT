...
function InterviewTestingEndPage() {
    useEffect(() => {
        fetchStatus();
    }, []);
    ...
    const fetchStatus = async () => {
        try {
            if ((responseid) && (responseid !== "null")) {
                ...
                let single_round = await axios.get(
                    `${SERVICE.GET_SINGLE_TEST_RESPONSE}/${responseid}`
                );

                setAnswerStatus(
                    single_round?.data?.sTestResponse?.interviewForm?.map((t, index) => ({
                        questionNo: index + 1,
...
export default InterviewTestingEndPage;