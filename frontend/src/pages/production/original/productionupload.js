...
function ProductionOriginalUpload() {
  ...
  const rowData = async (id, vendor, from, to, uniqid, filename) => {
    ...
    if (checkdate > 0 || checkdaypoint > 0) {
      ...
      try {
        ...
        // });
        setDeleteRound(id);
        let resNew = await axios.post(SERVICE.PRODUCTION_UPLOAD_GETDELETEDATASALL, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      if (Productionsid) {
        ...
        if (deleteallDatas.length > 0) {
          ...
          for (const chunk of deleteDataChunks) {
            const deletePromise = axios.post(SERVICE.PRODUCTION_UPLOAD_DELETEMULTI, {
              headers: {
                Authorization: `Bearer ${auth.APIToken}`,
          ...
          Promise.all(deletePromises)
            .then(async () => {
              await axios.delete(`${SERVICE.PRODUCTION_ORGINAL_SINGLE}/${e}`, {
                headers: {
                  Authorization: `Bearer ${auth.APIToken}`,
                  'Content-Type': 'application/json;charset=UTF-8',
                },
              ...
              setloadingdeloverall(false);
              await fetchProductionoriginalDelete();
              handleCloseMod();
              setSelectedRows([]);
              setPage(currentpage);
            })
  ...
  const delProductioncheckbox = async () => {
    try {
      const deletePromises = selectedRows?.map((item) => {
        return axios.delete(`${SERVICE.PRODUCTION_ORGINAL_SINGLE}/${item}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });
  ...
  const handleDownloadReturnBulk = async (id, date, vendor, orgid) => {
    setDownloader(orgid);
    try {
      let res = await axios.post(SERVICE.PRODUCTION_UPLOAD_FILENAMEONLY_BULKDOWNLOAD, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const getviewCode = async (e, vendor, from, to, id) => {
    try {
      ...
      setgetedit(id);
      let res = await axios.post(SERVICE.PRODUCTION_UPLOAD_FILENAMEONLY, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const getviewCodeall = async (filename, id) => {
    try {
      ...
      let res = await axios.post(SERVICE.PRODUCTION_UPLOAD_FILENAMELIST, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        filename: String(filename),
  ...
  const getinfoCode = async (e) => {
    try {
      let res = await axios.get(`${SERVICE.PRODUCTION_ORGINAL_SINGLE}/${e}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const rowDataviewall = async (filename, id, vendor, from, to) => {
    ...
    if (checkdate > 0 || checkdaypoint > 0) {
      ...
      setShowAlert('Loading...');
      try {
        let res = await axios.post(SERVICE.PRODUCTION_UPLOAD_GETDELETEDATAS, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
          filename: String(filename),
  ...
  const sendEditRequest = async () => {
    try {
      let res = await axios.put(`${SERVICE.PRODUCTION_ORGINAL_SINGLE}/${subprojectsid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const fetchProductionoriginal = async () => {
    ...
    setSelectedTodate('');
    try {
      let res_vendor = await axios.get(SERVICE.PRODUCTION_ORGINAL_LIMITED, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const fetchProductionoriginalDelete = async () => {
    ...
    try {
      if (selectedVendor.length === 0 || selectedFromdate == '' || selectedTodate == '') {
        ...
        let res_vendor = await axios.post(SERVICE.PRODUCTION_ORGINAL_LIMITED_FILTER, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const checkUpdatedFileSts = async (id, count, idval, fromdate, vendor) => {
    ...
    try {
      let resNew = await axios.post(SERVICE.PRODUCTION_UPLOAD_CHECKSTATUS, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        fromdate: fromdate,
      ...
      let resupdate = await axios.put(`${SERVICE.PRODUCTION_ORGINAL_SINGLE}/${idval}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        percent: total,
  ...
  return (
    <Box>
      <Headtitle title={'Production Original Upload List'} />
      {/* ****** Header Content ****** */}
      <PageHeading title=" Production Original Upload List" modulename="Production" submodulename="Upload" mainpagename="Original" subpagename="Production Upload List" subsubpagename="" />
      ...
      {isUserRoleCompare?.includes('lproductionuploadlist') && (
        <>
          ...
          <Box sx={userStyle.container}>
            <Grid container>
              <Grid item md={8} sm={8} xs={12}>
                <Typography sx={userStyle.importheadtext}>Production Original Upload List</Typography>
              </Grid>
              <Grid item md={4} sm={4} xs={12} sx={{ display: 'flex', justifyContent: 'end' }}>
                <Link to="/production/productionoriginalupload">
      ...
      <Box>
        ...
        <Dialog open={openInfo} onClose={handleCloseinfo} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
          <Box sx={{ width: '550px', padding: '20px 50px' }}>
            <>
              <Typography sx={userStyle.HeaderText}>Production Original Upload Info</Typography>
              <br />
              <br />
              <Grid container spacing={2}>
                <Grid item md={12} xs={12} sm={12}>
