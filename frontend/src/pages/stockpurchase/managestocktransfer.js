...
function ManageStockTransfer() {
      ...
      if (newData.length > 0) {
        ...
        setShowAlert(
          <>
            ...
            <p style={{ fontSize: '20px', fontWeight: 900 }}>{'Product Name Already Added'}</p>
          </>
        );
        handleClickOpenerr();
      }
  ...
  const fetchLocation = async () => {
    try {
      ...
      const locationall = [
        ...res_unit?.data?.locationdetails.map((d) => ({
          ...d,
          label: d.name,
          value: d.name,
        })),
  ...
  useEffect(() => {
    ...
    const filteredCategories = floors
  ...
  useEffect(() => {
    // Filter categories based on the selected project and vendors
    const filteredCategoriesedit = floors
  ...
  useEffect(() => {
    ...
    const filteredCategoriesproductname = assetmaster
      ?.filter((u) => u.assethead === selectedProducthead)
      .map((u) => ({
        ...u,
        label: u.name,
        value: u.name,
  ...
  useEffect(() => {
    ...
    const filteredCategoriesproductnameedit = assetmaster
      ?.filter((ue) => ue.assethead === selectedProductheadedit)
      .map((ue) => ({
        ...ue,
        label: ue.name,
        value: ue.name,
  ...
  const columns = [
    ...
    { title: 'From Location ', field: 'fromlocation' },
    { title: 'Product Head ', field: 'producthead' },
  ...
  return (
    <Box>
      ...
      <PageHeading title="Manage Stock Transfer" modulename="Asset" submodulename="Stock" mainpagename="Stock Manage Transfer" subpagename="" subsubpagename="" />
      {!statusCheck ? (
          ...
          <Box sx={userStyle.selectcontainer}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}> Add Stock Transfer </Typography>{' '}
                </Grid>
              </Grid>
              <br />
              ...
              <Grid container spacing={2}>
                ...
                <Grid item md={4} sm={6} xs={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects options={filteredFloor} styles={colourStyles} placeholder={'please select'} value={{ label: selectedFloor, value: selectedFloor }} onChange={(e) => setSelectedFloor(e.value)} />
                  </FormControl>
                </Grid>
                <Grid item md={4} sm={6} xs={12}>
                  <FormControl fullWidth size="small">
                ...
                <Grid item md={4} sm={6} xs={12}>
                  <FormControl size="small" fullWidth>
                    <Typography>
                      Product Head <b style={{ color: 'red' }}>*</b>{' '}
                    </Typography>
                    <Selects options={account} styles={colourStyles} value={{ label: selectedProducthead, value: selectedProducthead }} onChange={handleProductChange} />
                  </FormControl>
                </Grid>
                <Grid item md={4} sm={6} xs={12}>
                  <FormControl size="small" fullWidth>
