...
import PenaltyDepartmentmonthlist from "./Penaltydepartmentmonthlist"
window.ResizeObserver = ResizeObserver;

function Penaltywaivermaster() {
    ...
    const fetchEmployee = async (e) => {
        ...
        try {
            ...
            let data_set = (e.value === "All" && penaltyWaiver.branch !== "All" && penaltyWaiver.processcode !== "All") ? allUsersData.filter((data) => penaltyWaiver.company === data.company && penaltyWaiver.branch === data.branch && processUniq?.includes(data.process) && penaltyComp.department === data.department) :
                (e.value === "All" && penaltyWaiver.branch !== "All" && penaltyWaiver.processcode === "All") ? allUsersData.filter((data) => penaltyWaiver.company === data.company && penaltyWaiver.branch === data.branch && penaltyComp.department === data.department) :
                    (e.value === "All" && penaltyWaiver.branch === "All" && penaltyWaiver.processcode !== "All") ? allUsersData.filter((data) => penaltyWaiver.company === data.company && processUniq?.map((item) => item)?.includes(data.process) && penaltyComp.department === data.department) :
                        (e.value === "All" && penaltyWaiver.branch === "All" && penaltyWaiver.processcode === "All") ? allUsersData.filter((data) => penaltyWaiver.company === data.company && penaltyComp.department === data.department) :
                            (e.value !== "All" && penaltyWaiver.branch === "All") ? allUsersData.filter((data) => e.value === data.process && penaltyWaiver.company === data.company && penaltyComp.department === data.department) :
                                allUsersData.filter((data) => e.value === data.process && penaltyWaiver.company === data.company && penaltyWaiver.branch === data.branch && penaltyComp.department === data.department)
                ;
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Penalty Waiver Master"),
    ...
    const rowData = async (id, name) => {
        setPageName(!pageName)
        try {
            let res = await axios.get(`${SERVICE.PENALTYWAIVERMASTER_SINGLE}/${id}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const delLeavecriteria = async () => {
        ...
        try {
            if (leavesid) {
                await axios.delete(`${SERVICE.PENALTYWAIVERMASTER_SINGLE}/${leavesid}`, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
                });
    ...
    const delLeavecheckbox = async () => {
        ...
        try {
            const deletePromises = selectedRows?.map((item) => {
                return axios.delete(`${SERVICE.PENALTYWAIVERMASTER_SINGLE}/${item}`, {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
                });
    ...
    const sendRequest = async () => {
        ...
        try {
            let subprojectscreate = await axios.post(SERVICE.PENALTYWAIVERMASTER_CREATE, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                // department: String(penaltyComp.department),
                department: penaltyComp.department ? String(penaltyComp.department) : "",
                year: penaltyComp.year ? String(penaltyComp.year) : "",
                monthname: penaltyComp.monthname ? String(penaltyComp.monthname) : "",
                fromdate: penaltyComp.fromdate ? String(penaltyComp.fromdate) : "",
                todate: penaltyComp.todate ? String(penaltyComp.todate) : "",
    ...
    const handleSubmit = async (e) => {
        ...
        const isNameMatch = penaltyWaivers.some(
            (item) =>
                ...
                item.department === penaltyComp.department &&
                item.year === penaltyComp.year &&
                item.fromdate === penaltyComp.fromdate &&
                item.monthname === penaltyComp.monthname &&
                item.todate === penaltyComp.todate

        );
        if (penaltyComp.department === "") {
    ...
    const getCode = async (e, name) => {
        setPageName(!pageName)
        try {
            let res = await axios.get(`${SERVICE.PENALTYWAIVERMASTER_SINGLE}/${e}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const sendEditRequest = async () => {
        ...
        try {
            let res = await axios.put(`${SERVICE.PENALTYWAIVERMASTER_SINGLE}/${subprojectsid}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                department: String(penaltyWaiverEdit.department),
                year: String(penaltyWaiverEdit.year),
                monthname: String(penaltyWaiverEdit.monthname),
                fromdate: String(penaltyWaiverEdit.fromdate),
                todate: String(penaltyWaiverEdit.todate),
    ...
    const editSubmit = async (e) => {
        ...
        const isNameMatch = penaltyWaiverAllEdit.some(
            (item) =>
                ...
                item.year === penaltyWaiverEdit.year &&
                item.fromdate === penaltyWaiverEdit.fromdate &&
                item.monthname === penaltyWaiverEdit.monthname &&
                item.todate === penaltyWaiverEdit.todate
        );
    ...
    const fetchLeavecriteria = async () => {
        setPageName(!pageName)
        try {
            let res_vendor = await axios.get(SERVICE.PENALTYWAIVERMASTER, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const columnDataTableLeaveCrit = [
        ...
        { field: "year", headerName: "Year", flex: 0, width: 120, hide: !columnVisibilityLeaveCrit.year, headerClassName: "bold-header" },
        { field: "monthname", headerName: "Month", flex: 0, width: 120, hide: !columnVisibilityLeaveCrit.monthname, headerClassName: "bold-header" },
        { field: "fromdate", headerName: "From Date", flex: 0, width: 120, hide: !columnVisibilityLeaveCrit.fromdate, headerClassName: "bold-header" },
        { field: "todate", headerName: "To Date", flex: 0, width: 120, hide: !columnVisibilityLeaveCrit.todate, headerClassName: "bold-header" },
        ...
        { field: "waiverallowupto", headerName: "Waiver Count", flex: 0, width: 120, hide: !columnVisibilityLeaveCrit.waiverallowupto, headerClassName: "bold-header" },
        { field: "waiveramountupto", headerName: "Waiver Amount", flex: 0, width: 120, hide: !columnVisibilityLeaveCrit.waiveramountupto, headerClassName: "bold-header" },
        { field: "waiverpercentageupto", headerName: "Waiver Percentage %", flex: 0, width: 120, hide: !columnVisibilityLeaveCrit.waiverpercentageupto, headerClassName: "bold-header" },
        ...
        { field: "clienterrorpercentage", headerName: "Client Error Percentage %", flex: 0, width: 120, hide: !columnVisibilityLeaveCrit.clienterrorpercentage, headerClassName: "bold-header" },
...

        {
            ...
            cellRenderer: (params) => (
                <Grid sx={{ display: "flex" }}>
                    <Box>
                        {isUserRoleCompare?.includes("epenaltywaivermaster") && (
                            <Button
                        ...
                        {isUserRoleCompare?.includes("dpenaltywaivermaster") && (
                            <Button
                        ...
                        {isUserRoleCompare?.includes("vpenaltywaivermaster") && (
                            <Button
                                sx={userStyle.buttonedit}
                                onClick={() => {
                        ...
                        {isUserRoleCompare?.includes("ipenaltywaivermaster") && (
                            <Button
                                sx={userStyle.buttonedit}
                                onClick={() => {
    ...
    let exportColumnNamescrt = [
        'Department', 'Year', 'Month', 'From Date', 'To Date', 'Company', 'Branch', 'Code',
        'Process', 'Employee', 'Waiver Amount Count', 'Waiver Amount', 'Wwaiver Percentage %', 'Client Error Count', 'Client Error Amount', 'Client Error Percentage %', 'Days']
    let exportRowValuescrt = [
        'department', 'year', 'monthname', 'fromdate', 'todate', 'company', 'branch', 'processcode',
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "Penalty Waiver Master",
        pageStyle: "print",
    });
    ...
    const handleCaptureImage = () => {
        if (gridRefImageLeaveCrit.current) {
            domtoimage.toBlob(gridRefImageLeaveCrit.current)
                .then((blob) => {
                    saveAs(blob, "Penalty Waiver Master.png");
                })
                .catch((error) => {
    ...
    return (
        <Box>
            <Headtitle title={"Penalty Waiver Master"} />
            {/* ****** Header Content ****** */}
            <PageHeading
                ...
                subsubpagename="Penalty Waiver Master"
            />
            {isUserRoleCompare?.includes("apenaltywaivermaster") && (
                <>
                    <PenaltyDepartmentmonthlist setChange={setPenaltyComp} change={penaltyComp} setPenaltywaiver={setPenaltywaiver} penaltyWaiver={penaltyWaiver} />
                    <br />
            ...
            {isUserRoleCompare?.includes("lpenaltywaivermaster") && (
                <>
                    <Box sx={userStyle.container}>
                        {/* ******************************************************EXPORT Buttons****************************************************** */}
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>Penalty Waiver Master List</Typography>
                        </Grid>
                        ...
                        {isUserRoleCompare?.includes("bdpenaltywaivermaster") && (
                            ...
                            <Grid container spacing={2}>
                                <Grid item md={12} xs={12} sm={12}>
                                    <Typography sx={userStyle.HeaderText} >
                                        <b>Edit Penalty Waiver Master</b>
                                    </Typography>
                                </Grid>
                            </Grid>
                            <Grid container spacing={2}>
                                ...
                                <Grid item md={2} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <OutlinedInput
                                            id="component-outlined"
                                            type="text"
                                            value={penaltyWaiverEdit.monthname}
                                ...
                                <Grid item md={2} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <OutlinedInput
                                            id="component-outlined"
                                            type="text"
                                            value={penaltyWaiverEdit.fromdate}
                                ...
                                <Grid item md={2} xs={12} sm={12}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <OutlinedInput
                                            ...
                                            type="text"
                                            placeholder="Please Enter Name"
                                            value={penaltyWaiverEdit.todate}
                ...
                <DialogContent sx={{ width: "1050px" }}>
                    <>
                        <Grid container spacing={2}>
                            <Grid item xs={8}>
                                <Typography sx={userStyle.importheadtext}>
                                    <b>View Penalty Waiver Master</b>
                                </Typography>
                            </Grid>
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item md={3} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6"> Waiver Count</Typography>
                                    <Typography>
                            ...
                            <Grid item md={3} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6"> Waiver Amount</Typography>
                                    <Typography>
                            ...
                            <Grid item md={3} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6"> Waiver Percentage %</Typography>
                                    <Typography>
                            ...
                            <Grid item md={3} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6"> Client Error Count</Typography>
                                    <Typography>
                            ...
                            <Grid item md={3} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6"> Client Error Amount</Typography>
                                    <Typography>
                            ...
                            <Grid item md={3} xs={12} sm={12}>
                                <FormControl fullWidth size="small">
                                    <Typography variant="h6"> Client Error Percentage %</Typography>
                                    <Typography>
            ...
            <ExportData
                ...
                filename={"Penalty Waiver Master"}
            ...
            <InfoPopup
                ...
                heading="Penalty Waiver Master Info"
                addedby={addedby}
                updateby={updateby}
            />
        </Box>
...
export default Penaltywaivermaster;