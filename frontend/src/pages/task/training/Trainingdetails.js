...
function TrainingDetails() {
  ...
  let exportColumnNames = [
    ...
    'Company', 'Branch',
    'Unit', 'Team',
    'Responsible Person', 'Online Test',
    'Test Names'
  ];
  let exportRowValues = [
  ...
  const fetchEmployeeOptions = async (e, type) => {
    ...
    setPageName(!pageName);
    try {
      let res_category = await axios.post(SERVICE.USER_TRAINING_DETAILS_EMPNAMES, {
  ...
  const fetchProcessQueue = async () => {
    ...
    try {
      ...
      const answer = res_freq?.data?.trainingdetails?.length > 0 ? res_freq?.data?.trainingdetails?.map((item, index) => ({
        id: item._id,
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          ...
          {isUserRoleCompare?.includes("itrainingdetails") && (
            <Link to={`/training/traininfuserlog/${params.data.id}`} style={{ textDecoration: "none", color: "#fff", minWidth: "0px" }}>
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading
        title="Schedule Training Details"
        modulename="Training"
        submodulename="Training Details"
        mainpagename=""
        subpagename=""
        subsubpagename=""
      />
                            ...
                            <MultiSelect
                              options={accessbranch?.filter(
                                (comp) =>
                                  selectedOptionsCompanyEdit
                                    .map((item) => item.value)
                                    .includes(comp.company) && selectedOptionsBranchEdit
                                      .map((item) => item.value)
                                      .includes(comp.branch)
                              )?.map(data => ({
                                label: data.unit,
                              ...
                              options={allTeam?.filter(
                                (comp) =>
                                  selectedOptionsCompanyEdit
                                    ...
                                    .includes(comp.company) && selectedOptionsBranchEdit
                                      .map((item) => item.value)
                                      .includes(comp.branch) && selectedOptionsUnitEdit
                                        .map((item) => item.value)
                                        .includes(comp.unit)
                              )?.map(data => ({
                                label: data.teamname,
      ...
      <InfoPopup
        ...
        heading="Schedule Training Details Info"
        addedby={addedby}
        updateby={updateby}
      />
      <br />
