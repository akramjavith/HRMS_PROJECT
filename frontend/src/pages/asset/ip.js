...
function ManageIP() {
  ...
  const downloadPdf = async (isfilter) => {
    if (isfilter === 'filtered') {
      try {
        ...
        //   defaultStyle: { font: 'Helvetica' }, // Use a built-in font instead of Roboto

        //   content: [
        //     { text: "IpMasters", style: "header", alignment: "center" },
      ...
      let response = await axios.get(
        SERVICE.IP_MASTER_UPLOAD_PDFDOWNLOAD,
  ...
  const fetchIpMaster = async () => {
    ...
    try {
      ...
      let subcatescount = res_vendor?.data?.totalCount;
  ...
  const columnDataTable = [
    ...
    //           setSelectedRowsCat([]);
    //         } else {
    //           const allRowIds = ipmasters.map((row) => row.id);
    //           const allRowIdsCat = ipmasters.map((row) =>
  ...
  return (
    <Box>
            ...
            {!reasonmasterCheck ? (
              ...
              <>
                ...
                <Box style={userStyle.dataTablestyle}>
                  <Box>
                    Showing {ipmasters.length > 0 ? (page - 1) * pageSize + 1 : 0} to {Math.min(page * pageSize, totalCount)} of {totalCount} entries
                  </Box>
                  <Box>
        ...
        <Table sx={{ minWidth: 700 }} id="ipmaster" ref={componentRef}>
          <TableHead>
            <TableRow>
              ...
              <TableCell>Category Nmae</TableCell>
              <TableCell>Subcategory</TableCell>
              <TableCell>IP Address</TableCell>
              <TableCell>Type</TableCell>
              <TableCell>IP Details </TableCell>
              <TableCell>Subnet </TableCell>
              <TableCell>Gateway </TableCell>
              <TableCell>DNS1 </TableCell>
              <TableCell>DNS2 </TableCell>
              <TableCell>DNS3 </TableCell>
              <TableCell>DNS4 </TableCell>
              <TableCell>DNS5 </TableCell>
            ...
            {ipmasters &&
              ipmasters.map((row, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>{row.ipdetails}</TableCell>
                  <TableCell>{row.subnet}</TableCell>
                  <TableCell>{row.gateway}</TableCell>
                  <TableCell>{row.dns1}</TableCell>
                  <TableCell>{row.dns2}</TableCell>
                  <TableCell>{row.dns3}</TableCell>
                  <TableCell>{row.dns4}</TableCell>
                  <TableCell>{row.dns5}</TableCell>
                  <TableCell>{row.available}</TableCell>
                  <TableCell>{row.starting}</TableCell>
                  <TableCell>{row.ending}</TableCell>
                </TableRow>
              ))}
          </TableBody>
        </Table>
      ...
      {/* <ExportData
        ...
        filteredDataTwo={(filteredChanges !== null ? filteredRowData : ipmaster) ?? []}
        itemsTwo={ipmasters ?? []}
        filename={"IP Master"}
