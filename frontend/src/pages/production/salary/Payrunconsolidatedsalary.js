...
function ConsolidatedSalary() {
  ...
  const fetchBranchAll = async (company) => {
    try {
      ...
      let branchOptfirstthree = res_location?.data?.branches;
  ...
  const fetchUnitAll = async (company, branch) => {
    try {
      ...
      let units = res_location?.data?.units;
      setUnits(units);
    } catch (err) {
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
  ...
  const fetchCompanies = async () => {
    setIsActive(true);
    try {
      const [RES_COM, RES_BRANCH, RES_USERS, RES_UNIT, RES_DESIG, RES_TEAMS, RES_TAX, RES_SHIFT, RES_SALARYSLAB, RES_SHORTAGE, RES_ERA, RES_REVENUE, RES_ACPOINT, RES_PAYRUNCONTROL, RES_ATTSTS, RES_ATTMODE, RES_TARGET] = await Promise.all([
      ...
      let customSort = ["Individual", "Team", "Unit", "Branch", "Department"];
  ...
  // const fetchDepartmentMonthsetsPaidDate = async () => {
  //   try {
  ...
  //       year: selectedYear,
  //     });
  //     let res_employeeNxt = await axios.post(SERVICE.DEPTMONTHSET_LIMITED, {
  //       headers: {
  //         Authorization: `Bearer ${auth.APIToken}`,
  //       },
  //       monthname: selectedMonthNum >= 12 ? "January" : monthNames[selectedMonthNum],
  //       year: selectedMonthNum >= 12 ? String(Number(selectedYear) + 1) : selectedYear,
  //     });
  //     let Res = await axios.post(SERVICE.PAIDSTATUSFIX_LIMITED, {
  //       headers: {
  //         Authorization: `Bearer ${auth.APIToken}`,
  //       },
  //       month: selectedMonth,
  //       year: selectedYear,
  //     });
  //     setPaidStatusFix(Res?.data?.paidstatusfixs);
  ...
  //     // let filteredMonthsetsNxt = res_employee.data.departmentdetails.filter((item) => item.year == selectedYear && item.monthname == selectedMonth);
  //     // setExpDptDates(res_employee.data.departmentdetails);
  //     setMonthsets(res_employee.data.departmentdetails);
  //     setMonthsetsNxt(res_employeeNxt.data.departmentdetails);
  //   } catch (err) {
  ...
  // useEffect(() => {
  //   fetchDepartmentMonthsetsPaidDate();
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Consolidated Salary List.png");
        });
      });
    }
  };
          ...
          async function sendBatchRequest(batch) {
            try {
              ...
              itemsWithSerialNumber.forEach((item, index, array) => {
                if (attStatusOption.includes(item.daystatus) && item.clockin === "00:00:00" && item.clockout === "00:00:00" && item.paidpresent === "Yes - Full Day") {
                  ...
                  if (hasRelevantStatus(previousItem)) {
                    ...
                    previousItem.lopcalculation = getFinalLop(getAttModeLop(previousItem.attendanceautostatus ? previousItem.attendanceautostatus : getattendancestatus(previousItem)), getAttModeLopType(previousItem.attendanceautostatus ? previousItem.attendanceautostatus : getattendancestatus(previousItem)));
                    previousItem.lopcount = getCount(getFinalLop(getAttModeLop(previousItem.attendanceautostatus ? previousItem.attendanceautostatus : getattendancestatus(previousItem)), getAttModeLopType(previousItem.attendanceautostatus ? previousItem.attendanceautostatus : getattendancestatus(previousItem))));
                    ...
                    previousItem.paidpresent = getFinalPaid(getAttModePaidPresent(previousItem.attendanceautostatus ? previousItem.attendanceautostatus : getattendancestatus(previousItem)), getAttModePaidPresentType(previousItem.attendanceautostatus ? previousItem.attendanceautostatus : getattendancestatus(previousItem)));
                    previousItem.lopday = getAssignLeaveDayForLop(getFinalLop(getAttModeLop(previousItem.attendanceautostatus ? previousItem.attendanceautostatus : getattendancestatus(previousItem)), getAttModeLopType(previousItem.attendanceautostatus ? previousItem.attendanceautostatus : getattendancestatus(previousItem))));
                  ...
                  if (hasRelevantStatus(nextItem)) {
                    ...
                    nextItem.paidpresentday = getAssignLeaveDayForPaid(getFinalPaid(getAttModePaidPresent(nextItem.attendanceautostatus ? nextItem.attendanceautostatus : getattendancestatus(nextItem)), getAttModePaidPresentType(nextItem.attendanceautostatus ? nextItem.attendanceautostatus : getattendancestatus(nextItem))));
                  }
                }
              });
              ...
              let resultdata = finalresult?.map((item, index) => {
                const finalPaidPresentDays = Number(item.paidpresentday) - (Number(item.holidayCount) + Number(item.leaveCount));
                ...
                return {
                  ...
                  paidpresentday: Number(item.paidpresentday) > Number(item.shift) ? Number(item.shift) - Number(item.lopcount) : Number(item.paidpresentday) - (Number(item.doublelopcount) + Number(item.doublehalflopcount)),
                };
              });
            ...
            .then((results) => {
              ...
              function processEmployeeItem(item, index, data) {
                try {
                  ...
                  // console.log(findTotalNoOfDays, 'sdfgdf');
                  ...
                  let getdayPointsMonth = dayPointsUser
                    .filter(
                      (d) =>
                        ...
                        d.date >= findDate && d.date <= enddateproduction && d.name === item.companyname
                    )
                    .reduce((acc, current) => {
                      const existingItemIndex = acc.findIndex((item) => item.name === current.name && item.companyname === current.companyname && item.branch === current.branch && item.empcode === current.empcode);
                      ...
                      if (existingItemIndex !== -1) {
                        ...
                        // existingItem.pointarray +=[ Number(current.point)];

                        // existingItem.daypoint += Number(current.daypoint);
                        existingItem.target += current.weekoff == "Not Allotted" || current.weekoff == "Not Allot" || current.weekoff == "" || current.weekoff == "Week Off" || current.daypointsts === "WEEKOFF" ? 0 : Number(current.target);
                        // existingItem.weekoffcount += current.weekoff == "Not Allotted" || current.weekoff == "Not Allot" || current.weekoff == "" || current.weekoff == "Week Off" || current.daypointsts === "WEEKOFF" ? 1 : 0;
                        existingItem.date.push(current.date);
                        // existingItem.pointarray.push(current.weekoff == "Not Allotted" || current.weekoff == "Not Allot" || current.weekoff == "" || current.weekoff == "Week Off" || current.daypointsts === "WEEKOFF" ? 0 : Number(current.target));
                        ...
                        if (current.shiftsts === "Enable" || current.allowancepoint >= 1) {
                        ...
                        existingItem.allowancetarget += current.shiftsts === "Enable" ? Number(current.target) : 0;
                        ...
                        acc.push({
                          ...
                          target: current.weekoff == "Not Allotted" || current.weekoff == "Not Allot" || current.weekoff == "" || current.weekoff == "Week Off" || current.daypointsts === "WEEKOFF" ? 0 : Number(current.target),
                          // _id: current.id,
                          ...
                          // weekoffcount: current.weekoff == "Not Allotted" || current.weekoff == "Not Allot" || current.weekoff == "" || current.weekoff == "Week Off" || current.daypointsts === "WEEKOFF" ? 1 : 0,
                          // doj: current.doj,
                          // department: current.department,
                          // prod: current.prod,
                          startDate: current.date,
                          endDate: current.date,
                          allowancetarget: current.shiftsts === "Enable" ? Number(current.target) : 0,
                          // pointarray: [current.weekoff == "Not Allotted" || current.weekoff == "Not Allot" || current.weekoff == "" || current.weekoff == "Week Off"  || current.daypointsts === "WEEKOFF" ? 0 : Number(current.target)],
                          ...
                          noallowancepoint: current.shiftsts === "Enable" || current.allowancepoint >= 1 ? 1 : 0,
                        });
                      }
                      return acc;
                    }, []);
                  ...
                  const enddateproductionnxtmonth = isHadreasondate ? item.reasondate : findNxtEndDate;
                  ...
                  let getdayPointsMonthNxtMonth = dayPointsUserNxtMonth
                    .filter(
                      (d) =>
                        // console.log(   d.date , findDate , d.date , (findexp ? findexp.todate : selectedMonEndDate))
                        d.date >= findNxtStartDate && d.date <= enddateproductionnxtmonth && d.name === item.companyname
                    )
                    .reduce((acc, current) => {
                  ...
                  let paiddayscalcvalfrommonthstatus = paidpresentdayvalue;
                  ...
                  let nightAllowancefinalcalculation = CHECKSHIFTALLOWANCE === "Enable" && totalNoOfDaysCalcVal > 0 ? ((1000 / tond) * noshiftlogvalfinal * (allowancepointCalcVal > 0 ? (allowancepointCalcVal * 100) / shiftallowancetargetfinal : 0)) / 100 : 0;
                  ...
                  let prodBasicValCalc = lopBasicValCal;
                  let prodHraValCalc = lopHRAValCal;
                  let prodConveyanceValCalc = lopConveyValCal;
                  let prodMEDAllowanceValCalc = lopMedicalValCal;
                  let prodOtherValCalc = lopOtherValCal;
                  let prodPRODValCalc = lopProductionAllowance;
                  if (CHECK_DEPARTMENT_ACCESS && CHECK_DEPARTMENT_ACCESS.prod === true) {
                    if (Number(prodAllowanceCalcVal) === 0) {
                      prodBasicValCalc = lopBasicValCal;
                      prodHraValCalc = lopHRAValCal;
                      prodConveyanceValCalc = lopConveyValCal;
                      prodMEDAllowanceValCalc = lopMedicalValCal;
                      prodOtherValCalc = lopOtherValCal;
                      prodPRODValCalc = lopProductionAllowance;
                    } else {
                      prodBasicValCalc = acheivedProductionAllowanceValCal == 0 ? 0 : acheivedProductionAllowanceValCal > 0 ? lopBasicValCal : lopBasicValCal + (lopBasicValCal * ((100 * acheivedProductionAllowanceValCal) / (lopBasicValCal + lopHRAValCal + lopConveyValCal + lopMedicalValCal + lopOtherValCal))) / 100;
                      prodHraValCalc = acheivedProductionAllowanceValCal == 0 ? 0 : acheivedProductionAllowanceValCal > 0 ? lopHRAValCal : lopHRAValCal + (lopHRAValCal * ((100 * acheivedProductionAllowanceValCal) / (lopBasicValCal + lopHRAValCal + lopConveyValCal + lopMedicalValCal + lopOtherValCal))) / 100;
                      prodConveyanceValCalc = acheivedProductionAllowanceValCal == 0 ? 0 : acheivedProductionAllowanceValCal > 0 ? lopConveyValCal : lopConveyValCal + (lopConveyValCal * ((100 * acheivedProductionAllowanceValCal) / (lopBasicValCal + lopHRAValCal + lopConveyValCal + lopMedicalValCal + lopOtherValCal))) / 100;
                      prodMEDAllowanceValCalc = acheivedProductionAllowanceValCal == 0 ? 0 : acheivedProductionAllowanceValCal > 0 ? lopMedicalValCal : lopMedicalValCal + (lopMedicalValCal * ((100 * acheivedProductionAllowanceValCal) / (lopBasicValCal + lopHRAValCal + lopConveyValCal + lopMedicalValCal + lopOtherValCal))) / 100;
                      prodMEDAllowanceValCalc = acheivedProductionAllowanceValCal == 0 ? 0 : acheivedProductionAllowanceValCal > 0 ? lopOtherValCal : lopOtherValCal + (lopOtherValCal * ((100 * acheivedProductionAllowanceValCal) / (lopBasicValCal + lopHRAValCal + lopConveyValCal + lopMedicalValCal + lopOtherValCal))) / 100;
                      prodPRODValCalc = acheivedProductionAllowanceValCal == 0 ? 0 : acheivedProductionAllowanceValCal > 0 ? acheivedProductionAllowanceValCal : 0;
                    }
                  } else {
                    prodBasicValCalc = lopBasicValCal;
                    prodHraValCalc = lopHRAValCal;
                  ...
                  // // console.log(acheivedProductionAllowanceValCal, 'acheivedProductionAllowanceValCal');
                  ...
                  // console.log(penaltyCalcVal, 'penaltyCalcVal', processcodeexpvaluesalary);
                  ...
                  let calcNetSalaryValCalc = Number(AcheivedPointsCalcVal) > 0 ? Number(lopBasicValCal) + Number(lopHRAValCal) + Number(lopConveyValCal) + Number(lopMedicalValCal) + Number(lopProductionAllowance) + Number(lopOtherValCal) : 0;

                  let ProdNetSalaryFinal = Number(prodBasicValCalc) + Number(prodHraValCalc) + Number(prodConveyanceValCalc) + Number(prodMEDAllowanceValCalc) + Number(prodPRODValCalc) + Number(prodOtherValCalc);

                  let lossDed = 0;
                  let OtherDed = 0;

                  let finalBasicValCalc = prodBasicValCalc === 0 ? 0 : prodBasicValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodBasicValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;
                  let finalHraValCalc = prodHraValCalc === 0 ? 0 : prodHraValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodHraValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;
                  let finalConveyValCalc =
                    prodConveyanceValCalc === 0 ? 0 : prodConveyanceValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodConveyanceValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;

                  let finalMedicalAllowValcCalc =
                    prodMEDAllowanceValCalc === 0 ? 0 : prodMEDAllowanceValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodMEDAllowanceValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;
                  let finalOtherValcCalc = prodOtherValCalc === 0 ? 0 : prodOtherValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodOtherValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;

                  let finalProductionValcCalc = prodPRODValCalc === 0 ? 0 : prodPRODValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodPRODValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;
                  ...
                  let ESI_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (finalNetSalaryValcCalc / totalshiftCalcVal) * pfDaysVal * Esiper : 0;
                  ...
                  let ncpDaysVal =
                    totalpaiddaycalVal > 0 && actualBasicCalcVal > 0
                      ? // ( totalNoOfDaysCalcVal - ((((finalBasicValCalc * totalNoOfDaysCalcVal) / actualBasicCalcVal) - (totalNoOfDaysCalcVal - pfDaysVal)).toFixed(0)) )
                        Number(totalNoOfDaysCalcVal) - Math.abs((finalBasicValCalc * totalNoOfDaysCalcVal) / actualBasicCalcVal - (totalNoOfDaysCalcVal - pfDaysVal))
                      : 0;

                  let finalLOPDaysCalcVal = Math.round(
                    Number(totalasbleaveCalcVal) + (Number(actualBasicCalcVal) > 0 ? Number(totalshiftCalcVal) - (Number(totalshiftCalcVal) * Number(finalBasicValCalc)) / Number(actualBasicCalcVal) : 0) < Number(totalshiftCalcVal)
                      ? Number(totalasbleaveCalcVal) + (Number(actualBasicCalcVal) > 0 ? Number(totalshiftCalcVal) - (Number(totalshiftCalcVal) * Number(finalBasicValCalc)) / Number(actualBasicCalcVal) : 0)
                  ...
                  let paySlipLopCalval = grossValue === 0 || totalNoOfDaysCalcVal === 0 || totalpaiddaycalVal === 0 ? 0 : totalshiftCalcVal - Math.round((totalshiftCalcVal * finalNetSalaryValcCalc) / grossValue) < 0 ? 0 : totalshiftCalcVal - Math.round((totalshiftCalcVal * finalNetSalaryValcCalc) / grossValue);
                  ...
                  let finalLeaveDeductionValCalc =
                    ...
                    grossValue === 0 || totalshiftCalcVal === 0 || totalNoOfDaysCalcVal === 0 || totalpaiddaycalVal === 0 ? 0 : (grossValue / totalshiftCalcVal) * (totalshiftCalcVal - (totalshiftCalcVal * finalNetSalaryValcCalc) / grossValue < 0 ? 0 : totalshiftCalcVal - (totalshiftCalcVal * finalNetSalaryValcCalc) / grossValue);

                  let findprofTaxAmt = profTaxMaster.find((d) => d.company == item.company && d.branch === item.branch && Number(d.fromamount) <= finalNetSalaryValcCalc && Number(d.toamount) + 0.99 >= finalNetSalaryValcCalc);
                  ...
                  // console.log(ERAAmountCalcVal, 'ERAAmountCalcVal');
                  ...
                  // console.log(acutalERAValCalc, 'acutalERAValCalc');

                  let pfAmount1 = findEmployerPercentage;
                  // console.log(finalSalaryCalcVal,ESI_deduction,PF_deduction,ESI_EMPR_deduction,PF_Emper_deduction,profTaxCalcVal,'profTaxCalcVal')

                  let PF_Emper_deduction = totalshiftCalcVal > 0 && pfAmount1 > 0 ? (finalBasicValCalc / totalshiftCalcVal) * pfDaysVal * (pfAmount1 / 100) : 0;
                  ...
                  let ESI_EMPR_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (finalNetSalaryValcCalc / totalshiftCalcVal) * pfDaysVal * ESI_EMPR_Perncetage : 0;
                  // console.log(finalSalaryCalcVal,ESI_deduction,PF_deduction,ESI_EMPR_deduction,PF_Emper_deduction,profTaxCalcVal,'profTaxCalcVal')
                  ...
                  let acutalDeductionCalcVal =
                    CHECK_DEPARTMENT_ACCESS && CHECK_DEPARTMENT_ACCESS.deduction === true ? (Number(totalPointsValueCalc) + Number(revenueAmountCalc) - (Number(CTC_Calcval) + Number(Shortage1ValCalc)) >= 0 ? 0 : Number(totalPointsValueCalc) + Number(revenueAmountCalc) - (Number(CTC_Calcval) + Number(Shortage1ValCalc))) : 0;
                  ...
                  // console.log(totalPointsValueCalc, CTC_Calcval, revenueAmountCalc, Shortage1ValCalc, 'minimumDeductionCalcVal');
                  ...
                  // console.log(calcNetSalaryValCalc, (Number(penaltyAmtCalculationVal) + lossDed + OtherDed))
                  // if (calcNetSalaryValCalc > 0) {
                  finalBasicValCalc = prodBasicValCalc === 0 ? 0 : prodBasicValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodBasicValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;
                  finalHraValCalc = prodHraValCalc === 0 ? 0 : prodHraValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodHraValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;
                  finalConveyValCalc =
                    prodConveyanceValCalc === 0 ? 0 : prodConveyanceValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodConveyanceValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;

                  finalMedicalAllowValcCalc =
                    prodMEDAllowanceValCalc === 0 ? 0 : prodMEDAllowanceValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodMEDAllowanceValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;
                  finalOtherValcCalc = prodOtherValCalc === 0 ? 0 : prodOtherValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodOtherValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;
                  // console.log(prodPRODValCalc, ProdNetSalaryFinal, penaltyAmtCalculationVal, lossDed, OtherDed, 'ProdNetSalaryFinal');
                  finalProductionValcCalc = prodPRODValCalc === 0 ? 0 : prodPRODValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) > 0 ? prodPRODValCalc * ((((ProdNetSalaryFinal - (penaltyAmtCalculationVal + lossDed + OtherDed)) * 100) / ProdNetSalaryFinal) * 0.01) : 0;
                  ...
                  ESI_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (finalNetSalaryValcCalc / totalshiftCalcVal) * pfDaysVal * Esiper : 0;
                  ...
                  ncpDaysVal = totalpaiddaycalVal > 0 ? (pfvalues.pfdeduction === true ? ncpvaluecalc : ncpvaluecalc1) : 0;
                  ...
                  // console.log(finalLOPDaysCalcVal, 'finalLOPDaysCalcVal');

                  paySlipLopCalval = totalNoOfDaysCalcVal === 0 || totalpaiddaycalVal === 0 || grossValue === 0 ? 0 : totalshiftCalcVal - Math.round((totalshiftCalcVal * finalNetSalaryValcCalc) / grossValue) < 0 ? 0 : totalshiftCalcVal - Math.round((totalshiftCalcVal * finalNetSalaryValcCalc) / grossValue);
                  ...
                  findprofTaxAmt = profTaxMaster.find((d) => d.company === item.company && d.branch === item.branch && Number(d.fromamount) <= finalNetSalaryValcCalc && Number(d.toamount) + 0.99 >= finalNetSalaryValcCalc);
                  ...
                  PF_Emper_deduction = pfAmount1 > 0 && Shortage1ValCalc > 0 ? (finalBasicValCalc / totalshiftCalcVal) * pfDaysVal * (pfAmount1 / 100) : 0;
                  // // let pfEmployerDeductionCalc = PF_Emper_deduction
                  //ESI-DEDCUTION
                  // ESI_EMPR_Perncetage = pfvalues.esideduction === true ? Number(findEmployerESIPercentage) / 100 : 0;

                  ESI_EMPR_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (finalNetSalaryValcCalc / totalshiftCalcVal) * pfDaysVal * ESI_EMPR_Perncetage : 0;
                  // console.log(finalSalaryCalcVal, ESI_deduction, PF_deduction, ESI_EMPR_deduction, PF_Emper_deduction, profTaxCalcVal, 'dfg');
                  ...
                  let Fixed_PF_deduction = totalshiftCalcVal > 0 && pfDaysVal > 0 ? ((Number(Fixed_Basic_CalcVal) / Number(totalshiftCalcVal)) * Number(pfDaysVal) * Number(pfAmount)).toFixed(2) : 0;

                  //    let Esiper = item.esideduction === true ? Number(findESIpercentage) / 100 : 0;

                  let Fixed_ESI_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (Fixed_NET_Salary1 / totalshiftCalcVal) * pfDaysVal * Esiper : 0;

                  let FixedfindprofTaxAmt = profTaxMaster.find((d) => d.company === item.company && d.branch === item.branch && Number(d.fromamount) <= Fixed_NET_Salary1 && Number(d.toamount) + 0.99 >= Fixed_NET_Salary1);
                  ...
                  let Fixed_ESI_EMPR_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (Fixed_NET_Salary1 / totalshiftCalcVal) * pfDaysVal * ESI_EMPR_Perncetage : 0;

                  let Fixed_Shift_Allowance = totalNoOfDaysCalcVal > 0 ? (1000 / tond) * totalNoOfDaysCalcVal : 0;
                  ...
                  let Fixed_Acutal_Deduction_Calc =
                    CHECK_DEPARTMENT_ACCESS && CHECK_DEPARTMENT_ACCESS.deduction === true ? (Number(totalPointsValueCalc) + Number(revenueAmountCalc) - (Number(Fixed_CTC_Calcval) + Number(Shortage1ValCalc)) < 0 ? Number(totalPointsValueCalc) + Number(revenueAmountCalc) - (Number(Fixed_CTC_Calcval) + Number(Shortage1ValCalc)) : 0) : 0;
                  ...
                  Fixed_NET_Salary = totalshiftCalcVal === 0 || grossValue === 0 ? 0 : (grossValue / totalshiftCalcVal) * totalpaiddaycalVal;
                  Fixed_Basic_CalcVal =
                    totalshiftCalcVal === 0
                      ? 0
                      : (actualBasicCalcVal / totalshiftCalcVal) * totalpaiddaycalVal === 0
                  ...
                  Fixed_PF_deduction = pfDaysVal > 0 && totalshiftCalcVal > 0 ? ((Number(Fixed_Basic_CalcVal) / Number(totalshiftCalcVal)) * Number(pfDaysVal) * Number(pfAmount)).toFixed(2) : 0;

                  Fixed_ESI_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (Fixed_NET_Salary1 / totalshiftCalcVal) * pfDaysVal * Esiper : 0;

                  findprofTaxAmt = profTaxMaster.find((d) => d.company == item.company && d.branch === item.branch && Number(d.fromamount) <= Fixed_NET_Salary1 && Number(d.toamount) + 0.99 >= Fixed_NET_Salary1);
                  ...
                  Fixed_ESI_EMPR_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (Fixed_NET_Salary1 / totalshiftCalcVal) * pfDaysVal * ESI_EMPR_Perncetage : 0;
                  ...
                  Fixed_Lop_Days = totalNoOfDaysCalcVal === 0 || totalpaiddaycalVal === 0 ? 0 : totalshiftCalcVal - (totalshiftCalcVal * Fixed_NET_Salary1) / grossValue < 0 ? 0 : grossValue > 0 ? totalshiftCalcVal - (totalshiftCalcVal * Fixed_NET_Salary1) / grossValue : 0;

                  Fixed_Leave_Dedcution = totalNoOfDaysCalcVal === 0 || totalpaiddaycalVal === 0 || grossValue === 0 ? 0 : (grossValue / totalshiftCalcVal) * (totalshiftCalcVal - (totalshiftCalcVal * Fixed_NET_Salary1) / grossValue < 0 ? 0 : totalshiftCalcVal - (totalshiftCalcVal * Fixed_NET_Salary1) / grossValue);
                  ...
                  // console.log(ERAAmountCalcVal, 'ERAAmountCalcVal1');
                  acutalERAValCalc = totalasbleaveCalcVal > 2 ? (ERAAmountCalcVal - (ERAAmountCalcVal / totalshiftCalcVal) * totalasbleaveCalcVal).toFixed(2) : Number(ERAAmountCalcVal);
                  // console.log(ERAAmountCalcVal, 'acutalERAValCalc1');
                  ...
                  //   &&(d.currentabsentmodes === "Less than or Equal" ? currentMonthAttendanceVal <= Number(d.currentabsentvalue) : d.currentabsentmodes === "Less than" ? currentMonthAttendanceVal < Number(d.currentabsentvalue) :  d.currentabsentmodes === "Greater than" ? currentMonthAttendanceVal > Number(d.currentabsentvalue) :  currentMonthAttendanceVal >=Number(d.currentabsentvalue))
                  //   &&(d.currentachievedmodes === "Less than or Equal" ? currentMonthAvgVal <= Number(d.currentachievedvalue) : d.currentachievedmodes === "Less than" ? currentMonthAvgVal < Number(d.currentachievedvalue) :  d.currentachievedmodes === "Greater than" ? currentMonthAvgVal > Number(d.currentachievedvalue) :  currentMonthAvgVal >= Number(d.currentachievedvalue))
                  ...
                  let PROD_PF_deduction = pfDaysVal > 0 && totalshiftCalcVal > 0 ? ((Number(PROD_BASIC_ValCalc) / Number(totalshiftCalcVal)) * Number(pfDaysVal) * Number(pfAmount)).toFixed(2) : 0;

                  // let Esiper = item.esideduction === true ? Number(findESIpercentage) / 100 : 0;

                  let PROD_ESI_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (PROD_NET_Salary1 / totalshiftCalcVal) * pfDaysVal * Esiper : 0;

                  let PRODfindprofTaxAmt = profTaxMaster.find((d) => d.company === item.company && d.branch === item.branch && Number(d.fromamount) <= PROD_NET_Salary1 && Number(d.toamount) + 0.99 >= PROD_NET_Salary1);
                  ...
                  let PROD_PF_Emper_deduction = PRODpfAmount1 > 0 ? (PROD_BASIC_ValCalc / totalshiftCalcVal) * pfDaysVal * PRODpfAmount1 : 0;
                  // // let pfEmployerDeductionCalc = PF_Emper_deduction
                  //ESI-DEDCUTION
                  // let ESI_EMPR_Perncetage = item.esideduction === true ? Number(findEmployerESIPercentage) / 100 : 0;

                  let PROD_ESI_EMPR_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (PROD_NET_Salary1 / totalshiftCalcVal) * pfDaysVal * ESI_EMPR_Perncetage : 0;
                  ...
                  let PROD_Shift_Allowance = shiftallowancetargetfinal > 0 ? ((1000 / tond) * noshiftlogvalfinal * (allowancepointCalcVal > 0 ? (allowancepointCalcVal * 100) / shiftallowancetargetfinal : 0)) / 100 : 0;
                  ...
                  let PROD_LOP_days = PROD_SALARY_Calcval < grossValue ? (PROD_SALARY_Calcval > 0 ? Math.round(totalshiftCalcVal - (PROD_SALARY_Calcval * totalshiftCalcVal) / grossValue) : totalshiftCalcVal) : 0;
                  ...
                  let PROD_Leave_Deduction = totalNoOfDaysCalcVal === 0 || totalpaiddaycalVal === 0 || grossValue === 0 ? 0 : (grossValue / totalshiftCalcVal) * (totalshiftCalcVal - (totalshiftCalcVal * PROD_NET_Salary1) / grossValue < 0 ? 0 : totalshiftCalcVal - (totalshiftCalcVal * PROD_NET_Salary1) / grossValue);
                  ...
                  PROD_ESI_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (PROD_NET_Salary1 / totalshiftCalcVal) * pfDaysVal * Esiper : 0;

                  PRODfindprofTaxAmt = profTaxMaster.find((d) => d.company === item.company && d.branch === item.branch && Number(d.fromamount) <= PROD_NET_Salary1 && Number(d.toamount) + 0.99 >= PROD_NET_Salary1);
                  ...
                  PROD_ESI_EMPR_deduction = grossValue >= findEsiMAXSalary || oldgross >= findEsiMAXSalary ? 0 : pfDaysVal > 0 && totalshiftCalcVal > 0 ? (PROD_NET_Salary1 / totalshiftCalcVal) * pfDaysVal * ESI_EMPR_Perncetage : 0;
                  ...
                  PROD_Shift_Allowance = shiftallowancetargetfinal > 0 ? ((1000 / tond) * noshiftlogvalfinal * (allowancepointCalcVal > 0 ? (allowancepointCalcVal * 100) / shiftallowancetargetfinal : 0)) / 100 : 0;
                  ...
                  PROD_LOP_days = PROD_SALARY_Calcval < grossValue ? (PROD_SALARY_Calcval > 0 ? Math.round(totalshiftCalcVal - (PROD_SALARY_Calcval * totalshiftCalcVal) / grossValue) : totalshiftCalcVal) : 0;
                  ...
                  PROD_Leave_Deduction = totalNoOfDaysCalcVal === 0 || totalpaiddaycalVal === 0 || grossValue === 0 ? 0 : (grossValue / totalshiftCalcVal) * (totalshiftCalcVal - (totalshiftCalcVal * PROD_NET_Salary1) / grossValue < 0 ? 0 : totalshiftCalcVal - (totalshiftCalcVal * PROD_NET_Salary1) / grossValue);
                  ...
                  paidStatusFix
                    .filter((data) => {
                      const isMonthAndDepartmentMatched = data.month.toLowerCase() === selectmonthname.toLowerCase() && data.department.includes(item.department) && data.year == selectedYear;

                      const isReasonStatusValid = item.reasondate >= findDate && (item.resonablestatus === "Absconded" || item.resonablestatus === "Terminate") ? data.frequency === "REJECT" : item.reasondate >= findDate && item.resonablestatus === "Releave Employee" ? data.frequency === "HOLD" : true;

                      const isValueAndPointValid = Number(data.fromvalue) <= Number(totalasbleaveCalcVal) && Number(data.tovalue) >= Number(totalasbleaveCalcVal) && Number(data.frompoint) <= Number(AcheivedPercentCalcVal) && Number(data.topoint) >= Number(AcheivedPercentCalcVal);
                  ...
                  // console.log(findSalaryTypeValue,acutalDeductionCalcVal,finalValueCalcVal,final_Value_PenaltyCalcval,minimumDeductionCalcVal,'final',-7629.176272197961 < 0 )
                  ...
                  return {
                    ...
                    // currentmonthavg: findPointsDetailsNxtMonth ? Number(findPointsDetailsNxtMonth.avgpoint).toFixed(2) : 0,
                    // currentmonthattendance:currentMonthAttendanceVal,
                    currentmonthavg: Number(currMonAvgFinalcalVal),
                    currentmonthattendance: currMonAttFinalcalVal,

                    paidstatus: paidStatusVal,
                    salarytype: findSalaryTypeValue,
                    deductiontype: findDeductionTypeVal,
                  };
                } catch (err) {
                  setIsActive(false);
                    ...
                    result.allResultsItems
                      ...
                      .map((item, index) => {
                        return {
                          ...
                          prodlop: item.prodlop ? Number(item.prodlop) : 0,
  ...
  const columnsData = [
    ...
    { title: "Holiday", dataKey: "holiday" },
    { title: "Total Absent/ Leave Shift", dataKey: "totalasbleave" },
    { title: "Total Paid Shift", dataKey: "totalpaidDays" },
    { title: "Gross", dataKey: "oldgross" },
    { title: "Basic", dataKey: "oldbasic" },
    { title: "HRA", dataKey: "oldhra" },
    { title: "Conveyance", dataKey: "oldconveyance" },
    ...
    { title: "Final Leave Deduction", dataKey: "finalleavededuction" },
    { title: "Professional Tax", dataKey: "professionaltax" },
    { title: "Total Deductions", dataKey: "totaldeductions" },
    { title: "UAN", dataKey: "uan" },
    { title: "IP Name", dataKey: "ipname" },
  ...
  const downloadPdf = (type) => {
    ...
    const totalPages = Math.ceil(columnsData.length / maxColumnsPerPage); // Calculate total pages needed

    for (let currentPage = 1; currentPage <= totalPages; currentPage++) {
      const startIdx = (currentPage - 1) * maxColumnsPerPage;
      const endIdx = Math.min(startIdx + maxColumnsPerPage, columnsData.length);

      const currentPageColumns = columnsData.slice(startIdx, endIdx);
      ...
      doc.autoTable({
        ...
        body: type === "filtered" ? paginatedData : rows,
      });
    ...
    doc.save("Consolidated Salary List.pdf");
  };

  // Excel
  const fileName = "Consolidated Salary List";

  //print...
  const componentRef = useRef();
  const handleprint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: "Consolidated Salary List",
    pageStyle: "print",
  });
  ...
  const columns = [
    ...
    { field: "department", headerName: "Department", flex: 0, width: 150, hide: !columnVisibility.department },
    { field: "company", headerName: "Company", flex: 0, width: 95, hide: !columnVisibility.company, ...headerStyle },
    { field: "branch", headerName: "Branch", flex: 0, width: 120, hide: !columnVisibility.branch, ...headerStyle },
    { field: "unit", headerName: "Unit", flex: 0, width: 80, hide: !columnVisibility.unit, ...headerStyle },
    { field: "team", headerName: "Team", flex: 0, width: 80, hide: !columnVisibility.team, ...headerStyle },
    { field: "designation", headerName: "Designation", flex: 0, width: 150, hide: !columnVisibility.designation, ...headerStyle },
    { field: "empcode", headerName: "Emp Code", flex: 0, pinned: "left", width: 120, filter: true, hide: !columnVisibility.empcode, ...headerStyle },

    { field: "employeename", headerName: "Employee Name", flex: 0, pinned: "left", width: 190, hide: !columnVisibility.employeename, ...headerStyle },
    { field: "aadharname", headerName: "Aadhar Name", flex: 0, width: 190, hide: !columnVisibility.aadharname, ...headerStyle },
    { field: "processcode", headerName: "Process Code", flex: 0, width: 140, hide: !columnVisibility.processcode, ...headerStyle },
    { field: "doj", headerName: "DOJ", flex: 0, width: 120, hide: !columnVisibility.doj, ...headerStyle },
    { field: "experienceinmonth", headerName: "Experience In Month", flex: 0, width: 105, hide: !columnVisibility.experienceinmonth, ...headerStyle },
    { field: "prodexp", headerName: "Prod Exp", flex: 0, width: 100, hide: !columnVisibility.prodexp, ...headerStyle },
    { field: "totalnumberofdays", headerName: "Total No.of Days", ...headerStyle, flex: 0, width: 90, hide: !columnVisibility.totalnumberofdays, ...headerStyle },
    { field: "totalshift", headerName: "Total Shift", flex: 0, width: 80, hide: !columnVisibility.totalshift, ...headerStyle },
    { field: "clsl", headerName: "C.L. / S.L.", flex: 0, width: 80, hide: !columnVisibility.clsl, ...headerStyle },
    { field: "weekoff", headerName: "Week Off", flex: 0, width: 80, hide: !columnVisibility.weekoff, ...headerStyle },
    { field: "holiday", headerName: "Holiday", flex: 0, width: 80, hide: !columnVisibility.holiday, ...headerStyle },
    { field: "totalasbleave", headerName: "Total Absent/ Leave Shift", flex: 0, width: 100, hide: !columnVisibility.totalasbleave, ...headerStyle },
    { field: "totalpaidDays", headerName: "Total Paid Shift", flex: 0, width: 80, hide: !columnVisibility.totalpaidDays, ...headerStyle },

    { field: "oldgross", headerName: "Gross", flex: 0, width: 100, hide: !columnVisibility.oldgross, ...headerStyle },
    { field: "oldbasic", headerName: "Basic", flex: 0, width: 100, hide: !columnVisibility.oldbasic, ...headerStyle },
    { field: "oldhra", headerName: "HRA", flex: 0, width: 100, hide: !columnVisibility.oldhra, ...headerStyle },
    { field: "oldconveyance", headerName: "Conveyance", flex: 0, width: 100, hide: !columnVisibility.oldconveyance, ...headerStyle },
    { field: "oldmedicalallowance", headerName: "Medical Allowance", flex: 0, width: 100, hide: !columnVisibility.oldmedicalallowance, ...headerStyle },
    { field: "oldproductionallowance", headerName: "Production Allowance", flex: 0, width: 105, hide: !columnVisibility.oldproductionallowance, ...headerStyle },
    { field: "oldproductionallowancetwo", headerName: "Production Allowance 2", flex: 0, width: 110, hide: !columnVisibility.oldproductionallowancetwo, ...headerStyle },
    { field: "oldotherallowance", headerName: "Other Allowance", flex: 0, width: 100, hide: !columnVisibility.oldotherallowance, ...headerStyle },

    { field: "newgross", headerName: "New Gross", flex: 0, width: 100, hide: !columnVisibility.newgross, ...headerStyle },
    { field: "actualbasic", headerName: "Actual Basic", flex: 0, width: 100, hide: !columnVisibility.actualbasic, ...headerStyle },
    { field: "actualhra", headerName: "Actual HRA", flex: 0, width: 100, hide: !columnVisibility.actualhra, ...headerStyle },
    { field: "actualconveyance", headerName: "Actual Conveyance", flex: 0, width: 100, hide: !columnVisibility.actualconveyance, ...headerStyle },
    { field: "actualmedicalallowance", headerName: "Actual Medical Allowance", flex: 0, width: 100, hide: !columnVisibility.actualmedicalallowance, ...headerStyle },
    { field: "actualproductionallowance", headerName: "Actual Production Allowance", flex: 0, width: 105, hide: !columnVisibility.actualproductionallowance, ...headerStyle },
    { field: "actualproductionallowancetwo", headerName: "Actual Production Allowance 2", flex: 0, width: 110, hide: !columnVisibility.actualproductionallowancetwo, ...headerStyle },
    { field: "actualotherallowance", headerName: "Actual Other Allowance", flex: 0, width: 100, hide: !columnVisibility.actualotherallowance, ...headerStyle },

    { field: "targetpoints", headerName: "Target Points", flex: 0, width: 100, hide: !columnVisibility.targetpoints, ...headerStyle },
    { field: "achievedpoints", headerName: "Achieved Points", flex: 0, width: 100, hide: !columnVisibility.achievedpoints, ...headerStyle },
    { field: "achieved", headerName: "Achieved %", flex: 0, width: 100, hide: !columnVisibility.achieved, ...headerStyle },
    { field: "achievedproductionallowance", headerName: "Achieved Production Allowance", flex: 0, width: 110, hide: !columnVisibility.achievedproductionallowance, ...headerStyle },
    { field: "actualnetsalary", headerName: "Actual Net Salary", flex: 0, width: 100, hide: !columnVisibility.actualnetsalary, ...headerStyle },
    { field: "lopbasic", headerName: "LOP Basic", flex: 0, width: 100, hide: !columnVisibility.lopbasic, ...headerStyle },
    { field: "lophra", headerName: "LOP HRA", flex: 0, width: 100, hide: !columnVisibility.lophra, ...headerStyle },
    { field: "lopconveyance", headerName: "LOP Conveyance", flex: 0, width: 110, hide: !columnVisibility.lopconveyance, ...headerStyle },
    { field: "lopmedicalallowance", headerName: "LOP Medical Allowance", flex: 0, width: 100, hide: !columnVisibility.lopmedicalallowance, ...headerStyle },
    { field: "lopproductionallowance", headerName: "LOP Production Allowance", flex: 0, width: 110, hide: !columnVisibility.lopproductionallowance, ...headerStyle },
    { field: "lopotherallowance", headerName: "LOP Other Allowance", flex: 0, width: 100, hide: !columnVisibility.lopotherallowance, ...headerStyle },
    { field: "lopnetsalary", headerName: "LOP Net Salary", flex: 0, width: 100, hide: !columnVisibility.lopnetsalary, ...headerStyle },
    { field: "prodbasic", headerName: "PROD Basic", flex: 0, width: 100, hide: !columnVisibility.prodbasic, ...headerStyle },
    { field: "prodhra", headerName: "PROD HRA", flex: 0, width: 100, hide: !columnVisibility.prodhra, ...headerStyle },
    { field: "prodconveyance", headerName: "PROD Conveyance", flex: 0, width: 105, hide: !columnVisibility.prodconveyance, ...headerStyle },
    { field: "prodmedicalallowance", headerName: "PROD Medical Allowance", flex: 0, width: 100, hide: !columnVisibility.prodmedicalallowance, ...headerStyle },
    { field: "prodproductionallowance", headerName: " PROD Production Allowance", flex: 0, width: 110, hide: !columnVisibility.prodproductionallowance, ...headerStyle },
    { field: "prodotherallowance", headerName: "PROD Other Allowance", flex: 0, width: 100, hide: !columnVisibility.prodotherallowance, ...headerStyle },
    { field: "attendancelop", headerName: "Attendance LOP", flex: 0, width: 100, hide: !columnVisibility.attendancelop, ...headerStyle },
    { field: "calculatednetsalary", headerName: "Calculated Net Salary", flex: 0, width: 100, hide: !columnVisibility.calculatednetsalary, ...headerStyle },
    { field: "actualpenaltyamount", headerName: "Actual Penalty Amount", flex: 0, width: 100, hide: !columnVisibility.actualpenaltyamount, ...headerStyle },
    { field: "penaltyamount", headerName: "Penalty Amount", flex: 0, width: 100, hide: !columnVisibility.penaltyamount, ...headerStyle },

    { field: "lossdeduction", headerName: "Loss Deduction", flex: 0, width: 100, hide: !columnVisibility.lossdeduction, ...headerStyle },
    { field: "otherdeduction", headerName: "Other Deduction", flex: 0, width: 100, hide: !columnVisibility.otherdeduction, ...headerStyle },

    { field: "finalbasic", headerName: "Final Basic", flex: 0, width: 100, hide: !columnVisibility.finalbasic, ...headerStyle },
    { field: "finalhra", headerName: "Final HRA", flex: 0, width: 100, hide: !columnVisibility.finalhra, ...headerStyle },
    { field: "finalconveyance", headerName: "Final Conveyance", flex: 0, width: 110, hide: !columnVisibility.finalconveyance, ...headerStyle },
    { field: "finalmedicalallowance", headerName: "Final Medical Allowance", flex: 0, width: 100, hide: !columnVisibility.finalmedicalallowance, ...headerStyle },
    { field: "finalproductionallowance", headerName: "Final Production Allowance", flex: 0, width: 110, hide: !columnVisibility.finalproductionallowance, ...headerStyle },
    { field: "finalotherallowance", headerName: "Final Other Allowance", flex: 0, width: 100, hide: !columnVisibility.finalotherallowance, ...headerStyle },
    { field: "finalnetsalary", headerName: "Final Net Salary", flex: 0, width: 100, hide: !columnVisibility.finalnetsalary, ...headerStyle },
    { field: "pfdays", headerName: "PF Days", flex: 0, width: 100, hide: !columnVisibility.pfdays, ...headerStyle },
    { field: "ncpdays", headerName: "NCP Days", flex: 0, width: 100, hide: !columnVisibility.ncpdays, ...headerStyle },
    { field: "pfdeduction", headerName: "PF Deduction", flex: 0, width: 100, hide: !columnVisibility.pfdeduction, ...headerStyle },
    { field: "esideduction", headerName: "ESI Deduction", flex: 0, width: 100, hide: !columnVisibility.esideduction, ...headerStyle },
    { field: "finallopdays", headerName: "Final-LOP ", flex: 0, width: 100, hide: !columnVisibility.finallopdays, ...headerStyle },
    { field: "paysliplop", headerName: "Final LOP Days", flex: 0, width: 100, hide: !columnVisibility.paysliplop, ...headerStyle },
    { field: "finalleavededuction", headerName: "Final Leave Deduction", flex: 0, width: 100, hide: !columnVisibility.finalleavededuction, ...headerStyle },
    { field: "professionaltax", headerName: "Professional Tax", flex: 0, width: 100, hide: !columnVisibility.professionaltax, ...headerStyle },
    { field: "totaldeductions", headerName: "Total Deductions", flex: 0, width: 105, hide: !columnVisibility.totaldeductions, ...headerStyle },
    { field: "uan", headerName: "UAN", flex: 0, width: 100, hide: !columnVisibility.uan, ...headerStyle },
    { field: "ipname", headerName: "IP Name", flex: 0, width: 140, hide: !columnVisibility.ipname, ...headerStyle },
    { field: "noallowanceshift", headerName: "No. Allowance Shift", flex: 0, width: 100, hide: !columnVisibility.noallowanceshift, ...headerStyle },
    { field: "shiftallowancepoint", headerName: "Shift Allowance Point", flex: 0, width: 100, hide: !columnVisibility.shiftallowancepoint, ...headerStyle },
    { field: "shiftallowancetarget", headerName: "Shift Allowance Target", flex: 0, width: 100, hide: !columnVisibility.shiftallowancetarget, ...headerStyle },
    { field: "nightshiftallowance", headerName: "Night Shift Allowance", flex: 0, width: 100, hide: !columnVisibility.nightshiftallowance, ...headerStyle },
    { field: "finalsalary", headerName: "Final Salary", flex: 0, width: 100, hide: !columnVisibility.finalsalary, ...headerStyle },
    { field: "finalsalarypenalty", headerName: "Final Salary + Penalty", flex: 0, width: 100, hide: !columnVisibility.finalsalarypenalty, ...headerStyle },
    { field: "totalpointsvalue", headerName: "Total Points Value", flex: 0, width: 100, hide: !columnVisibility.totalpointsvalue, ...headerStyle },
    { field: "era", headerName: "ERA", flex: 0, width: 100, hide: !columnVisibility.era, ...headerStyle },
    { field: "actualera", headerName: "Actual ERA", flex: 0, width: 100, hide: !columnVisibility.actualera, ...headerStyle },
    { field: "pfemployerdeduction", headerName: "PF Employer Deduction", flex: 0, width: 100, hide: !columnVisibility.pfemployerdeduction, ...headerStyle },
    { field: "esiemployerdeduction", headerName: "ESI Employer Deduction", flex: 0, width: 100, hide: !columnVisibility.esiemployerdeduction, ...headerStyle },
    { field: "ctc", headerName: "CTC", flex: 0, width: 100, hide: !columnVisibility.ctc, ...headerStyle },
    { field: "revenueallowance", headerName: "Revenue Allowance", flex: 0, width: 100, hide: !columnVisibility.revenueallowance, ...headerStyle },
    { field: "finalvalue", headerName: "Final Value", flex: 0, width: 100, hide: !columnVisibility.finalvalue, ...headerStyle },
    ...
    { field: "finalvaluepenalty", headerName: "Final Value-Penalty", flex: 0, width: 100, hide: !columnVisibility.finalvaluepenalty, ...headerStyle },
    { field: "shortage", headerName: "Shortage", flex: 0, width: 100, hide: !columnVisibility.shortage, ...headerStyle },
    { field: "shortageone", headerName: "Shortage 1", flex: 0, width: 100, hide: !columnVisibility.shortageone, ...headerStyle },
    { field: "actualdeduction", headerName: "Actual Deduction", flex: 0, width: 100, hide: !columnVisibility.actualdeduction, ...headerStyle },
    { field: "minimumdeduction", headerName: "Minimum Deduction", flex: 0, width: 100, hide: !columnVisibility.minimumdeduction, ...headerStyle },
    { field: "finalvaluereview", headerName: "Final Value Review", flex: 0, width: 120, hide: !columnVisibility.finalvaluereview, ...headerStyle },
    { field: "finalvaluestatus", headerName: "Final Value Status", flex: 0, width: 120, hide: !columnVisibility.finalvaluestatus, ...headerStyle },
    { field: "finalvaluepenaltystatus", headerName: "Final Value Penalty Status", flex: 0, width: 120, hide: !columnVisibility.finalvaluepenaltystatus, ...headerStyle },

    //FIXED
    { field: "fixedlossdeduction", headerName: "Fixed Loss Deduction", flex: 0, width: 100, hide: !columnVisibility.fixedlossdeduction, ...headerStyle },
    { field: "fixednetsalary", headerName: "Fixed NET Salary", flex: 0, width: 100, hide: !columnVisibility.fixednetsalary, ...headerStyle },
    { field: "fixedbasic", headerName: "Fixed Basic", flex: 0, width: 100, hide: !columnVisibility.fixedbasic, ...headerStyle },
    { field: "fixedhra", headerName: "Fixed HRA", flex: 0, width: 100, hide: !columnVisibility.fixedhra, ...headerStyle },
    { field: "fixedconveyance", headerName: "Fixed Conveyance", flex: 0, width: 100, hide: !columnVisibility.fixedconveyance, ...headerStyle },
    { field: "fixedmedicalallowance", headerName: "Fixed Medical Allowance", flex: 0, width: 100, hide: !columnVisibility.fixedmedicalallowance, ...headerStyle },
    { field: "fixedproductionallowance", headerName: "Fixed Production Allowance", flex: 0, width: 110, hide: !columnVisibility.fixedproductionallowance, ...headerStyle },
    { field: "fixedotherallowance", headerName: "Fixed Other Allowance", flex: 0, width: 100, hide: !columnVisibility.fixedotherallowance, ...headerStyle },
    { field: "fixednetsalaryone", headerName: "Fixed NET Salary1", flex: 0, width: 110, hide: !columnVisibility.fixednetsalaryone, ...headerStyle },
    { field: "fixedemppf", headerName: "Fixed Emp_pf", flex: 0, width: 100, hide: !columnVisibility.fixedemppf, ...headerStyle },
    { field: "fixedempesi", headerName: "Fixed Emp_Esi", flex: 0, width: 100, hide: !columnVisibility.fixedempesi, ...headerStyle },
    { field: "fixedempptax", headerName: "Fixed Emp_ptax", flex: 0, width: 100, hide: !columnVisibility.fixedempptax, ...headerStyle },
    { field: "fixedemprpf", headerName: "Fixed Empr_pf", flex: 0, width: 100, hide: !columnVisibility.fixedemprpf, ...headerStyle },
    { field: "fixedempresi", headerName: "Fixed Empr_Esi", flex: 0, width: 100, hide: !columnVisibility.fixedempresi, ...headerStyle },
    { field: "fixedshiftallowance", headerName: "Fixed Shift Allowance", flex: 0, width: 100, hide: !columnVisibility.fixedshiftallowance, ...headerStyle },
    { field: "fixedtotaldeductions", headerName: "Fixed Total Deductions", flex: 0, width: 110, hide: !columnVisibility.fixedtotaldeductions, ...headerStyle },
    { field: "fixedsalary", headerName: "Fixed Salary", flex: 0, width: 100, hide: !columnVisibility.fixedsalary, ...headerStyle },
    { field: "fixedsalarypenalty", headerName: "Fixed Salary + Penalty", flex: 0, width: 100, hide: !columnVisibility.fixedsalarypenalty, ...headerStyle },
    { field: "fixedlop", headerName: "Fixed-LOP", flex: 0, width: 100, hide: !columnVisibility.fixedlop, ...headerStyle },
    { field: "fixedlopdays", headerName: "Fixed LOP Days", flex: 0, width: 100, hide: !columnVisibility.fixedlopdays, ...headerStyle },
    { field: "fixedleavededuction", headerName: "Fixed Leave Deduction", flex: 0, width: 100, hide: !columnVisibility.fixedleavededuction, ...headerStyle },
    { field: "fixedctc", headerName: "Fixed CTC", flex: 0, width: 100, hide: !columnVisibility.fixedctc, ...headerStyle },
    { field: "fixedfinalvalue", headerName: "Fixed Final Value", flex: 0, width: 100, hide: !columnVisibility.fixedfinalvalue, ...headerStyle },
    { field: "fixedactualdeduction", headerName: "Fixed Actual Deduction", flex: 0, width: 100, hide: !columnVisibility.fixedactualdeduction, ...headerStyle },
    { field: "fixedminimumdeduction", headerName: "Fixed Minimum Deduction", flex: 0, width: 100, hide: !columnVisibility.fixedminimumdeduction, ...headerStyle },

    //PRODUCTION
    { field: "prodlossdeduction", headerName: "PROD Loss Deduction", flex: 0, width: 100, hide: !columnVisibility.prodlossdeduction, ...headerStyle },
    { field: "prodnetsalary", headerName: "PROD NET_Salary", flex: 0, width: 110, hide: !columnVisibility.prodnetsalary, ...headerStyle },
    { field: "prodbasicp", headerName: "PROD Basic", flex: 0, width: 100, hide: !columnVisibility.prodbasicp, ...headerStyle },
    { field: "prodhrap", headerName: "PROD HRA", flex: 0, width: 100, hide: !columnVisibility.prodhrap, ...headerStyle },
    { field: "prodconveyancep", headerName: "PROD Conveyance", flex: 0, width: 110, hide: !columnVisibility.prodconveyancep, ...headerStyle },
    { field: "prodmedicalallowancep", headerName: "PROD Medical Allowance", flex: 0, width: 100, hide: !columnVisibility.prodmedicalallowancep, ...headerStyle },
    { field: "prodproductionallowancep", headerName: "PROD Production Allowance", flex: 0, width: 110, hide: !columnVisibility.prodproductionallowancep, ...headerStyle },
    { field: "prodotherallowancep", headerName: "PROD Other Allowance", flex: 0, width: 100, hide: !columnVisibility.prodotherallowancep, ...headerStyle },
    { field: "prodnetsalaryonep", headerName: "PROD NET_Salary1", flex: 0, width: 110, hide: !columnVisibility.prodnetsalaryonep, ...headerStyle },
    { field: "prodemppf", headerName: "PROD Emp_pf", flex: 0, width: 90, hide: !columnVisibility.prodemppf, ...headerStyle },
    { field: "prodempesi", headerName: "PROD Emp_esi", flex: 0, width: 90, hide: !columnVisibility.prodempesi, ...headerStyle },
    { field: "prodempptax", headerName: "PROD Emp_ptax", flex: 0, width: 100, hide: !columnVisibility.prodempptax, ...headerStyle },
    { field: "prodemprpf", headerName: "PROD Empr_pf", flex: 0, width: 90, hide: !columnVisibility.prodemprpf, ...headerStyle },
    { field: "prodempresi", headerName: "PROD Empr_Esi", flex: 0, width: 90, hide: !columnVisibility.prodempresi, ...headerStyle },
    { field: "prodshiftallowance", headerName: "PROD Shift_Allowance", flex: 0, width: 100, hide: !columnVisibility.prodshiftallowance, ...headerStyle },
    { field: "prodtotaldeductions", headerName: "PROD Total Deductions", flex: 0, width: 110, hide: !columnVisibility.prodtotaldeductions, ...headerStyle },
    { field: "prodsalary", headerName: "PROD Salary", flex: 0, width: 100, hide: !columnVisibility.prodsalary, ...headerStyle },
    { field: "prodsalarypenalty", headerName: "PROD Salary+ Penalty", flex: 0, width: 100, hide: !columnVisibility.prodsalarypenalty, ...headerStyle },
    { field: "prodlopdays", headerName: "PROD LOP Days", flex: 0, width: 100, hide: !columnVisibility.prodlopdays, ...headerStyle },
    { field: "prodlop", headerName: "PROD LOP", flex: 0, width: 100, hide: !columnVisibility.prodlop, ...headerStyle },
    { field: "prodleavededuction", headerName: "PROD Leave Deduction", flex: 0, width: 100, hide: !columnVisibility.prodleavededuction, ...headerStyle },
    { field: "prodctc", headerName: "PROD CTC", flex: 0, width: 100, hide: !columnVisibility.prodctc, ...headerStyle },
    { field: "prodfinalvalue", headerName: "PROD Final Value", flex: 0, width: 100, hide: !columnVisibility.prodfinalvalue, ...headerStyle },
    { field: "prodactualdeduction", headerName: "PROD Actual Deduction", flex: 0, width: 100, hide: !columnVisibility.prodactualdeduction, ...headerStyle },
    { field: "prodminimumdeduction", headerName: "PROD Minimum Deduction", flex: 0, width: 100, hide: !columnVisibility.prodminimumdeduction, ...headerStyle },

    { field: "currentmonthavg", headerName: `Current (${monthNames[Number(selectedMonthNum) + 1 > 12 ? 0 : Number(selectedMonthNum)]}) Month Avg`, flex: 0, width: 110, hide: !columnVisibility.currentmonthavg, ...headerStyle },
    { field: "currentmonthattendance", headerName: `Current (${monthNames[Number(selectedMonthNum) + 1 > 12 ? 0 : Number(selectedMonthNum)]}) Month Attendance`, flex: 0, width: 110, hide: !columnVisibility.currentmonthattendance, ...headerStyle },
    { field: "paidstatus", headerName: "Paid Status", flex: 0, width: 150, hide: !columnVisibility.paidstatus, ...headerStyle },

    { field: "salarytype", headerName: "Salary Type", flex: 0, width: 150, hide: !columnVisibility.salarytype, ...headerStyle },
    { field: "deductiontype", headerName: "Deduction Type", flex: 0, width: 150, hide: !columnVisibility.deductiontype, ...headerStyle },
  ];
  ...
  return (
    <Box>
      <Headtitle title={"Consolidated Salary List"} />
      {/* ****** Header Content ****** */}
      <Typography sx={userStyle.HeaderText}> Manage Consolidated Salary List</Typography>
          ...
          <Box sx={userStyle.dialogbox}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>Add Consolidated Salary List</Typography>
                </Grid>
              </Grid>
              <Grid container spacing={2}>
      ...
      {isUserRoleCompare?.includes("lfinalsalarylist") && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid container spacing={2}>
              <Grid item xs={8} md={8} lg={12} sm={8}>
                <Typography sx={userStyle.importheadtext}>Consolidated Salary List</Typography>
              </Grid>
            </Grid>
...
export default ConsolidatedSalary;
