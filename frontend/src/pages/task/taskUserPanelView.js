    ...
    const handleUpdatesubmitTable = async (indexs, file) => {
        ...
        try {
            ...
            let res = await axios.put(
                `${SERVICE.SINGLE_TASKFORUSER_MULTER}/${ids}`,
                formData,
                {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                                                                            ...
                                                                            return (
                                                                                <StyledTableRow id={indexs}>
                                                                                    ...
                                                                                    <StyledTableCell>{
                                                                                        ...
                                                                                        />}</StyledTableCell>
                                                                                    <StyledTableCell>{
                                                                                        ((item?.reason && item?.files?.length > 0) && !item?.status) && <Button sx={buttonStyles.buttonsubmit} variant="contained" onClick={() => handleUpdatesubmitTable(indexs, item?.files)}>Add</Button>}</StyledTableCell>
                ...
                <DialogContent sx={{ minWidth: "750px", height: "850px" }}>
                    <Grid container spacing={2}>
                        <Grid item lg={12} md={12} sm={12} xs={12}>
                            ...
                            <div
                                onDragOver={combinedArray?.length > 0 ? (e) => e.preventDefault() : handleDragOver}
                                onDrop={combinedArray?.length > 0 ? (e) => e.preventDefault() : handleDrop}
                            >
