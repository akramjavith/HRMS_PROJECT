...
function Stockmaster() {
      ...
      try {
        ...
        setStockArray([
          ...stockArray,
          {
            ...
            totalbillamount: totalQuantityStock * stockmaster.rate,
          },
        ]);
  ...
  const sendEditRequest = async () => {
    try {
      let res = await axios.put(`${SERVICE.STOCKPURCHASE_SINGLE}/${maintenanceid}`, {
        ...
        uniqueId: stockmasteredit.uniqueId,
  ...
  const handleDownloadExcel = async (type) => {
    try {
      if (type === "filtered") {
        ...
        setLoader(true);
        // Send the POST request to get the Excel file
        let res_employee = await axios.post(
          SERVICE.EXCEL_DOWNLOAD_ASSET,
          {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
        ...
        if (res_employee && res_employee.data) {
          const blob = res_employee.data;
          // Log the response headers and check for valid content type
          // console.log("Response Headers: ", res_employee.headers);
  ...
  const handleDownloadExcelPDf = async (type) => {
    try {
      if (type === "filtered") {
        ...
        const dataWithSerial = rowDataTable;
  ...
  const handleCloseFilterOpenPdf = async () => {
    // let res_employee = await axios.get(SERVICE.EXCEL_DOWNLOAD_ASSET, {
    //   headers: {
    //     Authorization: `Bearer ${auth.APIToken}`,
    //   },
    // });

    // setOverallFilterdata(res_employee?.data?.stock?.length > 0 ?
  ...
  return (
    <Box>
      ...
      <Dialog
        ...
        fullWidth={true}
      >
        <VendorPopup setVendorAuto={setVendorAuto} handleCloseviewalertvendor={handleCloseviewalertvendor} sendDataToParent={handleDataFromChild} isLoading={isLoading} setIsLoading={setIsLoading} />
      </Dialog>
