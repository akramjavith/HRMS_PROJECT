...
exports.getLogDistributionDetails = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    console.log(assetmaterialcode, "assetmaterialcode")
    // Using aggregation to process and filter the data
    const result = await Employeeasset.find(
      {
...
const mergeChunksEmpDistribrution = async (fileName, totalChunks) => {
  const parentDir = path.join(__dirname, ".."); // C:/Users/backend
  const chunkDir = path.join(parentDir, "..", "..", "mergeChunks");
  const mergedFilePath = path.join(parentDir, "..", "..", "empdistribution_files");
...
exports.uploadChunkEmpDistribrution = [
  upload.single("file"),
  catchAsyncErrors(async (req, res, next) => {
    ...
    try {
      ...
      if (chunkNumber === totalChunks - 1) {
        await mergeChunksEmpDistribrution(fileName, totalChunks);
...
exports.getAllEmpDistribrutionTodoDelete = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    filenames.forEach((file) => {
      const parentDir = path.join(__dirname, ".."); // C:/Users/backend
      const filePath = path.join(parentDir, "..", "..", "empdistribution_files", file);
...
exports.getAllEmpDistribrutionTodoEditFetch = catchAsyncErrors(async (req, res, next) => {
  try {
    const { filename } = req.body;

    const parentDir = path.join(__dirname, ".."); // C:/Users/backend
    const filePath = path.join(parentDir, "..", "..", "empdistribution_files", filename);
    console.log(filePath, "filePath");
    if (!fs.existsSync(filePath)) {
