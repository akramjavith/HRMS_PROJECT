...
const hotInstance = new Handsontable(document.getElementById('hot'), {
  data: data,
  colHeaders: true,
  columns: [
    {}, // 0
    {}, // 1
    { renderer: hyperlinkRenderer }, // 2
    { renderer: hyperlinkRenderer }, // 3
    {}, // 4
    {}, // 5
    {}, // 6
    {}, // 7
    ...
    if (source === 'edit' || source === 'paste') {
      changes.forEach(([row, prop, oldValue, newValue]) => {
        if ([2, 3].includes(prop)) {
          const match = newValue.match(/<a.*?href="(.*?)"/i);
          if (match) {
            const url = match[1];
            const targetCol = prop === 2 ? 9 : 10;
            this.setDataAtCell(row, targetCol, url);
          }
        }
      });
    }
  ...
  //     hotInstance.destroy();
  //   };
  // }, [penaltyErrorUpload.type,document]);
...
const pasteHandler = (event) => {
  ...
  rows.forEach((row, rowOffset) => {
    row.forEach((cellValue, colOffset) => {
      ...
      const colHeader = hotInstance.getColHeader(targetCol);
      
      if (["errorfilename", "documentnumber"].includes(colHeader)) {
        // Set the display value
        hotInstance.setDataAtCell(targetRow, targetCol, cellValue);
        
        // Extract and store the original URL
        const originalUrl = extractOriginalUrl(cellValue);
        hotInstance.setCellMeta(targetRow, targetCol, 'hyperlink', {
