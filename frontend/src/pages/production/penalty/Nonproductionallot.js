...
function Nonproductionallot() {
    ...
    const fetchUserAllotBy = async () => {
        ...
        try {
            let res = await axios.post(`${SERVICE.NONPRODUCTIONUNITALLOT_PAGINATION}`, queryParams, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });

            setAssignedby(res?.data?.result?.map((item, index) => ({
                ...item,
                serialNumber: (page - 1) * pageSize + index + 1
            })));
            setTotalProjects(res?.data?.result?.length > 0 ? res?.data?.totalProjects : 0);
            setTotalPages(res?.data?.result?.length > 0 ? res?.data?.totalPages : 0);
            setPageSize((data) => {
                return res?.data?.result?.length > 0 ? data : 10;
            });
            setPage((data) => {
                return res?.data?.result?.length > 0 ? data : 1;
            });
                ...
                <Grid sx={{ display: 'flex' }}>
                    {isUserRoleCompare?.includes("enon-productionunitallot") && (
                        <Button sx={userStyle.buttonedit} onClick={() => {
                            ...
                            fetchNonProductionByEdit(params.data.id)
                            // fetchNonProductionUpdateBy(params.data.id)
