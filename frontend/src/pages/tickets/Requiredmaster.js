...
function RequiredMaster() {
  ...
  const addTodo = () => {
    ...
    if (duplicateDetails || duplicateDetailsPreValue) {
      ...
      setPopupContentMalert(`${addRequired.options === "Pre-Value"
  ...
  const addTodoEdit = () => {
    ...
    if (duplicateDetails) {
      ...
      setPopupContentMalert(`${editRequiredEdit.options === "Pre-Value"
        ? "Please Select Detail"
        : "Please Enter  Detail"}`);
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
  ...
  const fetchRequiredMaster = async () => {
    ...
    try {
      ...
      const answer = res_data.data.required?.length > 0 ? res_data.data.required?.map((item, index) => ({
        ...item,
        ...
        subsubcategory: item.subsubcategory
          ?.map((t, i) => `${i + 1 + ". "}` + t)
          .join("\n"),
        options: item.overalldetails.map((data) => data.options.toString()).map((t, i) => `${i + 1 + ". "}` + t).join("\n"),
        details: item.overalldetails.map((data) => data.details.toString())?.map((t, i) => `${i + 1 + ". "}` + t).join("\n"),
      })) : [];
      setRequiredMaster(answer);
    } catch (err) { setTypemastercheck(true); handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
  };
  ...
  const editSubmit = (e) => {
    ...
    if (categoryValueCateEdit?.length == 0) {
      ...
      setPopupContentMalert(`${editRequired.options === "Pre-Value"
        ? "Please Select Detail"
        : "Please Enter Detail"}`);
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  if (
                                    ...
                                    setPopupContentMalert(`${optionsTodo === "Pre-Value"
                                      ? "Please Select Detail"
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  if (
                                    ...
                                    setPopupContentMalert(`${optionsTodoEdit === "Pre-Value"
                                      ? "Please Select Detail"
                                      : "Please Enter Details"}`);
                                    setPopupSeverityMalert("info");
                                    handleClickOpenPopupMalert();
