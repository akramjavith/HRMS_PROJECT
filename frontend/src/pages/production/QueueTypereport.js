...
function QueueTypeReport() {
  ...
  let exportColumnNames = ['Project', 'Vendor', 'Date', 'Category', 'Type', 'Sub Category', 'ORate', 'Flag Count', 'Old Total', 'New Rate', 'New Total', 'Difference'];

  let exportRowValues = ['vendor', 'vendornew', 'formatteddate', 'filenameupdated', 'type', 'category', 'orate', 'flagcount', 'oldtotal', 'newrate', 'newtotal', 'difference'];
  //    today date fetching
  ...
  const fetchAllCategory = async (e) => {
    try {
      ...
      vendorall = vendorall.filter((item, index, self) => index === self.findIndex((t) => t.label === item.label));
  ...
  const handleCategoryChange = (options) => {
    const selectedValues = options.map((a) => a.value);
    fetchAllTypemaster(
      selectedProject.map((item) => item.value),
      selectedValues
    );
  ...
  const fetchBatchFilter = async (batchNum) => {
    ...
    try {
      let res_employee = await axios.post(SERVICE.PRODUCTIONUPLOAD_QUEUE_TYPE_MASTER, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        filename: selectedOptionsCategory.map((item) => item.value),
        category: selectedOptionsSubCategory.map((item) => item.value).includes('All') ? [] : selectedOptionsSubCategory.map((item) => item.value),
        // vendor: selectedVendor.map(item => item.value),
        project: selectedProject.map((item) => item.value),
      ...
      if (batchNum * 10000 > res_employee?.data?.totalProduction && res_employee?.data?.prodresult.length === 0) {
  ...
  const fetchBatch = async (batchNum) => {
    ...
    try {
      ...
      if ((batchNum * 10000 > res_employee?.data?.totalProduction && res_employee?.data?.prodresult.length === 0) || batchresultzero >= 3) {
        const filtered = res_employee?.data?.prodresult.filter((item) => item != null);
  ...
  const tableCellStyle = { wrapHeaderText: true, autoSizeStrategy: { type: 'fitGridWidth' } };
  ...
  const columnDataTable = [
    ...
    { field: 'vendor', headerName: 'Project', flex: 0, width: 140, hide: !columnVisibility.vendor, headerClassName: 'bold-header', ...tableCellStyle },
    { field: 'vendornew', headerName: 'Vendor', flex: 0, width: 100, hide: !columnVisibility.vendornew, headerClassName: 'bold-header', ...tableCellStyle },
    { field: 'filenameupdated', headerName: 'Category', flex: 0, width: 350, hide: !columnVisibility.filenameupdated, headerClassName: 'bold-header', ...tableCellStyle },
    { field: 'type', headerName: 'Type', flex: 0, width: 180, hide: !columnVisibility.type, headerClassName: 'bold-header', ...tableCellStyle },
    { field: 'category', headerName: 'SubCategory', flex: 0, width: 400, hide: !columnVisibility.category, headerClassName: 'bold-header', ...tableCellStyle },
    { field: 'orate', headerName: 'Orate', flex: 0, width: 100, hide: !columnVisibility.orate, headerClassName: 'bold-header', ...tableCellStyle },
    { field: 'flagcount', headerName: 'Flag Count', flex: 0, width: 100, hide: !columnVisibility.flagcount, headerClassName: 'bold-header', ...tableCellStyle },
    { field: 'oldtotal', headerName: 'Old Total', flex: 0, width: 100, hide: !columnVisibility.oldtotal, headerClassName: 'bold-header', ...tableCellStyle },
    { field: 'newrate', headerName: 'New Rate', flex: 0, width: 100, hide: !columnVisibility.newrate, headerClassName: 'bold-header', ...tableCellStyle },
    { field: 'newtotal', headerName: 'New Total', flex: 0, width: 100, hide: !columnVisibility.newtotal, headerClassName: 'bold-header', ...tableCellStyle },
    { field: 'difference', headerName: 'Difference', flex: 0, width: 150, hide: !columnVisibility.difference, headerClassName: 'bold-header', ...tableCellStyle },
  ];
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Queue Type Report" modulename="Production" submodulename="SetUp" mainpagename="Queue Type Report" subpagename="" subsubpagename="" />
      <>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth>
                  ...
                  <MultiSelect options={categoryOpt} value={selectedOptionsCategory} onChange={handleCategoryChange} valueRenderer={customValueRendererCategory} labelledBy="Please Select Category" />
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <MultiSelect options={typeMaster} value={selectedTypeFrom} onChange={handleTypeChangeFrom} valueRenderer={customValueRendererTypeFrom} labelledBy="Please Select Type" />
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth>
                  ...
                  <MultiSelect options={subcategory} value={selectedOptionsSubCategory} onChange={handleSubCategoryChange} valueRenderer={customValueRendererSubCategory} labelledBy="Please Select SubCategory" />
                </FormControl>
              </Grid>
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth size="small">
