...
function RejoinEmployeeList() {
    ...
    const { isUserRoleAccess, isUserRoleCompare, isAssignBranch, pageName, setPageName, buttonStyles, allUsersData, allTeam } = useContext(UserRoleAccessContext);
    ...
    const isChecklistAssigned = async () => {
        try {
            ...
            const isAvailable = res?.data?.checklistverificationmasters?.some((item) => item.subsubpage === 'Rejoined Employee List');
            if (isAvailable) {
                setIsAssigned(true);
            } else {
                setIsAssigned(false);
                ...
                $and: [
                    // Enquiry status filter
                    {
                        $or: [
                            { resonablestatus: 'Rejoined' },
                            {
                                $and: [{ rejoineddetails: { $exists: true, $ne: [] } }, { rejoineduser: { $exists: true, $ne: '' } }, { successfullyrejoined: { $exists: false } }],
                            },
                        ],
                    },
                    // {
    ...
    const deleteChecklist = async () => {
        setDeleteBtn(true);
        const searchItem = datasAvailedDB.find((item) => item.commonid === postID && item.module === 'Human Resources' && item.submodule === 'HR' && item.mainpage === 'Employee' && item.subpage === 'Employee Action Details' && item.subsubpage === 'Rejoined Employee List');
        setPageName(!pageName);
        try {
    ...
    const sendRequestReason = async () => {
        ...
        try {
            if (isAssigned) {
                if (searchItem) {
                    let assignbranches = await axios.put(`${SERVICE.MYCHECKLIST_SINGLE}/${searchItem?._id}`, {
                        headers: {
                            Authorization: `Bearer ${auth.APIToken}`,
                        },
            ...
            const removeExistingUser = async () => {
                await axios.post(`${SERVICE.REMOVEOVERALLEMPLOYEENAME}`, {
                    companyname,
                });
            };
  ...
  `;
    ...
    return (
        <Box>
            ...
            <PageHeading title="Rejoined Employee List" modulename="Human Resources" submodulename="HR" mainpagename="Employee" subpagename="Employee Action Details" subsubpagename="Rejoined Employee List" />
