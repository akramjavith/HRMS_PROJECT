...
function TrainingForUserOnProgress({ com }) {
    ...
    const downloadPdf = (isfilter) => {
        ...
        doc.save("User_Training _Panel-Completed.pdf");
    };
    ...
    // Excel
    const fileName = "User_Training_Panel-OnProgress";
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "User_Training_Panel-OnProgress",
        pageStyle: "print",
    });
    ...
    const handleCaptureImage = () => {
        if (gridRef.current) {
            html2canvas(gridRef.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "User_Training_Panel-OnProgress.png");
                });
            });
        }
    };
    ...
    const ListPageLoadDataOnProgress = async () => {
        try {
            let res_task = await axios.post(SERVICE.ALL_TRAINING_FOR_USER_ONPROGRESS, {
                headers: {
            ...
            setRaiseTicketList(res_task?.data?.trainingforuser.filter(data => data?.username
                === isUserRoleAccess?.companyname && [ "Paused" , "Pending"]?.includes(data?.taskstatus) && data.taskdetails !== "Manual"))
            setQueueCheck(true);
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>User Training Panel - OnProgress Tasks</Typography>
...
export default TrainingForUserOnProgress;
