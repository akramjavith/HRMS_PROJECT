...
function NotPaidlist() {
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Not Paid Report.png");
        });
      });
    }
  };
  // Excel
  const fileName = "Not Paid Report";

  //print...
  const componentRef = useRef();
  const handleprint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: "Not Paid Report",
    pageStyle: "print",
  });
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Not Paid Report"),
    ...
    try {
      ...
      if (filterUser.source === "Expense") {
        ...
        otherMappedData = res.data.finaldata.map((data) => ({
          ...
          billstatus: data.paidbillstatus,
        }));
      }
  ...
  return (
    <Box>
      <Headtitle title={"NOT PAID REPORT"} />
      <PageHeading
        title="Not Paid Report"
        modulename="Expenses"
        submodulename="Not Paid Report"
        mainpagename=""
        subpagename=""
        subsubpagename=""
      />
      {isUserRoleCompare?.includes("lnotpaidreport") && (
        <>
          ...
          <>
            <Box sx={userStyle.container}>
              ...
              <Grid container spacing={2} style={userStyle.dataTablestyle}>
                ...
                >
                  <Box>
                    {isUserRoleCompare?.includes("excelnotpaidreport") && (
                      <>
                        <Button
                          onClick={(e) => {
                    ...
                    {isUserRoleCompare?.includes("csvnotpaidreport") && (
                      <>
                        <Button
                          onClick={(e) => {
                            setIsFilterOpen(true);
                    ...
                    {isUserRoleCompare?.includes("printnotpaidreport") && (
                      <>
                    ...
                    {isUserRoleCompare?.includes("pdfnotpaidreport") && (
                      <>
                        <Button
                          sx={userStyle.buttongrp}
                          onClick={() => {
                    ...
                    {isUserRoleCompare?.includes("imagenotpaidreport") && (
                      <>
                        <Button
                          sx={userStyle.buttongrp}
