...
function TodayLeaveApproved() {
  ...
  const { isUserRoleCompare, isAssignBranch, allProjects, isUserRoleAccess,listPageAccessMode} = useContext(UserRoleAccessContext);
  ...
  const handleCheckListSubmit = async () => {
    ...
    if (!nextStep) {
      setShowAlert(
        <>
          <ErrorOutlineOutlinedIcon sx={{ fontSize: '100px', color: 'orange' }} />
          <p style={{ fontSize: '20px', fontWeight: 900 }}>{'Please Check All the Fields!'}</p>
        </>
      );
      handleClickOpenerr();
    } else {
    ...
    try {
      ...
      let answer =
        isUserRoleAccess?.role?.includes('Manager') || isUserRoleAccess?.role?.includes('HiringManager') || isUserRoleAccess?.role?.includes('HR') || isUserRoleAccess?.role?.includes('Superadmin')
          ...
          : res_vendor?.data?.applyleaves.filter((data) => data.employeename === isUserRoleAccess.companyname);
  ...
  const fetchApplyleave = async () => {
    try {
      ...
      let res_vendor = await axios.post(SERVICE.APPLY_LEAVE_HOME_LIST_TEAM, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
hierarchyempnames: [isUserRoleAccess.companyname],
      ...
      let Approve =
        isUserRoleAccess?.role?.includes('Manager') || isUserRoleAccess?.role?.includes('HiringManager') || isUserRoleAccess?.role?.includes('HR') || isUserRoleAccess?.role?.includes('Superadmin')
          ? res_vendor?.data?.applyleaves.filter((data) => data.status === 'Approved')
          : res_vendor?.data?.applyleaves.filter((data) => data.employeename === isUserRoleAccess.companyname && data.status === 'Approved');
  ...
  return (
    <Box>
      ...
      <PageHeading title="Today Leave Approved" modulename="Dashboard" submodulename="Employee" mainpagename="Today Leave" subpagename="" subsubpagename="" />
                        ...
                        <Grid container spacing={1.2} marginTop={1}>
                          ...
                          <Grid item md={1} sm={12} xs={12} marginTop={1}>
                            <Button
                              ...
                              color="success"
                              type="button"
                              sx={{ height: '30px', minWidth: '30px', padding: '6px 10px', marginTop: '4px' }}
                              onClick={(e) => {
                        ...
                        <Grid container spacing={1.2} marginTop={1}>
                                ...
                                onChange={(e) => {
                                  if (leaveRestrictionEdit === true) {
                                    ...
                                    setAppleaveEdit({
                                      ...appleaveEdit,
                                      todate:
                                        isUserRoleAccess.role.includes('SuperAdmin') || isUserRoleAccess.role.includes('Manager')
                                          ? new Date(e.target.value) >= new Date(appleaveEdit.date)
                                            ? e.target.value
                                            : ''
                                          : (new Date(e.target.value) - new Date(DateFrom)) / (1000 * 3600 * 24) < 0
                                            ? ''
                                            : new Date(e.target.value) >= new Date(appleaveEdit.date)
                                              ? e.target.value
                                              : '',
                          ...
                          <Grid item md={0.5} sm={12} xs={12} marginTop={1}>
                            <Button
                              ...
                              color="success"
                              type="button"
                              sx={{ height: '30px', minWidth: '30px', padding: '6px 10px', marginTop: '-2px' }}
                              onClick={(e) => {
                                if (AccessdropEdit === 'HR' && appleaveEdit.employeename === 'Please Select Employee Name') {
                                  setShowAlert(
                                    <>
                            ...
                            <React.Fragment key={index}>
                              ...
                              <Grid item md={4} sm={6} xs={12} fullWidth>
                                <Box sx={{ display: 'block', margin: '0.5px' }}>
                                  <Selects size="small" options={leaveStatusOptions} styles={colourStyles} value={leaveStatusOptions.find((option) => option.value === column.leavestatus)} onChange={(selectedOption) => multiLeaveStatusInputsEdit(index, selectedOption.value)} />
                                </Box>
                              </Grid>
                              <Grid item md={2} sm={6} xs={12} fullWidth>
                                <Box sx={{ display: 'block', margin: '0.5px' }}>
                                  <OutlinedInput id="component-outlined" type="text" value={column.shiftcount} />
                                </Box>
                              </Grid>
                              <Grid item md={1} sm={6} xs={1} sx={{ display: 'flex' }}>
                                <Button variant="contained" color="error" type="button" onClick={(e) => handleDeleteEdit(index)} sx={{ height: '30px', minWidth: '30px', marginTop: '4px', padding: '6px 10px' }}>
                                  x
                                </Button>
                              </Grid>
                            </React.Fragment>
                ...
                <>
                  <Grid item md={3} xs={12} sm={6}>
                    <FormControl fullWidth size="small">
                      ...
                      <Typography>{Array.isArray(selectedOptionsCompanyEdit) ? selectedOptionsCompanyEdit.map((item, index) => `${index + 1}. ${item.value} `) : ''}</Typography>
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={6}>
                    <FormControl fullWidth size="small">
                      <Typography variant="h6"> Branch</Typography>
                      <Typography>{Array.isArray(selectedOptionsBranchEdit) ? selectedOptionsBranchEdit.map((item, index) => `${index + 1}. ${item.value} `) : ''}</Typography>
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={6}>
                    <FormControl fullWidth size="small">
                      <Typography variant="h6"> Unit </Typography>
                      <Typography>{Array.isArray(selectedOptionsUnitEdit) ? selectedOptionsUnitEdit.map((item, index) => `${index + 1}. ${item.value} `) : ''}</Typography>
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={6}>
                    <FormControl fullWidth size="small">
                      <Typography variant="h6"> Team </Typography>
                      <Typography>{Array.isArray(selectedOptionsTeamEdit) ? selectedOptionsTeamEdit.map((item, index) => `${index + 1}. ${item.value} `) : ''}</Typography>
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12}>
                    <FormControl fullWidth size="small">
