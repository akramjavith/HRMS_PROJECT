...
import {
    Box, Button, Checkbox, Dialog, DialogActions, InputAdornment,
    DialogContent, Divider, FormControl, Grid, Tooltip,
    IconButton, List, ListItem, ListItemText, MenuItem, OutlinedInput, RadioGroup,
    Popover, Select, TextField, Typography, Radio, FormControlLabel,
...
function PenaltyErrorUpload() {
    ...
    const fetchEmployeeSearch = async () => {
        ...
        try {
            ...
            setTargetPointsOverallExports(res_employee?.data?.totalProjectsAllData?.length > 0 ? res_employee?.data?.totalProjectsAllData?.map((item, index) => ({
                ...item,
                serialNumber: index + 1
            })) : []);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const isNameMatch = targetPointsForDuplicate?.some(
            (item) =>
                ...
                <List sx={{ overflow: "auto", height: "100%" }}>
                    {filteredColumnsFilename.map((column) => (
                        <ListItem key={column.field}>
                            <ListItemText
                                sx={{ display: "flex" }}
                                primary={<Switch sx={{ marginTop: "-5px" }} size="small" checked={columnVisibilityFilename[column.field]} onChange={() => toggleColumnVisibilityFilename(column.field)} />}
                                secondary={column.field === "checkbox" ? "Checkbox" : column.headerName}
                            />
                        </ListItem>
                    ))}
    ...
    const readExcel = (file) => {
        ...
        if (!acceptedExtensions.includes(fileExtension) && file !== null) {
                    ...
                    fileReader.onload = (e) => {
                        ...
                        const parseAndFormatDate = (dateValue) => {
                            if (dateValue instanceof Date) {
                                ...
                                let parsedDate = moment(dateValue, [
                                    "DD-MM-YYYY", "DD.MM.YYYY", "YYYY-MM-DD", "DD/MM/YYYY", "YYYY/MM/DD",
                                    "D-MM-YYYY", "D.MM.YYYY", "YYYY-MM-D", "D/MM/YYYY",
                                    "DD-M-YYYY", "DD.M.YYYY", "YYYY-M-DD", "DD/M/YYYY",
                                    "D-M-YYYY", "D.M.YYYY", "YYYY-M-D", "D/M/YYYY"
                                ], true);
            ...
            promise.then((d) => {
                ...
                else if (uniqueArrayfinal.length !== d.length) {
                    setPopupContentMalert(
                        uniqueArrayfinal.length != d.length
                            ? ` Duplicate data and Points field Not a number data's are Removed`
                            : uniqueArrayfinal.length != d.length
                                ? `${d.length - uniqueArrayfinal.length}  Duplicate or data Removed`
                                : ` Data's Points field is Not a Number Removed`
                    );
                    setPopupSeverityMalert("info");
                    handleClickOpenPopupMalert();
                }
    ...
    const sendJSON = async () => {
        ...
        if (selectedSheet === "Please Select Sheet") {
            ...
            let uniqueArrayfinalDup = uploadExceldata.filter(
                (item, index, self) =>
                    index ===
                    self.findIndex(
                        (tp) =>
                    ...
                    {isUserRoleCompare?.includes("lerrorupload") && (
                        <>
                                ...
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item lg={1} md={1} sm={1} xs={6} >
                                        <Box sx={(theme) => ({ mt: { lg: 3, md: 2, sm: 1, xs: 0, } })}>
                                            <LoadingButton onClick={handleFilterSubmitOne} loading={loadingdeloverallOne} sx={buttonStyles.buttonsubmit} loadingPosition="end" variant="contained" >Filter</LoadingButton>
                                        </Box>
                                    </Grid>
                                    <Grid item lg={1} md={1} sm={1} xs={6} >
                                        <Box sx={(theme) => ({ mt: { lg: 3, md: 2, sm: 1, xs: 0, } })}>
                                            <Button onClick={handleFilterClearOne} sx={buttonStyles.btncancel}>Clear</Button>
                                        </Box>
                                    </Grid>
                                </Grid>
                            </Box> <br />
                            <Box sx={userStyle.container}>
                                ...
                                <Button sx={userStyle.buttongrp} onClick={handleShowAllColumnsFilename}>Show All Columns</Button>&ensp;
                                <Button sx={userStyle.buttongrp} onClick={handleOpenManageColumnsFilename}>Manage Columns</Button>
                                <Popover
                                    id={id}
                            ...
                            <Box sx={userStyle.container}>
                                ...
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item md={1} xs={12} sm={12} marginTop={3}>
                                        <Button variant="contained" sx={buttonStyles.buttonsubmit} onClick={handleFilterSubmitTwo} loading={loadingdeloverallTwo}>Filter</Button>
                                    </Grid>
                                    <Grid item md={0.5} xs={12} sm={12} marginTop={3}>
                                        <Button onClick={handleFilterClearTwo} sx={buttonStyles.btncancel}>Clear</Button>
                                    </Grid>
                                </Grid>  <br />
                                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                                    <Grid item md={2} xs={12} sm={12}>
