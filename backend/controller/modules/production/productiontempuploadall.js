    ...
    if (req.body.shift == 'Shift Based') {
      try {
        let mergedDataallfirst = allData.map((upload, index) => {
          const loginInfo = filteredLogs.find((login) => login.userid === upload.user && login.projectvendor === upload.vendor);
      ...
      .flatMap((item) => {
        ...
        function filterData(data) {
          ...
          if (isFirstShiftWeekoff && isSecondShiftWeekoff) {
            ...
            data = { shiftFromTime, shiftEndTime, shiftsts: 'Disable', shifttiming: firstEntry.shifttiming, shiftOnlyFromTime, shiftOnlyEndTime };
...
//overall
exports.getTempOverallAttendanceBasedClaim = catchAsyncErrors(async (req, res, next) => {
  ...
  const  date  = req.body.fromdate;
    ...
    isDayPointsCreated = await DayPointsUploadTemp.countDocuments({ date: date });
...
//individual
exports.getTempIndividualAttendanceBasedClaim = catchAsyncErrors(async (req, res, next) => {
  let finalProductions = [],isDayPointsCreated=0;
    let mergedData = [];
  try {
  const  date  = req.body.fromdate;
...
exports.getAllTempHierarchyListAttendanceBasedClaim = catchAsyncErrors(async (req, res, next) => {
...
exports.getAllTempTeamAttendanceBasedClaim = catchAsyncErrors(
  async (req, res, next) => {
    let result,
