...
function PaymentDueReminder() {
    ...
    if (isfilter === "filtered") {
      ...
      exportToCSV(
        documentsList?.map((item, index) => ({
          "S.No": index + 1,
          Date: item.currdate?.join(","),
          "Bill No": item.billno?.join(","),
          "Vendor Name": item.vendor,
          Frequency: item.vendorfrequency,
  ...
  const downloadPdf = (isfilter) => {
    ...
    const dataWithSerial =
      isfilter === "filtered"
        ...
        : documentsList?.map((item, index) => ({
            serialNumber: index + 1,
            currdate: item.currdate?.join(","),
            billno: item.billno?.join(","),
            vendor: item.vendor,
            vendorfrequency: item.vendorfrequency,
            source: item.source?.join(","),
            amount: item.amount,
    ...
    doc.save("Payment Due Reminder.pdf");
  };
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Payment Due Reminder.png");
        });
      });
    }
  };
  ...
  const handleFilterClick = () => {
    if (frequencyValue === "") {
      ...
      if (frequencyValue === "Daily") {
        sendRequest(frequencyValue, dailyDate);
      } else if (frequencyValue === "Weekly") {
        sendRequest(frequencyValue, weeklyDate);
      } else {
        sendRequest(frequencyValue, periodState?.month, periodState?.year);
      }
    }
  };

  const sendRequest = async (e, filterdates, filteryear) => {
    try {
      setLoading(false);
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Payment Due Reminder",
    pageStyle: "print",
  });
      ...
      renderCell: (params) => (
        <Grid sx={{ display: "flex", padding: "10px" }}>
          ...
          {isUserRoleCompare?.includes("lpaymentduereminder") && (
            <>
              {Array.isArray(params.row.checklog) &&
                params?.row?.checklog.length > 1 && (
                  <Link
                    to={`/expense/paymentduereminderlog/${
                      params.row.filteredfrom
                    }/${params.row.vendorid}/${
                      params?.row?.filterdates === "" ||
                      params?.row?.filterdates == undefined
                        ? "NODATE"
                        : params?.row?.filterdates
                    }/${
                      params?.row?.filteryear === "" ||
                      params?.row?.filteryear == undefined
                        ? "NOYEAR"
                        : params?.row?.filteryear
                    }`}
                    style={{
  ...
  return (
    <Box>
      <Headtitle title={"PAYMENT DUE LIST"} />
      {/* ****** Header Content ****** */}

      <PageHeading
        title="Manage Payment Due Reminder List"
        modulename="Expenses"
        submodulename="Remainder"
        mainpagename="Payment Due Reminder"
        subpagename=""
        subsubpagename=""
      />
          ...
          <>
            <Box sx={userStyle.container}>
              ...
              <Grid container spacing={1}>
                ...
                <Grid
                  item
                  md={frequencyValue === "Months" ? 1.5 : 3}
                  xs={12}
                  sm={12}
                >
