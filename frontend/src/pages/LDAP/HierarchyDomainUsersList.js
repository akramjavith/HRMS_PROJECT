...
function DomainUsersList() {
  ...
  const unLockUserOne = async (username) => {
    setPageName(!pageName);
    setIsLoadingNew(true);
    try {
      let res = await axios.post(
        SERVICE.UNLOCKDOMAINUSER,
        {
          username: username,
        },
        {
  ...
  const lockUserOne = async (username) => {
    setPageName(!pageName);
    setIsLoadingNew(true);
    try {
      let res = await axios.post(
        SERVICE.LOCKDOMAINUSER,
        {
          username: username,
        },
        {
  ...
  const [isEditOpenReset, setIsEditOpenReset] = useState(false);
  const [editDetailsReset, setEditDetailsReset] = useState({});
  const [oldDetailsReset, setOldDetailsReset] = useState({});
  const handleClickOpenEditReset = () => {
    setIsEditOpenReset(true);
  };
  const handleCloseModEditReset = (e, reason) => {
    if (reason && reason === "backdropClick") return;
    setIsEditOpenReset(false);
    setPasswordStrength("");
  };
  ...
  const deleteFunction = async () => {
    ...
    try {
      setIsLoading(true);
      await axios.post(`${SERVICE.DELETEDOMAINSINGLEUSER}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Hierarchy Domain Users List"),
  ...
  const fetchRockeChatRoles = async () => {
    try {
      let response = await axios.get(SERVICE.ALL_ORGANIZATIONALUNITDB, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });

      setOUOptions(response?.data?.organizationalUnit);
    } catch (err) {
      handleApiError(
        err,
        setPopupContentMalert,
  ...
  const [ounewOptions, setOUNewOptions] = useState([]);
  const handleRocketchatRoleChangeAdd = (event) => {
    let filteredDatas = ouOptions
      ?.filter((item) => item?.type === event?.value)
      ?.map((data) => ({
        label: data?.name,
        value: data?.name,
      }));
    setOUNewOptions(filteredDatas);
    setAddDetails((prev) => ({ ...prev, type: event.value }));
  };

  const customValueRendererRocketchatRoleAdd = (
    valueRocketchatTeamCat,
    _categoryname
  ) => {
    return valueRocketchatTeamCat?.length
  ...
  const handleCaptureImage = () => {
    if (gridRefTableImg.current) {
      domtoimage
        ...
        .then((blob) => {
          saveAs(blob, "Hierarchy Domain Users List.png");
        })
        .catch((error) => {
  ...
  const renderWho = (who, status) => {
    ...
    if (who === "Employee" && status === "Matched") {
      icon = <WorkIcon {...iconProps} />;
      color = "#1976d2"; // Blue
    } else if (who === "Employee" && status === "Mismatched") {
      icon = <InfoIcon {...iconProps} />;
      color = "#ccc"; // Default gray
    } else {
    ...
    return (
      <Tooltip title={who} arrow>
        ...
        >
          ...
          >
            {who === "Employee" && status === "Matched"
              ? who
              : who === "Employee" && status === "Mismatched"
                ? "Not Mentioned"
                : who}
          </Typography>
        </Button>
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Hierarchy Domain Users List",
    pageStyle: "print",
  });
  ...
  const columnDataTable = [
    ...
    },
    {
      ...
      cellRenderer: (params) => renderAccountStatus(params?.data?.matchstatus),
      hide: !columnVisibility.matchstatus,
      pinned: "left",
    },
    ...
    },
    {
      ...
      cellRenderer: (params) =>
        renderWho(params?.data?.employeementtype, params?.data?.matchstatus),
...

    {
      ...
      cellRenderer: (params) => (
        ...
        //   {isUserRoleCompare?.includes("edomainuserslist") && (
        //     <Button
        ...
        //     </Button>
        //   )}
        //   {isUserRoleCompare?.includes("ddomainuserslist") && (
        //     <Button
        ...
        //     </Button>
        //   )}
        //   {isUserRoleCompare?.includes("vdomainuserslist") && (
        //     <Button
        //       sx={userStyle.buttonedit}
        //       onClick={() => {
        ...
        //   <br />
        //   {isUserRoleCompare?.includes("edomainuserslist") &&
        //     (params?.data?.accounttype === "Account Disabled" ? (
        //       <>
        //         <Button
        //           color="success"
        //           sx={userStyle.buttonedit}
        ...
        <Stack direction="row" spacing={1} alignItems="center">
          {/* {isUserRoleCompare?.includes("edomainuserslist") && (
            <Tooltip title="Edit User">
              <IconButton
                sx={userStyle?.iconButton}
                onClick={() => {
                  setEditDetails({ ...params?.data });
                  setOldDetails({ ...params?.data });
                  handleClickOpenEdit();
                }}
              >
          ...
          {isUserRoleCompare?.includes("dhierarchydomainuserslist") && (
          ...
          {isUserRoleCompare?.includes("vhierarchydomainuserslist") && (
            <Tooltip title="View Details">
              ...
              >
                <VisibilityOutlined sx={{ color: "#0288d1" }} />
              </IconButton>
            </Tooltip>
          )}

          {isUserRoleCompare?.includes("ehierarchydomainuserslist") && (
            <Tooltip title="Reset Password">
              <Button
                ...
                onClick={() => {
                  setEditDetailsReset({ ...params?.data });
                  setOldDetailsReset({ ...params?.data });
                  handleClickOpenEditReset();
                }}
              >
                Reset Password
              </Button>
  ...
  const editSubmit = (e) => {
    ...
    if (!editDetails.username) {
      ...
      handleClickOpenPopupMalert();
    } else if (!editDetails.cn) {
      setPopupContentMalert("Please Enter CN!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (!editDetails.sn) {
      setPopupContentMalert("Please Enter SN!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else {
      sendEditRequest();
  ...
  const editSubmitReset = (e) => {
    e.preventDefault();

    if (!editDetailsReset.password) {
      setPopupContentMalert("Please Enter Password!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else {
      sendEditRequestReset();
    }
  };
  const handleSubmitOld = (e) => {
    ...
    if (!addDetails.cn) {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (addDetails.type === "Please Select Type") {
      setPopupContentMalert("Please Select Type!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (addDetails.ou === "Please Select OU") {
      setPopupContentMalert("Please Select OU!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else {
      sendRequest();
  ...
  const sendRequest = async () => {
    ...
    try {
      setIsLoadingNew(true);
      const response = await axios.post(SERVICE.HIERARCHYBASEDDOMAINUSERSLIST, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        ...
        pagename: 'menuhierarchydomainuserslist',

      });
      if (response?.data?.resultedTeam?.length > 0 && response?.data?.resultAccessFilter?.length < 1 && ['myallhierarchy', 'allhierarchy']?.includes(modeselection.value)) {
        alert('Some employees have not been given access to this page.');
        setLoader(false);
        setIsLoadingNew(false);
      }
      const itemsWithSerialNumber = response?.data?.resultAccessFilter.map(
        (item, index) => ({
          ...item,
  ...
  const sendEditRequest = async () => {
    ...
    try {
      setIsLoading(true);
      let res = await axios.post(
        `${SERVICE.UPDATEINDIVIDUALUSERDETAILS}`,
        {
          oldUsername: String(oldDetails.username),
          newUsername: editDetails.username,
          newCN: editDetails.cn,
          newSN: editDetails.sn,
        },
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
  ...
  const sendEditRequestReset = async () => {
    setPageName(!pageName);

    try {
      setIsLoading(true);
      let res = await axios.post(
        `${SERVICE.RESETDOMAINUSERPASSWORD}`,
        {
          username: String(oldDetailsReset.username),
          newPassword: editDetailsReset.password,
        },
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
  ...
  const [selectedOptionsEmployementType, setSelectedOptionsEmployementType] =
    useState(employemnetTypeOptions);
  let [valueEmployementTypeCat, setValueEmployementTypeCat] = useState([
    "Employee",
    "Internship",
  ]);
  ...
  const customValueRendererEmployementType = (
    valueEmployementTypeCat,
    _categoryname
  ) => {
  ...
  const accountStatusOptions = [
    ...
    { label: "Workstation Trust Account", value: "4096" },
    { label: "Account Disabled", value: "514" },
    { label: "Domain Controller Account", value: "532480" },
    { label: "Server Trust Account", value: "546" },
  ];
  ...
  const [selectedOptionsAccountStatus, setSelectedOptionsAccountStatus] =
    useState([
      { label: "Normal account + Password Required", value: "512" },
      { label: "Normal account + Password Not Required", value: "544" },
      { label: "Password never expires", value: "66048" },
  ...
  let [valueStatusCatNew, setValueStatusCatNew] = useState([
    "Matched",
    "Mismatched",
  ]);

  const handleAccountStatusChangeNew = (options) => {
    let arrayString = options?.map((a, index) => {
      return a.value;
    });

    setValueStatusCatNew(arrayString);
    setSelectedStatusOptions(options);
  };

  const customValueRendererStatusNew = (valueStatusCatNew, _categoryname) => {
    return valueStatusCatNew?.length
      ? valueStatusCatNew?.map(({ label }) => label)?.join(", ")
  ...
  const customValueRendererAccountStatus = (
    valueAccountStatusCat,
    _categoryname
  ) => {
  ...
  const [selectedOptionsStatus, setSelectedOptionsStatus] =
  ...
  const [
    selectedOptionsRegistrationStatus,
    setSelectedOptionsRegistrationStatus,
  ] = useState(RegistrationStatusOptions);
  ...
  const customValueRendererRegistrationStatus = (
    valueRegistrationStatusCat,
    _categoryname
  ) => {
  ...
  const customValueRendererOU = (valueOUCat, _categoryname) => {
    return valueOUCat?.length
      ? valueOUCat?.map(({ label }) => label)?.join(", ")
      : "Please Select OU";
  };
  ...
  const handleFilter = () => {
    if (selectedStatusOptions?.length === 0) {
      ...
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (selectedOptionsAccountStatus?.length === 0) {
      setPopupContentMalert("Please Select Account Type!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (selectedOptionsEmployementType?.length === 0) {
  ...
  const fetchRocketChatUsers = async () => {
    ...
    setTableLoader(true);
    try {
      let response = await axios.post(SERVICE.GETALLUSERSBYACCOUNTCONTROL, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        type: filterState?.type,
        accounttype: valueAccountStatusCat?.map((item) => Number(item)) || [],
        company:
        ...
        status: valueStatusCatNew || [],
      });

      const itemsWithSerialNumber = response?.data?.users.map(
        (item, index) => ({
          ...item,
          serialNumber: index + 1,
          id: item._id,
  ...
  return (
    <Box>
      <NotificationContainer />
      <Headtitle title={"Hierarchy Domain Users List"} />
      {/* ****** Header Content ****** */}
      <PageHeading
        title="Hierarchy Domain Users List"
        modulename="LDAP"
        submodulename="Hierarchy Domain Users List"
        mainpagename=""
        subpagename=""
        subsubpagename=""
      />
      {isUserRoleCompare?.includes("ahierarchydomainuserslist") && (
        <>
          <Box sx={userStyle.selectcontainer}>
            <>
              <Typography sx={userStyle.importheadtext}>Hierarchy Domain Users Filter</Typography>
              <br />
      ...
      {tableLoader ? (
        ...
        <>
          {isUserRoleCompare?.includes("lhierarchydomainuserslist") && (
            <>
              <Box sx={userStyle.container}>
                {/* ******************************************************EXPORT Buttons****************************************************** */}
                <Grid container spacing={2}>
                ...
                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                  ...
                  >
                    <Box>
                      {isUserRoleCompare?.includes("excelhierarchydomainuserslist") && (
                        <>
                          <Button
                            onClick={(e) => {
                      ...
                      {isUserRoleCompare?.includes("csvhierarchydomainuserslist") && (
                        <>
                          <Button
                            onClick={(e) => {
                              setIsFilterOpen(true);
                      ...
                      {isUserRoleCompare?.includes("printhierarchydomainuserslist") && (
                        <>
                          <Button
                            sx={userStyle.buttongrp}
                            onClick={handleprint}
                      ...
                      {isUserRoleCompare?.includes("pdfhierarchydomainuserslist") && (
                        <>
                          <Button
                            sx={userStyle.buttongrp}
                            onClick={() => {
                      ...
                      {isUserRoleCompare?.includes("imagehierarchydomainuserslist") && (
                        <Button
                          sx={userStyle.buttongrp}
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={4} xs={12} sm={12}>
                <FormControl fullWidth size="small">
                  ...
                  <Typography>{viewDetails.empcode}</Typography>
                </FormControl>
              </Grid>
            </Grid>
            <br /> <br /> <br /> <br />
            ...
            <>
              ...
              <Grid container spacing={2}>
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      ...
                      placeholder="Please Enter Username" // Optional placeholder
                      value={editDetails.username} // Change field name to match email
                      onChange={(e) => {
                        setEditDetails((prev) => ({
                          ...prev,
                          username: e.target.value, // Update corresponding state field
                        }));
                      }}
                    />
                  </FormControl>
                ...
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      id="component-outlined-email"
                      type="email" // Change type to "email"
                      placeholder="Please Enter CN" // Optional placeholder
                      value={editDetails.cn} // Change field name to match email
                      onChange={(e) => {
                        setEditDetails((prev) => ({
                          ...prev,
                          cn: e.target.value, // Update corresponding state field
                        }));
                      }}
                    />
                  </FormControl>
                ...
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    <Typography>
                      SN<b style={{ color: "red" }}>*</b>
                    </Typography>
                    <OutlinedInput
                      id="component-outlined-email"
                      type="email" // Change type to "email"
                      placeholder="Please Enter SN" // Optional placeholder
                      value={editDetails.sn} // Change field name to match email
                      onChange={(e) => {
                        setEditDetails((prev) => ({
                          ...prev,
                          sn: e.target.value, // Update corresponding state field
                        }));
                      }}
                    />
                  </FormControl>
      ...
      <Box>
        <Dialog
          open={isEditOpenReset}
          onClose={handleCloseModEditReset}
          aria-labelledby="alert-dialog-title"
          aria-describedby="alert-dialog-description"
          maxWidth="md"
          fullWidth={true}
            ...
            <>
              <Grid container spacing={2}>
                <Typography sx={userStyle.HeaderText}>
                  ...
                  <span style={{ color: "red", fontSize: "0.8rem" }}>
                    (Length: At least 12-16 characters, Mix of Characters:
                ...
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      ...
                      type="email" // Change type to "email"
                      placeholder="Please Enter Username" // Optional placeholder
                      value={oldDetailsReset.username} // Change field name to match email
                      // onChange={(e) => {
                      //   setEditDetails((prev) => ({
                      //     ...prev,
                      //     username: e.target.value, // Update corresponding state field
                ...
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small" variant="outlined">
                    ...
                    <OutlinedInput
                      ...
                      value={editDetailsReset.password}
              ...
              >
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <Button
                    sx={buttonStyles.btncancel}
                    onClick={handleCloseModEditReset}
                  >
                    {" "}
                    Cancel{" "}
                  </Button>
            ...
            <>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      styles={colourStyles}
                      value={{
                        label: addDetails.type ?? "Please Select Type",
                        value: addDetails.type ?? "Please Select Type",
                      }}
                      onChange={(e) => {
                ...
                <Grid item md={4} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    <Typography>
                      OU<b style={{ color: "red" }}>*</b>
                    </Typography>
                    <Selects
                      options={ounewOptions}
                      styles={colourStyles}
                      value={{
                        label: addDetails.ou ?? "Please Select OU",
                        value: addDetails.ou ?? "Please Select OU",
                      }}
                      onChange={(e) => {
                        setAddDetails((prev) => ({ ...prev, ou: e.value }));
                      }}
                    />
                  </FormControl>
                </Grid>
