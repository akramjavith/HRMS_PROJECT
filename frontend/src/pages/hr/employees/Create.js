...
function MultistepForm() {
      ...
      renderCell: (params) => (
        <Grid sx={{ display: 'flex' }}>
          <Button
            variant="contained"
            onClick={() => {
              getCode(params.row.totalValue, params.row.code, params.row.experience, params.row.targetpoints);
            }}
          >
            Allot
          </Button>
  ...
  const handleButtonClick = (e) => {
    ...
    if (loginNotAllot?.time === 'Hrs' || loginNotAllot?.timemins === 'Mins' || (loginNotAllot?.time === '00' && loginNotAllot?.timemins === '00')) {
    ...
    if (salaryOption !== 'Experience Based' && salarySetUpForm.mode === 'Manual' && formValue?.gross === '') {
      ...
      missingFieldsthree.push('Enter Gross Amount');
    }
  ...
  const { isUserRoleAccess, isUserRoleCompare, isAssignBranch, buttonStyles, workStationSystemName } = useContext(UserRoleAccessContext);
  // for attendance mode
  ...
  const fetchUserDefaultDatas = async () => {
    ...
    try {
      ...
      branchCode = req?.data?.branch?.filter((item) => item.name == migrateData?.branch);
      ...
      setEmployee((prev) => ({
        ...
        paddresstype: migrateData?.addesstype ? migrateData?.addesstype : "",
        ppersonalprefix: migrateData?.personalprefix ? migrateData?.personalprefix : "",
        presourcename: migrateData?.referencename ? migrateData?.referencename : "",
        plandmarkandpositionalprefix: migrateData?.landmarkpositionprefix ? migrateData?.landmarkpositionprefix : "",
        pgpscoordination: migrateData?.gpscoordinate ? migrateData?.gpscoordinate : "",
        plandmark: migrateData?.landmarkname ? migrateData?.landmarkname : "",

        ppincode: migrateData?.ppincode ? migrateData?.ppincode : "",
        pcountry: migrateData?.pcountry ? migrateData?.pcountry : "",
        pstate: migrateData?.pstate ? migrateData?.pstate : "",
        pcity: migrateData?.pcity ? migrateData?.pcity : "",
        pdoorno: migrateData?.houseflatnumber ? migrateData?.houseflatnumber : "",
        pstreet: migrateData?.streetroadname ? migrateData?.streetroadname : "",
        parea: migrateData?.localityareaname ? migrateData?.localityareaname : "",

        // caddresstype: migrateData?.caddresstype ? migrateData?.caddresstype : "",
        // cpersonalprefix: migrateData?.cpersonalprefix ? migrateData?.cpersonalprefix : "",
        // cresourcename: migrateData?.cresourcename ? migrateData?.cresourcename : "",
        // clandmarkandpositionalprefix: migrateData?.clandmarkandpositionalprefix ? migrateData?.clandmarkandpositionalprefix : "",
        // cgpscoordination: migrateData?.cgpscoordination ? migrateData?.cgpscoordination : "",
        ...
        gender: migrateData?.gender === 'Other' ? 'Others' : migrateData?.gender,
        ...
        profileimage: migrateData?.faceDescriptor?.length > 0 && migrateData?.uploadedimage ? migrateData?.uploadedimage : '',
        faceDescriptor: migrateData?.faceDescriptor?.length > 0 && migrateData?.uploadedimage ? migrateData?.faceDescriptor : [],
        prod: true,
        addedby: [
          {
      ...
      setAssignExperience({
        ...assignExperience,
        assignExpMode: migrateData?.assignExpMode || 'Auto Increment',
        assignExpvalue: migrateData?.assignExpvalue || 0,
      });
      setLoginNotAllot({
        ...loginNotAllot,
        process: migrateData?.process || 'Please Select Process',
      });
      setoverallgrosstotal(migrateData?.overallgrosstotal || '');
      setTargetpts(migrateData?.targetpts || '');
      setModeexperience(migrateData?.assignExpvalue || '');
      setTargetexperience(migrateData?.assignExpvalue || '');

      let newEmpCode = await EmployeeCodeAutoGenerate(migrateData?.company || '', migrateData?.branch || '', branchCode[0]?.code || '', moment().format('YYYY-MM-DD'));
  ...
  //     setModeexperience(migrateData?.assignExpvalue || "");
  //     setTargetexperience(migrateData?.assignExpvalue || "");
  //     let branchCode = await fetchbranchCodes(migrateData?.company || "", migrateData?.branch || "");
  ...
  //     setNewval(newEmpCode)
  //   } catch (err) {
  //     console.log(err, "migratedDatwdays")
  //   }
  // };

  const fetchbranchCodes = async (selectedBranch, selectedCompany) => {
    try {
      let req = await axios.get(SERVICE.BRANCH, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
  ...
  // const [getRolesDropDown, setGetRolesDropdown] = useState("");
      ...
      if (missingFields.length > 0) {
        ...
        if (Object.keys(newErrors).length === 0) {
          ...
          if (res?.data?.success) {
            ...
            if (migrateData && employee?.faceDescriptor?.length > 0 && employee?.profileimage && !imageUploaded) {
              const response = await axios.post(`${SERVICE.DUPLICATECANDIDATEFACEDETECTVISITOR}`, {
                headers: {
                  Authorization: `Bearer ${auth.APIToken}`,
                },
                id: migrateData?._id || null,
  ...
  const fetchOverAllSettings = async () => {
    try {
      ...
      let filter = lastObject?.todos.filter((item) => item.branch.includes(selectedBranch) && item.company == selectedCompany);

      setOverallsettings(filter);
      setEmpsettings(lastObject?.empdigits);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  const fetchqualification = async () => {
    try {
      ...
      setQualinames(req?.data?.qualificationdetails);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  const allotWorkStation = async () => {
    try {
      ...
      let finalUsername = enableLoginName ? String(third) : employee?.username;
  ...
  // get settings data
  const fetchUserDatasintern = async () => {
    try {
      let req = await axios.get(SERVICE.USER, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
      ...
      let internusers = req.data.users?.filter((item) => {
        if (item.workmode == 'Internship') {
          return item;
        }
      });
      setInternId(internusers);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  //     setDesignationsFileNames(req?.data?.designation);
  //   } catch (err) {
  //     handleApiError(
  //       err,
  ...
  const handleTeamChange = (event) => {
    ...
    // processTeamDropdowns( event.value)
  ...
  const getRolesDatas = async () => {
    try {
      ...
      // Optional logging
      // console.log(modulename, submodulename, mainpagename, subpagename, subsubpagename);
  ...
  const handleEmployeesChange = (options) => {
    const maxOptions = Number(employee?.employeecount) - 1;
  ...
  const nextStepLog = () => {
    ...
    if (!enableLoginName && employee.username === '') {
      ...
      missingFieldstwo.push('Username Already Exist');
    }
    // Check the validity of field1
    if (!employee.password) {
      newErrorsLog.password = <Typography style={{ color: 'red' }}>Password must be required</Typography>;
    ...
    if (employee.wordcheck === true && employee?.empcode?.length > 12) {
  ...
  const renderStepOne = () => {
    return (
      <>
        <Headtitle title={'EMPLOYEE CREATE'} />
        <Grid container spacing={2}>
          ...
          <Grid item md={1} xs={12} sm={12} container justifyContent={{ xs: 'center', md: 'flex-end' }} alignItems="center">
            ...
            >
              ...
              {/* </Link> */}
              {employee.firstname !== '' && employee.lastname !== '' && employee.legalname !== '' && employee.dob !== '' && employee.aadhar !== '' && employee.emergencyno !== '' && (
                <LoadingButton
                  size="small"
                  variant="contained"
                  loading={draftLoader}
                ...
                <Grid item md={4} sm={6} xs={12}>
                  {enableLoginName ? (
                    <FormControl size="small" fullWidth>
                      ...
                      <OutlinedInput id="component-outlined" type="text" placeholder="login Name" value={third} readOnly />
                    </FormControl>
                  ) : (
                    <FormControl size="small" fullWidth>
                      <Typography>
                    ...
                    <Grid item md={4} sm={12} xs={12}>
                      <FormControl size="small" fullWidth>
                        ...
                        <Selects
                          ...
                          isDisabled={employee?.employeecount === '0' || employee?.employeecount === ''}
                          onChange={(e) => {
  ...
  const handleChangeBasic = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === '') {
      setFormValue({
        ...formValue,
        basic: inputValue,
        gross: Number(e.target.value) + Number(formValue?.hra) + Number(formValue?.conveyance) + Number(formValue?.medicalallowance) + Number(formValue?.productionallowance) + Number(formValue?.productionallowancetwo) + Number(formValue?.otherallowance),
      });
    }
  };
  ...
  const handleChangeHra = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === '') {
      setFormValue({
        ...formValue,
        hra: e.target.value,
        gross: Number(e.target.value) + Number(formValue?.basic) + Number(formValue?.conveyance) + Number(formValue?.medicalallowance) + Number(formValue?.productionallowance) + Number(formValue?.productionallowancetwo) + Number(formValue?.otherallowance),
      });
    }
  };
  ...
  const handleChangeConveyance = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === '') {
      setFormValue({
        ...
        gross: Number(e.target.value) + Number(formValue?.basic) + Number(formValue?.hra) + Number(formValue?.medicalallowance) + Number(formValue?.productionallowance) + Number(formValue?.productionallowancetwo) + Number(formValue?.otherallowance),
      });
    }
  };
  //change form
  const handleChangeMedAllow = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === '') {
      setFormValue({
        ...
        gross: Number(e.target.value) + Number(formValue?.hra) + Number(formValue?.conveyance) + Number(formValue?.basic) + Number(formValue?.productionallowance) + Number(formValue?.productionallowancetwo) + Number(formValue?.otherallowance),
      });
    }
  };
  ...
  const handleChangeProdAllow = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === '') {
      setFormValue({
        ...
        gross: Number(e.target.value) + Number(formValue?.basic) + Number(formValue?.hra) + Number(formValue?.conveyance) + Number(formValue?.medicalallowance) + Number(formValue?.medicalallowance) + Number(formValue?.productionallowancetwo) + Number(formValue?.otherallowance),
      });
    }
  };
  ...
  const handleChangeProdAllowtwo = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === '') {
      setFormValue({
        ...
        gross: Number(e.target.value) + Number(formValue?.basic) + Number(formValue?.hra) + Number(formValue?.conveyance) + Number(formValue?.medicalallowance) + Number(formValue?.medicalallowance) + Number(formValue?.productionallowance) + Number(formValue?.otherallowance),
      });
    }
  };
  //change form
  const handleChangeOtherAllow = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === '') {
      setFormValue({
        ...
        gross: Number(e.target.value) + Number(formValue?.basic) + Number(formValue?.hra) + Number(formValue?.conveyance) + Number(formValue?.medicalallowance) + Number(formValue?.medicalallowance) + Number(formValue?.productionallowance) + Number(formValue?.productionallowancetwo),
      });
    }
  };
  ...
  const fetchProfessionalTax = async (process, salarycode) => {
    try {
      ...
      setCtc(OptSlab[0].basic + OptSlab[0].hra + OptSlab[0].conveyance + OptSlab[0].medicalallowance + OptSlab[0].productionallowance + OptSlab[0].productionallowancetwo + OptSlab[0].otherallowance);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  const handleAccessibleBranchTodo = () => {
    let newObject = {
      ...
      unit: selectedUnit,
      employee: companycaps,
      employeecode: String(employee.wordcheck === false ? newval : employee.empcode),
    };
                    ...
                    <>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          ...
                          <OutlinedInput id="component-outlined" type="text" value={formValue?.startmonthlabel} />
                        </FormControl>
                      </Grid>
                      <Grid item md={3} sm={12} xs={12}>
                        <FormControl fullWidth size="small">
                          ...
                          <OutlinedInput id="component-outlined" type="text" value={formValue?.startyear} />
                        </FormControl>
                      </Grid>
                    </>
                    {/* )} */}
              ...
              <Grid container spacing={2}>
                ...
                <Grid item md={3} xs={6} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput
                      ...
                      type="text"
                      // placeholder="Please Enter IFSC Code"
                      value={String(overallgrosstotal)}
                    // onChange={(e) => {
                    //   setEmployee({ ...employee, ifsccode: e.target.value });
                    // }}
                    />
                ...
                <Grid item md={3} xs={6} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput id="component-outlined" type="text" value={targetpts} />
                  </FormControl>
                </Grid>
              </Grid>
              <br />
              ...
              <Grid container spacing={1}>
                <Grid item md={8} xs={0} sm={4}>
                  <Typography
                    sx={userStyle.SubHeaderText}
                    onClick={() => {
                      console.log(companyEmailDomain);
                    }}
                  >
              ...
              <Grid container spacing={2}>
                <Grid item md={2.8} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput value={enableLoginName ? String(third) : employee.username} readOnly />
                  </FormControl>
                </Grid>
                <Grid item md={2.8} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput type="password" value={employee.password} readOnly />
                  </FormControl>
                </Grid>
                <Grid item md={2.8} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput value={companycaps} readOnly />
                  </FormControl>
                </Grid>
