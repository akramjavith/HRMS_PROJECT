...
function TaskForUserCompleted({ com }) {
    ...
    let exportRowValues = [
        'taskstatus', 'taskassigneddate',
        'priority', 'username',
        'completedbyuser', 'tasktime',
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("Task/Completed Task"),
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "Task User Panel-Completed Task List.png");
                })
                .catch((error) => {
    ...
    const ListPageLoadDataCompleted = async () => {
        ...
        try {
            let res_task = await axios.post(SERVICE.COMPLETEDALL_TASKFORUSER, queryParams, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                }
            });
            ...
            const ansTotal = res_task?.data?.overallList?.length > 0 ? res_task?.data?.overallList : []
            ...
            setItemsTotal(ansTotal?.map((item, index) => ({
                ...item,
                serialNumber: (page - 1) * pageSize + index + 1,
                taskstatus: item.taskstatus,
                taskassigneddate: item?.taskdetails === "Manual" ? moment(item?.taskdate).format("DD-MM-YYYY") : item.taskassigneddate,
            ...
            setQueueCheck(true); handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
        }
    }
    ...
    return (
        <Box>
            ...
            {/* ****** Header Content ****** */}
            <PageHeading
                title="Task User Panel- Completed Tasks"
                modulename="Task"
                submodulename="Completed Task"
                mainpagename=""
                subpagename=""
                subsubpagename=""
            />
                                ...
                                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                                    <Grid item md={2} xs={12} sm={12}>
                                        ...
                                        >
                                            ...
                                            <MenuItem value={itemsTotal?.length}>All</MenuItem>
                                        </Select>
            ...
            <ExportData
                ...
                itemsTwo={itemsTotal ?? []}
                filename={"Task User Panel-Completed Task List"}
