...
function DayPointsUpload() {
  ...
  let exportRowValuesView = [
    ...
    'target', 'production',
    'manual', 'point',
    'allowancepoint', 'nonallowancepoint',
    'avgpoint'
  ]
  ...
  const rowDataViewIndividualUser = async (date, name, type, users, fromtodate, company, branch, unit, team, empcode, exper, target, processcode) => {
    ...
    try {
      if (type !== "Created") {
        ...
        console.log(res.data.productionupload.length, fromtodate, 'res.data.productionuploadManual');
        ...
        console.log(checkfromdate, checktodate, new Date(checkfromdate), new Date(checktodate), 'new Date(checkfromdate), new Date(checktodate),')
        ...
        let datafiltered = [...res.data.productionupload, ...res.data.productionuploadManual].map((item) => {
          ...
          let toDateonly = item.mode == "Manual" ? (item.fromdate + " " + item.time + ":00") : `${UploadDateVal.split(" ")[0]} ${UploadDateVal.split(" ")[1]}`
          ...
          let datenew = (item.mode == "Manual" ? item.fromdate + " " + item.time + ":00" : item.dateval?.split(" IST")[0]);
          ...
          if (new Date(toDateonly) >= new Date(checkfromdate) && new Date(toDateonly) <= new Date(checktodate)) {
            return {
              ...
              date: date,
              datenew: datenew,
              points: item.mode == "Manual" ? Number(item.updatedunitrate) * 8.333333333333333 : (finalunitrate) * finalflag * 8.333333333333333,
              company: company,
              branch: branch,
              unit: unit,
              team: team,
  ...
  const fetchTargetCategoryProcessMap = async () => {
    try {
      ...
      let targetPointsval = res_Target.data.targetpoints;
      setTargetPoints(targetPointsval);
      let res_Cate = await axios.get(SERVICE.CATEGORYPROCESSMAP_LIMITED, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      let categoryProcessMapval = res_Cate.data.categoryprocessmaps;
      setCategoryProcessMap(categoryProcessMapval);
    } catch (err) {
      console.log(err);
      setIsSingleLoader(false);
      handleApiError(err, setShowAlert, handleClickOpenerr);
    ...
    try {
      ...
      const itemsWithSerialNumber = singleUserDataView?.map((item, index) => {
        ...
        return {
          ...
          conpoints: conPoints > 0 ? Number(Number(conPoints).toFixed(6)) : Number(conPoints),
          conavg: conavg > 0 ? Number(Number(conavg).toFixed(6)) : Number(conavg),
        };
      });
  ...
  return (
    <Box>
      ...
      <PageHeading
        title="Manage Production Points-Upload"
        modulename="Production"
        submodulename="Upload"
        mainpagename="Original"
        subpagename="Day Points Uplaod"
        subsubpagename=""
      />
