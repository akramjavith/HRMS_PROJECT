...
function CategoryDateChange() {
  ...
  const customValueRendererSubCategoryEdit = (valueSubCategoryCatEdit, _categoryname) => {
    return valueSubCategoryCatEdit?.length ? valueSubCategoryCatEdit.map(({ label }) => label)?.join(', ') : 'Please Select Sub Category';
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const isNameMatch = categoryDateChangeData.some((item) => item.project === categoryDateChange.project && item.category === categoryDateChange.category && item.subcategory.some((item) => valueSubCategoryCat.includes(item)));
  ...
  const getCode = async (e, name) => {
    ...
    try {
      ...
      setValueSubCategoryCatEdit(
        res?.data?.categorydatechange?.subcategory.map((a, index) => {
          return a;
        })
      );
  ...
  const editSubmit = async (e) => {
    ...
    const isNameMatch = resdata.some((item) => item.project === categoryDateChangeEdit.project && item.category === categoryDateChangeEdit.category && item.subcategory.some((item) => valueSubCategoryCatEdit.includes(item)));
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Category Date Change" modulename="Production" submodulename="SetUp" mainpagename="Category Date Change" subpagename="" subsubpagename="" />
      ...
      />
      {/* INFO */}
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Category Date Change Info" addedby={addedby} updateby={updateby} />
