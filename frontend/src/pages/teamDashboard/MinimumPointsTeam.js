...
function MinPointsCalc() {
  ...
  useEffect(() => {
    const fetchTime = async () => {
      ...
      setServerTime(time);
      let currentMonth = monthsArr[Number(moment(time).format('MM')) - 1];


      setSelectedYear(moment(time).format('YYYY'))
      setSelectedMonth(currentMonth)
      setSelectedMonthNum(moment(time).format('MM'))
    };
  ...
  let exportColumnNames = ['Company', 'Branch', 'Unit', 'Team', 'Emp Code', 'Name', 'Process + Salary Exp', 'Gross Salary', 'Revenue Allowance', 'Shortage', 'Total No.Of Days', 'Total Paid Days', 'Month Point', 'Day Point'];
  let exportRowValues = ['company', 'branch', 'unit', 'team', 'empcode', 'companyname', 'processcodeexp', 'gross', 'revenueallow', 'shortage', 'totalnumberofdays', 'totalpaidDays', 'monthPoint', 'dayPoint'];
  ...
  const { isUserRoleAccess, isUserRoleCompare, pageName, setPageName, isAssignBranch,listPageAccessMode } = useContext(UserRoleAccessContext);
  ...
  const fetchSalarySlabs = async () => {
    ...
    try {
      let res_employee = await axios.post(
        SERVICE.SALARYSLAB_LIMITEDASSIGNBRANCH,
        {
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      let [res_applyleave, res_employee1] = await Promise.all([
        ...
        axios.post(
          SERVICE.USERSEXCELDATAASSIGNBRANCHE_TEAM,
          {
            assignbranch: accessbranch,
            month: selectedMonth,
            year: selectedYear,
      ...
      async function fetchData(pageone) {
        try {
          const response = await axios.post(
            SERVICE.GETUSE_TOTALSHIFT_DAYS_TEAM,
            {
              page: pageone,
              pageSize: batchSize,
      ...
      fetchData(pageone)
        .then(async () => {
          ...
          const absentItems = batches?.filter((d) => d.clockinstatus === 'Absent' && d.clockoutstatus === 'Absent' && d.clockin === '00:00:00' && d.clockout === '00:00:00');
          ...
          updatedResult.forEach((item, index, array) => {
          ...
          updatedResult.forEach((item) => {
            ...
            //     weekoffdayscount = 1;
            //   }
            //   if (item.clockinstatus === 'Week Off' && item.clockoutstatus === 'Week Off' && item.appliedthrough === 'Manual' && item.lopcalculation !== 'Yes - Full Day') {
            //     weekoffdayscount = 1;
            //   }
            ...
            //     holidaydayscount = 1;
            //   }
            //   if (item.clockinstatus === 'Holiday' && item.clockoutstatus === 'Holiday' && item.appliedthrough === 'Manual' && item.lopcalculation !== 'Yes - Full Day') {
            //     holidaydayscount = 1;
            //   }
          ...
          const itemsWithSerialNumber = res_employee1.data.users?.map(async (item, index) => {
            try {
              ...
              if (item?.processlog && item?.processlog?.length > 0) {
                // Group items by month
              ...
              console.log(err,"pppppp")
            }
          });
          const results = await Promise.all(itemsWithSerialNumber);
          setItems(results.filter((item) => item !== undefined && item !== null));
          setBankdetail(false);
        })
        .catch((error) => {
          setBankdetail(false);
...
//   },[])
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Minimum Points Calculation" modulename="Production" submodulename="SetUp" mainpagename="Minimum Points Calculation" subpagename="" subsubpagename="" />
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={2} xs={12} sm={6}>
                <FormControl fullWidth size="small">
                  ...
                  <Selects options={years} sx={{ borderColor: 'green !important' }} value={{ label: selectedYear, value: selectedYear }} onChange={handleYearChange} />
                </FormControl>
              </Grid>
              <Grid item md={2} xs={12} sm={6}>
                <FormControl fullWidth size="small">
