...
function HolidayFilter() {
      ...
      exportToCSV(
        rowDataTable.map((item, index) => {
          return {
            ...
            Branch: item.applicablefor,
            unit: item.unit,
            team: item.team,
            employee: item.employee,
            description: item.description,
            Reminder: item.noofdays,
          };
        }),
        fileName
      );
  ...
  const columns = [
    ...
    { title: "Branch ", field: "applicablefor" },
  ...
  const fetchHolidayfilter = async () => {
    setPageName(!pageName);
    try {
      let res_employee = await axios.post(SERVICE.HOLIDAYFILTER, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },

        selectedfilter: holidayState.date,
      });
      setStatusCheck(true);

      const result = res_employee?.data?.holidayfilternew.filter(
        (data, index) => {
          if (
      ...
      const resdata = isUserRoleAccess.role.includes("Manager")
        ? res_employee?.data?.holidayfilternew
        : result;
  ...
  // Excel
  const fileName = "HolidayFilter";
  // get particular columns for export excel
  const getexcelDatas = () => {
    setPageName(!pageName);
    try {
      var data = holidayArray?.map((t, index) => ({
        Sno: index + 1,
        Name: t.name,
        Date: moment(t.date).format("DD-MM-YYYY"),
        ApplicableFor: t.applicablefor?.toString(),
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Holiday Filter",
    pageStyle: "print",
  });
  //serial no for listing items
  const addSerialNumber = () => {
    const itemsWithSerialNumber = holidayArray?.map((item, index) => ({
      ...item,
      serialNumber: index + 1,
  ...
  return (
    <Box>
      <Headtitle title={"HOLIDAY FILTER"} />
      {/* ****** Header Content ****** */}
      {/* ****** Table Start ****** */}
      <PageHeading
        ...
        subpagename="Holiday Filter"
        subsubpagename=""
      />

      <>
        {isUserRoleCompare?.includes("lholidayfilter") && (
          <>
            <Box sx={userStyle.container}>
              ...
              <Grid container spacing={2} style={userStyle.dataTablestyle}>
                ...
                >
                  <Box>
                    {isUserRoleCompare?.includes("excelholidayfilter") && (
                      <>
                        <Button
                          onClick={(e) => {
                    ...
                    {isUserRoleCompare?.includes("csvholidayfilter") && (
                      <>
                        <Button
                          onClick={(e) => {
                            setIsFilterOpen(true);
                    ...
                    {isUserRoleCompare?.includes("printholidayfilter") && (
                      <>
                    ...
                    {isUserRoleCompare?.includes("pdfholidayfilter") && (
                      <>
                        <Button
                          sx={userStyle.buttongrp}
                          onClick={() => {
                    ...
                    {isUserRoleCompare?.includes("imageholidayfilter") && (
                      <Button
                        sx={userStyle.buttongrp}
                  ...
                  <FormControl fullWidth size="small">
                    <Selects
                      ...
                      value={repeatTypeOption.find(
                        (option) => option.value === holidayState.date
                      )}
                      onChange={(e) => {
                        setHolidayState({ ...holidayState, date: e.value });
                      }}
                    />
                  </FormControl>
                </Grid>
