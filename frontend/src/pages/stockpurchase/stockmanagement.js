...
function StockManagement() {
  ...
  //alert model for vendor details
  const [openviewalertvendormanual, setOpenviewalertvendromanual] =
    useState(false);
  // view model
  const handleClickOpenviewalertvendormanual = () => {
    setOpenviewalertvendromanual(true);
  };

  const handleCloseviewalertvendormanual = () => {
    setOpenviewalertvendromanual(false);
  };
  ...
  const getCodeStock = async (
    ...
    try {
      ...
      if (status == "Stock") {
        ...
        handleClickOpenviewalertvendormanual();
      }
  ...
  const getCodeStockLog = async (data) => {
    try {
      ...
      let finaldata = res.data.stock.flatMap(item =>
        item.tododetails.filter(d => d.materialnew === data.productname).map(stockItem => ({
          // ...stockItem,
      ...
      // setStockLog(res.data.stock.map(item => (item.tododetails))?.filter(d => d.materialnew === data.productname))?.map(t => ({
      //   ...t,
      //   company: data.company,
      //   branch: data.branch,
      //   unit: data.unit,
  ...
  const sendRequestStock = async () => {
    try {
      ...
      else if (handover.status === "Manual") {
  ...
  const sendRequestStockReturn = async () => {
    ...
    try {
      ...
      const updatePromises = postdata?.map((item) => {
        const finalurl = item.status === "Stock" ? SERVICE.STOCKPURCHASE_CREATE : SERVICE.MANUAL_STOCKPURCHASE_CREATE
  ...
  return (
    <Box>
          ...
          <Box sx={{ padding: "20px 50px" }}>
            <>
              <Grid container spacing={2}>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <Typography sx={userStyle.HeaderText}>
                    <Typography>Purchase Count : <b style={{ color: "cornflowerblue" }}>{handover.requestmode === "Asset Material" ? handover.purchasecount : handover.purchasecountstock}</b></Typography>
                  </Typography>
                </Grid>
                <Grid item md={3} xs={12} sm={12}>
                  <Typography sx={userStyle.HeaderText}>
      ...
      <Dialog
        open={openviewalertvendormanual}
        onClose={handleClickOpenviewalertvendormanual}
        aria-labelledby="alert-dialog-title"
        aria-describedby="alert-dialog-description"
        maxWidth="lg"
        sx={{
