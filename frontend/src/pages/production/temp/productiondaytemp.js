...
function ProductionDayTemp() {
  ...
  const handleRerun = async (selcDate, uniqueid, id) => {
    try {
      ...
      if (isPayrunGenerated) {
        ...
        const [res_Day_temp, res_DayPoint_temp, res_Curr_Daypoint] = await Promise.all([
          axios.post(SERVICE.PRODUCTIONDAY_TEMP_LASTDATE, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
  ...
  const handleRerunRow = async (selcDate, type, uniqueid, beforeProdDay, beforeDayPoint, currDaypoint) => {
    ...
    try {
      ...
      let [result, ResChevkCount, filestatusCheckFileupload] = await Promise.all([
            ...
            try {
              if (allData === -1) {
                ...
                if (datavalue.length > 0) {
                  ...
                  let resCreate;
                  let filestatusfinal = filestatusCheckFileupload.data.productiontempcheckfileupload === 0 ? "Pending Upload" : filestatusCheckFileupload.data.productiontemp >= 3 ? "Final" : "Partial";
          ...
          <Grid sx={{ display: "flex" }}>
            {isUserRoleCompare?.includes("vproductiondaytemp") &&
              (params.row.status == "Created" ? (
                ...
                <LoadingButton loading={isloading === params.row.dateold} onClick={(e) => handleClickOpenDaypointCreateConfirmPopup(params.row.dateold)} sx={{ textTransform: "capitalize", backgroundColor: "#1b8971", "&:hover": { backgroundColor: "#1b8971" }, color: "white", padding: "4px" }} loadingPosition="end">
  ...
  const handleFilterSave = async (selcDate, type, beforeProdDay, beforeDayPoint) => {
    try {
      let alertval = "";
      handleProgressUpdate(alertval, "Checking...");
      handleLoaderDialogOpen();
                ...
                try {
                  if (allData === -1) {
                    ...
                    if (datavalue.length > 0) {
                      ...
                      handleProgressUpdate(9, "Creating...");
                      let resCreate;
                      let filestatusfinal = filestatusupload.data.productiontempcheckfileupload == 0 ? "Pending Upload" : filestatusupload.data.productiontemp >= 3 ? "Final" : "Partial";
  ...
  const handlelastProdDayDelCreate = async (date) => {
    try {
      ...
      handleFilterSaveLastDateRerun(date, "popup");
    } catch (err) {
      console.log(err, "err");
      handleApiError(err, setShowAlert, handleClickOpenerr);
    }
