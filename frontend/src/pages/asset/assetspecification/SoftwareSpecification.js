...
function ApplicationName() {
  ...
  let exportColumnNames = ["Name", "Type"];
  let exportRowValues = ["name", "type"];
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Software Specification"),
  ...
  const rowData = async (id, name) => {
    ...
    try {
      let res = await axios.get(`${SERVICE.SINGLE_SOFTWARESPECIFICATION}/${id}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setDeleteApplicationName(res?.data?.ssoftwarespecification);
      handleClickOpen();
    } catch (err) {
  ...
  const delAsset = async () => {
    setPageName(!pageName)
    try {
      await axios.delete(`${SERVICE.SINGLE_SOFTWARESPECIFICATION}/${assetid}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const sendRequest = async () => {
    ...
    setIsBtn(true);
    try {
      let freqCreate = await axios.post(SERVICE.CREATE_SOFTWARESPECIFICATION, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  const handleSubmit = (e) => {
    ...
    const isNameMatch = applicationNameArray?.some(
      (item) => item.name?.toLowerCase() === applicationName.name?.toLowerCase() &&
        item.type?.toLowerCase() === applicationName.type?.toLowerCase()
    );
    if (applicationName.name === "") {
      setPopupContentMalert("Please Enter Name!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (applicationName.type === "Please Select Type") {
  ...
  const getCode = async (e) => {
    setPageName(!pageName)
    try {
      let res = await axios.get(`${SERVICE.SINGLE_SOFTWARESPECIFICATION}/${e}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
      setApplicationNameEdit(res?.data?.ssoftwarespecification);
      handleClickOpenEdit();
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  ...
  const sendEditRequest = async () => {
    ...
    try {
      let res = await axios.put(
        `${SERVICE.SINGLE_SOFTWARESPECIFICATION}/${frequencyId}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const editSubmit = (e) => {
    ...
    const isNameMatch = allApplicationNameEdit?.some(
      (item) =>
        item.name?.toLowerCase() === applicationNameEdit.name?.toLowerCase() &&
        item.type?.toLowerCase() === applicationNameEdit.type?.toLowerCase()
    );
    if (applicationNameEdit.name === "") {
      setPopupContentMalert("Please Enter Name!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    } else if (applicationNameEdit.type === "Please Select Type" || applicationNameEdit.type === "" || applicationNameEdit.type === undefined) {
      setPopupContentMalert("Please Select Type!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    }
  ...
  const fetchApplicationName = async () => {
    setPageName(!pageName)
    try {
      let res_freq = await axios.get(SERVICE.ALL_SOFTWARESPECIFICATION, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
      });
  ...
  const bulkDeleteFunction = async () => {
    ...
    try {
      const deletePromises = selectedRows?.map((item) => {
        return axios.delete(`${SERVICE.SINGLE_SOFTWARESPECIFICATION}/${item}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });
  ...
  //get all Software Specification Name.
  const fetchApplicationNameAll = async () => {
    ...
    try {
      ...
      setAllApplicationNameEdit(
        res_freq?.data?.softwarespecification.filter(
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Software Specification.png");
        });
      });
    }
  };
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Software Specification",
    pageStyle: "print",
  });
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("esoftwarespecification") && (
            <Button
          ...
          {isUserRoleCompare?.includes("dsoftwarespecification") && (
            <Button
          ...
          {isUserRoleCompare?.includes("vsoftwarespecification") && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
          ...
          {isUserRoleCompare?.includes("isoftwarespecification") && (
            <Button
              sx={userStyle.buttonedit}
              onClick={() => {
  ...
  return (
    <Box>
      <Headtitle title={"SOFTWARE SPECIFICATION"} />
      <PageHeading
        title="Software Specification"
        modulename="Asset"
        submodulename="Asset Specifications"
        mainpagename="Software"
        subpagename="Software Specification"
        subsubpagename=""
      />
      {isUserRoleCompare?.includes("asoftwarespecification") && (
        <>
      ...
      {/* ****** Table Start ****** */}
      {isUserRoleCompare?.includes("lsoftwarespecification") && (
        <Box sx={userStyle.container}>
          {/* ******************************************************EXPORT Buttons****************************************************** */}
          <Grid item xs={8}>
            <Typography sx={userStyle.importheadtext}>
              Software Specification List
            </Typography>
          </Grid>
          <Grid container spacing={2} style={userStyle.dataTablestyle}>
            ...
            >
              <Box>
                {isUserRoleCompare?.includes("excelsoftwarespecification") && (
                  <>
                    <Button
                      onClick={(e) => {
                ...
                {isUserRoleCompare?.includes("csvsoftwarespecification") && (
                  <>
                    <Button
                      onClick={(e) => {
                        setIsFilterOpen(true);
                ...
                {isUserRoleCompare?.includes("printsoftwarespecification") && (
                  <>
                ...
                {isUserRoleCompare?.includes("pdfsoftwarespecification") && (
                  <>
                    <Button
                      sx={userStyle.buttongrp}
                      onClick={() => {
          ...
          &ensp;
          {isUserRoleCompare?.includes("bdsoftwarespecification") && (
            <Button
              variant="contained"
      ...
      <ExportData
        ...
        filename={"Software Specification"}
      ...
      <InfoPopup
        ...
        heading="Software Specification Info"
        addedby={addedby}
        updateby={updateby}
      />
