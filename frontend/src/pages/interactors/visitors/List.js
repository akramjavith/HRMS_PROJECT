...
function ListVisitors() {
      ...
      renderCell: (params) => (
        <Grid sx={{ display: "flex" }}>
          ...
          {isUserRoleCompare?.includes("elistvisitors") && (
            <Link
              to={`/interactor/master/editvisitors/${params.row.id}/list`}
          ...
          {isUserRoleCompare?.includes("vlistvisitors") && (
            <Link
              to={`/interactor/master/viewvisitors/${params.row.id}/list`}
              style={{ textDecoration: "none", color: "#fff", minWidth: "0px" }}
            >
              <Button sx={userStyle.buttonedit} style={{ minWidth: "0px" }}>
                <VisibilityOutlinedIcon style={{ fontSize: "large" }} />
          ...
          {isUserRoleCompare?.includes("vlistvisitors") && (
            <Link
              to={`/interactor/master/followupvisitor/${params.row.id}`}
              style={{ textDecoration: "none", color: "#fff", minWidth: "0px" }}
            >
              <Button sx={userStyle.buttonedit} style={{ minWidth: "0px" }}>
  ...
  const fetchVendor = async () => {
    ...
    setPageName(!pageName);
    try {
      let res = await axios.post(SERVICE.SKIPPED_VISITORS, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        page: Number(page),
