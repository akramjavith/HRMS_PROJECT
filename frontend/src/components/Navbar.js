...
const Navbar = () => {
    ...
    const fetchFilterSidebarItems = async () => {
      try {
        ...
        let mappedDatas = res?.data?.tooldescription?.map((data) => ({
          ...data,
          commonname: `${data.modulename}-${data.submodulename}-${data.mainpagename ? data.mainpagename : ""
            }-${data.subpagename ? data.subpagename : ""}-${data.subsubpagename ? data.subsubpagename : ""
            }`,
        }));
  ...
  const isScreenSize930px = useMediaQuery("(min-width: 1000px)");
