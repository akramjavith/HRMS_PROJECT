...
function Expensecategory() {
  ...
  const sendRequestEdit = async () => {
    ...
    try {
      ...
      let categoryUpdate = await axios.post(SERVICE.EXPENSE_CATEGORY_OVERALL_UPDATE, {
  ...
  const getcode = async (id, categoryname, subcategory) => {
    ...
    try {
      ...
      setOldCategory(res?.data?.sexpensecategory?.categoryname);
  ...
  const checkUsedByOther = async (data) => {
    ...
    try {
      let res = await axios.post(SERVICE.EXPENSE_CATEGORY_OVERALL_DELETE, {
        combinedData
      })
      if (res.data.exists) {
        setPopupContentMalert(`This Data was used in these pages ${res.data.popupMessage}`);
        setPopupSeverityMalert("info");
        handleClickOpenPopupMalert();
      } else {
        handleClickOpen();
    ...
    if (isNameMatch) {
      ...
      if (singleCategory.categoryname !== oldCategory) {
        const [resNew] = await Promise.all([
          ...
          axios.post(SERVICE.EXPENSE_CATEGORY_OVERALL_EDIT, {
          ...
          setShowAlertNew(
            <>
              <ErrorOutlineOutlinedIcon
                sx={{ fontSize: "100px", color: "orange" }}
              />
              <p style={{ fontSize: "20px", fontWeight: 900 }}>{`This Category is used in these pages ${resNew.data.message},Do you want to update?`}</p>
            </>
          );
