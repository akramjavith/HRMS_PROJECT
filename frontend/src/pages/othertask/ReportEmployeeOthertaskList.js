...
function ReportEmployeeOthertaskList() {
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String("ReportEmployeeOtherTask"),
    ...
    const handleResetSearch = async () => {
        ...
        try {
            let res_employee = await axios.post(SERVICE.ALL_OTHERTASKEMPLOYEE_SORT, queryParams, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            ...
            const ChangeData = itemsWithSerialNumbers.map(item => {
                ...
                // Update item if a corresponding productionUnitCount object is found
                const points = { ...item, points: (item.mrate * item.conversion).toFixed(4), }
                ...
                } : points  // Return item unchanged if no match is found
            });
            ...
            setOverallFilterdata(ChangeData.map((item, index) => {
                return {
                    ...
                    // flagstatus: item.flagstatus,
                    conversion: item.conversion,
                    points: item?.points === "NaN" ? "" : item.points,
                    totalflag: item.totalflag,
                    mratetotal: item?.mratetotal === undefined ? "" : Number(item?.mratetotal).toFixed(4),
    ...
    const fetchEmployee = async () => {
        setStatusCheck(false)
        setPageName(!pageName)
        console.time("fetchDataTimeEmployee");
        const queryParams = {
            page: Number(page),
            pageSize: Number(pageSize),
            project: valueProjectAdd,
        ...
        try {
            ...
            setStatusCheck(true)
            // setSearchQuery("")
            console.timeEnd("fetchDataTimeEmployee");
    ...
    const fetchEmployeeExports = async () => {
        setStatusCheck(false)
        setPageName(!pageName)
        console.time("fetchDataTimeEmployeeExport");
        setIsLoading(true)
        ...
        try {
            let res_employee = await axios.post(SERVICE.ALL_OTHERTASKEMPLOYEE_SORT_EXPORT, queryParams, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
            ...
            setManageothertasksArray(ansOverall)
            ...
            // setSearchQuery("")
            console.timeEnd("fetchDataTimeEmployeeExport");
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage.toBlob(gridRefTableImg.current)
                .then((blob) => {
                    saveAs(blob, "Employee Other Task Report.png");
                })
                .catch((error) => {
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: "Report Employee Other Task",
        pageStyle: "print",
    });
    ...
    const columnDataTable = [
        ...
        },
        {
            ...
            flex: 0,
            width: 120,
            hide: !columnVisibility.mratetotal,
            headerClassName: "bold-header",
        },
        {
            field: "ticket",
    ...
    let exportColumnNames = ["Project", "Category", " Sub Category", "Date",
        "Time", "Assignedby", "Assignedmode",
        "Conversion", "Points", "Flag", "Total Point",
    ...
    let exportRowValues = ["project", "category", "subcategory",
        ...
        "conversion", "points", "total", "mratetotal",
        "ticket", "duedate", "duetime", "estimation", "estimationtime"];

    return (
        <Box>
            <Headtitle title={"Report Employee Other Task"} />
            {/* ****** Header Content ****** */}
            {/* <Typography sx={userStyle.HeaderText}>Manage Other Task</Typography> */}
            <PageHeading
                title="Report Employee Other Task"
                modulename="Other Tasks"
                submodulename="Employee Other Task Report"
                mainpagename=""
                subpagename=""
                subsubpagename=""
            />
            ...
            <>
                {isUserRoleCompare?.includes("aemployeeothertaskreport") && (
                    <Box sx={userStyle.selectcontainer}>
                        <> <Grid container spacing={2}>
                            <Grid item xs={8}>
                                <Typography sx={userStyle.importheadtext}>
                                    Report Employee Other Task Filter
                                </Typography>
                            </Grid>
                        </Grid>
            ...
            {/* ****** Table Start ****** */}
            {isUserRoleCompare?.includes("lemployeeothertaskreport") && (
                <>
                    <Box sx={userStyle.container}>
                        <Grid item xs={8}>
                            <Typography sx={userStyle.importheadtext}>
                                Report Employee Other Task List
                            </Typography>
                        </Grid>
                        <Grid container spacing={2} style={userStyle.dataTablestyle}>
                            ...
                            >
                                <Box>
                                    {isUserRoleCompare?.includes("excelemployeeothertaskreport") && (
                                        <>
                                            <LoadingButton onClick={(e) => {
                                                setIsFilterOpen(true)
                                                // fetchMangeothertaskArray()
                                                setFormat("xl")
                                            }} loading={isLoading} sx={userStyle.buttongrp}><FaFileExcel />&ensp;Export to Excel&ensp;</LoadingButton>
                                        </>
                                    )}
                                    {isUserRoleCompare?.includes("csvemployeeothertaskreport") && (
                                        <>
                                            <LoadingButton onClick={(e) => {
                                                setIsFilterOpen(true)
                                                // fetchMangeothertaskArray()
                                                setFormat("csv")
                                            }} loading={isLoading} sx={userStyle.buttongrp}><FaFileCsv />&ensp;Export to CSV&ensp;</LoadingButton>
                                        </>
                                    )}
                                    {isUserRoleCompare?.includes("printemployeeothertaskreport") && (
                                        <>
                                    ...
                                    {isUserRoleCompare?.includes("pdfemployeeothertaskreport") && (
                                        <>
                                            <LoadingButton
                                    ...
                                    {isUserRoleCompare?.includes("imageemployeeothertaskreport") && (
                                        <Button
                                            sx={userStyle.buttongrp}
            ...
            <ExportData
                ...
                filename={"Employee Other Task Report"}
...
export default ReportEmployeeOthertaskList;