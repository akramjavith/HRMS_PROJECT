...
function Stockmaster() {
  ...
  const { isUserRoleCompare, allProjects, pageName, setPageName, isAssignBranch, isUserRoleAccess, allCompany, allBranch, allUnit, allTeam, buttonStyles } = useContext(UserRoleAccessContext);
  ...
  const editSubmit = (e) => {
    ...
    if (stockmasteredit.company === 'Please Select Company') {
      ...
      setPopupContentMalert('Please Enter Warranty Time or Check Purchase and Expiry Date!');
      setPopupSeverityMalert('info');
      handleClickOpenPopupMalert();
  ...
  const fetchAll = async () => {
    ...
    try {
      const [RES_COMPANY, RES_BRANCH, RES_UNIT, RES_FLOOR, RES_AREA, RES_LOC, RES_VOM, RES_VGROUP] = await Promise.all([
      ...
      setBranchs(branchOptfirstthree);
      setBranchsEdit(branchOptfirstthree);
      ...
      setUnits(units);
      setUnitsEdit(units);
      let floor = RES_FLOOR?.data?.floors
        .filter((item, index, self) => {
          return self.findIndex((i) => i.branch === item.branch && i.name === item.name) === index;
        })
        .map((t) => ({
      ...
      let areas = RES_AREA?.data?.areagroupings
        .filter((item, index, self) => {
          return self.findIndex((i) => i.branch === item.branch && i.floor === item.floor && i.name === item.name) === index;
        })
        .map((t) => ({
          ...t,
          label: t.name,
      ...
      let location = RES_LOC?.data?.locationgroupings
        .filter((item, index, self) => {
          return self.findIndex((i) => i.branch === item.branch && i.floor === item.floor && i.area === item.area && i.name === item.name) === index;
        })
        .map((t) => ({
          ...t,
          label: t.name,
      ...
      setLocationsEdit(location);

      let codeValues = RES_VOM?.data?.vommaster.map((data) => ({
        name: data.name,
        code: data.code,
      }));
      ...
      const allGroup = Array.from(new Set(RES_VGROUP?.data?.vendorgrouping.map((d) => d.name))).map((item) => {
        return {
          label: item,
          value: item,
        };
      ...
      setVendorOverall(RES_VGROUP?.data?.vendorgrouping);
    } catch (err) {
  ...
  const fetchspecification = async (e) => {
    try {
      let res = await axios.get(SERVICE.ASSETWORKSTAION_LIMITED_BY_WORKSTATION, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        workstation: e,
      ...
      const resultall = res.data.assetworkstation.map((d) => ({
        ...d,
        label: d.categoryname,
        value: d.categoryname,
      }));
  ...
  // console.log(stockmaster.warranty, "warr")
  return (
    <Box>
