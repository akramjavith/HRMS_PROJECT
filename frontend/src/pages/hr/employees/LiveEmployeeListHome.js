...
import { Box, Button, Checkbox, Chip, DialogActions, DialogContent, FormControl, Grid, IconButton, List, ListItem, ListItemText, MenuItem, Popover, Select, TextField, Tooltip, Typography } from '@mui/material';
...
function LiveEmployeeList() {
  ...
  const { isUserRoleCompare,listPageAccessMode, isUserRoleAccess, isAssignBranch, allTeam, pageName, setPageName, buttonStyles, allUsersData, allUsersLimit } = useContext(UserRoleAccessContext);
  ...
  const fetchEmployee = async () => {
    try {
      ...
      let mappedData = res?.data?.allusers?.map((item, index) => {
        const lastExpLog = item?.assignExpLog?.length > 0 ? item.assignExpLog[item?.assignExpLog?.length - 1] : {};
        return {
          ...item,
          Sno: index + 1,
          status: item.status || '',
  ...
  const formatData = (data) => {
    return data?.map((item, index) => {
      return {
        ...
        Role: Array.isArray(item?.role) && item.role.length > 0 ? item.role?.join(',') : '',
        Aadhar: item?.aadhar,
        ...
        WorkstationPrimary: Array.isArray(item?.workstation) && item.workstation.length > 0 ? item.workstation.slice(0, 1).join(',') : '',
        WorkstationSecondary: Array.isArray(item?.workstation) && item.workstation.length > 1 ? item.workstation.slice(1).join(',') : '',
        Weekoff: Array.isArray(item?.weekoff) && item.weekoff.length > 0 ? item.weekoff?.join(',') : '',
        Wordcheck: item?.wordcheck ? 'Yes' : 'No',
        Salarysetup: item?.salarysetup ? 'Yes' : 'No',
      };
    });
  };
  const exportColumnNames = ['Status', 'Empcode', 'Employee Name', 'Username', 'Email', 'Branch', 'Unit', 'Team', 'Work Mode', 'Experience', 'DOJ'];
  const exportRowValues = ['status', 'empcode', 'companyname', 'username', 'email', 'branch', 'unit', 'team', 'workmode', 'experience', 'doj'];
  //image
...
console.log(pastedText,"pastedText")
  ...
  const updateEmployees = (pastedNames) => {
    ...
    const availableOptions = allUsersData
      ?.filter((comp) =>
         hierarchyemployee.includes(comp.companyname) &&
         valueCompanyCat?.includes(comp.company) && valueBranchCat?.includes(comp.branch) && valueUnitCat?.includes(comp.unit) && valueTeamCat?.includes(comp.team) && comp.workmode !== 'Internship')
  ...
  const handleAutoSelect = async () => {
    try {
      ...
      let selectedTeam = allTeam?.filter((u) => selectedCompany?.includes(u.company) && 
      selectedBranch?.includes(u.branch) && selectedUnit?.includes(u.unit)).map((u) => u.teamname);

      let mappedemployees = allUsersData
        ?.filter((u) =>
             hierarchyemployee.includes(u.companyname) &&
            selectedCompany?.includes(u.company) && selectedBranch?.includes(u.branch) && 
            selectedUnit?.includes(u.unit) && selectedTeam?.includes(u.team) &&
      ...
      let employees = allUsersData?.filter((u) =>
         hierarchyemployee.includes(u.companyname) &&
         selectedCompany?.includes(u.company) && 
         selectedBranch?.includes(u.branch) && selectedUnit?.includes(u.unit) &&
          selectedTeam?.includes(u.team) && u.workmode !== 'Internship').map((u) => u.companyname);
  ...
  useEffect(() => {
    handleAutoSelect();
  }, [isAssignBranch,hierarchyemployee]);
...
fetchHierarchyBasedEmployee()
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Live Employee Details" modulename="Human Resources" submodulename="HR" mainpagename="Employee" subpagename="Employee Status Details" subsubpagename="Live Employee List" />
          ...
          <Box sx={userStyle.selectcontainer}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>Live Employee List Filter</Typography>
                </Grid>
              </Grid>
              <br />
              <Grid container spacing={2}>
      ...
      {isUserRoleCompare?.includes('lliveemployeelist') && (
        <>
          <Box sx={userStyle.container}>
            <Grid container spacing={2}>
              <Grid item xs={8}>
                <Typography sx={userStyle.SubHeaderText}>Live Employees List</Typography>
              </Grid>
            </Grid>
            <br />
            <br />
