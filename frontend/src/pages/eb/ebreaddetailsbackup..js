...
function EBReadingDetails() {
    ...
    //           const resultcheck = ebreadingdetails.filter(item =>
    //             item.readingmode === "Month Closing"
    ...
    //             item.company === ebreadingdetail.company && item.branch === ebreadingdetail.branch && item.unit === ebreadingdetail.unit
    //             && item.floor === ebreadingdetail.floor && item.area.includes(ebreadingdetail.area) && item.servicenumber === ebreadingdetail.servicenumber
    //           );
    //           let lastvalue = result ? result : servicemasterval
    ...
    //       setEbreadingdetail({
    //         ...ebreadingdetail,
    //         readingmode: "", date: formattedDate, time: "", openkwh: "", kvah: "", kwhunit: "", kvahunit: "", pf: "", md: "", description: "",
    //         pfrphase: "", pfyphase: "", pfbphase: "", pfcurrent: "", pfaverage: "", mdrphase: "", mdyphase: "", mdbphase: "", mdcurrent: "",
    //         mdaverage: "",
    //       })
    ...
    //     //   setloadingdeloverall(false)
    //     //   setPopupContentMalert(
    //     //     `You have Already Added two more dates after this Date ${ebreadingdetail.date}`
    //     //   );
    //     //   setPopupSeverityMalert("info");
    //     //   handleClickOpenPopupMalert();
    ...
    const handleClear = (e) => {
        ...
        setEbreadingdetail({
            company: "Please Select Company",
            branch: "Please Select Branch",
            unit: "Please Select Unit", floor: "Please Select Floor",
            area: "Please Select Area",
            readingmode: "", description: "", date: formattedDate, time: "", openkwh: "", kvah: "", kwhunit: "", kvahunit: "", pf: "", md: "",
            pfrphase: "", pfyphase: "", pfbphase: "", pfcurrent: "", pfaverage: "", mdrphase: "", mdyphase: "", mdbphase: "", mdcurrent: "",
            mdaverage: "",
        })
        setReadingmode("Daily Closing");
    ...
    const fetchEbreadingdetailsFilterDate = async (date) => {
        ...
        try {
            ...
            if (res_read?.data?.ebreadingfiltertime.length > 0) {
                ...
                let mostRecentDate = {}
                let mostRecentDatereading
                if (openEntries.length > 0) {

                    let logbeforecreated = openEntries.some(current => new Date(current.date) > new Date(current.createdAt))
                    console.log(logbeforecreated, "logbeforecreated")
                    if (logbeforecreated) {

                        mostRecentDatereading = varfilter.filter(item => item.readingmode !== "Open Entry").reduce((prev, current) => (new Date(current.createdAt) > new Date(prev.createdAt) ?
                            current : prev));
                        mostRecentDate = openEntries.reduce((prev, current) => (new Date(current.date) > new Date(prev.date) ?
                            current : prev));

                        console.log(mostRecentDatereading, mostRecentDate, "ssssss")
                        //equals log
                        ...
                        //date of(17)    
                        else if (new Date(mostRecentDate.date) < new Date(date) && mostRecentDate.date == (mostRecentDatereading.date) && new Date(mostRecentDate.createdAt) >= new Date(mostRecentDatereading.createdAt)) {
                            mostRecentDate = mostRecentDate
                        }
                        //date of(20)    

                        else if (new Date(mostRecentDate.date) < new Date(date) && new Date(mostRecentDate.date) > new Date(mostRecentDatereading.date)) {
                            mostRecentDate = mostRecentDate
                        }
                        ...
                        mostRecentDate = varfilter.reduce((prev, current) => (new Date(current.createdAt) > new Date(prev.createdAt) ?
                            current : prev));
                    }
                }
                else {
                    ...
                    let serviceoldRecentDate = serviceold.reduce((prev, current) => (new Date(current.servicedate) > new Date(prev.servicedate) ?
                        current : prev));

                    mostRecentDate = varfilter.reduce((prev, current) => (new Date(current.date) > new Date(prev.date) ?
                        current : prev));

                    if (new Date(mostRecentDate.createdAt) >= new Date(serviceoldRecentDate.createdAt)) {
            ...
            if (res_read?.data?.ebreadingfiltertime.length > 0) {
                ...
                if (openEntries.length > 0) {
                    ...
                    if (logbeforecreated) {
                        ...
                        //date of(17)    
                        else if (new Date(mostRecentDate.date) < new Date(ebreadingdetail.date) && mostRecentDate.date == (mostRecentDatereading.date) && new Date(mostRecentDate.createdAt) >= new Date(mostRecentDatereading.createdAt)) {
                            mostRecentDate = mostRecentDate
                        }
                        //date of(20)    

                        else if (new Date(mostRecentDate.date) < new Date(ebreadingdetail.date) && new Date(mostRecentDate.date) > new Date(mostRecentDatereading.date)) {
                            mostRecentDate = mostRecentDate
                        }
                ...
                if (Number(kwhval) > res_read?.data?.ebreadingfiltertimefuture[0]?.openkwh) {
                    ...
                    if (openEntries.length > 0) {
                        ...
                        if (logbeforecreated) {
                            ...
                            console.log(mostRecentDatereading, mostRecentDate, "ssssss")
                            //equals log
                            if ((mostRecentDate.date) == (ebreadingdetail.date) && openEntries.length > 0) {
                                mostRecentDate = mostRecentDate
                            }
                            else if (new Date(mostRecentDate.date) < new Date(ebreadingdetail.date) && mostRecentDate.date == (mostRecentDatereading.date) && new Date(mostRecentDate.createdAt) >= new Date(mostRecentDatereading.createdAt) && openEntries.length > 0 && ReadingEntries.length > 0) {
                                mostRecentDate = mostRecentDate
                            }
                            else if (new Date(mostRecentDate.date) < new Date(ebreadingdetail.date) && new Date(mostRecentDate.date) > new Date(mostRecentDatereading.date) && openEntries.length > 0 && ReadingEntries.length > 0) {
                                mostRecentDate = mostRecentDate
                            }
                            else {
                                mostRecentDate = mostRecentDatereading
                        ...
                        if (new Date(mostRecentDate.createdAt) >= new Date(serviceoldRecentDate.createdAt) && serviceold.length > 0 && varfilter.length > 0) {
                            mostRecentDate = serviceoldRecentDate
                        } else {
                            mostRecentDate = mostRecentDate
                        }
                        ...
                        <Box sx={userStyle.dialogbox}>
                            <>
                                ...
                                <Grid container spacing={2}>
                                        ...
                                        <FormControl fullWidth size="small">
                                            ...
                                            <Selects
                                                options={isAssignBranch?.filter(
                                                    (comp) =>
                                                        ebreadingdetail.company === comp.company
                                                )?.map(data => ({
                                                    label: data.branch,
                                                    value: data.branch,
                                                })).filter((item, index, self) => {
                                    ...
                                    <Grid item md={3} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            ...
                                            <Selects
                                                options={isAssignBranch?.filter(
                                                    (comp) =>
                                                        ebreadingdetail.company === comp.company && ebreadingdetail.branch === comp.branch
                                                )?.map(data => ({
                                                    label: data.unit,
                                                    value: data.unit,
                                                })).filter((item, index, self) => {
                                                ...
                                                onChange={(e) => {
                                                    setEbreadingdetail({
                                                        ...ebreadingdetail, unit: e.value, servicenumber: "Please Select Service",
                                                        floor: "Please Select Floor",
                                                        area: "Please Select Area",
                                                    });
                                    ...
                                    <Grid item md={3} xs={12} sm={12}>
                                        <FormControl fullWidth size="small">
                                            <Typography>KVAH Reading <b style={{ color: "red" }}>*</b> </Typography>
                                            <OutlinedInput
                                                id="component-outlined"
                                                type="number"
                                                sx={userStyle.input}
