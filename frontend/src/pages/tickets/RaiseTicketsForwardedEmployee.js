...
function RaiseTicketForwardedEmployee() {
    ...
    let exportColumnNames = ['Status', 'Ticket Number',
        'Raised By', 'Raised Date',
        'Due Date', 'Ticket Due Date',
        'Remaining Hours', 'Reason ',
    ...
    const fetchAllRaisedTicketsed = async () => {
        ...
        try {
            ...
            const answer = response?.data?.raisetickets?.length > 0 ? response?.data?.raisetickets?.map((item, index) => ({
