...
function HierarchyApprovalEmployeeDocuments() {
    ...
    const fetchAppprovalTemplateDocuments = async () => {
        ...
        try {
            ...
            setLoader(true);
            if (res_vendor?.data?.resultedTeam?.length > 0 && res_vendor?.data?.resultAccessFilter?.length < 1 && ["myallhierarchy", "allhierarchy"]?.includes(modeselection.value)) {
    ...
    return (
        <Box>
            ...
            <PageHeading title="Hierarchy Approval Employee Documents" modulename="Human Resources" submodulename="HR Documents" mainpagename="Employee Documents" subpagename="Hierarchy Approval Employee Documents" subsubpagename="" />
