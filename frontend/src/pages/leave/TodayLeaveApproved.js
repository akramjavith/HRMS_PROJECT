...
function TodayLeaveApproved() {
    ...
    const { isUserRoleCompare, allProjects, isUserRoleAccess, isAssignBranch } = useContext(UserRoleAccessContext);
    ...
    const fetchLeaveCriteriaEdit = async (empname, empdepartment, empdesignation, leavetype, empdoj, empweekoff, empid, empcompany, empbranch, empunit, empteam) => {
    ...
    const getCode = async (e, name) => {
        try {
            ...
            fetchLeaveCriteriaEdit(
                ...
                (Accessdrop === "HR" ? res?.data?.sapplyleave.company : isUserRoleAccess.company),
                (Accessdrop === "HR" ? res?.data?.sapplyleave.branch : isUserRoleAccess.branch),
                (Accessdrop === "HR" ? res?.data?.sapplyleave.unit : isUserRoleAccess.unit),
                (Accessdrop === "HR" ? res?.data?.sapplyleave.team : isUserRoleAccess.team),
            );
    ...
    const fetchApplyleave = async () => {
        try {
            let res_vendor = await axios.post(SERVICE.LEAVE_HOME_LIST, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
