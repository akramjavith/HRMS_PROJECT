...
function Stockmaster({ sendDataToParentUIStock, frequencyValue, openpop, stockmaterialedit,Specificationeditcomp,
  handleCloseviewalertvendorstock,stockmaterialedittodo, vendorNew, vendorGroup, stockManagehand, handover }) {
  ...
  const [expensecreate, setExpensecreate] = useState({
    ...
    totalbillamount: '',
    duedate:stockmaterialedit ? stockmaterialedit?.duedate : "",
    date,
    files: '',
    vendorfrequency: '',
  ...
  const [todoDetails, setTodoDetails] = useState({
    category: stockmaterialedittodo.category,
    subcategory: stockmaterialedittodo.subcategory,
    materialnew: stockmaterialedittodo.materialnew,
    uomnew: stockmaterialedittodo.uomnew,
    rate: stockmaterialedittodo.rate,
    quantitynew: '',
    amount: stockmaterialedittodo.amount,
    productdetailsnew: stockmaterialedittodo.productdetailsnew,
  });
  ...
  const [setFrequencyValue] = useState('');
  ...
  const [setVendorGroup] = useState('Please Select Vendor Group');
  const [vendorGroupOpt, setVendorGroupopt] = useState([]);
  const [vendorOverall, setVendorOverall] = useState([]);
  const [setVendorNew] = useState('Please Select Vendor');
...
//     workcheck: false,
//     producthead:stockmaterialedit.producthead,
//     vendorname: 'Please Select Vendor',
...
//     productname:stockmaterialedit.productname,
...
//     quantity: '',
//     rate: '',
//     billdate: stockmaterialedit.billdate,
//     files: '',
//     warrantyfiles: '',
//     totalbillamount: stockmaterialedit.reqeustmode === "Stock Material" ? stockmaterialedit.totalbillamountstock : stockmaterialedit.totalbillamount,
...
//   });
   ...
   const [stockmaster, setStockmaster] = useState({
        ...
        assettype: stockmaterialedit.assettype,
        asset: stockmaterialedit.producthead,
  ...
  const fetchVomMaster = async (e) => {
    try {
      ...
      let getdata = res_vom.data.managestockitems.filter((data) => {
        return data.itemname === stockmaster.materialnew && data.stockcategory === stockmaster.stockcategory && data.stocksubcategory === e;
      });
  ...
  const fetchSubcategoryBased = async (e) => {
    try {
      ...
      let data_set = res_category.data.managestockitems.filter((data) => {
  ...
  const [Specification, setSpecification] = useState(Specificationeditcomp);
  const [specificationedit, setSpecificationedit] = useState([]);
console.log(Specificationeditcomp,"Specificationeditcomp")
  ...
  const delProject = async () => {
    try {
      ...
      setShowAlert(
        <>
          <DeleteOutlineOutlinedIcon sx={{ fontSize: '100px', color: 'red' }} />
  ...
  const sendRequest = async () => {
    ...
    const totalQuantityStock = educationtodo.reduce((sum, item) => {
    ...
    try {
      let stockcreate = await axios.post(SERVICE.MANUAL_STOCKPURCHASE_SINGLE, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        ...
        countquantity: String(totalQuantityStock),
...
if(stockmaster.requestmode === "Stock Material"){
   ...
   if (stockmaster.company === 'Please Select Company') {
      ...
      handleClickOpenPopupMalert();
        ...
        else if (handover.balancedcount <= stockmaster.quantity) {
      setPopupContentMalert("Please Enter Less Than  Count!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    }
}
  ...
  useEffect(() => {
    ...
    const result = filteredWorks.flatMap((item) => {
      const cabinNamesArray =
        item.combinstation.length > 0 && item.combinstation[0].subTodos.length > 0 ? item.combinstation[0].subTodos.map((subTodo) => ({ label: subTodo.subcabinname, value: subTodo.subcabinname })) : [{ label: item.combinstation[0].cabinname, value: item.combinstation[0].cabinname }];
...
//   }, [isEditOpen]);
  ...
  useEffect(() => {
      handleAddInput(stockmaterialedit.component)
    }, [stockmaterialedit.component, Specification, stockmaterialedit.productname])
  ...
  const educationTodo = () => {
    const isNameMatch = educationtodo?.some((item) => {
      if (stockmaster?.requestmode === 'Stock Material') {
        return item?.category === todoDetails?.category && item?.subcategory === todoDetails?.subcategory && item?.materialnew?.toLowerCase() === todoDetails?.materialnew?.toLowerCase() && item?.uomnew?.toLowerCase() === todoDetails?.uomnew?.toLowerCase();
      } else {
    ...
    else if (handover.balancedcount < todoDetails.quantitynew) {
      setPopupContentMalert("Please Enter Less Than  Count!");
      setPopupSeverityMalert("info");
      handleClickOpenPopupMalert();
    }
