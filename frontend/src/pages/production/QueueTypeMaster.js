...
function QueueTypeMaster() {
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      const batches = [];
      const types = selectedTypeFrom.map((d) => d.value);
      // Flatten the combinations of subcategories and types
      const combinedDataAlerted = selectedSubCategoryFrom.flatMap((subcategory) => types.map((type) => ({ subcategory: subcategory.value, orate: subcategory.orate, type })));

      const orates = selectedOrate.map((d) => d.value);
      const combinedData = combinedDataAlerted.flatMap((item) => {
        if (item.subcategory === 'All') {
          return orates.map((rate) => ({ subcategory: item.subcategory, type: item.type, orate: rate }));
        } else {
          return item;
        }
      });
      console.log(combinedData, selectedSubCategoryFrom, 'combinedData');
  ...
  const handleNewRateChange = (index, value) => {
    setDialogData((prevData) => prevData.map((item, idx) => (idx === index ? { ...item, newrate: value } : item)));
  };
  ...
  const fetchAllQueueTypeMaster = async () => {
    ...
    try {
      ...
      setGroups(
        res_grp?.data?.queuetypemasters.map((item, index) => ({
          ...item,
          id: item._id,
  ...
  const fetchAllTypemaster = async (proj) => {
    ...
    try {
      ...
      const dynamicOptions = Array.from(new Set(res_module?.data?.typemasters.map((t) => t.name))).map((name) => ({
        label: name,
        value: name,
      }));
      ...
      const categoryOpt = Array.from(new Map(combinedOptions.map((option) => [option.label.toLowerCase(), option])).values());
  ...
  const fetchAllSubCategory = async (category, orate) => {
    ...
    try {
      ...
      const addAll = [{ label: 'All', value: 'All' }, ...branchdata];

      setSubcategories(addAll);
    } catch (err) {
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
    }
  };
  ...
  const fetchorate = async (proj) => {
    ...
    try {
      ...
      const rate = managecategoryEdit.type === 'Other task queues' ? (['0', '0.000', '', '0.00'].includes(mrate) ? 0 : mrate) : ['0', '0.000', '', '0.00'].includes(orate) ? 0 : orate;
  ...
  const handleOrateEdit = (e) => {
    setManageCategoryEdit({
      ...managecategoryEdit,
      orate: e.target.value,
    });
  };
  ...
  const handleTypeChangeFrom = (selected) => {
    const isOtherTaskSelected = selected.some((option) => ['Other task queues', 'Other task upload'].includes(option.value));

    // If "Other task queues" is selected, keep only that option
    if (isOtherTaskSelected) {
      setSelectedTypeForm(selected.filter((option) => ['Other task queues', 'Other task upload'].includes(option.value)));
    } else {
  ...
  return (
    <Box>
      ...
      <PageHeading title="Queue Type Master" modulename="Production" submodulename="SetUp" mainpagename="Queue Type Master" subpagename="" subsubpagename="" />
          ...
          <Box sx={userStyle.dialogbox}>
            <>
              <Grid container spacing={2}>
                <Grid item xs={8}>
                  <Typography sx={userStyle.importheadtext}>Add Queue Type Master</Typography>
                </Grid>
              </Grid>
              <br />
              <Grid container spacing={2}>
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect
                      options={typeMaster.map((option) => ({
                        ...option,
                        disabled: selectedTypeFrom.some((selected) => ['Other task queues', 'Other task upload'].includes(selected.value)) && !['Other task queues', 'Other task upload'].includes(option.value),
                      }))}
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect options={Oratedrop} value={selectedOrate} onChange={handleOrate} valueRenderer={customValueRendererOrate} labelledBy="Please Select Orate" />
                  </FormControl>
                </Grid>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <MultiSelect options={subCtegoryOpt} value={selectedSubCategoryFrom} onChange={handleSubCategoryChangeFrom} valueRenderer={customValueRendererSubcategoryFrom} labelledBy="Please Select SubCategory" />
                  </FormControl>
                </Grid>
              ...
              <form onSubmit={editSubmit}>
                <Grid container spacing={2}>
                  <Grid item md={12} xs={12} sm={12}>
                    <Typography sx={userStyle.HeaderText}>Edit QueueType Master</Typography>
                  </Grid>
                </Grid>
                <br />
                <Grid container spacing={2}>
                  ...
                  <Grid item md={3} xs={12} sm={12} lg={3}>
                    <FormControl fullWidth size="small">
                      ...
                      <OutlinedInput id="component-outlined" disabled={managecategoryEdit.subcategory != 'All'} onChange={handleOrateEdit} type="Number" sx={userStyle.hideArrows} value={managecategoryEdit.orate} />
                    </FormControl>
                  </Grid>
                  <Grid item md={3} xs={12} sm={12} lg={3}>
                    <FormControl fullWidth size="small">
          ...
          <Box sx={userStyle.container}>
            ...
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              <Grid item md={2} xs={6} sm={6}>
                <Box>
                  <AggregatedSearchBar columnDataTable={columnDataTable} setItems={setItems} addSerialNumber={addSerialNumber} setPage={setPage} maindatas={groups} setSearchedString={setSearchedString} searchQuery={searchQuery} setSearchQuery={setSearchQuery} paginated={false} totalDatas={groups} />
                </Box>
              </Grid>
            </Grid>
            <br />
          ...
          <>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={6} xs={12} sm={12} lg={6}>
                <FormControl fullWidth size="small">
                  <Typography variant="h6">O-Rate/M-Rate</Typography>
                  <Typography>{viewcode.orate}</Typography>
                </FormControl>
              </Grid>
              <Grid item md={6} xs={12} sm={12} lg={6}>
                <FormControl fullWidth size="small">
                  <Typography variant="h6">New-Rate</Typography>
      ...
      <Dialog open={isEditOpensub} onClose={handleClickOpenEditSub} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" fullWidth={true} maxWidth="lg" sx={{ marginTop: '95px' }}>
        <Box sx={{ padding: '20px' }}>
          <>
            ...
            {dialogData.map((data, index) => (
              <Grid container spacing={2}>
                ...
                <Grid item md={2} xs={12} sm={12} lg={2}>
                  <FormControl fullWidth size="small">
                    <Typography variant="h6">O-Rate</Typography>
                    <Typography
                      sx={{
                        whiteSpace: 'pre-wrap',
                        wordBreak: 'break-word',
                ...
                <Grid item md={2} xs={12} sm={12} lg={2}>
                  <FormControl fullWidth size="small">
                    ...
                    <OutlinedInput id="component-outlined" type="Number" sx={userStyle.hideArrows} value={data.newrate} onChange={(e) => handleNewRateChange(index, e.target.value)} />
                  </FormControl>
                </Grid>
              </Grid>
            ))}
      ...
      <Dialog open={openviewalert} onClose={handleClickOpenviewalert} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        <Box sx={{ width: '550px', padding: '20px 50px' }}>
          <>
            ...
            <Grid container spacing={2}>
              <Grid item md={2} xs={12} sm={12}>
                <Button variant="contained" sx={buttonStyles.buttonsubmit} onClick={handleCloseviewalert}>
                  Save
                </Button>
              </Grid>
              ...
              <Grid item md={2} xs={12} sm={12}>
                <Button variant="contained" sx={buttonStyles.btncancel} onClick={handleCloseviewalert}>
                  {' '}
                  Cancel
                </Button>
              </Grid>
      ...
      />
      {/* INFO */}
      <InfoPopup openInfo={openInfo} handleCloseinfo={handleCloseinfo} heading="Queue Type Master Info" addedby={addedby} updateby={updateby} />
      {/*SINGLE DELETE ALERT DIALOG ARE YOU SURE? */}
      <DeleteConfirmation open={isDeleteOpen} onClose={handleCloseMod} onConfirm={deleGroup} title="Are you sure?" confirmButtonText="Yes" cancelButtonText="Cancel" />
      {/*BULK DELETE ALERT DIALOG ARE YOU SURE? */}
      <DeleteConfirmation open={isDeleteOpencheckbox} onClose={handleCloseModcheckbox} onConfirm={delGroupcheckbox} title="Are you sure?" confirmButtonText="Yes" cancelButtonText="Cancel" />
