...
function ProductionOriginalSummaryView() {
    ...
    let exportRowValues =
        ['vendor', 'vendornew', 'formatteddate', "matched", 'uploaddate', 'processdate', 'filenameupdated',
            'type',
            'category', 'matchcount',
            'orate', 'flagcount',
            'oldtotal',
    ...
    const fetchBatchFilter = async (data) => {
        setProjmaster([]);
        setLoaderList(true);
        try {
            ...
            let res_employee = await axios.post(
                SERVICE.PRODUCTION_ORIGINAL_SUMMARY_REPORT_VIEW,
                {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
...
export default ProductionOriginalSummaryView;