...
function TempDayPoints() {
  ...
  let exportRowValuesViewAll = ['name', 'empcode', 'company', 'branch', 'unit', 'team', 'datenew', 'user', 'unitid', 'filename', 'category', 'exper', 'target', 'points', 'avg', 'processcode', 'aprocess', 'sprocess', 'contarget', 'conpoints', 'conavg'];
  ...
  const addSerialNumberviewAll = () => {
    ...
    const itemsWithSerialNumber = productiontempviewAll?.map((item, index) => {
      ...
      return {
        ...
        point: Number(item.point).toFixed(5),
        production: Number(item.production).toFixed(5),
        manual: Number(item.manual).toFixed(5),
        avgpoint: Number(item.avgpoint).toFixed(5),
        ...
        allowancepoint: item.type === 'Created' ? (item.shiftsts === 'Enable' ? Number(item.conshiftpoints).toFixed(5) : 0) : item.allowancepoint,
        nonallowancepoint: diff < 1e-1 ? 0 : Number(diff) > 0 ? Number(diff).toFixed(5) : 0,
      };
    });
  ...
  const downloadPdfviewall = (isfilter) => {
    ...
    // });
    doc.save('Production Temp Point Creation.pdf');
  };
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Grid sx={{ display: 'flex' }}>
          ...
          {isUserRoleCompare?.includes('vtempdaypoints') && (
            <Button
  ...
  return (
    <Box>
      ...
      <ExportData
        ...
        itemsTwo={itemsList ?? []}
        filename={'Production Temp Day Point'}
      ...
      <ExportDataView
        ...
        filename={'Production Temp Day Points'}
