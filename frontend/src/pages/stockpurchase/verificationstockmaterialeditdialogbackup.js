...
function Stockmaterialeditdialog({
  ...
  const getinfoCode = async (e) => {
    ...
    try {
      ...
      let quantityNew = res?.data?.sstock.stockmaterialarray.map((data, newindex) => {
        return ` ${data.quantitynew}`;
      });
      setQuantityNeww(quantityNew.toString());

      let materialNew = res?.data?.sstock.stockmaterialarray.map((data, newindex) => {
        return ` ${data.materialnew}`;
      });
      setMaterialNeww(materialNew.toString());

      let productdetailsNew = res?.data?.sstock.stockmaterialarray.map((data, newindex) => {
        return ` ${data.productdetailsnew}`;
      });
      setProductdetailsNeww(productdetailsNew.toString());

      let quantityAndUom = res?.data?.sstock.stockmaterialarray.map((data, newindex) => {
      ...
      isfilter === 'filtered'
        ...
        : stockmanages.map((row) => {
            ...
            return {
              ...
              quantitynew: row.stockmaterialarray.map((data) => ` ${data.quantitynew}`),
              uomnew: row.stockmaterialarray.map((data) => ` ${data.quantitynew}#${data.uomcodenew}`),
              materialnew: row.stockmaterialarray.map((data) => ` ${data.materialnew}`),
              productdetailsnew: row.stockmaterialarray.map((data) => ` ${data.productdetailsnew}`),
            };
          });
    ...
    if (isfilter === 'filtered') {
      ...
      exportToCSV(
        stockmanages.map((item, index) => ({
          ...
          Quantity: item.stockmaterialarray.map((data) => ` ${data.quantitynew}`).join(','),

          'Quantity & UOM': item.stockmaterialarray.map((data) => ` ${data.quantitynew}#${data.uomcodenew}`).join(','),

          Material: item.stockmaterialarray.map((data) => ` ${data.materialnew}`).join(','),

          'Product Details': item.stockmaterialarray.map((data) => ` ${data.productdetailsnew}`).join(','),
          'GST No': item.gstno,
          'Bill No': item.billno,
          'Warranty Details': item.warrantydetails,
          Warranty: item.warranty,
          'Purchased Date': item.purchasedate != '' ? moment(item.purchasedate).format('DD/MM/YYYY') : '',
