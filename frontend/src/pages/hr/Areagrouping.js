...
function AreaGrouping() {
  ...
  const checkUsedByOther = async (data) => {
    let AreaDatas = data?.area.split(",");

    let combinedData = AreaDatas?.map((area) => ({
      company: data?.company,
      branch: data?.branch,
      unit: data?.unit,
      floor: data?.floor,
    ...
    try {
      let res = await axios.post(SERVICE.AREAGROUPINGOVERALLDELETECONDITION, {
        combinedData,
      });
      if (res.data.exists) {
  ...
  const getCode = async (e, data) => {
    ...
    try {
      const [res, resNew] = await Promise.all([
        axios.get(`${SERVICE.AREAGROUPING_SINGLE}/${e}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        }),
        axios.post(SERVICE.AREAGROUPINGOVERALLDELETECONDITION, {
          combinedData,
        }),
      ]);
