...
function BranchLogList({ boardinglogs, userID }) {
  ...
  const rowDataArray = async () => {
    try {
      ...
      const getresarr = getarr.filter((data, index) => {
        return data.ischangebranch === true
      });
  ...
  const [boardinglogsBranch, setBoardinglogsCompany] = useState([]);
  const [items, setItems] = useState([]);
  const [selectedRows, setSelectedRows] = useState([]);
  const [branchlogcheck, setBranchlogcheck] = useState(false);
  ...
  const fetchAllUsersLimit = async () => {
    try {
      ...
      setBranchlogcheck(true);
    } catch (err) { setBranchlogcheck(true); handleApiError(err, setShowAlert, handleClickOpenerr); }
  };
  ...
  // Excel
  const fileName = "Branch Log List";

  // print...
  const componentRef = useRef();
  const handleprint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: "Branch Log List",
    pageStyle: "print",
  });
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Branch Log List.png");
        });
      });
    }
  };
  ...
  const addSerialNumber = () => {
    const itemsWithSerialNumber = boardinglogsBranch?.map((item, index) => ({
      ...item,
      ...
      <TableContainer component={Paper} sx={userStyle.printcls}>
        ...
        >
          ...
          <TableBody align="left">
            {boardinglogsBranch &&
              boardinglogsBranch.map((row, index) => (
                <TableRow key={index}>
