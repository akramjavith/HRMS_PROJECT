...
function ProductionTempSummary() {
  ...
  let exportColumnNames = ["Project", "Vendor", "Date", "Category", "Sub Category", "Count", "ORate", "Flag Count", "Total"];

  let exportRowValues = ["vendor", "vendornew", "formatteddate", "filenameupdated", "category", "matchcount", "orate", "flagcount", "oldtotal"];
  ...
  //get all project.
  const fetchProjVendorCategory = async () => {
    try {
      const [res_project, res_vendor, res_module, RES_SUBCATEGORY] = await Promise.all([
        axios.get(SERVICE.PROJECTMASTER, {
          headers: { Authorization: `Bearer ${auth.APIToken}` },
        }),
        axios.get(SERVICE.VENDORMASTER_LIMITED_NAMEONLY, {
          headers: { Authorization: `Bearer ${auth.APIToken}` },
        }),
        axios.get(SERVICE.CATEGORY_PROD_LIMITED_PROJECT, {
          headers: { Authorization: `Bearer ${auth.APIToken}` },
        }),
        axios.get(SERVICE.SUBCATEGORYPROD_REPORT_LIMITED, { headers: { Authorization: `Bearer ${auth.APIToken}` } }),
      ]);

      const projFilt = res_vendor?.data?.vendormaster?.map((t) => ({
        ...t,
        label: t.name,
        value: t.name,
      }));
      ...
      const categoryOpt = res_module?.data?.categoryprod?.map((t) => ({
        ...t,
        label: t.name,
        value: t.name,
      }));
      ...
      const subcategories = RES_SUBCATEGORY?.data?.subcategoryprod.map((t) => ({
        ...t,
        label: t.name,
        value: t.name,
      }));
  ...
  //     try {
  //       const [res_employee, res_bulk] = await Promise.all([
  //         axios.post(SERVICE.PRODUCTION_TEMP_SUMMARY_REPORT, {
  //           headers: {
  //             Authorization: `Bearer ${auth.APIToken}`,
  //           },
  ...
  //           todate: todate,
  //         }),
  //         axios.post(SERVICE.PRODUCTON_SUMMMARY_BULK_FILTER, {
  //           headers: {
  //             Authorization: `Bearer ${auth.APIToken}`,
  //           },
  //           fromdate: fromdate,
  ...
  //           return d.category === item.category && d.filenameupdated === item.filenameupdated && d.project === item.vendor && d.vendor === item.vendornew && fromDateValid && toDateValid;
  //         });
  ...
  const fetchBatchFilter = async () => {
    ...
    try {
      async function fetchDataInBatches() {
        ...
        let allusers = [];
        let apiUrl;
        let totalBatchNumber = Math.ceil(selectedOptionsSubCategory.length / selectedOptionsSubCategory.length);

        let finalsubcategory = [...new Set(subcategory?.filter((comp) => selectedOptionsCategory.map((item) => item.value).includes(comp.categoryname))?.map((com) => com.name))].length === [...new Set(selectedOptionsSubCategory.map((item) => item.value))].length ? [] : [...new Set(selectedOptionsSubCategory.map((item) => item.value))];
        setLoaderList(true);
        while (hasMoreData) {
          try {
            apiUrl = SERVICE.PRODUCTION_TEMP_SUMMARY_REPORT;

            const currentBatch = Number(batchNumber) * 2000;
            const beforeBatch = (Number(batchNumber) - 1) * 2000;
            ...
            if (batchNumber > 0 && batchNumber > totalBatchNumber) {
              ...
              const response = await axios.post(
                apiUrl,
                {
                  filename: categoryOpt.filter((item) => selectedProject.map((t) => t.value).includes(item.project)).length === selectedOptionsCategory.map((item) => item.value).length ? [] : selectedOptionsCategory.map((item) => item.value),
                  ...
                  totalBatchNumber: totalBatchNumber,
                },
                {
                  headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
      ...
      fetchDataInBatches().then(async (allData) => {
        try {
          let res_bulk = await axios.post(SERVICE.PRODUCTON_SUMMMARY_BULK_TEMP_FILTER, {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
            fromdate: fromdate,
          ...
          const bulkdata = res_bulk.data.productiontempbulks;
  ...
  const handleprint = useReactToPrint({
    ...
    documentTitle: "Production Temp Summary ",
    pageStyle: "print",
  });
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Grid sx={{ display: "flex" }}>
          ...
          {params.data.matched === true ? <Chip sx={{ height: "25px", borderRadius: "0px" }} color={"success"} variant="outlined" label={"Created"} /> : <Chip sx={{ height: "25px", borderRadius: "0px" }} color={"warning"} variant="outlined" label={"Pending"} />}
        </Grid>
      ),
    },
  ...
  const rowDataTable = filteredData.map((item, index) => {
    return {
      ...
      oldtotal: Number(item.oldtotal)?.toFixed(2),
      flagcount: item.flagcount,
      newrate: item.newrate,
      newtotal: Number(item.newtotal)?.toFixed(2),
  ...
  const getapi = async () => {
    let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
      ...
      pagename: String("Production Temp Summary "),
  ...
  const delGroupcheckbox = async () => {
    ...
    if (selectedRowsBulk.filter((item) => item.matched != true).length === 0) {
      ...
      try {
        const deletePromises = selectedRowsBulk
          .filter((item) => item.matched != true)
          ?.map((item) => {
            return axios.post(`${SERVICE.GET_PRODUCTIONTEMP_BULK_CREATE}`, {
              headers: {
                Authorization: `Bearer ${auth.APIToken}`,
              },
              project: item.vendor,
  ...
  const delGroupcheckboxundo = async () => {
    ...
    try {
      ...
      const bulkundo = await axios.post(`${SERVICE.PRODUCTON_SUMMMARY_TEMP_BULK_UNDO}`, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
        rows: deletePromises,
  ...
  const handleCaptureImageview = () => {
    if (gridRefview.current) {
      html2canvas(gridRefview.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, "Production Temp Summary View .png");
        });
      });
    }
  };
  ...
  const handleprintview = useReactToPrint({
    ...
    documentTitle: "Production Temp Summary View",
    pageStyle: "print",
  });
  ...
  const getviewCode = async (data) => {
    try {
      ...
      let res = await axios.post(SERVICE.PRODUCTION_TEMP_SUMMARY_REPORT_VIEW, {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
  ...
  return (
    <Box>
      <Headtitle title={"Production Temp Summary"} />
      {/* ****** Header Content ****** */}
      <PageHeading title="Production Temp Summary" modulename="Production" submodulename="SetUp" mainpagename="Production Temp Summary" subpagename="" subsubpagename="" />{" "}
      <>
        <Box sx={userStyle.selectcontainer}>
          <Grid item xs={8}>
            <Typography sx={userStyle.importheadtext}>Production Temp Summary </Typography>
          </Grid>
          <>
            <Grid container spacing={2}>
              ...
              <Grid item md={3} xs={12} sm={12}>
                <FormControl fullWidth>
                  ...
                  <MultiSelect options={categoryOpt.filter((item) => selectedProject.map((t) => t.value).includes(item.project))} value={selectedOptionsCategory} onChange={(e) => handleCategoryChange(e)} valueRenderer={customValueRendererCategory} labelledBy="Please Select Category" />
                </FormControl>
              </Grid>
      ...
      <br />
      {/* ****** Table Start ****** */}
      {isUserRoleCompare?.includes("lproductionmonthupload") && (
        <>
          <Box sx={userStyle.container}>
            {/* ******************************************************EXPORT Buttons****************************************************** */}
            <Grid item xs={8}>
              <Typography sx={userStyle.importheadtext}>Production Temp Summary List</Typography>
            </Grid>
            <Grid container spacing={2} style={userStyle.dataTablestyle}>
              ...
              <Grid item md={8} xs={12} sm={12} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                <Box>
                  {isUserRoleCompare?.includes("excelproductionmonthupload") && (
                    <>
                      <Button
                        onClick={(e) => {
                  ...
                  {isUserRoleCompare?.includes("csvproductionmonthupload") && (
                    <>
                      <Button
                        onClick={(e) => {
                          setIsFilterOpen(true);
                  ...
                  {isUserRoleCompare?.includes("printproductionmonthupload") && (
                    <>
                  ...
                  {isUserRoleCompare?.includes("pdfproductionmonthupload") && (
                    <>
                      <Button
                        sx={userStyle.buttongrp}
                        onClick={() => {
                  ...
                  {isUserRoleCompare?.includes("imageproductionmonthupload") && (
                    <>
              ...
              <Grid item md={2} xs={12} sm={12}>
                <Box>
                  <AggregatedSearchBar columnDataTable={columnDataTable} setItems={setItems} addSerialNumber={addSerialNumber} setPage={setPage} maindatas={projmaster} setSearchedString={setSearchedString} searchQuery={searchQuery} setSearchQuery={setSearchQuery} paginated={false} totalDatas={projmaster} />
                </Box>
              </Grid>
            </Grid>
            <br />
            ...
            &ensp;
            <Button onClick={handleClickOpenalert} color="primary" variant="contained">
              {" "}
              Bulk Update
            </Button>
            &ensp;
            <Button onClick={handleClickOpenalertundo} color="error" variant="contained">
              {" "}
              Bulk Undo
            </Button>
            <br />
      ...
      <ExportData
        ...
        itemsTwo={projmaster ?? []}
        filename={"Production Temp Summary"}
      ...
      <ExportDataView
        ...
        filename={"Production Temp Summary View"}
      ...
      <Dialog open={isDeleteOpencheckbox} onClose={handleCloseModcheckbox} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        <DialogContent sx={{ width: "350px", textAlign: "center", alignItems: "center" }}>
          ...
          <Typography variant="h5" sx={{ color: "primary", textAlign: "center" }}>
        ...
        <DialogActions>
          ...
          <Button autoFocus variant="contained" color="primary" onClick={(e) => delGroupcheckbox(e)}>
            {" "}
            Yes{" "}
          </Button>
        </DialogActions>
      ...
      <Dialog open={isDeleteOpencheckboxundo} onClose={handleCloseModcheckboxundo} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        <DialogContent sx={{ width: "350px", textAlign: "center", alignItems: "center" }}>
          <ErrorOutlineOutlinedIcon sx={{ fontSize: "80px", color: "orange" }} />
          <Typography variant="h5" sx={{ color: "primary", textAlign: "center" }}>
            Are You Want To Undo?
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseModcheckboxundo} sx={userStyle.btncancel}>
            Cancel
          </Button>
          <Button autoFocus variant="contained" color="error" onClick={(e) => delGroupcheckboxundo(e)}>
            {" "}
            Yes{" "}
          </Button>
        </DialogActions>
      </Dialog>
      <Dialog open={isDeleteOpenalertundo} onClose={handleCloseModalertundo} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
        <DialogContent sx={{ width: "350px", textAlign: "center", alignItems: "center" }}>
          <ErrorOutlineOutlinedIcon sx={{ fontSize: "70px", color: "orange" }} />
          <Typography variant="h6" sx={{ color: "black", textAlign: "center" }}>
            Please Select any Row
        ...
        <DialogActions>
          <Button autoFocus variant="contained" color="error" onClick={handleCloseModalertundo}>
            {" "}
            OK{" "}
          </Button>
        </DialogActions>
      ...
      {/* view model */}
      <Dialog open={openview} onClose={handleClickOpenview} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" fullWidth={true} maxWidth="lg" sx={{ marginTop: "95px" }}>
        <DialogContent>
          <>
            <Typography sx={userStyle.HeaderText}>Production Temp Summary View</Typography>
            <br />
...
export default ProductionTempSummary;