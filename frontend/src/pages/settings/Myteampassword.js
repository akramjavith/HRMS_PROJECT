...
const Myteampassword = () => {
  ...
  const TableHierachylist = async (e) => {
    ...
    try {
      ...
      const vpnsecrets = Res?.data?.vpnsecrets?.length > 0
        ? Res?.data?.vpnsecrets.map(data => {
      ...
      const postfixUsersData = Res?.data?.postfixusers?.users?.length > 0
        ? Res?.data?.postfixusers?.users.map(data => {
      ...
      const rocketchatusers = Res?.data?.rocketchatusers?.users?.length > 0
        ? Res?.data?.rocketchatusers?.users.map(data => {
          const matchedItems = allUsersData?.find(item => item?.companyname === data?.name);
          if (matchedItems) {
            return {
              _id: data?._id,
      ...
      if (Res?.data?.resultedTeam?.length > 0 && Res?.data?.resultAccessFilter?.length < 1 && ["myallhierarchy", "allhierarchy"]?.includes(modeselection.value)) {
        alert("Some employees have not been given access to this page.")
      }
      setEmphierarchy(Res?.data?.resultAccessFilter);
      setAllCredentials([...vpnsecrets, ...postfixUsersData, ...rocketchatusers]);
      setBankdetail(false);
    } catch (err) {
      setBankdetail(false);
      handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert);
  ...
  return (
    <Box>
      <PageHeading
        ...
        modulename="Settings"
        submodulename="Team Credentials"
        mainpagename=""
        subpagename=""
        subsubpagename=""
      />
