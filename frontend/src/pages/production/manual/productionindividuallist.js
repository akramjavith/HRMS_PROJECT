    ...
    const downloadPdf = async (isfilter) => {
        try {
            if (isfilter === "filtered") {
                try {
                    const headers = ["Vendor", "Date Mode", "Date", "Time", "Category", "SubCategory",
                        "Identifier", "Login Id", "Section", "Flag Count", "All Login", "Doc Number",
                        "Doc Link", "Start Mode", "Start Date", "Start Time", "Status Mode",
                        "Total Pages", "Pending Pages", "Start Page", "Remark/Notes"];
                    // Transform data
                    ...
                    pdfMake.createPdf(docDefinition).download("Production_Individual_List.pdf"); // Trigger downloa
                    setIsPdfFilterOpen(false);
                } catch (err) {
                    console.log(err);
                }
                ...
                let response = await axios.post(
                    SERVICE.PRODUCTION_INDIVIDUAL_OVERALL_PDF,
                    {
                        vendor: projvendor,
                        filename: selectedOptionsCategory.map(item => item.value),
                        category: selectedOptionsSubCategory.map(item => item.value),
                ...
                const link = document.createElement("a");
                link.href = URL.createObjectURL(blob);
                link.download = "Production_Month_Upload.pdf";
                link.click();
