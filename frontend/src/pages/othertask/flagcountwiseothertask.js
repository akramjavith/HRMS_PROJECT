...
function FlagCountOthertaskList() {
    ...
    const fetchEmployee = async () => {
        ...
        try {
            let res_employee = await axios.post(SERVICE.FLAG_COUNT_SORT, queryParams, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const fetchEmployeeExport = async () => {
        setSourcecheck(true);
        setPageName(!pageName);
        console.time("fetchEmployeeExport");
        setIsLoading(true)

        try {
            let res_employee = await axios.post(SERVICE.FLAG_COUNT_SORT_EXPORT, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
                page: Number(page),
            ...
            setManageothertasksArray(ans)
            setSourcecheck(false);
            setSearchQuery("");
            console.timeEnd("fetchEmployeeExport");
            setIsLoading(false)
        } catch (err) {
            setIsLoading(false)
            setSourcecheck(false);
