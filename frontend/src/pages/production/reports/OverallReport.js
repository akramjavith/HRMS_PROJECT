...
function OverallReport() {
    ...
    const fetchVendor = async (e) => {
        ...
        try {
            ...
            //    let ALL = vendoryOptfirstthree.filter(d=>d.name).filter((d, index )=> index <=2).map((t) => ({
    ...
    const fetchAllSubCategory = async () => {
        try {
            let res_module = await axios.get(SERVICE.SUBCATEGORYPROD_REPORT_LIMITED, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    //                         SERVICE.PRODUCTION_UPLOAD_FILTER,
    //                         {
    ...
    //                 // Usage
    //                 let uniqueArray = removeDuplicates(allData, ['user', 'unitid', 'dateval', 'category', 'filename',
    //                     'company', 'branch', 'unit', 'team', 'project', 'vendor', 'section', 'flagcount']);
