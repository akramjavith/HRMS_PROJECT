...
function BulkErrorUpload() {
  ...
  const [datedrop, setYeardrop] = useState("dd");
  const [monthdrop, setMonthdrop] = useState("MM");
  const [yeardrop, setDatedrop] = useState("yyyy");
  ...
  const [selectedRowsBulkerror, setSelectedRowsBulkerror] = useState([]);
...
console.log(targetPointsFilename,"targetPointsFilename")
  ...
  const fetchClientUserIDFilter = async (proj) => {
    ...
    try {
      ...
      const filterProjBased = res_freq?.data?.clientuserid.filter((item) =>
        proj.includes(item.projectvendor)
      );
      const loginIdOpt = [
  ...
  const fetchProcessQueueFilter = async (projname) => {
    ...
    try {
      ...
      const processFilter = res_freq?.data?.productionprocessqueue.filter(
        (item) => projname.includes(item.projectvendor)
      );
  ...
  const addSerialNumberviewAll = (datas) => {
    ...
    const itemsWithSerialNumber = datas?.map((item, index) => ({
      ...
      date:
        item.mode == "Bulkupload"
          ? moment(item?.dateformatted).format("DD-MM-YYYY")
  ...
  useEffect(() => {
    fetchClientUserID("SDS_Quickclaim-HFF");
  }, []);
        ...
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("ebulkerrorupload") && (
            <Button
              ...
              onClick={() => {
                rowdatasingleeditView(params.data.id, params.data.olddate);
              }}
            >
              <EditOutlinedIcon
          ...
          {isUserRoleCompare?.includes("dbulkerrorupload") && (
            <Button
              ...
              onClick={(e) => {
                rowDataSingleDeleteView(params.data.id, params.data.olddate);
              }}
            >
  ...
  const [isManageColumnsOpenFilename, setManageColumnsOpenFilename] =
    useState(false);
  const [anchorElFilename, setAnchorElFilename] = useState(null);
  const [selectAllCheckedFilename, setSelectAllCheckedFilename] =
    useState(false);
  const [searchQueryManageFilename, setSearchQueryManageFilename] =
    useState("");
  ...
  const handleSelectionChange = (newSelection) => {
    setSelectedRowsBulkerror(newSelection.selectionModel);
  };
  ...
  //Delete model
  const [isDeleteOpencheckbox, setIsDeleteOpencheckbox] = useState({
    open: false,
    data: [],
  });
  ...
  const handleCloseModcheckbox = () => {
    ...
    setSelectedRowsBulkerror([]);
    setSelectAllChecked(false);
  };
  const [isDeleteOpenalert, setIsDeleteOpenalert] = useState(false);

  // console.log(selectedRowsBulkerror, "selectedRowsBulkerror")
  const handleClickOpenalert = async () => {
    setIsHandleChange(true);
    if (selectedRowsBulkerror.length === 0) {
      setIsDeleteOpenalert(true);
    } else {
      let resPayrun = await axios.post(
        `${SERVICE.PENALTYDAY_CREATED_BULK_DELETE}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },

          dates: selectedRowsBulkerror.map((item) => ({ date: item.olddate })),
        }
      );
        ...
        setShowAlert(
          <>
            ...
            <p style={{ fontSize: "20px", fontWeight: 900 }}>
              {"Penalty Day Already Generated for these dates!"}
            </p>
          </>
        );
        handleClickOpenerr();
      } else if (
        resPayrun.data.count > 0 &&
        resPayrun.data.notIncluded.length > 0
      ) {
        let selectedRowsfiltered = selectedRowsBulkerror.filter((item) =>
          resPayrun.data.notIncluded.some((d) => item.olddate == d)
        );
        setSelectedRowsBulkerror(selectedRowsfiltered);
        console.log(
          selectedRowsfiltered,
          resPayrun.data.notIncluded,
          "selectedRowsfiltered"
        );
        setAlertMsg(
          "Penalty Day  already generated,some dates are linked, do you want to delete others?"
        );
        setIsDeleteOpencheckbox({ open: true, data: selectedRowsfiltered });
      } else {
        setAlertMsg("Are you sure?");
        setIsDeleteOpencheckbox({ open: true, data: selectedRowsBulkerror });
      }
  ...
  const getRowClassName = (params) => {
    if (selectedRowsBulkerror.includes(params.row.id)) {
  ...
  const fetchClientUserID = async (e) => {
    ...
    try {
      ...
      const filterProjBased = res_freq?.data?.clientuserid.filter(
        (item) => item.projectvendor === e
      );
      const loginIdOpt = [
        ...filterProjBased.map((d) => ({
          ...d,
  ...
  const fetchTargetPointsData1 = async () => {
    ...
    try {
      ...
      setTargetPoints(Res?.data?.bulkerroruploadpoints);
      let getFilenames = Res?.data?.bulkerroruploadpoints.filter(
      ...
      setTargetPointsFilenameArray(
        uniqueArray.map((row, index) => ({
          ...
          createddate: row.addedby
            ? moment(row.addedby[0]?.date).format("DD-MM-YYYY hh:mm:ss a")
            : "",
  ...
  const handleSubmit = async (e) => {
    ...
    let Respenalty = await axios.post(
      SERVICE.PENALTYERRORUPLOADS_DUPLICATE_CHECK_WITH_BULKERRORUPLOAD,
      {
        headers: {
          Authorization: `Bearer ${auth.APIToken}`,
        },
    ...
    const isNameMatchPeanltyDuplicate =
      Respenalty.data.penaltyerroruploadpoints > 0;

    // const isNameMatch = targetPoints?.some((item) =>
    const isNameMatch = Res?.data?.bulkerroruploadpoints?.some(
      (item) =>
        item.projectvendor?.toLowerCase() ==
          penaltyErrorUpload.projectvendor?.toLowerCase() &&
        item.process?.toLowerCase() ==
          penaltyErrorUpload.process?.toLowerCase() &&
        item.loginid?.toLowerCase() ==
          penaltyErrorUpload.loginid?.toLowerCase() &&
        item.errorfilename?.toLowerCase() ==
          penaltyErrorUpload.errorfilename?.toLowerCase() &&
        item.documentnumber?.toLowerCase() ==
          penaltyErrorUpload.documentnumber?.toLowerCase() &&
        item.documenttype?.toLowerCase() ==
          penaltyErrorUpload.documenttype?.toLowerCase() &&
        item.fieldname?.toLowerCase() ==
          penaltyErrorUpload.fieldname?.toLowerCase() &&
        item.line?.toLowerCase() == penaltyErrorUpload.line?.toLowerCase() &&
        item.errorvalue?.toLowerCase() ==
          penaltyErrorUpload.errorvalue?.toLowerCase() &&
        item.correctvalue?.toLowerCase() ==
          penaltyErrorUpload.correctvalue?.toLowerCase() &&
    ...
    if (
      ...
      penaltyErrorUpload.projectvendor === "" ||
    ...
    try {
      ...
      if (checkpenaltyday) {
        ...
        let Res = await axios.get(
          `${SERVICE.BULK_ERROR_UPLOADS_SINGLE}/${id}`,
          {
            headers: {
              Authorization: `Bearer ${auth.APIToken}`,
            },
  ...
  const rowDataSingleDeleteView = async (id, olddate) => {
    setPageName(!pageName);
    try {
      let res_data = await axios.post(SERVICE.CHECK_PENALTYDAY_CREATED, {
        headers: {
  ...
  const deleteSingleList = async () => {
    ...
    try {
      const deletePromises = await axios.delete(
        `${SERVICE.BULK_ERROR_UPLOADS_SINGLE}/${deleteSingleid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const [infosingleData, setinfosingleData] = useState({
    experience: "",
    processcode: "",
    code: "",
    points: "",
  ...
  const fetchTargetPointsAllData = async (id) => {
    ...
    try {
      ...
      let getArray = Res?.data?.bulkerroruploadpoints.filter(
        (item) => item._id !== id
      );
    ...
    try {
      ...
      if (checkpenaltyday) {
        ...
        if (prevData) {
          ...
          const updatedDate = moment(prevData.date, "DD-MM-YYYY").format(
            "YYYY-MM-DD"
          );
  ...
  const rowdatasingleeditView = async (id, olddate) => {
    setPageName(!pageName);
    try {
      let res_data = await axios.post(SERVICE.CHECK_PENALTYDAY_CREATED, {
        headers: {
  ...
  const editSubmit = async (e) => {
    ...
    const isNameMatch = getArray?.some(
      (item) =>
        item.projectvendor?.toLowerCase() ==
          editsingleData.projectvendor?.toLowerCase() &&
        ...
        item.errorfilename?.toLowerCase() ==
          editsingleData.errorfilename?.toLowerCase() &&
        item.documentnumber?.toLowerCase() ==
          editsingleData.documentnumber?.toLowerCase() &&
        item.documenttype?.toLowerCase() ==
          editsingleData.documenttype?.toLowerCase() &&
        item.fieldname?.toLowerCase() ==
          editsingleData.fieldname?.toLowerCase() &&
        item.line?.toLowerCase() == editsingleData.line?.toLowerCase() &&
        item.errorvalue?.toLowerCase() ==
          editsingleData.errorvalue?.toLowerCase() &&
        item.correctvalue?.toLowerCase() ==
          editsingleData.correctvalue?.toLowerCase() &&
  ...
  const sendEditRequest = async () => {
    ...
    try {
      let res = await axios.put(
        `${SERVICE.BULK_ERROR_UPLOADS_SINGLE}/${editid}`,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
  ...
  const rowsWithCheckboxes = rowDataTable.map((row) => ({
    ...
    checkbox: selectedRowsBulkerror.includes(row.id),
  }));
  // Show All Columns functionality
  ...
  const rowDatafileNameDelete = async (filename, oldfromdate, oldtodate) => {
    ...
    try {
      let res_data = await axios.post(
        SERVICE.CHECK_PENALTYDAY_CREATED_UPLOAD_WISE,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        ...
        <Grid sx={{ display: "flex" }}>
          {isUserRoleCompare?.includes("dbulkerrorupload") && (
            <Button
              sx={userStyle.buttondelete}
              onClick={(e) => {
                rowDatafileNameDelete(
                  params.data.filename,
                  params.data.oldfromdate,
                  params.data.oldtodate
                );
              }}
            >
              <DeleteOutlineOutlinedIcon
  ...
  const [penaltyEditUpdate, setPenaltyEditUpdate] = useState([]);
  ...
  const readExcel = (file) => {
    ...
    if (penaltyErrorUpload.fromdate === "") {
      ...
      file &&
      targetPointsData.some((item) => item.filename == file?.name)
    ) {
  ...
  const utc_days = Math.floor(serial - 25569); // Excel epoch starts on Jan 1, 1900
...
function formatDate(date, format) {
  console.log(date,format,"nfewfs")
      ...
      promise
        .then(async (d) => {
          ...
          const missingColumns = requiredColumns.filter(
            (column) => !Object.keys(firstRow).includes(column)
          );
          if (missingColumns.length > 0) {
            ...
            // const isNameMatch = targetPoints?.some((item) =>
            const isNameMatchbulkDuplicate =
              Respenaltyupdate.data.penaltyerroruploadpoints;
            console.log(
              isNameMatchbulkDuplicate,
              d,
              "isNameMatchbulkDuplicate"
            );
            ...
            let newdataMatcheddata = isNameMatchbulkDuplicate.map((tp) => {
              ...
              return {
                ...tp,
                link: Match ? Match.HyperLink : "",
                doclink: Match ? Match["HyperLink Doc"] : "",
              };
            });
  ...
  self.findIndex((tp) =>
    ...
    // tp["HyperLink Doc"] === item["HyperLink Doc"]
            ...
            const existingKeys = new Set(
              Respenalty.data.penaltyerroruploadpoints
            );
            // 3. Create key for each item in bulkerror and filter only unique items
            const getBulkKey = (item) =>
              `${item["Project Vendor"]}$${item["Process"]}$${
                item["Login Id"]
              }$${formatToDate(
                item["Audit Date"],
                `${datedrop}${symboldrop}${monthdrop}${symboldrop}${yeardrop}`
              )}$${item["File Name"]}$${item["Document Number"]}$${
                item["Document Type"]
              }$${item["Field Name"]}$${item["Line"]}$${item["Error Value"]}$${
                item["Correct Value"]
              }`;

            const existingKeysbulk = new Set(uniqueArrayfinal.map(getBulkKey));

            const uniqueToUpload = uniqueArrayfinal.filter(
              (item) => !existingKeys.has(getBulkKey(item))
            );
          ...
          handleClickOpenerrpop();
              ...
              setPopupContentMalert(
                uniqueToUpload.length != d.length
                  ...
                  : uniqueToUpload.length != d.length
                  ? `${
                      d.length - uniqueToUpload.length
                    }  Duplicate or data Removed`
              ...
              datesArray.some((d) => getDateFormat(d) != expectedFormat)
            ) {
  ...
  const sendJSON = async () => {
    ...
    if (selectedSheet === "Please Select Sheet") {
      ...
      try {
        ...
        xmlhttp.open("POST", SERVICE.BULK_ERROR_UPLOAD_EXCEL);
...
const ExportsHead = () => {
  ...
  // Update monthdrop and yeardrop based on datedrop
  useEffect(() => {
    if (formatPresets[datedrop]) {
      setMonthdrop(formatPresets[datedrop]?.month);
      setDatedrop(formatPresets[datedrop]?.date);
    }
  }, [datedrop]);
  ...
  const fetchTargetPointsData = async (from, to) => {
    ...
    try {
      ...
      let Resunique = await axios.get(
        SERVICE.BULK_ERROR_UPLOADS_FILENAME_UNIQUE,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      let getFilenames = res_project?.data?.bulkerroruploadpoints.filter(
        (item) => item.filename !== "nonexcel"
      );
      const uniqueArray = Array.from(
        new Set(getFilenames.map((obj) => obj.filename))
  ...
  const fetchFilteredDatas = async () => {
    ...
    try {
      let subprojectscreate = await axios.post(
        SERVICE.BULK_ERROR_UPLOADS_FILTER_LIST,
        {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
      ...
      setSelectedRowsBulkerror([]);
      setFilterList(
        subprojectscreate?.data?.bulkerroruploadpoints.map((item, index) => ({
          ...item,
          serialNumber: index + 1,
          olddate: item.mode == "Bulkupload" ? item.dateformatted : item.date,
          date:
  ...
  return (
    <Box>
          ...
          <>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={6} xs={12} sm={12}>
                <Typography>
                  Date Format<b style={{ color: "red" }}>*</b>
                </Typography>
                ...
                <FormControl fullWidth size="small">
                  ...
                  <Selects
                    ...
                    options={sheets.filter(
                      (d) => !updateSheet.includes(d.index)
                    )}
              ...
              <Grid item md={4} xs={12} sm={6}>
                <Grid container>
                  <Grid item md={5} xs={12} sm={6}>
                    <Typography>
                      {" "}
                      Project Vendors<b style={{ color: "red" }}>*</b>
                    </Typography>
                  </Grid>
                  ...
                  <Grid item md={2} xs={12} sm={12}>
                    <Box>
                      ...
                      >
                        ...
                        <MenuItem value={targetPointsFilename?.length}>
                          All
                        </MenuItem>
                      </Select>
                    </Box>
              ...
              <Box sx={userStyle.container}>
                ...
                {loaderList ? (
                  ...
                  <AggridTable
                    ...
                    items={items}
                    pagenamecheck={"Bulk Error Upload"}
                    setSelectedRows={setSelectedRows}
                    selectedRows={selectedRows}
                    selectedRowsBulkerror={selectedRowsBulkerror}
                    setSelectedRowsBulkerror={setSelectedRowsBulkerror}
            ...
            <Grid item md={4} xs={12} sm={6}>
              <FormControl fullWidth size="small">
                ...
                <Selects
                  ...
                  options={projOpt}
                  value={{
                    label: editsingleData.projectvendor,
                    value: editsingleData.projectvendor,
                  }}
                  onChange={(e) => {
                    setEditsingleData({
                      ...editsingleData,
            ...
            <Grid item md={4} xs={12} sm={6}>
              <FormControl fullWidth size="small">
                ...
                <Selects
                  ...
                  value={{
                    label: editsingleData.process,
                    value: editsingleData.process,
                  }}
                  onChange={(e) => {
                    setEditsingleData({
                      ...editsingleData,
            ...
            <Grid item md={4} xs={12} sm={6}>
              <FormControl fullWidth size="small">
                ...
                <Selects
                  ...
                  value={{
                    label: editsingleData.loginid,
                    value: editsingleData.loginid,
                  }}
                  onChange={(e) => {
                    setEditsingleData({
                      ...editsingleData,
      ...
      <ExportData
        ...
        filteredDataTwo={
          (filteredChangesFilename !== null
            ? filteredRowDataFilename
      ...
      {/* <DeleteConfirmation
            ...
            /> */}
      <Dialog
        open={isDeleteOpencheckbox.open}
        onClose={handleCloseModcheckbox}
        aria-labelledby="alert-dialog-title"
        aria-describedby="alert-dialog-description"
      >
        ...
        <DialogActions>
          ...
          <Button
            ...
            color="error"
            onClick={(e) =>
              deleteSingleBulkdataDelete(isDeleteOpencheckbox.data)
            }
          >
            {" "}
            OK{" "}
