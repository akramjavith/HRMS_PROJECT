...
import { Box, Button, Checkbox, Dialog, DialogActions, DialogContent, FormControl, Grid, IconButton, InputLabel, List, ListItem, ListItemText, MenuItem, OutlinedInput, Paper, Select, Table, TableBody, TableContainer, TableHead, TextareaAutosize, Typography } from "@mui/material";
...
function Noticeperiodactionemployeelist() {
  ...
  const getCode = async (e, name, details) => {
    ...
    try {
      ...
      let searchItem = res1?.data?.mychecklist.find((item) => item.commonid == userDetails?._id && item.module == "Human Resources" && item.submodule == "HR" && item.mainpage == "Employee" && item.subpage == "Employee Action Details" && item.subsubpage == "Action Employee List")
  ...
  const handleCaptureImage = () => {
    if (gridRef.current) {
      html2canvas(gridRef.current).then((canvas) => {
        canvas.toBlob((blob) => {
          saveAs(blob, 'Quick_Action_Employeelist.png');
        });
      });
    }
  };
        ...
        <Box sx={{ padding: "20px 50px" }}>
          <>
            ...
            <Box sx={{ padding: "20px 10px", width: '100%' }}>
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography sx={{ ...userStyle.SubHeaderText, fontWeight: '600' }} onClick={() => { console.log(groupDetails) }} >
                    My Check List
                  </Typography>
                  <div>
