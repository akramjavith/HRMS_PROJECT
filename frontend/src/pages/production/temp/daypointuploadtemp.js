...
function DayPointsUpload() {
    ...
    const addSerialNumberviewAll = () => {
        const itemsWithSerialNumber = productionoriginalviewAll?.map((item, index) => {
            ...
            return {
                ...
                production: Number(item.production).toFixed(3),
                manual: Number(item.manual).toFixed(3),
                avgpoint: Number(item.avgpoint).toFixed(3),
                ...
                allowancepoint: item.type === "Created" ? (item.shiftsts === "Enable" ? Number(item.conshiftpoints).toFixed(3) : 0) : item.allowancepoint,
                nonallowancepoint: diff < 1e-1 ? 0 : Number(diff) > 0 ? Number(diff).toFixed(3) : 0,
            };
        });
            ...
            renderCell: (params) => (
                <Grid sx={{ display: "flex", gap: "20px" }}>
                    {isUserRoleCompare?.includes("edaypointsupload") && (
                        <Link to={`/production/temppointsuploadedit/${params.row.id}/${fileNameID}`}>
    ...
    const rowDataViewIndividualUser = async (date, name, type, users, fromtodate, company, branch, unit, team, empcode, exper, target, processcode) => {
        ...
        try {
            if (type !== "Created") {
                ...
                let datafiltered = [...res.data.productionupload, ...res.data.productionuploadManual].map((item) => {
                    ...
                    let checkfromdate = `${fromdate} ${fromtime.split("Z")[0]}:00`;

                    let [todate, totime] = secondDate.split("T");
                    totime = `${totime.split("Z")[0]}:00` === `00:00:00` ? "12:00:00" : `${totime.split("Z")[0]}:00`;
                    let checktodate = `${todate} ${totime}`;

                    let toDateonly = item.mode == "Manual" ? (item.fromdate + " " + item.time + ":00") : item.formatteddatetime
                    ...
                    let datenew = item.mode == "Manual" ? (item.fromdate + " " + item.time + ":00") : item.formatteddatetime;
        ...
        } catch (err) { setIsSingleLoader(false); handleApiError(err, setShowAlert, handleClickOpenerr); }
    };
    // Alert delete popup
    ...
    const handleCaptureImageView = () => {
        if (gridRefView.current) {
            html2canvas(gridRefView.current).then((canvas) => {
                canvas.toBlob((blob) => {
                    saveAs(blob, "Temp Day_points.png");
                });
            });
        }
    };
    ...
    const fetchTargetCategoryProcessMap = async () => {
        ...
        } catch (err) { setIsSingleLoader(fales); handleApiError(err, setShowAlert, handleClickOpenerr); }
    };
    ...
    return (
        <Box>
            ...
            <Dialog open={openSingleUserView} onClose={handleClickSingleUserOpenView} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" fullWidth={true} maxWidth="1400px">
                <DialogContent>
                    <>
                        <Typography sx={userStyle.HeaderText}>Temp Production Points</Typography>
                        {/* <br /> */}
                        <Grid container style={userStyle.dataTablestyle}>
                            <Grid item md={2} xs={12} sm={12}>
                                <Box>
