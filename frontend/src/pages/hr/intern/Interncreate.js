...
function Interncreate() {
  ...
  const fetchUserDefaultDatas = async () => {
    ...
    try {
      ...
      branchCode = req?.data?.branch?.filter(
      ...
      setEmployee((prev) => ({
        ...
        profileimage:
          migrateData?.faceDescriptor?.length > 0 && migrateData?.uploadedimage
            ? migrateData?.uploadedimage
            : "",
        faceDescriptor:
          migrateData?.faceDescriptor?.length > 0 && migrateData?.uploadedimage
            ? migrateData?.faceDescriptor
            : [],
        prod: true,
        addedby: [
          {
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      let employees_data = await axios.post(SERVICE.USER_CREATE, {
        ...
        assignExpMode: String(
          salaryOption === "Experience Based"
            ? assignExperience.assignExpMode
            : ""
        ),

        assignExpvalue: String(
          salaryOption === "Experience Based"
            ? assignExperience.assignExpvalue
            : ""
        ),
        ...
        endtardate:
          salaryOption === "Experience Based"
            ...
            : "",
        updatedate: String(
          salaryOption === "Experience Based" ? assignExperience.updatedate : ""
        ),
  ...
  const SendDraftRequest = async () => {
    ...
    try {
      let employees_draft = await axios.post(SERVICE.DRAFT_CREATE, {
        ...
        createrocketchat: Boolean(
          createRocketChat?.create ? createRocketChat?.create : false
        ),
        rocketchatemail: String(
          createRocketChat?.create ? createRocketChat?.email : ""
        ),
            ...
            : [
              {
                ...
                gross: String(formValue?.gross || ""),
                medicalallowance: String(formValue?.medicalallowance || ""),
                productionallowance: String(
                  formValue?.productionallowance || ""
                ),
                otherallowance: String(formValue?.otherallowance || ""),
                productionallowancetwo: String(
                  formValue?.productionallowancetwo || ""
  ...
  const nextStepLog = () => {
    ...
    if (
      (employee?.employeecount === "" || employee?.employeecount === "0") &&
      employee?.prod
    ) {
                    ...
                    <Grid item md={4} sm={12} xs={12}>
                      <FormControl size="small" fullWidth>
                        ...
                        <MultiSelect
                          ...
                          disabled={
                            employee?.employeecount === "" ||
                            Number(employee?.employeecount) < 1 ||
                            primaryWorkStation ===
                            "Please Select Primary Work Station"
                          }
                        />
