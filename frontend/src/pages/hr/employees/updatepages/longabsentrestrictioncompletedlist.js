...
import LongabsentrestrictionCompletedManual from './longabsentrestrictioncompletedmanual';
...
function LongabsentrestrictionCompleted() {
    ...
    let exportColumnNames = ['Current Status', 'Company', 'Branch', 'Unit', 'Team', 'Work Mode', 'Leave Applied', 'Emp Code', 'Name'];
    let exportRowValues = ['userstatus', 'company', 'branch', 'unit', 'team', 'workmode', 'leaveapplied', 'empcode', 'companyname'];

    const [employees, setEmployees] = useState([]);
    const [isManualUser, setIsManualUser] = useState([]);
    const [selectedUserType, setSelectedUserType] = useState('Employee');
    const [searchQuery, setSearchQuery] = useState('');
    const { isUserRoleAccess, isUserRoleCompare, isAssignBranch, allUnit, allTeam, allCompany, allBranchs, pageName, setPageName, buttonStyles, allUsersData, allUsersLimit } = useContext(UserRoleAccessContext);
    ...
    const getapi = async () => {
        let userchecks = axios.post(`${SERVICE.CREATE_USERCHECKS}`, {
            ...
            pagename: String('Long Absent Restriction Completed List'),
    ...
    const handleCaptureImage = () => {
        if (gridRefTableImg.current) {
            domtoimage
                ...
                .then((blob) => {
                    saveAs(blob, 'Long Absent Restriction Completed Self List.png');
                })
                .catch((error) => {
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: 'Long Absent Restriction Completed Self List',
        pageStyle: 'print',
    });
    ...
    const columnDataTable = [
        ...
        {
            field: 'leaveapplied',
            headerName: 'Leave Applied',
            flex: 0,
            width: 150,
            hide: !columnVisibility.leaveapplied,
            headerClassName: 'bold-header',
        },
        {
            field: 'empcode',
    ...
    const handleAllowLogin = async (row, action) => {
        try {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        // setIsActive(true)
        if (selectedOptionsCompany?.length === 0 && selectedOptionsBranch?.length === 0 && selectedOptionsUnit?.length === 0 && selectedOptionsTeam?.length === 0) {
    ...
    const updateEmployees = (pastedNames) => {
        ...
        const availableOptions = allUsersData?.filter((comp) => valueCompanyCat?.includes(comp.company) && valueBranchCat?.includes(comp.branch) && valueUnitCat?.includes(comp.unit) && valueTeamCat?.includes(comp.team))?.map((data) => data.companyname.replace(/\s*\.\s*/g, '.').trim());
    ...
    const sendRequest = async () => {
        ...
        setTableLoader(true);
        try {
            let subprojectscreate = await axios.post(SERVICE.GETFILTEREUSERDATALONGABSEND_COMPLETED, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            ...
            let ans = subprojectscreate?.data?.filterallDatauser?.filter(
                (item) => accessbranch.some((branch) => branch.company === item.company && branch.branch === item.branch && branch.unit === item.unit) && (dateArray?.some((data) => item.longleaveabsentaprooveddate?.includes(data)) || item.checklistassigned)
            );

            const resselfdata = ans.filter((data, index) => {
                return !data.attendancemode?.includes('Hrms-Manual');
            });

            const itemsWithSerialNumber = resselfdata?.map((item, index) => ({
                id: item?._id,
            ...
            const resmanualdata = ans.filter((data, index) => {
                return data.attendancemode?.includes('Hrms-Manual');
            });

            const itemsWithSerialNumberManual = resmanualdata?.map((item, index) => ({
                serialNumber: index + 1,
                company: item?.company,
                userstatus: item.userstatus,
                branch: item?.branch,
            ...
            setIsManualUser(itemsWithSerialNumberManual);
    ...
    return (
        <Box>
            ...
            <Headtitle title={'LONG ABSENT RESTRICTION COMPLETED LIST'} />
            {/* ****** Header Content ****** */}
            <PageHeading title="Long Absent Restriction Completed List" modulename="Human Resources" submodulename="HR" mainpagename="Employee" subpagename="Employee Status Details" subsubpagename="Long Absent Restriction Completed List" />
                    ...
                    <Box sx={userStyle.dialogbox}>
                        <>
                            <Grid container spacing={2}>
                                <Grid item xs={12}>
                                    <Typography sx={userStyle.importheadtext}>Long Absent Restriction Completed List</Typography>
                                </Grid>
                    ...
                    {isUserRoleCompare?.includes('llongabsentrestrictioncompletedlist') && (
                        <>
                            <Box sx={userStyle.container}>
                                ...
                                <Grid container spacing={2}>
                                    <Grid item xs={8}>
                                        <Typography sx={userStyle.SubHeaderText}>Long Absent Restriction Completed Self List</Typography>
                                    </Grid>
                                    <Grid item xs={4}>
                                ...
                                <Grid container spacing={2} style={userStyle.dataTablestyle}>
                                    ...
                                    >
                                        <Box>
                                            {isUserRoleCompare?.includes('excellongabsentrestrictioncompletedlist') && (
                                                <>
                                                    <Button
                                                        onClick={(e) => {
                                            ...
                                            {isUserRoleCompare?.includes('csvlongabsentrestrictioncompletedlist') && (
                                                <>
                                                    <Button
                                                        onClick={(e) => {
                                                            setIsFilterOpen(true);
                                            ...
                                            {isUserRoleCompare?.includes('printlongabsentrestrictioncompletedlist') && (
                                                <>
                                            ...
                                            {isUserRoleCompare?.includes('pdflongabsentrestrictioncompletedlist') && (
                                                <>
                                                    <Button
                                                        sx={userStyle.buttongrp}
                                                        onClick={() => {
                                            ...
                                            {isUserRoleCompare?.includes('imagelongabsentrestrictioncompletedlist') && (
                            ...
                            <br />
                            <LongabsentrestrictionCompletedManual isManualUser={isManualUser} handleAllowLogin={handleAllowLogin} />
                        </>
                    )}
                </>
            )}
            ...
            <ExportData
                ...
                itemsTwo={employees ?? []}
                filename={'Long Absent Restriction Completed Self List'}
                                                            ...
                                                            <TableCell>
                                                                <div>
                                                                        ...
                                                                        {row.files && (
                                                                            <Grid container spacing={2}>
                                                                                ...
                                                                                <Grid item lg={1} md={1} sm={1} xs={1}>
                                                                                    <Button sx={buttonStyles.buttondelete} onClick={() => handleFileDeleteEdit(index)}>
                                                                                        <DeleteIcon />
                                                                                    </Button>
                                                                                </Grid>
                                                                            </Grid>
                                                                    ...
                                                                    <Dialog open={isWebcamOpen} onClose={webcamClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                                                                        ...
                                                                        <DialogActions>
                                                                            <Button variant="contained" sx={buttonStyles.buttonsubmit} onClick={webcamDataStore}>
                                                                                OK
                                                                            </Button>
                ...
                >
                    ...
                    >
                        <>
                            ...
                            <p style={{ fontSize: "20px", fontWeight: 900 }}>
                                {`Are You Sure? Do You Want to Undo Login Permission for ${rowDetails?.companyname}?`}
                            </p>
                        </>
                    ...
                    <DialogActions sx={{ justifyContent: "center", pb: 2 }}>
                        <Button
                            ...
                            color="error"
                            onClick={() => {
                                handleAllowLogin(rowDetails, "undo")// Redirect
                            }}
                        >
                            Yes
                        </Button>
...
export default LongabsentrestrictionCompleted;
