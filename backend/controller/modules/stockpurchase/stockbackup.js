...
exports.getAllStockPurchaseLimitedTransfer = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
   ...
   let  stocks = await Stock.find({ status: "Transfer" },
    ...
    let manual = await Manualstock.find({ status: "Transfer" },
      { requestmode: 1, company: 1, status: 1, branch: 1, unit: 1, floor: 1, area: 1, location: 1, productname: 1, tododetails:1, quantity: 1 });
...
//console.log(req.body.assetmat,"assetmat")
 //console.log(stock.length, "stocklimited")
...
exports.getAllStockPurchaseLimitedBalanceCount = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    if (req.body.requestmode === "Asset Material") {
      ...
      filterConditions = {
        requestmode: req.body.requestmode,
        "tododetails.materialnew": req.body.productname,
        handover: { $exists: false },
        status: { $ne: "Transfer" }
      };
    }
    ...
    const [stocklimited, stockhandovereturn, manuallimited, manualhandoverreturn] = await Promise.all([
      ...
      Stock.find(filterreturnhandover, {
        requestmode: 1, company: 1, branch: 1, unit: 1,
        floor: 1, area: 1, location: 1, productname: 1, status: 1,tododetails:1, 
      }
      ),
