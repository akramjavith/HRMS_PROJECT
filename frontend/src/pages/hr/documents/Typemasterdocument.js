...
import {
    Box, Typography, OutlinedInput, Select, MenuItem, Dialog, DialogContent, DialogActions,
    FormControl, Grid, Button, List, ListItem, ListItemText, Popover, Checkbox, TextField, IconButton
} from "@mui/material";
...
function TypeMasterDocument() {
    ...
    const rowData = async (id, name) => {
        ...
        try {
            let res = await axios.get(`${SERVICE.TYPEMASTERDOCUMENT_SINGLE}/${id}`, {
                headers: {
                    Authorization: `Bearer ${auth.APIToken}`,
                },
            });
    ...
    const sendEditRequest = async () => {
        ...
        try {
            let res = await axios.put(
                `${SERVICE.TYPEMASTERDOCUMENT_SINGLE}/${subprojectsid}`,
                {
                    headers: {
                        Authorization: `Bearer ${auth.APIToken}`,
                    },
    ...
    const editSubmit = (e) => {
        e.preventDefault();
        fetchTypemasterAll();
        const isNameMatch = allTypemasteredit.some(
            (item) =>
                item.name.toLowerCase() ===
                typemasterEdit.name.toLowerCase()
        );
    ...
    const fetchTypemaster = async () => {
        ...
        } catch (err) { setTypemastercheck(false); handleApiError(err, setPopupContentMalert, setPopupSeverityMalert, handleClickOpenPopupMalert); }
    };
    ...
    const fetchTypemasterAll = async () => {
        ...
        try {
            ...
            setAllTypemasteredit(
                res_meet?.data?.typemasterdouments.filter(
    ...
    return (
        <Box>
            ...
            <PageHeading
                ...
                submodulename="Type Master Document"
                mainpagename=""
                subpagename=""
                subsubpagename=""
            />
            ...
            <InfoPopup
                ...
                heading="Type Master Document Info"
                addedby={addedby}
                updateby={updateby}
            />
