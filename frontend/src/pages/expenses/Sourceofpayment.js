...
function Sourceofpayments() {
  ...
  const delSource = async (e) => {
    ...
    try {
      if (Sourcesid) {
        await axios.delete(`${SERVICE.SOURCEOFPAYMENT_SINGLE}/${e}`, {
          headers: {
            Authorization: `Bearer ${auth.APIToken}`,
          },
        });
    ...
    if (interactorspurpose?.length === 0) {
      ...
      setShowAlert(
        <>
          ...
          />
          <p style={{ fontSize: "20px", fontWeight: 900 }}>
            {"Source of Payment already exists!"}
          </p>
        </>
      );
      handleClickOpenerr();
  ...
  return (
    <Box>
      ...
      <PageHeading
        ...
        modulename="Expenses"
        submodulename="Source of Payments"
        mainpagename=""
        subpagename=""
        subsubpagename=""
      />
              ...
              {rowDataTable &&
                rowDataTable.map((row, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{row.paymentmode.toString()}</TableCell>
