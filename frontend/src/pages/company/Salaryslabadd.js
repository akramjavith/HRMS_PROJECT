...
function SalarySlabAdd() {
  ...
  const [salarypfmaster, setSalaryPfMaster] = useState({});
  ...
  const ExportsHead = () => {
    ...
    if (salarySlab.company === 'Please Select Company' || salarySlab.branch === 'Please Select Branch' || (salarySlab.processqueue === 'Please Select Process' && selectedprocess === '')) {
      ...
      let alertMsg =
        salarySlab.company === 'Please Select Company' && salarySlab.branch === 'Please Select Branch' && salarySlab.processqueue === 'Please Select Process' && selectedprocess === ''
      ...
      new CsvBuilder(fileDownloadName)
        .setColumns(['ProcessCode', 'SalaryCode', 'Basic', 'Hra',
          'Conveyance', 'MedicalAllowance', 'ProductionAllowance',
          'Productionallowance1', 'OtherAllowance',
          'ShiftAllowance', 'IsESI', 'ESIMaxSalary', 'ESIEmployeePercentage', 'ESIEmployerPercentage',
          'IsPF', 'PfEmployeePercentage', 'PfEmployerPercentage',
          'EPFContribution', 'EPSPension',
          'EDLIInsurance', 'EPFAdminCharges'])
        .exportFile();
    }
  };
  ...
  const readExcel = (file) => {
    if (!(file instanceof Blob)) {
      ...
      promise.then((d) => {
        ...
        let uniqueArrayd = d.filter((item, index, self) => {
          return self.findIndex((i) => i.SalaryCode === item.SalaryCode) === index;
        })

        let uniqueArray = uniqueArrayd.filter(
          (item) =>
            !salarySlabArray.some(
              (tp) =>
                tp.company === salarySlab.company &&
                tp.branch === salarySlab.branch &&
                tp.processqueue == (salarySlab.processqueue == 'Please Select Process' && selectedprocess == '' ? item.ProcessCode : salarySlab.processqueue !== 'Please Select Process' ? salarySlab.processqueue : selectedprocess) &&
                tp.salarycode == item.SalaryCode
              // &&
              // tp.medicalallowance == item.MedicalAllowance &&
              // tp.productionallowance == item.ProductionAllowance &&
              // tp.productionallowancetwo == item.Productionallowance1 &&
              // tp.otherallowance == item.OtherAllowance &&
              // tp.shiftallowance == item.ShiftAllowance &&
              // tp.esideduction == (item.IsESI == 'Y' || item.IsESI == 'y' ? true : false) &&
              // tp.esipercentage == item.ESIEmployerPercentage &&
              // tp.esimaxsalary == item.ESIMaxSalary &&
              // tp.pfdeduction == (item.IsPF == 'Y' || item.IsPF == 'y' ? true : false) &&
              // tp.pfpercentage == item.EPFEmployeePercentage &&
              // tp.epspercentage == item.EPSEmployerPercentage &&
              // tp.esiemployeepercentage == item.ESIEmployeePercentage &&
              // tp.pfemployeepercentage == item.PFEmployeePercentage &&
              // !isNaN(item.MedicalAllowance) &&
              // !isNaN(item.ProductionAllowance) &&
              // !isNaN(item.Productionallowance1) &&
              // !isNaN(item.OtherAllowance) &&
              // !isNaN(item.ShiftAllowance) &&
              // !isNaN(item.ESIPercentage) &&
              // !isNaN(item.ESIMaxSalary) &&
              // !isNaN(item.ESIEmployer) &&
              // !isNaN(item.PfPercentage && isValidBoolean(item.IsPF) && isValidBoolean(item.IsESI))
            )
        );
        console.log(uniqueArray, d, "uniqueArray")
        ...
        const dataArray = uniqueArray.map((item) => ({
          ...
          esiemployeepercentage: item.ESIEmployeePercentage,
          esipercentage: item.ESIEmployerPercentage,
          pfemployeepercentage: item.PfEmployeePercentage,
          pfpercentage: item.PFEmployerPercentage,
          epfcontribution: item.EPFContribution,
          epspension: item.EPSPension,
          edliinsurance: item.EDLIInsurance,
          epfadmincharges: item.EPFAdminCharges,
  ...
  const fetchBranch = async (e) => {
    ...
    try {
      ...
      let data_set = e.value.toUpperCase() === 'ALL' ? res_branch.data.branch : res_branch.data.branch.filter((data) => e.value === data.company);
  ...
  const fetchProcessQueue = async (e) => {
    ...
    try {
      ...
      let data_set = e.value.toUpperCase() === 'ALL' ? res_freq?.data?.processqueuename : res_freq?.data?.processqueuename.filter((data) => e.value === data.branch);
        ...
        (tp) =>
          !salarySlabArray.some(
            (item) =>
              ...
              tp.epfcontribution === item.epfcontribution &&
              tp.epspension == item.epspension &&
              tp.edliinsurance == item.edliinsurance &&
              tp.epfadmincharges == item.epfadmincharges


          )
      );
      console.log(uniqueArray, "uniqueArraysend")
      if (uniqueArray.length === 0) {
        setPopupContentMalert('No Data To Upload');
        setPopupSeverityMalert('info');
        handleClickOpenPopupMalert();
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      let brandCreate = await axios.post(SERVICE.CREATE_SALARYSLAB, {
        ...
        grosstotal: Number(salarySlab.basic) + Number(salarySlab.hra) + Number(salarySlab.conveyance) + Number(salarySlab.medicalallowance) + Number(salarySlab.productionallowance),
        ...
        esiemployeepercentage: Number(salarypfmaster.esiemployeepercentage),
        esipercentage: Number(salarypfmaster.esiemployerpercentage),
        pfemployeepercentage: Number(salarypfmaster.pfemployeepercentage),
        pfpercentage: Number(salarypfmaster.pfemployerpercentage),
        epfcontribution: Number(salarypfmaster.epfcontribution),
        epspension: Number(salarypfmaster.epspension),
        edliinsurance: Number(salarypfmaster.edliinsurance),
        epfadmincharges: Number(salarypfmaster.epfadmincharges),
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const isNameMatch = salarySlabArray?.some((item) => item.company === salarySlab.company &&
      item.branch === salarySlab.branch &&
      item.processqueue.toLowerCase() === salarySlab.processqueue.toLowerCase() && item.salarycode.toLowerCase() === salarySlab.salarycode.toLowerCase());
  ...
  const fetchAllSalaryPfMaster = async () => {
    setPageName(!pageName);
    try {
      ...
      setSalaryPfMaster(res_grp?.data?.salarypf);
  ...
  return (
    <Box>
      ...
      {/* ****** Header Content ****** */}
      <PageHeading title="Manage Salary Slab" modulename="PayRoll" submodulename="Salary Slab" mainpagename="Salary Slab Add" subpagename="" subsubpagename="" />
      <>
            ...
            <>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  <FormControl fullWidth size="small">
                    ...
                    <Selects
                      ...
                      styles={colourStyles}
                      options={
                        salarySlab?.company.toUpperCase() === 'ALL'
                          ? [
                            { label: 'ALL', value: 'ALL' },
                            ...isAssignBranch
                              ?.map((data) => ({
                          ...
                          : [
                            { label: 'ALL', value: 'ALL' },
                            ...isAssignBranch
                              ?.filter((comp) => comp.company === salarySlab?.company)
                              ?.map((data) => ({
                                label: data.branch,
                                value: data.branch,
                              }))
                  ...
                  <Grid container spacing={2}>
                    <Grid item md={4.5} xs={12} sm={6}>
                      <Button
                        ...
                        disabled={
                          // salarySlab.processqueue == "Please Select Process" ||
                          salarySlab.salarycode !== '' ||
                          salarySlab.basic !== '' ||
                          salarySlab.hra !== '' ||
                          salarySlab.conveyance != '' ||
                          salarySlab.medicalallowance !== '' ||
                          salarySlab.productionallowance != '' ||
                          salarySlab.productionallowancetwo !== '' ||
                          salarySlab.otherallowance != '' ||
                          salarySlab.shiftallowance !== '' ||
                          salarySlab.esideduction === true ||
                          salarySlab.pfdeduction === true
                        }
                        component="label"
              ...
              <Grid container spacing={2}>
                ...
                <Grid item md={3} xs={12} sm={12}>
                  ...
                  <FormGroup>
                    <FormControlLabel control={<Checkbox checked={salarySlab.esideduction} />} disabled={fileUploadName != '' && splitArray.length > 0} onChange={(e) => setSalarySlab({ ...salarySlab, esideduction: !salarySlab.esideduction })} label="ESI Deduction" />
                  </FormGroup>
                </Grid>
                <Grid item md={3} xs={12} sm={12}>
                  <Typography> &nbsp; </Typography>
                  <FormGroup>
                    <FormControlLabel control={<Checkbox checked={salarySlab.pfdeduction} />} disabled={fileUploadName != '' && splitArray.length > 0} onChange={(e) => setSalarySlab({ ...salarySlab, pfdeduction: !salarySlab.pfdeduction })} label="PF Deduction" />
                  </FormGroup>
                </Grid>
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          value={salarypfmaster.esiemployeepercentage}
                          readOnly
                        // onChange={(e) => {
                        //   setSalarySlab({
                        //     ...salarySlab,
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          readOnly
                          sx={userStyle.input}
                          value={salarypfmaster.esiemployerpercentage}
                        // onChange={(e) => {
                        //   setSalarySlab({
                        //     ...salarySlab,
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          sx={userStyle.input}
                          placeholder="Please Enter PF Percentage"
                          value={salarySlab.epspercentage}
                          onChange={(e) => {
                            setSalarySlab({
                              ...salarySlab,
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          sx={userStyle.input}
                          readOnly
                          value={salarypfmaster.pfemployeepercentage}
                        // onChange={(e) => {
                        //   setSalarySlab({
                        //     ...salarySlab,
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          value={salarypfmaster.pfemployerpercentage}
                        // onChange={(e) => {
                        //   setSalarySlab({
                        //     ...salarySlab,
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          readOnly
                          sx={userStyle.input}
                          value={salarypfmaster.epfcontribution}
                        // onChange={(e) => {
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          sx={userStyle.input}
                          readOnly
                          value={salarypfmaster.epspension}
                        // onChange={(e) => {
                        //   const value = Number(e.target.value);
                        //   setSalarySlab({
                        //     ...salarySlab,
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          readOnly
                          sx={userStyle.input}
                          value={salarypfmaster.edliinsurance}
                        // onChange={(e) => {
                        //   const value = Number(e.target.value);
                        //   setSalarySlab({
                        //     ...salarySlab,
                    ...
                    <Grid item md={3} xs={12} sm={12}>
                      <FormControl fullWidth size="small">
                        ...
                        <OutlinedInput
                          ...
                          readOnly
                          sx={userStyle.input}
                          value={salarypfmaster.epfadmincharges}
                        // onChange={(e) => {
                        //   const value = Number(e.target.value);
